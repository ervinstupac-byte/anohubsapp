...
const InstallationGuarantee: React.FC = () => {
    ...
    const handleResetAll = () => {
        if (window.confirm('RESET ALL PROTOCOLS? This action is irreversible.')) {
            ...
            localStorage.removeItem(LOCAL_STORAGE_KEY_PROGRESS); // Clear storage
        }
    };
    ...
    const handleSendReport = () => {
        ...
        // 2. Open Email Draft (Optional, for easy sending)
...
const VerificationModal: React.FC<{
    ...
    const handleSubmit = () => {
        if (step.risk === 'Critical' && !value) {
            alert('CRITICAL PROTOCOL: A measured value is MANDATORY.');
            return;
        }
