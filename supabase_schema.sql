...
CREATE TABLE IF NOT EXISTS public.assets (
    ...
    turbine_type TEXT DEFAULT 'PELTON',
...
-- TELEMETRY HISTORY CACHE (precomputed time-series cache for dashboards & AI)
CREATE TABLE IF NOT EXISTS public.telemetry_history_cache (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    asset_id UUID REFERENCES public.assets(id) ON DELETE CASCADE,
    history JSONB NOT NULL, -- array of telemetry snapshots or compressed time-series
    summary JSONB DEFAULT '{}'::jsonb, -- precomputed aggregates (min/max/median/etc)
...
-- HPP_STATUS (realtime channel source for `realtime_hpp_status`)
-- This table is intended to be the low-latency, latest-state table that realtime replication/publishers push into.
CREATE TABLE IF NOT EXISTS public.hpp_status (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    asset_id UUID REFERENCES public.assets(id) ON DELETE CASCADE,
    status TEXT DEFAULT 'OPTIMAL',
    payload JSONB DEFAULT '{}'::jsonb, -- full telemetry snapshot for realtime consumers
    updated_at TIMESTAMPTZ DEFAULT now(),
    created_at TIMESTAMPTZ DEFAULT now()
);
CREATE INDEX IF NOT EXISTS hpp_status_asset_idx ON public.hpp_status (asset_id);

-- Note: Supabase realtime can be wired to broadcast changes on `public.hpp_status` via replication
-- and will surface events under the `realtime_hpp_status` logical channel used in code.
