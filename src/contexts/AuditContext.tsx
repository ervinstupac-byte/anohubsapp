...
export const AuditProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const logAction = async (
    ...
    if (isSupabaseConfigured) {
      ...
      try {
        ...
        console.debug(
          `[AuditContext] Remote log suppressed (${err.message || 'Unknown'}). Using local storage.`
        );
  ...
  return <AuditContext.Provider value={{ logAction, logs }}>{children}</AuditContext.Provider>;
};
