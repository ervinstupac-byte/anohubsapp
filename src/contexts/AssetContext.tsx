...
export const AssetProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
        ...
        try {
          ...
          if (data) {
            ...
            const normalizeTurbine = (t: any) => {
              ...
              if (['pelton', 'francis', 'kaplan', 'crossflow'].includes(lower))
          ...
          console.debug(
            `[AssetContext] Remote fetch suppressed (${supaErr.message || 'Unknown'}). FORCE-LOADING LOCAL DATA.`
          );
      ...
      try {
        ...
        assetChannelRef.current.onmessage = event => {
          if (event.data?.type === 'ASSET_CHANGED') {
            ...
            const numeric =
              typeof maybeId === 'number' ? maybeId : maybeId ? Number(maybeId) : null;
        ...
        if (isGuest) {
          ...
          try {
            ...
            newAsset = {
              ...
              status: data.status,
              turbine_type: (function () {
                const t = data.turbine_type || data.type;
                if (!t) return 'PELTON';
            ...
            console.warn(
              '[AssetContext] Online creation failed. Falling back to OFFLINE STORAGE.',
              err
            );
            ...
            logAction('ASSET_REGISTER', `${newAsset.name} (OFFLINE)`, 'SUCCESS', {
              level: 'WARNING',
            });
          }
        }
  ...
  const value = useMemo(
    () => ({
      ...
      updateAsset: async (id: number | string, patch: any) => {
        console.warn('[AssetContext] updateAsset called (deprecated).', id);
      },
      logActivity: (assetId: number | string, eventType: string, payload?: any) => {
        console.warn('[AssetContext] logActivity called (deprecated).', assetId, eventType);
      },
    }),
    [
      assets,
