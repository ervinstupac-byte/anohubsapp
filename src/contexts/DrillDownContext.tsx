...
export const DrillDownProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const openMetricDrilldown = useCallback(
    (key: string, label: string, value: number, unit: string, threshold: any) => {
    ...
    <DrillDownContext.Provider value={value}>
      ...
      {activeMetric && (
        <TelemetryDrilldownModal
          ...
          status={
            activeMetric.value > activeMetric.threshold.critical
              ? 'critical'
              : activeMetric.value > (activeMetric.threshold.warning || 0)
                ? 'warning'
                : 'nominal'
          }
        />
