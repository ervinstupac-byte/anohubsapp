...
export const InventoryProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const INITIAL_GUEST_INVENTORY: InventoryItem[] = [
    {
      id: '1',
      name: 'Runner Blade (Francis)',
      partNumber: 'FR-BLD-01',
      category: 'Mechanical',
      quantity: 3,
      minStockThreshold: 2,
      unitPrice: 15000,
      turbineTypes: ['francis'],
      maintenanceSpecs: { tools: ['Crane'], instructions: ['Inspect for cavitation'] },
    },
    {
      id: '2',
      name: 'Guide Vane Seal',
      partNumber: 'GV-SEAL-05',
      category: 'Seals',
      quantity: 50,
      minStockThreshold: 20,
      unitPrice: 120,
      turbineTypes: ['francis', 'kaplan'],
      maintenanceSpecs: { tools: ['Seal Puller'], instructions: ['Replace annually'] },
    },
    {
      id: '3',
      name: 'Thrust Bearing Pad',
      partNumber: 'TB-PAD-09',
      category: 'Bearings',
      quantity: 4,
      minStockThreshold: 6,
      unitPrice: 4500,
      turbineTypes: ['pelton', 'francis', 'kaplan'],
      maintenanceSpecs: { tools: ['Micrometer'], instructions: ['Check for Babbitt wear'] },
    },
  ];
  ...
  const fetchInventory = async () => {
    ...
    try {
      ...
      console.debug(
        `[InventoryContext] Fetch suppressed (${err.message || 'Unknown'}). Using Guest Fallback.`
      );
  ...
  const getMissingParts = useCallback(
    (turbineType: string) => {
  ...
  return (
    <InventoryContext.Provider
      value={{ inventory, getMissingParts, getTotalInventoryValue, updateStock, loading }}
    >
      {children}
    </InventoryContext.Provider>
  );
