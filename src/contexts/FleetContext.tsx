...
export const FleetProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const fleetReports = useMemo(() => {
    return assets.map(asset => {
      ...
      const diagnostic = activeDiagnoses.find(
        d => d.message.includes(idAdapter.toStorage(asset.id)) || d.source === 'CORRELATED'
      );
      ...
      let snowCover =
        currentMonth >= 10 || currentMonth <= 3 ? 65 + Math.random() * 20 : 10 + Math.random() * 15;
      ...
      return {
        ...
        status: (score < 50
          ...
          : efficiencyIndex < 92
            ? 'LOW_PERFORMANCE'
            : 'OPTIMAL') as 'OPTIMAL' | 'LOW_PERFORMANCE' | 'CRITICAL_RISK',
      };
    });
