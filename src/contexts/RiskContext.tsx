...
export const RiskProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const updateDisciplineRiskScore = useCallback(
    (points: number, action: 'add' | 'set' | 'reset') => {
  ...
  const calculateAndSetQuestionnaireRisk = useCallback(
    (answers: Answers) => {
      let score = 0;
  ...
  const updateEngineeringHealth = (status: 'PENDING' | 'PASSED' | 'FAILED', deviations: number) => {
    ...
    setRiskState(prev => ({
      ...prev,
      criticalFlags:
        (prev.isAssessmentComplete ? 0 : 0) +
        (riskState.criticalFlags - engineeringHealthState.criticalDeviations) +
        deviations,
    }));
  };

  const updateThresholds = (
    newThresholds: Record<string, { high: string[]; medium: string[] }>
  ) => {
    setRiskState(prev => ({
      ...prev,
