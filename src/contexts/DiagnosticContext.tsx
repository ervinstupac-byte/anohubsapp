...
import { useTelemetry, TelemetryData } from './TelemetryContext.tsx';
...
export const DiagnosticProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    if (lowerText.includes('vibrira') || lowerText.includes('vibracije')) {
      if (lowerText.includes('cijevi') || lowerText.includes('fix')) {
        return {
          ...
          source: 'FIELD_LOG',
          message:
            'Sličan simptom zabilježen prije havarije #2024-KM. Preporučena hitna provjera fiksatora cijevi.',
        };
      }
    }
  ...
  useEffect(() => {
    const analyzeCorrelations = async () => {
      ...
      const assetIds = Object.keys(telemetry)
        .map(id => Number(id))
      ...
      Object.entries(telemetry).forEach(([assetId, t]) => {
        const tv = t as TelemetryData | undefined;
        // guard against malformed / empty telemetry entries
        if (!tv || typeof tv !== 'object') return;

        if (tv.status === 'CRITICAL') {
          results.push({
            symptom: 'TELEMETRY_ALARM',
            source: 'TELEMETRY',
            message: `Critical alarm detected on ${assetId}: ${tv.incidentDetails || ''}`,
          });
        }
  ...
  useEffect(() => {
    ...
    Object.entries(telemetry).forEach(async ([aid, t]) => {
      ...
      const sanitizedFromPQH = sanitizeEtaInputs({ P: tv?.P, Q: tv?.Q, H: tv?.H });

      let etaVal: number | null = null;
      if (sanitizedFromPQH) {
        etaVal = sanitizedFromPQH.eta;
      } else if (tv?.efficiency !== undefined && tv?.efficiency !== null) {
        // telemetry efficiency may be percentage (0-100) or fraction (0-1)
        const raw = Number(tv.efficiency);
      ...
      if (sigma > 0.05) {
        ...
        showToast(
          `Process Instability detected for asset ${numeric} (σ=${sigma.toFixed(3)})`,
          'warning'
        );
        ...
        try {
          ...
          await client
            .from('process_instability_events')
            .insert([
              { asset_id: assetDbId, sigma, samples: buf, timestamp: new Date().toISOString() },
            ]);
        } catch (e) {
  ...
  const recordLessonLearned = async (lesson: {
    symptom: string;
    cause: string;
    resolution: string;
  }) => {
