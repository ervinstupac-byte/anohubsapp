...
export const AIPredictionProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const { telemetry: telemetryContext } = useTelemetry();
  ...
  const [prescriptions, setPrescriptions] = useState<Map<string, PrescriptiveRecommendation>>(
    new Map()
  );
    ...
    const evaluateAI = () => {
      ...
      Object.entries(telemetry).forEach(([assetId, tData]) => {
        ...
        if (synergeticRisk.detected && !synergeticRisks[assetId]?.detected) {
          ...
          showToast(
            `🕷️ SYNERGETIC RISK DETECTED on ${assetId}: ${synergeticRisk.probability}% failure probability!`,
            'warning'
          );
        }
        ...
        const operatingHours =
          maintenanceContext.operatingHours[idAdapter.toStorage(numericAssetId)] || 5000;
        ...
        if (patternMatch) {
          ...
          const existingMatch = incidentPatterns.find(
            p => p.matchedIncidentId === patternMatch.matchedIncidentId
          );
          if (!existingMatch) {
            showToast(
              `👻 INCIDENT GHOST: Pattern match detected - ${patternMatch.pattern}!`,
              'error'
            );
          }
        }
        ...
        rulEstimatesForAsset.forEach(rul => {
          const failureProbability =
            rul.remainingHours < rul.criticalThreshold
          ...
          if (failureProbability > 50) {
            ...
            newPrescriptions.set(
              `${idAdapter.toStorage(numericAssetId)}-${rul.componentType}`,
              prescription
            );
              ...
              if (!existingOrder) {
                ...
                sendMobileNotification(
                  ...
                  (action, details, status) =>
                    logAction(action, String(details || ''), status ? 'SUCCESS' : 'FAILURE')
                );
  ...
  const acknowledgeWorkOrder = (orderId: string) => {
    setAutonomousOrders(prev =>
      prev.map(order => (order.id === orderId ? { ...order, status: 'ACKNOWLEDGED' } : order))
    );
