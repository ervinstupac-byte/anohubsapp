...
interface MaintenanceContextType {
  ...
  createLogEntry: (
    taskId: string,
    entry: Omit<LogEntry, 'id' | 'timestamp' | 'summaryDE' | 'pass'>
  ) => Promise<void>;
...
export const protocols = [
  {
    ...
    threshold: 2000,
    description: 'Check for wear and temperature stability.',
  },
  {
    id: 'P2',
    name: 'Oil Analysis',
    threshold: 4000,
    description: 'Spectral analysis of lubricating oil.',
  },
  {
    id: 'P3',
    name: 'Major Overhaul',
    threshold: 10000,
    description: 'Full disassembly and component validation.',
  },
];
...
export const MaintenanceProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const predictServiceDate = useCallback(
    (assetId: number, threshold: number) => {
  ...
  const validateEntry = useCallback(
    (taskId: string, value: number) => {
      ...
      if (task.componentId === 'TURBINE' && value > task.recommendedSpec) {
        return {
          valid: false,
          message: `Vrijednost izvan tolerancije! (${value} > ${task.recommendedSpec})`,
        };
      }
  ...
  const createLogEntry = useCallback(
    async (taskId: string, entry: Omit<LogEntry, 'id' | 'timestamp' | 'summaryDE' | 'pass'>) => {
      // Validation Check
      const validation = entry.measuredValue
        ? validateEntry(taskId, entry.measuredValue)
        : { valid: true };
  ...
  const getTasksByComponent = useCallback(
    (id: string) => tasks.filter(t => t.componentId === id),
    [tasks]
  );
  ...
  const updateWorkOrder = useCallback(
    async (
      orderId: string,
      updates: Partial<
        Pick<WorkOrder, 'status' | 'assignedTechnician' | 'estimatedHoursToComplete'>
      >
    ): Promise<void> => {
      // Optimistic
      setWorkOrders(prev =>
      ...
      if (updates.estimatedHoursToComplete)
        dbUpdates.estimated_hours = updates.estimatedHoursToComplete;
      dbUpdates.updated_at = new Date().toISOString();

      const { error } = await supabase.from('work_orders').update(dbUpdates).eq('id', orderId);
  ...
  const completeWorkOrder = useCallback(
    async (orderId: string, completionNotes: string): Promise<void> => {
      // Optimistic
      setWorkOrders(prev =>
        prev.map(order =>
          order.id === orderId
