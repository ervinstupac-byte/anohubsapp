...
// Define SpeechRecognition and AudioContext types for TS
...
export const VoiceAssistantProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const playPing = () => {
    ...
    const audioContext = new w.AudioContext();
  ...
  const processCommand = useCallback(
    async (transcript: string) => {
      ...
      if (
        lowerText.includes('ano agent') ||
        lowerText.includes('ano-agent') ||
        lowerText.includes('anoagent')
      ) {
        playPing();
        ...
        const vibrationMatch = command.match(
          /zabilježi vibraciju (?:na|za)? ležaj[au]? ([a-z0-9]+) (?:od|vrijednosti)? ([\d,.]+)/i
        );
        if (vibrationMatch) {
          ...
          try {
            ...
            speak(
              `Primljeno. Vibracija ležaja ${bearing} postavljena na ${valRaw} milimetara po metru. U granicama normale.`
            );
        ...
        if (
          command.includes('alat') &&
          (command.includes('unit') || command.includes('sistem') || command.includes('filter'))
        ) {
          // Find relevant inventory item with maintenance specs
          const filterItem = inventory.find(
            item =>
        ...
        if (
          command.includes('uzroci') ||
          command.includes('zasto') ||
          command.includes('problem')
        ) {
          speak(
            'Na osnovu trenutnog pritiska hlađenja i zabilježenih vibracija, najvjerovatniji uzrok je zaprljanost izmjenjivača toplote ili blago odstupanje u centriranju koje smo ranije evidentirali.'
          );
          setIsProcessing(false);
          return;
        }
