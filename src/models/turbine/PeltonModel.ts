...
export class PeltonModel implements ITurbineModel {
  ...
  detectAnomalies(historicalData: CompleteSensorData[]): Anomaly[] {
    ...
    if (historicalData.length >= 2) {
      ...
      if (deflectorClosed && pressureSpike) {
        anomalies.push({
          ...
          expectedRange: [0, 50],
          recommendation:
            '🔴 WATER HAMMER DETECTED! Rapid deflector closure without proper lead time. Install surge tank or air chamber. Program deflector to close gradually (minimum 2 seconds).',
    ...
    if (peltonData.nozzle_openings.length > 1) {
      const avgOpening =
        peltonData.nozzle_openings.reduce((a, b) => a + b, 0) / peltonData.nozzle_openings.length;
      const maxDeviation = Math.max(
        ...peltonData.nozzle_openings.map(n => Math.abs(n - avgOpening))
      );
      ...
      if (maxDeviation > 5.0) {
        anomalies.push({
          ...
          expectedRange: [0, 3.0],
          recommendation:
            'Nozzle opening imbalance detected. Check servomotor calibration. Possible mechanical binding in one or more nozzles.',
          timestamp: latest.timestamp,
        });
      }
    }
    ...
    if (peltonData.bucket_wear_index > 0.3) {
      anomalies.push({
        ...
        expectedRange: [0, 0.2],
        recommendation:
          'Bucket erosion rate high. Check water sediment content. Consider coating or material upgrade. Inspect jet alignment - misaligned jets accelerate wear.',
        timestamp: latest.timestamp,
      });
    }
    ...
    if (peltonData.nozzle_alignment) {
      ...
      if (maxMisalignment > 0.1) {
        anomalies.push({
          ...
          expectedRange: [0, 0.1],
          recommendation:
            'Critical jet misalignment detected. Realign nozzles to ±0.1mm. Misalignment causes asymmetric bucket wear and efficiency loss of 3-5%.',
          timestamp: latest.timestamp,
        });
      }
    }
    ...
    if (peltonData.jet_velocities && peltonData.jet_velocities.length > 1) {
      const avgVelocity =
        peltonData.jet_velocities.reduce((a, b) => a + b, 0) / peltonData.jet_velocities.length;
      const maxDeviation = Math.max(
        ...peltonData.jet_velocities.map(v => Math.abs(v - avgVelocity))
      );
      ...
      if (maxDeviation > 5.0) {
        anomalies.push({
          ...
          expectedRange: [0, 3.0],
          recommendation:
            'Jet velocity imbalance. Check for nozzle wear or partial blockage. Inspect distributor and penstock for debris.',
          timestamp: latest.timestamp,
        });
      }
    }
    ...
    return [
      {
        ...
        solution:
          'Install surge tank or air chamber upstream of nozzle. Program deflector lead time (deflector must close before nozzle). Minimum closure time: 2 seconds.',
      ...
      },
      {
        ...
        solution:
          'Realign all nozzles to ±0.1mm tolerance. Consider hardfacing or stainless steel overlay on buckets. Install sediment trap upstream.',
        historicalIncidents: [],
      },
      {
        ...
        solution:
          'Inspect and clean nozzle interior. Check distributor and penstock screens. Consider automated strainer system.',
        historicalIncidents: [],
      },
    ];
  }
