...
export class FrancisModel implements ITurbineModel {
  ...
  detectAnomalies(historicalData: CompleteSensorData[]): Anomaly[] {
    ...
    const vibration =
      (latest.specialized as FrancisSensorData)?.stay_ring_vibration || latest.common.vibration;
    ...
    if (
      francisData.draft_tube_pressure < -0.8 &&
      francisData.guide_vane_opening < 40 &&
      vibration > 3.0
    ) {
      anomalies.push({
        ...
        recommendation:
          'Cavitation surge detected at part load. Inject air into draft tube or increase load above 40%.',
        timestamp: latest.timestamp,
      });
    }
    ...
    if (temp > FRANCIS_CONSTANTS.MAX_BEARING_TEMP) {
      anomalies.push({
        ...
        recommendation:
          'Bearing temperature high. Check oil level, oil cooler flow, and alignment.',
        timestamp: latest.timestamp,
      });
    }
    ...
    return [
      {
        ...
        solution:
          'Check head vs design head. If head is high, restrict GV opening. Weld repair required if potting > 2mm.',
      ...
      },
      {
        ...
        solution:
          'Identify blocked GV. Inspect for driftwood or debris. Replace shear pin only after clearing obstruction.',
        historicalIncidents: [],
      },
    ];
  }
