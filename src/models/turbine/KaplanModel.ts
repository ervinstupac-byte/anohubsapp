...
export class KaplanModel implements ITurbineModel {
  ...
  getSpecificParameters(): string[] {
    return ['blade_angle', 'wicket_gate_position', 'servo_oil_pressure', 'hub_position'];
  }
  ...
  detectAnomalies(historicalData: CompleteSensorData[]): Anomaly[] {
    ...
    if (Math.abs(kaplanData.blade_angle - expectedAngle) > 5) {
      anomalies.push({
        ...
        recommendation:
          'Unit is operating OFF-CAM. Adjust governor cam curve to prevent cavitation and vibration.',
