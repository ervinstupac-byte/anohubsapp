...
export const VERTICAL_THRUST_BEARING_PASSPORT: ComponentPassport = {
  ...
  mechanicalSpecs: {
    clearances: {
      axial: 0.25, // mm - CRITICAL! Axijalni zazor
  ...
  spareParts: {
    criticalSpares: ['MB-850-PAD-SET (8 thrust pads)', 'BABBITT-LAYER-KIT', 'LEVELING-PLATE-SET'],
    recommendedSpares: ['OIL-SEAL-850', 'TEMP-SENSOR-PT100-THRUST'],
...
export class ThrustBearingMonitor {
  ...
  checkThrustTemperature(thrustTemp: number): ThrustBearingStatus {
    ...
    if (thrustTemp >= CRITICAL) {
      return {
        ...
        action:
          'EMERGENCY SHUTDOWN NOW! Stop turbine immediately to prevent catastrophic bearing failure!',
        withinLimits: false,
      };
    }
    ...
    if (thrustTemp >= ALARM) {
      return {
        ...
        action:
          'REDUCE LOAD to 60% within 5 minutes. Check thrust pads immediately at next shutdown!',
        withinLimits: false,
      };
    }
  ...
  checkAxialDisplacement(
    ...
    const tolerance = 0.1; // mm - acceptable deviation
