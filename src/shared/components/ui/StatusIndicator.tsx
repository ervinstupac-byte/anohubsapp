...
export const StatusIndicator: React.FC<StatusIndicatorProps> = ({
  ...
  if (variant === 'led') {
    return (
      <div className={`flex items-center gap-1.5 ${className}`}>
        ...
        {label && (
          <span
            className={`${LABEL_SIZE_CLASSES[size]} font-mono text-slate-500 uppercase tracking-widest`}
          >
            {label}
          </span>
        )}
  ...
  return (
    ...
    >
      ...
      {label && (
        <span
          className={`${LABEL_SIZE_CLASSES[size]} ${colors.text} font-black uppercase tracking-wider`}
        >
          {label}
        </span>
      )}
