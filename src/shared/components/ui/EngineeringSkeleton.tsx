...
export const EngineeringSkeleton: React.FC<EngineeringSkeletonProps> = ({
  ...
  if (variant === 'stat') {
    return (
      <div
        className={`${GLASS.base} ${RADIUS.cardLg} ${spacing.cardPadding} border-l-4 border-slate-700 ${className}`}
      >
  ...
  if (variant === 'instrument') {
    return (
      <div
        className={`relative ${RADIUS.card} border-2 border-slate-700/50 bg-slate-900/50 ${className}`}
      >
        {!isCompact && (
          <>
