...
export const ModernInput: React.FC<ModernInputProps> = ({
  ...
  return (
    <div className={`${fullWidth ? 'w-full' : 'w-auto'} ${className} mb-4`}>
      ...
      {helperText && !error && (
        <p className="mt-1 ml-1 text-xs text-slate-500 font-mono tracking-wide">{helperText}</p>
      )}

      {error && (
        <p className="mt-1 ml-1 text-xs text-red-400 font-medium animate-fade-in">{error}</p>
      )}
    </div>
  );
};
