...
export const LoadingShimmer: React.FC<LoadingShimmerProps> = ({ className = '' }) => {
  return (
    <div
      className={`w-full h-full flex flex-col items-center justify-center bg-slate-950 p-12 ${className}`}
    >
      <div className="w-full max-w-4xl space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[1, 2, 3, 4, 5, 6].map(i => (
            <div
              key={i}
              className="h-48 bg-slate-900/40 border border-white/5 rounded-none p-6 space-y-4 shimmer-overlay"
            >
