...
export const ModuleFallback: React.FC<ModuleFallbackProps> = ({
  ...
  const getIcon = () => {
    switch (icon) {
      case 'Box':
        return <Box className="w-8 h-8 text-cyan-500/50" />;
      case 'File':
        return <FileText className="w-8 h-8 text-cyan-500/50" />;
      case 'Ban':
        return <Ban className="w-8 h-8 text-red-500/50" />;
      default:
        return <AlertTriangle className="w-8 h-8 text-amber-500/50" />;
    }
  };
  ...
  return (
    <GlassCard className="h-full min-h-[300px] flex flex-col items-center justify-center p-8 text-center border-dashed border-slate-700 bg-slate-900/20">
      <div className="p-4 bg-slate-900/50 rounded-none mb-4 border border-white/5">{getIcon()}</div>
