...
test('stress integrity: 250 rapid state changes then snapback to #250', async () => {
  ...
  const stateUpdates = journal.filter(
    e => e.type === 'state_update' || e.type === 'telemetry_update'
  );
  console.log(
    `Journal recorded ${journal.length} events, of which ${stateUpdates.length} are state/telemetry updates.`
  );
  ...
  chronological.forEach(ev => {
    try {
      ...
      if (p && p.snapshot && p.snapshot.simulated && typeof p.snapshot.simulated.seq === 'number')
        seqs.push(p.snapshot.simulated.seq);
      if (p && p.state && typeof p.state.seq === 'number') seqs.push(p.state.seq);
      if (p && p.payload && p.payload.simulated && typeof p.payload.simulated.seq === 'number')
        seqs.push(p.payload.simulated.seq);
    } catch (e) {}
  });
