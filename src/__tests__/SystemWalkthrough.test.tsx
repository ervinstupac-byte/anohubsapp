...
vi.mock('framer-motion', () => ({
  motion: {
    div: ({ children, dragMomentum, dragConstraints, dragElastic, whileInView, ...props }: any) => (
...
describe('System Walkthrough: Guest Journey & Modals', () => {
  ...
  test('Scenario 2: Command Palette - Maintenance Protocol', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.queryByPlaceholderText(/Type a command or search assets/i)
  ...
  test('Scenario 6: Drill-Down & Asset Passport Interaction', async () => {
    ...
    await waitFor(
      ...
      { timeout: 2000 }
    ); // Increase timeout for the 800ms delay
  ...
  test('Scenario 12: Stress Test Simulation', async () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith(
      expect.stringContaining('STRESS TEST: Triggering Cavitation Event')
    );
    ...
    expect(consoleSpy).toHaveBeenCalledWith(
      expect.stringContaining('STRESS TEST: Resetting to Normal Operation')
    );
