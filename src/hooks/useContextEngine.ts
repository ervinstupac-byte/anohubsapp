...
export const useContextEngine = () => {
  ...
  const techState = useMemo(
    () =>
      ({
        ...
        physics: {
          ...
          waterHammerPressureBar:
            ...
            DEFAULT_TECHNICAL_STATE.physics.waterHammerPressureBar,
          specificWaterConsumption:
            physics.specificWaterConsumption ??
            DEFAULT_TECHNICAL_STATE.physics.specificWaterConsumption,
      ...
      }) as unknown as TechnicalProjectState,
    [
      identity,
      mechanical,
      hydraulic,
  ...
  const handleReinforcePattern = useCallback(
    async (patternId: string, feedback: 'CONFIRMED' | 'REJECTED' | 'OVERRIDE') => {
      setPatternWeights(prev => {
  ...
  const diagnosticsAndMetrics = useMemo(() => {
    if (!activeComponentId)
      return { diagnostics: [], metrics: { structuralSafetyMargin: 100 }, recoveryPaths: [] };
    ...
    const sentinelResults = SentinelKernel.evaluateMatrix(metricHistory, patterns, {
  ...
  useEffect(() => {
    ...
    const relevantNodes = KNOWLEDGE_BASE.filter(node => {
      return node.triggers.some(trigger =>
        evaluateTrigger(trigger, engineCurrentRoute, currentSensors)
      );
    });
  ...
  useEffect(() => {
    ...
    const relevantTaskIds = new Set(
      tasks
        .filter(
          t =>
            t.componentId.toLowerCase().includes(keyword) || t.title.toLowerCase().includes(keyword)
        )
        .map(t => t.id)
    );
  ...
  useEffect(() => {
    ...
    const relevantWOs = workOrders.filter(
      wo =>
  ...
  useEffect(() => {
    ...
    diagnostics.forEach(diag => {
      if (diag.type === 'critical') {
        ...
        const existsInState = activeWorkOrders.some(
          wo =>
            wo.status !== 'COMPLETED' &&
            wo.status !== 'CANCELLED' &&
        ...
        if (!existsInState && !isPending) {
          ...
          const precedent =
            diag.precedent ||
            'Similar instability observed 14 months ago during rapid load rejection.';
  ...
  const liveMetrics = useMemo(() => {
    ...
    const getDynamicStatus = (
      metricLabel: string,
      baseStatus: 'safe' | 'warning' | 'critical' = 'safe'
    ) => {
    ...
    if (activeComponentId.includes('penstock')) {
      ...
      metrics.push({
        label: 'Vibration',
        value: hist['vibration']?.[hist['vibration'].length - 1] || 0,
        unit: 'mm/s',
        status: vibStatus,
        history: hist['vibration'],
        color: getTrendColor(vibStatus),
        source: { id: 'VIB-901-A', cal: '12 Oct 2024' },
      });
      metrics.push({
        label: 'DT Pressure',
        value: hist['draftTubePressure']?.[hist['draftTubePressure'].length - 1] || 0,
        unit: 'bar',
        status: preStatus,
        history: hist['draftTubePressure'],
        color: getTrendColor(preStatus),
        source: { id: 'PRE-202-B', cal: '05 Nov 2024' },
      });
      ...
      metrics.push({
        label: 'Bearing Temp',
        value: hist['bearingTemp']?.[hist['bearingTemp'].length - 1] || 0,
        unit: '°C',
        status: tempStatus,
        history: hist['bearingTemp'],
        color: getTrendColor(tempStatus),
        source: { id: 'TMP-404-X', cal: '01 Jan 2024' },
      });
      metrics.push({
        label: 'Oil Pressure',
        value: hist['oilPressure']?.[hist['oilPressure'].length - 1] || 0,
        unit: 'bar',
        status: 'safe',
        history: hist['oilPressure'],
        color: '#22d3ee',
        source: { id: 'PRE-101-Z', cal: '15 Dec 2023' },
      });
    } else {
      metrics.push({
        label: 'System Load',
        ...
        history: hist['load'],
        color: '#22d3ee',
        source: { id: 'SCADA-MAIN', cal: 'Realtime' },
      });
    }
  ...
  return {
    ...
    estimatedFailureDate: (() => {
      ...
      const extension =
        (techState.structural.extendedLifeYears || 0) -
        parseFloat(techState.structural.longevityLeak || '0');
