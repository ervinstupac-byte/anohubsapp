...
export const useEngineeringMath = () => {
  ...
  return useMemo(() => {
    ...
    const isStructuralLoosenessConfirmed =
      eccentricity.greaterThan(eccentricityLimit) &&
    ...
    if (burstSafetyFactor.lessThan(1.5)) {
      iecRecommendation =
        '🚨 IEC 60041 CRITICAL: Safety Factor below 1.5. Immediate pressure reduction or structural inspection required!';
    } else if (burstSafetyFactor.lessThan(2.0)) {
      iecRecommendation =
        '⚠️ IEC 60041 WARNING: SF < 2.0. Monitor transient pressure peaks closely.';
    }
    ...
    const shaftLengthProxy = new Decimal(identity?.machineConfig?.runnerDiameterMM || 1500).mul(
      1.5
    ); // Estimate
    ...
    return {
      thrust: {
        totalKN: deltaP_Bar
          .mul(1.5)
          .mul(new Decimal(0.4).plus(new Decimal(hydraulic.flow > 40 ? 0.1 : 0)))
          .toNumber(),
