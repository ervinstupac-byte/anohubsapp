...
export const mapAssetToEnhancedAsset = (asset: Asset): EnhancedAsset => {
  ...
  const config: TurbineConfiguration = {
    ...
    runner_diameter: identity?.machineConfig?.runnerDiameterMM
      ? identity.machineConfig.runnerDiameterMM / 1000
      : 0,
    commissioning_date: identity?.commissioningYear
      ? new Date(identity.commissioningYear, 0, 1).toISOString()
