...
export class TruthDeltaEngine extends BaseGuardian {
   ...
   */
  static calculateDeltaMap(diagnostics: DiagnosticInsight[], humanLogs: any[]): TruthDeltaMap {
    // Component mapping
  ...
  private static calculateComponentDelta(
    ...
    const sensorDiag = diagnostics.find(
      d =>
    ...
    const sensorStatus = sensorDiag
      ? sensorDiag.type === 'critical'
        ? 'critical'
        : sensorDiag.type === 'warning'
          ? 'warning'
          : 'healthy'
      : 'unknown';
