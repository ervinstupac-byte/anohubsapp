...
export const generateDiagnosticDossier = (
  ...
  doc.text(
    `DIGITAL LEDGER: #DL-${Math.floor(Math.random() * 999999)} (Authenticity Verified)`,
    pageWidth - 20,
    35,
    { align: 'right' }
  );
  ...
  if (snapshotImage) {
    try {
      ...
      if (snapshotImage.startsWith('data:image')) {
        ...
        const isOrbit =
          safeInsight.name?.toLowerCase().includes('orbit') ||
          safeInsight.name?.toLowerCase().includes('stability');
  ...
  const narrative =
    safeInsight.physicsNarrative ||
    'Energy signature analysis indicates deviation from baseline efficiency.';
  ...
  doc.text('ANO-HUB ENGINEERING EXCELLENCE', pageWidth - 20, doc.internal.pageSize.height - 10, {
    align: 'right',
  });
...
export const generateAssetPassport = (
  asset: Asset,
  ...
  doc.setFontSize(10);
  doc.text(
    `${t('pdf.passport.generated')}: ${new Date().toLocaleDateString()}`,
    pageWidth - 60,
    20
  );
  doc.text(
    `${t('pdf.passport.id')}: ${String(asset.id).substring(0, 8).toUpperCase()}`,
    pageWidth - 60,
    30
  );
  ...
  doc.setFontSize(10);
  doc.setTextColor(150, 150, 150);
  doc.text(`${t('pdf.passport.page')} 1 of 2`, pageWidth / 2, doc.internal.pageSize.height - 10, {
    align: 'center',
  });
  ...
  if (logs && logs.length > 0) {
    ...
    autoTable(doc, {
      ...
      head: [
        [
          t('pdf.passport.date'),
          t('pdf.passport.category'),
          t('pdf.passport.action'),
          t('pdf.passport.user'),
        ],
      ],
      body: tableData,
  ...
  doc.setFontSize(10);
  doc.setTextColor(150, 150, 150);
  doc.text(`${t('pdf.passport.page')} 2 of 2`, pageWidth / 2, doc.internal.pageSize.height - 10, {
    align: 'center',
  });
...
export const createRiskReportBlob = (
  ...
  const splitText = doc.splitTextToSize(
    riskData.consultation || t('questionnaire.noDataAvailable'),
    pageWidth - 40
  );
...
export const createMasterDossierBlob = (
  ...
  doc.text(
    `${t('contractManagement.pdf.date')} ${new Date().toLocaleDateString()}`,
    pageWidth / 2,
    260,
    { align: 'center' }
  );
  ...
  if (designData) {
    ...
    doc.text(
      `${t('hppStudio.steps.selection')}: ${designData.recommended_turbine || 'N/A'}`,
      20,
      50
    );
    ...
    if (designData.calculations) {
      doc.text(
        `${t('hppStudio.labels.calculatedCapacity')}: ${designData.calculations.powerMW} MW`,
        20,
        70
      );
      doc.text(
        `${t('modules.fleetOutput')}: ${designData.calculations.energyGWh || designData.calculations.annualGWh} GWh`,
        20,
        80
      );
    }
...
export const generateAuditReport = (
  ...
  doc.setTextColor(255, 255, 255);
  doc.setFontSize(10);
  doc.text(`${t('common.generatedBy') || 'Engineer'}: ${engineerName}`, pageWidth - 20, 15, {
    align: 'right',
  });
  ...
  if (metrics && metrics.length > 0) {
    ...
    autoTable(doc, {
      startY: y,
      head: [
        [t('common.metric', 'Metric'), t('common.value', 'Value'), t('common.status', 'Status')],
      ],
      body: tableData,
      headStyles: { fillColor: [15, 23, 42], textColor: [34, 211, 238] },
      alternateRowStyles: { fillColor: [241, 245, 249] },
  ...
  if (diagnostics && diagnostics.length > 0) {
    ...
    diagnostics.forEach(diag => {
      ...
      doc.setTextColor(0, 0, 0);
      doc.text(
        `${diag.type === 'critical' ? '⚠️ CRITICAL' : 'ℹ️ INFO'}: ${diag.messageKey || diag.message}`,
        25,
        y + 9
      );
      y += 18;
  ...
  if (returnBlob) return doc.output('blob');
  doc.save(
    `Audit_${contextTitle.replace(/\s+/g, '_')}_${new Date().toISOString().slice(0, 10)}.pdf`
  );
};
