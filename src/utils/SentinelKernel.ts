...
export class SentinelKernel {
   ...
   */
  static generatePhysicsNarrativeFromPattern(
    pattern: HeuristicPattern,
    vectors: string[],
    probability: number
  ): string {
    if (!pattern.physicsNarrative)
    ...
    patterns.forEach(pattern => {
      ...
      pattern.conditions.forEach(cond => {
        ...
        if (isMatch) {
          ...
          proofParts.push(
            `${cond.variableId}[${currentValue.toFixed(1)}] * w(${cond.weight.toFixed(2)})`
          );
        }
      ...
      if (totalWeight > 0) {
        ...
        if (synchronizedTrends >= 2) {
          ...
          contributingVectors.push(
            `Exponential Coupling: ${synchronizedTrends} vectors synchronized.`
          );
        }
        ...
        if (pattern.id === 'cavitation-complex' && context?.timeAtState) {
          if (context.timeAtState > 15) {
            ...
            contributingVectors.push(
              `Prolonged Exposure: ${context.timeAtState.toFixed(0)}m in critical zone.`
            );
          }
        }
        ...
        if (probability > 0.6) {
          ...
          insights.push({
            ...
            physicsNarrative: this.generatePhysicsNarrativeFromPattern(
              pattern,
              contributingVectors,
              probability
            ), // The Narrative
   ...
   */
  static exportWeightMap(plantId: string, weights: Record<string, number>): any {
    // Returns WeightMap type
    return {
      plantId,
      timestamp: Date.now(),
...
export const SENTINEL_PATTERNS: HeuristicPattern[] = [
  {
    ...
    physicsNarrative: (prob: number, vectors: string[]) =>
      `Cavitation intensity at ${(prob * 100).toFixed(0)}%. ${vectors[0]}`,
  },
  {
    ...
    actions: [
      {
        type: 'FOCUS_3D',
        label: 'Zoom to Guide Bearing',
        targetId: 'Mesh_Bearing_01',
        icon: 'Disc',
      },
