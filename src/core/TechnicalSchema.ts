...
export interface MechanicalStream {
  ...
  wicketGatePosition?: number; // Unit: Percentage [%]
  wicketGateSetpoint?: number; // Unit: Percentage [%]
  cylinderPressure?: number; // Unit: Bar [bar]
  actuatorPosition?: number; // Unit: Percentage [%]
...
export interface TechnicalProjectState {
  ...
  site: {
    ...
    reservoirLevel?: number; // Unit: Meters [m]
    tailwaterLevel?: number; // Unit: Meters [m]
  };
...
export interface EngineeringConstants {
  ...
  electrical: {
    nominalGridFrequency: number;
    gridFrequencyTolerance: number;
    criticalFrequencyThreshold: number;
  };
  hydraulic: {
    cavitationFlowThreshold: number;
    cavitationHeadThreshold: number;
    targetRunnerClearance: number;
  };
...
export type ProjectAction =
  ...
  | {
      ...
      payload: { assetId: number; componentId: string; healthData: ComponentHealthData };
    }
  | {
      type: 'UPDATE_SPECIALIZED_MODULE';
      payload: { moduleId: string; status: SpecializedModuleStatus };
    }
  ...
  | {
      type: 'ADD_INSPECTION_IMAGE';
      payload: { assetId: number; component: string; imageData: InspectionImage };
    }
  | { type: 'RESET_TO_DEMO' };
...
export const DEFAULT_TECHNICAL_STATE: TechnicalProjectState = {
  identity: {
    ...
    environmentalBaseline: {
      noiseLevel: {
        operatingDB: 75,
        locations: { powerhouse: 70, turbinePit: 85, controlRoom: 60 },
