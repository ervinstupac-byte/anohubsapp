...
export const PhysicsEngine = {
  ...
  recalculatePhysics: (state: TechnicalProjectState): PhysicsResult => {
    ...
    const hoopSF = new Decimal(penstock.materialYieldStrength).div(
      hoopStress.gt(0) ? hoopStress : 1
    );

    const powerKW = powerMW.mul(1000);
    const specificWaterConsumption = powerKW.isZero()
      ? new Decimal(0)
      : flow.mul(3600).div(powerKW);

    const designPower = new Decimal(state.site.designPerformanceMW || 5.0);
    const performanceGap = designPower.isZero()
      ? new Decimal(100)
      : powerMW.div(designPower).mul(100);
    ...
    if (profile?.math.formulas.calculateVolumetricLoss) {
      volumetricLoss = new Decimal(
        profile.math.formulas.calculateVolumetricLoss(state, profile.math.constants)
      );
    }

    if (
      specificWaterConsumption.gt(
        designSWC.mul(SYSTEM_CONSTANTS.THRESHOLDS.LEAKAGE.CRITICAL_MULTIPLIER)
      )
    )
      leakageStatus = 'CRITICAL';
    else if (
      specificWaterConsumption.gt(
        designSWC.mul(SYSTEM_CONSTANTS.THRESHOLDS.LEAKAGE.DEGRADING_MULTIPLIER)
      )
    )
      leakageStatus = 'DEGRADING';

    if (
      hoopSF.lt(SYSTEM_CONSTANTS.THRESHOLDS.SAFETY_FACTOR.CRITICAL) ||
      eccentricity.gt(SYSTEM_CONSTANTS.THRESHOLDS.ECCENTRICITY.CRITICAL) ||
      axialThrustKN.gt(SYSTEM_CONSTANTS.THRESHOLDS.AXIAL_THRUST.CRITICAL_KN) ||
      leakageStatus === 'CRITICAL'
    )
      status = 'CRITICAL';
    else if (
      hoopSF.lt(SYSTEM_CONSTANTS.THRESHOLDS.SAFETY_FACTOR.WARNING) ||
      eccentricity.gt(SYSTEM_CONSTANTS.THRESHOLDS.ECCENTRICITY.WARNING) ||
      axialThrustKN.gt(SYSTEM_CONSTANTS.THRESHOLDS.AXIAL_THRUST.WARNING_KN) ||
      leakageStatus === 'DEGRADING'
    )
      status = 'WARNING';
  ...
  simulateTimeStep: (
    ...
    const pidOutput =
      governor.kp.toNumber() * error + integral + governor.kd.toNumber() * derivative;
    ...
    const hLoss =
      frictionFactor * (PENSTOCK_L / PENSTOCK_D) * (Math.pow(velocity, 2) / (2 * GRAVITY));
  ...
  recalculateProjectPhysics: (state: TechnicalProjectState): TechnicalProjectState => {
    ...
    const typEfficiency = PhysicsLogic.calculateTypicalEfficiency(
      turbineType,
      state.hydraulic.head
    );
    const typThresholds = PhysicsLogic.getTurbineThresholds(turbineType);
    const typSpecs = PhysicsLogic.generateTurbineSpecs(
      turbineType,
      state.hydraulic.head,
      state.hydraulic.flow
    );
    ...
    let leakageStatus: LeakageStatus = 'NOMINAL';
    if (
      result.specificWaterConsumption.gt(
        designSWC.mul(SYSTEM_CONSTANTS.THRESHOLDS.LEAKAGE.CRITICAL_MULTIPLIER)
      )
    )
      leakageStatus = 'CRITICAL';
    ...
    const finalState = {
      ...
      // financials are calculated by a separate service to keep PhysicsEngine pure
      financials: (newState.financials || {}) as any,
  ...
  calculateStructuralLife: (state: TechnicalProjectState, physics: PhysicsResult) => {
    ...
    if (state.demoMode.active) {
      ...
      if (state.demoMode.scenario === 'WATER_HAMMER') {
        ...
        wearIndex +=
          ASSET_THRESHOLDS.structural.fatigue.water_hammer_wear_multiplier * gridStressFactor;
      } else if (state.demoMode.scenario === 'GRID_LOSS') {
        cycleIncrement = ASSET_THRESHOLDS.structural.fatigue.grid_loss_cycle_increment;
        wearIndex +=
          ASSET_THRESHOLDS.structural.fatigue.grid_loss_wear_multiplier * gridStressFactor;
      }
    ...
    const drf =
      intensity > drfRule
        ? Math.pow(intensity / drfRule, 3) * (drfRule * 100)
        : intensity * (drfRule * 100);
    ...
    const actualYearlyWear =
      idealYearlyWear *
      (1 + (normalizedDRF / 100) * (ASSET_THRESHOLDS.structural.life.max_wear_multiplier - 1)); // Up to 5x faster wear
    ...
    const waterPenalty =
      (fluid.waterContentPPM || 0) > SYSTEM_CONSTANTS.DURABILITY.LIMITS.WATER_CONTENT_PPM
        ? SYSTEM_CONSTANTS.DURABILITY.PENALTIES.WATER_CONTENT_YEARS
        : 0; // 3.5 years lost
    const tanPenalty =
      (fluid.tan || 0) > SYSTEM_CONSTANTS.DURABILITY.LIMITS.TAN
        ? SYSTEM_CONSTANTS.DURABILITY.PENALTIES.TAN_YEARS
        : 0; // 2.5 years lost
  ...
  calculateGridStress: (state: TechnicalProjectState): number => {
    const freq =
      ...
      SYSTEM_CONSTANTS.PHYSICS.GRID.NOMINAL_FREQUENCY;
  ...
  getVibrationVerdict: (
    ...
    if (velocityMMs < ASSET_THRESHOLDS.vibration.iso10816.unsatisfactory_max)
   ...
   */
  getInsulationVerdict: (
    resistanceMOhm: number,
    ratedVoltageKV: number = 0.4
  ...
  calculateMaintenanceUrgency: (state: TechnicalProjectState): number => {
    ...
    if (vibVerdict === 'Unacceptable')
      score = Math.max(score, ASSET_THRESHOLDS.maintenance.urgency.vibration_unacceptable_score);
    else if (vibVerdict === 'Unsatisfactory')
      score = Math.max(score, ASSET_THRESHOLDS.maintenance.urgency.vibration_unsatisfactory_score);
    else if (vibVerdict === 'Satisfactory')
      score = Math.max(score, ASSET_THRESHOLDS.maintenance.urgency.vibration_satisfactory_score);

    // Temperature Impact
    const tempVerdict = PhysicsEngine.getBearingTempVerdict(mech.bearingTemp);
    if (tempVerdict === 'Critical')
      score = Math.max(score, ASSET_THRESHOLDS.maintenance.urgency.temperature_critical_score);
    else if (tempVerdict === 'Warning')
      score = Math.max(score, ASSET_THRESHOLDS.maintenance.urgency.temperature_warning_score);

    // Insulation Impact
    const insVerdict = PhysicsEngine.getInsulationVerdict(mech.insulationResistance || 500);
    if (insVerdict === 'Critical')
      score = Math.max(score, ASSET_THRESHOLDS.maintenance.urgency.insulation_critical_score);
    else if (insVerdict === 'Degraded')
      score = Math.max(score, ASSET_THRESHOLDS.maintenance.urgency.insulation_degraded_score);

    // Operational History Stress
    const hours = state.identity.hoursSinceLastOverhaul || 0;
    if (hours > SYSTEM_CONSTANTS.DURABILITY.LIMITS.HOURS_OVERDUE_MAJOR)
      score = Math.max(score, ASSET_THRESHOLDS.maintenance.urgency.hours_major_overdue_score); // Major service past due
    else if (hours > SYSTEM_CONSTANTS.DURABILITY.LIMITS.HOURS_OVERDUE_MINOR)
      score = Math.max(score, ASSET_THRESHOLDS.maintenance.urgency.hours_minor_overdue_score);

    const starts = state.identity.startStopCount || 0;
    if (starts > SYSTEM_CONSTANTS.DURABILITY.LIMITS.START_STOP_HIGH_STRESS)
      score = Math.max(score, ASSET_THRESHOLDS.maintenance.urgency.start_stop_high_stress_score); // High cycling stress
  ...
  validateManufacturerBid: (bidEfficiencyPercent: number, turbineType: string) => {
    ...
    return {
      ...
      message: isLie
        ? `Bid ${bidEfficiencyPercent}% exceeds limit ${limit}%!`
        : 'Efficiency within physical bounds.',
    };
  },
};
