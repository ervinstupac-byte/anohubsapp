...
type JournalPayload = {
  snapshot?: TechnicalProjectState;
  stateDelta?: Partial<TechnicalProjectState>;
  id?: string;
  stateId?: string;
};

const toJournalPayload = (p: unknown): JournalPayload => {
...
class ProjectStateManagerClass {
  ...
  snapback(id: string) {
    ...
    try {
      ...
      for (const ev of replay) {
        try {
          const p = toJournalPayload(ev.payload);
          if (p.snapshot) {
          ...
          if (ev.id === id || (p && (p.id === id || p.stateId === id))) {
            ...
            this.history.unshift({
              ...
              snapshot: cloneDeep(this.state),
            });
            if (this.history.length > 50) this.history.pop();
            EventJournal.append('snapback_reconstructed', {
              requestedId: id,
              base: oldest.id,
              entryId,
            });
    ...
    try {
      ...
      try {
        dispatch.reconstructionStart({ requestedId: id, base: oldest.id });
      } catch (e) {
        /* ignore */
      }
      ...
      EventJournal.fetchOlderEvents(oldest.ts, rec => {
          ...
          if (rec.id === id || (p && (p.id === id || p.stateId === id))) {
            ...
            this.notify();
            try {
              dispatch.reconstructionComplete({ requestedId: id, entryId });
            } catch (e) {}
        ...
        .then(({ count }) => {
          if (!found) {
            try {
              dispatch.reconstructionComplete({ requestedId: id, entryId: null });
            } catch (e) {}
          }
        })
        .catch(err => {
          try {
            dispatch.reconstructionError({ message: String(err?.message || err) });
          } catch (e) {}
        });
  ...
  setState(newState: Partial<TechnicalProjectState> | TechnicalProjectState) {
    ...
    try {
      ...
      this.history.unshift({
        id: entryId,
        ts: new Date().toISOString(),
        snapshot: shallowCopy(this.state),
      });
      if (this.history.length > 5) this.history.pop();
      EventJournal.append('state_update', {
        id: entryId,
        summary: { keys: Object.keys(newState) },
      });
    } catch (e) {
      /* swallow */
    }
  ...
  updateFromTelemetry(telemetry: Record<string, any>, assets: any[]) {
    ...
    if (teleKeys.length > 0) {
      ...
      if (t) {
        s.hydraulic = {
          ...(s.hydraulic as any),
          head: t.reservoirLevel || (s.hydraulic as any).head,
          flow: t.pumpFlowRate || (s.hydraulic as any).flow,
          efficiency:
            typeof t.efficiency === 'number' ? t.efficiency / 100 : (s.hydraulic as any).efficiency,
        } as any;
        s.mechanical = {
          ...(s.mechanical as any),
          rpm: t.rpm || (s.mechanical as any).rpm,
          bearingTemp: t.temperature || (s.mechanical as any).bearingTemp,
        } as any;
        try {
          const result = PhysicsEngine.recalculatePhysics({
            identity: s.identity,
            hydraulic: s.hydraulic,
          ...
          if (result) {
            s.physics = {
              ...(s.physics as any),
              netHead: result.netHead ?? (s.physics as any).netHead,
              volumetricLoss: result.volumetricLoss ?? (s.physics as any).volumetricLoss,
              eccentricity: result.eccentricity ?? (s.physics as any).eccentricity,
            } as any;
          }
        } catch (e) {
          /* ignore physics failures */
        }
      }
    }
    ...
    if (asset) {
      s.identity = {
        ...
        commissioningYear: asset.commissioningYear || s.identity.commissioningYear,
      } as any;
    }
  ...
  private notify() {
    const snapshot = shallowCopy(this.state);
    this.subs.forEach(s => {
      try {
        s(snapshot);
      } catch (e) {
