...
const MaintenanceDashboard = lazy(() =>
  import('../components/MaintenanceDashboard.tsx').then(m => ({ default: m.MaintenanceDashboard }))
);
const MaintenanceLogbook = lazy(() =>
  import('../components/MaintenanceLogbook.tsx').then(m => ({ default: m.MaintenanceLogbook }))
);
const HydraulicMaintenance = lazy(() =>
  import('../components/HydraulicMaintenance').then(m => ({ default: m.HydraulicMaintenance }))
);
const BoltTorqueCalculator = lazy(() =>
  import('../components/BoltTorqueCalculator').then(m => ({ default: m.BoltTorqueCalculator }))
);
const SOPManager = lazy(() =>
  import('../components/SOPManager').then(m => ({ default: m.SOPManager }))
); // Shadow Engineer
const ShiftLog = lazy(() => import('../components/ShiftLog').then(m => ({ default: m.ShiftLog }))); // Intuition Log
const ARManager = lazy(() =>
  import('../components/ARManager').then(m => ({ default: m.ARManager }))
);
const TechnicalPassport = lazy(() =>
  import('../components/TechnicalPassport').then(m => ({ default: m.TechnicalPassport }))
);

// NC-20600: Re-wired orphan components
const MounterQuickCardLazy = lazy(() =>
  import('../components/MounterQuickCard').then(m => ({ default: m.MounterQuickCard }))
);
const SpecialistDamageCardLazy = lazy(() =>
  import('../components/SpecialistDamageCard').then(m => ({ default: m.SpecialistDamageCard }))
);
const AssetPassportCard = lazy(() =>
  import('../components/dashboard/AssetPassportCard').then(m => ({ default: m.AssetPassportCard }))
);
...
const MounterQuickCardPage: React.FC = () => {
  ...
  return <MounterQuickCardLazy asset={asset} onClose={() => window.history.back()} />;
};
  ...
  return (
    <Suspense fallback={<LoadingShimmer />}>
      <Routes>
        ...
        <Route
          path={ROUTES.MAINTENANCE.INTELLIGENT_DIAGNOSTICS}
