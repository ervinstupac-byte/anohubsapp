...
describe('FinancialImpactEngine - probabilistic maintenance model', () => {
  it('expectedMaintenanceCost increases with sigma', () => {
    const low = FinancialImpactEngine.calculateImpact(baseState as any, basePhysics as any, {
      sigma: 0.01,
    });
    const high = FinancialImpactEngine.calculateImpact(baseState as any, basePhysics as any, {
      sigma: 0.2,
    });
  ...
  it('inventory value reduces expectedMaintenanceCost and shortens break-even', () => {
    const invLow = FinancialImpactEngine.calculateImpact(baseState as any, basePhysics as any, {
      sigma: 0.05,
      inventoryValue: 0,
    });
    const invHigh = FinancialImpactEngine.calculateImpact(baseState as any, basePhysics as any, {
      sigma: 0.05,
    ...
    const annualNetLowInv =
      P_avg_MW * 1000 * 24 * 365 * (pricePerMWh / 1000) - (invLow.expectedMaintenanceCost || 0);
    const annualNetHighInv =
      P_avg_MW * 1000 * 24 * 365 * (pricePerMWh / 1000) - (invHigh.expectedMaintenanceCost || 0);
