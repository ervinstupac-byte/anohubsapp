...
describe('SafetyInterlockEngine design constraints & water hammer', () => {
  it('trips when Kaplan head is impossible (>1000 m)', () => {
    ...
    const res = SafetyInterlockEngine.checkProtections(80, 5, false, 3, {
      family: 'KAPLAN',
      telemetry: { wicketGatePct: 50, bladeAngleDeg: 12 },
    });
  ...
  it('blocks Pelton sequencing when ΔQ/Δt excessive', () => {
    ...
    const res = SafetyInterlockEngine.checkProtections(60, 5, false, 2, {
      family: 'PELTON',
      telemetry: { jetRateChange: 6.0 },
    });
