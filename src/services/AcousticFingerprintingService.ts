...
export class AcousticFingerprintingService {
   ...
   */
  static classifyAcousticSignature(
    ...
    for (const [key, pattern] of Object.entries(this.PATTERNS)) {
      ...
      scores[key] = this.calculatePatternMatch(filteredSpectrum, pattern, runningSpeed);
    }
    ...
    const primaryPattern = Object.entries(probabilities).reduce(
      (max, [key, prob]) => (prob > max.prob ? { key, prob } : max),
  ...
  private static filterGeneratorNoise(spectrum: number[]): number[] {
    ...
    for (const freq of notchFreqs) {
      ...
      for (
        let i = Math.max(0, centerBin - widthBins);
        i < Math.min(fftSize, centerBin + widthBins);
        i++
      ) {
    ...
    switch (pattern) {
      case 'BEARING_DAMAGE':
        if (confidence > 70) {
          return {
            recommendation:
              '🔴 CRITICAL: Bearing damage detected with high confidence. Schedule immediate inspection. Check for metal particles in oil. Shutdown if RMS exceeds 7.0 mm/s.',
            severity: 'CRITICAL',
          };
        } else if (confidence > 50) {
          return {
            recommendation:
              '⚠️ Possible bearing damage. Increase vibration monitoring frequency to every 1 hour. Schedule bearing inspection within 7 days.',
            severity: 'INVESTIGATE',
          };
        } else {
          return {
            recommendation:
              'Monitor bearing temperatures and vibration trends. No immediate action required.',
            severity: 'MONITOR',
          };
        }
      ...
      case 'NORMAL_CAVITATION':
        if (rmsLevel > 5.0) {
          return {
            recommendation:
              'Cavitation levels elevated. Check runner clearance and tailwater level. Consider operating at higher load.',
            severity: 'MONITOR',
          };
        } else {
          return {
      ...
      case 'DRAFT_TUBE_VORTEX':
        return {
          recommendation:
            'Draft tube vortex core detected. Operating at part-load. Install air admission system or increase load to design point.',
          severity: 'INVESTIGATE',
        };
      ...
      case 'MECHANICAL_LOOSENESS':
        return {
          recommendation:
            '⚠️ Mechanical looseness detected. Inspect foundation bolts, coupling, and mounting hardware. Retorque as per OEM specifications.',
          severity: 'INVESTIGATE',
        };
  ...
  static analyzeAcousticTrend(
    ...
    const timeSpanDays =
      (historicalSignatures[historicalSignatures.length - 1].timestamp -
        historicalSignatures[0].timestamp) /
      (1000 * 60 * 60 * 24);
    ...
    if (trend === 'DEGRADING' && degradationRate > 0) {
      ...
      forecastDaysToFailure =
        (failureThreshold - currentRMS) / (degradationRate / 20 / Math.log10(Math.E));
    }
