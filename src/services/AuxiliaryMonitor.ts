...
export class AuxiliaryMonitor {
   ...
   */
  checkCoolingWater(deltaPBar: number, backwashAutoEnabled: boolean): AuxHealthStatus {
    if (deltaPBar > 0.5) {
  ...
  checkHPUMuscle(
    ...
    return {
      system: 'HPU',
      status: 'HEALTHY',
      message: `Muscle strong. Oil Clean (ISO ${isoCode}) & Cool (${oilTempC}°C).`,
    };
  }
   ...
   */
  checkTransformer(buchholzTrip: boolean, windingTempC: number): AuxHealthStatus {
    if (buchholzTrip) {
      return {
        system: 'TRANSFORMER',
        status: 'CRITICAL',
    ...
    return {
      system: 'TRANSFORMER',
      status: 'HEALTHY',
      message: 'Humming quietly. Insulation secure.',
    };
  }
