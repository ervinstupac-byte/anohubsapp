...
export class SandErosionTracker {
  ...
  trackErosion(
    ...
    if (bucketThinningRate > 5000)
      severity = 'EXTREME'; // > 5mm/year!
    else if (bucketThinningRate > 1000)
      severity = 'HIGH'; // > 1mm/year
    ...
    const estimatedBucketLife =
      bucketThinningRate > 0 ? allowableWearMicrons / bucketThinningRate : 999;

    // 4. Recommendation
    let recommendation = 'Water is clean. Buckets are happy.';
    if (severity === 'EXTREME') {
      recommendation =
        '🚨 SAND STORM! Shut down or switch to settling basins! Buckets will vanish in months!';
    } else if (severity === 'HIGH') {
      recommendation =
        '⚠️ High Wear Rate. Inspect nozzles and buckets monthly. Check splitter Tips.';
