...
export class LegacyHandover {
  ...
  public static initiateSuccession(
    ...
    console.log(
      `Previous architect last seen: ${new Date(this.architectKey!.lastActivity).toISOString()}`
    );
    console.log('👑'.repeat(40) + '\n');
  ...
  public static generateSuccessionReport(): string {
    const status = this.checkTimeLockStatus();
    const heirsList = Array.from(this.heirs.values()).sort(
      (a, b) => a.unlockPriority - b.unlockPriority
    );
