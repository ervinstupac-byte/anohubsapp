...
export async function runForensicPulseCheck(): Promise<IntegrityReport> {
  ...
  try {
    ...
    // Call RPC
    const { data: rpcRes, error: rpcErr } = await supabase.rpc('compute_eta_from_pqh', {
      p_kw: samplePkw,
      q_cms: sampleQ,
      h_m: sampleH,
    });
    ...
    if (!rpcErr && rpcRes != null) {
      ...
      rpcEta =
        typeof rpcRes === 'object' && Array.isArray(rpcRes) ? Number(rpcRes[0]) : Number(rpcRes);
    }
    const expected = (samplePkw * 1000) / (RHO * G * sampleQ * sampleH);
    const ok =
      Math.abs(expected - rpcEta) < 1e-8 ||
      Math.abs((expected - rpcEta) / Math.max(1e-12, expected)) < 1e-6;
  ...
  try {
    ...
    if (insErr || !insData || !insData.id) {
      ...
      const { data: lossRes, error: lossErr } = await supabase.rpc('compute_loss_from_aggregates', {
        agg_id: aggId,
      });
      let lossVal = 0;
      if (!lossErr && lossRes != null) {
        lossVal =
          typeof lossRes === 'object' && Array.isArray(lossRes)
            ? Number(lossRes[0])
            : Number(lossRes);
      }
