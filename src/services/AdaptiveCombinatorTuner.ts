...
export class AdaptiveCombinatorTuner {
  ...
  public static getOptimizedPhi(
    ...
    const neighbors = history.filter(
      p => Math.abs(p.head - head) < 1.0 && Math.abs(p.guideVaneY - guideVaneY) < 2.0
    );
  ...
  public static learn(
    ...
    console.log(
      `[Combinator] Learned: Unit ${unitId} @ ${head.toFixed(1)}m/${guideVaneY.toFixed(1)}% -> Eff ${efficiency.toFixed(2)}%`
    );
  }
}
