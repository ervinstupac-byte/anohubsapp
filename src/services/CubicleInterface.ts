...
export class CubicleInterface {
  ...
  monitorFieldResistance(
    ...
    const resistanceDeviation =
      ratedResistance > 0 ? ((fieldResistance - ratedResistance) / ratedResistance) * 100 : 0;
  ...
  private generateExcitationAlarms(
    ...
    if (resistanceDeviation < -15) {
      alarms.push({
        ...
        diagnosis:
          '🔥 ROTOR CRAMP DETECTED! Field resistance dropped by 15%. This indicates SHORTED TURNS in the rotor winding!',
        recommendation:
          '⚠️ Check for shorted turns BEFORE the giant starts to shake! Inspect rotor winding for insulation breakdown. This can cause severe vibration and bearing damage.',
        triggeredAt: now,
        acknowledged: false,
      });
    }
    ...
    if (resistanceDeviation > 15) {
      alarms.push({
        ...
        deviation: resistanceDeviation,
        diagnosis:
          '⚡ Field resistance increased by 15%. This indicates LOOSE CONNECTIONS or OVERHEATING!',
        recommendation:
          'Check brush contact pressure, slip ring condition, and rotor winding temperature. Tighten connections if needed.',
        triggeredAt: now,
        acknowledged: false,
      });
    }
  ...
  linkCubicleToAsset(
    ...
    const hasAlarm = excitationData.alarms.some(
      a => a.severity === 'ALARM' || a.severity === 'TRIP'
    );
    ...
    const combinedHealth =
      electricalHealth === 'ALARM' || mechanicalHealth === 'ALARM'
        ? 'ALARM'
        : electricalHealth === 'WARNING' || mechanicalHealth === 'WARNING'
          ? 'WARNING'
          : 'HEALTHY';
