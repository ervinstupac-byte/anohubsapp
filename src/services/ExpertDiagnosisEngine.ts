...
export const ExpertDiagnosisEngine = {
    ...
    runExpertDiagnosis: (
        ...
        if (safetyFactor.lt(1.5)) {
            ...
            reports.messages.push({
                ...
                bs: 'Kritično naprezanje stijenke: Faktor sigurnosti ispod 1.5. Potrebno hitno smanjenje pritiska.'
            });
    ...
    runDiagnostics: (
        ...
        const thermalSeverity = ambientTemp > 45 ? 'HIGH' : (ambientTemp > 35 ? 'MEDIUM' : 'LOW');
        const gridSeverity = (gridFrequency < 49.5 || gridFrequency > 50.5) ? 'CRITICAL' : 'LOW';
        const cavitationSeverity = flow > 42.5 ? 'CRITICAL' : 'LOW';

        return {
            thermalRisk: {
                detected: ambientTemp > 35,
