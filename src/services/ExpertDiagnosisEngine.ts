...
export class ExpertDiagnosisEngine {
    ...
    if (ambientTempC > constants.thermal.criticalAmbientTemp && lubrication === 'GREASE') {
      if (clearanceMM < constants.thermal.minClearanceForGrease) {
        ...
        return {
          ...
          severity: 'MEDIUM',
          description:
            'Thermal expansion warning: Monitor clearances closely. Grease expands more than oil at high temperatures.',
          descriptionDE:
            'Wärmeausdehnungswarnung: Spiele genau überwachen. Schmierfett dehnt sich bei hohen Temperaturen stärker aus als Öl.',
        };
      }
    }
   ...
   */
  static assessAxialThrust(frontClearanceMM: number, backClearanceMM: number): AxialThrustRisk {
    ...
    if (diffPercent > 15) {
      return {
        ...
        severity: 'HIGH',
        recommendation:
          'Asymmetric clearances (>15% difference) cause unbalanced axial thrust. Inspect thrust bearing and balance chamber pressures.',
        recommendationDE:
          'Asymmetrische Spiele (>15% Unterschied) verursachen unausgeglichenen Axialschub. Drucklager und Ausgleichskammerdruck prüfen.',
      };
   ...
   */
  static assessSensorCoverage(sensorMatrix: SensorMatrix): SensorCoverageRisk {
  ...
  static assessJackingSystem(
    ...
    const requiredPressure =
      ((rotorWeightTons * 1000 * constants.physics.gravity) / (jackingAreaCM2 * 100)) *
      constants.maintenance.jackingSafetyFactor;
  ...
  static assessGridFrequency(frequencyHz: number, constants: EngineeringConstants): GridRisk {
    const minFreq =
      constants.electrical.nominalGridFrequency - constants.electrical.gridFrequencyTolerance;
    const maxFreq =
      constants.electrical.nominalGridFrequency + constants.electrical.gridFrequencyTolerance;

    if (
      frequencyHz >= constants.electrical.criticalFrequencyThreshold ||
      frequencyHz > maxFreq ||
      frequencyHz < minFreq
    ) {
      return {
  ...
  static calculateFinancialImpact(
    ...
    const lossMW = isCritical
      ? power
      : power * (100 - healthScore) * constants.maintenance.efficiencyLossPerHealthPoint;
    ...
    return {
      ...
      recommendationDE: isCritical
        ? 'SOFORTIGE ABSCHALTUNG & INSPEKTION'
   ...
   */
  static calculateHealthScore(diagnostics: DiagnosticResults): HealthScore {
    let score = 100;
  ...
  static runDiagnostics(
    ...
    const safeConstants =
      constants ||
      ({
        thermal: { criticalAmbientTemp: 30, minClearanceForGrease: 0.1 },
        electrical: {
          nominalGridFrequency: 50.0,
          gridFrequencyTolerance: 0.5,
          criticalFrequencyThreshold: 98.2,
        },
        hydraulic: {
          cavitationFlowThreshold: 42.5,
          cavitationHeadThreshold: 152.0,
          targetRunnerClearance: 0.4,
        },
    ...
    const thermalRisk = this.assessThermalRisk(
      ambientTempC,
      lubrication,
      avgClearance,
      safeConstants
    ...
    const finalGridFreq =
      gridFrequency !== undefined ? gridFrequency : safeConstants.electrical.nominalGridFrequency;
    const gridRisk = this.assessGridFrequency(finalGridFreq, safeConstants);

    // Cavitation Risk using reactive constants
    let cavitationRisk = {
      severity: 'OPTIMAL' as RiskSeverity,
      message: 'Hydraulic conditions stable.',
    };
    if (currentFlow !== undefined && currentHead !== undefined) {
      if (
        Math.abs(currentFlow - safeConstants.hydraulic.cavitationFlowThreshold) < 0.2 &&
    ...
    const jackingRisk =
      assetIdentity.shaftJacking && rotorWeightTons
