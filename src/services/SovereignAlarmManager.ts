...
export class SovereignAlarmManager {
  ...
  public static raiseAlarm(alarm: Omit<Alarm, 'id' | 'timestamp' | 'state'>): string {
    ...
    console.log(
      `[AlarmMgr] ${AlarmPriority[newAlarm.priority]} ALARM: ${newAlarm.message} [${id}]`
    );
  ...
  private static checkSuppressionRules(alarm: Alarm): boolean {
    ...
    for (const [rootId, secondaries] of this.suppressionRules.entries()) {
      ...
      if (rootAlarm && rootAlarm.state === AlarmState.ACTIVE) {
        ...
        const shouldSuppress = secondaries.some(
          pattern => alarm.category?.includes(pattern) || alarm.message.includes(pattern)
        );
  ...
  public static registerSuppressionRule(rootCategory: string, secondaryCategories: string[]): void {
    ...
    console.log(
      `[AlarmMgr] Suppression rule: ${rootCategory} suppresses ${secondaryCategories.join(', ')}`
    );
  }
  ...
  public static getStatistics(): {
    ...
    const active = Array.from(this.alarms.values()).filter(
      a => a.state === AlarmState.ACTIVE
    ).length;
    const acknowledged = Array.from(this.alarms.values()).filter(
      a => a.state === AlarmState.ACKNOWLEDGED
    ).length;
    const suppressed = Array.from(this.alarms.values()).filter(
      a => a.state === AlarmState.SUPPRESSED
    ).length;
  ...
  public static exportHistory(startTime: number, endTime: number): Alarm[] {
    return this.alarmHistory.filter(a => a.timestamp >= startTime && a.timestamp <= endTime);
  }
}

// Initialize common suppression rules
SovereignAlarmManager.registerSuppressionRule('TEMPERATURE_HIGH', [
  'VIBRATION_SECONDARY',
  'EFFICIENCY_LOW',
]);
SovereignAlarmManager.registerSuppressionRule('VORTEX_DETECTED', [
  'VIBRATION_DRAFT_TUBE',
