...
import { getLastAutoTableFinalY } from '../utils/pdfHelpers';
...
export class AutoReportService {
  ...
  private generate12MonthRecommendations(
    ...
    const highVib = measurements.find(
      m => m.parameter.toLowerCase().includes('vibration') && m.asLeft > m.standard * 0.8
    );
    if (highVib) {
      recs.push({
        priority: 'CRITICAL',
        ...
        consequence:
          'Nedetektiran rast vibracija može dovesti do havarije ležajeva i zastoja od 5-10 dana',
      });
    }
    ...
    if (alignment && alignment.asLeft > alignment.standard * 0.7) {
      recs.push({
        ...
        consequence:
          'Progresivna dezalinacija uzrokuje ubrzano trošenje ležajeva (smanjenje životnog vijeka za 30-40%)',
      });
    }
    ...
    if (turbineFamily === 'KAPLAN') {
      recs.push({
        ...
        consequence:
          'Degradirano ulje smanjuje brzinu odziva lopatica i može uzrokovati blokadu u kritičnom momentu',
      });
    }
