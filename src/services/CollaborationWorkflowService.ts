...
export class CollaborationWorkflowService {
  ...
  private static async validateGeodeticMeasurement(data: any): Promise<AIValidationResult> {
    ...
    if (!passed) {
      deviations.push(
        `Shaft deviation ${shaftDeviation.toFixed(3)} mm/m exceeds 0.05 mm/m standard by ${(shaftDeviation - STANDARD_005_MM_M).toFixed(3)} mm/m`
      );
    ...
    const complianceScore = Math.max(
      0,
      (1 - (shaftDeviation - STANDARD_005_MM_M) / STANDARD_005_MM_M) * 100
    );
  ...
  private static async validateHydraulicChange(data: any): Promise<AIValidationResult> {
    ...
    return {
      ...
      requiresConsultantReview:
        result.recommendation === 'APPROVE_WITH_MODIFICATIONS' ||
  ...
  private static async validateVibrationMeasurement(data: any): Promise<AIValidationResult> {
    ...
    return {
      ...
      complianceScore: passed
        ? 100
        : Math.max(0, (1 - (vibrationLevel - THRESHOLD) / THRESHOLD) * 100),
      deviationsFrom005Standard: passed
        ? []
        : [`Vibration ${vibrationLevel.toFixed(2)} mm/s exceeds ${THRESHOLD} mm/s`],
