...
export class H2Synthesizer {
  ...
  public static startElectrolysis(
    ...
    console.log(
      `Expected H2 Production: ${this.calculateH2Production(powerInput, 1).toFixed(2)} Nm³/hour`
    );
    console.log('⚡'.repeat(40) + '\n');
  ...
  private static runElectrolysis(session: ElectrolysisSession): void {
    const productionInterval = setInterval(() => {
      ...
      if (Math.random() > 0.95) {
        // Periodic status
        console.log(
          `[H2] Producing... ${session.h2Produced.toFixed(2)} Nm³ total, Storage: ${this.storage.fillLevel.toFixed(1)}%`
        );
      }
    }, 1000);
  ...
  private static stopElectrolysis(session: ElectrolysisSession): void {
    ...
    console.log(
      `Storage Level: ${this.storage.fillLevel.toFixed(1)}% (${this.storage.volume.toFixed(0)} Nm³)`
    );
