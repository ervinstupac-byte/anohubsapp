 ...
 */
export const DAMAGE_IDENTIFICATION_GUIDE: Record<
  DamageType,
  {
    name: string;
    description: string;
...
> = {
  [DamageType.CAVITATION]: {
    ...
    description:
      'Caused by vapor bubbles collapsing on metal surface, creating tiny shock waves that erode material',
  ...
  [DamageType.EROSION_SAND]: {
    name: 'Sand Erosion',
    description:
      'Caused by hard particles (sand, silt) impacting blade surface at high velocity, gradually wearing away material',
  ...
  [DamageType.EROSION_SILT]: {
    name: 'Silt Erosion',
    description:
      'Similar to sand erosion but caused by finer particles, creates widespread uniform thinning',
  ...
  [DamageType.FATIGUE_CRACK]: {
    ...
    visualCharacteristics: {
      appearance: 'Crack',
      location: 'Stress concentrations',
      texture: 'Smooth',
      pattern: 'Linear',
      color: 'Dark',
      depth: 'Deep',
...
export class SpecialistNotebook {
  ...
  static diagnoseDamage(
    ...
    for (const [type, guide] of Object.entries(DAMAGE_IDENTIFICATION_GUIDE)) {
      ...
      if (operatingConditions) {
        if (operatingConditions.sigma !== undefined && operatingConditions.sigma < 0.1) {
          if (type === DamageType.CAVITATION) {
            score += 20;
        ...
        if (
          operatingConditions.sedimentPPM !== undefined &&
          operatingConditions.sedimentPPM > 200
        ) {
  ...
  private static estimateSeverity(
    description: string
