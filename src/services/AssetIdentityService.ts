...
export class AssetIdentityService {
  ...
  static calculateAxialThrustBalance(specialized: SpecializedAdvancedModule): {
    ...
    const diff = Math.abs(
      specialized.draftTubePressure.nominalBar - specialized.backRunnerPressure.nominalBar
    );
   ...
   */
  static checkExcessiveHeat(ambientC: number, bearingTemps: number[]): boolean {
   ...
   */
  static calculateEfficiency(powerMW: number, headM: number, flowM3S: number): number {
    // η = P / (ρ * g * Q * H) * 100
  ...
  static addOperatingPoint(
    ...
    const uniqueBins = new Set(updatedPoints.map(p => Math.round(p.vaneOpeningPercent / 5) * 5));
    const coveragePercent = (uniqueBins.size / 20) * 100;

    // Find best efficiency point
    const bestPoint = updatedPoints.reduce(
      (best, current) => {
  ...
  static generateUpgradeRecommendations(
    ...
    if (sensorMatrix.vibrationSensors.generator.length === 0) {
      recommendations.push({
        ...
        description:
          'Generator has no vibration sensors. Early detection of bearing failure can prevent catastrophic damage (avg. €25,000 emergency repair cost).',
    ...
    if (sensorMatrix.vibrationSensors.turbine.length === 0) {
      recommendations.push({
        ...
        description:
          'Turbine vibration monitoring essential for detecting cavitation, misalignment, and bearing wear.',
    ...
    if (machineConfig.transmissionType === 'GEARBOX') {
      if (
        !sensorMatrix.vibrationSensors.gearbox ||
        sensorMatrix.vibrationSensors.gearbox.length === 0
      ) {
        recommendations.push({
          ...
          description:
            'Gearbox failures are expensive (€50,000+). Vibration monitoring provides 2-4 weeks advance warning.',
    ...
    if (sensorMatrix.temperatureSensors.bearings.length < 2) {
      recommendations.push({
        ...
        description:
          'Only ' +
          sensorMatrix.temperatureSensors.bearings.length +
          ' bearing temperature sensor(s) installed. Recommend minimum 4 (all critical bearings).',
    ...
    if (sensorMatrix.temperatureSensors.powerhouse.length === 0) {
      recommendations.push({
        ...
        description:
          'Ambient temperature correlation helps diagnose bearing overheating vs. environmental factors.',
