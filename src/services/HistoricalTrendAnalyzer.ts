...
export class HistoricalTrendAnalyzer {
   ...
   */
  static calculateLinearRegression(measurements: HistoricalMeasurement[]): {
    slope: number;
    intercept: number;
    rSquared: number;
  } {
   ...
   */
  static analyzeTrend(history: MeasurementHistory, criticalThreshold: number): TrendProjection {
   ...
   */
  static getTrendStatus(trend: TrendProjection): {
  ...
  static generateChartData(
    ...
    if (history.trend) {
      ...
      for (let day = 0; day <= projectionEnd; day += 7) {
        // Weekly intervals
