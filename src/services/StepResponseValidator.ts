...
export class StepResponseValidator {
  ...
  public static analyzeResponse(
    ...
    const t10 = timeSeries.find(
      p => p.t > stepStartTime && (stepSize > 0 ? p.val >= range10 : p.val <= range10)
    )?.t;
    const t90 = timeSeries.find(
      p => p.t > stepStartTime && (stepSize > 0 ? p.val >= range90 : p.val <= range90)
    )?.t;
