...
export class TurbineMasterCore {
   ...
   */
  public static calculateShaftOrbit(sensorX: number, sensorY: number, rpm: number): ShaftOrbit {
    ...
    if (ratio > 5 || ratio < 0.2)
      orbitShape = 'FLAT'; // High unbalance in one plane or misalignment
    else if (ratio > 1.5 || ratio < 0.75)
      orbitShape = 'ELLIPTICAL'; // Typical unbalance
  ...
  public static balanceNozzleLoads(
    ...
    if (maxDeviation > 20)
      recommendation =
        'INSPECT JETS: Severe imbalance detected. Check for clogged nozzles or erosive wear.';
  ...
  public static estimateLabyrinthLeakage(
    ...
    const sealEfficiency = Math.max(
      0,
      100 - ((estimatedLeakage - designLeakageLps) / designLeakageLps) * 100
    );
