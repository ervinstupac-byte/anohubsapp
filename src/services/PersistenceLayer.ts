...
export class PersistenceLayer {
  ...
  public static async saveEnrichedTelemetry(enriched: EnrichedTelemetry): Promise<void> {
    try {
      const asRecord = enriched as unknown as Record<string, unknown>;
      const record = {
        timestamp: enriched.timestamp,

        // Foundation layer: store full hydraulic and mechanical snapshots
        raw_telemetry: {
          hydraulic: enriched.hydraulic,
          mechanical: enriched.mechanical,
          physics: enriched.physics ?? null,
          identity: enriched.identity ?? null,
        },

        // Middle layer - Correlation
        correlation_state: enriched.correlationState ?? null,
        ...
        healing_action: enriched.healingAction ?? null,

        // Economics layer (optional custom field)
        roi_impact: (asRecord.roiImpact as unknown) ?? null,

        // Sovereignty layer (optional custom field)
        sovereignty_signature: (asRecord.sovereigntySignature as unknown) ?? null,
  ...
  public static async saveBatch(
    ...
    console.log(
      `[PersistenceLayer] Batch saving: ${telemetry.length} telemetry, ${traces.length} traces, ${entries.length} ledger entries`
    );
