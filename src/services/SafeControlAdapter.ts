...
export class SafeControlAdapter {
  ...
  canApplySequence(state: ControlState) {
    ...
    if (state.frequencyHz && (state.frequencyHz < 49.5 || state.frequencyHz > 50.5))
      return { ok: false, reason: 'Frequency outside safe window' };
    return { ok: true };
  }

  applySequenceIfSafe(
    seq: { activeNozzles: number; sequenceOrder?: number[] },
    state: ControlState,
    requestedBy = 'SafeControlAdapter'
  ) {
    ...
    try {
      ...
      if (typeof requestedClose === 'number' && requestedClose < safeClose * 0.5) {
        return {
          applied: false,
          reason: `Requested close duration ${requestedClose}s violates governor safe close time ${safeClose}s`,
        };
      }
    } catch (e) {
      // ignore and continue
