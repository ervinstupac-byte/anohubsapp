...
export class HydraulicTransientSafety {
  ...
  static simulateHardwareChange(
    ...
    const waterHammerRisk =
      proposedSpec.pipeDiameterMM > currentSpec.pipeDiameterMM && !proposedSpec.accumulators;
    if (waterHammerRisk) {
      warnings.push(
        'Water hammer risk increased due to higher flow potential without added accumulation.'
      );
    }
    ...
    if (currentSpec.pipeDiameterMM === 12 && proposedSpec.pipeDiameterMM === 16) {
      warnings.push(
        'KNOWN HAZARD (Lesson Learned #442): 12mm->16mm upgrade often causes servo instability.'
      );
    ...
    return {
      ...
      metrics: {
        ...
        naturalFrequencyNew: new Decimal(100)
          .mul(new Decimal(1).plus(frequencyShiftPct.div(100)))
          .toNumber(),
