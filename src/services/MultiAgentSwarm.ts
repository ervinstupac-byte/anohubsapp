...
export class MultiAgentSwarm {
  ...
  public static async decideOptimalStrategy(context: {
    ...
    for (const [agentId, agent] of this.agents.entries()) {
      ...
      console.log(
        `  ${agentId}: ${vote.recommendation} (confidence: ${(vote.confidence * 100).toFixed(0)}%)`
      );
    }
   ...
   */
  private static computeNashEquilibrium(votes: AgentVote[], context: any): NashEquilibrium {
