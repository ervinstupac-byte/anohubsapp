...
export class StatorInsulationGuardian extends BaseGuardian {
  ...
  addMeasurement(m: PDMeasurement): StatorAction {
    ...
    if (qtrend && qtrend.pctIncrease !== null && qtrend.pctIncrease >= this.qMaxTrendPctThreshold) {
      return {
        action: 'CRITICAL_INSULATION_DEGRADATION',
        qMaxIncreasePct: Number(qtrend.pctIncrease.toFixed(2)),
        reason: `Q_max increased ${qtrend.pctIncrease.toFixed(2)}% over 30-day window.`,
      };
    }
    ...
    if (
      severity === 'HIGH' ||
      severity === 'CRITICAL' ||
      (qtrend && qtrend.pctIncrease !== null && qtrend.pctIncrease >= this.boroscopeThresholdPct)
    ) {
      return {
        action: 'BOROSCOPE_INSPECTION_RECOMMENDED',
        severity,
        reason: `PD activity ${severity}. Consider visual boroscope inspection.`,
      };
    }
  ...
  estimateRULHours(): { rulHours: number; details: string } {
    ...
    return {
      ...
      details: `avgTemp=${avgTemp.toFixed(1)}°C avgQ=${avgQ.toFixed(1)}pC avgPPS=${avgPPS.toFixed(1)} agingFactor=${agingFactor.toFixed(3)}`,
    };
  }

  getHealthImpactForAction(a: StatorAction) {
    switch (a.action) {
      case 'CRITICAL_INSULATION_DEGRADATION':
        return {
          overallDelta: -70,
          recommendation: 'Immediate borescope and replace winding sections.',
        };
      case 'BOROSCOPE_INSPECTION_RECOMMENDED':
        return { overallDelta: -30, recommendation: 'Schedule boroscope inspection.' };
      case 'SCHEDULE_MAINTENANCE':
        return { overallDelta: -20, recommendation: 'Schedule maintenance.' };
      default:
        return { overallDelta: 0, recommendation: 'No action' };
    }
  }
