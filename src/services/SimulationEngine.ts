...
export class SimulationEngine {
  ...
  public static runWhatIfAnalysis(input: SimulationInput): SimulationResult[] {
    ...
    const realAggressiveProfit = this.recalculateRealProfit(
      input.history,
      aggressiveResult.actions,
      input.baselineContext
    );

    const winner =
      balancedResult.profit > realAggressiveProfit ? 'SOVEREIGN_BALANCED' : 'AGGRESSIVE';
  ...
  private static simulatePath(
  ...
  ): { profit: number; wear: number; actions: PrescriptiveAction[][] } {
    ...
    history.forEach(telemetry => {
      ...
      const result = SovereignStrategist.calculateBridge(telemetry, context, {
  ...
  private static recalculateRealProfit(
    history: TelemetryStream[],
    actions: PrescriptiveAction[][],
    realContext: FinancialContext
  ): number {
