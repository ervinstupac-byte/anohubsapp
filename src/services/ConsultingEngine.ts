...
export class ConsultingEngine extends BaseGuardian {
  /**
  ...
  static generateOptimizationReport(
    ...
    if (thermoMeasurements.length > 0) {
      findings.push(...this.analyzeThermalSignature(thermoMeasurements, asset.turbine_family));
    }
  ...
  private static analyzeAlignment(
    ...
    if (shaftDeviation > alignmentTolerance) {
      ...
      const severityLevel: any =
        excessDeviation > 0.1 ? 'CRITICAL' : excessDeviation > 0.03 ? 'HIGH' : 'MEDIUM';
    ...
    if (foundationSettlement > 2.0) {
      findings.push({
        ...
        suggestedAction:
          'Geotechnical survey required. Foundation grouting or underpinning may be necessary. Monitor quarterly.',
  ...
  private static analyzeVibration(
    ...
    if (vibrationLevel > vibrationLimit) {
      ...
      findings.push({
        ...
        suggestedAction:
          'Perform detailed vibration diagnostics with FFT analysis. Shutdown if vibration exceeds 7.0 mm/s.',
  ...
  private static analyzeThermalSignature(
    ...
    if (generatorTemp > 95) {
      findings.push({
        ...
        suggestedAction:
          'Reduce load. Check cooling water flow and air filters. Inspect for blocked cooling ducts.',
  ...
  private static analyzeOilQuality(measurements: SpecialMeasurement[]): Finding[] {
    ...
    if (waterContent > 500) {
      findings.push({
        ...
        suggestedAction:
          'Replace oil. Inspect seals and breathers for water ingress points. Install desiccant breather.',
    ...
    if (particleCount > 5000) {
      findings.push({
        ...
        suggestedAction:
          'Install finer filters (ISO 16/14/11 target). Flush system. Check for pump wear.',
  ...
  private static prioritizeReconstructionOptions(
    ...
    return recommendations.sort(
      (a, b) => b.expectedBenefit / b.estimatedCost - a.expectedBenefit / a.estimatedCost
    );
  }
