...
export class HVShield {
   ...
   */
  public static performDGA(transformerId: string, gases: DGAAnalysis['gases']): DGAAnalysis {
    ...
    if (gases.acetylene > 35) {
      diagnosis = 'ARCING';
      recommendation =
        'CRITICAL: Arcing detected. Schedule immediate outage for internal inspection.';
    }
    ...
    // High hydrogen with low hydrocarbons indicates partial discharge
      ...
      recommendation =
        'WARNING: Partial discharge activity. Monitor closely, schedule inspection within 3 months.';
    }
   ...
   */
  public static updateRelayStatus(relayId: string, status: ProtectionRelay['status']): void {
  ...
  public static getSystemStatus(): {
    ...
    if (criticalDGA || relayTrips || criticalHotspots) {
      ...
      transformers.some(t => t.status !== 'NORMAL') ||
      recentHotspots.some(h => h.severity === 'HIGH')
    ) {
      overallHealth = 'WARNING';
    } else if (
      transformers.some(t => t.status === 'PARTIAL_DISCHARGE') ||
      recentHotspots.some(h => h.severity === 'ELEVATED')
    ) {
