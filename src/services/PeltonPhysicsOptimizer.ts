...
export function detectNeedleErosion(
  history: PeltonInput[],
  tolerancePct = 10
): { eroded: boolean; trendSlope?: number; note?: string } {
  ...
  return {
    eroded,
    trendSlope: slope,
    note: `Q/s relative change ${relDropPct.toFixed(2)}% per sample`,
  };
}

export function optimizeNozzles(
  input: PeltonInput,
  options?: { maxNozzles?: number; minNozzles?: number }
): NozzleSequence {
  ...
  let best: NozzleSequence = {
    activeNozzles: input.activeNozzles,
    expectedEfficiencyPct: calculatePeltonEfficiency(
      input.bucketHours || 0,
      input.jetPressureBar,
      estimatedFlow_m3s(input.jetPressureBar, input.needlePositionPct)
    ),
  };
  ...
  for (let n = minN; n <= maxN; n++) {
    ...
    const totalQ =
      estimatedFlow_m3s(input.jetPressureBar, input.needlePositionPct) * input.activeNozzles;
    ...
    if (totalEff > best.expectedEfficiencyPct) {
      best = {
        activeNozzles: n,
        expectedEfficiencyPct: +totalEff.toFixed(3),
        notes: `qPerNozzle=${qPerNozzle.toExponential(2)}m3/s vibPenalty=${vibrationPenalty.toFixed(2)}%`,
      };
    }
  }
