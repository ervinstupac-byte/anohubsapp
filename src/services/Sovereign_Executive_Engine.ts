  ...
  executeCycle(
    ...
    if (inputs.physicsAnalysis) {
      ...
      if (surgePressureBar && surgePressureBar > 40) {
        return this.emergencyShutdown(
          `CRITICAL SURGE PRESSURE (${surgePressureBar.toFixed(1)} bar) - WATER HAMMER DETECTED`
        );
      }
    ...
    if (!deadManStatus.safe) {
      if (metadata?.isReplay) {
        console.warn(
          `🚨 EMERGENCY OVERRIDE by SYSTEM_REFLEX: DEAD MAN SWITCH TRIPPED (${deadManStatus.action}). REPLAY MODE ACTIVE -> TRANSITIONING TO MONITOR_ONLY.`
        );
    ...
    const erosionSynergy = ErosionCorrosionSynergy.calculateSynergy(
      inputs.erosion,
      inputs.ph,
      0.05
    );
    ...
    if (
      inputs.turbineType === 'KAPLAN' &&
      inputs.bladeAngle !== undefined &&
      inputs.hubOilPressure !== undefined
    ) {
      // A. Combinator Check
      ...
      const combinator = this.kaplanPhysics.calculateEfficiency(
        estimatedGate,
        inputs.bladeAngle,
        20
      ); // Head=20m
      ...
      if (combinator.isOffCam) {
        protections.push(
          `OFF-CAM: Eff drop ${combinator.efficiency.toFixed(1)}%. Deviation ${combinator.deviation.toFixed(1)}°.`
        );
        opMsg = 'OPTIMIZING: Blade Angle adjusting to Cam Curve...';
      }

      // B. Hub Health
      const hubHealth = this.kaplanHub.checkHubHealth(
        inputs.hubOilPressure,
        inputs.tailwaterLevel || 10,
        2
      ); // default Levels
    ...
    const molecularHealth = this.molecularMonitor.calculateCrystalStress(
      'U1',
      50,
      inputs.vibration,
      24
    ...
    const transformerCritical =
      Array.isArray(overrides) && overrides.some((o: any) => o && o.transformerCritical);
    ...
    if (inputs.pelton && typeof inputs.pelton.jetPressureBar === 'number') {
      try {
        ...
        const seq = this.peltonOptimizer.optimizeNozzles(peltonInp, {
          maxNozzles: Math.max(8, peltonInp.activeNozzles + 4),
          minNozzles: 1,
        });

        // Market-driven decision
        const oracle = { hourlyPricesEurPerMWh: [(inputs.market && inputs.market.price) || 50] };
        const baselineEff = this.peltonOptimizer.optimizeNozzles({
          ...peltonInp,
          activeNozzles: peltonInp.activeNozzles,
        ...
        protections.push(
          `PELTON_OPTIMIZER: suggested ${seq.activeNozzles} nozzles (eff ${seq.expectedEfficiencyPct}%). Mode=${decision.mode} netBenefit=${decision.expectedNetBenefitEurPerHour.toFixed(2)}€/h`
        );
        ...
        if (
          (decision.mode === 'PERFORMANCE' || decision.mode === 'BALANCE') &&
          this.safeControlAdapter
        ) {
          if (tier === PermissionTier.AUTONOMOUS) {
            const controlState = { stableLoad: inputs.vibration <= 1.5, frequencyHz: 50 };
            const applied = this.safeControlAdapter.applySequenceIfSafe(
              { activeNozzles: seq.activeNozzles, sequenceOrder: seq.sequenceOrder },
              controlState,
              'Sovereign_Executive_Engine'
            );
            protections.push(
              `PELTON_CONTROL: applySequence result applied=${applied.applied} reason=${(applied as any).reason || 'ok'}`
            );
