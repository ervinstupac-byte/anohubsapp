...
export class VPPSynchronizer {
  ...
  private static checkPeerHealth(): void {
    ...
    for (const [nodeId, node] of this.peerNodes.entries()) {
      if (now - node.lastSeen > timeout) {
        console.log(
          `[VPP] ⚠️ Peer node ${nodeId} offline (last seen ${Math.floor((now - node.lastSeen) / 1000)}s ago)`
        );
        node.status = 'OFFLINE';
      }
    }
   ...
   */
  public static shareHydrologicalPrediction(
    prediction: Omit<HydrologicalPrediction, 'sourceNode' | 'timestamp'>
  ): void {
    if (!this.localNode) return;
   ...
   */
  public static getBasinPrediction(
    basinId: string,
    hoursAhead: number
  ): {
    hour: number;
    ...
    const aggregated: Map<number, { sum: number; count: number; confidenceSum: number }> =
      new Map();
   ...
   */
  public static sendCoordinationCommand(
    command: Omit<VPPCoordinationCommand, 'commandId' | 'timestamp'>
  ): VPPCoordinationCommand {
  ...
  public static getNetworkStatus(): {
    ...
    const onlinePeers = Array.from(this.peerNodes.values()).filter(
      n => n.status === 'ONLINE'
    ).length;
  ...
  public static optimizeBasinLoad(basinId: string): {
    ...
    const basinNodes = Array.from(this.peerNodes.values()).filter(
      n => n.basinId === basinId && n.status === 'ONLINE'
    );
