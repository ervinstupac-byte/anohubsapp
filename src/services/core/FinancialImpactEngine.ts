...
class FinancialImpactEngineClass extends BaseGuardian {
  ...
  calculateNPV(criteria: NPVCriteria): NPVResult {
    ...
    const totalReturn =
      criteria.annualCashFlows.reduce((a, b) => a + b, 0) + (criteria.salvageValue || 0);
  ...
  calculateImpact(state: TechnicalProjectState, physics: PhysicsResult, opts?: any) {
    ...
    const pricePerMWh =
      pricePerMWhOverride !== undefined ? pricePerMWhOverride : defaultPricePerMWh;

    const baselinePower = state.hydraulic.baselineOutputMW
      ? typeof (state.hydraulic.baselineOutputMW as any).toNumber === 'function'
        ? (state.hydraulic.baselineOutputMW as any).toNumber()
        : Number(state.hydraulic.baselineOutputMW)
      : profile?.metadata.nominalPowerMW || 100;
    const currentPower =
      typeof (physics.powerMW as any)?.toNumber === 'function'
        ? (physics.powerMW as any).toNumber()
        : Number(physics.powerMW);
    ...
    const ageFactor = (() => {
      const hours =
        state.selectedAsset?.totalOperatingHours ||
        state.selectedAsset?.hoursSinceLastOverhaul ||
        0;
    ...
    const inventoryMultiplier =
      inventoryValue && inventoryValue > 0 ? 1 - Math.min(0.5, inventoryValue / 1_000_000) : 1;
    ...
    const maintenanceSavingsEuro =
      (emergencyRepairCost - preventativeActionCost) * detectionConfidence;
  ...
  public getConfidenceScore(state?: TechnicalProjectState, physics?: PhysicsResult): number {
    ...
    const hasBaseline =
      typeof state.hydraulic.baselineOutputMW === 'number' ||
      (state.hydraulic.baselineOutputMW as any)?.toNumber;
...
export const FinancialImpactEngine = {
  ...
  analyzeMaintenanceInvestment:
    financialImpactEngine.analyzeMaintenanceInvestment.bind(financialImpactEngine),
