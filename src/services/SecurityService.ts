...
export class SecurityService {
   ...
   */
  public static sign(keyId: string, data: string): QuantumSignature {
   ...
   */
  public static verify(keyId: string, data: string, signature: QuantumSignature): boolean {
    const keyPair = this.keyPairs.get(keyId);
    if (!keyPair) {
      throw new Error(`Key pair "${keyId}" not found`);
    }
   ...
   */
  public static exportPublicKeys(): Map<
    string,
    {
      algorithm: string;
