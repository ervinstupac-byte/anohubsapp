...
export class CavitationAcousticAnalyser {
  ...
  public static analyzeAcousticEmissions(
    ...
    const peakFrequency =
      this.CAVITATION_FREQUENCIES[Math.floor(Math.random() * this.CAVITATION_FREQUENCIES.length)];
   ...
   */
  public static async findSweetSpot(currentHead: number, maxLoad: number): Promise<SweetSpot> {
