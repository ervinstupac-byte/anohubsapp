...
export class ThePulseEngine {
  ...
  public aggregateHealth(inputs: {
    ...
    const revenueScore =
      inputs.financial.revenue > 0 ? Math.min(100, (inputs.financial.revenue / 10000) * 100) : 0;
    const costScore =
      inputs.financial.costs > 0 ? Math.max(0, 100 - (inputs.financial.costs / 5000) * 100) : 100;
    const financialScore = revenueScore * 0.6 + costScore * 0.4;

    // 3. Environmental Health Score (20% weight)
    const waterQualityScore =
      inputs.environmental.waterQuality === 'Excellent'
        ? 100
        : inputs.environmental.waterQuality === 'Good'
          ? 80
          : inputs.environmental.waterQuality === 'Fair'
            ? 60
            : inputs.environmental.waterQuality === 'Poor'
              ? 30
              : 50;

    const temperatureEnvScore = Math.max(
      0,
      100 - Math.abs(inputs.environmental.temperature - 20) * 2
    );
    const flowScore =
      inputs.environmental.flow > 0 ? Math.min(100, (inputs.environmental.flow / 50) * 100) : 0;

    const environmentalScore =
      waterQualityScore * 0.4 + temperatureEnvScore * 0.3 + flowScore * 0.3;

    // 4. Cyber Health Score (10% weight) - reduced to make room for grid
    const connectionScore =
      inputs.cyber.connectionStatus === 'CONNECTED'
        ? 100
        : inputs.cyber.connectionStatus === 'DEGRADED'
          ? 50
          : inputs.cyber.connectionStatus === 'DISCONNECTED'
            ? 0
            : 25;
    ...
    const lastUpdateScore = inputs.cyber.lastUpdate
      ? Math.max(
          0,
          100 - (Date.now() - new Date(inputs.cyber.lastUpdate).getTime()) / (1000 * 60 * 5)
        )
      : 50;
  ...
  public static calculatePulse(
    ...
    const globalIndex =
      physicalScore * 0.4 + cyberScore * 0.2 + environmentalScore * 0.2 + financialScore * 0.2;
