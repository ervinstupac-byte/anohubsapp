...
export default class CoolingSystemGuardian extends BaseGuardian {
  ...
  public analyze(
    ...
    options?: { rotationIntervalHours?: number }
  ): CoolingAnalysis {
    ...
    const foulingDropPct =
      U && this.cleanBaselineU ? ((this.cleanBaselineU - U) / this.cleanBaselineU) * 100 : null;
    ...
    const pfailBase = foulingDetected
      ? 0.6
      : foulingDropPct
        ? Math.min(0.4, (foulingDropPct / 100) * 0.5)
        : 0.05;
    ...
    const now = Date.now();
    const rotationDue = this.lastRotationAt
      ? (now - this.lastRotationAt) / (1000 * 3600) >= rotationIntervalHours
      : true;
  ...
  public getConfidenceScore(samples: CoolingSample[] = [], vibrationSeries: number[] = []): number {
    ...
    const Useries: number[] = samples
      .map(s => this.estimateU(s) || 0)
      .filter(v => typeof v === 'number');
    const vib =
      vibrationSeries && vibrationSeries.length >= Useries.length
        ? vibrationSeries.slice(-Useries.length)
        : samples.map(s => s.pumpVibration || 0);
    ...
    if (lastU && this.cleanBaselineU && (this.cleanBaselineU - lastU) / this.cleanBaselineU > 0.25)
      score -= 20;
