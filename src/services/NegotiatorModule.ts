...
export class NegotiatorModule {
  ...
  public static generateOffer(
    ...
    console.log(
      `Delivery: ${new Date(deliveryStart).toISOString()} - ${new Date(deliveryEnd).toISOString()}`
    );
  ...
  public static broadcastOffer(offer: PPAOffer, counterparties: string[]): void {
    console.log(
      `[Negotiator] Broadcasting offer ${offer.offerId} to ${counterparties.length} counterparties`
    );
