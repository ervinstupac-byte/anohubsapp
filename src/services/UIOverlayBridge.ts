...
export class UIOverlayBridge {
  ...
  private static broadcastFrame(): void {
    ...
    if (frame.latency > this.TARGET_LATENCY_MS) {
      console.warn(
        `[HoloBridge] ⚠️ Latency warning: ${frame.latency.toFixed(1)}ms (target: <${this.TARGET_LATENCY_MS}ms)`
      );
    }
    ...
    for (const [clientId, ws] of this.subscribers.entries()) {
      try {
        ...
        if (frame.frameId % 100 === 0) {
          // Log every 100 frames (2 seconds at 50Hz)
          console.log(
            `[HoloBridge] Streaming to ${clientId}: Frame ${frame.frameId}, Latency ${frame.latency.toFixed(1)}ms`
          );
        }
      } catch (error) {
