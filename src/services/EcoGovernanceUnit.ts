...
export class EcoGovernanceUnit {
  ...
  public static monitorEnvironmentalFlow(
    ...
    if (!compliance) {
      console.log(
        `[EcoGov] ⚠️ E-FLOW VIOLATION: Required ${requiredEflow.toFixed(1)} m³/s, Actual ${actualFlow.toFixed(1)} m³/s`
      );
    }
  ...
  public static calculateBiologicalCompliance(): BiologicalCompliance {
    ...
    const eflowCompliance = eflowData.length > 0 ? (eflowCompliant / eflowData.length) * 100 : 100;
    ...
    const doCompliance = doData.length > 0 ? (doCompliant / doData.length) * 100 : 100;
    ...
    const lastViolation = violations.length > 0 ? Math.max(...violations.map(v => v.timestamp)) : 0;
  ...
  public static getEnvironmentalStatus(): {
    ...
    const currentEflow =
      this.eflowHistory.length > 0 ? this.eflowHistory[this.eflowHistory.length - 1] : null;

    const currentQuality =
      this.waterQualityHistory.length > 0
