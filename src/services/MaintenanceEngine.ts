import { DiagnosisReport } from '../core/TechnicalSchema';
...
const DETAILED_KNOWLEDGE_BASE: Record<
  string,
  { steps: ActionStep[]; priority: 'HIGH' | 'MEDIUM' | 'LOW' }
> = {
  VIBRATION_CRITICAL: {
    priority: 'HIGH',
    steps: [
      {
        step: 1,
        description: 'EMERGENCY: Assess coupling bolts for shear indicators.',
        critical: true,
      },
      {
        step: 2,
        description: 'Check foundation anchor torque (Target: 1200Nm for M36).',
        critical: true,
      },
      { step: 3, description: 'Execute laser alignment audit on shaft coupling.' },
      {
        step: 4,
        description:
          'Inspect runner for heavy pitting (Forensic: Check for metallic dust in draft tube).',
      },
    ],
  },
  BEARING_TEMP_CRITICAL: {
    ...
    steps: [
      {
        step: 1,
        description: 'Verify oil level and foaming status (Potential water ingress).',
        critical: true,
      },
  ...
  STRUCTURAL_RISK_HIGH: {
    ...
    steps: [
      {
        step: 1,
        description: 'MANDATORY: Reduce load/pressure to 50% immediately.',
        critical: true,
      },
  ...
  CAVITATION_DANGER: {
    ...
    steps: [
      ...
      {
        step: 2,
        description: 'Operate unit outside of critical flow zones (42.5m3/s range).',
        critical: true,
      },
