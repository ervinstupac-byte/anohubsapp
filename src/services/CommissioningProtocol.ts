...
export const COMMISSIONING_CHECKLIST: CommissioningCheckItem[] = [
  ...
  },
  {
    ...
    acceptanceCriteria:
      'Valve opens freely, no blockage. CRITICAL: Clogged valve = cavitation returns in 1 month!',
...
export class VibrationFingerprintChecker {
  ...
  static checkPostRepairVibration(
    ...
    if (vibrationIncrease > 0.5) {
      issues.push(
        `Vibration increased by ${vibrationIncrease.toFixed(2)} mm/s (tolerance: 0.5 mm/s)`
      );
    ...
    if (postRepair.measurements.oneX > 2.5) {
      issues.push(
        `1x RPM vibration ${postRepair.measurements.oneX.toFixed(1)} mm/s exceeds 2.5 mm/s limit`
      );
...
export class WarmUpMonitor {
  ...
  checkWarmUpStatus(): {
    ...
    const tempRiseRate =
      (latest.bearingTemp - previous.bearingTemp) /
  ...
  getExpectedTemp(elapsedMinutes: number, load: number): number {
    ...
    return (
      ambientTemp + (steadyStateTemp - ambientTemp) * (1 - Math.exp(-elapsedMinutes / timeConstant))
    );
  }
}
