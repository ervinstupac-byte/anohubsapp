...
import { getLastAutoTableFinalY, setGStateOpacity, getPageCount } from '../utils/pdfHelpers';
...
export class ForensicReportService {
  ...
  public generateForensicSnapshotReport(snapshot: DiagnosticSnapshot): Blob {
    ...
    if (snapshot.physicsAnalysis) {
      ...
      if (snapshot.energyHarvest || snapshot.lifeExtension) {
        ...
        let text = '';
        if (snapshot.energyHarvest)
          text += `HARVEST: ${snapshot.energyHarvest.powerW.toFixed(0)}W (€${snapshot.energyHarvest.annualEur.toFixed(0)}/yr)  `;
        if (snapshot.lifeExtension)
          text += `LIFE EXTENSION: +${snapshot.lifeExtension.yearsAdded.toFixed(1)} Years`;
      ...
      if (
        snapshot.transformerAnalysis ||
        snapshot.statorAnalysis ||
        snapshot.shaftSealAnalysis ||
        snapshot.governorAnalysis
      ) {
  ...
  private drawPolarPlot(
    kinetic: { eccentricity: number; phase: number },
    x: number,
    y: number,
    size: number
  ) {
  ...
  private applyProfessionalHeader(title: string) {
    ...
    doc.text(
      `REF_ID: ${Math.random().toString(36).substring(7).toUpperCase()} | VER: ${PLATFORM_VERSION} | TS: ${timestamp}`,
      14,
      45
    );
  ...
  private drawCertifiedSeal(x: number, y: number) {
    ...
    doc.setFontSize(6);
    doc.text(CERTIFICATION_SEAL.split(' ')[0] + ' ' + CERTIFICATION_SEAL.split(' ')[1], x, y, {
      align: 'center',
    });
  ...
  public generateTechnicalReport(data: TechnicalReportData): Blob {
    ...
    // Calculations Table
    const lastY = getLastAutoTableFinalY(this.doc) + 15;
    ...
    // Recommendation Box
    const finalY = getLastAutoTableFinalY(this.doc) + 20;
  ...
  public generateFinancialProspectus(data: FinancialReportData): Blob {
    ...
    autoTable(this.doc, {
      ...
      body: [
        [
          ...
          `€${SOVEREIGN_ECONOMICS.MARKET_PRICE_MWH} / MWh`,
          'EconomicsCore (NC-13000)',
        ],
        [
          'Target Efficiency',
          `${(SOVEREIGN_ECONOMICS.TARGET_EFFICIENCY * 100).toFixed(1)}%`,
          'Sovereign Standard',
        ],
    ...
    const disclaimer =
      'Note: These financial projections are generated by the AnoHUB Predictive Engine based on current technical parameters and market volatility factors. Final feasibility studies required.';
  ...
  public generateIncidentReport(data: IncidentReportData): Blob {
    ...
    doc.setTextColor(148, 163, 184);
    doc.text(
      'Automated high-pressure bypass initiated. Manual inspection mandated within 4h.',
      14,
      finalY + 8
    );
  ...
  public generatePurchaseOrder(data: PurchaseOrderData): Blob {
    ...
    const totalRow = [
      {
        content: 'TOTAL PROJECTED COST',
        colSpan: 4,
        styles: { halign: 'right', fontStyle: 'bold' } as any,
      },
      {
        content: `€${data.parts.reduce((total, p) => total + p.quantity * p.unitPrice, 0).toFixed(2)}`,
  ...
  public static openAndDownloadBlob(
    blob: Blob,
    filename: string,
    _openPreview: boolean = true,
    _options: any = {}
  ) {
  ...
  public generateGenericProtocolPDF(data: any): Blob {
    ...
    if (metrics && metrics.length > 0) {
      autoTable(doc, {
        ...
        body: metrics.map((m: any) => [m.label || '', m.value || '', m.unit || '-']),
        theme: 'striped',
  ...
  public generateProjectPDF(state: TechnicalProjectState): Blob {
    ...
    autoTable(doc, {
      ...
      body: [
        [
          ...
          `${(currentEff * 100).toFixed(1)}% `,
          effGap > 0 ? ` - ${(effGap * 100).toFixed(1)}% vs Target` : 'OPTIMAL',
        ],
        ['Active Power Output', `${powerMW.toFixed(2)} MW`, '-'],
        [
          'Revenue Efficiency Gap',
          effGap > 0 ? 'NEEDS OPTIMIZATION' : 'OPTIMIZED',
          effGap > 0
    ...
    const prediction = calculateMaintenancePrediction({
      config: {
        id: idAdapter.toStorage(state.identity.assetId),
        name: state.identity.assetName,
      ...
      telemetry: {
        ...
        cavitationIndex: state.mechanical.acousticMetrics?.cavitationIntensity
          ? state.mechanical.acousticMetrics.cavitationIntensity / 100
          : 0,
      },
    });
    ...
    autoTable(doc, {
      ...
      body: [
        [
          'Degradation Factor',
          `${prediction.degradationFactor}x Acceleration`,
          prediction.degradationFactor > 1.2 ? 'ACCELERATED WEAR' : 'NOMINAL',
        ],
        [
          'Est. Remaining Life (ERL)',
          `~${prediction.remainingLifeHours.toLocaleString()} Hours`,
          `${prediction.urgency} STATUS`,
        ],
        [
          'Forecasted Failure Event',
          new Date(prediction.predictedFailureDate).toLocaleDateString(),
          prediction.primaryStressor,
        ],
      ],
      theme: 'grid',
    ...
    currentY = Math.max(getLastAutoTableFinalY(doc), 115) + 15;

    // Middle Section: Combined Pressure & Stress (Full Width)
    doc.setTextColor(6, 182, 212);
    doc.text(
      i18n.t(
        'report.sections.pressureStress',
        'VI. PRESSURE & STRESS FUSION (IEC 60041 Compliant)'
      ),
      14,
      currentY
    );
    ...
    doc.text(
      i18n.t('report.sections.mechanical', 'VII. MECHANICAL & ORBIT INTEGRITY'),
      14,
      currentY
    );
    currentY += 5;
    ...
    autoTable(doc, {
      ...
      body: [
        [
          ...
          state.physics.eccentricity > 0.8
            ? 'CRITICAL'
            : state.physics.eccentricity > 0.7
              ? 'WARNING'
              : 'NOMINAL',
        ],
        ...
        [
          ...
          '-',
          '-',
          state.mechanical.bearingTemp > 70 ? 'CRITICAL' : 'OPTIMAL',
        ],
      ],
      theme: 'grid',
    ...
    currentY = getLastAutoTableFinalY(doc) + 20;
    ...
    doc.setFont('helvetica', 'bold');
    doc.text(
      i18n.t('report.sections.diagnostic', 'VIII. DIAGNOSTIC SUMMARY & AI INSIGHTS'),
      20,
      currentY + 10
    );
    ...
    const diagnosisMessages =
      state.diagnosis?.messages.map((m: unknown) => (m as any)[currentLang]).join(' ') ||
      (state.riskScore > 50
        ? currentLang === 'bs'
  ...
  public generateModulePDF(moduleName: string, state: TechnicalProjectState): Blob {
    ...
    if (moduleName.toLowerCase().includes('mechan')) {
      ...
      doc.text(
        'No specific module sensors mapped for high-fidelity report in this version.',
        14,
        80
      );
    }
