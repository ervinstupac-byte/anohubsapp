...
export class GlobalHealthDashboard {
  ...
  assessStationHealth(systems: SystemHealth[]): GlobalHealthMap {
    ...
    const degradedSystems = sortedSystems.filter(
      s => s.healthStatus === 'WARNING' || s.healthStatus === 'ALARM'
    ).length;
    const criticalSystems = sortedSystems.filter(s => s.healthStatus === 'CRITICAL').length;

    // Determine overall health
    const overallHealth =
      criticalSystems > 0 ? 'CRITICAL' : degradedSystems > 0 ? 'DEGRADED' : 'HEALTHY';

    // Count issues
    const totalIssues = sortedSystems.reduce((sum, s) => sum + s.activeIssues.length, 0);
    const criticalIssues = sortedSystems.reduce(
      (sum, s) => sum + s.activeIssues.filter(i => i.severity === 'CRITICAL').length,
      0
    );
  ...
  calculatePriorityBreakdown(system: SystemHealth): SystemHealth['priorityBreakdown'] {
    ...
    const totalSafetyImpact = system.activeIssues.reduce(
      (sum, issue) => sum + issue.safetyImpact,
      0
    );
    const totalProductionImpact = system.activeIssues.reduce(
      (sum, issue) => sum + issue.productionImpact,
      0
    );
  ...
  applyTieBreaker(systemA: SystemHealth, systemB: SystemHealth): TieBreakerResult {
    ...
    if (systemA.systemType === 'TRANSFORMER' || systemA.systemType === 'GENERATOR') {
      const hasElectricalFault = systemA.activeIssues.some(
        i =>
    ...
    if (systemB.systemType === 'TRANSFORMER' || systemB.systemType === 'GENERATOR') {
      const hasElectricalFault = systemB.activeIssues.some(
        i =>
          i.description.includes('overcurrent') ||
          i.description.includes('ground fault') ||
          i.description.includes('field resistance')
    ...
    if (systemA.priorityBreakdown.productionImpact !== systemB.priorityBreakdown.productionImpact) {
      const winner =
        systemA.priorityBreakdown.productionImpact > systemB.priorityBreakdown.productionImpact
          ? systemA
          : systemB;
  ...
  private calculateDegradationRate(system: SystemHealth): number {
    ...
    const avgTime =
      system.activeIssues.reduce((sum, i) => sum + i.timeSinceDetection, 0) /
      system.activeIssues.length;
  ...
  createSystemHealth(
    ...
    // Determine urgency
    const urgency = hasCritical ? 'CRITICAL' : hasAlarm ? 'HIGH' : hasWarning ? 'MEDIUM' : 'LOW';

    // Generate recommended action
    const recommendedAction =
      issues.length > 0 ? `Address ${issues[0].description}` : 'Continue normal operation';
