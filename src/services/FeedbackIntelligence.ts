...
export class FeedbackIntelligence {
   ...
   */
  public static async getLearningModifiers(
    actionType: string,
    simulatedHistory?: FeedbackItem[]
    ...
    const relevantVetoes = history.filter(
      h =>
    ...
    if (vetoCount > 0) {
      ...
      // Simple mode
      commonReason = reasons
        .sort((a, b) => reasons.filter(v => v === a).length - reasons.filter(v => v === b).length)
        .pop();
    }
