...
import {
  computeBearingOilCooling,
  BearingOilCoolingInput,
  BearingOilCoolingResult,
} from './BearingOilCoolingSystem';
...
export class ThermalManagementCore extends BaseGuardian {
  ...
  public getConfidenceScore(samples: any[] = [], vibrationSeries: number[] = []): number {
    ...
    const Useries = samples
      .map(s => {
        try {
          return (this.coolingGuardian as any).estimateU
            ? (this.coolingGuardian as any).estimateU(s)
            : 0;
        } catch {
          return 0;
        }
