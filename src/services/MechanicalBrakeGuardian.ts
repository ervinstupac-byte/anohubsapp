  ...
  generateSafeStopSequence(currentRpmPct: number, targetRpmPct: number = 0): BrakeStep[] {
    ...
    if (start > this.safeThresholdPct) {
      ...
      for (let i = 0; i < fastSteps; i++) {
        ...
        seq.push({
          targetRpmPct: stepLower,
          brakeTorquePct: Math.min(60, 20 + i * 10),
          durationSec: 6,
        });
      }
    }
    ...
    for (let i = 0; i < gentleSteps; i++) {
      ...
      seq.push({
        targetRpmPct: Math.max(0, Number(t.toFixed(2))),
        brakeTorquePct: torque,
        durationSec: 8,
      });
    }
    ...
    try {
      ...
      if (totalDuration < safeClose * 0.5) {
        ...
        seq.forEach(step => {
          step.durationSec = Math.ceil((step.durationSec || 1) * scale);
        });
      }
    } catch (e) {
