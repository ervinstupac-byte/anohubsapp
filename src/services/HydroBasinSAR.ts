...
export class HydroBasinSAR {
   ...
   */
  public static async querySARData(basinBounds: {
    north: number;
    south: number;
    east: number;
    west: number;
  ...
  public static calculateLagAdjustment(
    ...
    console.log(
      `  Moisture adjustment: ${moistureAdjustment > 0 ? '+' : ''}${moistureAdjustment} hours`
    );
  ...
  public static async updateHydroForecast(
    ...
    const expectedInflow =
      ((rainfallMM / 1000) * (basinArea * 1e6) * runoffCoefficient) /
      (lagAdjustment.adjustedLag * 3600);
