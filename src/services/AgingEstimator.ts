...
export function computeEtaFromSample(s: any): Decimal {
  try {
    ...
    const Q =
      (s.francis_data && (s.francis_data.flow || s.francis_data.flowRate)) ?? s.flow ?? s.Q ?? 0;
    const H =
      (s.francis_data && (s.francis_data.head || s.francis_data.GrossHead)) ?? s.H ?? s.head ?? 0;
  ...
  try {
    if (variant) {
      const { data: vdata } = await supabase
        .from('expert_efficiency_curves')
        .select('id, curve_json, created_at')
        .eq('turbine_variant', variant)
...
export function matchOptimalEta(
  telemetryWindow: any[],
  expertCurve: { points: Array<{ q: number; eta: number }> } | null
) {
  if (!expertCurve || !Array.isArray(expertCurve.points) || expertCurve.points.length === 0)
    return { meanDevPct: null, rmsDevPct: null, mappingCount: 0 };
  ...
  for (const s of telemetryWindow) {
    ...
    const expectedEta =
      q <= points[0].q
        ? points[0].eta
        : q >= points[points.length - 1].q
          ? points[points.length - 1].eta
          : linearInterpolate(p0.q, p0.eta, p1.q, p1.eta, q);
...
export function evaluateAging(
  telemetryWindow: any[],
  designEta?: number,
  context?: { design_flow_cms?: number; gross_head_m?: number },
  expertCurve?: { points: Array<{ q: number; eta: number }> } | null
): AgingResult {
  ...
  if (valid.length === 0)
    return {
      agingScore: 0,
      warningLevel: 'OK',
      explanation: 'Insufficient valid efficiency samples',
    };
  ...
  try {
    ...
    if (flows.length > 0 && context?.design_flow_cms) {
      ...
      if (ratio < 0.6)
        reasons.push(
          'Operating at part-load (flow < 60% of design) — efficiency and cavitation risk increase.'
        );
      else if (ratio < 0.9)
        reasons.push('Operating below design flow (partial loading) — efficiency reduced.');
  ...
  try {
    const vibSamples = telemetryWindow
      .map(
        s =>
  ...
  explanationParts.push(
    `Mean η = ${meanEta.times(100).toFixed(2)}%. Deviation: ${dev.toFixed(2)}%.`
  );
  if (curveInfo)
    explanationParts.push(`Deviation computed against expert curve (n=${curveInfo.mappingCount}).`);
  if (reasons.length) explanationParts.push(...reasons);
  if (agingScore >= 60)
    explanationParts.push(
      'Action: Consider load redistribution or planned outage to inspect for cavitation and runner damage.'
    );
  else if (agingScore >= 30)
    explanationParts.push('Action: Monitor closely and schedule inspection if trend continues.');

  const explanation = explanationParts.join(' ');
  const fiftyYearGuidance =
    'Sustained off-design operation, cavitation, and elevated vibration materially reduce remaining life; corrective action and inspections extend life toward 50+ years.';
...
export function computeRevenueLoss(
  actualEta: number | string | Decimal,
  optimalEta: number | string | Decimal,
  powerKw: number,
  pricePerKwh: number
) {
  try {
