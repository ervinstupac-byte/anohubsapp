...
export class OilRegenerationLogic {
  ...
  public static monitorOilQuality(
    ...
    if (
      spectrometry.viscosity < this.THRESHOLDS.viscosity.min ||
    ...
    if (spectrometry.acidNumber > this.THRESHOLDS.acidNumber) {
      reasons.push(
        `Acid number high: ${spectrometry.acidNumber} mg KOH/g (max: ${this.THRESHOLDS.acidNumber})`
      );
    }

    if (spectrometry.waterContent > this.THRESHOLDS.waterContent) {
      reasons.push(
        `Water contamination: ${spectrometry.waterContent} ppm (max: ${this.THRESHOLDS.waterContent})`
      );
    }

    if (spectrometry.particleCount > this.THRESHOLDS.particleCount) {
      reasons.push(
        `Particle contamination: ISO ${spectrometry.particleCount} (max: ${this.THRESHOLDS.particleCount})`
      );
    }

    if (spectrometry.oxidation > this.THRESHOLDS.oxidation) {
      reasons.push(
        `Oxidation level: ${spectrometry.oxidation} Abs/cm (max: ${this.THRESHOLDS.oxidation})`
      );
    }

    if (spectrometry.antioxidantDepletion > this.THRESHOLDS.antioxidantDepletion) {
      reasons.push(
        `Antioxidant depleted: ${spectrometry.antioxidantDepletion}% (max: ${this.THRESHOLDS.antioxidantDepletion}%)`
      );
    }
  ...
  public static triggerRegeneration(
    ...
    if (beforeQuality.oxidation > 0.1 || beforeQuality.acidNumber > 0.3) {
  ...
  private static completeRegeneration(cycle: RegenerationCycle): void {
    ...
    console.log('\nQUALITY IMPROVEMENT:');
    console.log(
      `  Viscosity: ${cycle.beforeQuality.viscosity} → ${cycle.afterQuality.viscosity} cSt`
    );
    console.log(
      `  Acid Number: ${cycle.beforeQuality.acidNumber} → ${cycle.afterQuality.acidNumber} mg KOH/g`
    );
    console.log(
      `  Water: ${cycle.beforeQuality.waterContent} → ${cycle.afterQuality.waterContent} ppm`
    );
    console.log(
      `  Oxidation: ${cycle.beforeQuality.oxidation} → ${cycle.afterQuality.oxidation} Abs/cm`
    );
