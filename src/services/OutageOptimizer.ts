...
export default class OutageOptimizer {
   ...
   */
  public static findOptimalOutageWindow(
    pfail: PfailMap,
    ...
    const mediumPriority = Object.keys(pfail).filter(
      k => (pfail[k] || 0) >= 0.2 && (pfail[k] || 0) <= 0.5
    );

    const bundles: BundleRecommendation[] = [];
    if (highPriority.length) {
      bundles.push({
        components: highPriority.concat(mediumPriority),
        reason: 'High-priority failure(s) detected — bundle medium-priority maintenance.',
      });
    }
