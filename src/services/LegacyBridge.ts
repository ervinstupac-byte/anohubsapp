...
export class LegacyBridge {
  ...
  generateGapAnalysis(legacyLimits: Record<string, number>): string {
    ...
    if (oldDeflector > masterDeflector) {
      report.push(
        `| Deflector Response | ${oldDeflector}s | **${masterDeflector}s** | 🚨 **DANGEROUS**: Old logic too slow for water hammer protection. |`
      );
    }
    ...
    if (oldJetOffset > masterJetOffset) {
      report.push(
        `| Jet Offset Tolerance | ${oldJetOffset}mm | **${masterJetOffset}mm** | ⚠️ **WEAR RISK**: Buckets loading unevenly. |`
      );
    }
    ...
    report.push(
      `| Vibration Logic | Overall RMS (${oldVibe}mm/s) | **FFT Specific (>2mm/s @ BPF)** | ℹ️ **BLINDSPOT**: Old system ignores hydraulic resonance. |`
    );
    ...
    if (oldFilter > masterFilter) {
      report.push(
        `| Cooling Filter dP | ${oldFilter} bar | **${masterFilter} bar** | ⚠️ **STARVATION RISK**: Filter cleans too late. |`
      );
    }
