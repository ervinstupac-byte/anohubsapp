...
// Lightweight typings for jspdf-autotable usage in this file
type AutoTableCell = { content: string; colSpan?: number; styles?: Record<string, unknown> };
type AutoTableRow = Array<string | number | AutoTableCell>;
...
export class ReportGenerator {
  ...
  private getDoc(): JsPDFWithAutoTable {
    return this.doc as unknown as JsPDFWithAutoTable;
  }

  private getLastAutoTableFinalY(defaultVal = 0): number {
    const last = this.getDoc().lastAutoTable;
    return last && typeof last.finalY === 'number' ? last.finalY : defaultVal;
  }

  private getPageCount(): number {
    return this.getDoc().internal?.getNumberOfPages?.() ?? 1;
  }
  ...
  private applyFooter() {
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.text(`AnoHUB Precision Systems © 2025 | Page ${i} of ${pageCount}`, 105, 285, {
        align: 'center',
      });
  ...
  public generateTechnicalReport(data: TechnicalReportData): Blob {
    ...
    // Calculations Table
    const lastY = this.getLastAutoTableFinalY() + 15;
    ...
    // Recommendation Box
    const finalY = this.getLastAutoTableFinalY() + 20;
  ...
  public generatePurchaseOrder(data: PurchaseOrderData): Blob {
    ...
    const tableBody: AutoTableRow[] = data.parts.map(p => [
      p.name,
      p.partNumber,
      p.quantity,
  ...
  public generateTechnicalAuditReport(data: TechnicalAuditData): Blob {
    ...
    currentY = this.getLastAutoTableFinalY() + 15;
