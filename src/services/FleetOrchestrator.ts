...
export class FleetOrchestrator {
  ...
  public static executeLoadShift(plan: LoadShiftPlan): void {
    ...
    for (const comp of plan.compensations) {
      console.log(
        `[Fleet]   → ${comp.assetId}: +${comp.additionalMW.toFixed(1)} MW (H_eff: ${comp.projectedHeff.toFixed(2)})`
      );
  ...
  public static getFleetCapacity(assets: FleetAsset[]): {
    ...
    const utilized = assets.reduce(
      (sum, a) => (a.state === 'OPERATIONAL' ? sum + a.currentLoad : sum),
      0
    );
