...
export class SovereignOrchestrator {
  ...
  private static wireROISync(): void {
    ...
    ROIMonitorService.recordMarketOptimization = (gain: number, action: string) => {
      ...
      ValueCompounder.recordValue('MARKET_GAIN', gain, action);
    };
    ...
    ROIMonitorService.recordProductionImpact = (loss: number, duration: number) => {
      ...
      ValueCompounder.recordValue('HEALING_COST', -loss, `Production impact (${duration}min)`);
    };
  }
  ...
  private static wireKernelObservers(): void {
    ...
    SovereignKernel.subscribe((telemetry: EnrichedTelemetry) => {
      try {
        const vib =
          telemetry && telemetry.mechanical && typeof telemetry.mechanical.vibration === 'number'
            ? telemetry.mechanical.vibration
            : 0;
        const now = Date.now();
  ...
  private static scheduleDailyReports(): void {
    ...
    this.dailyReportSchedule = setInterval(
      async () => {
