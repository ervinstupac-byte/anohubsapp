...
export class ComputerVisionService {
  ...
  public static processFrame(frame: VideoFrame): DetectionResult {
    ...
    if (critical.length > 0) {
      ...
      critical.forEach(d => {
        console.log(
          `  - ${d.type}: ${d.description} (confidence: ${(d.confidence * 100).toFixed(0)}%)`
        );
      });
    }
  ...
  public static getStatistics(): {
    ...
    const avgProcessingTime =
      this.detectionHistory.length > 0
        ? this.detectionHistory.reduce((sum, r) => sum + r.processingTimeMs, 0) /
          this.detectionHistory.length
        : 0;

    const totalDetections = this.detectionHistory.reduce((sum, r) => sum + r.detections.length, 0);

    return {
      framesProcessed: this.detectionHistory.length,
      avgProcessingTime,
      detectionRate:
        this.detectionHistory.length > 0 ? totalDetections / this.detectionHistory.length : 0,
    };
  }
}
