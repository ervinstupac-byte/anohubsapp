...
export const LiveMathSync = {
  calculateSystemHealth: (site: SiteParameters, tech: TechnicalProjectState): SystemHealth => {
    ...
    const headLoss = PhysicsLogic.calculateHeadLoss(
      f,
      new Decimal(penstock.length),
      diameterM,
      velocity
    );
    ...
    // 3. Power Output
    // P = rho * g * H * Q * eta
    const powerOutputKW = WATER_DENSITY.mul(GRAVITY)
      .mul(netHead)
      .mul(designFlow)
      .mul(efficiency)
      .div(1000);
    ...
    switch (material) {
      case 'STEEL':
        waveSpeed = new Decimal(1200);
        break;
      case 'GRP':
        waveSpeed = new Decimal(400);
        break;
      case 'PEHD':
        waveSpeed = new Decimal(300);
        break;
      case 'CONCRETE':
        waveSpeed = new Decimal(900);
        break;
    }
    ...
    const roiYears = estimatedRevenue.gt(0)
      ? estimatedCapEx.div(estimatedRevenue)
      : new Decimal(99);

    // 7. Analysis
    const cavitationRisk = netHead.lt(10) && velocity.gt(5);
    const resonanceRisk =
      new Decimal(penstock.length).div(waveSpeed).gt(0.5) &&
      new Decimal(penstock.length).div(waveSpeed).lt(1.5);
