...
export class TrashRackRobot {
  ...
  public static async checkCleaningTrigger(
    ...
    if (trashRackDP < this.DP_THRESHOLD) {
      return {
        shouldClean: false,
        reason: `ΔP below threshold (${trashRackDP.toFixed(3)} < ${this.DP_THRESHOLD})`,
      };
    }
  ...
  private static runCleaningSequence(cycle: CleaningCycle): void {
    ...
    setTimeout(
      ...
      this.CLEANING_DURATION_MIN * 60 * 1000
    ); // Convert to ms
  }
  ...
  private static completeCycle(cycle: CleaningCycle): void {
    ...
    console.log(
      `Improvement: ${(cycle.dpBefore - cycle.dpAfter).toFixed(3)} bar (${((improvement / cycle.dpBefore) * 100).toFixed(1)}%)`
    );
    ...
    const opportunities = priceForecast
      ...
      .map(forecast => {
        const energyConsumption =
          (this.ENERGY_CONSUMPTION_KW * (this.CLEANING_DURATION_MIN / 60)) / 1000; // MWh
  ...
  public static getStatistics(): {
    ...
    const avgCycleDuration =
      completed.length > 0
        ? completed.reduce((sum, c) => sum + ((c.endTime || 0) - c.startTime), 0) /
          completed.length /
          (60 * 1000) // minutes
        : 0;

    const lastCleaning = completed.length > 0 ? Math.max(...completed.map(c => c.startTime)) : 0;
