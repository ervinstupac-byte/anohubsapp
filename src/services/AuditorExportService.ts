...
export class AuditorExportService {
  ...
  constructor(scadaHistory: SCADASnapshot[], journal: ServiceLogEntry[]) {
  ...
  generateComplianceReport(daysToLookBack: number = 30): ComplianceReport {
    ...
    const periodHistory = this.scadaHistory.filter(
      snapshot => snapshot.timestamp >= startDate && snapshot.timestamp <= now
    );
