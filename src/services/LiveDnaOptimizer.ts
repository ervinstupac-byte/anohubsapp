...
export class LiveDnaOptimizer {
  ...
  private static performBayesianUpdate(assetId: string): void {
    ...
    console.log(
      `[LiveDNA] ✅ Updated Hill Chart for ${assetId} with ${adaptedPoints.length} points`
    );

    // Calculate improvement
    const avgDelta = data.slice(-100).reduce((sum, d) => sum + Math.abs(d.delta), 0) / 100;
    console.log(
      `[LiveDNA] Avg prediction error: ${avgDelta.toFixed(2)}% (vs manufacturer defaults)`
    );
  }
  ...
  public static getAdaptedEfficiency(
    ...
    for (const point of chart.dataPoints) {
      const dist = Math.sqrt(Math.pow(point.head - head, 2) + Math.pow(point.flow - flow, 2));
      if (dist < minDist) {
        minDist = dist;
        nearest = point;
      }
