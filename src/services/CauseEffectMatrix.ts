...
export type ShutdownLevel =
  | 'LEVEL_0_INFO'
  | 'LEVEL_1_ALARM'
  | 'LEVEL_2_NORMAL_STOP'
  | 'LEVEL_3_EMERGENCY_TRIP';
...
export class CauseEffectMatrix {
  ...
  private static getLevelDescription(level: ShutdownLevel): string {
    switch (level) {
      case 'LEVEL_3_EMERGENCY_TRIP':
        return 'FAST CLOSE + BREAKER OPEN';
      case 'LEVEL_2_NORMAL_STOP':
        return 'UNLOAD > BREAKER OPEN > STOP';
      case 'LEVEL_1_ALARM':
        return 'ANNUNCIATOR ONLY';
      default:
        return 'LOG ONLY';
    }
  }
