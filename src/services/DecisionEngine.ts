...
export class DecisionEngine extends BaseGuardian {
  ...
  static evaluateTriage(inputs: DiagnosticInputs): TriageDecision {
    ...
    if (
      ...
      severityScore += 100;
      factors.push(
        'Kombinacija: Kritična kavitacija + prekoračene vibracije + metalne čestice u ulju'
      );
    ...
    if (hydraulicInstability.isUnstable) {
      ...
      return {
        ...
        actionPlan: {
          step1_immediate:
            'HITNO: Blokiraj dalje komande pozicioniranja. Zaustavi turbinu kontrolisano.',
          step2_field:
            'Provjeriti servo ventile i pozicionere. Mjeri stvarnu poziciju lopatica mehaničkim mjerilom.',
          step3_longterm:
            'Zamjena servo sistema. Nadogradnja na veće cijevi (16mm ili 20mm) za stabilniji odziv.',
        },
    ...
    if (inputs.oil.babbittContent > 50) {
      severityScore += 80;
      factors.push(
        `Kritičan nivo Babbitt metala (${inputs.oil.babbittContent} ppm) - ležaj se raspada`
      );
    ...
    if (inputs.performance.efficiencyIndex < 90 && inputs.oil.temperatureStable) {
      severityScore += 30;
      factors.push(
        'Smanjena efikasnost uz stabilnu temperaturu - sumnja na naslage/prljavo radno kolo'
      );
  ...
  private static checkSensorMalfunction(inputs: DiagnosticInputs): {
    ...
    if (
      ...
      return {
        ...
        message:
          '⚠️ Sumnja na kvar senzora vibracija - provjeriti ožičenje. Zvuk turbine (Sonic modul) normalan i struja generatora stabilna.',
      };
    }
