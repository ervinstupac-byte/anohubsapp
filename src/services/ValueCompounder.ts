...
export class ValueCompounder {
  ...
  public static async initialize(): Promise<void> {
    ...
    if (stored) {
      this.ledger = stored;
      console.log(
        `[ValueCompounder] 📚 Ledger loaded: €${stored.currentTotal.toLocaleString()} since Genesis`
      );
    } else {
  ...
  public static recordValue(
    ...
    console.log(
      `[ValueCompounder] 💰 €${amount.toLocaleString()} | Total: €${entry.runningTotal.toLocaleString()}`
    );
