...
export class WicketGateKinematics {
  ...
  addMeasurement(m: WicketMeasurement): WicketAction {
      ...
      if (this.lastActual !== null && this.lastCommand !== null) {
        ...
        if (this.lastForcePerDeg !== null) {
          ...
          if (factor >= this.shearPinFactorThreshold || invFactor >= this.shearPinFactorThreshold) {
            ...
            return {
              action: 'SHEAR_PIN_BROKEN',
              reason: `Force/deg changed abruptly by factor ${factor.toFixed(2)} (fpd=${fpd.toFixed(2)}).`,
            };
          }
        }
    ...
    const avgBacklash = this.backlashSamples.length
      ? this.backlashSamples.reduce((s, v) => s + v, 0) / this.backlashSamples.length
      : 0;
    if (avgBacklash >= this.backlashCriticalPct) {
      this.lastCommand = cmd;
      this.lastActual = actual;
      return {
        action: 'BACKLASH_CRITICAL',
        backlashPct: avgBacklash,
        reason: `Average backlash ${avgBacklash.toFixed(2)}% exceeds critical ${this.backlashCriticalPct}%`,
      };
    }
    ...
    if (avgBacklash >= this.backlashWarningPct) {
      ...
      this.lastActual = actual;
      return {
        action: 'BACKLASH_WARNING',
        backlashPct: avgBacklash,
        reason: `Average backlash ${avgBacklash.toFixed(2)}% exceeds warning ${this.backlashWarningPct}%`,
      };
    }
    ...
    if (typeof m.regulatingRingForceN === 'number') {
      ...
      if (entry && entry.count >= this.baselineSamplesRequired) {
        ...
        if (increasePct >= this.frictionIncreaseThresholdPct) {
          ...
          this.lastActual = actual;
          return {
            action: 'LUBRICATION_DEFICIENCY',
            increasePct: Number(increasePct.toFixed(2)),
            reason: `Force increased ${increasePct.toFixed(2)}% over baseline at ${bin}% bin.`,
          };
        }
      }
    }
  ...
  getHealthImpactForAction(a: WicketAction) {
    switch (a.action) {
      case 'SHEAR_PIN_BROKEN':
        return { overallDelta: -40, limitGateOpenPct: 30, details: a.reason };
      case 'BACKLASH_CRITICAL':
        return { overallDelta: -25, limitGateOpenPct: 50, details: a.reason };
      case 'BACKLASH_WARNING':
        return { overallDelta: -8, limitGateOpenPct: 80, details: a.reason };
      case 'LUBRICATION_DEFICIENCY':
        return { overallDelta: -15, limitGateOpenPct: 70, details: a.reason };
      default:
        return { overallDelta: 0, limitGateOpenPct: 100, details: a.reason || 'No impact' };
    }
  }
