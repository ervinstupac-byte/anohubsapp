...
export class InspectionAnomalyLibrary {
  ...
  public static getDefectDescription(type: DefectType): string {
    switch (type) {
      case 'CRACK_CONCRETE':
        return 'Structural fissure in concrete lining.';
      case 'SPALLING':
        return 'Surface flaking exposing aggregate/rebar.';
      case 'CORROSION_PITTING':
        return 'Localized chemical/oxidation attack on metal.';
      case 'FOREIGN_OBJECT':
        return 'Debris or obstruction in flow path.';
      default:
