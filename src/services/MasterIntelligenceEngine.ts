...
} from './SpecialMeasurementSyncService';
...
export class MasterIntelligenceEngine extends BaseGuardian {
  ...
  public static async analyzeAsset(asset: EnhancedAsset, history: CompleteSensorData[]) {
    try {
      ...
      const runningSpeed =
        latest.common.vibration > 0 ? asset.turbine_config.rated_speed || 500 : 0;
      ...
      const [aiResults, acousticResults, measurementResults, dynamicThresholds] = await Promise.all(
        [
          ...
          this.runSpecialMeasurements(
            asset,
            latest.specialized?.geodeticData,
            latest.specialized?.magneticData,
            {
      ...
      const diagnosis: UnifiedDiagnosis = {
        ...
        operatingZone: PerformanceGuardService.checkOperatingZone(
          ...
          152,
          42.5 // Hardcoded Design Targets for Francis demo
        ),
        ...
        isoJustification: this.getISOJustification(
          this.calculateVibrationZone(latest.common.vibration)
        ),
      ...
      if (
        diagnosis.trendProjections?.['vibration'] &&
        diagnosis.trendProjections['vibration'].daysUntilCritical <= 3
      ) {
        diagnosis.serviceNotes?.push({
          service: 'Predictive Intelligence',
          severity: 'CRITICAL',
          message:
            'PREDICTIVE_VIB_BREACH: Linear trend suggests ISO threshold breach within 72 hours.',
          recommendation:
            'Immediate load reduction or proactive bearing inspection required. [PROTOCOL_PdM_ACTIVE]',
          sourceFiles: [
            {
              filename: 'case-studies/cs-predictive-maintenance-roi/index.html',
              justification:
                'Linear extrapolation of vibration trends indicates categorical structural risk within the 72h window.',
            },
          ],
        });
      }
      ...
      try {
        ...
        const significant =
          forecast &&
          ((forecast.tStatistic && Math.abs(forecast.tStatistic) > 2) ||
            (forecast.confidence && forecast.confidence >= 0.95));
        // If confidence is low, recommend Data Collection Phase instead of maintenance
        const lowConfidence =
          forecast && ((forecast.confidence || 0) < 0.5 || (forecast.sampleCount || 0) < 720);
        // Critical maintenance if failure probability > 50% and breach is immediate (weeksUntil <= 1)
        const criticalPf =
          forecast &&
          (forecast.pf || 0) > 50 &&
          forecast.weeksUntil !== null &&
          forecast.weeksUntil <= 1;
        ...
        if (forecast && lowConfidence) {
          diagnosis.serviceNotes?.push({
            ...
            recommendation:
              'Initiate scheduled high-resolution telemetry ingestion and pause automated maintenance advisories until sufficient samples are collected.',
            sourceFiles: [
              {
                filename: 'services/AIPredictionService.ts',
                justification:
                  'Forecast confidence below operational threshold; escalate to data collection.',
              },
            ],
          });
        } else if (criticalPf) {
          diagnosis.serviceNotes?.push({
            ...
            recommendation:
              'Isolate affected unit, reduce load to minimum, and execute emergency inspection protocol. [AUTOMATED_CRITICAL] ',
            sourceFiles: [
              {
                filename: 'services/AIPredictionService.ts',
                justification:
                  'High Pf derived from residual process sigma; triggers critical alert.',
              },
              {
                filename: 'services/MasterIntelligenceEngine.ts',
                justification: 'Final decision engine: Pf > 50% and imminent breach.',
              },
            ],
          });
        } else if (
          ...
          diagnosis.serviceNotes?.push({
            ...
            recommendation:
              'Schedule preventive maintenance and detailed inspection within the advisory window. [AUTOMATED_ADVISORY]',
            sourceFiles: [
              {
                filename: 'services/AIPredictionService.ts',
                justification:
                  'Linear regression forecast on persisted telemetry (30-sample window).',
              },
            ],
          });
      ...
      if (latest.specialized?.acoustic?.harmonics) {
        ...
        if (h2x > 1.5 * h1x) {
          diagnosis.serviceNotes?.push({
            ...
            recommendation:
              'Precision laser alignment required. deviation suspect > 0.15mm/m. [PROTOCOL_IMS_VERIFIED]',
            sourceFiles: [
              {
                filename: 'case-studies/cs-francis-misalignment/index.html',
                justification:
                  'Dominant 2x RPM harmonic is a categorical indicator of geometric misalignment.',
              },
              {
                filename: 'insights/insight-harmonic-vib/index.html',
                justification:
                  'Cross-referenced against historical vibration patterns for similar Francis hub twins.',
              },
            ],
          });
        }
      ...
      const lastAlignment =
        asset.turbine_config.lastAlignmentDate || asset.turbine_config.commissioning_date;
      if (lastAlignment) {
        const daysSinceAlignment = Math.floor(
          (Date.now() - new Date(lastAlignment).getTime()) / (1000 * 60 * 60 * 24)
        );
        if (daysSinceAlignment > 180) {
          diagnosis.serviceNotes?.push({
            ...
            recommendation:
              'Mandatory laser alignment verification required per 180-day tropical rule. [PROTOCOL_IMS_VERIFIED]',
            sourceFiles: [
              {
                filename: 'Turbine_Friend/Francis_SOP_Precision/index.html',
                justification:
                  'Mandatory +/- 0.05mm run-out tolerance for Francis shafts requires 180-day verification.',
              },
              {
                filename: 'Turbine_Friend/Francis_SOP_Shaft_Alignment/index.html',
                justification:
                  'Laser alignment verification mandated every 180 days due to tropical foundation moisture absorption.',
              },
            ],
          });
        }
      ...
      if (asset.turbine_family === 'PELTON') {
        const peltonResults = PeltonJetSyncService.syncMultiNozzle(
          [],
          asset.turbine_config.runner_diameter || 1.2
        );
      ...
      const geoComparison = SpecialMeasurementSyncService.compareWithBlueprint(
        [],
        FRANCIS_IDEAL_BLUEPRINT
      );
      const effGap = SpecialMeasurementSyncService.calculateEfficiencyGap(
        geoComparison,
      ...
      try {
        const detected =
          TurbineClassifier.detectType(asset, latest) ||
          (asset.turbine_config as any)?.turbine_type ||
          (asset.turbine_config as any)?.turbineType ||
          null;
        const bladeCount =
          (latest.specialized as any)?.bladeCount ||
          (asset.turbine_config as any)?.blade_count ||
          (asset.turbine_config as any)?.runner_blades ||
          (asset.turbine_config as any)?.numberOfBlades ||
          4;
        ...
        if (detected) {
          ...
          if (detected === 'VERTICAL_KAPLAN' || detected === 'PROPELLER') {
            adjustedSigma = KaplanBladePhysics.adjustedSigmaLimit(
              consts.sigma_limit,
          ...
          const measuredSigma =
            (latest.specialized as any)?.sigmaMeasured || (latest.common as any)?.sigma || null;
          const proximity = measuredSigma && adjustedSigma ? measuredSigma / adjustedSigma : null;
          (diagnosis as any).turbineCavitation = {
            baseSigma: consts.sigma_limit,
            adjustedSigma,
            measuredSigma,
            proximity,
          };
          ...
          if (proximity !== null && proximity < 1.0) {
            diagnosis.serviceNotes?.push({
              ...
              recommendation:
                'Schedule detailed cavitation inspection and review blade count effects.',
            });
          }
        }
      } catch (e) {
      ...
      try {
        ...
        const components = [
          'shaftSealTelemetry',
          'thrustBearingTelemetry',
          'generatorAirGapTelemetry',
          'statorInsulationTelemetry',
          'governorHPUTelemetry',
          'transformerOilTelemetry',
          'coolingTelemetry',
        ];
        ...
        if (coolingSamples && coolingSamples.length) {
          try {
            ...
            const backupStarted =
              last && typeof last.backupPumpStarted === 'boolean' ? last.backupPumpStarted : null;
            if (analysis.foulingDetected && backupStarted === false) {
              diagnosis.automatedActions.push({
                type: 'IMMEDIATE',
                action: 'LOAD_REDUCTION',
                priority: 'CRITICAL',
                message: 'Cooling fouling detected and backup pump not started.',
              });
            }
          } catch (e) {
            pfail['cooling'] = 0.1;
        ...
        otherComponents.forEach(c => {
          ...
          if (arr && arr.length) {
            ...
            const p =
              (last && (last.p_fail || last.pFail || last.P_fail || last.pFailProbability)) || null;
            pfail[c.replace(/Telemetry$/, '')] =
              typeof p === 'number' ? Math.max(0, Math.min(1, p)) : 0.1;
          } else {
      ...
      try {
        ...
        if (diagnosis.serviceNotes && diagnosis.serviceNotes.length) {
          diagnosis.serviceNotes.forEach((s: any) =>
            findings.push({
              source: s.service,
              severity: s.severity,
              summary: s.message,
        ...
        if (diagnosis.automatedActions && diagnosis.automatedActions.length) {
          diagnosis.automatedActions.forEach((a: any) =>
            findings.push({
              source: 'AutomatedAction',
              severity: a.priority || a.status || 'INFO',
              summary: a.action || a.details || '',
              details: a.details,
            })
          );
        }

        const report = translator.generateWisdomReport(asset.id, findings, {
        ...
        const telemetryRef =
          latest && ((latest as any).id || (latest as any).timestamp)
            ? `telemetry:${(latest as any).id || (latest as any).timestamp}`
            : `telemetry_ts:${Date.now()}`;
        ...
        (diagnosis as any).wisdomReport = {
          id: persisted.id,
          generatedAt: annotated.generatedAt,
          systemConfidence: annotated.systemConfidence,
        };
    ...
    if (latest.specialized?.acoustic?.cavitationLevel || latest.specialized?.erosionPoints) {
      ...
      if (unitType === 'PELTON' && intakeTurbidity > 50) {
        results.cavitation = {
          ...
          justification:
            'As per AnoHUB Article-13: Pelton units are highly vulnerable to abrasion from solid particles like sand and silt.',
        };
      } else {
  ...
  private static async runAIPrediction(asset: EnhancedAsset, history: CompleteSensorData[]) {
    ...
    const incidentPatterns = aiPredictionService.matchHistoricalPattern(
      numericId,
      simulatedTelemetry
    );
  ...
  private static async runAcousticAnalysis(
    asset: EnhancedAsset,
    acousticData: any,
    runningSpeed: number
  ) {
  ...
  private static performCrossCorrelation(ai: any, acoustic: any, special: any, expert: any) {
    ...
    return {
      ...
      expertInsights: {
        ...
        hydraulicEfficiency: expert.hydraulic?.netHead
          ? (expert.hydraulic.netHead /
              (expert.hydraulic.netHead + (expert.hydraulic.headLoss || 0))) *
            100
          : 100,
      },
    };
  ...
  private static async triggerAutomatedActions(
    ...
    for (const rul of criticalRUL) {
      ...
      try {
        ...
        if ((res as any).inserted) {
          ...
          console.log(
            'ℹ️ Audit persistence result:',
            (res as any).message || (res as any).error || (res as any)
          );
        }
      } catch (err) {
    ...
    const hydraulicRunaway = diagnosis.aiPrediction.incidentPatterns.find(p =>
      p.pattern.includes('Hydraulic Runaway')
    );
    ...
    try {
      ...
      if (sealTelemetry && Array.isArray(sealTelemetry) && sealTelemetry.length > 0) {
        ...
        diagnosis.overallHealthScore = Math.max(
          0,
          Math.min(100, diagnosis.overallHealthScore + impact.overallDelta)
        );
        diagnosis.automatedActions.push({
          action: 'SHAFT_SEAL_GUARD',
          status: 'COMPLETED',
      ...
      if (thrustTelemetry && Array.isArray(thrustTelemetry) && thrustTelemetry.length > 0) {
        ...
        for (const s of thrustTelemetry) {
          lastAction = thrustMaster.addMeasurement({
            ...
            jackingPressureBar:
              typeof s.jackingPressureBar === 'number' ? s.jackingPressureBar : undefined,
        ...
        if (lastAction) {
          ...
          try {
            const confidence = (thrustMaster as any).getConfidenceScore
              ? (thrustMaster as any).getConfidenceScore(
                  thrustTelemetry as any[],
                  (history || []).map(h => (h as any).common?.vibration)
                )
              : null;
            diagnosis.serviceNotes?.push({
              service: 'ThrustBearingConfidence',
              severity: confidence !== null && confidence < 70 ? 'WARNING' : 'INFO',
              message: `confidence:${confidence}`,
              recommendation: 'Include sensor cross-check or manual verification if <70%',
            });
            diagnosis.automatedActions.push({
              action: 'THRUST_CONFIDENCE',
              status: 'INFO',
              metrics: { confidence },
            });
          } catch (e) {
          ...
          if (lastAction.action === 'LUBRICATION_CRISIS') {
            diagnosis.automatedActions.push({
              action: 'DERATE',
              status: 'PENDING',
              details: `Recommend derate due to lubrication crisis: ${lastAction.reason}`,
            });
            ...
            const sensorSnapshot = latestSnapshot
              ? {
                  timestamp: Date.parse(
                    (latestSnapshot as any).t ||
                      (latestSnapshot as any).timestamp ||
            ...
            const recentHistory = (history || []).slice(-6).map(h => ({
              timestamp: Date.parse(
                (h as any).t || (h as any).timestamp || new Date().toISOString()
              ),
            ...
            if (integrity.sensorAnomaly) {
              ...
              diagnosis.serviceNotes?.push({
                ...
                recommendation:
                  'Place unit in Throttled Safe State and schedule sensor inspection.',
                sourceFiles: [
                  {
                    filename: 'services/SensorIntegritySentinel.ts',
                    justification: integrity.note || 'Sensor correlation flagged anomaly',
                  },
                ],
              });
      ...
      if (wicketTelemetry && Array.isArray(wicketTelemetry) && wicketTelemetry.length > 0) {
        ...
        for (const s of wicketTelemetry) {
          lastAction = wicket.addMeasurement({
            ...
            gateActualPct:
              typeof s.gateActualPct === 'number' ? Number(s.gateActualPct) : undefined,
            regulatingRingForceN:
              typeof s.regulatingRingForceN === 'number'
                ? Number(s.regulatingRingForceN)
                : undefined,
        ...
        if (lastAction) {
          ...
          diagnosis.automatedActions.push({
            action: 'WICKET_KINEMATICS_GUARD',
            status: 'COMPLETED',
            details: impact.details,
          });
          ...
          if (lastAction.action === 'SHEAR_PIN_BROKEN') {
            diagnosis.automatedActions.push({
              action: 'LIMIT_GATE_OPENING',
              status: 'PENDING',
              details: `Limit gate opening to ${impact.limitGateOpenPct}% due to shear pin broken.`,
            });
          } else if (lastAction.action === 'BACKLASH_CRITICAL') {
            diagnosis.automatedActions.push({
              action: 'LIMIT_GATE_OPENING',
              status: 'PENDING',
              details: `Reduce max gate opening to ${impact.limitGateOpenPct}% (backlash critical).`,
            });
          } else if (lastAction.action === 'LUBRICATION_DEFICIENCY') {
            diagnosis.automatedActions.push({
              action: 'SCHEDULE_LUBRICATION',
              status: 'PENDING',
              details: lastAction.reason,
            });
          }
    ...
    try {
      ...
      if (governorTelemetry && Array.isArray(governorTelemetry) && governorTelemetry.length > 0) {
        ...
        for (const s of governorTelemetry) {
          lastAction = guardian.addMeasurement({
            timestamp: s.timestamp || new Date().toISOString(),
            mainHeaderPressureBar:
              typeof s.mainHeaderPressureBar === 'number'
                ? Number(s.mainHeaderPressureBar)
                : undefined,
            accumulatorPressureBar:
              typeof s.accumulatorPressureBar === 'number'
                ? Number(s.accumulatorPressureBar)
                : undefined,
            pumpRunTimeSec:
              typeof s.pumpRunTimeSec === 'number' ? Number(s.pumpRunTimeSec) : undefined,
            pumpOn: !!s.pumpOn,
            oilTempC: typeof s.oilTempC === 'number' ? Number(s.oilTempC) : undefined,
            filterDeltaPBar:
              typeof s.filterDeltaPBar === 'number' ? Number(s.filterDeltaPBar) : undefined,
            emergencyCloseDurationSec:
              typeof s.emergencyCloseDurationSec === 'number'
                ? Number(s.emergencyCloseDurationSec)
                : undefined,
          });
        }
        ...
        if (lastAction) {
          ...
          if (lastAction.action === 'BLOCK_OPENING') {
            ...
            diagnosis.automatedActions.push({
              action: 'FAILSAFE_CLOSE',
              status: 'PENDING',
              details: 'Prepare failsafe close due to low HPU pressure.',
            });
          } else if (lastAction.action === 'SAFETY_SYSTEM_DEGRADED') {
            diagnosis.automatedActions.push({
              action: 'SCHEDULE_SAFETY_INSPECTION',
              status: 'PENDING',
              details: lastAction.reason,
            });
          ...
          try {
            const confidence = (guardian as any).getConfidenceScore
              ? (guardian as any).getConfidenceScore(governorTelemetry as any[])
              : null;
            diagnosis.serviceNotes?.push({
              service: 'GovernorConfidence',
              severity: confidence !== null && confidence < 70 ? 'WARNING' : 'INFO',
              message: `confidence:${confidence}`,
              recommendation: 'If <70% verify hydraulic response and emergency close timings.',
            });
            diagnosis.automatedActions.push({
              action: 'GOVERNOR_CONFIDENCE',
              status: 'INFO',
              metrics: { confidence },
            });
          } catch (e) {
      ...
      if (gapTelemetry && Array.isArray(gapTelemetry) && gapTelemetry.length > 0) {
        ...
        if (lastAction) {
          ...
          if (lastAction.action === 'EXCITATION_TRIP') {
            ...
            diagnosis.automatedActions.push({
              action: 'MECHANICAL_TRIP',
              status: 'PENDING',
              details: 'Mechanical trip recommended following excitation removal.',
            });
          } else if (lastAction.action === 'MAGNETIC_INSTABILITY_WARNING') {
            diagnosis.automatedActions.push({
              action: 'SCHEDULE_BORE_SCOPE',
              status: 'PENDING',
              details: lastAction.reason,
            });
          }
    ...
    try {
      ...
      if (pdTelemetry && Array.isArray(pdTelemetry) && pdTelemetry.length > 0) {
        ...
        if (lastAction) {
          ...
          diagnosis.overallHealthScore = Math.max(
            0,
            Math.min(100, diagnosis.overallHealthScore + (impact.overallDelta || 0))
          );
          diagnosis.automatedActions.push({
            action: 'STATOR_INSULATION_GUARD',
            status: 'COMPLETED',
            details: impact.recommendation || (impact as any).details || lastAction.reason,
          });
          // Store guardian confidence score
          if (!(diagnosis as any).guardianConfidence) (diagnosis as any).guardianConfidence = {};
          ...
          if (lastAction.action === 'CRITICAL_INSULATION_DEGRADATION') {
            diagnosis.automatedActions.push({
              action: 'SCHEDULE_EMERGENCY_OUTAGE',
              status: 'PENDING',
              details: lastAction.reason,
            });
            diagnosis.automatedActions.push({
              action: 'BOROSCOPE_INSPECTION',
              status: 'PENDING',
              details: 'Immediate boroscope inspection recommended.',
            });
    ...
    try {
      ...
      if (txTelemetry && Array.isArray(txTelemetry) && txTelemetry.length > 0) {
        ...
        if (lastAction) {
          ...
          diagnosis.automatedActions.push({
            action: 'TRANSFORMER_OIL_GUARD',
            status: 'COMPLETED',
            details: impact.recommendation || lastAction.reason,
          });
          ...
          if (lastAction.action === 'CRITICAL_FAULT') {
            diagnosis.automatedActions.push({
              action: 'TRANSFORMER_CRITICAL_TRIP',
              status: 'PENDING',
              details: lastAction.reason,
            });
            diagnosis.automatedActions.push({
              action: 'RESTRICT_POWER',
              status: 'PENDING',
              details: 'Transformer critical - restrict plant power until inspected.',
            });
            // Persist critical flag into SovereignMemory for system-wide interlocks
            try {
              const memory = new SovereignMemory();
              memory.saveOverrideRecord({
                transformerCritical: true,
                timestamp: Date.now(),
            ...
            diagnosis.automatedActions.push({
              action: 'SCHEDULE_TRANSFORMER_INSPECTION',
              status: 'PENDING',
              details: lastAction.reason,
            });
          }
          ...
          try {
            const confidence = (guardian as any).getConfidenceScore
              ? (guardian as any).getConfidenceScore(txTelemetry as any[])
              : null;
            diagnosis.serviceNotes?.push({
              service: 'TransformerConfidence',
              severity: confidence !== null && confidence < 70 ? 'WARNING' : 'INFO',
              message: `confidence:${confidence}`,
              recommendation: 'If <70% verify DGA lab and manual sampling.',
            });
            diagnosis.automatedActions.push({
              action: 'TRANSFORMER_CONFIDENCE',
              status: 'INFO',
              metrics: { confidence },
            });
          } catch (e) {
  ...
  private static async notifyConsultant(
    asset: EnhancedAsset,
    diagnosis: UnifiedDiagnosis
  ): Promise<void> {
  ...
  private static aggregateServiceNotes(
    expert: any,
    acoustic: any
    ...
    if (expert.oil?.findings) {
      expert.oil.findings.forEach((f: any) =>
        notes.push({
  ...
  private static calculateTrendProjections(
    latest: CompleteSensorData
        ...
        measurements: [
          {
            ...
            value: latest.common.vibration,
            technicianName: 'CEREBRO_AI',
          },
        ],
      },
    ...
    projections['vibration'] = {
      ...
      projectedDate:
        vibTrend.projectedCriticalDate ||
        new Date(Date.now() + 365 * 24 * 6e7).toISOString().split('T')[0],
    };
    ...
    const tempTrend = HistoricalTrendAnalyzer.analyzeTrend(
      {
        ...
        measurements: [
          {
            timestamp: new Date().toISOString(),
            value: latest.common.temperature || 45,
            technicianName: 'CEREBRO_AI',
          },
        ],
      },
    ...
    projections['bearingTemp'] = {
      ...
      projectedDate:
        (tempTrend as any).projectedDate ||
        new Date(Date.now() + 365 * 24 * 6e7).toISOString().split('T')[0],
    };
  ...
  public static getISOJustification(zone: UnifiedDiagnosis['vibrationZone']): string {
    switch (zone) {
      case 'ZONE_A':
        return 'Zone A: Vibration values are typically found in new commissioned machines.';
      case 'ZONE_B':
        return 'Zone B: Machines are normally considered acceptable for long-term operation.';
      case 'ZONE_C':
        return 'Zone C: Machines are considered unsatisfactory for long-term continuous operation.';
      case 'ZONE_D':
        return 'Zone D: Vibration values are considered to be of sufficient severity to cause damage.';
      default:
        return '';
    }
  }
  ...
  private static compareWithHistoricalBaseline(latest: CompleteSensorData, asset: EnhancedAsset) {
    ...
    if (latest.common.vibration > baselines.vibration) {
      deviations.push({
        ...
        context:
          'Historical 10-year baseline for Francis units suggests a 15% increase in structural fatigue at this level.',
      });
    }
