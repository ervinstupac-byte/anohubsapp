...
async function checkCacheTableExists(): Promise<boolean> {
  ...
  try {
    ...
    if (
      error &&
      (error.code === '42P01' || error.message.includes('406') || (error as any).status === 406)
    ) {
...
export class AIPredictionService {
  ...
  detectSynergeticRisk(assetId: number, telemetry: TelemetryData): SynergeticRisk {
    ...
    const acousticVariance =
      acousticValues.reduce((sum, val) => sum + Math.pow(val - acousticMean, 2), 0) /
      acousticValues.length;
    ...
    const tempGrowthRate =
      (tempValues[tempValues.length - 1] - tempValues[0]) / (tempValues.length * 0.1); // °C per 0.1 min interval
    ...
    const pressureVariance =
      pressureValues.reduce((sum, val) => sum + Math.abs(val - pressureMean), 0) /
  ...
  async forecastEtaBreakEven(
    ...
    if (memHistory && memHistory.length > 0) {
      if (memHistory.length < 5)
        return { weeksUntil: null, predictedTimestamp: null, confidence: 0 };
      const pts = memHistory
        .map(h => ({ t: h.timestamp, y: h.efficiency }))
            ...
            const points = cacheRes.history
              .map((p: unknown) => {
                if (
                  p &&
                  typeof p === 'object' &&
                  't' in (p as Record<string, unknown>) &&
                  'y' in (p as Record<string, unknown>)
                ) {
                  const obj = p as { t: number; y: number };
                  return typeof obj.t === 'number' && typeof obj.y === 'number'
                    ? { t: obj.t, y: obj.y }
                    : null;
                }
                return null;
  ...
  private _computeLinearForecast(points: { t: number; y: number }[], threshold: number) {
    ...
    if (!isFinite(a) || Math.abs(a) < 1e-16)
      return {
        weeksUntil: null,
        predictedTimestamp: null,
        confidence: Math.min(1, n / HISTORY_WINDOW_SIZE),
        slope: a,
        intercept: b,
        tStatistic: tStat,
        sampleCount: n,
    ...
    if (tCross <= now)
      return {
        weeksUntil: 0,
        predictedTimestamp: Math.floor(tCross),
        confidence: Math.min(1, n / HISTORY_WINDOW_SIZE),
        slope: a,
        intercept: b,
        tStatistic: tStat,
          ...
          const filtered = cacheRes.history
            .map((p: unknown) => {
              if (
                ...
                const obj = p as { t: number; y: number };
                return typeof obj.t === 'number' && typeof obj.y === 'number'
                  ? { t: obj.t, y: obj.y, d: new Date(obj.t).toISOString().slice(0, 10) }
                  : null;
              }
              return null;
            })
            .filter(
              (p: any): p is { t: number; y: number; d: string } => !!p && typeof p.d === 'string'
            )
   ...
   */
  estimateEfficiencyFromTelemetry(telemetry: TelemetryData): {
    eta: number | null;
    ...
    try {
      ...
      const H_m =
        typeof telemetry.reservoirLevel === 'number' && typeof telemetry.tailwaterLevel === 'number'
  ...
  calculateRUL(
    ...
    const cavitationHours =
      ((history.filter(h => h.cavitationIntensity > 5).length / 6) * operatingHours) /
      HISTORY_WINDOW_SIZE;
  ...
  matchHistoricalPattern(assetId: number, telemetry: TelemetryData): IncidentPattern | null {
    ...
    for (const incident of HISTORICAL_INCIDENTS) {
      ...
      if (incident.type === 'HYDRAULIC_RUNAWAY') {
        ...
        if (
          telemetry.pipeDiameter === incident.triggerCondition.pipeDiameter &&
   ...
   */
  generatePrescription(component: string, failureProbability: number): PrescriptiveRecommendation {
  ...
  assessCavitationRisk(
    ...
    const g = 9.80665;
    const Q_m3s =
      typeof telemetry.pumpFlowRate === 'number' && Number.isFinite(telemetry.pumpFlowRate)
        ? telemetry.pumpFlowRate / 1000
        : null;
    // Prefer canonical netHead from projectState when available
    const H_m =
      projectState &&
      projectState.physics &&
      typeof projectState.physics.netHead === 'number' &&
      Number.isFinite(projectState.physics.netHead) &&
      projectState.physics.netHead > 0
        ? projectState.physics.netHead
        : typeof opts?.netHead === 'number' && Number.isFinite(opts.netHead)
          ? opts.netHead
          : null;
    ...
    const p_atm =
      typeof opts?.atmosphericPressurePa === 'number' && Number.isFinite(opts.atmosphericPressurePa)
        ? opts.atmosphericPressurePa
        : typeof telemetry.piezometricPressure === 'number'
          ? telemetry.piezometricPressure > 50
            ? telemetry.piezometricPressure
            : telemetry.piezometricPressure * 1e5
          : 101325;
    ...
    if (
      !head &&
      typeof telemetry.reservoirLevel === 'number' &&
      typeof telemetry.tailwaterLevel === 'number'
    ) {
    ...
    const conf = Math.min(
      0.99,
      (typeof head === 'number' && head > 0 ? 0.5 : 0) + (Q_m3s ? 0.3 : 0) + (ns ? 0.2 : 0)
    );
   ...
   */
  predictBearingThermalTTT(
    assetId: number,
    telemetry: TelemetryData,
    criticalTemp: number = 75,
    projectState?: TechnicalProjectState
  ): PredictionReport {
    const key = String(assetId);
    const history = telemetryHistory.get(key) || [];
    ...
    if (samples.length < 3) {
      ...
      return {
        probability: prob,
        timeToFailureHours: prob ? 0 : null,
        confidenceScore: samples.length / 12,
        mitigationAction: prob
          ? 'Immediate shutdown and bearing inspection'
          : 'Collect more telemetry for trend analysis',
        details: { samples: samples.length },
      };
    }
    ...
    if (Math.abs(slope) < 1e-12) {
      return {
        ...
        confidenceScore: Math.min(0.9, n / 12),
        mitigationAction: 'Temperature stable — continue monitoring',
        details: { slope },
      };
    }
    ...
    const ambient =
      projectState && projectState.site && typeof projectState.site.temperature === 'number'
        ? projectState.site.temperature
    ...
    const action =
      prob > 0.5
        ? 'Inspect bearing lubrication and cooling; consider reduce load and schedule immediate maintenance.'
        : 'Monitor bearing temperature and verify oil cooling flow.';

    return {
      probability: prob,
      timeToFailureHours: hoursTo > 0 ? hoursTo : null,
      confidenceScore: conf,
      mitigationAction: action,
      details: { slope, latestTemp: tempNow, samples: n, ambientAdjustedCritical },
    };
  }
  ...
  predictEfficiencyDecay(assetId: number, lookbackHours: number = 72): PredictionReport {
    ...
    if (history.length < 3)
      return {
        ...
        timeToFailureHours: null,
        confidenceScore: 0,
        mitigationAction: 'Insufficient data',
        details: { samples: history.length },
      };
    ...
    if (window.length < 2)
      return {
        ...
        timeToFailureHours: null,
        confidenceScore: 0.2,
        mitigationAction: 'Insufficient recent data',
        details: { windowSize: window.length },
      };

    const effValues = window.map(w => w.efficiency).filter(v => typeof v === 'number');
    if (effValues.length < 2)
      return {
        probability: 0,
        timeToFailureHours: null,
        confidenceScore: 0.2,
        mitigationAction: 'No efficiency samples in window',
        details: {},
      };
    ...
    let cause = 'Unknown';
    if ((recent.rotorEccentricity ?? 0) > 0.2 || (recent.bearingGrindIndex ?? 0) > 2)
      cause = 'runner_erosion';
    if ((recent.seepageRate ?? 0) > 20 || (recent.ultrasonicLeakIndex ?? 0) > 3)
      cause = 'seal_wear';

    let probability = 0;
    if (pctDrop > 1.5) probability = Math.min(1, 0.3 + (pctDrop - 1.5) / 10);

    const timeToFailureHours =
      probability > 0
        ? Math.max(
            1,
            ((100 - endEff) /
              Math.max(
                0.01,
                Math.abs(
                  (endEff - startEff) / (window[window.length - 1].timestamp - window[0].timestamp)
                )
              )) *
              3600 *
              24
    ...
    const action =
      cause === 'runner_erosion'
        ? 'Inspect runner and blade surfaces; consider on-site NDT and reduced operation.'
        : cause === 'seal_wear'
          ? 'Inspect labyrinth seals and check for increased leakage; prepare for seal replacement.'
          : 'Investigate performance drop: review intake, seals, and runner; schedule inspection.';
  ...
  generatePredictionReport(
    assetId: number,
    telemetry: TelemetryData
  ): { cavitation: PredictionReport; bearing: PredictionReport; efficiency: PredictionReport } {
    ...
    const physicsNetHeadNumber = physics?.netHead
      ? physics.netHead instanceof Decimal
        ? physics.netHead.toNumber()
        : Number(physics.netHead)
      : undefined;
    const cav = this.assessCavitationRisk(
      telemetry,
      { rpm: undefined, netHead: physicsNetHeadNumber, temperatureC: telemetry.temperature },
      projectState || undefined
    );
    const bear = this.predictBearingThermalTTT(
      assetId,
      telemetry,
      undefined,
      projectState || undefined
