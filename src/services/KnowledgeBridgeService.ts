...
export class KnowledgeBridgeService {
  ...
  static deriveBaselines(project: ProjectDNA): OperationalBaseline {
    ...
    if (genesis.siteParams.grossHead > 200) {
      // High head Pelton/Francis
      baseInterval *= 0.8;
    }
   ...
   */
  static getSmartSchedule(
    project: ProjectDNA
  ): { action: string; dueInHours: number; reason: string }[] {
    ...
    const hoursUntilInspection =
      baselines.runnerInspectionIntervalHours -
      (currentRunHours % baselines.runnerInspectionIntervalHours);
    schedule.push({
