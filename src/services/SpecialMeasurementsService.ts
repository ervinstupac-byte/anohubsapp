...
export class SpecialMeasurementsService {
  ...
  static correlateSettlementWithEccentricity(
    ...
    if (deviation < 0.05) {
      status = 'ALIGNED';
      recommendation =
        'Settlement-eccentricity correlation within expected range. Continue quarterly monitoring.';
    } else if (deviation < 0.1) {
      status = 'MINOR_DEVIATION';
      recommendation =
        'Minor deviation detected. Schedule precision alignment check within 30 days. Monitor monthly.';
    } else if (deviation < 0.2) {
      status = 'SIGNIFICANT_DEVIATION';
      recommendation =
        '⚠️ Significant deviation from predicted correlation. Other factors may be contributing (thermal expansion, coupling wear). Schedule detailed vibration diagnostics and alignment survey within 14 days.';
    } else {
      status = 'CRITICAL';
      recommendation =
        '🔴 CRITICAL: Large unexplained eccentricity. Possible foundation crack, bearing failure, or rotor bow. Emergency inspection required. Consider load reduction.';
    }
  ...
  static integrateGeodeticVibration(
    ...
    const vibrationLevel = Math.sqrt(
      vibration.horizontalVibration ** 2 + vibration.verticalVibration ** 2
    );
   ...
   */
  static validateFoundationStability(historicalGeodetic: GeodeticMeasurement[]): {
  ...
  static generateIntegrationReport(
    ...
    geodeticVibrationIntegration: ReturnType<
      typeof SpecialMeasurementsService.integrateGeodeticVibration
    >;
    ...
    if (correlation.status === 'CRITICAL' || integration.alignmentQuality === 'POOR') {
      urgency = 'CRITICAL';
      overallAssessment =
        '🔴 CRITICAL: Multiple indicators show severe alignment/foundation issues. Emergency intervention required.';
    } else if (correlation.status === 'SIGNIFICANT_DEVIATION' || integration.correctionRequired) {
      urgency = 'HIGH';
      overallAssessment =
        '⚠️ HIGH PRIORITY: Significant deviations detected. Schedule corrective action within 14 days.';
    } else if (
      correlation.status === 'MINOR_DEVIATION' ||
      integration.alignmentQuality === 'FAIR'
    ) {
      urgency = 'MEDIUM';
      overallAssessment =
        'MEDIUM: Minor issues detected. Plan corrective maintenance during next scheduled outage.';
    } else {
      urgency = 'LOW';
      overallAssessment =
        '✅ All measurements within acceptable range. Continue routine monitoring.';
    }
