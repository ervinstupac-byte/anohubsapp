...
export class AdditiveManufacturingService {
  ...
  public static checkLogisticsGap(
    ...
    if (materialAvailable < component.materialRequired) {
      ...
      console.log(
        `  Required: ${component.materialRequired} kg, Available: ${materialAvailable} kg`
      );
      return null;
    }
  ...
  private static completePrinting(job: PrintJob): void {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        console.log(
          `Total time: ${((job.actualCompletion - job.startTime) / 1000 / 60).toFixed(0)} minutes`
        );
  ...
  public static getStatistics(): {
    ...
    return {
      ...
      inProgress: this.printQueue.filter(
        j => j.status === 'PRINTING' || j.status === 'POST_PROCESS'
      ).length,
  ...
  public static calculateMaterialRunway(material: string): {
    ...
    const componentsUsingMaterial = Array.from(this.componentLibrary.values()).filter(
      c => c.material === material
    );

    const avgConsumption =
      componentsUsingMaterial.length > 0
        ? componentsUsingMaterial.reduce((sum, c) => sum + c.materialRequired, 0) /
          componentsUsingMaterial.length
        : 0;
