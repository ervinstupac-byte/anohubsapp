...
export class SeismicResponseMonitor {
  ...
  public static analyze(
    ...
    const kRatio =
      (currentPeakFreqHz * currentPeakFreqHz) / (this.BASELINE_FREQ_HZ * this.BASELINE_FREQ_HZ);
    ...
    if (stiffnessChange < -5.0) {
      ...
      console.warn(
        `[Civil] 📉 NATURAL FREQUENCY DROP: ${this.BASELINE_FREQ_HZ} -> ${currentPeakFreqHz}Hz. Est Stiffness Loss: ${stiffnessChange.toFixed(1)}%`
      );
    }
