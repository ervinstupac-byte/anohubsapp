...
export function duvalMethod1(sample: TransformGasSample): TransformerOilAnalysis {
  ...
  if (sum <= 0)
    return { reason: 'Insufficient hydrocarbon gases', recommendedAction: 'Take full DGA sample' };
...
export function detectAcetyleneTrend(
  samples: TransformGasSample[],
  lookback = 5
): { trendingUp: boolean; slope?: number } {
...
export class TransformerOilGuardian extends BaseGuardian {
  ...
  addMeasurement(m: TransformerMeasurement): TransformerAction {
    ...
    if (c2h2 > 0) {
      return {
        action: 'CRITICAL_FAULT',
        reason: `Acetylene detected (${c2h2} ppm) — immediate critical transformer fault audit required.`,
      };
    }
    ...
    if (pctC2H4 > 50 || (c2h4 > c2h6 && c2h4 > ch4 && (m.topOilTempC || 40) > 90)) {
      return {
        action: 'THERMAL_FAULT',
        details: `C2H4 dominant (${c2h4} ppm) suggests thermal fault`,
        reason: 'Duval-like thermal indicator',
      };
    }

    if (h2 > 100 && ch4 < 50) {
      return {
        action: 'ELECTRICAL_FAULT',
        details: `High H2 (${h2} ppm) suggests partial discharge / electrical stressing`,
        reason: 'Hydrogen-rich signature',
      };
    }
    ...
    if (qTrend && qTrend.pctIncrease !== null && qTrend.pctIncrease >= 20) {
      return {
        ...
        reason: `Transformer gas maximum increased ${qTrend.pctIncrease.toFixed(1)}% over 30 days.`,
      };
    }
    ...
    if (yearsLeft < 2) {
      return {
        ...
        limitPct: 50,
        reason: `Estimated paper life low: ${yearsLeft.toFixed(2)} years remaining — restrict power to 50% and schedule outage.`,
      };
    }
  ...
  computeQMaxTrend30d(
    refTimestampIso: string
  ): { pctIncrease: number | null; baselineMax?: number; recentMax?: number } | null {
  ...
  estimatePaperRULHours(): { rulHours: number; details: string } {
    ...
    return {
      rulHours,
      details: `avgTemp=${avgTemp.toFixed(1)}C avgMoist=${avgMoist.toFixed(1)}ppm agingFactor=${agingFactor.toFixed(2)}`,
    };
  }
  ...
  getHealthImpactForAction(a: TransformerAction) {
    switch (a.action) {
      case 'CRITICAL_FAULT':
        return {
          overallDelta: -100,
          recommendation: 'Immediate transformer isolation and detailed gas analysis.',
        };
      case 'THERMAL_FAULT':
        return { overallDelta: -40, recommendation: 'Investigate cooling, oil, and load profile.' };
      case 'ELECTRICAL_FAULT':
        return {
          overallDelta: -50,
          recommendation: 'Investigate winding integrity and partial discharge sources.',
        };
      case 'RESTRICT_POWER':
        return {
          overallDelta: -60,
          recommendation: 'Restrict plant output to preserve transformer.',
        };
      case 'SCHEDULE_TRANSFORMER_INSPECTION':
        return {
          overallDelta: -30,
          recommendation: 'Schedule inspection and detailed oil sampling.',
        };
      default:
        return { overallDelta: 0, recommendation: 'No impact' };
    }
  }
  ...
  public getConfidenceScore(samples: TransformerMeasurement[] = []): number {
    ...
    const acet = recent.map(s =>
      s.gasesPpm && typeof s.gasesPpm.C2H2 === 'number' ? s.gasesPpm.C2H2 : 0
    );
