...
export class CivilSecurityModule {
  ...
  public static registerPiezometer(data: PiezometerData): void {
    ...
    console.log(
      `[CivilSecurity] Piezometer ${data.id}: ${data.pressure.toFixed(2)} bar (${upliftForce.toFixed(0)} kN/m² uplift)`
    );
  }
  ...
  public static registerSeismicEvent(data: SeismicData): void {
    ...
    let activity: SeismicData['magnitude'] extends number
    ...
    if (activity === 'HIGH' || activity === 'MODERATE') {
      console.log(
        `[CivilSecurity] ⚠️ SEISMIC EVENT: ${data.stationId} - ${data.peakGroundAcceleration.toFixed(3)}g`
      );
  ...
  private static updateStabilityGauge(): void {
    ...
    const avgUpliftPressure =
      piezometerArray.length > 0
    ...
    const maxPGA =
      seismicArray.length > 0 ? Math.max(...seismicArray.map(s => s.peakGroundAcceleration)) : 0;
   ...
   */
  private static calculateSimplifiedSafetyFactor(
    waterLevel: number,
