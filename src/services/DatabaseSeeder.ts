...
export class DatabaseSeeder {
   ...
   */
  public static async seedIfEmpty(onProgress?: (progress: SeedProgress) => void): Promise<boolean> {
    const raceTimeout = new Promise<boolean>(resolve =>
      setTimeout(() => {
        console.warn(
          '[DatabaseSeeder] ⚠️ Seed check timed out after 2s — proceeding with local data'
        );
  ...
  private static async _seedIfEmptyCore(
    ...
    try {
      ...
      if (assetCount > 0) {
        this.updateProgress(
          'SKIPPED',
          `Database already contains ${assetCount} asset(s)`,
          onProgress
        );
    ...
    try {
      ...
      for (const unit of units) {
        ...
        if (assetError) {
          ...
          if (assetData?.id) {
            await this.seedHppStatus(assetData.id, tablesSeeded);
          }
        }
      }
  ...
  private static async seedHppStatus(assetId: string, tablesSeeded: string[]) {
    ...
    try {
      const { error: statusError } = await supabase.from('hpp_status').insert([
        {
          asset_id: assetId,
          ...HPP_STATUS_SEED,
        },
