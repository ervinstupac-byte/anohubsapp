...
export class InstitutionalKnowledgeService {
  ...
  static async verifyKnowledge(
    ...
    if (!entry.verifiedBy.includes(verifierId)) {
      ...
      console.log(
        `✅ ${verifierName} verified "${entry.incidentPattern}" (confidence: ${(entry.confidenceScore * 100).toFixed(0)}%)`
      );
    }
  }
   ...
   */
  static async getTopContributors(limit: number = 10): Promise<
    Array<{
      engineerId: string;
  ...
  > {
    ...
    return contributors.sort((a, b) => b.totalUpvotes - a.totalUpvotes).slice(0, limit);
  }

  /**
   * Add engineer field note (quick tip)
   */
  static async addFieldNote(
    note: Omit<EngineerNote, 'id' | 'upvotes' | 'isPinned' | 'createdAt'>
  ): Promise<EngineerNote> {
...
> = [
  {
    ...
    rootCause:
      '12mm hydraulic hose replaced with 16mm without system recharacterization. Increased oil volume reduced damping, pressure spike exceeded design limits.',
    solution:
      '1. Replace hose with correct 12mm diameter\n2. Increase accumulator volume to compensate if 16mm required\n3. Install pressure relief valve at 15 bar\n4. Retune PID controller for new hydraulic dynamics',
    fieldNotes:
      'Happened at 3am during emergency repair. Technician used "what we had in stock". The larger hose seemed like an upgrade but caused catastrophic failure. ALWAYS simulate hydraulic changes before implementation. The sound was like a gunshot - woke up the entire village.',
    lessonLearned:
      'Hydraulic system is HIGHLY sensitive to component changes. Even "minor" pipe diameter changes can cause disasters. NEVER bypass safety validation.',
    preventiveMeasures: [
  ...
  },
  {
    ...
    rootCause:
      'Operating at 45% load creates unstable vortex core in draft tube. Vortex shedding frequency (1.2 Hz) excites draft tube resonance.',
    solution:
      '1. Install air admission system at draft tube inlet\n2. Increase load to >70% design point\n3. If must operate at part load, adjust guide vane settings to minimize swirl\n4. Consider installing vortex suppression cone retrofit',
    fieldNotes:
      'Customer complained of "washing machine sound" during low water season when forced to run at 40% load. The vibration was felt throughout the powerhouse. Air admission immediately solved it - like magic. Cost $5k, saved $500k in potential damage.',
    lessonLearned:
      'Francis turbines HATE part-load operation. Design point is king. If you must operate off-design, install air admission system from day one.',
    preventiveMeasures: [
  ...
  },
  {
    ...
    rootCause:
      'Emergency shutdown activated deflector without lead time. Nozzle closed simultaneously, causing water hammer (pressure spike to 180 bar from 100 bar design).',
    solution:
      '1. ALWAYS close deflector BEFORE nozzle (minimum 2 second lead)\n2. Install surge tank upstream of nozzle\n3. Program emergency sequence: Deflector → 2s delay → Nozzle\n4. Add pressure relief valve on penstock',
    fieldNotes:
      'Grid fault triggered emergency stop. Operator heard "BOOM" from penstock. Inspection found weld crack in elbow joint. This was a $200k repair that could have destroyed the entire penstock. Emergency sequences MUST be properly programmed - test them!',
    lessonLearned:
      'Pelton emergency stops are NOT instant. Hydraulic transients will destroy your system if you close too fast. Deflector is your friend - use it first.',
    preventiveMeasures: [
