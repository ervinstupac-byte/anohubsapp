...
export class DamStabilityService {
  ...
  public static calculateSafetyFactor(data: DamStabilityData): StabilityAnalysis {
    ...
    const safetyFactor =
      effectiveWeight > 0 && drivingMoment > 0 ? resistingMoment / drivingMoment : 0;
  ...
  private static calculateUpliftPressure(piezometricPressures: number[]): number {
    ...
    const avgPressure =
      piezometricPressures.reduce((sum, p) => sum + p, 0) / piezometricPressures.length;
  ...
  private static analyzeStability(safetyFactor: number, data: DamStabilityData): StabilityAnalysis {
    ...
    if (maxDisplacement > this.MAX_DISPLACEMENT_THRESHOLD) {
      warnings.push(
        `Excessive displacement detected: ${maxDisplacement.toFixed(1)} mm (threshold: ${this.MAX_DISPLACEMENT_THRESHOLD} mm)`
      );
    ...
    if (data.seepageRate > this.MAX_SEEPAGE_RATE) {
      warnings.push(
        `Excessive seepage: ${data.seepageRate.toFixed(1)} L/s (max: ${this.MAX_SEEPAGE_RATE} L/s)`
      );
    ...
    const avgPorePressure =
      data.piezometricPressures.reduce((sum, p) => sum + p, 0) / data.piezometricPressures.length;
    if (avgPorePressure > 5.0) {
      // >5 bar is concerning
