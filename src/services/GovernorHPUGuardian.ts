...
export class GovernorHPUGuardian extends BaseGuardian {
  ...
  addMeasurement(m: GovernorMeasurement): GovernorAction {
    ...
    if (this.pumpStarts.length >= 3) {
      ...
      for (let i = 1; i < this.pumpStarts.length; i++)
        intervals.push((this.pumpStarts[i] - this.pumpStarts[i - 1]) / 1000); // seconds
      const lastInterval = intervals[intervals.length - 1];
      const prevAvg =
        intervals.slice(0, -1).reduce((s, v) => s + v, 0) / Math.max(1, intervals.length - 1);

      // require roughly constant load: low variance in mainHeaderPressure
      const pressures = this.samples
        .map(s => s.mainHeaderPressureBar)
        .filter(v => typeof v === 'number') as number[];
      const pressureStd = this.stdDev(pressures);

      if (prevAvg > 1 && lastInterval < prevAvg * this.prechargeTrendFactor && pressureStd < 0.5) {
        return {
          action: 'NITROGEN_PRECHARGE_DEPLETION',
          trend: { lastInterval, prevAvg },
          reason: `Pump starts interval decreasing (${lastInterval}s < ${(prevAvg * this.prechargeTrendFactor).toFixed(1)}s) while pressure stable (σ=${pressureStd.toFixed(2)}bar).`,
        };
      }
    }
    ...
    if (deltas.length >= 10 && temps.length >= 10) {
      ...
      if (
        corr >= this.varnishCorrThreshold &&
        avgTemp >= this.varnishOilTempThresholdC &&
        avgDelta >= this.varnishDeltaPThresholdBar
      ) {
        return {
          action: 'VARNISH_RISK_ALERT',
          corr,
          reason: `High correlation between filter ΔP and oil temp (corr=${corr.toFixed(2)}), avgTemp=${avgTemp.toFixed(1)}°C, ΔP=${avgDelta.toFixed(3)}bar.`,
        };
      }
    }
    ...
    if (typeof m.emergencyCloseDurationSec === 'number') {
      if (this.emergencyBaselineSec === null)
        this.emergencyBaselineSec = m.emergencyCloseDurationSec;
      else {
        const current = m.emergencyCloseDurationSec;
        if (current > this.emergencyBaselineSec * 1.2) {
          return {
            action: 'SAFETY_SYSTEM_DEGRADED',
            baselineSec: this.emergencyBaselineSec,
            currentSec: current,
            reason: `Emergency close time increased from ${this.emergencyBaselineSec}s to ${current}s (>20%).`,
          };
        }
      }
    }
    ...
    if (
      typeof m.mainHeaderPressureBar === 'number' &&
      m.mainHeaderPressureBar < this.minMainHeaderPressureBar
    ) {
      return {
        action: 'BLOCK_OPENING',
        reason: `Main header pressure low (${m.mainHeaderPressureBar} bar) — blocking opening and preparing failsafe close.`,
      };
    }
    ...
    if (typeof m.emergencyCloseDurationSec === 'number') {
      ...
      if (m.emergencyCloseDurationSec < safeTimeSec * 0.5) {
        return {
          ...
          reason: `Emergency close too fast (${m.emergencyCloseDurationSec}s) relative to safe penstock time ${safeTimeSec}s — potential water hammer.`,
        };
      }
    }
  ...
  getHealthImpactForAction(a: GovernorAction) {
    switch (a.action) {
      case 'NITROGEN_PRECHARGE_DEPLETION':
        return { overallDelta: -20, details: a.reason };
      case 'VARNISH_RISK_ALERT':
        return { overallDelta: -25, details: a.reason };
      case 'SAFETY_SYSTEM_DEGRADED':
        return { overallDelta: -40, details: a.reason };
      case 'BLOCK_OPENING':
        return { overallDelta: -30, details: a.reason };
      case 'FAILSAFE_CLOSE':
        return { overallDelta: -50, details: a.reason };
      default:
        return { overallDelta: 0, details: a.reason || 'No impact' };
    }
  }
  ...
  public getConfidenceScore(samples: GovernorMeasurement[] = []): number {
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      if (prevRun === 0 && curRun > 0) {
        ...
        let cmdIdx = i - 1;
        while (cmdIdx >= 0 && Math.abs(Date.parse(data[cmdIdx].timestamp) - eventTime) < 120000)
          cmdIdx--;
  ...
  applyNozzleSequence(seq: {
    ...
    return {
      accepted: true,
      applied: seq,
      message: `Nozzle sequencing applied (${seq.activeNozzles} active)`,
    };
  }
}
