...
export type AirGapAction =
  ...
  | {
      action: 'MAGNETIC_INSTABILITY_WARNING';
...
export class GeneratorAirGapSentinel extends BaseGuardian {
  ...
  constructor(opts?: Partial<Record<string, number>>) {
    super();
    if (opts?.['eccentricityWarningPct'])
      this.eccentricityWarningPct = opts['eccentricityWarningPct'];
    if (opts?.['eccentricityCriticalPct'])
      this.eccentricityCriticalPct = opts['eccentricityCriticalPct'];
  ...
  addMeasurement(m: AirGapMeasurement): AirGapAction {
    ...
    const umpN =
      k * Math.abs(m.excitationCurrentA) * (eccentricityPct / 100) * (m.rotorSpeedRpm / 1000);

    // suppress alarms during thermal startup: if stator warming and rotor slow
    const isThermalStartup =
      deltaTemp > this.suppressTempRiseC && m.rotorSpeedRpm < this.rotorSpeedSuppressionRpm;
    if (isThermalStartup) {
      return {
        action: 'NO_ACTION',
        reason: `Thermal startup suppression active (ΔT=${deltaTemp.toFixed(1)}°C, rpm=${m.rotorSpeedRpm}).`,
      };
    }
    ...
    if (minGap <= this.minGapThreshold_m) {
      ...
      return {
        action: 'CRITICAL_GAP_REDUCTION',
        minGap_m: minGap,
        reason: `Minimum gap ${(minGap * 1000).toFixed(2)} mm below threshold ${(this.minGapThreshold_m * 1000).toFixed(2)} mm.`,
      };
    }
    ...
    if (eccentricityPct >= this.eccentricityCriticalPct || umpN >= this.umpCriticalN) {
      return {
        ...
        reason: `Critical magnetic instability detected (ecc=${eccentricityPct.toFixed(1)}%, UMP=${umpN.toFixed(0)}N).`,
      };
    }
    ...
    if (eccentricityPct >= this.eccentricityWarningPct || umpN >= this.umpWarningN) {
      return {
        action: 'MAGNETIC_INSTABILITY_WARNING',
        eccentricityPct: Number(eccentricityPct.toFixed(2)),
        umpN: Number(umpN.toFixed(0)),
        reason: `Magnetic asymmetry detected (ecc=${eccentricityPct.toFixed(1)}%, UMP=${umpN.toFixed(0)}N).`,
      };
    }

    return {
      action: 'NO_ACTION',
      reason: `avgGap=${(avgGap * 1000).toFixed(2)} mm, ecc=${eccentricityPct.toFixed(2)}%`,
    };
  }

  getHealthImpactForAction(a: AirGapAction) {
    switch (a.action) {
      case 'CRITICAL_GAP_REDUCTION':
        return {
          overallDelta: -80,
          followup: ['EXCITATION_TRIP', 'MECHANICAL_TRIP'],
          details: a.reason,
        };
      case 'EXCITATION_TRIP':
        return { overallDelta: -60, followup: ['EXCITATION_TRIP'], details: a.reason };
      case 'MAGNETIC_INSTABILITY_WARNING':
        return { overallDelta: -25, followup: [], details: a.reason };
      default:
        return { overallDelta: 0, followup: [], details: a.reason || 'No impact' };
    }
  }
  ...
  public getConfidenceScore(samples: AirGapMeasurement[] = []): number {
    ...
    const umps = samples.map(s => {
      ...
      return (
        200 * Math.abs(s.excitationCurrentA || 0) * (ecc / 100) * ((s.rotorSpeedRpm || 0) / 1000)
      );
    });
