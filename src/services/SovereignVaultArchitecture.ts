...
export class SovereignVaultArchitecture {
  ...
  public static validateCompliance(actualSpecs: Partial<VaultSpecifications>): {
    ...
    if (
      (actualSpecs.physical?.shielding?.attenuationDb || 0) < specs.physical.shielding.attenuationDb
    ) {
      violations.push(
        `RF attenuation insufficient: ${actualSpecs.physical?.shielding?.attenuationDb}dB < ${specs.physical.shielding.attenuationDb}dB required`
      );
    }
    ...
    if ((actualSpecs.redundancy?.servers || 0) < specs.redundancy.servers) {
      violations.push(
        `Insufficient redundancy: ${actualSpecs.redundancy?.servers} < ${specs.redundancy.servers} required`
      );
    }
