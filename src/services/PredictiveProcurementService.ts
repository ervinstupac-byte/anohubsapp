...
export class PredictiveProcurementService {
  ...
  public static checkComponentProcurement(
    ...
    if (criticalStockout || urgentProcurement) {
      ...
      return this.createRequisition(componentId, part, rulDays, 'ROUTINE', logisticsGapDays);
    }
  ...
  private static createRequisition(
    ...
    if (urgency === 'CRITICAL') {
      reason =
        `CRITICAL: Component failure predicted in ${rulDays} days, ZERO stock available, ` +
