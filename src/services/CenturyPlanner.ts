...
export function centuryPlan(input: CenturyPlanInput, horizons = [10, 20, 50]) {
  ...
  const baselineAnnualRevenue = computeAnnualRevenue(
    input.currentPowerKw,
    input.pricePerKwh,
    capacityFactor
  );
  ...
  for (const y of horizons) {
    ...
    for (let year = 0; year < y; year++) {
      ...
      const lossPerHour = computeRevenueLoss(
        etaThis.toNumber(),
        input.optimalEta,
        input.currentPowerKw,
        input.pricePerKwh
  ...
  return {
    baselineAnnualRevenue: baselineAnnualRevenue.toNumber(),
    degradationRatePerYear: degr.toNumber(),
    projections: results,
  };
}

export async function persistCenturyPlanForAsset(
  assetId: number,
  input: CenturyPlanInput,
  name = 'Auto Century Plan'
) {
  // run centuryPlan and persist into century_plans
  const result = centuryPlan(input);
  try {
    const { error } = await supabase.from('century_plans').insert([
      {
        asset_id: assetId,
        name,
        input_json: input,
