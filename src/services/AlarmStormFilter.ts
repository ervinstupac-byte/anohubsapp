...
export class AlarmStormFilter {
   ...
   */
  public static updateAlarm(id: string, active: boolean, explicitParentId?: string): boolean {
    const now = Date.now();
  ...
  public static getVisibleAlarms(): AlarmDefinition[] {
    return Array.from(this.activeAlarms.values()).filter(a => a.active && !a.suppressed);
  }
  ...
  public static getSuppressedCount(): number {
    return Array.from(this.activeAlarms.values()).filter(a => a.active && a.suppressed).length;
  }
}
