...
export class KaplanOptimizer {
  ...
  public static calculateEfficiencyGap(
    ...
    if (efficiencyGap > 1.0) {
      ...
      if (mechanicalOffset > 0) {
        recommendation =
          `HEALING_PROTOCOL_OPTIMIZE_CAM (Backlash Compensated): ` +
        ...
        recommendation =
          `HEALING_PROTOCOL_OPTIMIZE_CAM: Adjust blade angle by ${angleDelta.toFixed(2)}° ` +
  ...
  private static getOptimalEfficiency(gateOpening: number, bladeAngle: number): number {
    ...
    return this.hillChart.get(key) || 0.9; // Default 90% if not in chart
  }
...
export class FrancisOptimizer {
  ...
  public static detectVortexResonance(state: FrancisState): {
    ...
    const inResonanceRange =
      state.vortexFrequency >= rheingansLow && state.vortexFrequency <= rheingansHigh;
    ...
    if (suppressionRequired) {
      recommendation =
        `HEALING_PROTOCOL_VORTEX_SUPPRESSION: ` +
...
export class PeltonOptimizer {
  ...
  public static detectWaterHammer(state: PeltonState): {
    ...
    if (waterHammerDetected) {
      ...
      recommendation =
        `HEALING_PROTOCOL_REDUCE_NEEDLE_SPEED: ` +
   ...
   */
  public static getOptimalNeedleProfile(
