...
import SensorIntegritySentinel, {
  SensorSnapshot,
  IntegrityResult,
} from './SensorIntegritySentinel';
...
export class TruthJudge {
  ...
  evaluateSignalHealth(
    ...
    if (newValue === lastState.lastValue && dt > 3600) {
      // Frozen for an hour?
      return 'BAD_FROZEN';
    }
  ...
  validateSensor(
    ...
    if (health === 'BAD_SLEW' || health === 'BAD_FROZEN') {
        ...
        try {
          if (
            ...
            (window as any).dispatchEvent(
              new CustomEvent('truthJudge:fallback_suppressed', {
                detail: {
                  sensorId,
      ...
      const fallbackVerdict: Verdict = {
        ...
        reason:
          health === 'BAD_SLEW'
            ? `Slew rate violation detected: ${sensorId}`
            : `Sensor appears frozen: ${sensorId}`,
        action: 'USE_FALLBACK',
      };
      ...
      try {
        if (typeof window !== 'undefined' && typeof (window as any).dispatchEvent === 'function') {
          (window as any).dispatchEvent(
            new CustomEvent('truthJudge:fallback', {
              detail: { sensorId, verdict: fallbackVerdict },
            })
          );
        }
      } catch (e) {
