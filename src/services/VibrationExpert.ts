...
export class VibrationExpert {
  ...
  checkFrequencyPeaks(
    ...
    const bpfPeak = peaks.find(
      p => p.frequencyHz >= bpfHz - tolerance && p.frequencyHz <= bpfHz + tolerance
    );
    ...
    if (maintenanceFactors) {
      ...
      if (
        maintenanceFactors.shaftPlumbnessDeviation &&
        maintenanceFactors.shaftPlumbnessDeviation > 0.05
      ) {
    ...
    if (bpfPeak) {
      if (bpfPeak.amplitudeMmS > bpfThreshold) {
        return {
          ...
          recommendation:
            'Hydraulic imbalance detected - check guide vane alignment or runner debris! 🍂' +
            warningSuffix,
          bpfHz,
        };
      }
    ...
    const unbalancePeak = peaks.find(
      p => p.frequencyHz >= rotationFreqHz - 0.5 && p.frequencyHz <= rotationFreqHz + 0.5
    );
    ...
    if (unbalancePeak && unbalancePeak.amplitudeMmS > unbalanceThreshold) {
      return {
        ...
        recommendation:
          'Mechanical Unbalance! Check rotor mass distribution or lost weights. ⚖️' + warningSuffix,
        bpfHz,
      };
    }

    // NC-130 Check for 2xRPM (Misalignment) - New Killer
    const misalignmentPeak = peaks.find(
      p => p.frequencyHz >= rotationFreqHz * 2 - 0.5 && p.frequencyHz <= rotationFreqHz * 2 + 0.5
    );
