...
export class LegacyKnowledgeService {
  ...
  static initialize() {
    ...
    this.addCase({
      ...
      severity: 'CRITICAL',
      symptom:
        'Cijevi na rotor-glavi se čudno trzaju. Blade position oscilira +/- 2° pri konstantnom opterećenju.',
      wrongDiagnosis: 'PID kontroler nije dobro tuniran. Servo ventil možda zaglavljen.',
      realCause:
        'Inženjer zamijenio 12mm hidrauličku cijev sa 16mm misleći da će biti bolje (veći protok = brži odziv). Rezultat: Promijenio se dinamički otpor sistema (System Stiffness). Natural frequency se pomaknula i sada rezonira sa PID kontrolerom.',
      solution: [
    ...
    this.addCase({
      ...
      severity: 'CRITICAL',
      symptom:
        'Nema očiglednih alarma. Vibracije 40% limita, kavitacija 4/10, temperatura lagano raste (0.5°C/dan). Generator radi normalno.',
      wrongDiagnosis: 'Sve u redu. Parametri u granicama.',
      realCause:
        'Progresivno trošenje Babbitt metala. Kombinacija: mala kavitacija + vibracije ispod limita + spor rast temperature = faza 2 od 3 u umiranju ležaja. Analizom ulja pronađeno 45 ppm Babbitt čestica.',
      solution: [
    ...
    this.addCase({
      ...
      severity: 'HIGH',
      symptom:
        'Glasne vibracije (bukva) na 30% opterećenja. Frekvencija 8-12 Hz. Nestaje na full load i idle.',
      wrongDiagnosis: 'Nešto labavo u draft tube. Možda pukla armatura.',
      realCause:
        'Part-load vortex rope - normalna pojava kod Francis turbina na djelimičnom opterećenju. Vortex u draft tube-u rezonira sa konstrukcijom. Nije kvar, ali prolongirani rad ovako oštećuje runner.',
      solution: [
    ...
    this.addCase({
      ...
      severity: 'LOW',
      symptom:
        'Vibration sensor pokazuje spike do 15 mm/s koji traju < 5 sekundi. Ponavljaju se nasumično.',
      wrongDiagnosis: 'Oštećen ležaj. Problem sa runnerom.',
      realCause:
        'Loš kontakt na BNC konektoru akcelerometra. Električni šum. Zvuk turbine normalan, struja generatora stabilna - klasičan false alarm.',
      solution: [
    ...
    this.addTip({
      ...
      rationale:
        'Vibracije na 50Hz uzrokuju da se vijci lagano olabavljuju. Dodatnih 5Nm kompenzira ovo bez rizika od pucanja.',
      author: 'Ervin Stupac',
    ...
    this.addTip({
      ...
      rationale:
        'Termalna ekspanzija kućišta mijenja zazor. Jutarnje mjerenje daje konzistentne rezultate.',
      author: 'Ervin Stupac',
    ...
    this.addTip({
      ...
      rationale:
        'Runner je pod naprezanjem. Brzo skidanje vijaka može dovesti do pucanja preostalih.',
      author: 'Ervin Stupac',
      criticality: 'MUST_FOLLOW',
    });
  ...
  static semanticSearch(query: string, turbineFamily?: string): WTFCase[] {
    ...
    for (const wtfCase of this.cases.values()) {
      ...
      if (
        turbineFamily &&
        wtfCase.turbineFamily !== 'ALL' &&
        wtfCase.turbineFamily !== turbineFamily
      ) {
        continue;
      }
   ...
   */
  static getValidationChecklist(turbineFamily: string, component: string): LegacyValidation | null {
   ...
   */
  static getTipsForProcedure(procedure: string, turbineFamily?: string): OldSchoolTip[] {
  ...
  static getAllCases(): WTFCase[] {
    return Array.from(this.cases.values()).sort((a, b) => b.timesEncountered - a.timesEncountered);
  }
...
// Initialize on load (DISABLED: Call manually to avoid blocking startup)
