...
export async function runBackfill(startDate: string, endDate: string) {
  ...
  try {
    const { data, error } = await supabase.rpc('backfill_eta_aggregates', {
    ...
    try {
      ...
      const { data: sensors } = await supabase
        ...
        .select('sensor_id, asset_id, calibrated')
        .in('sensor_id', sensorIds);
      const calibrated = new Set(
        (sensors || []).filter((s: any) => s.calibrated).map((s: any) => s.sensor_id)
      );
