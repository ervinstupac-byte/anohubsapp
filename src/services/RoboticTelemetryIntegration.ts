...
export class RoboticTelemetryIntegration {
  ...
  public static processMissionResults(
    ...
    const update: TelemetryUpdate = {
      ...
      timestamp: Date.now(),
      source: mission.unitId.includes('ROV')
        ? 'ROV'
        : mission.unitId.includes('UAV')
          ? 'UAV'
          : 'CLEANING_ARM',
    ...
    console.log(
      `  Safety factor adjustment: ${safetyAdjustment > 0 ? '+' : ''}${safetyAdjustment.toFixed(3)}`
    );
  ...
  private static applyToSafetyModel(update: TelemetryUpdate): void {
    ...
    console.log(
      `  Adjustment: ${update.findings.safetyFactorAdjustment > 0 ? '+' : ''}${(update.findings.safetyFactorAdjustment * 100).toFixed(1)}%`
    );
