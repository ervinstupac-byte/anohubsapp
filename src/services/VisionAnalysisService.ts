...
export class VisionAnalysisService {
  ...
  static analyzeInspectionImages(
    ...
    const cavitationImages = images.filter(img =>
      img.aiTags.some(
        tag => tag.toLowerCase().includes('kavitation') || tag.toLowerCase().includes('cavitation')
      )
    );
    ...
    const corrosionImages = images.filter(img =>
      img.aiTags.some(
        tag =>
          tag.toLowerCase().includes('korrosion') ||
          tag.toLowerCase().includes('corrosion') ||
          tag.toLowerCase().includes('rust')
      )
    );

    const erosionImages = images.filter(img =>
      img.aiTags.some(
        tag => tag.toLowerCase().includes('erosion') || tag.toLowerCase().includes('abrasion')
      )
    );

    const crackImages = images.filter(img =>
      img.aiTags.some(
        tag => tag.toLowerCase().includes('crack') || tag.toLowerCase().includes('riss')
      )
    );
    ...
    const result: VisionAnalysisResult = {
      ...
      detectedIssues: {
        ...
        corrosion: {
          ...
          type:
            corrosionImages.length > 2
              ? 'PITTING'
              : corrosionImages.length > 0
                ? 'UNIFORM'
                : 'NONE',
        ...
        cracks: {
          ...
          propagationRisk:
            crackImages.length >= 2 ? 'HIGH' : crackImages.length === 1 ? 'MEDIUM' : 'LOW',
        },
        vibrationDamage: {
          detected: technicalState.physics.boltSafetyFactor < 1.5,
          indicators:
            technicalState.physics.boltSafetyFactor < 1.5
    ...
    return {
      ...
      hydraulics: {
        ...
        flowVelocity:
          ...
          (Math.PI * Math.pow(technicalState.penstock.diameter / 2000, 2)),
