...
export class StrategicPlanningService {
  ...
  static calculateFeasibility(site: SiteParameters): FeasibilityResult {
    try {
      ...
      const fdcSafe =
        site.flowDurationCurve && site.flowDurationCurve.length > 0
          ? site.flowDurationCurve
          : [
              { flow: 10, probability: 0 },
      ...
      if (hNet.lt(30)) {
        ...
        if (usefulFlow.lt(qDesign.mul(0.3))) {
          // If often running part load
          recommendation.count = 2;
      ...
      return {
        ...
        annualProductionMWh: isNaN(totalEnergyKWh.toNumber())
          ? 0
          : totalEnergyKWh.div(1000).toNumber(),
  ...
  static validateImpact(site: SiteParameters): ImpactAnalysis {
    ...
    let lifespan = 50;
    if (
      site.waterQuality !== 'CLEAN' &&
      site.corrosionProtection === 'NONE' &&
      site.pipeMaterial === 'STEEL'
    ) {
      lifespan = 10;
  ...
  static evaluateBid(bid: Bid, site: SiteParameters): BidEvaluation {
    ...
    const maxTheoreticalEta = (
      {
        ...
        PELTON: 92.5,
        CROSSFLOW: 90.0,
      } as Record<TurbineType, number>
    )[bid.turbineType];

    if (bid.efficiencyAtBestPoint > maxTheoreticalEta) {
      risks.push(
        `Sumnjivo visoka efikasnost (${bid.efficiencyAtBestPoint}%). Fizički limit je oko ${maxTheoreticalEta}%. Tražiti IEC 60041 test izvještaj.`
      );
      score -= 20;
    }
    ...
    if (bid.turbineType === 'KAPLAN' && H > 70) {
      risks.push(
        'LEGACY WARNING: Kaplan na padu > 70m nosi ogroman rizik od kavitacije. Potrebna duboka potopljenost (negativna kota).'
      );
      score -= 30;
    }
    ...
    if (bid.turbineType === 'FRANCIS' && H < 20) {
      risks.push(
        'Ekonomska neisplativost: Francis na padu < 20m zahtijeva ogroman spiralni cjevovod.'
      );
      score -= 15;
    }
  ...
  static checkLogistics(
    ...
    if (runnerDiameterM + 0.5 > constraints.tunnelHeight) {
      warnings.push(
        `Rotor (${runnerDiameterM.toFixed(2)}m) je viši od tunela (${constraints.tunnelHeight}m).`
      ); // Needs param logic update in separate ticket if critical
    }
