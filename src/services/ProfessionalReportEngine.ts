...
export const ProfessionalReportEngine = {
  generateTechnicalAudit: (state: TechnicalProjectState, projectID: string = 'ANOHUB-2025-X') => {
    ...
    const hasCavitation =
      (state as any).images?.some((img: any) => img.aiTags.includes('Kavitation')) || false;
...
const drawTechnicalDetails = (doc: jsPDF, state: TechnicalProjectState) => {
  ...
  const data = [
    ...
    [
      'Mechanik',
      'Gemessenes Spiel',
      `${state.mechanical.radialClearance} mm`,
      state.mechanical.radialClearance > 0.5 ? 'KRITISCH' : 'OK',
    ],
...
const drawFooter = (doc: jsPDF, pageNum: number) => {
  ...
  doc.text(
    `AnoHUB Platform | Vertraulicher Bericht | ${new Date().toLocaleDateString()}`,
    MARGIN,
    PAGE_HEIGHT - 10
  );
};
