...
export function calculateIncrementalPID(
  ...
  const dTerm = kd
    .mul(error.minus(previousError.mul(2)).plus(new Decimal(state.integralError)))
    .div(dt);
...
export function optimizeSetpointForBEP(
  ...
  const bepValues = {
    ...
    KAPLAN: { flowRatio: 0.9, headRatio: 1.0, efficiency: 0.93 },
    PELTON: { flowRatio: 0.8, headRatio: 1.0, efficiency: 0.92 },
  };
