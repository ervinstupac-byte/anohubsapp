...
export class ErrorHandlerService {
  ...
  monitorCavitation(telemetry: TelemetryStream): CavitationStatus {
    ...
    if (sigma >= 0.12) {
      ...
      recommendation =
        'Consider reducing load to operate closer to BEP. Monitor for crackling sounds.';
    } else if (sigma >= 0.08) {
      riskLevel = 'DANGER';
      message =
        '🔴 CAVITATION LIKELY! Bubbles are biting the metal. Reduce load or increase tailwater.';
      recommendation =
        'URGENT: Reduce flow to increase σ above 0.10. Increase tailwater level if possible. Inspect runner at next shutdown.';
    } else {
      riskLevel = 'CRITICAL';
      message = '🚨 SEVERE CAVITATION! Runner damage occurring NOW. Emergency action required!';
      recommendation =
        'EMERGENCY: Reduce load immediately! Runner pitting damage is actively occurring. Plan emergency shutdown for inspection.';
    }
  ...
  monitorErosion(
    ...
    if (sediment < 100) {
      ...
      recommendation =
        'Apply hard-facing weld overlay on leading edges. Inspect every 3 months. Install upstream sand excluder if possible.';
    } else {
      riskLevel = 'SEVERE';
      message =
        '🚨 EXTREME SEDIMENT! Rapid material loss. Runner life <2 years. Install sand excluder!';
      recommendation =
        'URGENT: Install settling basin or sand excluder upstream. Reduce operating hours during monsoon. Plan runner replacement within 2 years.';
    }
...
export class DamageLearningService {
  ...
  predictDamageRisk(
    ...
    const cavitationRisk = cavStatus.sigma < 0.1 ? ((0.1 - cavStatus.sigma) / 0.1) * 100 : 0;
