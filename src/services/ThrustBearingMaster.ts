...
export class ThrustBearingMaster extends BaseGuardian {
  ...
  addMeasurement(m: ThrustMeasurement): ThrustAction {
    ...
    const hMin = ThrustBearingMaster.calculateFilmThickness(
      m.axialLoadN,
      m.oilViscosityPas,
      m.shaftSpeedRpm,
      m.padAreaM2 ?? this.defaultPadArea,
      m.radiusM ?? this.defaultRadius,
      m.padTempsC.length
    );
    ...
    if (m.duringStartup && typeof m.jackingPressureBar === 'number') {
      ...
      if (m.jackingPressureBar < minRequired) {
        return {
          action: 'BLOCK_START',
          reason: `Jacking oil pressure low (${m.jackingPressureBar} bar) — blocking startup.`,
        };
      }
    }
    ...
    if (hMin <= this.tripHMin) {
      return {
        ...
        hMin_m: hMin,
        reason: `h_min=${(hMin * 1e6).toFixed(2)} μm below trip threshold (${this.tripHMin * 1e6} μm).`,
      };
    }

    if (hMin <= this.warningHMin) {
      return {
        action: 'LUBRICATION_CRISIS',
        hMin_m: hMin,
        reason: `h_min=${(hMin * 1e6).toFixed(2)} μm below warning (${this.warningHMin * 1e6} μm).`,
      };
    }
    ...
    if (sigma >= this.padMisalignmentThreshold) {
      return {
        action: 'MECHANICAL_MISALIGNMENT',
        sigmaC: sigma,
        reason: `Pad temperature stddev ${sigma.toFixed(2)}°C exceeds ${this.padMisalignmentThreshold}°C.`,
      };
    }

    if (sigma >= this.padDivergenceThreshold) {
      return {
        action: 'PAD_TEMPERATURE_DIVERGENCE',
        sigmaC: sigma,
        reason: `Pad temperature deviation ${sigma.toFixed(2)}°C.`,
      };
    }

    return {
      action: 'NO_ACTION',
      reason: `h_min=${(hMin * 1e6).toFixed(2)} μm, pad sigma=${sigma.toFixed(2)}°C`,
    };
  }
  ...
  getHealthImpactForAction(a: ThrustAction) {
    switch (a.action) {
      case 'SOVEREIGN_TRIP':
        return { overallDelta: -60, details: a.reason };
      case 'LUBRICATION_CRISIS':
        return { overallDelta: -30, details: a.reason };
      case 'MECHANICAL_MISALIGNMENT':
        return { overallDelta: -25, details: a.reason };
      case 'PAD_TEMPERATURE_DIVERGENCE':
        return { overallDelta: -10, details: a.reason };
      case 'BLOCK_START':
  ...
  public getConfidenceScore(
    measurements: ThrustMeasurement[] = [],
    vibrationSeries: number[] = []
  ): number {
    // Not enough data
    if (!measurements || measurements.length < 3) return 40; // low confidence

    // Build series
    const temps = measurements.map(m =>
      m.padTempsC && m.padTempsC.length
        ? m.padTempsC.reduce((s, n) => s + n, 0) / m.padTempsC.length
        : m.padTempsC && typeof m.padTempsC === 'number'
          ? (m.padTempsC as any)
          : m.oilViscosityPas
            ? 0
            : 0
    ...
    const vib =
      vibrationSeries && vibrationSeries.length >= measurements.length
        ? vibrationSeries.slice(-measurements.length)
        : [];
