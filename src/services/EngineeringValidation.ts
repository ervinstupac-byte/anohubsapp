...
const PHYSICAL_LIMITS = {
  ...
  insulationResistance: {
    ...
    max: 10000,
    unit: 'MΩ',
    name: 'validation.fields.insulationResistance',
  },
...
class EngineeringValidationClass extends BaseGuardian {
  ...
  validateField(field: keyof typeof PHYSICAL_LIMITS, value: number): ValidationResult {
    ...
    if (value < limits.min) {
      return {
        ...
        field: fieldName,
        value,
        message: i18n.t('validation.limits.negative', {
          field: fieldName,
          min: limits.min,
          unit: limits.unit,
        }),
    ...
    if (value > limits.max) {
      return {
        ...
        field: fieldName,
        value,
        message: i18n.t('validation.limits.exceeds', {
          field: fieldName,
          value,
          unit: limits.unit,
          max: limits.max,
  ...
  validateAlignment(alignmentMmM: number): ValidationResult {
    ...
    if (alignmentMmM > GOLDEN_STANDARD * 4) {
      // > 0.20 mm/m
      return {
        isValid: true,
        field: i18n.t('validation.fields.alignment'),
        value: alignmentMmM,
        message: i18n.t('validation.alignment.critical', {
          value: alignmentMmM,
          ratio: (alignmentMmM / GOLDEN_STANDARD).toFixed(1),
        }),
        severity: 'error',
      };
    }
...
export const CrossSectorEngine = {
  ...
  predictBearingTempIncrease(
    ...
    return {
      ...
      warning,
      message: warning
        ? i18n.t('validation.cross_sector.thermal_stress', {
            alignment: alignmentMmM.toFixed(3),
            stress: ((stressMultiplier - 1) * 100).toFixed(0),
          })
  ...
  analyzeCrossSectorEffects(state: {
    ...
    if (alignment > this.GOLDEN_ALIGNMENT) {
      ...
      effects.push({
        ...
        stressMultiplier,
        message: i18n.t('validation.cross_sector.thermal_stress', {
    ...
    if (alignment > this.GOLDEN_ALIGNMENT) {
      ...
      effects.push({
        ...
        message: i18n.t('validation.cross_sector.oil_degradation', {
          years: oilImpact.yearsLost.toFixed(1),
        }),
      });
    }
    ...
    if (vibration > 2.8) {
      // ISO 10816 "Satisfactory" threshold
    ...
    if (freqDeviation > 0.5) {
      ...
      effects.push({
        ...
        message: i18n.t('validation.cross_sector.insulation_stress', {
          deviation: freqDeviation.toFixed(2),
        }),
      });
    }
  ...
  getHeritageCertification(state: {
    ...
    if (alignment > this.GOLDEN_ALIGNMENT) {
      reasons.push(
        i18n.t('validation.certification.alignment_fail', { value: alignment.toFixed(3) })
      );
    }
    if (vibration > 1.1) {
      // ISO "Good" threshold
      reasons.push(
        i18n.t('validation.certification.vibration_fail', { value: vibration.toFixed(2) })
      );
    }
    if (temp > 65) {
