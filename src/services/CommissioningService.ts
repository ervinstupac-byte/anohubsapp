...
export class CommissioningService {
  ...
  static async recordAlignmentPoint(
    ...
    if (isHorizontal && session.alignmentData.rotorPositions.length > 4) {
      session.alignmentData.shaftSag = this.calculateShaftSag(session.alignmentData.rotorPositions);
    }
  ...
  private static analyzeAcousticBaseline(
    spectrum: number[]
  ): BaselineFingerprint['acousticSignature'] {
    ...
    const highFreqEnergy = spectrum
      .slice(Math.floor(spectrum.length * 0.7))
   ...
   */
  private static checkPressureLinearity(
