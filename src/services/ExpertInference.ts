  ...
  analyze: (state: TechnicalProjectState): InferenceResult => {
    ...
    if (structuralAudit.status !== 'NORMAL') {
      result.alerts.push({
        ...
        reasoning: i18n.t('expert_inference.alerts.structural_risk', {
          margin: structuralAudit.margin.toFixed(1),
          pressure: structuralAudit.currentPressure.toFixed(1),
          limit: structuralAudit.mawp.toFixed(1),
        }),
    ...
    if (vib > standards.vibration.unsatisfactory) {
      result.alerts.push({
        ...
        severity: 'CRITICAL',
        reasoning: i18n.t('expert_inference.alerts.vibration_critical', {
          value: vib,
          standard: standards.vibration.standard,
          limit: standards.vibration.unsatisfactory,
        }),
    ...
    if (bTemp > standards.bearingTemperature.critical) {
      result.alerts.push({
        ...
        severity: 'CRITICAL',
        reasoning: i18n.t('expert_inference.alerts.bearing_critical', {
          value: bTemp,
          limit: standards.bearingTemperature.critical,
          rise: relativeRise.toFixed(1),
        }),
      ...
      result.alerts.push({
        ...
        severity: 'WARNING',
        reasoning: i18n.t('expert_inference.alerts.bearing_rise', {
          rise: relativeRise.toFixed(1),
          ambient,
        }),
      });
    ...
    if (alignment > golden.alignment.ideal) {
      ...
      result.alerts.push({
        ...
        severity,
        reasoning: i18n.t('expert_inference.alerts.alignment_breach', {
          value: alignment,
          limit: golden.alignment.ideal,
          erosion: (Math.pow(alignment / golden.alignment.ideal, 3) * 10).toFixed(1),
        }),
    ...
    if (axial > golden.axialPlay.max) {
      result.alerts.push({
        ...
        severity: 'CRITICAL',
        reasoning: i18n.t('expert_inference.alerts.axial_breach', {
          value: axial,
          limit: golden.axialPlay.max,
        }),
    ...
    if (megger < golden.insulation.min) {
      result.alerts.push({
        ...
        severity: 'CRITICAL',
        reasoning: i18n.t('expert_inference.alerts.insulation_risk', {
          value: megger,
          limit: golden.insulation.min,
        }),
    ...
    if (waterContent > oilThresholds.waterContent.warning || tanValue > oilThresholds.tan.warning) {
      result.alerts.push({
        ...
        severity: 'CRITICAL',
        reasoning: i18n.t('expert_inference.alerts.oil_chemistry', {
          condition:
            waterContent > 500
              ? `Water (${waterContent} ppm) > 500 ppm`
              : `TAN (${tanValue}) > 0.5`,
        }),
    ...
    const grease = SentinelKernel.checkGreaseRisk(
      isStandby ? 'STBY' : 'RUNNING',
      state.identity.startStopCount % 30
    ); // Use modulo as proxy
    if (grease.risk) {
      result.alerts.push({
    ...
    if (magnetic.risk) {
      ...
      (result.conclusions[result.conclusions.length - 1] as any).vectors = [
        'Stator Delta',
        'Excitation High',
      ];
    }
    ...
    result.recoveryPaths = activeSops
      .map(sopCode => LifeExtensionEngine.getRecoveryPath(sopCode, state))
    ...
    if (result.metrics.fatigueRiskFactor > 48) {
      result.alerts.push({
        ...
        severity: 'CRITICAL',
        reasoning: i18n.t('expert_inference.alerts.fatigue_risk', {
          risk: result.metrics.fatigueRiskFactor,
        }),
 ...
 */
function generateChiefEngineerVerdict(
  ...
  const hasTribologyIssue =
    state.physics?.leakageStatus === 'CRITICAL' || state.physics?.leakageStatus === 'DEGRADING';
