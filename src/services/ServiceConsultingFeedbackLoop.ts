...
export class ServiceConsultingFeedbackLoop {
   ...
   */
  static async markWorkCompleted(trackingId: string, actualCost: number): Promise<void> {
    ...
    console.log(
      `   Delta: ${(((actualCost - entry.investmentCost) / entry.investmentCost) * 100).toFixed(1)}%`
    );
  }
  ...
  private static generateLearnings(entry: OptimizationTrackingEntry): void {
    ...
    if (entry.deltaEfficiency > 1) {
      lessons.push(
        `Over-predicted efficiency gain by ${entry.deltaEfficiency.toFixed(1)}%. Consider more conservative estimates for similar turbine types.`
      );
      entry.correctionFactors.efficiency_multiplier =
        entry.actualEfficiencyGain! / entry.predictedEfficiencyGain;
    }

    // Efficiency under-prediction
    if (entry.deltaEfficiency < -1) {
      lessons.push(
        `Under-predicted efficiency gain by ${Math.abs(entry.deltaEfficiency).toFixed(1)}%. Our intervention was more effective than expected.`
      );
      entry.correctionFactors.efficiency_multiplier =
        entry.actualEfficiencyGain! / entry.predictedEfficiencyGain;
    }
    ...
    if (entry.actualInvestmentCost && entry.actualInvestmentCost > entry.investmentCost * 1.1) {
      const overrun = (
        ((entry.actualInvestmentCost - entry.investmentCost) / entry.investmentCost) *
        100
      ).toFixed(1);
      lessons.push(
        `Cost overrun of ${overrun}%. Factor in more contingency for ${entry.assetName} turbine family.`
      );
    ...
    if (entry.accuracyScore && entry.accuracyScore > 90) {
      lessons.push(
        `🎯 Excellent prediction accuracy (${entry.accuracyScore.toFixed(0)}/100). Our model is well-calibrated for this scenario.`
      );
    }
  ...
  static async generateAccuracyReport(): Promise<AccuracyReport> {
    ...
    // Calculate averages
    const avgDeltaEff =
      measuredEntries.reduce((sum, e) => sum + (e.deltaEfficiency || 0), 0) /
      measuredEntries.length;
    const avgDeltaROI =
      measuredEntries.reduce((sum, e) => sum + (e.deltaROI || 0), 0) / measuredEntries.length;
    const avgAccuracy =
      measuredEntries.reduce((sum, e) => sum + (e.accuracyScore || 0), 0) / measuredEntries.length;

    // Best and worst
    const sorted = [...measuredEntries].sort(
      (a, b) => (b.accuracyScore || 0) - (a.accuracyScore || 0)
    );
    ...
    if (avgAccuracy > 85) {
      ...
      learnings.push(
        '⚠️ Prediction quality needs improvement (<70% accuracy). Review model assumptions.'
      );
    }
    ...
    if (Math.abs(avgDeltaEff) > 0.5) {
      ...
      learnings.push(
        `Systematic bias detected: ${direction} efficiency gains by ${Math.abs(avgDeltaEff).toFixed(1)}%`
      );
    }
    ...
    if (costOverruns > measuredEntries.length * 0.3) {
      learnings.push(
        `High frequency of cost overruns (${costOverruns}/${measuredEntries.length}). Increase contingency budgets.`
      );
    }
  ...
  static async getCorrectionFactors(turbineFamily: string): Promise<{
    ...
    const familyEntries = Array.from(this.trackingDatabase.values()).filter(
      e => e.correctionFactors && Object.keys(e.correctionFactors).length > 0
    );
