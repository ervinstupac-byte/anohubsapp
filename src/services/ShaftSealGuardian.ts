...
export class ShaftSealGuardian extends BaseGuardian {
  ...
  addMeasurement(m: SealMeasurement): SealAction {
    ...
    if (pFail >= 0.85) {
      return {
        action: 'HARD_TRIP',
        probability: pFail,
        reason: `Immediate sovereign shutdown (P_fail=${pFail.toFixed(2)}).`,
      };
    }
  ...
  private estimateFailureProbability(): number {
    ...
    const evidenceScore =
      this.tempWeight * tempRateScore +
      this.flowWeight * flowRateScore +
      0.3 * pressureRateScore +
      this.corrWeight * corrScore;
  ...
  predictLeakageWindow(threshold = 100): number | null {
    ...
    const levels = this.samples
      .map(s => s.leakagePitLevel)
  ...
  public getConfidenceScore(samples: SealMeasurement[] = []): number {
    ...
    let score = this.corrToScore
      ? this.corrToScore(Math.abs(corr))
      : Math.round(((Math.abs(corr) + 1) / 2) * 100);
