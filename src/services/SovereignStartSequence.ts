  ...
  public static tick(
    ...
    if (
      ...
      if (!permissives.readyToStart) {
        ...
        return this.abort(
          `Safety Violation: ${permissives.missingConditions.join(', ')}`,
          permissives
        );
      }
    }
    ...
    switch (this.currentState) {
      ...
      case 'START_AUX_HPU':
        ...
        return this.status(
          `Pressurizing HPU (${hpuStatus.pressureBar.toFixed(0)} bar)...`,
          Math.min(100, (hpuStatus.pressureBar / 130) * 100),
          permissives
        );
      ...
      case 'START_AUX_COOLING':
        ...
        return this.status(
          ...
          Math.min(100, (coolingFlowLps / 50) * 100),
          permissives
        );
      ...
      case 'PREPARE_GOVERNOR':
        ...
        return this.status(
          ...
          ((Date.now() - this.stepStartTime) / 3000) * 100,
          permissives
        );
      ...
      case 'RELEASE_BRAKES':
        ...
        return this.status(
          'Releasing Mechanical Brakes...',
          ((Date.now() - this.stepStartTime) / 2000) * 100,
          permissives
        );
