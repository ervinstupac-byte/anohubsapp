...
import { getLastAutoTableFinalY, getPageCount } from '../utils/pdfHelpers';
...
export class PdfService {
  ...
  private static applyFooter(doc: jsPDF) {
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      // Center: Branding
      doc.text('Generated by AnoHUB Platform V2.5', pageWidth / 2, pageHeight - 10, {
        align: 'center',
      });
  ...
  public static generateAuditReport(
    ...
    if (diagnostics && diagnostics.length > 0) {
      ...
      diagnostics.forEach((diag: any) => {
        ...
        doc.setTextColor(0, 0, 0);
        doc.text(
          `${diag.type === 'critical' ? 'CRITICAL' : 'INFO'}: ${diag.messageKey || diag.message}`,
          25,
          y + 9
        );
        y += 18;
