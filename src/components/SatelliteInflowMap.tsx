...
export const SatelliteInflowMap: React.FC<SatelliteInflowMapProps> = ({ snowCover, assetName }) => {
  return (
    <GlassCard
      title={`Satellite Hydration Analysis - ${assetName}`}
    ...
    >
      <div className="mt-4 aspect-video bg-slate-900 rounded-2xl relative overflow-hidden border border-white/5">
        ...
        <div className="absolute top-4 right-4 flex gap-2">
          <span className="text-[8px] font-black text-cyan-400 bg-black/60 px-2 py-0.5 rounded border border-cyan-500/30">
            SENTINEL-2 LIVE
          </span>
          <span className="text-[8px] font-black text-slate-400 bg-black/60 px-2 py-0.5 rounded">
            SWE SENSOR ACTIVE
          </span>
        </div>
