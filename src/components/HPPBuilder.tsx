...
const TurbineChart: React.FC<{ head: number; flow: number }> = ({ head, flow }) => {
  ...
  return (
    <div className="relative w-full h-72 bg-[#0B0F19] rounded-2xl border border-slate-700/50 overflow-hidden shadow-inner group">
      ...
      ></div>
      <div className="absolute top-[15%] left-[10%] text-slate-600/50 text-xs font-black tracking-[0.2em] rotate-[-15deg] pointer-events-none">
        {t('hppBuilder.chart.peltonZone')}
      </div>
      <div className="absolute top-[50%] left-[40%] text-slate-600/50 text-xs font-black tracking-[0.2em] rotate-[-15deg] pointer-events-none">
        {t('hppBuilder.chart.francisZone')}
      </div>
      <div className="absolute bottom-[20%] right-[20%] text-slate-600/50 text-xs font-black tracking-[0.2em] rotate-[-15deg] pointer-events-none">
        {t('hppBuilder.chart.kaplanZone')}
      </div>
      <div className="absolute left-3 top-3 text-[10px] text-cyan-500/80 font-mono font-bold">
        {t('hppBuilder.chart.headAxis')} ▲
      </div>
      <div className="absolute right-3 bottom-3 text-[10px] text-cyan-500/80 font-mono font-bold">
        {t('hppBuilder.chart.flowAxis')} ►
      </div>
      <div
...
export const HPPBuilder: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (head > 200) {
      ...
      console.log(
        `🎯 CEREBRO: High-head detected (${head}m) - Enforcing strict 0.05mm/m alignment`
      );
    } else {
    ...
    if (specificSpeed > 350) {
      ...
      console.log(
        `🏗️ CEREBRO: Kaplan detected (Nq=${specificSpeed}) - Foundation monitoring active`
      );
    }
  ...
  const recommendations = useMemo((): TurbineRecommendation[] => {
    try {
      ...
      const recs = engines.map(engine => {
        try {
          const { score, reasons } = engine.getRecommendationScore(
            safeHead,
            safeFlow,
            safeFlowVariation,
            safeWaterQuality,
  ...
  const updateSettings = (key: keyof HPPSettings, value: any) => {
    setSettings(prev => {
      ...
      if (key === 'head')
        setHydraulic({ head: Number(value) || DEFAULT_TECHNICAL_STATE.hydraulic.head });
      if (key === 'flow')
        setHydraulic({ flow: Number(value) || DEFAULT_TECHNICAL_STATE.hydraulic.flow });
      return newSettings;
    });
  };
  ...
  const handleGeneratePDF = () => {
    ...
    ForensicReportService.openAndDownloadBlob(
      blob,
      `HPP_Spec_${selectedAsset.name}_${Date.now()}.pdf`,
      true,
      {
  ...
  const handleSaveConfiguration = async () => {
    ...
    try {
      ...
      const assetDbId =
        numericForUpdate !== null ? idAdapter.toDb(numericForUpdate) : selectedAsset.id;
      ...
      try {
        ...
        ProjectStateManager.setState({
          identity: { assetId: internalId, assetName: selectedAsset.name } as any,
          assetPassport: { ...newSpecs } as any,
        });
        loggingService.logAction(internalId, 'DESIGN_UPDATE', {
          configName,
          bestTurbine,
          newSpecs,
        });
  ...
  const handleTurbineSelect = (type: string) => {
    ...
    const newIdentity: AssetIdentity = {
      ...
      fluidIntelligence: {
        oilSystem: {
          oilType: 'MINERAL_ISO_VG_46',
          oilCapacityLiters: 5000,
          currentHours: 0,
          changeIntervalHours: 20000,
      ...
      environmentalBaseline: {
        noiseLevel: {
          operatingDB: 85,
          locations: { powerhouse: 85, turbinePit: 90, controlRoom: 60 },
        ...
        relativeHumidity: 45,
        penstockType: 'STEEL',
        penstockDiameterMM: 2000,
        penstockLengthM: 100,
        penstockThicknessMM: 20,
    ...
    if (turbineType === 'FRANCIS') {
      newIdentity.specializedAdvanced = {
        frontRunnerClearanceMM: 0.35,
        backRunnerClearanceMM: 0.35,
        spiralClearanceMM: 1.2,
        labyrinthGaps: { upperLabyrinthMM: 0.4, lowerLabyrinthMM: 0.4, sealType: 'METALLIC' },
  ...
  return (
    <ErrorBoundary
      fallback={
        <div className="p-8 text-center text-red-500">{t('hppStudio.errors.crashRecovery')}</div>
      }
    >
      <div className="animate-fade-in max-w-7xl mx-auto space-y-8 pb-24">
        ...
        <div className="flex justify-center gap-4 border-b border-white/5 pb-8">
          {steps.map((label, idx) => (
            <div
              key={idx}
              className={`flex items-center gap-3 px-6 py-3 rounded-full border transition-all ${step === idx + 1 ? 'bg-cyan-950/40 border-cyan-500/50 text-cyan-400 shadow-[0_0_20px_rgba(6,182,212,0.15)]' : 'bg-slate-900/40 border-white/5 text-slate-500'}`}
            >
              <span
                className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-black ${step === idx + 1 ? 'bg-cyan-500 text-black' : 'bg-slate-700 text-slate-300'}`}
              >
                {idx + 1}
              </span>
        ...
        {step === 1 && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-fade-in">
              ...
              <div className="space-y-8 p-4">
                <div>
                  <div className="flex justify-between text-xs font-bold text-slate-400 uppercase mb-2">
                    ...
                    <span className="text-cyan-400 font-mono bg-cyan-950/20 px-2 py-0.5 rounded border border-cyan-500/20">
                      {settings.head} m
                    </span>
                  </div>
                  <input
                    ...
                    step="1"
                    value={settings.head}
                    onChange={e => updateSettings('head', parseInt(e.target.value))}
                ...
                <div>
                  ...
                  <input
                    ...
                    step="0.1"
                    value={settings.flow}
                    onChange={e => updateSettings('flow', parseFloat(e.target.value))}
                    className="w-full h-1.5 bg-slate-700 rounded-lg appearance-none cursor-pointer accent-cyan-500"
                  />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <ModernInput
                    label={t('hppStudio.labels.waterType')}
                    as="select"
                  ...
                  <ModernInput
                    label={t('hppStudio.labels.flowProfile')}
                    as="select"
            ...
            <div className="flex flex-col gap-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <StatCard
                  label={t('hppStudio.labels.specificSpeed')}
                  value={calculations.n_sq}
                  subtitle={t('hppStudio.labels.topologyIndex')}
                />
                <StatCard
                  label={t('hppStudio.labels.potentialPower')}
                  value={calculations.powerMW.toFixed(1)}
                  unit="MW"
                  subtitle={t('hppStudio.labels.calculatedCapacity')}
                />
              </div>
              <ModernButton
                onClick={() => setStep(2)}
        ...
        {step === 2 && (
          <div className="animate-fade-in space-y-6">
            <h3 className="text-xl font-bold text-white text-center">
              {t('hppStudio.labels.recommendedMatches')}
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {recommendations.map(rec => (
                <div
                  key={rec.key}
                  onClick={() => {
                    if (rec.score > 0) handleTurbineSelect(rec.key);
                  }}
                  className={`
                ...
                >
                  <div className="flex justify-between items-start mb-4">
                    <h4
                      className={`text-2xl font-black uppercase ${rec.isBest ? 'text-emerald-400' : 'text-slate-300'}`}
                    >
                      {rec.key}
                    </h4>
                    {rec.isBest && (
                      <span className="bg-emerald-500 text-black text-[10px] font-black px-2 py-1 rounded uppercase">
                        {t('hppStudio.labels.bestMatch')}
                      </span>
                    )}
                  </div>
                  <div className="space-y-2 mb-6">
                    {rec.reasons.slice(0, 3).map((r, i) => (
                      <div key={i} className="flex gap-2 text-xs text-slate-400">
                        <span className={r.startsWith('+') ? 'text-emerald-500' : 'text-red-500'}>
                          ●
                        </span>
                        {r.substring(2)}
                      </div>
                  ...
                  <div className="border-t border-white/5 pt-4 flex justify-between items-center">
                    <span className="text-xs text-slate-500 font-mono">
                      {t('hppStudio.labels.matchScore')}
                    </span>
                    <span
                      className={`text-xl font-black ${rec.isBest ? 'text-white' : 'text-slate-500'}`}
                    >
                      {rec.score}%
                    </span>
                  </div>
            ...
            <div className="flex justify-between pt-8">
              <ModernButton onClick={() => setStep(1)} variant="ghost">
                {t('hppStudio.buttons.back')}
              </ModernButton>
              <ModernButton onClick={() => setStep(3)} variant="secondary">
                {t('hppStudio.buttons.skipToExport')}
              </ModernButton>
            </div>
          </div>
        )}
          ...
          <div className="animate-fade-in">
            <div className="mb-6 flex justify-between items-center bg-slate-900/50 p-4 rounded-xl border border-white/5">
              ...
              <div className="flex gap-2">
                <span className="px-3 py-1 rounded bg-amber-500/10 text-amber-500 text-xs font-mono border border-amber-500/20">
                  NC-9.0 COMPLIANT
                </span>
              </div>
            </div>
            ...
            <div className="flex justify-between pt-8">
              <ModernButton onClick={() => setStep(2)} variant="ghost">
                {t('hppStudio.buttons.back')}
              </ModernButton>
              <ModernButton onClick={() => setStep(4)} variant="primary">
                {t('hppStudio.buttons.continue')}
              </ModernButton>
            </div>
          </div>
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <ModernButton
                    ...
                    variant="primary"
                    icon={<span>cloud_upload</span>}
                    fullWidth
                    isLoading={isLoading}
                  >
                    {t('hppStudio.buttons.saveToCloud')}
                  </ModernButton>
                  <ModernButton
                    onClick={handleGeneratePDF}
                    variant="secondary"
                    icon={<span>picture_as_pdf</span>}
                    fullWidth
                  >
                    {t('hppStudio.buttons.generateReport')}
                  </ModernButton>
                </div>
              </div>
            </GlassCard>
            <div className="flex justify-start">
              <ModernButton onClick={() => setStep(3)} variant="ghost">
                {t('hppStudio.buttons.back')}
              </ModernButton>
            </div>
          </div>
