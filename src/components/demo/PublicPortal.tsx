...
export const PublicPortal: React.FC = () => {
  const [scenario, setScenario] = useState<'NOMINAL' | 'CAVITATION' | 'MISALIGNMENT' | 'LOOSENESS'>(
    'NOMINAL'
  );
      ...
      <div className="w-80 border-r border-slate-800 bg-slate-900/50 p-6 flex flex-col">
        ...
        <div className="space-y-6 flex-1">
          ...
          <div className="bg-cyan-950/30 border border-cyan-800 p-4 rounded text-xs text-cyan-200/80 italic leading-relaxed">
            "The Architect sees the invisible. Click a fault to see how the RCA Engine deconstructs
            the physics of failure."
          </div>
        </div>

        <div className="text-[10px] text-slate-600 font-mono text-center">DEMO MODE // NC-200</div>
      </div>
              ...
              {demoDiagnosis ? (
                ...
                <div className="h-96 flex flex-col items-center justify-center border border-slate-800 rounded-xl bg-slate-900/50 border-dashed">
                  ...
                  <p className="text-slate-400 mt-2">
                    No active faults detected across 148 sensors.
                  </p>
                  <p className="text-slate-600 text-sm mt-8 animate-pulse">
                    Waiting for telemetry injection...
                  </p>
                </div>
              )}
            </motion.div>
...
const CheckCircleIcon = ({ active }: { active: boolean }) => (
  <div
    className={`w-4 h-4 rounded-full border flex items-center justify-center ${active ? 'border-green-400 bg-green-400' : 'border-slate-600'}`}
  >
