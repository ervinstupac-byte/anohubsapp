...
const SCENARIOS: Scenario[] = [
  {
    ...
    duration: 15,
    description:
      'Gradual vibration rise + 2x RPM peak + thermal gradient alarm (dT/dt > 0.6°C/min)',
    fieldNote:
      'Always fails at 11 PM on a Friday. When the hot-side coupling expands, the cold-side lags behind.',
  ...
  },
  {
    ...
    fieldNote:
      'If it sounds like gravel in a washing machine, your runner blades are already eroding.',
    execute: (progress, store, prevState) => {
      const now = Date.now();
      const rpm = 375;
      const f0 = rpm / 60;
      ...
      const efficiency =
        baseEfficiency - (baseEfficiency - droppedEfficiency) * Math.min(progress * 2, 1);
...
export const ScenarioControl: React.FC<{ className?: string }> = ({ className = '' }) => {
  ...
  return (
    <div
      className={`bg-slate-900/80 backdrop-blur-sm border border-slate-700/50 rounded-lg overflow-hidden ${className}`}
    >
      {/* Header */}
          ...
          >
            <div className={`p-4 ${currentScenario.bgColor}`}>
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  <div
                    className={`p-1.5 rounded ${currentScenario.bgColor} ${currentScenario.color}`}
                  >
      ...
      <div className="p-3 space-y-2">
        {SCENARIOS.map(scenario => (
          <button
            ...
            className={`w-full p-3 rounded border text-left transition-all group ${
          ...
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div
                    className={`text-xs font-bold ${activeScenario === scenario.id ? scenario.color : 'text-slate-300'}`}
                  >
                    {scenario.name}
                  </div>
      ...
      <div className="px-3 py-2 bg-slate-900/30 border-t border-slate-800 flex items-center justify-between">
        ...
        <div className="flex items-center gap-1">
          <div
            className={`w-1.5 h-1.5 rounded-full ${activeScenario ? 'bg-red-400 animate-pulse' : 'bg-emerald-400'}`}
          />
          <span className="text-[8px] text-slate-600">{activeScenario ? 'ACTIVE' : 'STANDBY'}</span>
        </div>
      </div>
    </div>
  );
