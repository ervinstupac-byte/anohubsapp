...
export const SOPManager: React.FC = () => {
  ...
  const protocols: ViewSOP[] = useMemo(() => {
    ...
    return rawProtocols.map(p => ({
      ...
      steps: p.steps.map(s => ({
        ...
        verificationType:
          s.detail.includes('Measure') || s.detail.includes('Limit') ? 'VALUE' : 'PHOTO',
  ...
  const activeSop = useMemo(
    () => protocols.find(s => s.id === activeSopId),
    [activeSopId, protocols]
  );
  ...
  const activeInsight = useMemo(() => {
      ...
      for (const token of tokens) {
        ...
        if (metric && metric.status === 'critical') {
          return {
            ...
            action:
              'Sentinel Model predicts 12% probability of downstream instability. TRAJECTORY: If current parameters persist, System Trip is predicted in T-minus 4.5 hours.',
            color: 'amber',
          };
        }
      }
    ...
    const isRiskRelevant = tokens
      ? tokens.some(t => t.includes('VIB') || t.includes('PRE'))
      : false;
  ...
  const handleVerifySOP = () => {
    ...
    if (currentStep.verificationType === 'VALUE') {
      ...
      if (verificationInput && verificationInput.length > 0) {
        ...
        const targetVal = currentStep.verificationTarget
          ? parseFloat(currentStep.verificationTarget)
          : NaN;
  ...
  const filteredDossiers = useMemo(() => {
    return DOSSIER_LIBRARY.filter(
      file =>
        file.path.toLowerCase().includes(searchQuery.toLowerCase()) ||
        file.justification.toLowerCase().includes(searchQuery.toLowerCase()) ||
    ...
    <div className="animate-fade-in space-y-8 pb-12">
      <div className="flex justify-between items-end bg-slate-950 p-6 rounded-none border-t border-b border-cyan-900/30 relative overflow-hidden">
        ...
        <div>
          ...
          <p className="text-slate-400 text-sm font-light max-w-lg">
            ...
            <strong className="text-cyan-400">Live Data Injection</strong> and{' '}
            <strong className="text-purple-400">Neural Guidance</strong>.
          </p>
        </div>
      ...
      {!selectedAsset && !showFullLibrary ? (
        <div className="flex justify-center py-20 px-6">
          <GlassCard
            variant="commander"
            className="max-w-md w-full text-center p-12 border-dashed border-white/10"
          >
            ...
            <p className="text-sm text-slate-500 font-mono mb-8 italic">
              Select an asset from the sidebar or click below to browse the verified master archive
              manually.
            </p>
            <ModernButton variant="primary" onClick={() => setShowFullLibrary(true)}>
              Open Master Archive
            </ModernButton>
          </GlassCard>
        </div>
          ...
          {selectedAsset && !protocols.length && !showFullLibrary && (
            <div className="p-4 bg-amber-500/10 border border-amber-500/20 rounded flex items-center gap-3">
              ...
              <span className="text-[10px] font-black text-amber-500 uppercase tracking-widest">
                No Specific Mesh Protocols - Displaying Verified Digital Dossier Library (50 IEC
                60041 Compliant Sources)
              </span>
            </div>
          )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 px-4">
          {protocols.map(sop => (
            ...
            >
              ...
              <p className="text-[10px] text-slate-500 font-mono mb-6 uppercase tracking-wider">
                {sop.targetModule}
              </p>
          ...
          <div className="bg-slate-950 border border-cyan-900/30 rounded-sm overflow-hidden shadow-2xl shadow-black relative">
              ...
              <div className="text-right">
                <span className="text-[24px] font-mono font-bold text-slate-700 leading-none">
                  ...
                  <span className="text-lg opacity-50">
                    /{String(activeSop?.steps.length).padStart(2, '0')}
                  </span>
                </span>
              </div>
            </div>
            ...
            <div className="h-0.5 w-full bg-slate-800">
              <div
                ...
                style={{
                  width: `${((currentStepIndex + 1) / (activeSop?.steps.length || 1)) * 100}%`,
                }}
              />
            </div>
            ...
            <div className="p-8 space-y-8 relative z-10">
              ...
              {currentStep?.requiredTool && (
                <div className="ml-11 p-4 bg-slate-900 border-l-2 border-amber-500/50 flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-[9px] text-amber-500 font-black uppercase tracking-widest mb-0.5">
                      Required Tool
                    </p>
              ...
              {activeInsight && (
                <div
                  className={`ml-11 p-4 border-l-2 rounded-r-sm flex items-start gap-4 mb-4 animate-fade-in ${
                    activeInsight.color === 'purple'
                      ? 'bg-purple-950/20 border-purple-500'
                      : 'bg-amber-950/20 border-amber-500'
                  }`}
                >
                  <div
                    className={`p-2 rounded-full ${activeInsight.color === 'purple' ? 'bg-purple-500/10 text-purple-400' : 'bg-amber-500/10 text-amber-500'}`}
                  >
                    <AlertTriangle className="w-5 h-5" />
                  </div>
                  <div>
                    <h5
                      className={`text-sm font-bold uppercase tracking-wider mb-1 ${activeInsight.color === 'purple' ? 'text-purple-400' : 'text-amber-500'}`}
                    >
                      {activeInsight.message}
                    </h5>
                    <p
                      className={`text-xs font-mono opacity-80 ${activeInsight.color === 'purple' ? 'text-purple-300' : 'text-amber-300'}`}
                    >
              ...
              <div className="ml-11 pt-8 border-t border-white/5">
                ...
                {currentStep?.verificationType === 'VALUE' ? (
                  ...
                  <div className="flex gap-4">
                    <button
                      className={`flex-grow border-2 border-dashed border-slate-700 h-24 flex flex-col items-center justify-center gap-2 rounded-sm transition-colors ${
                        isStepVerified
                          ? 'border-emerald-500 bg-emerald-950/10'
                          : activeInsight
                            ? 'hover:border-amber-500 hover:bg-amber-950/10'
              ...
              <div className="pt-8 flex justify-between gap-4 ml-11">
                ...
                <button
                  className={`px-8 py-3 bg-purple-600 hover:bg-purple-500 text-white font-bold uppercase text-xs tracking-widest rounded-sm shadow-lg shadow-purple-900/20 transition-all hover:translate-y-[-1px] disabled:opacity-50 disabled:translate-y-0 disabled:shadow-none ${
                    !isStepVerified ? 'opacity-50 cursor-not-allowed grayscale' : ''
                  }`}
                  disabled={!isStepVerified} // Next Step requires verification, but verification is NOT blocked
                  onClick={handleNextStep}
                >
                  {currentStepIndex === (activeSop?.steps.length || 0) - 1
                    ? 'Complete Protocol'
                    : 'Next Step'}
                </button>
              </div>
