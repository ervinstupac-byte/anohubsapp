...
export const AssetOnboardingWizard: React.FC<{
  ...
  const [frontClearance, setFrontClearance] = useState(0.4);
  const [backClearance, setBackClearance] = useState(0.4);
  const [upperLabyrinth, setUpperLabyrinth] = useState(0.4);
  const [lowerLabyrinth, setLowerLabyrinth] = useState(0.4);
  ...
  const handleComplete = () => {
    ...
    if (showFrancisModule) {
      ...
      const balance = AssetIdentityService.calculateAxialThrustBalance(
        identity.specializedAdvanced
      );
    ...
    identity.sensorMatrix.upgradeRecommendations =
      AssetIdentityService.generateUpgradeRecommendations(
        identity.sensorMatrix,
        identity.machineConfig
      );

    // Calculate HPU Health
    identity.fluidIntelligence.healthScore = AssetIdentityService.calculateHPUHealth(
      identity.fluidIntelligence
    );
    ...
    addAsset(newAsset)
      ...
      .catch(err => {
        ...
        try {
          ...
          alert(
            'Offline Mode: Asset created locally. Sync will occur when connection is restored.'
          );
        } catch (e) {
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between max-w-4xl mx-auto">
            <StepIndicator
              ...
              complete={currentStep !== 'general'}
              icon={<MapPin />}
              label="General"
            />
            <div className="flex-1 h-1 bg-slate-800 mx-2" />
            <StepIndicator
              active={currentStep === 'physical'}
              complete={currentStep !== 'physical' && currentStep !== 'general'}
              icon={<Settings />}
              label="Physical"
            />
            <div className="flex-1 h-1 bg-slate-800 mx-2" />
            <StepIndicator
              active={currentStep === 'sensors'}
              complete={
                currentStep !== 'sensors' && currentStep !== 'physical' && currentStep !== 'general'
              }
              icon={<Cpu />}
              label="Sensors"
            />
            ...
            <div className="flex-1 h-1 bg-slate-800 mx-2" />
            <StepIndicator
              active={currentStep === 'hydraulics'}
              icon={<Droplets />}
              label="Hydraulics"
            />
            <div className="flex-1 h-1 bg-slate-800 mx-2" />
            <StepIndicator
              active={currentStep === 'environmental'}
              icon={<Mountain />}
              label="Environment"
            />
          </div>
        ...
        <AnimatePresence mode="sync">
              ...
              <div className="space-y-6">
                ...
                <div>
                  <label className="block text-sm font-bold mb-3">
                    Penstock Material (affects water hammer)
                  </label>
              ...
              {manualInspectionRequired && (
                <div className="mb-6 p-4 bg-amber-950/30 border border-amber-500/50 rounded">
                  ...
                  <p className="text-sm text-amber-200 mt-2">
                    No vibration sensors detected. Asset will be marked for manual inspection during
                    service checklists.
                  </p>
                </div>
              )}
          ...
          {currentStep === 'francis' && showFrancisModule && (
            <motion.div
              ...
              className="max-w-2xl mx-auto"
            >
              <h2 className="text-3xl font-bold mb-2 text-[#2dd4bf]">
                Francis Specific Parameters
              </h2>
                  ...
                  <>
                    <div className="p-4 bg-blue-950/20 border border-blue-500/30 rounded mb-4">
                      ...
                      <p className="text-sm text-blue-300 mt-1">
                        Vertical orientation detected - jacking parameters required
                      </p>
                    </div>
                    <MeasurementInput
          ...
          {currentStep === 'review' && (
            ...
            >
              ...
              <div className="space-y-4 bg-slate-900 border border-slate-700 rounded p-6">
                ...
                <ReviewItem
                  label="Generator Vib Sensor"
                  value={hasGeneratorVibSensor ? 'Installed' : 'Not Installed'}
                />
                <ReviewItem
                  label="Turbine Vib Sensor"
                  value={hasTurbineVibSensor ? 'Installed' : 'Not Installed'}
                />
                {showJackingModule && (
                  <>
                ...
                <ReviewItem
                  label="Sludge Cleaner"
                  value={hasSludgeCleaner ? 'YES' : 'NO (Erosion +5)'}
                />
...
const SensorCheckbox: React.FC<{
  ...
  <div
    ...
    className={`p-4 rounded cursor-pointer transition-all ${
      checked
        ? 'bg-emerald-950/30 border-2 border-emerald-500'
