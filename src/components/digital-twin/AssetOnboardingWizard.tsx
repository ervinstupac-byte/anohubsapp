...
import { ChevronRight, ChevronLeft, Check, AlertCircle, Settings, Cpu, Gauge, Droplets, Mountain, X } from 'lucide-react';
...
export const AssetOnboardingWizard: React.FC<{ isOpen?: boolean; onClose: () => void; onComplete?: () => void }> = ({ isOpen = true, onClose, onComplete }) => {
    ...
    const handleComplete = async () => {
        ...
        try {
            await addAsset({
                name: identity.assetName || `Unit-${Math.floor(Math.random() * 1000)}`,
                type: 'HPP',
                location: 'AnoHub Command',
                coordinates: [44.81, 15.87],
                capacity: 10,
                status: 'Operational',
                turbine_type: turbineType,
                specs: {
                    machineConfig: identity.machineConfig,
                    sensorMatrix: identity.sensorMatrix,
                    fluidIntelligence: identity.fluidIntelligence
                }
            });
    ...
    return createPortal(
        <div className="fixed inset-0 z-[9999] bg-black/95 overflow-y-auto">
            ...
            <div className="fixed inset-0 z-0 bg-gradient-to-b from-[#0a0a0a]/70 via-[#0a0a0a]/90 to-[#0a0a0a]" />

            <div className="min-h-screen text-white p-6 relative z-10">
                <button 
                    onClick={onClose}
                    className="absolute top-6 right-6 p-2 bg-slate-800/50 hover:bg-slate-700 rounded-full transition-colors z-50"
                >
                    <X className="w-6 h-6" />
                </button>
            {/* Progress Steps */}
