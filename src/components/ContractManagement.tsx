...
export const ContractManagement: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (riskState.isAssessmentComplete) {
      if (riskState.criticalFlags > 0) {
        ...
        newStatus.breach_reason = t('contractManagement.logic.breachReasonCritical', {
          flags: riskState.criticalFlags,
        });
  ...
  const generateLegalNotice = () => {
    ...
    doc.setFontSize(18);
    doc.text(t('contractManagement.pdf.noticeTitle', 'NOTICE OF NON-COMPLIANCE'), 105, 20, {
      align: 'center',
    });
    ...
    doc.text(
      `${t('contractManagement.pdf.date', 'Date:')} ${new Date().toLocaleDateString()}`,
      20,
      50
    );
    ...
    doc.text(
      t(
        'contractManagement.pdf.intro1',
        'Pursuant to the Master Service Agreement, AnoHUB system has detected'
      ),
      20,
      80
    );
    doc.text(
      t('contractManagement.pdf.intro2', 'deviations from the mandatory Standard of Excellence.'),
      20,
      86
    );
    ...
    if (contract.breach_reason) {
      ...
      doc.text(
        `${t('contractManagement.pdf.violation', 'VIOLATION:')} ${contract.breach_reason}`,
        20,
        100
      );
    ...
    doc.text(
      `${t('contractManagement.pdf.penalty', 'Estimated Contract Penalty:')} EUR ${contract.penalty_amount.toLocaleString()}`,
      20,
      120
    );
    ...
    <div className="animate-fade-in pb-12 max-w-7xl mx-auto space-y-8">
      ...
      <div className="text-center space-y-4">
        <h2 className="text-4xl font-black text-white tracking-tighter">
          {t('contractManagement.title', 'Smart Contract')}{' '}
          <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-500">
            {t('contractManagement.titleHighlight', 'Intelligence')}
          </span>
        </h2>
      ...
      {!selectedAsset ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-fade-in-up">
          ...
          >
            <div
              className={`p-10 text-center relative h-full flex flex-col justify-center ${
                contract.status === 'BREACHED'
                  ? 'bg-red-950/30'
                  : contract.status === 'WARNING'
                    ? 'bg-amber-950/30'
            ...
            >
              ...
              <h3 className="text-5xl font-black text-white mb-2 tracking-tight">
                {t(
                  `contractManagement.statusCodes.${contract.status.toLowerCase()}`,
                  contract.status
                )}
              </h3>
              <p className="text-xs uppercase tracking-[0.3em] text-slate-400 font-bold mb-8">
                {t('contractManagement.contractStanding')}
              </p>

              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-900/60 p-4 rounded-xl border border-white/5">
                  <p className="text-[10px] text-slate-500 uppercase font-bold mb-1">
                    {t('contractManagement.warrantyStatus')}
                  </p>
                  <p
                    className={`font-bold text-lg ${contract.warranty_valid ? 'text-emerald-400' : 'text-red-400'}`}
                  >
                    {contract.warranty_valid
                      ? t('contractManagement.valid')
                      : t('contractManagement.void')}
                  </p>
                </div>
                <div className="bg-slate-900/60 p-4 rounded-xl border border-white/5">
                  <p className="text-[10px] text-slate-500 uppercase font-bold mb-1">
                    {t('contractManagement.penaltyAccrued')}
                  </p>
              ...
              {contract.breach_reason && (
                <div className="mt-6 p-4 bg-red-500/10 border border-red-500/30 rounded-xl text-left animate-pulse">
                  <p className="text-[10px] font-bold text-red-400 uppercase mb-1">
                    {t('contractManagement.detectedViolation')}
                  </p>
          ...
          <div className="space-y-6">
            {/* LIVE FEED PANEL */}
            <GlassCard
              title={t('contractManagement.liveRiskFeed')}
              subtitle={t('contractManagement.dataSource')}
            >
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-center p-3 bg-slate-900 rounded-lg border border-white/5">
                    <span className="text-xs text-slate-400">
                      {t('contractManagement.calcRiskScore')}
                    </span>
                    <span className="text-sm font-mono font-bold text-white">
                      {riskState.riskScore}/100
                    </span>
                  </div>
                  <div className="flex justify-between items-center p-3 bg-slate-900 rounded-lg border border-white/5">
                    <span className="text-xs text-slate-400">
                      {t('contractManagement.criticalFlags')}
                    </span>
                    <span
                      className={`text-sm font-mono font-bold ${riskState.criticalFlags > 0 ? 'text-red-500' : 'text-emerald-500'}`}
                    >
            ...
            <GlassCard className="bg-gradient-to-br from-slate-900 to-slate-800">
              <div className="text-center mb-6">
                <h3 className="text-lg font-bold text-white mb-2">
                  {t('contractManagement.legalEnforcement')}
                </h3>
                <p className="text-xs text-slate-400">
                  {t('contractManagement.legalEnforcementDesc')}
                </p>
              </div>
              ...
              >
                {contract.status === 'ACTIVE'
                  ? t('contractManagement.genComplianceCert')
                  : t('contractManagement.genNoticeBreach')}
              </ModernButton>
            </GlassCard>
          </div>
        </div>
