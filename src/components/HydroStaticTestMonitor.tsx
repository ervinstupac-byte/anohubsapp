...
export const HydroStaticTestMonitor: React.FC<HydroStaticTestMonitorProps> = ({
  sessionId,
  onComplete,
}) => {
  ...
  const currentPressure =
    readings.length > 0 ? readings[readings.length - 1].pressure : initialPressure;
  const elapsedTime = readings.length > 0 ? readings[readings.length - 1].time : 0;
  const dropRate =
    readings.length > 10 ? ((readings[0].pressure - currentPressure) / elapsedTime) * 60 : 0;
              ...
              {testResult.suspectedIssue && (
                <div className="mt-2 p-2 bg-red-950/30 rounded">
                  ...
                  <p className="text-xs text-slate-300">
                    Sumnja na{' '}
                    {testResult.suspectedIssue === 'MICROCRACK'
                      ? 'mikropukotinu'
                      : testResult.suspectedIssue === 'AIR_IN_SYSTEM'
                        ? 'zrak u sistemu'
                        : 'curenje brtvila'}
                  </p>
                </div>
...
const MetricCard: React.FC<{
  ...
  return (
    <div
      className={`p-3 bg-slate-800/30 rounded-lg border border-slate-700/50 ${isLive ? 'animate-pulse' : ''}`}
    >
...
const PressureGraph: React.FC<{
  ...
  const yScale = (pressure: number) =>
    height -
    padding -
    ((pressure - minPressure) / (maxPressure - minPressure)) * (height - 2 * padding);
  ...
  return (
    <div className="p-4 bg-slate-900/50 rounded-lg border border-slate-700/50">
      ...
      <svg width="100%" height={height} viewBox={`0 0 ${width} ${height}`}>
        ...
        <path
          d={readings
            .map((r, i) => `${i === 0 ? 'M' : 'L'} ${xScale(r.time)} ${yScale(r.pressure)}`)
            .join(' ')}
          fill="none"
          stroke="#06b6d4"
          strokeWidth="3"
        ...
        {/* Data points */}
        {readings.map((r, i) => (
          <circle key={i} cx={xScale(r.time)} cy={yScale(r.pressure)} r="3" fill="#06b6d4" />
        ))}
      </svg>
    </div>
  );
