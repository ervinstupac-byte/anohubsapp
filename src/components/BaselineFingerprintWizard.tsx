...
export const BaselineFingerprintWizard: React.FC<BaselineFingerprintWizardProps> = ({
  ...
  const recordBaseline = async (loadLevel: number) => {
    ...
    if (recordedLoads.size === 4) {
      // Will be 5 after this one
      onComplete();
    }
  };
    ...
    <GlassCard>
      ...
      <div className="p-4 bg-cyan-950/20 border border-cyan-500/30 rounded-lg">
        ...
        <ol className="text-xs text-slate-300 space-y-1 list-decimal list-inside">
          <li>
            Set turbine to <strong>0% load (idle)</strong> and click "Record Baseline"
          </li>
          <li>Wait for recording to complete (3 seconds)</li>
          <li>
            Gradually increase load to <strong>25%</strong>, stabilize, then record
          </li>
          <li>
            Repeat for <strong>50%</strong>, <strong>75%</strong>, and <strong>100%</strong> load
          </li>
        ...
        >
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">
              Your turbine's "healthy state" fingerprint is complete.
            </p>
          </div>
        </motion.div>
      )}
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-3">
        <Icon
          className={`w-5 h-5 ${
            isRecorded ? 'text-emerald-400' : isRecording ? 'text-cyan-400' : 'text-slate-500'
          }`}
        />
        <span
          className={`text-xs font-bold px-2 py-1 rounded ${
            isRecorded ? 'bg-emerald-500/30 text-emerald-300' : 'bg-slate-700/50 text-slate-400'
          }`}
        >
