...
export const PeltonJetVisualizer: React.FC<PeltonJetVisualizerProps> = ({
  sessionId,
  asset,
  onComplete,
}) => {
  ...
  const recommendations =
    jetAnalyses.length > 0 && forceBalance
            ...
            <div className="flex gap-2">
              {[2, 4, 6].map(count => (
                <button
                  ...
                  className={`flex-1 px-4 py-3 rounded-none font-bold transition-all ${
                    nozzleCount === count
            ...
            <div className="p-4 bg-slate-800/30 rounded-lg border border-slate-700/50">
              ...
              <div className="grid grid-cols-3 gap-4 mb-4">
                <div className="text-center">
                  ...
                  <p
                    className={`text-2xl font-black ${
                      forceBalance.imbalanceRatio > 0.15
                        ? 'text-red-400'
                        : forceBalance.imbalanceRatio > 0.08
                          ? 'text-amber-400'
                          : 'text-emerald-400'
                    }`}
                  >
                ...
                <div className="text-center">
                  ...
                  <p
                    className={`text-2xl font-black ${
                      forceBalance.bearingLoadIncrease > 15
                        ? 'text-red-400'
                        : forceBalance.bearingLoadIncrease > 8
                          ? 'text-amber-400'
                          : 'text-emerald-400'
                    }`}
                  >
      ...
      <div className="space-y-2 text-xs">
        ...
        <div className="flex justify-between">
          ...
          <span className="text-white font-bold">
            {acousticSignature.whistleIndex.toFixed(1)}/10
          </span>
        </div>
        <div className="flex justify-between">
...
const ForceVectorDiagram: React.FC<{
  ...
  return (
    <div className="p-4 bg-slate-900/50 rounded-lg">
      <p className="text-xs text-slate-400 uppercase font-bold mb-2 text-center">
        Force Vector Diagram
      </p>
      <svg width="300" height="300" viewBox="0 0 300 300" className="mx-auto">
        ...
        {resultantForce.magnitude > 0 && (
          <line
            ...
            x2={
              centerX +
              Math.cos(((resultantForce.angle - 90) * Math.PI) / 180) *
                (resultantForce.magnitude / maxForce) *
                radius
            }
            y2={
              centerY +
              Math.sin(((resultantForce.angle - 90) * Math.PI) / 180) *
                (resultantForce.magnitude / maxForce) *
                radius
            }
            stroke="#f59e0b"
...
const RecommendationCard: React.FC<{ recommendation: string }> = ({ recommendation }) => {
  const severity = recommendation.startsWith('🔴')
    ? 'CRITICAL'
