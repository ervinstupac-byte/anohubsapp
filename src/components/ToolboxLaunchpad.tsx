...
export const ToolboxLaunchpad: React.FC = () => {
  ...
  const lossTracer = useMemo(() => {
    const observedEffRaw = Number(
      hydraulic?.efficiency ?? physics?.efficiency ?? totalUnitEfficiency.current * 100
    );
  ...
  return (
    <div className="h-full w-full flex flex-col p-4 lg:p-6 overflow-hidden bg-slate-950/20 font-sans text-slate-300 relative">
      {/* INDUSTRIAL BACKGROUND LAYERS — CSS-only (offline-safe) */}
      <div className="absolute inset-0 pointer-events-none z-[-1]">
        {/* Brushed metal: repeating fine lines */}
        <div className="absolute inset-0 surface-brushed opacity-40" />
        {/* Hex/grid: CSS-only grid overlay */}
        <div className="absolute inset-0 surface-grid opacity-60" />
        {/* Scanlines: CRT effect */}
        <div className="absolute inset-0 surface-scanlines opacity-30" />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-4 lg:gap-6 shrink-0 mb-4 lg:mb-6">
        ...
        <div
          className="md:col-span-4 h-32 lg:h-36 cursor-pointer hover:scale-[1.02] transition-all duration-200 active:scale-95 z-10"
          onClick={() => navigate('/executive')}
        >
          <div
            className={`h-full border border-white/5 rounded-xl p-4 bevel-industrial relative overflow-hidden ${
              lossTracer.status === 'critical'
                ? 'bg-red-950/20 border-red-500/30'
                : lossTracer.status === 'warning'
          ...
          >
            ...
            <div className="relative z-10 h-full flex flex-col justify-between">
              <div className="flex justify-between items-start">
                <div>
                  <div className="text-[clamp(7px,0.8vh,9px)] font-black text-slate-500 uppercase tracking-[0.3em] mb-1">
                ...
                <div
                  className={`w-8 h-8 rounded-full flex items-center justify-center ${
                    totalUnitEfficiency.current >= 0.9
                      ? 'bg-emerald-500/20 border-emerald-500/30'
                      : totalUnitEfficiency.current >= 0.8
              ...
              <div className="flex justify-between items-end">
                <div className="text-[clamp(6px,0.7vh,8px)] font-mono text-slate-400">
                  η<sub>turbine</sub>:{' '}
                  {Math.max(0, Math.min(100, totalUnitEfficiency.turbine)).toFixed(1)}%
                </div>
                <div className="text-[clamp(6px,0.7vh,8px)] font-mono text-slate-400">
        ...
        <div
          className="md:col-span-4 h-32 lg:h-36 cursor-pointer hover:scale-[1.02] transition-all duration-200 active:scale-95 z-10"
          onClick={() => navigate('/executive/finance')}
        >
          <div
            className={`h-full border border-white/5 rounded-xl p-4 bevel-industrial relative overflow-hidden ${
              lossTracer.status === 'critical'
          ...
          >
            ...
            <div className="relative z-10 h-full flex flex-col justify-between">
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <div className="text-[clamp(24px,3vh,36px)] font-black text-white tabular-nums flex items-baseline">
                    ...
                    <span className="text-[clamp(10px,1.2vh,14px)] font-mono text-slate-400 ml-1">
                      /h
                    </span>
                  </div>
                </div>
              ...
              <div className="flex justify-between items-end">
                <div className="text-[clamp(6px,0.7vh,8px)] font-mono text-slate-400">
                  {lossTracer.isRope
                    ? `Vortex Active (+€${lossTracer.vortexTax.toFixed(0)}/h)`
                    : 'Normal Operation'}
                </div>
                <div className="text-[clamp(6px,0.7vh,8px)] font-mono text-slate-400">
          ...
          >
            ...
            <div className="h-full w-full flex items-center justify-center p-2">
              <div className="flex flex-col items-center gap-2 text-center">
                <div className="text-[clamp(8px,1vh,10px)] font-black text-slate-500 uppercase tracking-[0.3em]">
                  STRATEGIC COMMAND
                </div>
                <div className="flex items-center gap-2">
                  <div
                    className={`w-2 h-2 rounded-full ${systemStatus.operational ? 'bg-emerald-400 animate-pulse' : 'bg-slate-600'}`}
                  />
        ...
        >
          <div
            className={`h-full border border-white/5 rounded-xl p-4 bevel-industrial relative overflow-hidden ${
              store.sovereignPulse.globalStatus === 'OPTIMAL'
                ? 'bg-emerald-950/20 border-emerald-500/30'
                : store.sovereignPulse.globalStatus === 'STRESSED'
                  ? 'bg-amber-950/20 border-amber-500/30'
                  : store.sovereignPulse.globalStatus === 'CRITICAL'
                    ? 'bg-red-950/20 border-red-500/30'
                    : 'bg-slate-900/40'
            }`}
          >
      ...
      <div className="flex-1 min-h-0 grid grid-rows-2 gap-4 lg:gap-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6 min-h-0">
          <div
            className="cursor-pointer hover:scale-[1.02] transition-all duration-200 active:scale-95 h-full z-10 min-h-0"
            onClick={() => navigate('/executive')}
          >
            <MetricCard
          ...
          <div
            className="cursor-pointer hover:scale-[1.02] transition-all duration-200 active:scale-95 h-full z-10 min-h-0 overflow-hidden"
            onClick={() => navigate('/maintenance/hydraulic')}
          >
            <LabyrinthCard />
          </div>
          <div
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 lg:gap-6 min-h-0 overflow-hidden">
          <div
            className="lg:col-span-3 h-full cursor-pointer hover:scale-[1.02] transition-all duration-200 active:scale-95 z-10 min-h-0"
            onClick={() => navigate('/precision-audit')}
          >
            <ActionCard />
          </div>
          <div
            className="lg:col-span-6 h-full cursor-pointer hover:scale-[1.01] transition-all duration-200 active:scale-95 z-10 min-h-0 overflow-hidden"
            onClick={() => navigate('/executive')}
          >
          ...
          <div
            className="lg:col-span-3 h-full cursor-pointer hover:scale-[1.02] transition-all duration-200 active:scale-95 z-10 min-h-0 overflow-hidden"
      ...
      <div className="shrink-0 pt-4 lg:pt-6 flex flex-col gap-4">
        ...
        >
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="w-px h-4 bg-white/10 shrink-0" />
            <div
              className={`text-[clamp(7px,0.8vh,8px)] font-mono font-black ${
                lossTracer.status === 'critical'
                  ? 'text-red-400'
                  : lossTracer.status === 'warning'
                    ? 'text-amber-400'
            ...
            >
              ...
              {lossTracer.isRope && lossTracer.vortexTax > 0
                ? ` (+€${lossTracer.vortexTax.toFixed(0)}/h Vortex Tax)`
                : ''}
            </div>
          </div>
        </motion.div>
