    ...
    return (
        <>
            ...
            <button
                ...
                className={`flex items-center gap-2 pl-1 pr-3 py-3 bg-slate-900/90 border-y border-r border-brand-500/50 rounded-r-xl shadow-xl backdrop-blur-md transition-all duration-300 hover:pl-2 ${isOpen ? 'translate-x-0' : '-translate-x-[calc(100%-10px)] hover:translate-x-[calc(100%-15px)]'}`}
            >
                <div className="h-24 w-1 bg-brand-500 rounded-full" />
                <div className="vertical-text flex flex-col items-center gap-4 py-2">
                    <Settings className={`w-5 h-5 text-brand-400 ${isOpen ? 'rotate-90' : ''} transition-transform duration-500`} />
                    {isOpen && (
                                ...
                                return (
                                    <div
                                        ...
                                        className={`group relative w-10 h-10 flex items-center justify-center rounded-lg border transition-all cursor-pointer
                                            ${isActive
                                                ? 'bg-brand-500/20 border-brand-400 shadow-[0_0_15px_rgba(6,182,212,0.3)]'
                                                : 'bg-slate-800/50 border-slate-700 hover:border-brand-500/50 hover:bg-slate-700'
                                            }
                                            ${isSwitching ? 'opacity-50 cursor-wait' : ''}
                                        `}
                                        title={`Switch to ${type}`}
                                    >
                                        <span className={`text-[10px] font-black uppercase tracking-wider ${isActive ? 'text-brand-300' : 'text-slate-400'}`}>
                                            {type.slice(0, 1)}
                                        </span>
            ...
            {/* Status Indicator */}
            {isSwitching && (
                <div className="absolute left-16 top-1/2 -translate-y-1/2 bg-black/80 text-brand-400 text-xs px-3 py-1 rounded-full border border-brand-500/30 flex items-center gap-2 animate-pulse whitespace-nowrap">
