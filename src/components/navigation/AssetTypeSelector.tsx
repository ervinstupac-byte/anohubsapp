      ...
      <div className="fixed left-0 top-1/2 -translate-y-1/2 z-20">
        ...
        >
          ...
          <div className="vertical-text flex flex-col items-center gap-4 py-2">
            <Settings
              className={`w-5 h-5 text-cyan-400 ${isOpen ? 'rotate-90' : ''} transition-transform duration-500`}
            />
            {isOpen && (
              <div className="flex flex-col gap-3 animate-fade-in">
                {(Object.keys(TURBINE_CONSTANTS) as TurbineType[]).map(type => {
                  ...
                  return (
                    ...
                    >
                      <span
                        className={`text-[10px] font-black uppercase tracking-wider ${isActive ? 'text-cyan-300' : 'text-slate-400'}`}
                      >
                        {type.slice(0, 1)}
                      </span>
