...
export const ForensicDepthAnalyzer: React.FC = () => {
  ...
  const correlationMatrix = useMemo(() => {
    ...
    events.push({
      time: -50,
      label: 'WICKET_GATE_CLOSE_CMD',
      detail: 'Hydro-governor initiates emergency shutdown.',
    });
    ...
    if (pressureJump !== -1) {
      events.push({
        time: pressureJump - incidentCenter,
        label: 'HYDRAULIC_SURGE_DETECTED',
        detail: `dp/dt exceeded 10 bar/s at T+${pressureJump - incidentCenter}ms.`,
      });
    }
    ...
    if (failure !== -1) {
      events.push({
        time: failure - incidentCenter,
        label: 'SYSTEM_RUPTURE',
        detail: 'Critical pressure loss. Mechanical integrity compromised.',
      });
    }
    ...
    <GlassCard title="Forensic Depth Analyzer (Root Cause Matrix)">
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <span className="text-[10px] bg-red-600 px-2 py-0.5 rounded font-black text-white uppercase animate-pulse">
            MS-Snap Active
          </span>
          <button
        ...
        <div className="pt-4 border-t border-white/5">
          ...
          <div className="flex justify-between mt-2">
            ...
            <span className="text-[10px] font-black text-white">
              REPLAY CURSOR: {replayIndex - Math.floor(frozenBuffer!.length / 2)}ms
            </span>
            <span className="text-[9px] font-mono text-slate-600">+30,000 ms</span>
          </div>
        </div>
        ...
        <div className="p-3 bg-red-500/10 rounded-xl border border-red-500/20">
          <p className="text-[9px] font-black text-red-500 uppercase mb-1">
            Ano-Agent transient Analysis
          </p>
          <p className="text-[10px] text-slate-300 italic">
            "Odstupanje u dinamici hidraulike: provjeriti viskozitet ulja ili zagušenje na
            prigušnicama."
          </p>
        </div>
      </div>
