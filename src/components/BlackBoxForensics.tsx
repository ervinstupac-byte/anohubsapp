...
export const BlackBoxForensics: React.FC = () => {
  ...
  return (
    <GlassCard
      title="Black Box Forensics (High-Speed Log)"
      className="border-l-4 border-l-red-600 bg-black/40 shadow-[0_0_30px_rgba(220,38,38,0.1)]"
    >
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-3 gap-2">
          <div className="bg-black/40 p-2 rounded border border-white/5">
            ...
            <p className="text-lg font-mono font-black text-red-500">
              {currentPoint.cylinderPressure.toFixed(1)}{' '}
              <span className="text-[8px] text-slate-600">bar</span>
            </p>
          </div>
          <div className="bg-black/40 p-2 rounded border border-white/5">
            ...
            <p className="text-lg font-mono font-black text-cyan-400">
              {currentPoint.actuatorPosition.toFixed(1)}{' '}
          ...
          <div className="bg-black/40 p-2 rounded border border-white/5">
            ...
            <p className="text-lg font-mono font-black text-amber-500">
              {currentPoint.oilTemperature.toFixed(1)}{' '}
              <span className="text-[8px] text-slate-600">°C</span>
            </p>
          </div>
        </div>
        ...
        <div className="bg-red-500/5 border border-red-500/20 p-3 rounded-xl">
          <div className="flex justify-between items-center mb-2">
            ...
            <span className="text-[9px] font-mono font-black text-red-500">
              dp/dt: {currentPoint.dpdt.toFixed(2)} bar/s
            </span>
          </div>
          <p className="text-[10px] text-slate-400 italic leading-snug">
            {currentPoint.dpdt > 15
              ? 'CRITICAL: Non-linear pressure surge detected. Pipe rupture imminent at T+' +
                msOffset +
                'ms.'
        ...
        <div className="bg-red-600/10 p-2 rounded flex items-center justify-center gap-2 border border-red-600/30">
          ...
          <span className="text-[8px] font-black text-red-400 uppercase tracking-widest">
            AR REPLAY MODE AVAILABLE ON GEAR
          </span>
        </div>
      </div>
    </GlassCard>
