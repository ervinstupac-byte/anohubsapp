...
export const SmartTooltip: React.FC<SmartTooltipProps> = ({ term, children }) => {
  ...
  return (
    ...
    >
      <span
        className={`${educationMode ? 'underline decoration-dotted decoration-sky-500/50' : ''}`}
      >
        {children || term}
      </span>
      ...
      {(educationMode || isHovered) && (
        <Info
          className={`w-3 h-3 ${educationMode ? 'text-sky-400' : 'text-slate-600 group-hover:text-sky-500'} transition-colors`}
        />
      )}
        ...
        {isHovered && (
          ...
          >
            <div className="flex items-center gap-2 mb-1 pb-1 border-b border-white/10">
              ...
              <span className="text-[10px] text-slate-500 px-1.5 py-0.5 bg-slate-800 rounded-none border border-slate-700">
                NC-11400
              </span>
            </div>
