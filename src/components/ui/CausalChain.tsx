  ...
  if (!analysis) {
    return (
      <div className={`tactical-module p-6 ${className}`}>
        <div className="text-center text-slate-500 text-sm">No root cause analysis available</div>
      </div>
    );
  }
        ...
        <div className="space-y-2">
          {analysis.causalChain.map((event: CausalEvent, index: number) => (
            <div key={event.id} className="relative">
              ...
              {/* Event Card */}
              <div
                className={`flex items-start gap-3 p-2 rounded-sm border ${getEventColor(event.eventType)} bg-slate-900/40`}
              >
