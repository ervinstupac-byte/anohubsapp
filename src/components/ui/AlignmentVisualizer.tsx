...
export const AlignmentVisualizer: React.FC<AlignmentVisualizerProps> = ({
  ...
  return (
    <div
      ...
      className="relative bg-slate-950 rounded-full border border-white/10 shadow-2xl overflow-hidden flex items-center justify-center"
    >
      {/* Grid Circles */}
      <div className="absolute inset-0 flex items-center justify-center">
        ...
        >
          {!isOutsideGolden && (
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="text-[10px] font-black text-cyan-400 uppercase tracking-tighter opacity-40">
                Precision
              </div>
            </div>
          )}
      ...
      >
        <div
          className={`w-1 h-1 rounded-full ${isOutsideGolden ? 'bg-red-500' : 'bg-emerald-400'}`}
        />
      </motion.div>
      ...
      <div className="absolute bottom-6 left-6 right-6 flex justify-between items-end z-40">
        <div>
          ...
          <div
            className={`text-xl font-black tabular-nums italic ${isOutsideGolden ? 'text-red-500' : 'text-emerald-400'}`}
          >
            {alignment.toFixed(3)}
          </div>
        </div>
        <div className="text-right">
          ...
          <div
            className={`text-[10px] font-black uppercase tracking-widest ${isOutsideGolden ? 'text-amber-500' : 'text-cyan-400'}`}
          >
