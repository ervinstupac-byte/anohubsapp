...
const variantConfig: Record<
  EmptyStateVariant,
  { icon: typeof Radio; defaultTitle: string; defaultDesc: string; color: string }
> = {
  telemetry: {
    icon: Radio,
    ...
    return (
      <div className={`flex flex-col items-center justify-center p-8 text-center ${className}`}>
        ...
        <motion.div
          ...
          className={`mb-4 p-4 rounded-full border border-white/10 ${
            variant === 'guided'
              ? 'bg-cyan-950/30'
              : variant === 'maintenance'
                ? 'bg-amber-950/30'
        ...
        {/* Description */}
        <p className="text-xs text-slate-500 max-w-xs">{displayDesc}</p>
