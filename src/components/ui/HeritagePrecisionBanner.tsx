...
/**
 * HERITAGE PRECISION BANNER (NC-9.0)
 *
...
export const HeritagePrecisionBanner: React.FC<HeritagePrecisionBannerProps> = ({
  ...
  const accentColor = isCritical ? 'text-red-400' : isDegraded ? 'text-amber-400' : 'text-cyan-400';
    ...
    >
      ...
      <div className="relative z-10 p-6 md:p-8 flex flex-col md:flex-row items-center justify-between gap-6">
        ...
        <div className="flex items-center gap-6">
          <div
            className={`p-4 rounded-xl border ${isNominal ? 'border-cyan-500/30 bg-cyan-500/10' : 'border-amber-500/30 bg-amber-500/10'}`}
          >
          ...
          <div>
            <div className="flex items-baseline gap-3">
              <span
                className={`text-4xl md:text-5xl font-black font-mono tracking-tighter ${accentColor}`}
              >
                0.05
              </span>
              <span className="text-xl md:text-2xl font-bold text-slate-400 font-mono">mm/m</span>
            </div>
        ...
        <div className="flex flex-col items-center text-center px-6 border-x border-white/5">
          ...
          <div
            className={`mt-2 flex items-center gap-2 px-3 py-1 rounded-full ${isNominal ? 'bg-cyan-500/10' : 'bg-amber-500/10'}`}
          >
          ...
          {isNominal ? (
            ...
            <div>
              <div className="flex items-baseline gap-1 justify-end">
                <span
                  className={`text-2xl md:text-3xl font-black font-mono ${isCritical ? 'text-red-400' : 'text-amber-400'}`}
                >
                  -{longevityLeakPercent.toFixed(1)}%
                </span>
              </div>
              <span
                className={`text-[10px] font-bold uppercase tracking-wider ${isCritical ? 'text-red-400' : 'text-amber-400'}`}
              >
          ...
          <div className="mt-3 w-40 h-2 bg-slate-800 rounded-full overflow-hidden ml-auto">
            <div
              className={`h-full transition-all duration-500 ${
                isNominal ? 'bg-cyan-500' : isDegraded ? 'bg-amber-500' : 'bg-red-500'
              }`}
              style={{
                width: `${Math.min(100, (GOLDEN_STANDARD / Math.max(currentAlignment, 0.001)) * 100)}%`,
              }}
            />
          </div>
        </div>
      ...
      <div className="border-t border-white/5 px-6 py-3 bg-black/20">
        <p className="text-[10px] md:text-xs text-slate-500 font-mono text-center italic">
          "
          {t(
            'heritage.ethicStatement',
            'Anything beyond 0.05 mm/m is not an error — it is an ethical deviation from engineering excellence.'
          )}
          "
        </p>
      </div>
