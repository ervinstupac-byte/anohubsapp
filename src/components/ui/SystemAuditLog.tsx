...
export const SystemAuditLog: React.FC<{ maxEntries?: number; className?: string }> = ({
  ...
  useEffect(() => {
    ...
    getAuditLogs(maxEntries).then(records => {
      if (records.length > 0) {
        ...
        addLog(
          'SYSTEM',
          `Restored ${records.length} audit records from local black box`,
          'success'
        );
      }
    });
    ...
    const handleSovereignLog = (e: Event) => {
      ...
      if (customEvent.detail) {
        ...
        addLog(
          ...
          r.active_protection !== 'NONE' ? 'warning' : 'info'
        );
      }
    };
    ...
    if (baselineState) {
      addLog(
        'DNA',
        `Turbine DNA verified - Commissioned ${new Date(baselineState.commissioningDate).toLocaleDateString()}`,
        'success'
      );
    } else {
    ...
    addLog(
      'FILTER',
      `Signal filter: ${isFilteredMode ? `${filterType} active` : 'Bypassed (Raw mode)'}`,
      'info'
    );
  ...
  return (
    <div
      className={`bg-slate-950 border border-slate-800 rounded-none overflow-hidden ${className}`}
    >
      {/* Header */}
        ...
        {logs.map((log, i) => {
          ...
          return (
            ...
            >
              ...
              <span className={`shrink-0 ${sourceConfig[log.source].color}`}>[{log.source}]</span>
              <span
                className={`${
                  log.type === 'success'
                    ? 'text-emerald-400'
        ...
        <div className="flex items-center gap-3">
          {['SYSTEM', 'PLC', 'RCA', 'DNA'].map(source => (
            <div key={source} className="flex items-center gap-1">
              <div
                className={`w-1 h-1 rounded-none ${sourceConfig[source as keyof typeof sourceConfig].color.replace('text-', 'bg-')}`}
              />
