  ...
  ({
    ...
    if (isWaiting || isInvalidValue) {
      return (
        <span className={`inline-flex items-baseline ${className}`}>
          {currency && (
            <span className={`metric-currency ${sizeStyle.value} text-slate-500`}>{currency}</span>
          )}
          <span className={`data-pending ${sizeStyle.value}`}>—</span>
          {unit && <span className={`metric-unit ${sizeStyle.unit} text-slate-500`}>{unit}</span>}
        </span>
      );
    }
    ...
    return (
      <span className={`inline-flex items-baseline ${className}`}>
        ...
        {unit && <span className={`metric-unit ${sizeStyle.unit}`}>{unit}</span>}
      </span>
    );
  }
);
