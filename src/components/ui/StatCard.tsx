...
export const StatCard: React.FC<StatCardProps> = React.memo(
  ({ label, value, unit, trend, icon, subtitle, loading = false }) => {
    if (loading) {
      return (
      ...
      <GlassCard className="relative group overflow-hidden">
        ...
        {(trend || subtitle) && (
          <div className="mt-4 flex items-center justify-between border-t border-white/5 pt-3">
            {trend ? (
              <div
                className={`flex items-center gap-1 text-[10px] font-bold uppercase tracking-tight ${trend.isPositive ? 'text-emerald-400' : 'text-red-400'}`}
              >
            ...
            {subtitle && (
              <span className="text-[10px] text-slate-500 font-medium italic">{subtitle}</span>
            )}
          </div>
        )}
      </GlassCard>
