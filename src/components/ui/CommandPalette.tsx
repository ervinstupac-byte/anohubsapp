...
export const CommandPalette = React.memo(() => {
  ...
  const { validateTask } = useValidationStore();
  ...
  const modules: CommandResult[] = [
    {
      ...
      subtitle: 'High-level KPI overview',
    },
    {
      id: 'mod-2',
      label: 'HPP Builder',
  ...
  const systemActions: CommandResult[] = [
    {
      ...
      action: () => {
        confirm({
          title: 'System Reset',
          message:
            'Are you sure you want to clear the local cache and reload the application? Unsaved data may be lost.',
          variant: 'danger',
  ...
  const results = useMemo(() => {
    ...
    // Filter Actions
    const actionResults = systemActions.filter(
      a =>
            ...
            >
              ...
              <div className="flex items-center gap-3 px-4 py-4 border-b border-slate-700 bg-slate-900/50">
                ...
                <div className="hidden sm:flex items-center gap-1">
                  <span className="text-xs text-slate-500 bg-slate-800 px-1.5 py-0.5 rounded-none border border-slate-700">
                    ESC
                  </span>
                </div>
              </div>
              ...
              <div className="px-4 py-2 bg-slate-950 border-t border-slate-800 text-[10px] text-slate-500 flex justify-between">
                <div className="flex gap-3">
                  <span>
                    <strong className="text-slate-400">↑↓</strong> to navigate
                  </span>
                  <span>
                    <strong className="text-slate-400">↵</strong> to select
                  </span>
                </div>
                <div>Sovereign Command v2.4</div>
              </div>
            </motion.div>
          </motion.div>
        )}
