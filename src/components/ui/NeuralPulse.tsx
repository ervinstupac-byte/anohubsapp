  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 h-12 bg-slate-950/95 backdrop-blur-sm border-t border-cyan-900/20 z-40">
      <div className="h-full px-6 flex items-center justify-between">
          ...
          {Object.entries(weights)
            ...
            .map(([pattern, weight]) => (
              <div key={pattern} className="flex items-center gap-1.5">
                ...
                <span className="text-[8px] font-mono text-slate-400">{pattern.slice(0, 12)}</span>
              </div>
            ))}
        </div>
        ...
        <div className="flex items-center gap-2">
          <div
            className={`w-1.5 h-1.5 rounded-full ${hiveStatus?.connected ? 'bg-green-500' : 'bg-red-500'}`}
          />
