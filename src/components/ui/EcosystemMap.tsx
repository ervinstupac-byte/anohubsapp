  ...
  return (
    <div className="w-full h-[400px] relative bg-slate-900/50 rounded-xl border border-white/5 overflow-hidden backdrop-blur-sm p-4">
      ...
      <svg className="absolute inset-0 w-full h-full pointer-events-none opacity-30">
        ...
        />
        {/* Operations -> Design */}
        <path d="M 45% 55% Q 35% 65% 25% 75%" stroke="transparent" />{' '}
        {/* Hidden return path logic visual only */}
        {/* Operations -> Analytics */}
        <path
          d="M 55% 45% Q 65% 35% 75% 25%"
          stroke="white"
          strokeWidth="2"
          strokeDasharray="5,5"
          markerEnd="url(#arrow)"
      ...
      {nodes.map(node => (
        <motion.button
          ...
          whileHover={{ y: -5 }}
        >
          <div className="p-2 rounded-full bg-slate-950/50 border border-white/10">{node.icon}</div>
          <div>
