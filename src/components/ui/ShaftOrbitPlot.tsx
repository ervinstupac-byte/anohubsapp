...
export const ShaftOrbitPlot = forwardRef<ShaftOrbitPlotHandle, ShaftOrbitPlotProps>(
  ({ vibrationX, vibrationY, size = 180, baselinePoints, centerPath, onAnalysis }, ref) => {
    ...
    useEffect(() => {
      ...
      const normalize = (val: number) => (val / 0.5) * radius;
      ...
      const eccentricity = a > 0 ? Math.sqrt(1 - Math.pow(b, 2) / Math.pow(a, 2)) : 0;
      const isElliptical = eccentricity > 0.75;

      ctx.strokeStyle = isElliptical ? '#ef4444' : '#22d3ee';
      ctx.stroke();
      ...
      // Arrow head
      const angle = Math.atan2(peakY - center, peakX - center);
      ctx.beginPath();
      ctx.moveTo(peakX, peakY);
      ctx.lineTo(
        peakX - 10 * Math.cos(angle - Math.PI / 6),
        peakY - 10 * Math.sin(angle - Math.PI / 6)
      );
      ctx.moveTo(peakX, peakY);
      ctx.lineTo(
        peakX - 10 * Math.cos(angle + Math.PI / 6),
    ...
    }, [vibrationX, vibrationY, size, onAnalysis]);

    return (
      <div className="relative group">
        <canvas
          ref={canvasRef}
          className="rounded-lg bg-slate-900/40 border border-white/5 shadow-2xl"
        />

        <div className="absolute top-2 left-2 flex flex-col gap-1">
          <span className="text-[10px] font-bold text-cyan-400 font-mono">
            X: {vibrationX.toFixed(3)}mm
          </span>
          <span className="text-[10px] font-bold text-cyan-400 font-mono">
            Y: {vibrationY.toFixed(3)}mm
          </span>
        </div>
