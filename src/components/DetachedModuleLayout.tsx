...
const ScadaCore = React.lazy(() =>
  import('./dashboard/ScadaCore').then(m => ({ default: m.ScadaCore }))
);
const ExecutiveWarRoom = React.lazy(() =>
  import('./dashboard/ExecutiveWarRoom').then(m => ({ default: m.ExecutiveWarRoom }))
);
const FinancialHealthPanel = React.lazy(() =>
  import('./dashboard/FinancialHealthPanel').then(m => ({ default: m.FinancialHealthPanel }))
);
const ForensicDashboard = React.lazy(() =>
  import('./forensics/ForensicDashboard').then(m => ({ default: m.ForensicDashboard }))
);
    ...
    <div className="w-screen h-screen bg-slate-950 text-slate-200 overflow-auto flex flex-col relative">
      ...
      <div className="fixed top-4 right-4 z-50 flex items-center gap-3 pointer-events-auto opacity-90 hover:opacity-100 transition-opacity">
        ...
        <div className="flex items-center gap-2 bg-slate-900/50 px-3 py-1.5 rounded-full border border-slate-800 backdrop-blur-sm">
          <div
            key={lastSync}
            className={`w-2 h-2 rounded-full ${lastSync > 0 ? 'bg-green-500 shadow-[0_0_8px_#22c55e]' : 'bg-slate-700'}`}
          />
