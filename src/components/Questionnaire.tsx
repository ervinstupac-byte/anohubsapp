...
export const Questionnaire: React.FC<QuestionnaireProps> = ({ onShowSummary, onRiskSync }) => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      const numeric = idAdapter.toNumber(selectedAsset.id);
      const assetDbId = numeric !== null ? idAdapter.toDb(numeric) : undefined;
      const { error } = await supabase.from('diagnostic_drafts').upsert(
        {
          user_id: user.id,
          asset_id: assetDbId,
          answers: answers,
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === 'Enter') {
        ...
        e.preventDefault();
        setFocusedOptionIndex(prev => (prev < currentQ.options.length - 1 ? prev + 1 : 0));
      } else if (e.key === 'ArrowUp' || e.key === 'ArrowLeft') {
        e.preventDefault();
        setFocusedOptionIndex(prev => (prev > 0 ? prev - 1 : currentQ.options.length - 1));
      }
    };
    ...
    <div className="max-w-3xl mx-auto">
      ...
      <div className="mb-8">
        <div className="flex justify-between text-xs text-slate-400 uppercase font-bold tracking-widest mb-2">
          ...
          <span>
            {Math.round(progress)}% {t('questionnaire.completed', 'Complete')}
          </span>
        </div>
      ...
      <GlassCard className="min-h-[500px] flex flex-col justify-between border-t-4 border-t-cyan-500">
                  ...
                  <div className="flex items-center justify-between relative z-10">
                    <div className="flex items-center gap-4">
                      ...
                      <span className="text-lg font-medium">
                        {t(`questions.${currentQ.id}.options.${index}`, option)}
                      </span>
                    </div>
        ...
        <div className="mt-12 pt-8 border-t border-white/5 flex justify-between items-center">
          ...
          >
            {isLastQuestion
              ? t('questionnaire.finalize', 'Finalize & Analyze')
              : t('questionnaire.next', 'Next Parameter')}
          </ModernButton>
        </div>
      </GlassCard>
    </div>
