...
const HERITAGE_INSIGHTS: Record<string, { title: string; desc: string }> = {
  // Mechanical
  runout: {
    title: 'The 0.05 mm/m Law',
    desc: 'Alignment exceeding 0.05 mm/m is an ethical failure. Precision anchors structural longevity.',
  },
  bearingClearance: {
    title: 'Cubic Wear Rule',
    desc: 'Doubling clearance increases dynamic impact by factor of 8 (2^3). maintain tight tolerances.',
  },
  axialPlay: {
    title: 'Thrust Management',
    desc: 'Excessive play (>0.15mm) causes hammer-effect on thrust pads during load rejection.',
  },
  governorDeadband: {
    title: 'Frequency Stability',
    desc: 'Deadband > 0.05Hz causes grid oscillation. Keep tight for island-mode stability.',
  },
  runnerGap: {
    title: 'Volumetric Sealing',
    desc: 'Each 0.1mm gap increase loses ~0.5% efficiency. Water must work, not bypass.',
  },
  labyrinthGap: {
    title: 'parasitic Flow',
    desc: 'Worn labyrinths bypass the runner, wasting potential energy directly to the draft tube.',
  },
  lastAlignmentCheck: {
    title: 'Seasonal Drift',
    desc: 'Concrete foundations shift with temperature. Re-align every season transition.',
  },

  // Electrical
  statorInsulation: {
    title: 'Dielectric Health',
    desc: 'Megger < 100MΩ indicates moisture ingress. Dry-out required to prevent flashover.',
  },
  rotorInsulation: {
    title: 'Field Integrity',
    desc: 'Low rotor insulation risks excitation failure. Clean carbon dust regularly.',
  },
  polarizationIndex: {
    title: 'Age vs Moisture',
    desc: 'PI < 2.0 suggests brittle insulation or water absorption. PI < 1.0 is immediate danger.',
  },
  dcBatteryVoltage: {
    title: 'The Last Defense',
    desc: 'During blackout, this voltage fires the emergency lube pump. Critical safety node.',
  },
  lastRelayTest: {
    title: 'Silent Sentinel',
    desc: 'Protection relays must be tested annually. A stuck relay guarantees catastrophic failure.',
  },

  // Auxiliary
  sealLeakageRate: {
    title: 'Run-Dry Risk',
    desc: 'Leakage cools the seal. Zero leakage is dangerous (burning); too much floods the pit.',
  },
  oilViscosity: {
    title: 'Fluid Film Wedge',
    desc: 'Viscosity drops 50% for every 10°C rise. Maintain 46 cSt at 40°C for hydrodynamic lift.',
  },
  oilAge: {
    title: 'Chemical Breakdown',
    desc: 'Oxidized oil becomes acidic, eating white metal bearings from the inside out.',
  },
  lastOilChange: {
    title: 'Fresh Blood',
    desc: 'Change oil before TAN > 0.5. Clean oil extends bearing life by 300%.',
  },
  vibrationSensors: {
    title: 'The Nervous System',
    desc: 'Accelerometers detect faults months before audible failure. Trust the trend.',
  },
  frequencySensors: {
    title: 'Grid Pulse',
    desc: 'Captures islanding events and load rejection spikes.',
  },
  acousticObservation: {
    title: 'The Song of Steel',
    desc: 'Grinding sounds = metal contact. Thumping = draft tube surge. Listen to the machine.',
  },

  // Pressure
  penstock: {
    title: 'Artery Pressure',
    desc: 'Head pressure drives the system. Sudden drops indicate rupture or massive leak.',
  },
  labyrinthFront: {
    title: 'Upper Seal',
    desc: 'High pressure here indicates worn lower labyrinth. Water is bypassing the runner.',
  },
  labyrinthRear: {
    title: 'Thrust Relief',
    desc: 'This pressure pushes up against the rotor weight. Balance is key.',
  },
  spiralCasing: {
    title: 'Containment Vessel',
    desc: 'Hoop stress is highest here. Watch for hairline cracks in the weld seams.',
  },
  draftTube: {
    title: 'Vacuum Recovery',
    desc: 'Negative pressure recovers energy. Loss of vacuum = 5-10% power loss.',
  },

  // Kinematics
  mivOpeningTime: {
    title: 'Hydraulic shock',
    desc: 'Opening too fast causes water hammer in the penstock.',
  },
  mivClosingTime: {
    title: 'Surge Protection',
    desc: 'Closing too fast bursts pipes. Closing too slow overspeeds the generator.',
  },
  distributorCylinderStrokeTime: {
    title: 'Governor Response',
    desc: 'Sluggish cylinders cause grid instability and hunting.',
  },
  bypassType: {
    title: 'Pressure Relief',
    desc: 'Synchronous bypass prevents water hammer during load rejection.',
  },
};
...
export const TechnicalPassport: React.FC = () => {
  ...
  const enhancedAsset = useMemo(
    () => (selectedAsset ? mapAssetToEnhancedAsset(selectedAsset) : null),
    [selectedAsset]
  );
  ...
  const profile = useMemo(
    () =>
      activeProfile ||
      (enhancedAsset ? ProfileLoader.getProfile(enhancedAsset.turbine_family) : undefined),
    [enhancedAsset, activeProfile]
  );

  // Initial state based on plugin fields or existing data
  const [passportData, setPassportData] = useState<AssetPassport>(
    selectedAsset?.assetPassport || {
      mechanical: {
        runout: 0,
        bearingClearance: 0,
        axialPlay: 0,
      ...
      auxiliary: {
        ...
        oilAge: 0,
        lastOilChange: '',
        vibrationSensors: false,
        frequencySensors: false,
        acousticObservation: 'Nominal',
      },
      pressureProfile: {
        penstock: 0,
        labyrinthFront: 0,
      ...
      kinematics: {
        mivOpeningTime: 0,
        mivClosingTime: 0,
        distributorCylinderStrokeTime: 0,
        bypassType: 'Manual',
      },
    }
  );
  ...
  const calculations = useMemo(() => {
    ...
    if (profile?.math.formulas.calculateVolumetricLoss) {
      totalLoss = profile.math.formulas.calculateVolumetricLoss(
        { assetPassport: passportData } as any,
        profile.math.constants
      );
    } else {
    ...
    if (runout > 0.1 || axialPlay > 0.25 || (acousticObservation as string) === 'Grinding')
      risk = 'High';
      ...
      <GlassCard className="p-6 border-cyan-500/30">
        <div className="flex justify-between items-start">
          <div>
            ...
            <p className="text-cyan-200 text-xs font-mono mt-1">
              {selectedAsset.name} • {String(selectedAsset.id).toUpperCase()} •{' '}
        ...
        <div className="grid grid-cols-3 gap-4 mt-6">
          <div className="p-4 bg-slate-900/50 rounded-lg border border-slate-700">
            ...
            <div
              className={`text-2xl font-black ${calculations.totalLoss > 2 ? 'text-red-400' : 'text-emerald-400'}`}
            >
          ...
          <div className="p-4 bg-slate-900/50 rounded-lg border border-slate-700">
            ...
            <div
              className={`text-2xl font-black ${calculations.risk === 'High' ? 'text-red-500' : calculations.risk === 'Medium' ? 'text-amber-500' : 'text-emerald-500'}`}
            >
          ...
          <div className="p-4 bg-slate-900/50 rounded-lg border border-slate-700">
            ...
            <div
              className={`text-2xl font-black ${calculations.isSurgeSafe ? 'text-cyan-400' : 'text-red-400'}`}
            >
