...
import type {
  TurbineDetail as TurbineDetailType,
  TurbineComponent,
} from '../data/turbineDetailData.ts';
...
const CriticalityBadge: React.FC<{ level: 'High' | 'Medium' | 'Low' }> = ({ level }) => {
  ...
  switch (level) {
    case 'High':
      style =
        'bg-red-500/10 text-red-400 border-red-500/20 shadow-[0_0_15px_rgba(248,113,113,0.25)]';
      icon = `⚠️ ${t('turbineDetail.badges.critical')}`;
      break;
    case 'Medium':
      style =
        'bg-amber-500/10 text-amber-400 border-amber-500/20 shadow-[0_0_15px_rgba(251,191,36,0.2)]';
      icon = `⚡ ${t('turbineDetail.badges.attention')}`;
      break;
    case 'Low':
      style =
        'bg-emerald-500/10 text-emerald-400 border-emerald-500/20 shadow-[0_0_15px_rgba(52,211,153,0.2)]';
  ...
  return (
    <span
      className={`text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-full border backdrop-blur-md ${style}`}
    >
      {icon}
    </span>
  );
...
const ComponentCard: React.FC<TurbineComponent> = ({ name, description, criticality }) => (
  ...
  >
    ...
    <div className="flex justify-between items-start mb-3 relative z-10">
      <h4
        className={`text-lg font-bold tracking-tight ${criticality === 'High' ? 'text-white' : 'text-slate-200 group-hover:text-white'}`}
      >
        {name}
      </h4>
      ...
      <div className="flex items-center justify-center min-h-[50vh]">
        <GlassCard className="text-center p-12 border-red-500/30 max-w-lg">
          ...
          <p className="text-slate-400 mb-8">
            {t('turbineDetail.notFoundDesc')}{' '}
            <span className="text-cyan-400 font-mono bg-cyan-900/30 px-2 py-1 rounded border border-cyan-500/30">
              '{turbineKey}'
            </span>
          </p>
    ...
    <div className="animate-fade-in pb-12 space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="animate-slide-in-left">
          <GlassCard
            ...
            action={
              <span className="text-2xl p-2 bg-cyan-500/10 rounded-lg text-cyan-400">⚙️</span>
            }
          >
        ...
        <div className="animate-slide-in-right" style={{ animationDelay: '100ms' }}>
          <GlassCard
            ...
            action={
              <span className="text-2xl p-2 bg-purple-500/10 rounded-lg text-purple-400">⚡</span>
            }
          >
            <div className="space-y-4 mt-2">
      ...
      <div className="animate-fade-in" style={{ animationDelay: '200ms' }}>
        <GlassCard
          ...
          subtitle={t(
            'turbineDetail.coolingSystemSubtitle',
            'Real-time Heat Exchanger Performance'
          )}
          className="border-l-4 border-l-emerald-500"
          action={
            <span className="text-2xl p-2 bg-emerald-500/10 rounded-lg text-emerald-400">💧</span>
          }
        >
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-4">
            <div className="bg-slate-900/40 p-5 rounded-2xl border border-white/5 space-y-4">
              <h4 className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">
                {t('turbineDetail.coolingPressure', 'Pressure Dynamics')}
              </h4>
              <div className="flex justify-between items-end">
                <div>
                  <p className="text-[9px] text-slate-400 uppercase font-bold">
                    {t('turbineDetail.inlet', 'Inlet')}
                  </p>
                  <p className="text-2xl font-black text-white">
                    4.2 <span className="text-xs text-slate-500">bar</span>
                  </p>
                </div>
                <div className="text-right">
                  <p className="text-[9px] text-slate-400 uppercase font-bold">
                    {t('turbineDetail.outlet', 'Outlet')}
                  </p>
                  <p className="text-2xl font-black text-white">
                    3.8 <span className="text-xs text-slate-500">bar</span>
                  </p>
                </div>
              </div>
            ...
            <div className="bg-slate-900/40 p-5 rounded-2xl border border-white/5 space-y-4">
              <h4 className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">
                {t('turbineDetail.coolingTemp', 'Thermal Gradient')}
              </h4>
              <div className="flex justify-between items-end">
                <div>
                  <p className="text-[9px] text-slate-400 uppercase font-bold">
                  ...
                  <p className="text-2xl font-black text-white">
                    12.4 <span className="text-xs text-slate-500">°C</span>
                  </p>
                </div>
                <div className="text-right">
                  <p className="text-[9px] text-slate-400 uppercase font-bold">
                  ...
                  <p className="text-2xl font-black text-white">
                    24.6 <span className="text-xs text-slate-500">°C</span>
                  </p>
                </div>
              </div>
              <div className="h-1.5 w-full bg-slate-800 rounded-full overflow-hidden">
            ...
            <div className="bg-slate-900/60 p-5 rounded-2xl border-2 border-emerald-500/20 flex flex-col justify-center items-center text-center">
              <h4 className="text-[10px] font-bold text-emerald-400 uppercase tracking-widest mb-2">
                {t('turbineDetail.deltaT', 'SYSTEM DELTA T')}
              </h4>
              ...
              <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/30">
                ...
                <span className="text-[10px] font-bold text-emerald-400 uppercase">
                  {t('turbineDetail.optimal', 'OPTIMAL')}
                </span>
              </div>
              <p className="text-[9px] text-slate-500 mt-2 font-medium italic">
                {t(
                  'turbineDetail.coolingEfficiencyNote',
                  'Efficiency within TurbineFactory parameters (+/- 0.5%)'
                )}
              </p>
            </div>
          </div>
