...
export const DigitalIntegrity: React.FC = () => {
  ...
  const [verificationStatus, setVerificationStatus] = useState<
    'IDLE' | 'VERIFYING' | 'SECURE' | 'COMPROMISED'
  >('IDLE');
  ...
  const fetchLiveTelemetry = () => {
    if (operation === 'Vibration Analysis' && mechanical?.vibration !== undefined) {
      ...
      setValue(
        mechanical.rpm > 1
          ? `Dynamic Run-out @ ${mechanical.rpm.toFixed(0)} RPM`
          : 'Static Alignment OK'
      );
      ...
      .on(
        'postgres_changes',
        {
          ...
          filter: selectedAsset
            ? `asset_id=eq.${idAdapter.toStorage(selectedAsset.id)}`
            : undefined,
        },
        (payload: any) => {
          ...
          setLedger(prev => [
            newBlock,
            ...prev.filter(b => b.block_index !== newBlock.block_index),
          ]);
        }
      )
      .subscribe();
  ...
  const handleSealRecord = async () => {
    if (!selectedAsset) {
      showToast(
        t('digitalIntegrity.toast.selectAsset', 'Please select a Target Asset first.'),
        'error'
      );
      return;
    }
    ...
    try {
      ...
      if (error) throw error;
      showToast(
        t('digitalIntegrity.toast.mined', {
          index: newBlock.block_index,
          defaultValue: `Block #${newBlock.block_index} successfully mined on-chain.`,
        }),
        'success'
      );
    } catch (error: any) {
  ...
  const handleVerifyChain = () => {
    ...
    setTimeout(() => {
      ...
      showToast(
        t(
          'digitalIntegrity.toast.verified',
          'Cryptographic integrity confirmed. Ledger is immutable.'
        ),
        'success'
      );
    }, 2000);
          ...
          <div className="flex justify-center gap-4">
            <div className="flex items-center gap-2 px-4 py-1.5 rounded-full bg-slate-900 border border-slate-700 shadow-lg">
              ...
              <span className="text-xs font-mono text-slate-400">
                {t('digitalIntegrity.nodeStatus', 'NODE STATUS')}:{' '}
                <span className="text-emerald-400 font-bold">{t('common.active', 'ACTIVE')}</span>
              </span>
            </div>
            <div className="flex items-center gap-2 px-4 py-1.5 rounded-full bg-slate-900 border border-slate-700 shadow-lg">
              <span className="text-xs font-mono text-slate-400">
                {t('digitalIntegrity.height', 'HEIGHT')}:{' '}
                <span className="text-cyan-400 font-bold">
                  {ledger.length} {t('digitalIntegrity.blocks', 'BLOCKS')}
                </span>
              </span>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <GlassCard className="h-fit border-l-4 border-l-blue-500">
          <div className="flex items-center gap-3 mb-6 border-b border-white/5 pb-4">
            ...
            <h3 className="text-lg font-bold text-white uppercase tracking-wider">
              {t('digitalIntegrity.miningConsole', 'Mining Console')}
            </h3>
          </div>

          {!selectedAsset ? (
            <div className="p-6 bg-red-500/10 border border-red-500/20 rounded-xl text-center">
              <p className="text-sm text-red-400 font-bold uppercase tracking-wide mb-1">
                {t('digitalIntegrity.noAssetTitle', 'Target Asset Required')}
              </p>
              <p className="text-xs text-red-200/70">
                {t(
                  'digitalIntegrity.noAssetDesc',
                  'Select a project above to enable mining operations.'
                )}
              </p>
            </div>
          ) : (
            <div className="space-y-5 animate-fade-in">
              <div className="bg-slate-900/50 p-4 rounded-xl border border-white/5">
                <label className="text-[10px] text-slate-500 uppercase font-bold tracking-wider block mb-1">
                  {t('digitalIntegrity.targetAsset', 'Target Asset')}
                </label>
              ...
              <div>
                <label className="text-xs text-slate-500 uppercase font-bold mb-2 block ml-1">
                  {t('digitalIntegrity.opType', 'Operation Type')}
                </label>
                <select
                  value={operation}
                ...
                >
                  <option value="Shaft Alignment Check">
                    {t('digitalIntegrity.operations.alignment', 'Shaft Alignment Check')}
                  </option>
                  <option value="Maintenance Protocol Seal">
                    {t('digitalIntegrity.operations.maintenance', 'Maintenance Protocol Seal')}
                  </option>
                  <option value="Part Installation (QR Confirmed)">
                    {t(
                      'digitalIntegrity.operations.partInstallation',
                      'Part Installation (QR Confirmed)'
                    )}
                  </option>
                  <option value="Vibration Analysis">
                    {t('digitalIntegrity.operations.vibration', 'Vibration Analysis')}
                  </option>
                  <option value="Component Replacement">
                    {t('digitalIntegrity.operations.replacement', 'Component Replacement')}
                  </option>
                  <option value="Firmware Update">
                    {t('digitalIntegrity.operations.firmware', 'Firmware Update')}
                  </option>
                  <option value="Safety Protocol Audit">
                    {t('digitalIntegrity.operations.audit', 'Safety Protocol Audit')}
                  </option>
                </select>
              </div>
              ...
              <div className="pt-4">
                ...
                >
                  {isMining
                    ? t('digitalIntegrity.btnHashing', 'HASHING BLOCK...')
                    : t('digitalIntegrity.btnSeal', 'SEAL TO BLOCKCHAIN')}
                </ModernButton>
              </div>
            </div>
          )}
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="flex justify-between items-center bg-slate-900/60 p-4 rounded-2xl border border-slate-700 backdrop-blur-md">
            <h3 className="text-sm font-bold text-white pl-2 uppercase tracking-wider">
              {t('digitalIntegrity.latestBlocks', 'Latest Blocks')}
            </h3>
            <button
            ...
            <FetchSkeleton loading={isLoading} count={5}>
              {ledger.map(block => (
                <div
                  ...
                  className="relative p-5 bg-slate-900/40 rounded-2xl border border-white/5 hover:border-cyan-500/30 hover:bg-slate-800/60 transition-all group overflow-hidden"
                >
                  {/* Connector Line */}
                  {block.block_index > 0 && (
                    <div className="absolute -top-6 left-[27px] w-0.5 h-10 bg-slate-800 z-0"></div>
                  )}
                      ...
                      <div className="flex-grow">
                        ...
                        <div className="bg-black/40 p-3 rounded-lg border border-white/5 font-mono text-[9px] text-slate-500 group-hover:text-slate-400 transition-colors">
                          <div className="flex gap-3 mb-1">
                            <span className="text-slate-600 select-none font-bold w-8 text-right">
                              HASH:
                            </span>
                            <span className="truncate w-full text-emerald-500/80">
                              {block.hash}
                            </span>
                          </div>
