...
const TabButton: React.FC<{
  ...
  >
    ...
    {count !== undefined && (
      <span className="px-2 py-0.5 bg-purple-500/20 rounded text-xs text-purple-400">{count}</span>
    )}
  </button>
);
  ...
  return (
    <motion.button
      ...
      className={`w-full p-3 rounded-lg border-2 transition-all text-left mb-2 ${
        selected ? severityColors[wtfCase.severity] : 'border-slate-700/50 bg-slate-800/30'
      }`}
    >
      <div className="flex items-start justify-between mb-2">
        <span
          className={`text-xs font-bold px-2 py-1 rounded ${
            wtfCase.severity === 'CRITICAL'
              ? 'bg-red-500/20 text-red-400'
              : wtfCase.severity === 'HIGH'
        ...
        <span className="text-xs text-slate-500">{wtfCase.timesEncountered}x encountered</span>
      </div>
...
const CaseDetailView: React.FC<{ case: WTFCase }> = ({ case: wtfCase }) => (
  <GlassCard className="p-6">
    ...
    <div className="mb-6">
      ...
      <p className="text-sm text-slate-300 bg-slate-900/50 p-4 rounded-lg">{wtfCase.symptom}</p>
    </div>
...
const TipCard: React.FC<{ tip: OldSchoolTip }> = ({ tip }) => (
  <GlassCard
    className={`p-4 border-2 ${
      tip.criticality === 'MUST_FOLLOW'
        ? 'border-red-500 bg-red-950/20'
        : tip.criticality === 'RECOMMENDED'
