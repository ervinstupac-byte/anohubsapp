    ...
    <div className={`relative overflow-hidden group ${className}`}>
      ...
      <div
        className={`absolute inset-0 transition-opacity duration-1000 opacity-20 ${
          isFinancialCritical ? 'bg-red-900/40 animate-pulse' : 'bg-emerald-900/20'
        }`}
      ></div>

      <div
        className={`relative z-10 flex items-center justify-between p-4 bg-slate-900/80 backdrop-blur-xl border-y sm:border sm:rounded-2xl transition-colors duration-500 ${
          isFinancialCritical
            ...
            <div className="flex items-baseline gap-2">
              <span
                className={`text-3xl font-black tabular-nums tracking-tight ${
                  isFinancialCritical ? 'text-white' : 'text-slate-200'
                }`}
              >
                -{currency}
                {revenueLossPerHour}
