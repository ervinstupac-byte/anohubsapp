...
const CategoryBadge: React.FC<{ category: string }> = ({ category }) => {
  ...
  return (
    <span
      className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold border uppercase tracking-wider ${colorClass}`}
    >
      {category}
    </span>
  );
...
export const HPPImprovements: React.FC = () => {
  ...
  useEffect(() => {
    fetchIdeas();
    const sub = supabase
      .channel('public:hpp_improvements')
      ...
      <div className="text-center space-y-4 pt-6">
        ...
        <div>
          <h2 className="text-4xl md:text-5xl font-black text-white tracking-tighter mb-4">
            {t('hppImprovements.title').split(' ')[0]}{' '}
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-500">
              {t('hppImprovements.title').split(' ')[1]}
            </span>
          </h2>
          <p className="text-slate-400 text-lg font-light">{t('hppImprovements.subtitle')}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-4">
          <GlassCard
            ...
            subtitle={t('hppImprovements.contributor', {
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          <div className="space-y-4 max-h-[800px] overflow-y-auto custom-scrollbar pr-2">
            ...
            {filteredIdeas.map(idea => (
              <GlassCard
                key={idea.id}
                className="p-0 hover:border-cyan-500/30 transition-colors group"
              >
                <div className="flex">
                  {/* VOTE COLUMN */}
