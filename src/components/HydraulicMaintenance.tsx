...
export const HydraulicMaintenance: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (maxAcousticMag > 0.8) {
      triggerVoiceAlert(
        'Detektovano struganje metala na stražnjem ležaju. Automatsko gašenje uzbude.'
      );
      const numeric = idAdapter.toNumber(selectedAsset.id);
      if (numeric !== null) {
  ...
  const handleApplyChange = () => {
    ...
    if (simResults.isCritical) {
      triggerVoiceAlert(
        'Upozorenje: Rizik od mehaničkog kidanja instalacija uslijed prevelikog protoka.'
      );
      ...
      <div className="flex justify-between items-center bg-slate-900/40 p-6 rounded-3xl border border-white/5 backdrop-blur-xl">
        <div>
          <h2 className="text-3xl font-black text-white tracking-widest uppercase mb-1">
            Field <span className="text-cyan-400">Safeguard</span>
          </h2>
          <p className="text-slate-400 text-sm font-light italic">
            Mechanical Failure Prevention & Retrofit Validation.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="space-y-6">
            <GlassCard title="Retrofit Validator" className="border-l-4 border-l-cyan-500">
              <div className="p-4 bg-slate-950 border border-white/5 rounded-xl mb-6">
                ...
                <div className="flex gap-2">
                  ...
                  />
                  <ModernButton onClick={handleApplyChange} variant="primary">
                    Validate
                  </ModernButton>
                </div>
              </div>
              ...
              {simResults && (
                <div
                  className={`p-4 rounded-xl border animate-pulse ${simResults.isCritical ? 'bg-red-500/10 border-red-500/30' : 'bg-cyan-500/10 border-cyan-500/30'}`}
                >
                  <h4
                    className={`text-xs font-black uppercase mb-1 ${simResults.isCritical ? 'text-red-500' : 'text-cyan-400'}`}
                  >
                    {simResults.isCritical ? 'CRITICAL VELOCITY' : 'Safe Retrofit'}
                  </h4>
                  <p className="text-[10px] text-slate-300">
                    Actuator Speed: +{simResults.velocityIncrease}%
                  </p>
                  {simResults.isCritical && (
                    <p className="text-[8px] text-red-400 font-bold mt-2 uppercase">
                      ANO-AGENT BLOCK ACTIVE
                    </p>
                  )}
                </div>
              )}
            ...
            <GlassCard title="Acoustic Spectrum" className="border-l-4 border-l-purple-500">
              ...
              <div className="mt-4 flex justify-between items-center bg-slate-950 p-3 rounded-xl">
                ...
                <span
                  className={`text-[10px] font-black ${assetTele?.excitationActive ? 'text-emerald-400' : 'text-red-500'}`}
                >
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <GlassCard title="Dynamic Tension Monitoring" className="relative group">
              {assetTele?.oilPressureRate &&
                assetTele.oilPressureRate > 15 &&
                assetTele.actuatorPosition < 5 && (
