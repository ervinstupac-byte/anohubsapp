  ...
  const spinnerContent = (
    <div className={`flex flex-col items-center justify-center gap-6 ${className}`}>
      <div className="relative flex items-center justify-center">
        {/* Glow Effect */}
        <div
          className={`absolute bg-cyan-500/30 rounded-full blur-xl animate-pulse ${size === 'xl' ? 'w-48 h-48' : 'w-full h-full scale-150'}`}
        ></div>

        {/* Outer Static Ring */}
        <div
          className={`${currentSize.w} ${currentSize.h} ${currentSize.border} border-slate-800 rounded-full absolute opacity-50`}
        ></div>
        ...
        {/* Center Dot */}
        <div
          className={`absolute bg-white rounded-full ${size === 'sm' ? 'w-1 h-1' : 'w-2 h-2'} animate-pulse`}
        ></div>
      </div>
      ...
      {text && (
        <div className="flex flex-col items-center gap-1">
          ...
          <div className="flex gap-1">
            <div
              className="w-1 h-1 bg-cyan-500/50 rounded-full animate-bounce"
