...
export const StandardOfExcellence: React.FC<{ onCommit?: () => void }> = ({ onCommit }) => {
  ...
  const handleCommit = () => {
    ...
    showToast(
      `${t('standardOfExcellence.oath.toast')} ${user?.email?.split('@')[0].toUpperCase()}`,
      'success'
    );
  };
    ...
    <div className="animate-fade-in pb-12 max-w-5xl mx-auto space-y-8">
      ...
      <div className="text-center space-y-6 pt-6 relative">
        ...
        <p className="text-slate-400 text-lg max-w-2xl mx-auto font-light leading-relaxed">
          ...
          <Trans
            i18nKey="standardOfExcellence.subtitle2"
            components={{ span: <span className="text-white font-bold" /> }}
          />
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
              ...
              <div className="p-6">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-4">
                    ...
                    <h3
                      className={`text-xl font-bold tracking-tight ${activeId === p.id ? 'text-white' : 'text-slate-400 group-hover:text-slate-200'}`}
                    >
                      {p.title}
                    </h3>
                  </div>
                  <span
                    className={`text-2xl transition-transform duration-300 ${activeId === p.id ? 'rotate-180 text-amber-400' : 'text-slate-600'}`}
                  >
                    ↓
                  </span>
                </div>
                ...
                <div
                  className={`overflow-hidden transition-all duration-300 ease-in-out ${activeId === p.id ? 'max-h-48 opacity-100 mt-4' : 'max-h-0 opacity-0'}`}
                >
                  <p className="text-slate-300 leading-relaxed text-sm border-l-2 border-amber-500/30 pl-4">
                    {p.description}
                  </p>
                  <div className="mt-4 text-xs font-mono text-amber-400/80 italic">{p.quote}</div>
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="lg:col-span-5 sticky top-24">
          <GlassCard className="text-center border-t-4 border-t-amber-500 bg-slate-900/80 backdrop-blur-xl group/card overflow-hidden">
            <div
              className={`absolute inset-0 bg-amber-500/5 transition-opacity duration-1000 ${signed ? 'opacity-100' : 'opacity-0'}`}
            />

            <div className="mb-6 relative z-10">
              <div
                className={`w-20 h-20 mx-auto bg-amber-500/10 rounded-full flex items-center justify-center border border-amber-500/30 mb-4 transition-all duration-500 ${signed ? 'scale-110 shadow-[0_0_20px_rgba(245,158,11,0.2)]' : ''}`}
              >
                <span
                  className={`text-4xl transition-transform duration-500 ${signed ? 'rotate-12 scale-110' : ''}`}
                >
                  {signed ? '📜' : '✒️'}
                </span>
              </div>
              <h3 className="text-2xl font-bold text-white mb-2">
                {t('standardOfExcellence.oath.title')}
              </h3>
              <p className="text-sm text-slate-400">{t('standardOfExcellence.oath.desc')}</p>
            </div>
            ...
            <div className="bg-black/30 rounded-lg p-4 mb-6 text-left font-mono text-xs text-slate-500 space-y-2 border border-white/5 relative z-10">
              ...
              <div className="flex justify-between">
                ...
                <span
                  className={`${signed ? 'text-emerald-400 font-bold glow-oath' : 'text-amber-500 font-bold'}`}
                >
                  {signed
                    ? t('standardOfExcellence.oath.ratified')
                    : t('standardOfExcellence.oath.pending')}
                </span>
              </div>
            </div>
            ...
            <ModernButton
              ...
              className={
                signed
                  ...
                  : 'bg-gradient-to-r from-amber-600 to-yellow-600 border-none text-black font-black hover:opacity-90'
              }
              icon={signed ? <span>✓</span> : <span>✒️</span>}
            >
              {signed
                ? t('standardOfExcellence.oath.btnActive')
                : t('standardOfExcellence.oath.btnCommit')}
            </ModernButton>
          ...
          <div className="mt-8 p-6 rounded-2xl border border-dashed border-slate-800 text-center opacity-50 hover:opacity-100 transition-opacity">
            <p className="text-[10px] text-slate-500 uppercase font-bold tracking-[0.2em] mb-2">
              {t('standardOfExcellence.poweredBy')}
            </p>
            <h4 className="text-xl font-black text-slate-700 tracking-tighter">
              AnoHUB <span className="text-slate-600">OS</span>
            </h4>
          </div>
        </div>
      </div>
