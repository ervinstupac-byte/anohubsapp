      ...
      <div className="bg-red-500/10 border border-red-500/30 p-6 rounded-2xl">
        ...
        <div className="grid gap-2">
          {steps.map((step, i) => (
            <div
              key={i}
              className="flex gap-3 bg-slate-900 border border-white/5 p-4 rounded-xl group hover:border-red-500/30 transition-colors"
            >
              <span className="text-red-500 font-mono font-bold">0{i + 1}</span>
              <span className="text-slate-200 text-sm font-bold uppercase tracking-wide">
                {step}
              </span>
            </div>
          ))}
        ...
        {!selectedAsset ? (
          ...
          <GlassCard className="text-center py-24 border-dashed border-slate-700/50 bg-slate-900/40">
            ...
            <p className="text-slate-400 max-w-lg mx-auto text-lg font-light leading-relaxed">
              <Trans
                i18nKey="riskAssessment.standbyDesc"
                components={{ strong: <strong className="text-cyan-400 font-bold" /> }}
              />
            </p>
          ...
          <div className="space-y-8">
            {activeIncident &&
              ...
              activeIncident.assetId === numericSelected && (
                <EmergencyProtocol type={activeIncident.type} />
              )}

            <Questionnaire onShowSummary={onShowSummary} onRiskSync={handleRiskSync} />
          </div>
        )}
      </div>
    </div>
