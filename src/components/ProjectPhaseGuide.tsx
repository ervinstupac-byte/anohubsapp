        ...
        <div className="animate-fade-in pb-12 max-w-5xl mx-auto space-y-12">
            ...
            <div className="relative text-center space-y-6 pt-6">
                ...
                <div className="pt-8">
                    <h2 className="text-4xl md:text-6xl font-black text-white tracking-tighter mb-4 drop-shadow-xl">
                        {t('projectPhase.title').split(' ')[0]} {t('projectPhase.title').split(' ')[1]} <span className="text-transparent bg-clip-text bg-gradient-to-r from-brand-400 to-blue-500">{t('projectPhase.title').split(' ')[2]}</span>
                    </h2>
            ...
            <div className="relative space-y-24 before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-slate-700 before:to-transparent">
                ...
                <div className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group">
                    {/* ICON MARKER */}
                    <div className="flex items-center justify-center w-14 h-14 rounded-full border-4 border-[#020617] bg-brand-900 text-brand-400 shadow-[0_0_20px_rgba(34,211,238,0.3)] shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 z-10 text-2xl">
                        📝
                    </div>

                    {/* CARD */}
                    <div className="w-[calc(100%-5rem)] md:w-[calc(50%-3rem)] p-1">
                        <GlassCard className="border-t-4 border-t-brand-500 relative overflow-hidden group-hover:border-brand-400/50 transition-colors">
                            <div className="absolute top-4 right-4 text-6xl font-black text-white/5 pointer-events-none select-none">01</div>

                            <div className="mb-4">
                                <span className="text-[10px] font-bold uppercase tracking-widest text-brand-400 bg-brand-950/30 px-2 py-1 rounded border border-brand-500/20">{t('projectPhase.phase1.tag')}</span>
