...
export function SiteHealthCertificate({
  ...
  const lossPercentage =
    (stats.totalMoneyLeakkW / (stats.totalGeneratedkW + stats.totalMoneyLeakkW)) * 100;
  ...
  return (
    <div className="w-full max-w-4xl mx-auto bg-slate-900 text-slate-100 rounded-xl overflow-hidden border-4 border-slate-700 shadow-2xl">
      ...
      <div className="bg-slate-800 p-6 border-b border-slate-700 flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-white mb-1">{stationName}</h1>
      ...
      <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-slate-800/50 rounded-lg p-4 border border-slate-700">
            ...
            <div className="flex justify-between mt-2 text-xs font-mono">
              <span className="text-emerald-400">
                {stats.totalGeneratedkW.toLocaleString()} kW GENERATED
              </span>
              <span className="text-red-400">
                {stats.totalMoneyLeakkW.toLocaleString()} kW LEAKING
              </span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div
              className={`p-4 rounded-lg border ${stats.safetyStatus === 'SECURE' ? 'bg-emerald-500/10 border-emerald-500/30' : 'bg-red-500/10 border-red-500/30'}`}
            >
              <div className="flex items-center gap-2 mb-2">
                <Shield
                  className={`w-5 h-5 ${stats.safetyStatus === 'SECURE' ? 'text-emerald-400' : 'text-red-400'}`}
                />
                <span className="font-bold text-sm">SAFETY SHIELD</span>
              </div>
              <div
                className={`text-lg font-bold ${stats.safetyStatus === 'SECURE' ? 'text-emerald-400' : 'text-red-400'}`}
              >
                {stats.safetyStatus}
              </div>
            </div>
            ...
            <div
              className={`p-4 rounded-lg border ${stats.complianceStatus === 'COMPLIANT' ? 'bg-blue-500/10 border-blue-500/30' : 'bg-orange-500/10 border-orange-500/30'}`}
            >
              <div className="flex items-center gap-2 mb-2">
                <Fish
                  className={`w-5 h-5 ${stats.complianceStatus === 'COMPLIANT' ? 'text-blue-400' : 'text-orange-400'}`}
                />
                <span className="font-bold text-sm">ECOLOGICAL</span>
              </div>
              <div
                className={`text-lg font-bold ${stats.complianceStatus === 'COMPLIANT' ? 'text-blue-400' : 'text-orange-400'}`}
              >
      ...
      <div className="bg-slate-950 p-4 border-t border-slate-800 text-center text-xs text-slate-500 font-mono">
        SIGNED BY: SCADA NERVOUS SYSTEM • {new Date().toLocaleDateString()} • ID:{' '}
