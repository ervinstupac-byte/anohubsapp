          ...
          {suggestedSOPs.length > 0 ? (
            suggestedSOPs.map(sop => (
              <motion.div
                ...
                exit={{ x: -20, opacity: 0 }}
                className={`p-4 rounded-xl border flex gap-4 items-start relative overflow-hidden group hover:scale-[1.02] transition-transform ${
              ...
              >
                <div
                  className={`mt-1 p-2 rounded-lg ${sop.priority === 'CRITICAL' ? 'bg-red-500/20 text-red-400' : 'bg-purple-500/20 text-purple-400'}`}
                >
                ...
                <div className="flex-1">
                  ...
                  <div className="flex gap-2">
                    <button
                      className={`px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest flex items-center gap-1.5 transition-colors ${
                        sop.priority === 'CRITICAL'
                ...
                <div className="absolute top-0 right-0 p-1">
                  <div
                    className={`w-1 h-1 rounded-full animate-pulse ${sop.priority === 'CRITICAL' ? 'bg-red-500' : 'bg-purple-500'}`}
                  ></div>
                </div>
              </motion.div>
            ))
            ...
            <div className="h-full flex flex-col items-center justify-center opacity-40 py-12">
              ...
              <p className="text-[10px] text-slate-500 uppercase font-black tracking-widest">
                No active critical SOPs required
              </p>
            </div>
          )}
        </AnimatePresence>
