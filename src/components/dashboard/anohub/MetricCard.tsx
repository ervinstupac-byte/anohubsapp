...
export const MetricCard: React.FC<MetricCardProps> = ({ title, value, unit, subtitle, chart, statusColor = 'text-cyan-400' }) => {
    ...
    return (
        <div className="bg-slate-900/80 border border-slate-700/50 p-4 rounded-lg relative overflow-hidden backdrop-blur-md group min-h-[140px] flex flex-col justify-between metallic-border hover:border-cyan-500/50 transition-all duration-300">
            ...
            <div className="relative z-10 w-full">
                ...
                <div className="flex flex-col gap-1">
                    <div className="flex items-baseline gap-1">
                        <span className={`text-3xl font-black font-mono tracking-tighter ${accentColor} ${glowClass} truncate`}>
                            {value}
                        </span>
                    ...
                    <div className="text-[9px] text-slate-400 font-mono font-bold uppercase tracking-wider opacity-60 truncate">
                        {subtitle}
                    </div>
                </div>
            </div>
