...
interface MetricCardProps {
    ...
    statusColor?: string; // Optional accent color (e.g. 'text-brand-400')
}

export const MetricCard: React.FC<MetricCardProps> = ({ title, value, unit, subtitle, chart, statusColor = 'text-brand-400' }) => {
    const isAmber = statusColor.includes('amber');
    const glowClass = isAmber ? 'bloom-glow-amber' : 'bloom-glow-cyan';
    const accentColor = isAmber ? 'text-amber-400' : 'text-brand-400';

    return (
        <div className="bg-slate-900/80 border border-slate-700/50 p-4 rounded-lg relative overflow-hidden backdrop-blur-md group h-full flex flex-col justify-between metallic-border hover:border-brand-500/50 transition-all duration-300 min-h-0">
            ...
            <div className={`absolute top-0 right-0 w-12 h-12 pointer-events-none opacity-20 group-hover:opacity-40 transition-opacity`}>
                <svg viewBox="0 0 100 100" className={`w-full h-full ${isAmber ? 'fill-amber-500' : 'fill-brand-500'}`}>
            ...
            <div className="relative z-10 w-full min-h-0">
                <h3 className="text-[clamp(8px,1vh,10px)] font-black text-slate-500 uppercase tracking-[0.2em] mb-2 flex items-center gap-2">
                    <div className={`w-1 h-3 ${isAmber ? 'bg-amber-500' : 'bg-brand-500'} rounded-full`} />
                    {title}
                </h3>
            ...
            {/* Bottom Glow Stripe */}
            <div className={`absolute bottom-0 left-0 right-0 h-[1px] ${isAmber ? 'bg-amber-500/30' : 'bg-brand-500/30'} ${glowClass}`} />
        </div>
    );
};
