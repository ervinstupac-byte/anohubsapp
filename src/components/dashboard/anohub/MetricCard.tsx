...
export const MetricCard: React.FC<MetricCardProps> = ({
  ...
  return (
    <div className="bg-slate-900/80 border border-slate-700/50 p-4 rounded-lg relative overflow-hidden backdrop-blur-md group h-full flex flex-col justify-between metallic-border hover:border-cyan-500/50 transition-all duration-300 min-h-0">
      ...
      <div
        className={`absolute top-0 right-0 w-12 h-12 pointer-events-none opacity-20 group-hover:opacity-40 transition-opacity`}
      >
        <svg
          viewBox="0 0 100 100"
          className={`w-full h-full ${isAmber ? 'fill-amber-500' : 'fill-cyan-500'}`}
        >
      ...
      {/* Bottom Glow Stripe */}
      <div
        className={`absolute bottom-0 left-0 right-0 h-[1px] ${isAmber ? 'bg-amber-500/30' : 'bg-cyan-500/30'} ${glowClass}`}
      />
    </div>
  );
};
