...
export const CyberCard: React.FC<CyberCardProps> = ({
  ...
  const getBorderColor = () => {
    switch (variant) {
      case 'primary':
        return 'stroke-cyan-500/30';
      case 'danger':
        return 'stroke-red-500/30';
      case 'warning':
        return 'stroke-yellow-500/30';
      default:
    ...
    <div className={`relative bg-slate-900/40 backdrop-blur-sm p-4 group ${className}`}>
      ...
      <div className="absolute inset-0 pointer-events-none">
        <svg className="w-full h-full" xmlns="http://www.w3.org/2000/svg">
          ...
          <path
            d="M 0 10 L 10 0 L 30 0"
            fill="none"
            strokeWidth="2"
            className={`${getBorderColor()} opacity-50`}
          />

          {/* Technical Accents - Bottom Right Corner */}
          <path
            d="M calc(100% - 30) 100% L calc(100% - 10) 100% L 100% calc(100% - 10)"
            fill="none"
            strokeWidth="2"
            className={`${getBorderColor()} opacity-50`}
          />
      ...
      {title && (
        <h3 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-3 border-b border-white/5 pb-2 flex items-center justify-between">
          {title}
          <div
            className={`w-1 h-1 rounded-full ${variant === 'primary' ? 'bg-cyan-500' : 'bg-slate-500'} shadow-[0_0_8px_currentColor]`}
          />
        </h3>
      )}
