...
export const ActionCard: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 opacity-40 surface-brushed pointer-events-none" />
      {!isLocked && (
      ...
      <div className="relative z-10 text-left">
        <div className="flex justify-between items-start mb-1">
          <span className="text-[9px] text-slate-400 font-black font-mono uppercase tracking-[0.2em] opacity-80">
            Protocol Genesis
          </span>
          {protocolVerified && (
      ...
      <div className="relative z-10 flex justify-end">
        <div
          className={`bg-slate-800 p-2 rounded border border-slate-900 shadow-inner ${!isLocked ? 'group-hover:bg-cyan-600' : ''} transition-colors`}
        >
...
const AnalyticsGraph: React.FC<{ title: string; color: string; glowClass?: string }> = ({
  title,
  color,
  glowClass,
}) => (
    ...
    <div className={`h-16 w-full flex items-end gap-1 relative z-10`}>
      {[...Array(15)].map((_, i) => {
        ...
        return (
          <div
            key={i}
            className={`flex-1 ${color} ${glowClass} opacity-60 group-hover:opacity-100 transition-all`}
            style={{
              height: `${h}%`,
...
const AlertItem: React.FC<{
  ...
  return (
    <div
      className={`flex items-center gap-3 p-2 rounded border ${c.bg} ${c.border} group/alert hover:border-white/20 transition-colors`}
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex justify-between items-baseline mb-0.5">
          <span className="text-xs font-black text-slate-300 uppercase tracking-tight truncate group-hover/alert:text-white transition-colors">
            {message}
          </span>
