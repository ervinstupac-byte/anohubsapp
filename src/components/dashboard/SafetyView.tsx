  ...
  return (
    <div className="w-full bg-slate-950 p-6 text-white border-2 border-slate-800 rounded-xl">
      ...
      <div className="grid grid-cols-3 gap-6">
        {/* 1. START PERMISSIVES */}
        <div
          className={`bg-slate-900/50 p-4 rounded-lg border ${permissives.ready ? 'border-emerald-500' : 'border-slate-700'}`}
        >
          ...
          {permissives.ready ? (
            ...
            <div className="space-y-2">
              ...
              {permissives.missing.map((cond, i) => (
                <div
                  key={i}
                  className="px-2 py-1 bg-red-950/30 border border-red-500/30 rounded text-xs text-red-300 font-mono"
                >
                  {cond}
                </div>
              ))}
        ...
        <div
          className={`bg-slate-900/50 p-4 rounded-lg border ${interlocks.tripActive ? 'border-red-500 animate-pulse' : 'border-slate-700'}`}
        >
          <h3 className="text-sm font-bold text-slate-300 mb-4 flex items-center gap-2">
        ...
        <div className="bg-slate-900/50 p-4 rounded-lg border border-slate-700">
          ...
          <div className="space-y-4">
            <div className="bg-slate-800 p-3 rounded">
              ...
              <div
                className={`text-xl font-bold font-mono ${esd.level.includes('LEVEL_3') ? 'text-red-500' : esd.level.includes('LEVEL_1') ? 'text-amber-400' : 'text-emerald-400'}`}
              >
