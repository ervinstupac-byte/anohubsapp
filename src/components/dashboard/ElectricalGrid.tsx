  ...
  return (
    <div className="w-full h-full bg-slate-950 p-6">
      ...
      <div className="h-[calc(100%-3rem)] relative bg-slate-900 border border-slate-700 rounded-lg p-8">
            ...
            <div className="space-y-1 text-xs font-mono">
              <div className="flex justify-between gap-4">
                ...
                <span className="text-emerald-400 font-bold">
                  {generatorData.activePower.toFixed(1)} MW
                </span>
              </div>
              <div className="flex justify-between gap-4">
              ...
              <div className="flex justify-between gap-4">
                ...
                <span
                  className={
                    generatorData.frequency > 49.9 && generatorData.frequency < 50.1
                      ? 'text-emerald-400'
                      : 'text-amber-400'
                  }
                >
        ...
        {/* Step-Up Breaker */}
        <Breaker label="CB-TX" status={breakerStatus.stepUp} position={{ left: 112, top: '50%' }} />

        {/* Bus Line to Grid */}
        <div className="absolute left-[36rem] top-1/2 w-40 h-1 bg-yellow-500" />

        {/* Grid Connection Breaker */}
        <Breaker label="CB-GRID" status={breakerStatus.grid} position={{ left: 148, top: '50%' }} />
    ...
    <div className="absolute transform -translate-x-1/2 -translate-y-1/2" style={position}>
      <div
        className={`w-16 h-16 rounded-full border-4 ${
          status === 'CLOSED' ? 'bg-emerald-600 border-emerald-500' : 'bg-red-600 border-red-500'
        } flex items-center justify-center`}
      >
        <div className="text-white font-bold text-lg">{status === 'CLOSED' ? '⚡' : '⊗'}</div>
      </div>
      <div
        className={`text-xs text-center mt-2 font-mono font-bold ${
          status === 'CLOSED' ? 'text-emerald-400' : 'text-red-400'
        }`}
      >
        {label}
      </div>
      <div className="text-[10px] text-center text-slate-400">{status}</div>
    </div>
  );
};
