  ...
  return (
    <div className="w-full bg-scada-bg p-6 text-scada-text border-2 border-scada-border rounded-none relative overflow-hidden">
        ...
        <div className="absolute top-16 right-6 z-50 w-80 bg-scada-panel border border-scada-border rounded-none p-4 shadow-none">
          ...
          <div className="space-y-3">
            ...
            <div className="p-2 bg-scada-bg rounded-none border border-scada-border">
              <div className="text-[9px] text-scada-muted uppercase tracking-widest mb-1 font-mono">
                1-MIN TREND
              </div>
            ...
            <div>
              ...
              <div className="text-sm font-medium text-scada-text font-mono">
            ...
            <div
              className={`p-3 rounded-none border ${activeDossier.Remediation.priority === 'CRITICAL' ? 'bg-status-error/10 border-status-error' : 'bg-scada-bg border-scada-border'}`}
            >
              <div className="text-xs font-bold mb-1 flex items-center justify-between font-mono">
                ...
                <span className="text-[10px] bg-status-error text-scada-bg px-1 rounded-none">
                  {activeDossier.Remediation.priority}
                </span>
              </div>
      ...
      <div className="absolute bottom-4 right-4 z-50 flex gap-2 opacity-50 hover:opacity-100 transition-opacity">
        <button
          onClick={() => handleHotspotClick('RUNNER')}
          className="text-[10px] bg-indigo-900 px-2 py-1 rounded-none border border-indigo-500"
        >
          Test: Runner Physics
        </button>
        <button
          onClick={() => handleHotspotClick('BEARING')}
          className="text-[10px] bg-indigo-900 px-2 py-1 rounded-none border border-indigo-500"
        >
          Test: Bearing Oil Whip
        </button>
      </div>
      ...
      <div className="flex items-center justify-between mb-8 relative z-10 pt-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-black tracking-tight text-scada-text font-header">
              Sovereign<span className="text-status-info">Core</span>
            </h1>
        ...
        <div className="text-right">
          <div className="text-xs text-scada-muted uppercase tracking-widest mb-1 font-mono">
            System Pulse
          </div>
          <div
            className={`text-4xl font-black font-mono tabular-nums ${data.pulse.index > 80 ? 'text-status-ok' : data.pulse.index > 50 ? 'text-status-warning' : 'text-status-error'}`}
          >
      ...
      <div className="grid grid-cols-12 gap-6 relative z-10">
        ...
        <div className="col-span-8 bg-scada-panel border border-scada-border rounded-none p-6 flex flex-col justify-between min-h-[250px] shadow-none">
          ...
          <div className="mt-6 flex justify-between items-end">
            <div className="flex gap-4">
              <div className="px-3 py-1 bg-scada-bg rounded-none border border-scada-border text-xs text-scada-muted flex items-center gap-2 font-mono">
                ...
                Dominant Module:{' '}
                <span className="text-scada-text font-bold">{data.hierarchy.winner}</span>
              </div>
              {data.hierarchy.overrides > 0 && (
                <div className="px-3 py-1 bg-scada-bg rounded-none border border-scada-border text-xs text-scada-muted font-mono">
                  Overrides Active:{' '}
                  <span className="text-status-warning font-bold">{data.hierarchy.overrides}</span>
                </div>
              )}
            </div>
          ...
          {manualOverride && (
            <div className="mt-4 p-4 bg-scada-bg rounded-none border border-status-warning/30 grid grid-cols-2 gap-4">
              <div>
                <label className="text-xs text-scada-muted block mb-1 font-mono uppercase">
                  Target Load (MW)
                </label>
                <input
                  type="number"
                ...
                <div className="text-[10px] text-scada-muted mt-1 font-mono">
                  Limit: {LIMITS.targetLoad.min}-{LIMITS.targetLoad.max} MW
                </div>
              </div>
              <div>
                <label className="text-xs text-scada-muted block mb-1 font-mono uppercase">
                ...
                <input
                  ...
                  value={bladeAngle}
                  onChange={e =>
                    setBladeAngle(Math.min(32, Math.max(0, parseFloat(e.target.value))))
                  }
                ...
                />
                <div className="text-[10px] text-scada-muted mt-1 font-mono">
                  Limit: {LIMITS.bladeAngle.min}-{LIMITS.bladeAngle.max}°
                </div>
              </div>
            </div>
          )}
        ...
        <div className="col-span-4 grid grid-cols-2 gap-4">
          <div className="bg-scada-panel border border-scada-border rounded-none p-4 flex flex-col items-center justify-center shadow-none">
            ...
            <div className="text-2xl font-bold font-mono text-scada-text">
              {data.indices.phy.toFixed(0)}
            </div>
            <div className="text-[10px] text-scada-muted uppercase font-mono tracking-wider">
              Physical
            </div>
          </div>
          <div className="bg-scada-panel border border-scada-border rounded-none p-4 flex flex-col items-center justify-center shadow-none">
        ...
        {data.pulse.risks.length > 0 && (
          <div className="col-span-12 bg-status-error/10 border border-status-error/30 rounded-none p-4 flex items-start gap-4">
            ...
            <div>
              <h4 className="text-sm font-bold text-status-error uppercase mb-1 font-mono tracking-wider">
                Systemic Correlation Detected
              </h4>
              <div className="text-sm text-scada-text font-mono">
