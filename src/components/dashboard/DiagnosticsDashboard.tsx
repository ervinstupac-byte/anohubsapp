...
const DiagnosticsDashboard: React.FC = () => {
  ...
  const [actuatorHistory, setActuatorHistory] = useState<
    { cmd: number; pos: number; time: number }[]
  >([]);
        ...
        <div className="bg-slate-900 border border-slate-700 rounded-lg p-6">
          <div className="flex justify-between items-center mb-4">
            ...
            <div className="flex items-center gap-2">
              <span
                className={`text-xs font-bold px-2 py-1 rounded ${actuatorStatus?.status === 'HEALTHY' ? 'bg-emerald-900 text-emerald-400' : 'bg-red-900 text-red-400'}`}
              >
          ...
          <div className="h-48 bg-slate-950 rounded border border-slate-800 relative overflow-hidden flex items-end">
            {actuatorHistory.map((pt, i) => (
              ...
              >
                {/* Command (Blue Dot) */}
                <div
                  className="bg-blue-500 w-full rounded-full absolute"
                  style={{ bottom: `${pt.cmd}%`, height: '4px', opacity: 0.5 }}
                />
                {/* Position (Green or Red Dot) */}
                <div
                  className={`w-full rounded-full absolute transition-colors ${simStiction ? 'bg-red-500' : 'bg-emerald-400'}`}
          ...
          <div className="grid grid-cols-2 gap-4 mt-4 text-center">
            <div className="bg-slate-800 rounded p-2">
              ...
              <div className="text-xl font-mono text-white">
                {actuatorStatus?.stictionIndex.toFixed(0)}
              </div>
            </div>
            <div className="bg-slate-800 rounded p-2">
              <div className="text-xs text-slate-500 uppercase">Deadband</div>
              <div className="text-xl font-mono text-white">
                {actuatorStatus?.deadbandPct.toFixed(1)}%
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="bg-slate-950 rounded border border-slate-800 p-4 min-h-[12rem]">
            ...
            <div className="space-y-2">
              {visibleAlarms.map(alarm => (
                <div
                  key={alarm.id}
                  className="bg-red-900/20 border border-red-500 text-red-200 px-3 py-2 rounded flex justify-between items-center animate-pulse"
                >
