    ...
    <div className="w-full">
      ...
      {criticalGaps.length > 0 && (
        <div className="mb-6 p-4 bg-red-950 border-2 border-red-500 rounded-lg">
          <div className="flex items-center gap-2 mb-2">
            ...
            <div className="text-lg font-bold text-red-300">🚨 CRITICAL LOGISTICS GAP DETECTED</div>
          </div>
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-slate-900 border border-red-500 rounded-lg p-4">
          ...
          <div className="text-2xl font-bold text-red-300 font-mono">{stockouts.length}</div>
        </div>
        ...
        <div className="bg-slate-900 border border-amber-500 rounded-lg p-4">
          ...
          <div className="text-2xl font-bold text-amber-300 font-mono">{requisitions.length}</div>
        </div>
      ...
      <div className="mb-6">
        ...
        <div className="space-y-3">
          {requisitions.length === 0 ? (
            ...
            requisitions.map(req => <RequisitionCard key={req.requisitionId} requisition={req} />)
          )}
        </div>
      </div>
            ...
            <tbody>
              {inventory.map(part => (
                <tr key={part.componentId} className="border-t border-slate-800">
                  ...
                  <td className="p-3">
                    {part.stockQuantity === 0 ? (
                      <div className="px-2 py-1 bg-red-950 text-red-300 rounded text-center">
                        STOCKOUT
                      </div>
                    ) : part.stockQuantity <= part.reorderPoint ? (
                      <div className="px-2 py-1 bg-amber-950 text-amber-300 rounded text-center">
                        LOW
                      </div>
                    ) : (
                      <div className="px-2 py-1 bg-emerald-950 text-emerald-300 rounded text-center">
                        OK
                      </div>
                    )}
                  </td>
...
const RequisitionCard: React.FC<{ requisition: ProcurementRequisition }> = ({ requisition }) => {
  ...
  return (
    <div
      className={`bg-slate-900 border-2 rounded-lg p-4 ${isLate ? 'border-red-500' : borderClass}`}
    >
      ...
      <div className="grid grid-cols-3 gap-4 text-xs mb-3">
        <div>
          ...
          <div
            className={`text-2xl font-bold font-mono ${
              requisition.rulDays < 30 ? 'text-red-400' : 'text-amber-400'
            }`}
          >
        ...
        <div>
          ...
          {isLate && <div className="text-[10px] text-red-400 font-bold">WILL ARRIVE LATE!</div>}
        </div>
      </div>
      ...
      {/* Action Button */}
      <button
        className={`mt-3 w-full py-2 rounded font-bold text-sm ${
          isLate
