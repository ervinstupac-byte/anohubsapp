    ...
    <div className="w-full bg-slate-950/95 border border-indigo-500/30 rounded-lg p-6 relative overflow-hidden">
      ...
      <div className="relative z-10">
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-xs text-indigo-500/70 font-mono">Vertical Synthesis Active</div>
            </div>
          </div>
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <Icon
                      className={`w-5 h-5 ${isActive ? `text-${layer.color}-400` : 'text-slate-500'}`}
                    />
                    <div>
                      <div
                        className={`text-sm font-bold font-mono ${isActive ? 'text-white' : 'text-slate-400'}`}
                      >
                        {layer.name}
                      </div>
                      <div className="text-xs text-slate-500 font-mono">{layer.description}</div>
                    </div>
                  </div>

                  {isActive && <Zap className={`w-4 h-4 text-${layer.color}-400 animate-pulse`} />}
                </div>
        ...
        <div className="mt-6 pt-4 border-t border-slate-800 flex items-center justify-between text-xs">
          ...
          <div className="text-indigo-400 font-mono">Foundation ⟹ Middle ⟹ Unit</div>
        </div>
      </div>
    </div>
  );
