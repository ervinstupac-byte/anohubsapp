...
export const AssetPassportCard: React.FC = () => {
  ...
  // NC-1100: Safety Clamp for Sensor Corruption (Fixes -2B degree bug)
  const rawTemp = mechanical.bearingTemp || 0;
  // Clamp between 0°C (frozen) and 200°C (melted), ignore physics glitches
  const clampedTemp = Number.isFinite(rawTemp) ? Math.max(0, Math.min(200, rawTemp)) : 20;

  const valLabyrinth = new Decimal(mechanical.radialClearance || 0);
  const valBearingTemp = new Decimal(clampedTemp);
  const valAxialPlay = new Decimal(mechanical.axialPlay || 0);
  const valOperatingHours = new Decimal(
    specs.totalOperatingHours || identity.totalOperatingHours || 0
  );
  ...
  else if (valAxialPlay.lt(shaftLiftMin.mul(1.1)) || valAxialPlay.gt(shaftLiftMax.mul(0.9)))
    shaftLiftStatus = 'warning';

  const hasWarnings =
    babbittStatus !== 'nominal' ||
    tanStatus !== 'nominal' ||
    shaftLiftStatus !== 'nominal' ||
    isLabyrinthBreach;
  ...
  const renderField = (field: PassportField) => (
    <div
      ...
      className={`
                p-3 rounded-none bg-slate-900 border relative group
    ...
    >
      <div className="flex items-center justify-between mb-1.5">
        <div className="text-[9px] text-slate-400 font-bold uppercase tracking-widest truncate font-mono">
          {field.label}
        </div>
        <div
          className={`opacity-50 group-hover:opacity-100 transition-opacity ${field.status !== 'nominal' ? 'animate-pulse' : ''}`}
        >
          {field.icon}
        </div>
      </div>
      ...
      >
        {field.value}
        {field.unit && (
          <span className="text-[10px] text-slate-500 ml-1 font-mono font-bold lowercase">
            {field.unit}
          </span>
        )}
      </div>
      ...
      {field.isoRef && (
        <div className="text-[8px] font-mono text-slate-500 border-t border-slate-700 pt-1 mt-1">
          REF: <span className="text-slate-400">{field.isoRef}</span>
        </div>
      )}
    </div>
  );
  ...
  return (
    <>
      <div
        className={`
                    relative overflow-hidden bg-slate-900 border rounded-none shadow-none
                    ${hasWarnings ? 'border-l-4 border-l-amber-500' : 'border-l-4 border-l-emerald-500'}
      ...
      >
        ...
        <div className="flex items-center justify-between p-4 border-b border-scada-border">
          <div className="flex items-center gap-3">
            <div
              className={`w-10 h-10 rounded-none flex items-center justify-center ${hasWarnings ? 'bg-status-warning/20' : 'bg-status-info/20'}`}
            >
              <FileText
                className={`w-5 h-5 ${hasWarnings ? 'text-status-warning' : 'text-status-info'}`}
              />
            </div>
            <div>
          ...
          {hasWarnings && (
            <div className="flex items-center gap-2 px-2 py-1 bg-status-warning/20 rounded-none border border-status-warning/30 animate-pulse">
        ...
        <div className="px-4 pt-3 grid grid-cols-2 gap-3">
          <div className="flex items-center justify-between p-2 rounded-none bg-scada-bg border border-scada-border">
            ...
            <span className="text-sm font-mono font-bold text-scada-text tabular-nums">
              {valOperatingHours.toNumber().toLocaleString()}{' '}
              <span className="text-[10px] text-scada-muted">hrs</span>
            </span>
          </div>
          <div
            className="flex items-center justify-between p-2 rounded-none bg-scada-bg border border-scada-border cursor-pointer hover:border-status-info/30 transition-colors"
            onClick={() => handleFieldClick(fieldOverhaul)}
          >
            <span className="text-[9px] uppercase font-bold text-scada-muted tracking-wider font-mono">
              Overhaul
            </span>
            <span className="text-sm font-mono font-bold text-status-info">{lastOverhaul}</span>
          </div>
        </div>

        {/* NC-5.4: Temporal Logic - 180-Day Alignment Rule */}
        <div className="px-4 pt-2">
          <div className="flex items-center justify-between p-2 rounded-none bg-status-warning/5 border border-status-warning/20">
              ...
              <span className="text-sm font-mono font-black text-status-warning tabular-nums">
                {Math.max(
                  ...
                  180 -
                    Math.floor(
                      (Date.now() -
                        new Date(identity.lastAlignmentDate || '2025-10-01').getTime()) /
                        (1000 * 60 * 60 * 24)
                    )
                )}
                <span className="text-[10px] ml-1 text-scada-muted font-bold uppercase">
                  Days Left
                </span>
              </span>
            </div>
        ...
        <div className="p-4 grid grid-cols-2 gap-4">
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-1 gap-2">{lubricationFields.map(renderField)}</div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-1 gap-2">{mechanicalFields.map(renderField)}</div>
          </div>
        </div>
        ...
        <div className="p-4 border-t border-scada-border flex gap-2">
          <ModernButton
            ...
            onClick={() =>
              navigate('/hpp-builder', {
                state: { highlightAsset: String(selectedAsset?.id), fromPassport: true },
              })
            }
          >
