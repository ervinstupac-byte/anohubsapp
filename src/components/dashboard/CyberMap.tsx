...
export const CyberMap: React.FC<CyberMapProps> = ({ connections, sovereigntyChainStatus }) => {
  ...
  const secureConnections = connections.filter(
    c => c.encryptionLevel === 'TLS-1.3' && c.certificateValid
  ).length;
          ...
          {connections.map((conn, i) => {
            ...
            return (
              <div key={i}>
                ...
                >
                  <div
                    className={`w-24 h-24 bg-slate-800 border-4 border-${color}-500 rounded-lg flex flex-col items-center justify-center cursor-pointer hover:bg-slate-700 transition-all`}
                  >
                    <div className="text-xs font-bold text-white mb-1">{conn.assetId}</div>
                    <div
                      className={`w-3 h-3 rounded-full bg-${color}-500 mb-1 ${conn.status === 'CONNECTED' ? 'animate-pulse' : ''}`}
                    />
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <div className="text-xs text-slate-400 font-mono">
              Blocks: {sovereigntyChainStatus.length} | Last:{' '}
              {sovereigntyChainStatus.lastBlock.substring(0, 16)}...
            </div>
          </div>
