...
const SparklineMetric: React.FC<SparklineMetricProps> = ({
  ...
  const trend = recentData.length > 1 ? recentData[recentData.length - 1] - recentData[0] : 0;
  ...
  return (
    <div
      className={`bg-slate-900 border transition-colors rounded-none ${
        status === 'alarm'
    ...
    >
      ...
      <div className="flex items-center justify-between mb-2">
        ...
        <div
          className={`flex items-center gap-1 text-[9px] ${
            trend > 0.5 ? 'text-red-400' : trend < -0.5 ? 'text-emerald-400' : 'text-slate-500'
          }`}
        >
          {trend > 0.5 ? (
      ...
      <div className="flex items-end justify-between gap-3">
        ...
        <div className="flex-1 h-8 flex items-center justify-end">
          <Sparkline
            data={
              data.map(d => (isNaN(d) ? 0 : d)).length > 0
                ? data.map(d => (isNaN(d) ? 0 : d))
                : [0, 0]
            }
            width={100}
            height={28}
  ...
  return (
    <div className={`space-y-3 ${className}`}>
      ...
      <div className="flex items-center justify-between">
        ...
        <span className="text-[9px] text-slate-600 font-mono">50-pt buffer</span>
      </div>
