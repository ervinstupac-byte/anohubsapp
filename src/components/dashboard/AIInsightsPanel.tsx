          ...
          >
            <div className="flex items-start gap-3">
              <div className="p-2 rounded-none bg-scada-bg border border-scada-border">
                ...
                ) : boost.finding.toLowerCase().includes('structural') ||
                  boost.finding.toLowerCase().includes('foundation') ? (
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between mb-1">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-[10px] text-status-ok font-mono font-black">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pb-2">
        ...
        {unifiedDiagnosis.trendProjections && (
          <div className="bg-scada-panel border border-scada-border rounded-none p-3 shadow-none">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-[9px] text-scada-text/70 font-mono font-black uppercase tracking-widest">
                Trend Projections
              </span>
            </div>
            <div className="space-y-2">
              {Object.entries(unifiedDiagnosis.trendProjections).map(([param, trend], i) => (
                <div
                  key={i}
                  className="flex justify-between items-end border-b border-scada-border pb-1"
                >
                  <span className="text-[10px] text-scada-text/60 font-mono capitalize">
                    {param}
                  </span>
                  <div className="text-right">
                    <div
                      className={`text-[10px] font-black font-mono ${trend.daysUntilCritical < 90 ? 'text-status-warning' : 'text-status-ok'}`}
                    >
                      {trend.daysUntilCritical} Days
                    </div>
                    <div className="text-[8px] text-scada-text/50 font-mono uppercase">
                      {trend.projectedDate}
                    </div>
                  </div>
                </div>
        ...
        {unifiedDiagnosis.operatingZone && (
          <div className="bg-scada-panel border border-scada-border rounded-none p-3 shadow-none">
            ...
            <div
              className={`text-xs font-black font-mono uppercase px-2 py-1 rounded-none mb-1 text-center ${unifiedDiagnosis.operatingZone.color === 'red' ? 'bg-status-error/10 text-status-error' : 'bg-status-ok/10 text-status-ok'}`}
            >
        ...
        <div className="bg-status-ok/5 border border-status-ok/20 rounded-none p-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[9px] text-status-ok font-mono font-black uppercase tracking-widest">
                Life Extension Alpha
              </span>
            </div>
            <span className="text-[10px] text-status-ok font-black font-mono">
              +{unifiedDiagnosis.lifeExtension.yearsAdded.toFixed(1)} Years
            </span>
          </div>
          <p className="text-[9px] text-scada-text/60 font-mono mt-1">
            Cross-referenced stress margins and thermal optimization ROI.
          </p>
        </div>
      )}
      ...
      {unifiedDiagnosis.serviceNotes && (
        <div className="pt-4 border-t border-scada-border space-y-3">
          <h4 className="text-[8px] text-scada-text/50 uppercase font-mono tracking-[0.2em] mb-2 flex justify-between items-center">
            ...
            <span className="text-[7px] text-scada-text/60 bg-scada-bg px-1 rounded-none">
              CEREBRO V4.9
            </span>
          </h4>
          <div
            ...
            {unifiedDiagnosis.serviceNotes.length > 0 ? (
              unifiedDiagnosis.serviceNotes.map((note, i) => (
                <div
                  key={i}
                  ref={el => {
                    noteRefs.current[note.service] = el;
                  }}
                  className={`p-2 rounded-none border relative overflow-hidden group/note transition-colors duration-300 ${
                    focusedService === note.service
                ...
                >
                  ...
                  />
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-[9px] font-black text-scada-text/80 uppercase tracking-tighter">
                      {note.service}
                    </span>
                    <span
                      className={`text-[7px] font-black font-mono px-1 rounded-none uppercase ${
                        note.severity === 'CRITICAL'
                  ...
                  <p className="text-[10px] text-scada-text/90 font-mono mb-1 leading-tight">
                    {note.message}
                  </p>
                  ...
                  {note.recommendation.includes('[PROTOCOL_IMS_VERIFIED]') && (
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-1 px-1.5 py-0.5 bg-status-ok/10 border border-status-ok/30 rounded-none w-fit">
                        ...
                        <span className="text-[7px] font-black text-status-ok uppercase tracking-widest">
                          ISO IMS Verified
                        </span>
                      </div>
                      ...
                      {note.sourceFiles && note.sourceFiles.length > 0 && (
                        <div className="relative group/evidence">
                          <button className="text-[7px] font-mono font-black text-scada-text/60 hover:text-status-info uppercase flex items-center gap-1 transition-colors">
                  ...
                  <div className="flex items-start gap-1 group/rec">
                    ...
                    <p className="text-[9px] text-status-info font-mono italic flex-1">
                      <span className="text-scada-text/60 not-italic uppercase text-[7px] font-bold mr-1">
                        Rec:
                      </span>
              ...
              <div className="text-center py-4 border border-dashed border-scada-border rounded-none">
                <p className="text-[9px] text-scada-text/60 font-mono italic">
                  No expert service warnings active.
                </p>
              </div>
            )}
          </div>
