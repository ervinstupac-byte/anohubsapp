...
export const CenturyROIChart: React.FC = () => {
  ...
  const PRICE_OPTIONS = [0.08, 0.1, 0.15];
    ...
    (async () => {
      try {
        ...
        if (mod && typeof mod.FinancialImpactEngine !== 'undefined') {
          ...
          const impact = mod.FinancialImpactEngine.calculateImpact(fakeState, physics as any, {
            pricePerMWh: price * 1000,
            sigma,
            inventoryValue,
          });
  ...
  const series = useMemo(() => {
    ...
    const inventoryValue = getTotalInventoryValue();
    const impact: any = impactOverride || {
      lostRevenueEuro: 0,
      leakageCostYearly: 0,
      maintenanceBufferEuro: 0,
      maintenanceSavingsEuro: 0,
      expectedMaintenanceCost: 0,
      lifeExtensionSavings: 0,
    };
    ...
    const baselinePowerMW =
      (fakeState.hydraulic?.baselineOutputMW &&
        Number((fakeState.hydraulic as any).baselineOutputMW)) ||
      5; // fallback
    ...
    const annualCosts =
      ...
      (impact.maintenanceBufferEuro || 0) -
      (impact.maintenanceSavingsEuro || 0) +
      (impact.expectedMaintenanceCost || 0) * pfFactor;
  ...
  useEffect(() => {
    ...
    const y = d3
      .scaleLinear()
      .domain(d3.extent(series, d => d.cum) as [number, number])
      .nice()
      .range([innerH, 0]);
