...
export const CenturyROIChart: React.FC = () => {
    ...
    // compute Pf from telemetry_history_cache if available
    useEffect(() => {
        let mounted = true;
        (async () => {
            try {
                ...
                if (!data) {
                    ...
                    if (mounted) setPf(Math.min(99.99, Math.max(0.01, pfVal * 100)));
                }
            } catch (e) {
                // ignore
            }
    ...
    const series = useMemo(() => {
        ...
        const impact = FinancialImpactEngine.calculateImpact(fakeState, physics as any, { pricePerMWh, sigma, inventoryValue });
