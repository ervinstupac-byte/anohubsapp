...
export const ForensicVisualizer: React.FC<ForensicVisualizerProps> = ({
  viewMode,
  onHotspotSelect,
}) => {
  ...
  const activeData = activeHotspot
    ? getStandardData(hotspots.find(h => h.id === activeHotspot)?.type || '')
    : null;
          ...
          >
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-start gap-2 bg-slate-800/50 p-2 rounded">
                ...
                <div className="text-xs text-slate-300 italic">"{activeData.context}"</div>
              </div>
            </div>
          </motion.div>
        )}
