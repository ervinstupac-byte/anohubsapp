...
const ExecutiveRibbon: React.FC = () => {
  ...
  const totalRev = Object.values(units).reduce(
    (acc: number, u: any) => acc + (u.financials?.currentRevenueEURh || 0),
    0
  );
...
export const MasterControlRoom: React.FC = () => {
  ...
  const hud = useMemo(() => {
    ...
    const badgeBg =
      delta > 3 ? 'bg-red-500/15 border-red-500/30' : 'bg-emerald-500/15 border-emerald-500/30';
          ...
          <div className="flex items-center gap-10">
            <div>
              <h1 className="text-4xl font-black tracking-tighter text-white flex items-center gap-3">
                ...
                <span
                  className={`text-xs font-mono ${theme.text} ${theme.bg} border ${theme.border} px-3 py-1 rounded-none ml-4 tracking-widest`}
                >
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-none p-4">
          <div className="flex items-center justify-between">
            ...
            <div className="text-[10px] font-mono text-slate-400">
              Selected: <span className="text-emerald-300 font-bold">{selectedVariant}</span>
            </div>
          </div>
        ...
        <div className="grid grid-cols-12 gap-8">
          ...
          <div className="col-span-5">
            ...
            <div className="mt-6 bg-slate-900 border border-slate-800 rounded-none p-6">
              ...
              <div className="mt-6">
                {selectedAsset &&
                  (selectedAsset.turbine_type || selectedAsset.type) === 'PELTON' && (
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-none p-6 space-y-6">
              ...
              <div className="space-y-6">
                <div className="flex items-center justify-between p-4 bg-slate-950 border border-slate-800 rounded-none">
                  <div>
                    <div className="text-[10px] text-slate-500 uppercase font-black tracking-widest">
                      Current Spot Revenue
                    </div>
                    <div className="text-2xl font-black text-white mt-1">
                      €180.00 <span className="text-xs font-normal text-slate-500">MWh</span>
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="text-[10px] text-slate-500 uppercase font-black tracking-widest">
                    ...
                    <div className="text-xl font-black text-emerald-400 tabular-nums">
                      ...
                      {(
                        Object.values(units).reduce(
                          (acc, u: any) =>
                            (acc as number) + (u.financials?.dailyTotalProducedMWh || 0) * 180,
                          0
                        ) as number
                      ).toLocaleString()}
                    </div>
      ...
      <div className="absolute bottom-6 right-6 z-[1000] w-80 pointer-events-none hidden md:block">
        <div className="bg-slate-950/85 backdrop-blur-md border border-slate-700/50 rounded-none p-4 shadow-2xl space-y-3 overflow-hidden relative">
          ...
          <div className="grid grid-cols-2 gap-3 text-[10px] font-mono relative z-10">
            ...
            <div
              className={`col-span-2 mt-1 px-2 py-1 rounded-none border ${hud.badgeBg} flex items-center justify-between`}
            >
