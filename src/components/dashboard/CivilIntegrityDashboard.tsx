...
export const CivilIntegrityDashboard: React.FC<CivilIntegrityProps> = ({
  damData,
  safetyFactor,
  structuralHealth,
}) => {
      ...
      {structuralHealth !== 'EXCELLENT' && structuralHealth !== 'GOOD' && (
        <div
          className={`mb-6 p-4 rounded-lg border-2 ${
            structuralHealth === 'CRITICAL'
              ? 'bg-red-950 border-red-500'
              : structuralHealth === 'WARNING'
        ...
        >
          <div className="flex items-center gap-2">
            <AlertTriangle
              className={`w-6 h-6 ${
                structuralHealth === 'CRITICAL' ? 'text-red-400 animate-pulse' : 'text-amber-400'
              }`}
            />
            <div className="text-lg font-bold">
              {structuralHealth === 'CRITICAL'
                ? '🚨 CRITICAL STRUCTURAL ALERT'
                : structuralHealth === 'WARNING'
          ...
          <div className="relative h-3 bg-slate-800 rounded-full overflow-hidden mb-4">
            <div
              className={`absolute inset-y-0 left-0 transition-all ${
                seepagePct > 80 ? 'bg-red-500' : seepagePct > 60 ? 'bg-amber-500' : 'bg-cyan-500'
              }`}
          ...
          <div className="grid grid-cols-4 gap-3">
            {damData.piezometers.map(piezo => (
              <div key={piezo.id} className="bg-slate-800 rounded p-3">
                ...
                <div className="text-xs text-slate-600 mt-1">@ {piezo.elevation.toFixed(0)}m</div>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="grid grid-cols-3 gap-3">
            {damData.pendulums.map(pend => {
              ...
              return (
                <div
                  key={pend.id}
                  className={`bg-slate-800 rounded p-3 border-2 ${
                    exceedsThreshold ? 'border-red-500' : 'border-slate-700'
                  }`}
                >
                  <div className="flex items-center justify-between mb-2">
                    <div className="text-xs font-bold text-white">{pend.id}</div>
                    {exceedsThreshold && <AlertTriangle className="w-4 h-4 text-red-400" />}
                  </div>
                  <div className="text-xs text-slate-500 mb-2">{pend.location}</div>

                  <div
                    className={`text-3xl font-bold font-mono mb-1 ${
                      exceedsThreshold ? 'text-red-400' : 'text-orange-300'
                    }`}
                  >
                    {pend.displacement.toFixed(1)}
                  </div>
                  <div className="text-xs text-slate-500 mb-2">mm {pend.direction}</div>

                  <div className="relative h-2 bg-slate-700 rounded-full overflow-hidden">
                    <div
                      className={`absolute inset-y-0 left-0 ${
                        exceedsThreshold ? 'bg-red-500' : 'bg-orange-500'
                      }`}
                      style={{ width: `${Math.min(100, pct)}%` }}
                    />
                  </div>
                  <div className="text-xs text-slate-600 mt-1">Threshold: {pend.threshold} mm</div>
                </div>
              );
            })}
          </div>
        ...
        <div className="col-span-2 bg-slate-900 border border-slate-700 rounded-lg p-4">
          <div className="grid grid-cols-3 gap-4">
            ...
            <div className="bg-slate-800 rounded p-3">
              ...
              <div className="text-sm text-slate-300 font-mono">30 days ago</div>
            </div>
          </div>
        </div>
      </div>
