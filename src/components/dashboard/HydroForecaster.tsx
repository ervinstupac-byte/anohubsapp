        ...
        <div className="col-span-2">
          <div className="bg-slate-900 border border-slate-700 rounded-lg p-4 h-full">
            <div className="text-xs text-slate-400 font-bold mb-4">
              Water Arrival Timeline (Next 24h)
            </div>
                ...
                {upstreamStations.map((station, i) => (
                  <div key={i} className="bg-slate-800 rounded p-2 border border-slate-700">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <CloudRain
                        className={`w-3 h-3 ${station.rainIntensity > 5 ? 'text-blue-400 animate-pulse' : 'text-slate-600'}`}
                      />
                    </div>
...
const ForecastMetric: React.FC<{ label: string; value: string; color?: string }> = ({
  label,
  value,
  color = 'blue',
}) => {
