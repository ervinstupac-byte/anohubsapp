    ...
    <div className="w-full bg-gradient-to-br from-slate-900 to-slate-950 border border-amber-500/30 rounded-lg p-6">
      ...
      <div className="space-y-3">
        {leaks.map(leak => {
          ...
          const statusColor =
            leak.currentCompensation > 70
              ? 'emerald'
              : leak.currentCompensation > 30
                ? 'amber'
                : 'red';
          ...
          return (
            ...
            >
              <div className="flex items-start justify-between mb-2">
                <div>
                  <div className="text-xs font-bold text-white font-mono">{leak.assetId}</div>
                  <div className="text-[10px] text-slate-400 font-mono">{leak.source}</div>
                </div>
              ...
              <div className="grid grid-cols-3 gap-2 text-[10px] font-mono">
                <div>
                  <span className="text-red-400">
                    Loss: €{(leak.baselineLoss / 1000).toFixed(0)}k
                  </span>
                </div>
                <div>
                  <span className="text-emerald-400">
                    Recovered: €{(leak.recoveredValue / 1000).toFixed(0)}k
                  </span>
                </div>
      ...
      <div className="mt-4 pt-4 border-t border-slate-700 text-xs text-slate-400 font-mono text-center">
        Software ROI:{' '}
        {totalRecovered > 0
          ? `€${totalRecovered.toLocaleString()}/year recovered`
          : 'Calculating...'}
      </div>
    </div>
  );
