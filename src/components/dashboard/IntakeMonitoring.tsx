...
export const IntakeMonitoring: React.FC<IntakeMonitoringProps> = ({
  intakeData,
  cleaningRequired,
  lastCleaning,
}) => {
  ...
  return (
    <div className="w-full bg-slate-950 p-6">
      ...
      <div className="grid grid-cols-3 gap-6">
        {/* Trash Rack ΔP */}
        <div
          className={`bg-slate-900 border-2 rounded-lg p-4 ${
            cleaningRequired ? 'border-amber-500' : 'border-emerald-500'
          }`}
        >
          <div className="flex items-center justify-between mb-3">
            <div>
              ...
              <div
                className={`text-3xl font-bold font-mono ${
                  cleaningRequired ? 'text-amber-300' : 'text-emerald-300'
                }`}
              >
            ...
            {cleaningRequired && <AlertTriangle className="w-8 h-8 text-amber-400 animate-pulse" />}
          </div>
          ...
          <div className="relative h-3 bg-slate-800 rounded-full overflow-hidden mb-2">
            <div
              className={`absolute inset-y-0 left-0 transition-all ${
                cleaningRequired ? 'bg-amber-500' : 'bg-emerald-500'
              }`}
        ...
        <div className="bg-slate-900 border border-slate-700 rounded-lg p-4">
          ...
          <div className="space-y-3">
            <StatusMetric label="Water Level" value={`${intakeData.waterLevel.toFixed(1)} m`} />
            <StatusMetric label="Flow Rate" value={`${intakeData.flowRate.toFixed(1)} m³/s`} />
            <StatusMetric label="Last Cleaning" value={`${daysSinceCleaning} days ago`} />
          </div>
        </div>
      </div>
        ...
        <div className="relative h-48 bg-slate-800 rounded p-4">
          ...
          <svg width="100%" height="100%" className="text-slate-400">
            ...
            <rect
              ...
              strokeWidth="2"
            />
            <text x="120" y="85" textAnchor="middle" fill="#38bdf8" fontSize="14" fontWeight="bold">
              Reservoir
            </text>
            ...
            <g>
              <rect
                ...
                height="100"
                fill="none"
                stroke={cleaningRequired ? '#f59e0b' : '#10b981'}
                strokeWidth="3"
              />
              {[0, 1, 2, 3, 4].map(i => (
                <line
              ...
              <text
                ...
                y="155"
                textAnchor="middle"
                fill={cleaningRequired ? '#f59e0b' : '#10b981'}
                fontSize="12"
                fontWeight="bold"
              >
                Trash Rack
            ...
            <text x="260" y="55" textAnchor="middle" fill="white" fontSize="10" fontWeight="bold">
              ΔP
            </text>
            ...
            <rect
              ...
              strokeWidth="2"
            />
            <text x="375" y="85" textAnchor="middle" fill="white" fontSize="12">
              → Turbine
            </text>
          </svg>
        </div>
...
const QualityMetric: React.FC<{ label: string; value: string; status: 'normal' | 'warning' }> = ({
  label,
  value,
  status,
}) => {
  return (
    <div className="flex items-center justify-between">
      ...
      <div
        className={`text-sm font-mono font-bold ${status === 'normal' ? 'text-emerald-400' : 'text-amber-400'}`}
      >
        {value}
      </div>
    </div>
