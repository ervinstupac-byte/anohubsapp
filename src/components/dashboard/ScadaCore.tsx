...
export const ScadaCore: React.FC<{ focusMode?: boolean; forensicMode?: boolean }> = ({
  focusMode = false,
  forensicMode = false,
}) => {
  ...
  const headM = useMemo(
    () => Number(hydraulic?.head ?? physics?.netHead ?? 0),
    [hydraulic, physics]
  );
  ...
  const ratedSpeed = useMemo(
    () => Number(identity?.machineConfig?.ratedSpeedRPM ?? 500),
    [identity]
  );
  ...
  const [hotspot, setHotspot] = useState<null | {
    key: 'bearing' | 'stator' | 'rotor' | 'head' | 'flow';
    label: string;
  }>(null);
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      ...
      const flowSeries = Array.isArray(telemetryHistory?.flow)
        ? (telemetryHistory.flow as any[])
        : [];
      ...
      const status = SafetyInterlockEngine.checkProtections(
        speedPct,
        powerMW,
        false,
        vibrationTotal,
  ...
  const gauge = useMemo(() => {
    ...
    const maxRpm =
      family === 'KAPLAN'
        ? 300
        : family === 'FRANCIS'
          ? TL.trip // 900 RPM (trip point)
          : 1200;
    const redline =
      family === 'KAPLAN'
        ? 255
        : family === 'FRANCIS'
          ? TL.overspeed // 840 RPM (overspeed)
          : 1000;
  ...
  const surgePressureBar = useMemo(() => {
    ...
    const fromNumbers = Number(
      physics?.surgePressureBar ?? physics?.waterHammerPressureBar ?? physics?.surgePressure ?? 0
    );
    return Number.isFinite(fromDecimal) && fromDecimal !== 0 ? fromDecimal : fromNumbers;
  }, [
    physics?.surgePressure,
    physics?.surgePressureBar,
    physics?.waterHammerPressureBar,
    physics?.surgePressure,
  ]);
  ...
  useEffect(() => {
    ...
    const result = PhysicsMathService.calculateHoopStress(
      headM,
      surgePressureBar,
      diameter,
      thickness
  ...
  const variantCfg = useMemo(
    () => cfgStore.getVariantConfig(selectedVariant) ?? cfgStore.getConfig(family as any),
    [cfgStore, selectedVariant, family]
  );
  ...
  const eta = useMemo(
    () => computeEfficiencyFromHillChart(family as any, headM, flowM3s),
    [family, headM, flowM3s]
  );
  const mechPowerMW = useMemo(
    () => (1000 * 9.81 * headM * flowM3s * eta) / 1e6,
    [headM, flowM3s, eta]
  );
  const gateOpening = useMemo(() => {
    const gv = Number(
      hydraulic?.guideVaneOpening ?? specializedState?.sensors?.guide_vane_opening ?? 0
    );
  ...
  const lossTracer = useMemo(() => {
    ...
    const { etaMax } = EfficiencyOptimizer.compute(
      headM || 0,
      flowM3s || 0,
      isNaN(observedEffPct) ? 0 : observedEffPct
    );
    ...
    const deltaEffFrac = Math.max(
      0,
      ((etaMax ?? 0) - (isNaN(observedEffPct) ? 0 : observedEffPct)) / 100
    );
    const baseLossEuroPerHour = Math.max(
      0,
      deltaEffFrac * Math.max(0, powerMW) * Math.max(0, pricePerMWh)
    );
    ...
    const rope = isPartLoad
      ? VortexDiagnostic.analyze([Number(mechanical?.vibrationX ?? 0)], 20)
      : ({ isRopeActive: false } as any);
    const taxThreshold = 0.88;
    const effDelta = Math.max(
      0,
      taxThreshold - Number(hydraulic?.efficiency ?? observedEffPct / 100)
    );
  ...
  useEffect(() => {
    ...
    if (specificSpeed < min || specificSpeed > max) {
      pushAlarm?.({
        id: `DESIGN_${Date.now()}_${Math.random().toString(36).substr(2, 5)}`,
        severity: 'WARN',
        message: 'Design Conflict: Specific Speed out of range',
      });
    }
    if (family === 'KAPLAN' && (cfgStore.getConfig('KAPLAN')?.ratedHeadHn ?? 0) > 100) {
      pushAlarm?.({
        id: `HEAD_${Date.now()}_${Math.random().toString(36).substr(2, 5)}`,
        severity: 'WARN',
        message: 'Design Conflict: Kaplan head exceeds limit',
      });
    }
    if (eta > 0.98) {
      pushAlarm?.({
        id: `ETA_${Date.now()}_${Math.random().toString(36).substr(2, 5)}`,
        severity: 'WARN',
        message: 'Design Conflict: Efficiency exceeds 98%',
      });
    }
  ...
  const sparkData = useMemo(() => {
    ...
    return (Array.isArray(src) ? src : []).map((d: any, idx: number) => ({
      x: idx,
      y: Number(d?.value ?? d ?? 0),
    }));
  ...
  const shakeStyle =
    vibrationTotal > 5
      ? { animation: `shake ${Math.max(0.1, 10 / vibrationTotal)}s infinite` }
      : {};
  ...
  const noData =
    !store ||
    ((Number.isNaN(headM) || headM === 0) &&
      (Number.isNaN(flowM3s) || flowM3s === 0) &&
      (Number.isNaN(rpm) || rpm === 0));
      ...
      <div className="col-span-8 bg-slate-900 border border-slate-600 rounded-none relative">
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800">
            ...
            <div className="flex items-center gap-4">
              <div className="text-[10px] font-mono text-slate-400">
                P_mech: {mechPowerMW.toFixed(2)} MW • η: {(eta * 100).toFixed(1)}% • v₂:{' '}
                {dischargeVelocityV2.toFixed(2)} m/s
              </div>
              <button
                onClick={() =>
                  window.open(
        ...
        {!focusMode && (
          <div className="px-6 py-1">
            {family === 'PELTON' && (
              <div className="text-[10px] font-mono text-slate-400">Pelton Wheel</div>
            )}
            {family === 'KAPLAN' && (
              <div className="text-[10px] font-mono text-slate-400">Inline Bulb</div>
            )}
            {family === 'FRANCIS' && (
              <div className="text-[10px] font-mono text-slate-400">Spiral Case</div>
            )}
            {family === 'BANKI' && (
              <div className="text-[10px] font-mono text-slate-400">Crossflow Runner</div>
            )}
          </div>
        )}

        <div
          className={`p-6 transition-all duration-500 ${isComfortMode ? 'sepia-[.3] contrast-125' : ''}`}
        >
          <style>
            {`
          ...
          <svg viewBox="0 0 1200 600" className="w-full h-[520px]">
            ...
            <g className="equipment-shadow">
              <rect
                ...
                fill={lossTracer.isRope ? '#3a0c0c' : '#0f1419'}
                stroke={lossTracer.isRope ? '#dc2626' : '#374151'}
              />
              <text
                x="950"
                y="50"
                className="sensor-label"
                fill={lossTracer.isRope ? '#fca5a5' : '#a7f3d0'}
                fontWeight="bold"
              >
              ...
              <>
                ...
                <g className="equipment-shadow">
                  ...
                  <line
                    ...
                    className="flow-path"
                    style={{
                      animation: `dashFlow ${Math.max(0.5, 3 - flowM3s / 30)}s linear infinite`,
                    }}
                  />
                  <foreignObject x="60" y="185" width="200" height="30">
                    <div className="text-center">
                      <SmartTooltip term="Hoop Stress">
                        <span className="sensor-label" style={{ fill: '#cbd5e1' }}>
                          PENSTOCK
                        </span>
                      </SmartTooltip>
                    </div>
                ...
                <g className="equipment-shadow">
                  <path
                    d="M 280 120 Q 380 100 460 140 T 580 180 L 580 220 Q 500 240 420 220 T 280 180 Z"
                    fill={
                      showThermalOverlay ? 'url(#thermalStressGradient)' : 'url(#metalGradient)'
                    }
                    stroke={showThermalOverlay ? '#ef4444' : '#2d3748'}
                    strokeWidth="2"
                  />
                ...
                <g transform="translate(600,300)" className="equipment-shadow" style={shakeStyle}>
                  <circle
                    ...
                    stroke={eta > 0.9 ? '#10b981' : eta < 0.7 ? '#ef4444' : '#2d3748'}
                    strokeWidth="3"
                  />
                  ...
                  {Array.from({ length: 16 }).map((_, i) => (
                    <g key={i} transform={`rotate(${i * 22.5})`}>
                      <path
                        d="M 0,-70 L 8,-60 L 12,-40 L 8,-20 L 0,-15 L -8,-20 L -12,-40 L -8,-60 Z"
                        fill="#4a5568"
                        stroke="#2d3748"
                        strokeWidth="1"
                      />
                  ...
                  <circle
                    ...
                    strokeWidth="1"
                    strokeDasharray="5,5"
                    opacity={rpm > 0 ? 0.8 : 0.3}
                    style={{
                      animation:
                        rpm > 0
                          ? `pulseRotate ${Math.max(2, 10 - rpm / 50)}s linear infinite`
                          : 'none',
                    }}
                  />
                ...
                <g className="equipment-shadow">
                  <path
                    d="M 560 380 L 540 450 L 660 450 L 640 380 Z"
                    fill="url(#metalGradient)"
                    stroke="#2d3748"
                    strokeWidth="2"
                  />
                  ...
                  <foreignObject x="700" y="380" width="100" height="28">
                    <div className="flex items-center justify-center h-full">
                      <SmartTooltip term="Erosion Rate">
                        <span
                          className="sensor-label"
                          style={{ fill: '#fca5a5', fontSize: '10px' }}
                        >
                          EROSION MON
                        </span>
                      </SmartTooltip>
                ...
                <g className="equipment-shadow">
                  ...
                  <text x="630" y="68" className="sensor-label" fill="#a7f3d0">
                    BEARING{' '}
                    {isNaN(
                      Number(
                        Array.isArray(telemetryHistory?.bearingTemp)
                          ? telemetryHistory.bearingTemp.slice(-1)[0]?.value
                          : undefined
                      )
                    )
                      ? 'N/A'
                ...
                <g className="equipment-shadow">
                  ...
                  <text x="830" y="447" className="sensor-label" fill="#93c5fd">
                    P {pressures.pStaticKpa.toFixed(1)} kPa
                  </text>
                </g>

                <g className="equipment-shadow">
                  <rect x="720" y="250" width="100" height="28" rx="4" className="digital-tag" />
                  <text x="730" y="267" className="sensor-label" fill="#fde68a">
                    V {vibrationTotal.toFixed(2)} mm/s
                  </text>
                </g>

                <g className="equipment-shadow">
                  <rect x="480" y="200" width="140" height="28" rx="4" className="digital-tag" />
                  <text x="490" y="217" className="sensor-label" fill="#7dd3fc">
                    WICKET GATES {gateOpening.toFixed(1)}%
                  </text>
                </g>
              ...
              <>
                ...
                <g className="equipment-shadow">
                  <rect
                    ...
                    strokeWidth="2"
                  />
                  <text x="210" y="165" textAnchor="middle" className="sensor-label" fill="#cbd5e1">
                    PENSTOCK
                  </text>
                  ...
                  {Array.from({
                    length: Math.max(
                      ...
                      Math.min(
                        6,
                        variantCfg?.pelton?.nozzleCount ??
                          cfgStore.getConfig('PELTON')?.pelton?.nozzleCount ??
                          2
                      )
                    ),
                  }).map((_, i) => (
                    <g key={i} transform={`translate(${380 + i * 80}, 160)`}>
                      ...
                      <line
                        ...
                        className="flow-path"
                        style={{
                          animation: `dashFlow ${Math.max(0.4, 2 - flowM3s / 40)}s linear infinite`,
                        }}
                      />
                    </g>
                  ))}
                ...
                <g transform="translate(640,280)" className="equipment-shadow">
                  ...
                  {Array.from({ length: 20 }).map((_, i) => (
                    <g key={i} transform={`rotate(${i * 18})`}>
                      <path
                        d="M 0,-100 L 15,-85 L 20,-70 L 15,-55 L 0,-50 L -15,-55 L -20,-70 L -15,-85 Z"
                        fill="#4a5568"
                        stroke="#2d3748"
                        strokeWidth="1"
                      />
                ...
                <g className="equipment-shadow">
                  <rect
                    ...
                    strokeWidth="2"
                  />
                  <text x="640" y="455" textAnchor="middle" className="sensor-label" fill="#cbd5e1">
                    CASING/DISCHARGE
                  </text>
                </g>
                ...
                <g className="equipment-shadow">
                  ...
                  <text x="730" y="97" className="sensor-label" fill="#a7f3d0">
                    BEARING{' '}
                    {isNaN(
                      Number(
                        Array.isArray(telemetryHistory?.bearingTemp)
                ...
                <g className="equipment-shadow">
                  ...
                  <text x="930" y="287" className="sensor-label" fill="#93c5fd">
                    P {pressures.pStaticKpa.toFixed(1)} kPa
                  </text>
                </g>
                ...
                <g className="equipment-shadow">
                  ...
                  <text x="530" y="367" className="sensor-label" fill="#fde68a">
                    V {vibrationTotal.toFixed(2)} mm/s
                  </text>
                </g>
            ...
            {family === 'KAPLAN' && (
              <>
                ...
                <g className="equipment-shadow">
                  ...
                  />
                  <text x="160" y="245" textAnchor="middle" className="sensor-label" fill="#cbd5e1">
                    INLET CONDUIT
                  </text>
                </g>
                ...
                <g transform="translate(600,250)" className="equipment-shadow">
                  ...
                  {Array.from({
                    length:
                      variantCfg?.kaplan?.bladeCount ??
                      cfgStore.getConfig('KAPLAN')?.kaplan?.bladeCount ??
                      4,
                  }).map((_, i) => (
                    <g
                      key={i}
                      transform={`rotate(${i * (360 / (variantCfg?.kaplan?.bladeCount ?? cfgStore.getConfig('KAPLAN')?.kaplan?.bladeCount ?? 4))})`}
                    >
                      <path
                        d="M 0,-50 L 12,-45 L 18,-30 L 15,-15 L 0,-10 L -15,-15 L -18,-30 L -12,-45 Z"
                        fill="#4a5568"
                        stroke="#2d3748"
                        strokeWidth="1"
                      />
                ...
                <g className="equipment-shadow">
                  <path
                    d="M 540 310 L 520 400 L 680 400 L 660 310 Z"
                    fill="url(#metalGradient)"
                    stroke="#2d3748"
                    strokeWidth="2"
                  />
                  <text x="600" y="365" textAnchor="middle" className="sensor-label" fill="#cbd5e1">
                    DRAFT TUBE
                  </text>
                </g>
                ...
                <g className="equipment-shadow">
                  ...
                  <text x="730" y="167" className="sensor-label" fill="#a7f3d0">
                    BEARING{' '}
                    {isNaN(
                      Number(
                        Array.isArray(telemetryHistory?.bearingTemp)
                ...
                <g className="equipment-shadow">
                  ...
                  <text x="930" y="247" className="sensor-label" fill="#93c5fd">
                    P {pressures.pStaticKpa.toFixed(1)} kPa
                  </text>
                </g>
                ...
                <g className="equipment-shadow">
                  ...
                  <text x="530" y="297" className="sensor-label" fill="#fde68a">
                    V {vibrationTotal.toFixed(2)} mm/s
                  </text>
                </g>
                ...
                <g className="equipment-shadow">
                  ...
                  <text x="850" y="287" className="sensor-label" fill="#7dd3fc">
                    BLADE ANGLE {gateOpening.toFixed(1)}°
                  </text>
                </g>
            ...
            {family === 'BANKI' && (
              <>
                ...
                <g className="equipment-shadow">
                  ...
                  />
                  <text x="160" y="265" textAnchor="middle" className="sensor-label" fill="#cbd5e1">
                    INLET PIPE
                  </text>
                </g>
                ...
                <g className="equipment-shadow">
                  ...
                  <rect
                    ...
                    opacity="0.3"
                  />
                  <text x="600" y="195" textAnchor="middle" className="sensor-label" fill="#cbd5e1">
                    CASING
                  </text>
                </g>
                ...
                <g className="equipment-shadow">
                  <rect
                    ...
                    strokeWidth="2"
                  />
                  <text x="880" y="295" textAnchor="middle" className="sensor-label" fill="#cbd5e1">
                    DISCHARGE
                  </text>
                </g>
                ...
                <g className="equipment-shadow">
                  ...
                  <text x="690" y="157" className="sensor-label" fill="#a7f3d0">
                    BEARING{' '}
                    {isNaN(
                      Number(
                        Array.isArray(telemetryHistory?.bearingTemp)
                ...
                <g className="equipment-shadow">
                  ...
                  <text x="1010" y="287" className="sensor-label" fill="#93c5fd">
                    P {pressures.pStaticKpa.toFixed(1)} kPa
                  </text>
                </g>
                ...
                <g className="equipment-shadow">
                  ...
                  <text x="490" y="357" className="sensor-label" fill="#fde68a">
                    V {vibrationTotal.toFixed(2)} mm/s
                  </text>
                </g>
            ...
            <div className="bg-slate-900 border border-slate-700 rounded-none w-[600px] shadow-none">
              ...
              <div className="p-4 max-h-[60vh] overflow-y-auto space-y-2">
                {preStartChecks.map(check => (
                  <div
                    key={check.id}
                    className={`p-3 rounded-none border flex justify-between items-center ${
                  ...
                  >
                    ...
                    <div
                      className={`text-[10px] font-black px-2 py-1 rounded-none ${
                        check.status === 'OK'
        ...
        {starting && (
          <div className="absolute top-4 right-4 bg-slate-900/90 border border-slate-700 rounded-none p-4">
            ...
            <div className="relative w-36 h-36 rounded-full border-4 border-slate-600 bg-slate-800 shadow-none">
              ...
              <div
                className="absolute left-1/2 top-1/2 w-0.5 h-14 bg-emerald-400 origin-bottom shadow-[0_0_10px_rgba(16,185,129,0.5)]"
            ...
            <div className="w-[520px] bg-slate-900 border border-slate-700 rounded-none shadow-none">
              ...
              <div className="p-4">
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      pushAlarm?.({
                        id: `OVRD_${Date.now()}`,
                        severity: 'WARNING',
                        message: `Manual override on ${hotspot.label}`,
                      });
                    }}
                  >
                    Manual Override
                ...
                <div className="text-xs font-mono text-slate-300 mb-2">
                  Last Verified Value:{' '}
                  {lastVerifiedValue.value !== null ? lastVerifiedValue.value.toFixed(2) : 'N/A'}
                </div>
                {sparkData.length > 0 ? (
      ...
      <div className="col-span-4 bg-[#0c0c0c] border border-[#222222] rounded-none p-6 space-y-6 overflow-y-auto max-h-[calc(100vh-200px)]">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="col-span-2">
              ...
              <div className="flex items-center gap-2">
                  ...
                  onChange={e => {
                    try {
                      ...
                      if (family === 'KAPLAN' && v > 100) {
                        pushAlarm?.({
                          id: `HEAD_INPUT_${Date.now()}`,
                          severity: 'WARN',
                          message: 'Blocked: Kaplan head cannot exceed 100 m',
                        });
                        return;
                      }
                ...
                />
                <div className="px-2 py-2 bg-[#0a0a0a] border border-[#2a2a2a] rounded-none text-[10px] font-mono text-slate-400">
                  m
                </div>
              </div>
            </div>
            <div className="col-span-2">
              ...
              <div className="flex items-center gap-2">
                  ...
                  onChange={e => {
                    try {
                      ...
                      if (family && cfgStore.getConfig(family)) {
                        ...
                        try {
                          const etaChk = computeEfficiencyFromHillChart(
                            family as any,
                            cfgStore.getConfig(family as any)?.ratedHeadHn ?? 0,
                            v
                          );
                          if (etaChk > 0.98) {
                            pushAlarm?.({
                              id: `ETA_INPUT_${Date.now()}`,
                              severity: 'WARN',
                              message: 'Design Conflict: Efficiency exceeds 98%',
                            });
                          }
                ...
                />
                <div className="px-2 py-2 bg-[#0a0a0a] border border-[#2a2a2a] rounded text-[10px] font-mono text-slate-400">
                  m³/s
                </div>
              </div>
            </div>
        ...
        <div>
          ...
          <div className="relative w-56 h-56 rounded-full border-4 border-slate-700 bg-slate-800 mx-auto">
            <div className="absolute inset-3 rounded-full border-2 border-slate-700" />
            <div
              className="absolute left-1/2 top-1/2 w-0.5 h-24 bg-cyan-400 origin-bottom shadow-[0_0_10px_rgba(34,211,238,0.5)]"
              style={{ transform: `translate(-50%, -100%) rotate(${gauge.angle}deg)` }}
            />
            ...
            <div className="absolute left-1/2 top-1/2 w-48 h-48 -translate-x-1/2 -translate-y-1/2">
              <svg viewBox="0 0 200 200" className="w-full h-full">
                ...
                />
                <path
                  d={`M${20 + (gauge.redStart > 0 ? 0 : 0)},100 A80,80 0 0,1 180,100`}
                  stroke="#ef4444"
                  strokeWidth="8"
                  fill="none"
                  opacity="0.2"
            ...
            <div className="absolute bottom-3 left-1/2 -translate-x-1/2 text-[12px] font-mono text-slate-300">
              RPM {rpm.toFixed(0)} / {gauge.maxRpm}
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="p-4 bg-slate-950 border border-slate-800 rounded-lg">
              ...
              <div className="text-2xl font-black text-white">
                {hoopStress !== null ? hoopStress.toFixed(2) : 'N/A'} MPa
              </div>
              <div className="mt-2 h-24 bg-slate-800 rounded relative overflow-hidden">
                <div
                  ...
                  style={{
                    height: `${Math.max(5, Math.min(100, hoopStress !== null ? hoopStress / 10 : 0))}%`,
                  }}
                />
              </div>
            </div>
            <div className="p-4 bg-slate-950 border border-slate-800 rounded-lg">
              ...
              <div className="text-2xl font-black text-white">
                {pressures.pStaticKpa.toFixed(1)}
              ...
              <div className="mt-2 h-24 bg-slate-800 rounded relative overflow-hidden">
                <div
                  className="absolute bottom-0 left-0 right-0 bg-emerald-500"
                  style={{
                    height: `${Math.max(5, Math.min(100, pressures.pStaticKpa / (family === 'PELTON' ? 30 : 10)))}%`,
                  }}
                />
              </div>
            </div>
            <div className="p-4 bg-slate-950 border border-slate-800 rounded-lg">
              ...
              <div className="text-2xl font-black text-white">
                {pressures.pDynamicKpa.toFixed(1)}
                <span className="text-xs text-slate-400 ml-1">kPa</span>
              </div>
              <div className="mt-2 h-24 bg-slate-800 rounded relative overflow-hidden">
                <div
                  ...
                  style={{
                    height: `${Math.max(5, Math.min(100, pressures.pDynamicKpa / (family === 'PELTON' ? 15 : 5)))}%`,
                  }}
                />
              </div>
            </div>
        ...
        <div className="bg-slate-900 border border-slate-700 rounded-none">
          <div className="flex items-center justify-between px-4 py-2 bg-slate-800/50 border-b border-slate-700">
            <div className="flex items-center gap-2">
              ...
              <span
                className={`w-2 h-2 rounded-full ${activeAlarms?.length ? 'bg-red-500 animate-pulse' : 'bg-emerald-500'}`}
              />
            </div>
            <button
              className="text-xs px-2 py-1 rounded-none bg-slate-800 border border-slate-600 hover:bg-slate-700 text-slate-300 font-mono"
              onClick={() => {
                EventLogger.log('ACKNOWLEDGE', null, null, 'ALL');
          ...
          <div className="px-4 py-2 space-y-1 max-h-32 overflow-y-auto">
            {(activeAlarms ?? []).length === 0 && (
              <div className="text-xs font-mono text-slate-500 italic">No active alarms</div>
            )}
            {(activeAlarms ?? []).slice(-5).map((a: any) => (
              <div
                key={a.id}
                className={`text-xs font-mono flex gap-2 ${a.severity === 'CRITICAL' ? 'text-red-400' : a.severity === 'WARNING' ? 'text-amber-300' : 'text-slate-300'}`}
              >
          ...
          <div className="px-4 py-2 space-y-1 max-h-48 overflow-y-auto">
            {(sessionLedger ?? [])
              ...
              .map((e: any, i: number) => (
                <div
                  key={`${e.hash}-${i}`}
                  className="text-xs font-mono text-slate-300 border-b border-slate-800/50 pb-1 mb-1 last:border-0"
                >
