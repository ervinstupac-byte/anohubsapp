      ...
      <div className="flex justify-between items-center mb-6 pt-2">
        <div>
          ...
          <p className="text-xs text-slate-500 font-mono mt-1">
            REAL-TIME ASSET SOVEREIGNTY MONITOR
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {plants.map(node => {
          ...
          return (
            <div
              key={node.id}
              className={`group relative bg-slate-900 border ${isSevere ? 'border-red-500/50' : isWarning ? 'border-amber-500/50' : 'border-slate-700'} p-4 rounded hover:bg-slate-800 transition-colors`}
            >
              {/* Status Indicator */}
              <div
                className={`absolute top-3 right-3 w-2 h-2 rounded-full ${isSevere ? 'bg-red-500 animate-ping' : isWarning ? 'bg-amber-500' : 'bg-green-500'}`}
              />
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between text-xs">
                  ...
                  <span className={`font-mono ${isSevere ? 'text-red-400' : 'text-white'}`}>
                    {node.efficiency.toFixed(1)}%
                  </span>
                </div>
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-end">
                    ...
                    <span
                      className={`text-xs font-bold ${isSevere ? 'text-red-500' : isWarning ? 'text-amber-500' : 'text-green-500'}`}
                    >
                      {drift > 0 ? '-' : '+'}
                  ...
                  {drift > 0 && (
                    <div className="flex items-center justify-between pt-2 border-t border-slate-800/50">
                      ...
                      <div className="flex items-center gap-1">
                        <span
                          className={`font-mono font-bold text-sm ${isSevere ? 'text-red-500' : 'text-amber-500'}`}
                        >
                          -€
                          {((drift / 100) * 100 * 24 * 50).toLocaleString(undefined, {
