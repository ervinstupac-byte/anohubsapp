...
export const HeritageSearchWidget: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // We'll just run it.
    const matches = LegacyKnowledgeService.semanticSearch(debouncedQuery, turbineFamily).slice(
      0,
      5
    );
    setResults(matches);
      ...
      {isExpanded && (
        <div className="p-4 space-y-4 animate-in slide-in-from-top-2 duration-300">
          ...
          <div className="relative group">
            <Search
              className={`absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 transition-colors ${isSearching ? 'text-status-info animate-pulse' : 'text-scada-muted'}`}
            />
            <input
              type="text"
              value={query}
          ...
          <div className="space-y-2 min-h-[100px]">
            ...
            {isSearching && (
              <div className="text-center py-4">
                <span className="text-xs text-status-info font-mono animate-pulse uppercase">
                  Searching Archive...
                </span>
              </div>
            )}
                    ...
                    <div className="flex items-start justify-between gap-3 relative z-10">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1.5">
                          <span
                            className={`px-1.5 py-0.5 rounded-sm text-[9px] font-bold uppercase border ${severityColors[caseItem.severity]}`}
                          >
                            {caseItem.severity}
                          </span>
            ...
            {!isSearching && debouncedQuery.length < 2 && (
              <div className="space-y-3 pt-2">
                ...
                {recommendedCases.length === 0 && (
                  <p className="text-[10px] text-scada-muted italic pl-2 font-mono">
                    No critical alerts for this unit type.
                  </p>
                )}
              </div>
            )}
