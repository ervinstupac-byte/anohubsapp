...
export const SensorHealthDashboard: React.FC<SensorHealthProps> = ({
  suspectSensors,
  votingStatus,
  alarmStats,
}) => {
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-xs text-slate-400">
            Assuming sensors are flawed until proven otherwise
          </p>
        </div>
      </div>
          ...
          {suspectSensors.length === 0 ? (
            ...
            <div className="space-y-2 max-h-48 overflow-y-auto">
              {suspectSensors.map(s => (
                <div
                  key={s.id}
                  className="p-2 bg-slate-800 rounded border border-amber-500/30 flex justify-between items-center"
                >
                  <div>
        ...
        <div className="bg-slate-900/50 p-4 rounded-lg border border-slate-700">
          ...
          <div className="space-y-4">
            <div className="flex justify-center gap-4">
              {votingStatus.sensors.map((healthy, idx) => (
                <div key={idx} className="flex flex-col items-center gap-1">
                  <div
                    className={`w-8 h-8 rounded-full flex items-center justify-center border-2 ${healthy ? 'bg-emerald-500/20 border-emerald-500' : 'bg-red-500/20 border-red-500'}`}
                  >
            ...
            <div className="p-2 bg-slate-800 rounded text-center">
              <span
                className={`text-sm font-bold ${votingStatus.status.includes('1oo1') ? 'text-red-400' : 'text-emerald-400'}`}
              >
                {votingStatus.status}
              </span>
            </div>
