        ...
        <div className="bg-slate-900 border border-blue-500 rounded-lg p-4">
          ...
          <div className="relative h-4 bg-slate-800 rounded-full overflow-hidden mb-3">
            <div
              className={`absolute inset-y-0 left-0 transition-all ${
                dcStation.batterySOC > 80
                  ? 'bg-emerald-500'
                  : dcStation.batterySOC > 50
                    ? 'bg-blue-500'
                    : dcStation.batterySOC > 30
                      ? 'bg-amber-500'
                      : 'bg-red-500'
              }`}
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            <div className="bg-slate-800 rounded p-2">
              ...
              <div className="text-blue-400 font-mono">
                {dcStation.batteryChargeCurrent.toFixed(1)} A
              </div>
            </div>
            <div className="bg-slate-800 rounded p-2">
              ...
              <div
                className={`font-bold ${
                  dcStation.batteryHealth === 'GOOD'
                    ? 'text-emerald-400'
                    : dcStation.batteryHealth === 'FAIR'
                      ? 'text-amber-400'
                      : 'text-red-400'
                }`}
              >
          ...
          <div className="mt-3 p-2 bg-slate-800 rounded text-xs">
            <div className="flex items-center justify-between">
              ...
              <span
                className={`font-bold ${
                  dcStation.chargerStatus === 'CHARGING'
                    ? 'text-blue-400'
                    : dcStation.chargerStatus === 'FLOAT'
                      ? 'text-emerald-400'
                      : 'text-red-400'
                }`}
              >
        ...
        <div
          className={`bg-slate-900 border-2 rounded-lg p-4 ${
            dieselGenerator.readiness === 'READY'
              ? 'border-emerald-500'
              : dieselGenerator.readiness === 'RUNNING'
                ? 'border-blue-500'
                : 'border-red-500'
          }`}
        >
          ...
          <div className="grid grid-cols-2 gap-2 text-xs mb-3">
            <div className="bg-slate-800 rounded p-2">
              ...
              <div className="text-amber-400 font-mono font-bold">
                {dieselGenerator.fuelLevel.toFixed(0)}%
              </div>
            </div>
            <div className="bg-slate-800 rounded p-2">
              <div className="text-slate-500 mb-1">Start Battery</div>
              <div className="text-blue-400 font-mono">
                {dieselGenerator.startBatteryVoltage.toFixed(1)} VDC
              </div>
            </div>
            <div className="bg-slate-800 rounded p-2">
              <div className="text-slate-500 mb-1">Engine Temp</div>
              <div className="text-emerald-400 font-mono">
                {dieselGenerator.engineTemperature.toFixed(0)}°C
              </div>
            </div>
            <div className="bg-slate-800 rounded p-2">
            ...
            <div className="bg-slate-800 rounded p-3">
              ...
              <div
                className={`text-2xl font-bold font-mono ${
                  Math.abs(stationService.acFrequency - 50) < 0.1
                    ? 'text-emerald-300'
                    : 'text-amber-300'
                }`}
              >
            ...
            <div className="bg-slate-800 rounded p-3">
              ...
              <div
                className={`text-lg font-bold ${
                  stationService.backupAvailable ? 'text-emerald-300' : 'text-red-300'
                }`}
              >
