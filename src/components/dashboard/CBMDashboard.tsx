    ...
    <div className="w-full bg-slate-950 p-6">
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-slate-900 border border-amber-500 rounded-lg p-4">
          ...
          <div className="text-3xl font-bold text-amber-300 font-mono">{criticalAssets.length}</div>
        </div>
        ...
        <div className="bg-slate-900 border border-blue-500 rounded-lg p-4">
          ...
          <div className="text-3xl font-bold text-blue-300 font-mono">
            {Math.round(
              assets.reduce((sum, a) => sum + a.remainingUsefulLife, 0) / assets.length / 8760
            )}
            y
          </div>
        </div>
  ...
  return (
    <div className={`bg-slate-900 border border-${statusColor}-500/30 rounded-lg p-4`}>
      <div className="flex items-center justify-between mb-3">
        ...
        <div
          className={`w-12 h-12 rounded-full border-4 border-${statusColor}-500 flex items-center justify-center`}
        >
...
const ComponentStatus: React.FC<{ component: any }> = ({ component }) => {
  ...
  const daysUntilMaintenance = Math.round(
    (component.nextMaintenance - Date.now()) / (1000 * 60 * 60 * 24)
  );
  ...
  return (
    <div className="bg-slate-800 rounded-none p-2">
      <div className="flex items-center justify-between mb-1">
        ...
        <div
          className={`text-[10px] px-2 py-0.5 rounded-none bg-${color}-950 text-${color}-300 border border-${color}-500/30`}
        >
          {component.condition}
        </div>
      </div>
      ...
      {component.oilHealth !== undefined && (
        <div className="flex items-center justify-between text-xs mb-1">
          ...
          <span
            className={`font-mono ${component.oilHealth > 70 ? 'text-emerald-400' : 'text-amber-400'}`}
          >
            {component.oilHealth}%
          </span>
        </div>
      ...
      <div className="flex items-center justify-between text-xs">
        ...
        <span
          className={`font-mono ${daysUntilMaintenance < 30 ? 'text-amber-400' : 'text-blue-400'}`}
        >
