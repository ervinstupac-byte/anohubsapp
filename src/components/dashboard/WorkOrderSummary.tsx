...
export const WorkOrderSummary: React.FC = () => {
    ...
    (async () => {
      try {
        ...
        if (f && f.suggestedWorkOrder) {
          ...
          setResidualStd(
            typeof f.residualStd === 'number'
              ? f.residualStd
              : f.residualStd
                ? Number(f.residualStd)
                : null
          );
          const anchorTs = f.predictedTimestamp
            ? new Date(f.predictedTimestamp).toISOString()
            : new Date().toISOString();
          setAnchorTimestamp(anchorTs);

          try {
            const costObj: any = FinancialImpactEngine.calculateImpact(
              { identity: { assetId: selectedAsset.id, assetName: selectedAsset.name } } as any,
              (f.physics as any) || ({} as any),
              { sigma: f.residualStd }
            );
  ...
  return (
    <GlassCard
      className={`relative overflow-hidden transition-all duration-300 ${hasHighPriority ? 'border-l-4 border-l-red-500 shadow-[0_0_20px_rgba(239,68,68,0.1)]' : 'border-l-4 border-l-amber-500'}`}
    >
      {/* Header */}
      <div className="flex items-center justify-between p-4 border-b border-white/5">
        <div className="flex items-center gap-3">
          <div
            className={`w-10 h-10 rounded-xl flex items-center justify-center ${hasHighPriority ? 'bg-red-500/20 animate-pulse-slow' : 'bg-amber-500/20'}`}
          >
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              <span className="text-[10px] text-slate-500 font-mono">{selectedAsset.name}</span>
              {hasHighPriority && (
          ...
          <div className="w-full p-3 rounded-lg border bg-gradient-to-r from-yellow-900/5 to-red-900/5 border-red-500/20">
            <div className="flex items-center justify-between">
              ...
              <div className="text-right">
                <div className="px-2 py-1 rounded bg-red-600 text-white text-[11px] font-black">
                  {suggestedWorkOrder.priority}
                </div>
                {costOfInaction !== null && (
                  <div className="text-[11px] text-slate-400 mt-1">
                    Cost of Inaction: €{new Intl.NumberFormat().format(Math.round(costOfInaction))}
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="mt-3 flex items-center justify-between gap-4">
              <div className="text-[12px] text-slate-300">
                ...
                <div className="text-sm font-black text-white">
                  €
                  {pfValue !== null && typeof pfValue === 'number'
                    ? new Intl.NumberFormat().format(
                        Math.round(
                          (FinancialImpactEngine.calculateImpact as any)(
                            { identity: { assetId: selectedAsset.id } } as any,
                            {} as any
                          ).hourlyLossEuro || 0
                        )
                      )
            ...
            <div className="mt-3">
              <div className="flex items-center justify-between mb-1">
                <div className="text-[11px] text-slate-400">
                  P_f: {pfValue !== null ? `${pfValue.toFixed(3)}%` : '—'}
                </div>
              ...
              <div className="w-full h-2 bg-slate-800 rounded overflow-hidden">
                <div
                  ...
                  style={{ width: `${Math.min(100, pfValue ? (pfValue / 55) * 100 : 0)}%` }}
                />
              </div>
            </div>
              ...
              >
                ...
                <div className="flex items-center gap-3 text-[10px] text-slate-500 border-t border-white/5 pt-2 mt-1 w-full">
                  ...
                  {order.estimatedHoursToComplete && (
                    <span className="font-mono">Est: {order.estimatedHoursToComplete}h</span>
                  )}
                </div>
              </button>
            );
