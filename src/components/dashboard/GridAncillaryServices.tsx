    ...
    <div className="w-full bg-slate-950 p-6">
      ...
      <div
        className={`mb-6 p-4 rounded-lg border-2 ${
          frequencyStatus.status === 'NORMAL'
            ? 'bg-emerald-950 border-emerald-500'
            : frequencyStatus.status === 'WARNING'
      ...
      >
        <div className="grid grid-cols-3 gap-4">
          ...
          <div>
            ...
            <div
              className={`text-4xl font-bold font-mono ${
                Math.abs(gridData.frequencyDeviation) < 0.05 ? 'text-emerald-300' : 'text-amber-300'
              }`}
            >
              {gridData.frequencyDeviation > 0 ? '+' : ''}
              {gridData.frequencyDeviation.toFixed(3)}
            </div>
            <div className="text-xs text-slate-500">Hz</div>
          </div>
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div
          className={`bg-slate-900 border-2 rounded-lg p-4 ${
            gridData.fcr.enabled ? 'border-blue-500' : 'border-slate-700'
          }`}
        >
          <div className="flex items-center justify-between mb-4">
            ...
            <div
              className={`px-3 py-1 rounded-full text-xs font-bold ${
                gridData.fcr.enabled ? 'bg-blue-950 text-blue-300' : 'bg-slate-800 text-slate-400'
              }`}
            >
        ...
        <div
          className={`bg-slate-900 border-2 rounded-lg p-4 ${
            gridData.afrr.enabled ? 'border-purple-500' : 'border-slate-700'
          }`}
        >
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between text-xs">
                ...
                <span className="text-emerald-300 font-mono">
                  {gridData.afrr.actual.toFixed(2)} MW
                </span>
              </div>
              <div className="flex items-center justify-between text-xs">
...
function calculateDroopResponse(
  ...
  const powerChange =
    (maxCapacity / (droopRatio * nominalFrequency)) * Math.abs(frequencyDeviation);
