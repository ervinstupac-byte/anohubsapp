...
const GlassCard = ({ children, className }: { children: React.ReactNode; className?: string }) => (
  <div
    className={`bg-slate-900/50 backdrop-blur-md border border-white/10 rounded-xl p-6 ${className}`}
  >
    {children}
  </div>
);
...
export const KnowledgeCapturePanel: React.FC<KnowledgeCapturePanelProps> = ({ activeContext }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await InstitutionalKnowledgeService.submitKnowledge('current-user-id', 'Current User', {
      incidentPattern,
  ...
  const handleSearch = async () => {
    const symptomList = searchSymptoms
      .split(',')
      .map(s => s.trim())
      .filter(s => s);
      ...
      {activeTab === 'SUBMIT' && (
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <p className="text-xs text-slate-500 text-center">
            Your submission will be verified by the engineering community. High-quality entries earn
            reputation points!
          </p>
        </form>
      )}
...
const TabButton: React.FC<TabButtonProps> = ({ icon: Icon, label, active, onClick }) => (
  <motion.button
    ...
    whileTap={{ scale: 0.98 }}
    onClick={onClick}
    className={`flex-1 px-4 py-3 rounded-lg border-2 transition-all font-bold text-sm flex items-center justify-center gap-2 ${
      active
