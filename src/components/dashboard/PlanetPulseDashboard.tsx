...
export const PlanetPulseDashboard: React.FC<PlanetPulseDashboardProps> = ({
  seismic,
  solar,
  atmospheric,
}) => {
    ...
    <div className="w-full bg-slate-950 p-6">
      ...
      <div className="grid grid-cols-3 gap-6">
        ...
        <div
          className={`bg-slate-900 border-2 rounded-lg p-4 ${
            seismic.earthquakeProbability > 50
              ? 'border-red-500'
              : seismic.stressLevel > 60
                ? 'border-amber-500'
                : 'border-emerald-500'
          }`}
        >
            ...
            <div className="bg-slate-800 rounded p-3">
              ...
              <div
                className={`text-3xl font-bold font-mono ${
                  seismic.stressLevel > 70
                    ? 'text-red-400'
                    : seismic.stressLevel > 50
                      ? 'text-amber-400'
                      : 'text-emerald-400'
                }`}
              >
        ...
        <div
          className={`bg-slate-900 border-2 rounded-lg p-4 ${
            solar.shieldActive ? 'border-orange-500' : 'border-blue-500'
          }`}
        >
          <div className="flex items-center gap-2 mb-4">
          ...
          <div className="space-y-3">
            <div className="bg-slate-800 rounded p-3">
              ...
              <div className="text-xs text-slate-500 mt-1">
                {solar.kpIndex >= 6 ? 'STORM' : solar.kpIndex >= 4 ? 'ACTIVE' : 'QUIET'}
              </div>
            </div>

            <div className="bg-slate-800 rounded p-3">
              <div className="text-xs text-slate-400 mb-1">X-Ray Class</div>
              <div className="text-2xl font-bold text-purple-400">{solar.xrayClass}</div>
            </div>

            <div
              className={`p-2 rounded text-center text-xs font-bold ${
                solar.shieldActive ? 'bg-orange-950 text-orange-300' : 'bg-blue-950 text-blue-300'
              }`}
            >
            ...
            <div className="bg-slate-800 rounded p-3">
              ...
              <div
                className={`text-2xl font-bold font-mono ${
                  atmospheric.headAdjustment > 0 ? 'text-emerald-400' : 'text-amber-400'
                }`}
              >
                {atmospheric.headAdjustment > 0 ? '+' : ''}
                {atmospheric.headAdjustment.toFixed(2)}m
              </div>
            </div>
      ...
      <div className="mt-6 p-4 bg-gradient-to-r from-blue-950 to-emerald-950 border border-blue-500 rounded-lg">
        <div className="text-sm font-bold text-blue-300">
          🌍 GAIA LINK: From local machine to planetary-aware entity. Connected to Earth's seismic
          pulse, Sun's magnetic storms, and atmospheric rhythms.
        </div>
      </div>
    </div>
  );
