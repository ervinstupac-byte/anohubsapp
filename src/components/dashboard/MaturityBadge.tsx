...
export const MaturityBadge: React.FC<MaturityBadgeProps> = ({ state }) => {
  const { completeness, missingFields } = useMemo(() => {
    const criticalFields = [
      ...
      {
        path: 'mechanical.insulationResistance',
        label: 'Megger Test',
        weight: 10,
        requiredValue: 0,
      },
  ...
  const color =
    completeness > 90 ? 'text-green-400' : completeness > 60 ? 'text-yellow-400' : 'text-red-400';
  const bgColor =
    completeness > 90
      ? 'bg-green-500/10'
      : completeness > 60
  ...
  return (
    <div
      className={`p-4 rounded-lg border ${borderColor} ${bgColor} backdrop-blur-sm group relative`}
    >
        ...
        <div className="mt-2 space-y-1">
          ...
          <div className="flex flex-wrap gap-1">
            {missingFields.slice(0, 3).map(f => (
              <span
                key={f}
                className="px-1.5 py-0.5 bg-black/40 rounded text-[8px] text-slate-400 border border-white/5"
              >
                {f}
              </span>
            ))}
      ...
      <div className="absolute left-0 bottom-full mb-2 w-72 p-3 bg-slate-900 border border-slate-700 rounded shadow-2xl opacity-0 group-hover:opacity-100 transition-opacity z-50 pointer-events-none">
        <p className="text-[10px] text-slate-300 leading-tight">
          ...
          Audit measures data fidelity across Mechanical and Hydraulic domains. Calculations are
          based on fundamental principles (Barlow, ISO, IEC).
          <span className="text-slate-500 block mt-1 italic font-mono text-[8px]">
            Precision = 50yr Longevity.
          </span>
        </p>
      </div>
    </div>
