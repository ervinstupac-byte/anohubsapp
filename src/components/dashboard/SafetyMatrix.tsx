    ...
    <div className="w-full bg-slate-950 p-6">
      ...
      <div
        className={`mb-6 p-4 rounded-lg border-2 ${
          permissionToStart ? 'bg-emerald-950 border-emerald-500' : 'bg-red-950 border-red-500'
        }`}
      >
      ...
      <div className="grid grid-cols-2 gap-6">
        {/* Oil System */}
        <InterlockGroup title="Lubrication Oil System" icon="🛢️" interlocks={interlocks.oil} />

        {/* Cooling System */}
        <InterlockGroup title="Cooling Water System" icon="❄️" interlocks={interlocks.cooling} />

        {/* Brake System */}
        <InterlockGroup title="Mechanical Brake" icon="🔧" interlocks={interlocks.brake} />
      ...
      <div className="mt-6 bg-slate-900 border border-slate-700 rounded-lg p-6">
        ...
        <div className="flex items-center justify-center">
          <svg width="800" height="200" className="text-slate-400">
            ...
            <g>
              <path
                d="M 180 30 L 180 170 L 220 170 L 220 30 Z M 220 100 Q 260 100 260 50 Q 260 150 220 150"
                fill="none"
  ...
  return (
    <div
      className={`bg-slate-900 border rounded-lg p-4 ${
        allOk ? 'border-emerald-500/30' : 'border-red-500/30'
      }`}
    >
            ...
            <div className="flex items-center gap-2">
              <div
                className={`w-2 h-2 rounded-full ${
                  interlock.status === 'OK' ? 'bg-emerald-500' : 'bg-red-500'
                }`}
              />
              <span>{interlock.name}</span>
              {interlock.required && <span className="text-[10px] text-amber-400">(Required)</span>}
            </div>
