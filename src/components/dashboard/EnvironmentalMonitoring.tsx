  ...
  return (
    <div className="w-full bg-slate-950 p-6">
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        {/* CCTV Integration */}
        <div
          className={`col-span-2 bg-slate-900 border-2 rounded-lg p-4 ${
            cctv.enabled ? 'border-emerald-500' : 'border-red-500'
          }`}
        >
          <div className="flex items-center justify-between mb-4">
            ...
            <div
              className={`px-3 py-1 rounded-full text-xs font-bold ${
                cctv.enabled ? 'bg-emerald-950 text-emerald-300' : 'bg-red-950 text-red-300'
              }`}
            >
          ...
          <div className="grid grid-cols-4 gap-4 mb-4">
            ...
            <div className="bg-slate-800 rounded p-3">
              ...
              <div className="text-2xl font-bold text-blue-300 font-mono">{cctv.totalCameras}</div>
            </div>
            ...
            <div className="col-span-2 bg-slate-800 rounded p-3">
              ...
              <div
                className={`text-lg font-bold ${
                  cctv.recordingStatus === 'ACTIVE'
                    ? 'text-emerald-300'
                    : cctv.recordingStatus === 'PAUSED'
                      ? 'text-amber-300'
                      : 'text-red-300'
                }`}
              >
          ...
          <div className="grid grid-cols-4 gap-2">
            {[...Array(8)].map((_, i) => (
              <div
                key={i}
                className="aspect-video bg-slate-800 rounded border border-slate-700 flex items-center justify-center"
              >
                <div className="text-center">
                  <Camera
                    className={`w-6 h-6 mx-auto mb-1 ${
                      i < cctv.activeFeeds ? 'text-emerald-400' : 'text-slate-600'
                    }`}
                  />
    ...
    <div className="bg-slate-800 rounded p-3">
      <div className="flex items-center justify-between mb-2">
        ...
        <div
          className={`text-xs px-2 py-0.5 rounded ${
            status === 'normal' ? 'bg-emerald-950 text-emerald-400' : 'bg-amber-950 text-amber-400'
          }`}
        >
          {status === 'normal' ? '✓' : '⚠'}
        </div>
      ...
      <div
        className={`text-2xl font-bold font-mono ${
          status === 'normal' ? 'text-emerald-300' : 'text-amber-300'
        }`}
      >
        {value} {unit}
      </div>
