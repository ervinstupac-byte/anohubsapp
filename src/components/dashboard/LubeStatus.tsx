...
export const LubeStatus: React.FC = () => {
  ...
  return (
    <div className="grid grid-cols-3 gap-4 bg-slate-900/50 p-4 rounded-xl border border-slate-800">
      {/* Pump Status */}
      <div
        className={`p-3 rounded-lg border ${status.pump.action === 'EMERGENCY_SHUTDOWN' ? 'bg-red-950/30 border-red-900' : 'bg-slate-950 border-slate-800'}`}
      >
        <div className="flex items-center gap-2 mb-2">
          <Activity
            className={`w-4 h-4 ${status.pump.action === 'NONE' ? 'text-emerald-400' : 'text-amber-400'}`}
          />
        ...
        {status.pump.standbyPumpShouldRun && (
          <div className="mt-2 text-[10px] text-amber-400 font-bold animate-pulse">
            STANDBY ACTIVE
          </div>
        )}
      </div>
      ...
      {/* Filter Status */}
      <div
        className={`p-3 rounded-lg border ${status.filter.status === 'CRITICAL' ? 'bg-red-950/30 border-red-900' : status.filter.status === 'CLOGGED' ? 'bg-amber-950/30 border-amber-900' : 'bg-slate-950 border-slate-800'}`}
      >
        <div className="flex items-center gap-2 mb-2">
          <Droplets
            className={`w-4 h-4 ${status.filter.status === 'CLEAN' ? 'text-emerald-400' : 'text-amber-400'}`}
          />
      ...
      {/* Cooling Status */}
      <div
        className={`p-3 rounded-lg border ${status.cooling.status === 'FAILED' ? 'bg-red-950/30 border-red-900' : 'bg-slate-950 border-slate-800'}`}
      >
        <div className="flex items-center gap-2 mb-2">
          <Thermometer
            className={`w-4 h-4 ${status.cooling.status === 'EXCELLENT' ? 'text-emerald-400' : 'text-amber-400'}`}
          />
          <span className="text-[10px] uppercase font-bold text-slate-400">Cooling Eff.</span>
        </div>
        <div className="text-xl font-mono text-white mb-1">
          {status.cooling.effectiveness.toFixed(0)}
          <span className="text-xs text-slate-500">%</span>
        </div>
        <div className="text-[10px] text-slate-400">
          Drop: {status.cooling.oilTempDrop.toFixed(1)}°C
        </div>
      </div>
    </div>
  );
