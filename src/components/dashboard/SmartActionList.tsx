        ...
        <AnimatePresence>
          {recommendations.length > 0 ? (
            recommendations.map(rec => (
              ...
              >
                <div className="mt-1">{getIcon(rec.actionType)}</div>
                <div className="flex-1">
                  <div className="flex justify-between items-start mb-1">
                    <h4 className="font-bold text-sm uppercase tracking-wide">{rec.title}</h4>
                    <span
                      className={`text-[10px] font-black px-2 py-0.5 rounded-full bg-black/20 uppercase`}
                    >
                      {rec.priority}
                    </span>
                  </div>
                  <p className="text-xs opacity-90 leading-relaxed mb-3">{rec.description}</p>
            ...
            <div className="flex flex-col items-center justify-center h-40 opacity-50">
              ...
              <p className="text-xs uppercase font-bold tracking-widest text-emerald-200">
                System Optimal
              </p>
