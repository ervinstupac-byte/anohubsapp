  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        const assetDbId =
          numeric !== null ? idAdapter.toDb(numeric) : idAdapter.toDb(selectedAsset.id);
            ...
            <li key={r.id} className="flex justify-between items-center text-sm">
              ...
              <div className="text-right">
                <div className="text-sm font-mono">
                  {r.computed_loss_cost
                    ? `${Number(r.computed_loss_cost).toLocaleString('de-DE', { style: 'currency', currency: 'EUR' })}`
                    : '—'}
                </div>
                <a
