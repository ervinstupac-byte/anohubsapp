    ...
    return (
      <div className="p-6 rounded-2xl border border-dashed border-slate-700 bg-slate-900/30 flex items-center justify-center gap-3">
        ...
        <span className="text-sm font-mono text-slate-500 uppercase">
          System Optimized. No Actions Required.
        </span>
      </div>
    );
  }
...
const ActionCard: React.FC<{ action: SmartAction }> = ({ action }) => {
  ...
  return (
    ...
    >
      {/* Background Gradient */}
      <div
        className={`absolute inset-0 bg-gradient-to-br ${config.gradient} opacity-20 group-hover:opacity-30 transition-opacity`}
      ></div>
      ...
      <div className="relative z-10 p-6">
        <div className="flex justify-between items-start mb-4">
          ...
          <span
            className={`text-[10px] font-black uppercase tracking-widest px-2 py-1 rounded bg-black/40 ${config.color}`}
          >
            {action.type}
          </span>
        </div>
