...
const AutostartDashboard: React.FC = () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      if (
        currentState === 'START_AUX_HPU' ||
        currentState === 'START_AUX_COOLING' ||
        currentState === 'PREPARE_GOVERNOR' ||
        currentState === 'RELEASE_BRAKES' ||
      ...
      let targetFlow = 0;
      if (
        ['START_AUX_COOLING', 'PREPARE_GOVERNOR', 'RELEASE_BRAKES', 'COMPLETED'].includes(
          currentState
        )
      ) {
        targetFlow = 65;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-slate-900/50 border border-slate-700 rounded-lg p-8 text-center relative overflow-hidden">
            <div
              className={`text-4xl font-black tracking-widest ${getStatusColor(sequence?.state || 'IDLE')}`}
            >
              {sequence?.state.replace(/_/g, ' ')}
            </div>
            <div className="text-slate-400 mt-2 font-mono h-6">{sequence?.message}</div>
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div className="bg-slate-900 border border-slate-700 p-4 rounded">
              ...
              <div
                className={`text-xs mt-1 ${hpu.mainPumpStatus === 'RUNNING' ? 'text-emerald-400' : 'text-slate-600'}`}
              >
            ...
            <div className="bg-slate-900 border border-slate-700 p-4 rounded">
              ...
              <div className="text-2xl font-mono text-white">
                {air.systemPressureBar.toFixed(1)}{' '}
                <span className="text-xs text-slate-500">bar</span>
              </div>
              <div
                className={`text-xs mt-1 ${air.brakeReady ? 'text-emerald-400' : 'text-amber-500'}`}
              >
            ...
            <div className="bg-slate-900 border border-slate-700 p-4 rounded">
              ...
              <div
                className={`text-xs mt-1 ${coolingFlow > 40 ? 'text-emerald-400' : 'text-slate-600'}`}
              >
        ...
        <div className="bg-slate-900 border border-slate-800 rounded p-4">
          <h3 className="text-sm font-bold text-slate-300 uppercase tracking-widest mb-4 flex items-center justify-between">
            ...
            {sequence?.permissives.readyToStart ? (
              ...
              <span className="bg-red-500/10 text-red-400 px-2 py-0.5 rounded text-[10px]">
                INHIBITED
              </span>
            )}
          </h3>
          ...
          <div className="space-y-1">
            {sequence?.permissives.permissives &&
              Object.entries(sequence.permissives.permissives).map(([key, val]) => (
