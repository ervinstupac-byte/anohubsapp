...
export const QuickCalcCard: React.FC = () => {
  ...
  const specCards = useMemo((): SpecCard[] => {
    ...
    const vibStatus = currentVib.gt(vibLimit.mul(1.5))
      ? 'critical'
      : currentVib.gt(vibLimit)
        ? 'warning'
        : 'nominal';
  ...
  return (
    <GlassCard
      className={`relative overflow-hidden ${hasWarnings ? 'border-l-4 border-l-amber-500' : 'border-l-4 border-l-blue-500'}`}
    >
      {/* Header */}
      <div className="flex items-center justify-between p-4 border-b border-white/5">
        <div className="flex items-center gap-3">
          <div
            className={`w-10 h-10 rounded-xl flex items-center justify-center ${hasWarnings ? 'bg-amber-500/20' : 'bg-blue-500/20'}`}
          >
