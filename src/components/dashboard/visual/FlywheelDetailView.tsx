...
// NC-9.0 Mechanical Backbone Hotspots
const MECH_COMPONENTS = [
  {
    ...
    param: 'Load Support',
    heritage:
      'Supporting the significant weight overhang of the flywheel. Oil film thickness must be monitored continuously.',
    icon: LocateFixed,
    pos: { x: 50, y: 30 },
  ...
  },
  {
    ...
    heritage:
      'Never operate without the flywheel guard installed. The stored kinetic energy is immense and catastrophic if released.',
    icon: Disc,
    pos: { x: 50, y: 45 },
  ...
  },
  {
    ...
    heritage:
      'Shaft run-out at this point should typically be < 0.03 mm TIR (Total Indicator Reading). Loose bolts here inevitably shear.',
    icon: Link,
    pos: { x: 50, y: 60 },
  ...
  },
  {
    ...
    heritage:
      'The bearing closest to the hydraulic chaos. If this bearing vibrates, check the Runner Wearing Rings first.',
    icon: CircleDot,
    pos: { x: 50, y: 75 },
  ...
  },
  {
    ...
    heritage:
      'Carbon rings must be "run-in" properly. A leaking seal floods the turbine, causing corrosion of the bearing housing.',
    icon: Shield,
    pos: { x: 50, y: 85 },
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-xl font-black text-white tracking-widest uppercase">
              {t('mechanical.detail.title', 'Mechanical Backbone')}{' '}
              <span className="text-cyan-500">{t('common.drillDown', 'Drill-Down')}</span>
            </h2>
        ...
        <div className="flex gap-2 items-center">
          ...
          <div className="px-3 py-1 rounded-full bg-cyan-500/10 border border-cyan-500/20 flex items-center gap-2">
            ...
            <span className="text-[10px] font-bold text-cyan-300 uppercase tracking-widest">
              {t('common.assetSyncActive', 'Asset Sync Active')}
            </span>
          </div>
        ...
        >
          <div className="absolute inset-0 flex items-center justify-center p-4">
            <motion.img
              ...
              transition={
                rpm > 10 ? { duration: 6000 / rpm, repeat: Infinity, ease: 'linear' } : {}
              }
          ...
          <AnimatePresence mode="sync">
            {activeData ? (
              ...
              >
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <div>
                    <h3 className="text-lg font-black text-white uppercase">
                      {getComponentData(activeData).name}
                    </h3>
                    <p className="text-xs font-bold text-cyan-400">
                      {getComponentData(activeData).param}
                    </p>
                  </div>
                </div>
                <div className="space-y-6">
                  {activeData.category === 'support' && vibration > 0.5 && (
                    <div
                      className={`p-4 rounded-r-lg border-l-2 ${vibration > 1.5 ? 'bg-red-950/40 border-red-500' : 'bg-amber-950/40 border-amber-500'}`}
                    >
                      <div className="flex justify-between items-center mb-1">
                        <span
                          className={`text-[10px] font-bold uppercase tracking-widest ${vibration > 1.5 ? 'text-red-400' : 'text-amber-400'}`}
                        >
                          Live Vibration
                        </span>
                  ...
                  <div className="p-4 bg-amber-500/10 border border-amber-500/20 rounded-xl relative overflow-hidden">
                    ...
                    <span className="text-[9px] font-black uppercase text-amber-500 block mb-1">
                      Field Engineer Instruction
                    </span>
                    <p className="text-xs text-amber-200 italic">
                      "{getComponentData(activeData).heritage}"
                    </p>
                  </div>
                </div>
              </motion.div>
              ...
              <div className="h-full flex flex-col items-center justify-center p-8 bg-cyan-500/5 border border-dashed border-cyan-500/20 rounded-2xl text-center">
                ...
                <p className="text-[10px] text-cyan-500/40 font-bold mt-2">
                  Explore Shaft & Coupling Physics
                </p>
              </div>
            )}
          </AnimatePresence>
