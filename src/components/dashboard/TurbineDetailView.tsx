...
const FrancisPID: React.FC<{ sensors: any }> = ({ sensors }) => {
  return (
    <div className="relative w-full h-full p-8">
      ...
      {/* Runner (center) */}
      <div
        className="absolute top-64 left-56 w-16 h-16 bg-gradient-to-br from-purple-600 to-purple-800 rounded-full flex items-center justify-center animate-spin"
      ...
      {/* Draft Tube */}
      <div
        className="absolute top-96 left-48 w-32 h-40 border-4 border-blue-500 flex items-center justify-center"
      ...
      <SensorTag
        label="PRESS_INLET"
        value={`${sensors.pressureInlet} bar`}
        position={{ top: 24, left: 180 }}
        status="normal"
      />
      <SensorTag
        label="VIB_BEARING_1"
        value={`${sensors.vibBearing1} mm/s`}
        position={{ top: 60, right: 80 }}
        status={sensors.vibBearing1 > 2.5 ? 'warning' : 'normal'}
      />
      <SensorTag
        label="TEMP_BEARING_1"
        value={`${sensors.tempBearing1}°C`}
        position={{ top: 90, right: 80 }}
        status={sensors.tempBearing1 > 50 ? 'warning' : 'normal'}
      />
      <SensorTag
        label="DRAFT_TUBE_PRESS"
        value={`${sensors.pressureDraft} bar`}
        position={{ bottom: 40, left: 160 }}
        status="normal"
      />
      <SensorTag
        label="GATE_OPENING"
        value={`${sensors.gateOpening}%`}
...
const KaplanPID: React.FC<{ sensors: any }> = ({ sensors }) => {
  return (
    <div className="relative w-full h-full p-8">
      ...
      <SensorTag
        label="BLADE_ANGLE"
        value={`${sensors.bladeAngle}°`}
        position={{ top: 120, right: 80 }}
        status="normal"
      />
      <SensorTag
        label="CONJUGATE_ERROR"
        value="0.8%"
...
const PeltonPID: React.FC<{ sensors: any }> = ({ sensors }) => {
  ...
  return (
    <div className="relative w-full h-full p-8">
      {/* Pelton Runner */}
      <div
        className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-64 h-64 border-4 border-yellow-600 rounded-full flex items-center justify-center animate-spin"
      ...
      {nozzles.map((position: number, i: number) => {
        ...
        return (
          ...
          >
            ...
            {i === 2 && <div className="text-[10px] text-amber-400 mt-1">⚠️ High freq</div>}
          </div>
        );
      })}
    ...
    <div className="relative w-full h-full p-8">
      ...
      <div className="absolute bottom-32 left-1/2 transform -translate-x-1/2 flex gap-4">
        {[1, 2, 3].map(i => (
          <div
            key={i}
            className="w-12 h-12 border-2 border-cyan-500 rounded-full flex items-center justify-center"
          >
      ...
      <SensorTag
        label="AIR_VALVE"
        value={`${sensors.airValve}%`}
        position={{ top: 120, right: 100 }}
        status="normal"
      />
      <SensorTag
        label="INTERNAL_PRESS"
        value={`1.02 bar`}
