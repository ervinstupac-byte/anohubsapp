...
export const CommanderTerminal: React.FC = () => {
  ...
  const state: TechnicalProjectState = {
    ...
    market: {
      ...
      energyPrice:
        telemetry.financials?.energyPrice || DEFAULT_TECHNICAL_STATE.market?.energyPrice || 85,
    },
    ...
    if (cmdName === '/explain') {
      if (param) {
        ...
        if (param.includes('vibration')) {
          ...
          addMessage(
            `No specific analysis for: ${param}. Try 'vibration', 'insulation', or 'barlow'.`,
            'WARNING'
          );
        }
      } else {
        ...
        addMessage(
          `1. VIBRATION: Standard ISO 10816 - 3 specifies Critical as > ${vibLimit} mm / s.`
        );
        addMessage(
          `2. INSULATION: Megger Law R_{ min } = kV + 1. Current required: > ${kv + 1} MOhm.`
        );
        const inferences = ExpertInference.analyze(state);
        addMessage(
          `3. BARLOW'S LINK (Spiral): Stress = (P * D) / (2 * t). Current Margin: ${inferences.metrics.structuralSafetyMargin.toFixed(1)}%.`
        );
      }
    } else if (cmdName === '/sop') {
      if (!param) {
        ...
        const sopCode = param.toUpperCase().includes('VIB')
          ? 'VIBRATION_CRITICAL'
          : param.toUpperCase().includes('BEAR')
            ? 'BEARING_TEMP_CRITICAL'
            : param.toUpperCase().includes('STRUC')
              ? 'STRUCTURAL_RISK'
              : null;
        if (sopCode) {
      ...
      if (param.includes('10816')) {
        addMessage(
          'ISO 10816-3: Evaluation of machine vibration by measurements on non-rotating parts.',
          'INFO'
        );
      ...
      addMessage(
        'AVAILABLE COMMANDS: /explain [param], /sop [type], /standard [code], /status, /clear',
        'INFO'
      );
    } else if (cmdName === '/status') {
      const status = (state as any).physics?.status || state.physics.leakageStatus || 'NOMINAL';
      addMessage(
        `System Status: ${status}. Critical Risks: ${state.specializedState?.activeRisks?.length || 0}`,
        'INFO'
      );
  ...
  useEffect(() => {
    ...
    if (structuralLife < 85 && structuralLife > 80) {
      addMessage(
        'Commander, Unit 01 is at 82% Health. Recommendation: Maximize output for 2 hours, then initiate cooldown.',
        'WARNING'
      );
    }
    ...
    if (energyPrice > 120) {
      addMessage(
        `Market anomaly detected: Energy prices at peak (€${energyPrice.toFixed(2)}/MWh). Optimize for max generation.`,
        'SUCCESS'
      );
    }
    ...
    if (Math.abs(50 - gridFreq) > 0.2) {
      addMessage(
        `Severe frequency deviation (${gridFreq.toFixed(2)}Hz). Grid services active. Material aging accelerated by 1.5x.`,
        'CRITICAL'
      );
    }
  }, [
    state.structural?.remainingLife,
        ...
        {isOpen && (
          ...
          >
              ...
              <AnimatePresence initial={false}>
                {messages.length === 0 ? (
                  <div className="text-slate-600 animate-pulse">
                    Initializing Neural Link... type /help
                  </div>
                ) : (
                  messages.map(msg => (
                    <motion.div
            ...
            <form
              ...
              className="p-2 px-4 bg-white/5 border-t border-white/10 flex items-center gap-2"
            >
