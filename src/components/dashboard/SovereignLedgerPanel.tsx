...
export const SovereignLedgerPanel: React.FC = () => {
  ...
  useEffect(() => {
    const loadLogs = async () => {
      ...
      const mapped: LedgerEntry[] = logs.map(l => ({
        ...
        severity: (l.active_protection && l.active_protection !== 'NONE'
          ? 'WARNING'
          : 'INFO') as LedgerEntry['severity'],
      }));
          ...
          [
            {
              ...
              severity: (r.active_protection && r.active_protection !== 'NONE'
                ? 'WARNING'
                : 'INFO') as LedgerEntry['severity'],
            },
            ...prev,
  ...
  const logCommanderAction = useCallback(
    async (
  ...
  const filteredEntries = ledgerHistory.filter(entry => {
    ...
    if (filter === 'SYSTEM')
      return entry.action === 'SYSTEM_OVERRIDE' || entry.action === 'PULSE_UPDATE';
  ...
  const avgPulseChange =
    ledgerHistory.length > 0
      ? ledgerHistory.reduce((acc, e) => acc + Math.abs(e.pulseDelta || 0), 0) /
    ...
    <div className="p-4 bg-scada-panel border border-scada-border rounded-none shadow-none">
      ...
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-bold text-scada-text uppercase tracking-tight">
              Sovereign Ledger
            </h2>
            <div className="flex items-center gap-2">
              <span className="text-xs text-scada-muted font-mono uppercase">
                NC-1100 Immutable Record
              </span>
      ...
      <div className="grid grid-cols-4 gap-3 mb-4">
        <div className="bg-scada-bg border border-scada-border rounded-none p-2 text-center">
          <div className="text-xl font-bold font-mono text-scada-text tabular-nums">
            {totalActions}
          </div>
          <div className="text-[10px] text-scada-muted uppercase font-mono">Total Actions</div>
        </div>
        <div className="bg-scada-bg border border-scada-border rounded-none p-2 text-center">
          <div className="text-xl font-bold font-mono text-status-warning tabular-nums">
            {commanderActions}
          </div>
          <div className="text-[10px] text-scada-muted uppercase font-mono">Commander</div>
        </div>
        <div className="bg-scada-bg border border-scada-border rounded-none p-2 text-center">
          <div className="text-xl font-bold font-mono text-status-info tabular-nums">
            {systemUpdates}
          </div>
            ...
            >
              ...
              >
                ...
                <div className="flex-1 text-left">
                  <div className="flex items-center gap-2">
                    <span className="font-bold text-xs font-mono uppercase tracking-tight">
      ...
      <div className="mt-4 pt-3 border-t border-scada-border flex items-center justify-between text-[10px] text-scada-muted font-mono uppercase">
        ...
        <span>
          Showing {filteredEntries.length} of {ledgerHistory.length} entries
        </span>
      </div>
    </div>
  );
