...
export const ProtocolLaunchpad: React.FC = () => {
    ...
    const hasPendingData = pendingSyncCount > 0;
    const pendingCount = pendingSyncCount;
    const syncStatus = hasPendingData ? 'SYNCING' : 'IDLE';
    ...
    const protocolStatus = useMemo(() => {
        const mechanicalData = mechanical as unknown as Record<string, unknown>;

        return PROTOCOLS.map(protocol => {
            const pathParts = protocol.dataSource.split('.');
            let hasData = false;
            let value: unknown = mechanicalData;
    ...
    const handleGenerateReport = async (protocol: Protocol) => {
        ...
        if (hasPendingData) {
            pushNotification('WARNING', t('dashboard.syncFirst', 'Sync pending changes before generating reports'));
            return;
        }
        ...
        try {
            ...
            const ledgerUUID = createLedgerUUID();
            ...
            if (protocol.generator === 'audit') {
                ...
                const blob = ForensicReportService.generateAuditReport({
                    ...
                    diagnostics: diagnosis ? [{ type: 'info', message: (diagnosis as { summary?: string }).summary || 'No diagnosis' }] : [],
                    logs: [],
                    physicsData: [],
                ...
                if (blob instanceof Blob) {
                    ...
                    reportService.saveReport({
                        ...
                        pdfPath: filename,
                        metadata: { protocolId: protocol.id, ledgerId: ledgerUUID }
                    }).catch((e: unknown) => console.warn('ProtocolLaunchpad.saveReport failed:', e));
                }
            } else {
                const { ForensicReportService } = await import('../../services/ForensicReportService');
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h3 className="text-sm font-black text-white uppercase tracking-wider">
                            {t('dashboard.protocolLaunchpad.title', 'Protocol Launchpad')}
                        </h3>
                ...
                {protocolStatus.map((protocol) => (
                    ...
                    >
                            ...
                            >
                                {generatingId === protocol.id ? (
                                    ...
                                    <>
                                        ...
                                        {t('dashboard.protocolLaunchpad.generatePdf', 'Generate PDF')}
                                    </>
                                ) : (
                                    <>
                        ...
                        <div className="mt-2 flex items-center gap-2">
                            ...
                            `}>
                                {protocol.hasData
                                    ? t('dashboard.protocolLaunchpad.completed', 'Completed')
                                    : t('dashboard.protocolLaunchpad.startSession', 'Start Session')}
                            </span>
                        </div>
                    </div>
                ))}
