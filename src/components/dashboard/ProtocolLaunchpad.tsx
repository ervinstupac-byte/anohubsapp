...
export const ProtocolLaunchpad: React.FC = () => {
  ...
  const handleViewLastAudit = async () => {
    ...
    try {
      ...
      ForensicReportService.openAndDownloadBlob(
        blob,
        `Audit_Report_${selectedAsset.name}_${Date.now()}.pdf`,
        true
      );
  ...
  const handleGenerateReport = async (protocol: Protocol) => {
    ...
    if (hasPendingData) {
      pushNotification('WARNING', t('dashboard.syncFirst'));
      return;
    }
      ...
      if (protocol.generator === 'audit') {
        ...
        const blob = ForensicReportService.generateProtocolReport({
          ...
          diagnostics: diagnosis
            ? [{ type: 'info', message: (diagnosis as any).summary || 'No diagnosis' }]
            : [],
          logs: [],
          physicsData: [],
        ...
        if (blob instanceof Blob) {
          ...
          reportService
            ...
            .catch((e: any) =>
              console.warn('ProtocolLaunchpad.saveReport failed:', e?.message || e)
            );
        }
      } else {
        const { ForensicReportService } = await import('../../services/ForensicReportService');
        ...
        <div className="flex items-center justify-between p-4 border-b border-white/5">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-500 font-mono">One-click field reports</p>
            </div>
          </div>
