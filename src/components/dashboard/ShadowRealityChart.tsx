...
export const ShadowRealityChart: React.FC<ShadowRealityChartProps> = ({ data }) => {
  ...
  if (cumulativeData.length === 0)
    return <div className="text-slate-500 text-xs text-center py-4">No Shadow Data</div>;
  ...
  return (
    <div className="bg-slate-900/50 border border-slate-800 rounded p-3 flex items-center justify-between gap-4">
      <div>
        <div className="text-[9px] text-slate-500 uppercase font-mono font-bold tracking-widest mb-1">
          Shadow vs. Reality
        </div>
        <div
          className={`text-lg font-mono font-black ${isPositive ? 'text-emerald-400' : 'text-red-400'} flex items-center gap-2`}
        >
          {isPositive ? '+' : ''}
          {lastVal.toFixed(2)} €
