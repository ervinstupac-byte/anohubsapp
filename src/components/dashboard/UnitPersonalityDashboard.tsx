...
export const UnitPersonalityDashboard: React.FC<UnitPersonalityProps> = ({
  unitId,
  type,
  performance,
  tuning,
      ...
      <div className="grid grid-cols-2 gap-6">
            ...
            <div>
              <div className="flex justify-between text-sm mb-1">
                ...
                <span className="text-slate-300 font-mono">
                  {performance.theoreticalEff.toFixed(1)}%
                </span>
              </div>
            ...
            <div>
              <div className="flex justify-between text-sm mb-1">
                ...
                <span className="text-emerald-300 font-mono">
                  {performance.actualEff.toFixed(1)}%
                </span>
              </div>
              <div className="h-2 bg-slate-800 rounded-full overflow-hidden">
                <div
            ...
            <div className="p-3 bg-slate-800 rounded border border-slate-600">
              ...
              <div
                className={`text-2xl font-black font-mono ${
                  performance.gap > 2 ? 'text-red-400' : 'text-emerald-400'
                }`}
              >
                {performance.gap > 0 ? '-' : '+'}
                {Math.abs(performance.gap).toFixed(2)}%
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-slate-900 border-2 border-purple-500 rounded-none p-4">
          ...
          <div className="space-y-4">
            <div className="bg-purple-900/20 p-3 rounded border border-purple-500/30">
              <div className="text-xs text-purple-300 uppercase font-bold mb-1">
                Optimization Gain
              </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {tuning.activeStrategies.map((strat, idx) => (
                  <span
                    key={idx}
                    className="px-2 py-1 bg-slate-800 text-cyan-400 text-xs rounded border border-cyan-900 font-mono"
                  >
                    {strat}
                  </span>
                ))}
        ...
        <div className="col-span-2 bg-slate-900 border border-cyan-500 rounded-none p-4">
          ...
          <div className="text-sm text-slate-300">
            {type === 'KAPLAN' && (
              <p>
                Using <b>Adaptive Combinator</b>. Learned 124 cam points. Next auto-calibration in
                40h.
              </p>
            )}
            {type === 'PELTON' && (
              <p>
                <b>Nozzle Sequencer</b> Active. Current State: 4 Jets (Balanced X-Pattern). Needle
                deviation 1.2%.
              </p>
            )}
            {type === 'FRANCIS' && (
              <p>
                <b>Rough Zone Manager</b> Active. Avoidance Bands: 20-30 MW, 42-44 MW. Transition
                Speed: MAX.
              </p>
            )}
          </div>
