    ...
    <div className="w-full bg-slate-950 p-6 text-white border-2 border-slate-800 rounded-xl">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">
              Generator Operating Point • {ratedMVA} MVA Rated
            </p>
          </div>
        </div>
        ...
        <div className="flex gap-2">
          <div
            className={`px-2 py-1 rounded text-xs font-bold border ${pssActive ? 'bg-emerald-900/30 border-emerald-500 text-emerald-400' : 'bg-slate-800 border-slate-600 text-slate-500'}`}
          >
            PSS: {pssActive ? 'ACTIVE' : 'OFF'}
          </div>
          <div
            className={`px-2 py-1 rounded text-xs font-bold border ${limiters.oel ? 'bg-red-900/30 border-red-500 text-red-400 animate-pulse' : 'bg-slate-800 border-slate-600 text-slate-500'}`}
          >
            OEL
          </div>
          <div
            className={`px-2 py-1 rounded text-xs font-bold border ${limiters.uel ? 'bg-amber-900/30 border-amber-500 text-amber-400 animate-pulse' : 'bg-slate-800 border-slate-600 text-slate-500'}`}
          >
            UEL
          </div>
        </div>
      ...
      <div className="grid grid-cols-3 gap-6">
        ...
        <div className="col-span-2 bg-slate-900 rounded-lg border border-slate-700 h-64 relative overflow-hidden flex items-center justify-center">
          ...
          {/* Axis */}
          <div className="absolute top-0 bottom-0 left-1/2 w-0.5 bg-slate-600 opacity-50" />{' '}
          {/* Y-Axis (MW) */}
          <div className="absolute left-0 right-0 bottom-8 h-0.5 bg-slate-600 opacity-50" />{' '}
          {/* X-Axis (MVAR) - shifted up */}
          ...
          <div className="absolute top-2 left-1/2 text-xs text-slate-500 transform -translate-x-1/2">
            + MW
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-slate-800 p-3 rounded border border-slate-700">
            ...
            <div
              className={`text-2xl font-bold font-mono ${reactivePowerMVAR > 0 ? 'text-amber-300' : 'text-cyan-300'}`}
            >
          ...
          <div className="bg-slate-800 p-3 rounded border border-slate-700">
            ...
            <div
              className={`text-xl font-bold font-mono ${coolingTempC > 40 ? 'text-red-400' : 'text-emerald-300'}`}
            >
