...
export const AutarkyDashboard: React.FC<AutarkyDashboardProps> = ({
  materials,
  consumables,
  manufacturing,
}) => {
  // Calculate overall autarky score (weighted average of runways)
  const avgMaterialRunway =
    (materials.titanium.runway + materials.steel.runway + materials.bronze.runway) / 3;
    ...
    <div className="w-full bg-slate-950 p-6">
            ...
            <div className="bg-slate-800 rounded p-3">
              ...
              <div className="flex justify-between text-xs">
                ...
                <span className="text-purple-400 font-mono">
                  {materials.titanium.runway} months
                </span>
              </div>
            </div>
          ...
          <div className="space-y-3">
            ...
            <div className="bg-slate-800 rounded p-3">
              ...
              <div className="mb-2">
                <div className="flex justify-between text-xs mb-1">
                  ...
                  <span
                    className={`font-mono ${
                      consumables.oil.purity >= 95 ? 'text-emerald-400' : 'text-amber-400'
                    }`}
                  >
                ...
                <div className="relative h-2 bg-slate-700 rounded-full overflow-hidden">
                  <div
                    className={`absolute inset-y-0 left-0 ${
                      consumables.oil.purity >= 95 ? 'bg-emerald-500' : 'bg-amber-500'
                    }`}
            ...
            <div className="bg-slate-800 rounded p-3">
              ...
              <div className="flex justify-between items-center mb-2">
                ...
                <span className="text-sm text-blue-400">{consumables.h2.fillLevel}%</span>
              </div>
              <div className="relative h-2 bg-slate-700 rounded-full overflow-hidden mb-1">
                <div
        ...
        <div className="col-span-2 bg-slate-900 border border-blue-500 rounded-lg p-4">
          ...
          <div className="space-y-2">
            ...
            {[
              {
                name: 'Titanium Alloy',
                current: `${materials.titanium.available} kg`,
                runway: `${materials.titanium.runway} mo`,
                status: materials.titanium.runway > 12 ? 'SECURE' : 'REORDER',
              },
              {
                name: 'Stainless Steel',
                current: `${materials.steel.available} kg`,
                runway: `${materials.steel.runway} mo`,
                status: materials.steel.runway > 12 ? 'SECURE' : 'REORDER',
              },
              {
                name: 'Bronze',
                current: `${materials.bronze.available} kg`,
                runway: `${materials.bronze.runway} mo`,
                status: materials.bronze.runway > 12 ? 'SECURE' : 'REORDER',
              },
              {
                name: 'Turbine Oil',
                current: `${consumables.oil.volume} L`,
                runway: `${consumables.oil.runway} yr`,
                status: 'INFINITE',
              },
              {
                name: 'Hydrogen',
                current: `${consumables.h2.volume} Nm³`,
                runway: `${consumables.h2.runway} mo`,
                status: 'ON-DEMAND',
              },
            ].map((resource, idx) => (
              <div
                key={idx}
                className="grid grid-cols-4 gap-4 text-xs py-2 border-t border-slate-800"
              >
                ...
                <div
                  className={`font-bold ${
                    resource.status === 'INFINITE'
                      ? 'text-purple-400'
                      : resource.status === 'ON-DEMAND'
                        ? 'text-cyan-400'
                        : resource.status === 'SECURE'
                          ? 'text-emerald-400'
                          : 'text-amber-400'
                  }`}
                >
      ...
      <div className="mt-6 p-4 bg-gradient-to-r from-purple-950 to-emerald-950 border border-purple-500 rounded-lg">
        ...
        <div className="grid grid-cols-3 gap-4 text-xs text-slate-300">
          <div>⚙️ Can 3D print {manufacturing.printableComponents} critical components on-site</div>
          <div>♻️ Oil regeneration extends life 4× (20+ years vs 5 years)</div>
          <div>⚡ H2 produced on-demand during excess power periods</div>
        </div>
      </div>
    </div>
  );
