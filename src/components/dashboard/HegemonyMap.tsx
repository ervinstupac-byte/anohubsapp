...
export const HegemonyMap: React.FC<HegemonyMapProps> = ({
  plantData,
  basinData,
  marketMetrics,
}) => {
    ...
    <div className="w-full bg-slate-950 p-6">
      ...
      <div
        className={`mb-6 p-6 rounded-lg border-2 ${
          isLeader ? 'bg-amber-950 border-amber-500' : 'bg-slate-900 border-slate-700'
        }`}
      >
        <div className="flex items-center justify-between">
          ...
          <div className="text-right">
            ...
            <div className="text-sm text-slate-500 mt-1">of basin capacity</div>
          </div>

          {isLeader && <Trophy className="w-24 h-24 text-amber-400" />}
        </div>
      </div>
          ...
          <div className="space-y-3">
            <div className="flex justify-between">
              ...
              <span className="text-white font-mono font-bold">
                {basinData.totalOutput.toFixed(0)} MW
              </span>
            </div>
            ...
            <div className="flex justify-between">
              ...
              <span className="text-emerald-400 font-mono font-bold">
                {plantData.currentOutput.toFixed(0)} MW
              </span>
            </div>
            ...
            <div className="flex justify-between">
              ...
              <span className="text-amber-400 font-mono font-bold">
                {basinData.peakDemand.toFixed(0)} MW
              </span>
            </div>
            ...
            <div className="mt-4">
              ...
              <div className="text-xs text-slate-500 mt-1">
                {plantData.currentOutput.toFixed(0)} / {plantData.capacity.toFixed(0)} MW (
                {((plantData.currentOutput / plantData.capacity) * 100).toFixed(0)}%)
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-slate-900 border border-emerald-500 rounded-lg p-4">
          ...
          <div className="space-y-3">
            <div className="flex justify-between">
              ...
              <span className="text-emerald-400 font-mono font-bold">
                €{marketMetrics.revenueToday.toLocaleString()}
              </span>
            </div>
            ...
            <div className="flex justify-between">
              ...
              <span className="text-blue-400 font-mono font-bold">
                {marketMetrics.profitMargin.toFixed(1)}%
              </span>
            </div>
            ...
            <div className="flex justify-between">
              ...
              <span className="text-purple-400 font-mono font-bold">
                €{marketMetrics.marketPrice.toFixed(2)}/MWh
              </span>
            </div>
            ...
            <div
              className={`mt-4 p-3 rounded ${
                marketMetrics.profitMargin > 30 ? 'bg-emerald-950' : 'bg-blue-950'
              }`}
            >
              <div
                className={`text-center font-bold ${
                  marketMetrics.profitMargin > 30 ? 'text-emerald-300' : 'text-blue-300'
                }`}
              >
          ...
          <div className="space-y-2">
            ...
            <div
              className={`p-3 rounded border-2 ${
                isLeader ? 'bg-amber-950 border-amber-500' : 'bg-blue-950 border-blue-500'
              }`}
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div
                    className={`text-2xl font-bold ${
                      isLeader ? 'text-amber-300' : 'text-blue-300'
                    }`}
                  >
                    #{plantData.position}
                  </div>
      ...
      <div className="mt-6 p-4 bg-gradient-to-r from-purple-950 to-blue-950 border border-purple-500 rounded-lg">
        ...
        <div className="grid grid-cols-2 gap-4 text-xs text-slate-300">
          <div>
            💡 To reach #1: Increase output by{' '}
            {isLeader ? '0' : (competitors[0].output - plantData.currentOutput).toFixed(0)} MW
          </div>
          <div>
            📊 Market concentration:{' '}
            {(competitors.length > 0
              ? plantData.marketShare + competitors[0].marketShare
              : plantData.marketShare
            ).toFixed(0)}
            % (top 2)
          </div>
          <div>⚡ Your dominance: {isLeader ? 'ESTABLISHED' : 'CHALLENGER'}</div>
          <div>
            🎯 Revenue opportunity: €
            {(
              (basinData.peakDemand - plantData.currentOutput) *
