...
export const DiagnosticAlertsPanel: React.FC = () => {
  ...
  const [learningIndicators, setLearningIndicators] = useState<
    Record<string, { confidence: number; recentlyLearned: boolean }>
  >({});

  const feedbackLoop = new ExpertFeedbackLoop();

  // NC-1500: Check for Sentinel sensor drift
  const [sentinelWarnings, setSentinelWarnings] = useState<
    Array<{
      code: string;
      severity: string;
      message: string;
  ...
  const handleValidate = (warningCode: string) => {
    ...
    const isValidated = window.confirm(
      `Validate warning: ${warningCode}? This teaches the system to recognize similar patterns.`
    );
      ...
      try {
        if (a.payload && typeof a.payload === 'object' && a.payload.code) {
          ...
          return {
            ...
            onClick: () => {
              window.location.hash = `/alerts#${a.id}`;
            },
          ...
          return (
            ...
            >
              <div className="flex items-start gap-2">
                <IconComponent
                  className={`w-3 h-3 mt-0.5 flex-shrink-0 ${warning.severity === 'CRITICAL' ? 'animate-pulse' : ''}`}
                />
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    <div
                      className={`flex items-center gap-1 px-2 py-0.5 rounded-full text-xs ${
                        (learningIndicators[warning.code]?.confidence || 5) > 80
                          ? 'bg-green-500/20 text-green-400'
                          : (learningIndicators[warning.code]?.confidence || 5) > 50
                    ...
                    >
                      ...
                      <span>
                        AI Confidence:{' '}
                        {(learningIndicators[warning.code]?.confidence || 5).toFixed(0)}%
                      </span>
