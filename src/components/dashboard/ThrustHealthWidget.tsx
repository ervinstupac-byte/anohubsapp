  ...
  if (!data) {
    return (
      <GlassCard className="p-3 bg-gradient-to-br from-slate-900/95 to-slate-800/95 backdrop-blur-xl border border-slate-700/50">
        ...
        <div className="text-center text-slate-500 text-xs">No Data</div>
      </GlassCard>
    );
  }
  ...
  return (
    <GlassCard className="p-3 bg-gradient-to-br from-slate-900/95 to-slate-800/95 backdrop-blur-xl border border-slate-700/50">
      <div className="flex items-center justify-between mb-3">
        ...
        <div
          className={`px-2 py-1 rounded-full text-xs font-medium ${getStatusBg(data.status)} ${getStatusColor(data.status)}`}
        >
      ...
      <div className="space-y-3">
        ...
        <div className="flex items-center justify-between">
          ...
          <span
            className={`text-sm font-mono ${data.oilFilmThickness <= 12 ? 'text-red-400' : data.oilFilmThickness <= 20 ? 'text-yellow-400' : 'text-green-400'}`}
          >
        ...
        <div className="flex items-center justify-between">
          ...
          <span
            className={`text-sm font-mono ${data.padSymmetry >= 12 ? 'text-red-400' : data.padSymmetry >= 5 ? 'text-yellow-400' : 'text-green-400'}`}
          >
        ...
        <div className="flex items-center justify-center pt-2">
          <div className="relative">
            <StatusIcon
              className={`w-8 h-8 ${getStatusColor(data.status)} ${data.status === 'SOVEREIGN_TRIP' ? 'animate-pulse' : ''}`}
            />
