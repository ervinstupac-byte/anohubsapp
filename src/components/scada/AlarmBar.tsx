...
export const AlarmBar: React.FC<AlarmBarProps> = React.memo(({ answers }) => {
  ...
  const riskStatus = useMemo(() => {
    ...
    if (riskScore >= 70)
      return { level: 'HIGH', color: 'red', message: 'HIGH RISK - INTERVENTION REQUIRED' };
    if (riskScore >= 40)
      return { level: 'MEDIUM', color: 'yellow', message: 'MEDIUM RISK - REVIEW RECOMMENDED' };
  ...
  return (
    ...
    >
      <span className="flex items-center gap-2 truncate max-w-full">
        <span
          className={`w-2 h-2 rounded-full shrink-0 ${riskStatus.color === 'red' ? 'bg-red-500 animate-pulse' : riskStatus.color === 'yellow' ? 'bg-yellow-500' : 'bg-emerald-500'}`}
        ></span>
