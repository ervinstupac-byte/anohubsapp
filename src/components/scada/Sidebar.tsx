    ...
    <div className="border-b border-cyan-900/30 bg-slate-950">
      ...
      >
        <div className="flex items-center gap-2">
          <span
            className={`text-[12px] font-mono font-black text-slate-500 uppercase tracking-[0.1em] transition-colors ${isExpanded ? 'text-cyan-400' : ''}`}
          >
      ...
      {isExpanded && (
        <div className="pb-4 animate-in slide-in-from-top-2 duration-200">
          <ErrorBoundary
            fallback={
              <div className="p-4 text-xs text-red-500 bg-red-950/30 border border-red-500/30 rounded-none">
...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const operationalModules = [
    {
      id: 'riskAssessment',
      title: t('modules.riskAssessment', 'Risk Diagnostics'),
      icon: '🛡️',
      route: `/${ROUTES.RISK_ASSESSMENT}`,
    },
    {
      id: 'francisHub',
      title: t('sidebar.francisLogic', 'Francis Logic Map'),
      icon: '🧠',
      route: `/${ROUTES.FRANCIS.HUB}`,
    },
    {
      id: 'maintenanceDashboard',
      title: t('modules.maintenance', 'Maintenance Engine'),
      icon: '⚙️',
      route: `/${ROUTES.MAINTENANCE.DASHBOARD}`,
    },
    {
      id: 'shaftAlignment',
      title: t('sidebar.shaftAlignment', 'Shaft Alignment'),
      icon: '🔄',
      route: `/${ROUTES.FRANCIS.ROOT}/${ROUTES.FRANCIS.SOP.ALIGNMENT}`,
    },
    {
      id: 'hydraulicMaintenance',
      title: t('sidebar.hydraulicMaintenance', 'Hydraulic Maintenance'),
      icon: '🚰',
      route: `/${ROUTES.MAINTENANCE.HYDRAULIC}`,
    },
    {
      id: 'boltTorque',
      title: t('sidebar.boltTorque', 'Bolt Torque'),
      icon: '🔩',
      route: `/${ROUTES.MAINTENANCE.BOLT_TORQUE}`,
    },
    {
      id: 'shadowEngineer',
      title: t('sidebar.shadowEngineer', 'Shadow Engineer'),
      icon: '👻',
      route: `/${ROUTES.MAINTENANCE.SHADOW_ENGINEER}`,
    },
    {
      id: 'intuitionLog',
      title: t('sidebar.intuitionLog', 'Intuition Log'),
      icon: '👂',
      route: `/${ROUTES.MAINTENANCE.INTUITION_LOG}`,
    },
    {
      id: 'structuralIntegrity',
      title: t('sidebar.structuralIntegrity', 'Structural Integrity'),
      icon: '🏗️',
      route: `/${ROUTES.STRUCTURAL_INTEGRITY}`,
    },
    {
      id: 'installationGuarantee',
      title: t('modules.installationGuarantee', 'Precision Audit'),
      icon: '📏',
      route: `/${ROUTES.INSTALLATION_GUARANTEE}`,
    },
    {
      id: 'hppBuilder',
      title: t('modules.hppBuilder', 'HPP Studio'),
      icon: '⚡',
      route: `/${ROUTES.HPP_BUILDER}`,
    },
    {
      id: 'mounterCard',
      title: 'Mounter Quick Card',
      icon: '🔧',
      route: `/${ROUTES.MAINTENANCE.ROOT}/${ROUTES.MAINTENANCE.MOUNTER_CARD}`,
    },
    {
      id: 'damageCard',
      title: 'Damage Diagnosis',
      icon: '🩺',
      route: `/${ROUTES.MAINTENANCE.ROOT}/${ROUTES.MAINTENANCE.DAMAGE_CARD}`,
    },
    {
      id: 'assetPassportCard',
      title: 'Asset Passport',
      icon: '🪪',
      route: `/${ROUTES.MAINTENANCE.ROOT}/${ROUTES.MAINTENANCE.ASSET_PASSPORT_CARD}`,
    },
      ...
      >
          ...
          <div className="p-4 space-y-4">
            ...
            <div className="flex bg-slate-950 p-1 rounded-none border border-cyan-900/30">
              {(['HUMAN', 'HISTORY', 'REALTIME'] as const).map(layer => (
                <button
                  ...
                  className={`flex-1 py-1.5 text-[10px] font-bold uppercase tracking-wider rounded-none transition-all ${
                    activeLayer === layer
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-baseline">
                ...
                <span
                  className={`text-[10px] font-mono ${signalQuality >= 0.8 ? 'text-emerald-500' : 'text-amber-500'}`}
                >
              ...
              <div className="space-y-2 pt-2 border-t border-white/5 animate-in slide-in-from-top-2">
                ...
                <div className="flex justify-center gap-4">
                  <button
                    ...
                    className={`p-1 transition-colors ${playback.isPlaying ? 'text-cyan-400' : 'text-slate-500'}`}
                  >
                    {playback.isPlaying ? (
                ...
                {liveMetrics.slice(0, 3).map((metric: any, i: number) => (
                  <div key={i} className="group relative">
                    ...
                    <div className="opacity-50 group-hover:opacity-100 transition-opacity">
                      {metric.history && (
                        <Sparkline data={metric.history} width={280} height={20} color="#06b6d4" />
                      )}
                    </div>
                  </div>
                ))}
              ...
              <div className="mt-4 pt-4 border-t border-cyan-900/30 space-y-3">
                {diagnostics.slice(0, 2).map((insight: any) => (
                  <div
                    ...
                    className={`p-3 rounded-none border-l-2 ${insight.type === 'CRITICAL' ? 'bg-red-950/20 border-red-500' : 'bg-amber-950/20 border-amber-500'} animate-in slide-in-from-right-4`}
                  >
                    <div className="flex justify-between items-start mb-1">
                      <div className="flex items-center gap-1.5">
                        <BrainCircuit
                          className={`w-3 h-3 ${insight.type === 'CRITICAL' ? 'text-red-400' : 'text-amber-400'}`}
                        />
                        <span
                          className={`text-[10px] font-bold uppercase ${insight.type === 'CRITICAL' ? 'text-red-400' : 'text-amber-400'}`}
                        >
                    ...
                    <p className="text-[10px] text-slate-300 leading-snug mb-2">{insight.slogan}</p>
            ...
            {operationalModules.map(item => {
              ...
              return (
                <button
                  ...
                  className={`w-full text-left px-3 py-2 rounded-none flex items-center gap-3 transition-all duration-200 group ${
                    isActive
                      ? 'bg-cyan-950/30 text-cyan-400 border border-cyan-500/20 shadow-none'
                      : 'text-slate-400 hover:bg-white/5 hover:text-cyan-200'
                  }`}
                >
                  <span className="text-lg opacity-80 group-hover:scale-110 transition-transform filter grayscale group-hover:grayscale-0">
                    {item.icon}
                  </span>
              ...
              >
                ...
                <div className="flex flex-col items-start">
                  <span className="text-[10px] font-black uppercase tracking-wider">
                    Generate 60s Forensic Report
                  </span>
                  <span className="text-[8px] font-mono opacity-70">
                    Capture Visuals + Logic Trace
                  </span>
                </div>
              </button>
            </div>
        ...
        <div className="p-4 border-t border-cyan-900/30 bg-slate-950">
          ...
          <div className="mt-3 text-[9px] font-mono text-slate-500 flex justify-between items-center">
            <span>HIVE LINK:</span>
            <span
              className={`font-bold ${hiveStatus?.connected ? 'text-emerald-500' : 'text-slate-600'}`}
            >
