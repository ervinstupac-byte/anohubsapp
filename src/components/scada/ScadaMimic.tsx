...
const TurbineUnit: React.FC<{
  ...
  mw: number;
}> = React.memo(({ name, status, mw }) => (
  <div className="relative group">
    <Tooltip
      content={`${name} is currently ${status === 'running' ? 'Active & Generating' : 'Standby'}`}
    >
      ...
      >
        ...
        <div
          className={`w-40 h-40 rounded-full border-[2px] border-dashed ${status === 'running' ? 'border-status-info animate-spin-slow' : 'border-scada-border'}`}
        ></div>
    ...
    >
      ...
      {mw === 0 && (
        <div className="text-xs font-black text-status-error tracking-tight px-2 py-1 bg-status-error/10 rounded-none border border-status-error/30">
          EMERGENCY SHUTDOWN
        </div>
      )}
    </div>
...
export const ScadaMimic: React.FC = React.memo(() => {
  ...
  const connectSCADAToExpertEngine = (flow: number, head: number, frequency: number) => {
    ...
    return {
      healthScore:
        diagnosis.severity === 'CRITICAL' ? 40 : diagnosis.severity === 'WARNING' ? 70 : 98,
      criticalAlarms,
  ...
  const bearingTemp = Number(
    Array.isArray(telemetryStore.telemetryHistory?.bearingTemp) &&
      telemetryStore.telemetryHistory.bearingTemp.length > 0
      ? telemetryStore.telemetryHistory.bearingTemp[
          telemetryStore.telemetryHistory.bearingTemp.length - 1
        ]?.value
  ...
  const powerMW = Number(
    (telemetryStore.physics as any)?.powerMW?.toNumber?.() ??
      telemetryStore.physics?.powerMW ??
      baseMw * 2
  );
  ...
  useEffect(() => {
    ...
    if (gridFreq >= 98.0) {
      console.error(
        'CRITICAL: Grid frequency at',
        gridFreq,
        'Hz - Risk of mechanical destruction!'
      );
    }
  }, [
    selectedAsset,
        ...
        <div className="relative z-10 max-w-2xl text-center space-y-8 animate-fade-in">
          ...
          <div className="space-y-4">
            ...
            <p className="text-xl text-scada-muted max-w-xl mx-auto font-light font-mono">
              Connect your turbines to AnoHUB's advanced monitoring and diagnostic platform in
              minutes.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-12 pt-12 border-t border-scada-border">
            <div className="p-6 bg-scada-panel border border-scada-border rounded-sm hover:border-status-info transition-all">
              ...
              <h3 className="text-scada-text font-bold mb-2 uppercase tracking-wide">
                Real-Time SCADA
              </h3>
              <p className="text-sm text-scada-muted font-mono">
                Live telemetry and performance monitoring
              </p>
            </div>
            <div className="p-6 bg-scada-panel border border-scada-border rounded-sm hover:border-status-info transition-all">
              <div className="text-3xl mb-3 grayscale opacity-70">🤖</div>
            ...
            <div className="p-6 bg-scada-panel border border-scada-border rounded-sm hover:border-status-info transition-all">
              ...
              <p className="text-sm text-scada-muted font-mono">
                Automated work orders & service tracking
              </p>
            </div>
          </div>
    ...
    <div className="flex-1 bg-scada-bg relative overflow-hidden flex flex-col items-center justify-center p-4 sm:p-6 md:p-12">
      ...
      <div className="relative z-10 w-full max-w-6xl border border-scada-border bg-scada-panel p-6 sm:p-8 md:p-16 rounded-sm shadow-scada-card overflow-hidden self-center">
        ...
        <Tooltip
          content={
            isCritical
              ? 'FATAL ERROR: Bearing Temperature Exceeded / High Vibration Detected'
              : 'Infrastructure Status: All metrics within operational thresholds.'
          }
        >
          <div
            className={`absolute top-4 right-4 sm:top-6 sm:right-6 text-xs sm:text-sm font-mono px-4 py-2.5 rounded-sm flex items-center gap-3 border transition-all ${
              isCritical
          ...
          >
            <span
              className={`w-2.5 h-2.5 rounded-full ${isCritical ? 'bg-status-error' : 'bg-status-ok'}`}
            ></span>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6 mt-12">
          <DigitalDisplay
            value={scadaAlarms.find(a => a.type === 'CAVITATION_CRITICAL') ? 'CAVITATE' : '42.5'}
            label="FLOW_RATE"
            unit="m³/s"
            color={scadaAlarms.find(a => a.type === 'CAVITATION_CRITICAL') ? 'red' : 'green'}
            className="!bg-scada-bg !border-scada-border"
          />
          <DigitalDisplay
            value={scadaAlarms.find(a => a.type === 'CAVITATION_CRITICAL') ? 'CAVITATE' : '152.0'}
            label="HEAD_PRES"
            unit="m"
            color={scadaAlarms.find(a => a.type === 'CAVITATION_CRITICAL') ? 'red' : 'green'}
            className="!bg-scada-bg !border-scada-border"
          ...
          <DigitalDisplay
            value={
              scadaAlarms.find(a => a.type === 'GRID_FREQUENCY_CRITICAL') ? 'DESTRUCT' : '98.2'
            }
            label="GRID_FREQ"
            unit="Hz"
            color={scadaAlarms.find(a => a.type === 'GRID_FREQUENCY_CRITICAL') ? 'red' : 'green'}
            className="!bg-scada-bg !border-scada-border"
          />
          <DigitalDisplay
            value={isCritical ? 'ALERT' : '12.2'}
            label="COOLING_ΔT"
            unit="°C"
          ...
          <div className="mt-6 p-6 bg-status-error/10 border border-status-error rounded-sm shadow-scada-card">
            ...
            <div className="space-y-2">
              {scadaAlarms.slice(0, 3).map((alarm, idx) => (
                <div
                  key={idx}
                  className="text-status-error text-sm font-medium flex items-start gap-2 p-2 bg-status-error/5 rounded-sm"
                >
              ...
              {scadaAlarms.length > 3 && (
                <div className="text-center pt-2">
                  <span
                    className="text-xs text-status-error/70 font-mono cursor-pointer hover:text-status-error"
                    onClick={() => setShowAlarmsModal(true)}
                  >
      ...
      {/* Active Alarms Modal */}
      <ActiveAlarmsModal isOpen={showAlarmsModal} onClose={() => setShowAlarmsModal(false)} />
    </div>
  );
});
