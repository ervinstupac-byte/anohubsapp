    ...
    <div className="relative w-full h-[400px] bg-slate-950/50 rounded-none overflow-hidden border border-white/5 shadow-none">
      ...
      {plants.map(plant => (
        ...
        >
          ...
          <div
            className={`w-4 h-4 rounded-none border-2 ${plant.healthScore < 90 ? 'bg-amber-500 border-amber-300' : 'bg-cyan-500 border-cyan-300'} shadow-none transition-all group-hover:scale-125`}
          ></div>
        ...
        <div className="bg-slate-900/40 rounded-none border border-white/5 flex flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
            {hiveEvents.map(event => (
              <div
                key={event.id}
                className="bg-slate-950/50 p-3 rounded-none border border-white/5 animate-in slide-in-from-right-2"
              >
                <div className="flex justify-between items-start mb-1">
                  <span
                    className={`text-[9px] font-bold px-1.5 py-0.5 rounded-none ${event.type === 'WEIGHT_SYNC' ? 'bg-purple-500/20 text-purple-400' : 'bg-amber-500/20 text-amber-400'}`}
                  >
