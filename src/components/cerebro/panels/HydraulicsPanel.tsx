    ...
    <div className="space-y-6">
      <GlassCard className="p-6 border-l-4 border-blue-500">
        <h3 className="text-sm font-bold text-white uppercase tracking-widest mb-4 flex items-center gap-2">
          ...
          {t('hpp.penstock', 'Penstock Hydro-Physics')}
        </h3>
          ...
          <div className="space-y-2">
            <label className="text-xs font-bold text-slate-500 uppercase flex justify-between">
              ...
              <span className="text-white font-mono">
                {formatNumber(wallThickness, language, 0)} mm
              </span>
            </label>
            <input
              type="range"
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="p-3 bg-blue-950/20 rounded border border-blue-500/20">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-[10px] text-blue-400 font-bold uppercase">
                {t('physics.waterHammer', 'Water Hammer')}
              </span>
            </div>
            <div className="text-2xl font-black text-white font-mono">
              {formatNumber(activeSurge, language, 1)}{' '}
          ...
          <div
            className={`p-3 rounded border ${activeHoopStress > 200 ? 'bg-red-950/20 border-red-500/20' : 'bg-emerald-950/20 border-emerald-500/20'}`}
          >
            <div className="flex items-center gap-2 mb-1">
              <Shield
                className={`w-4 h-4 ${activeHoopStress > 200 ? 'text-red-500' : 'text-emerald-500'}`}
              />
              <span
                className={`text-[10px] font-bold uppercase ${activeHoopStress > 200 ? 'text-red-400' : 'text-emerald-400'}`}
              >
                {t('physics.hoopStress', 'Hoop Stress')}
              </span>
            </div>
            <span className="text-xl font-mono text-white">
              {activeHoopStress.toFixed(1)} <span className="text-xs">MPa</span>
            </span>
          ...
          <div className="p-3 bg-slate-900 border border-white/5 rounded">
            ...
            <div className="text-xl font-black text-amber-500 font-mono">
              {formatNumber(activeHeadLoss, language, 2)}{' '}
          ...
          <div className="p-3 bg-slate-900 border border-white/5 rounded">
            ...
            <div className="text-xl font-black text-cyan-500 font-mono">
              {formatNumber(activeNetHead, language, 2)}{' '}
              <span className="text-xs text-slate-600">m</span>
            </div>
