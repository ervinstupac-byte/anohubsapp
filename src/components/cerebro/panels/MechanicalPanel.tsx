    ...
    <div className="space-y-6 animate-fade-in">
      <GlassCard className="p-6 border-l-4 border-[#2dd4bf]">
        <h3 className="text-sm font-bold text-white uppercase tracking-widest mb-4 flex items-center gap-2">
          ...
          {t('hpp.mechanical', 'Mechanical Integrity')}
        </h3>
        ...
        <div className="grid grid-cols-2 gap-6">
          ...
          <div className="space-y-2">
            <label className="text-xs font-bold text-slate-500 uppercase">
              {t('glossary.bolt_grade', 'Bolt Grade')} (ISO)
            </label>
              ...
              {['4.6', '5.6', '8.8', '10.9'].map(grade => (
                <button
                  ...
                  className={`px-3 py-1 text-xs font-bold rounded border transition-all ${
                    liveMechanical.boltSpecs.grade === grade
        ...
        <div className="mt-6 p-4 rounded bg-black/20 border border-white/5">
          <div className="flex justify-between items-center mb-2">
            <span className="text-xs text-slate-400 font-mono uppercase">
              {t('physics.boltLoad', 'Calculated Load')}
            </span>
            <span className="text-sm text-white font-mono font-bold">
              {formatNumber(activeBoltLoad, language, 2)} kN
            </span>
          </div>
          <div className="flex justify-between items-center mb-2">
            <span className="text-xs text-slate-400 font-mono uppercase">
              {t('physics.boltCapacity', 'Bolt Capacity')}
            </span>
            <span className="text-sm text-white font-mono font-bold">
              {formatNumber(activeBoltCapacity, language, 2)} kN
            </span>
          </div>
          ...
          <div className="flex justify-between items-center">
            ...
            <span
              className={`text-lg font-black font-mono ${
                activeSafetyFactor < 1.5 ? 'text-red-500' : 'text-emerald-500'
              }`}
            >
      ...
      <GlassCard className="p-6">
        <div className="flex justify-between items-start">
          <div>
            <h3 className="text-sm font-bold text-white uppercase tracking-widest mb-2">
              {t('physics.shaftAlignment', 'Shaft Alignment')}
            </h3>
        ...
        <div className="relative h-4 bg-slate-800 rounded-full overflow-hidden">
          ...
          />
          <div className="absolute top-0 bottom-0 w-0.5 bg-red-500 left-[80%]" />{' '}
          {/* Limit Marker */}
        </div>
              ...
              return (
                ...
                >
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <div
                      className={`px-2 py-1 rounded border text-xs font-bold flex items-center gap-1 ${statusColors[health.status]}`}
                    >
