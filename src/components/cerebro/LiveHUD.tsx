      ...
      >
        ...
        <div className="grid grid-cols-2 gap-6">
          ...
          <div className="space-y-1">
            ...
            {powerDelta !== 0 && (
              <div
                className={`text - xs font - bold flex items - center ${isPowerRising ? 'text-emerald-500' : 'text-red-500'} `}
              >
                {isPowerRising ? '+' : ''}
                {powerDelta.toFixed(1)} kW
                {isPowerRising ? (
                  <TrendingUp className="w-3 h-3 ml-1" />
                ) : (
                  <TrendingUp className="w-3 h-3 ml-1 rotate-180" />
                )}
              </div>
            )}
          </div>
          ...
          <div className="space-y-1">
            ...
            {roiDelta !== 0 && (
              <div
                className={`text - xs font - bold ${isRoiImproving ? 'text-emerald-500' : 'text-amber-500'} `}
              >
                {roiDelta > 0 ? '+' : ''}
                {roiDelta.toFixed(1)} yrs
              </div>
            )}
          </div>
        </div>
