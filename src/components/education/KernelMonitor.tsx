      ...
      <div className="space-y-2 relative">
        ...
        {lastTrace.stages.map((stage, i) => (
          <div key={i} className="relative pl-8 group">
            <div
              className={`absolute left-[0.65rem] top-1.5 w-2 h-2 rounded-full border-2 border-slate-950 ${
                stage.stage === 'HEAL'
                  ? 'bg-emerald-500'
              ...
              {stage.stage === 'DIAGNOSE' && (
                <div className="text-[10px] text-purple-300 flex items-center gap-2">
                  ...
                  {typeof stage.output === 'string'
                    ? stage.output
                    : JSON.stringify(stage.output).slice(0, 30)}
                </div>
              )}
