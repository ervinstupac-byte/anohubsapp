...
const PRESETS: Scenario[] = [
  {
    ...
    steps: [
      {
        durationMs: 2000,
        telemetry: { flowRate: 45, netHead: 150, vibration: 0.05 },
        description: 'Nominal Operation',
      },
      {
        durationMs: 2000,
        telemetry: { flowRate: 48, netHead: 148, vibration: 0.08 },
        description: 'Flow Increasing',
      },
      {
        durationMs: 2000,
        telemetry: { flowRate: 52, netHead: 145, vibration: 0.15 },
        description: 'Cavitation Inception',
      },
      {
        durationMs: 2000,
        telemetry: { flowRate: 55, netHead: 142, vibration: 0.45 },
        description: 'Severe Cavitation',
      },
      {
        durationMs: 2000,
        telemetry: { flowRate: 45, netHead: 150, vibration: 0.05 },
  ...
  },
  {
    ...
    steps: [
      {
        durationMs: 2000,
        telemetry: { activePower: 42, rpm: 428, vibration: 0.05 },
        description: 'Full Load',
      },
      {
        durationMs: 500,
        telemetry: { activePower: 0, rpm: 450, vibration: 0.2 },
        description: 'Breaker Open',
      },
      {
        durationMs: 1000,
        telemetry: { activePower: 0, rpm: 580, vibration: 0.8 },
        description: 'Overspeed Peak',
      },
      {
        durationMs: 3000,
        telemetry: { activePower: 0, rpm: 428, vibration: 0.1 },
        description: 'Governor Response',
      },
    ],
  },
];
    ...
    <div className="p-4 bg-slate-900 rounded-lg border border-slate-700">
      ...
      <div className="grid grid-cols-1 gap-4">
        {scenarios.map(s => (
          <div
            key={s.id}
            className="p-4 bg-slate-800 rounded border border-slate-600 hover:border-cyan-500 transition-colors"
          >
            <div className="flex justify-between items-start mb-2">
              ...
              >
                ...
                {activeScenario === s.id
                  ? `Running Step ${currentStepIndex + 1}/${s.steps.length}`
                  : 'Run Demo'}
              </button>
            </div>
      ...
      <div className="mt-4 p-4 bg-yellow-900/20 border border-yellow-700/50 rounded text-yellow-200 text-sm">
        <p>
          ⚠️ <strong>Educational Mode Active:</strong> Scenarios inject synthetic data patterns to
          demonstrate system response. Real telemetry is overridden.
        </p>
      </div>
    </div>
  );
