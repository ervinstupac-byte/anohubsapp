    ...
    <div className="p-4 bg-slate-950 rounded-lg border border-slate-800 font-mono text-xs h-full flex flex-col">
      ...
      <div className="grid grid-cols-3 gap-2 mb-4 shrink-0">
        ...
        <div className="bg-slate-900 p-2 rounded border border-slate-800 text-center">
          ...
          <span
            className={`font-bold ${errorServices > 0 ? 'text-red-400 animate-pulse' : 'text-slate-600'}`}
          >
            {errorServices}
          </span>
        </div>
      ...
      <div className="flex-1 overflow-y-auto space-y-1 pr-1">
        ...
        {services.map(service => (
          <div
            key={service.id}
            className="bg-slate-900/50 p-2 rounded border border-slate-800 hover:bg-slate-800 hover:border-slate-600 transition-colors group"
          >
            ...
            <div className="grid grid-cols-3 gap-1 text-[9px] text-slate-500">
              <div className="bg-slate-950 px-1.5 py-1 rounded flex items-center gap-1">
                ...
                <span>
                  {Date.now() - service.lastHeartbeat < 1000
                    ? 'Live'
                    : `${((Date.now() - service.lastHeartbeat) / 1000).toFixed(0)}s ago`}
                </span>
              </div>
