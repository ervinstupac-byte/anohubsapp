  ...
  const getComponentData = (id: string) => {
    ...
    return {
      ...
      risks: Array.isArray(risks)
        ? (risks as { text: string; level: 'High' | 'Medium' | 'Low' }[])
        : [],
    };
  };
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-4 shrink-0 pt-4">
        <div className="flex items-center gap-4">
          ...
          <h2 className="text-3xl font-bold text-white tracking-tight hidden md:block">
            ...
            <span className="text-cyan-400">{t('componentLibrary.titleHighlight')}</span>
          </h2>
        </div>
          ...
          {COMPONENT_IDS.map(id => (
            ...
            >
              <div className="flex justify-between items-center relative z-10">
                <h3
                  className={`font-bold text-sm transition-colors ${selectedId === id ? 'text-white' : 'text-slate-400 group-hover:text-slate-200'}`}
                >
              ...
              {/* Hover Effect */}
              <div
                className={`absolute left-0 top-0 bottom-0 w-1 bg-cyan-500 transition-all duration-300 ${selectedId === id ? 'opacity-100' : 'opacity-0 group-hover:opacity-50'}`}
              ></div>
            </button>
          ))}
        </div>
              ...
              <div className="space-y-4">
                ...
                <ul className="space-y-2">
                  {selectedComponent.kpis.map((kpi, idx) => (
                    <li
                      key={idx}
                      className="flex items-start gap-3 bg-slate-900/40 p-3 rounded-lg border border-white/5 hover:border-cyan-500/30 transition-colors"
                    >
              ...
              <div className="space-y-4">
                ...
                <ul className="space-y-2">
                  {selectedComponent.risks.map((risk, idx) => (
                    <li
                      key={idx}
                      className="flex items-start gap-3 bg-red-900/10 p-3 rounded-lg border border-red-500/10 hover:border-red-500/30 transition-colors"
                    >
                      <span
                        className={`
