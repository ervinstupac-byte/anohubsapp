    ...
    <GlassCard
      ...
      className={
        fatigueData.isCritical
          ? 'border-l-4 border-l-red-500 bg-red-950/20 shadow-[0_0_20px_rgba(239,68,68,0.2)]'
      ...
      <div className="space-y-6">
        <div className="flex justify-between items-end">
          <div>
            ...
            <p
              className={`text-3xl font-mono font-black ${fatigueData.isCritical ? 'text-red-500 animate-pulse' : 'text-purple-400'}`}
            >
          ...
          <div className="h-3 bg-slate-900 rounded-full overflow-hidden p-[2px] border border-white/5">
            <div
              className={`h-full rounded-full transition-all duration-1000 ${
                fatigueData.percentage > 90
                  ? 'bg-red-500'
                  : fatigueData.percentage > 60
                    ? 'bg-amber-500'
                    : 'bg-purple-500'
              }`}
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs font-bold text-white uppercase">
                  NDT INSPECTION MANDATED (ULTRAZVUK / MAGNETOFLUKS)
                </p>
              </div>
            </div>
            <p className="text-[10px] text-slate-400 italic bg-black/40 p-2 rounded border border-white/5">
              Points exceeded safety threshold due to hydraulic shocks ({'>'}5 bar/s). Structural
              integrity cannot be guaranteed without non-destructive testing.
            </p>
            <button
              onClick={() => {
        ...
        {!fatigueData.isCritical && (
          <div className="bg-slate-900/40 border border-white/5 p-3 rounded-xl flex items-center gap-4">
            <div className="w-8 h-8 rounded-full bg-purple-500/10 flex items-center justify-center text-purple-400 text-lg">
              🛡️
            </div>
            <div>
              <p className="text-[10px] text-slate-500 font-black uppercase mb-0.5">
                Ano-Agent Assessment
              </p>
              <p className="text-[10px] text-white font-bold leading-tight uppercase">
                Cycle load within nominal range. {100 - parseFloat(fatigueData.points)} pts
                remaining until mandatory ultrasound.
              </p>
            </div>
          </div>
        )}
