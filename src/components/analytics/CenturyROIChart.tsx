...
export default function CenturyROIChart({
  assetId,
  pricePerKwh = 0.08,
}: {
    ...
    (async () => {
      ...
      try {
        const { data: cp } = await supabase
          .from('century_plans')
          .select('asset_id, input_json, projections')
          .limit(10);
        ...
        if (cp && cp.length > 0) {
          ...
          const plan = assetId
            ? cp.find((p: any) => String(p.asset_id) === String(assetId))
            : cp[0];
          if (plan && plan.projections) {
            ...
            yearSeries =
              plan.projections && Array.isArray(plan.projections) ? plan.projections : [];
          }
        }
        ...
        if (yearSeries.length === 0) {
          ...
          const seedLoss =
            af && af.length > 0
              ? af.reduce((acc: number, r: any) => acc + Number(r.computed_loss_cost || 0), 0)
              : 0;
          ...
          for (let y = START_YEAR; y <= END_YEAR; y++) {
            ...
            yearSeries.push({
              ...
              asset_book_value,
              total_investment: bookValue,
              net_position: cumulative - (bookValue - asset_book_value),
            });
          }
        }
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const y = d3
        .scaleLinear()
        .domain([
          d3.min(data, (d: any) => d.net_position),
          d3.max(data, (d: any) => d.cumulative_profit) * 1.1,
        ])
        .range([height, 0]);

      svg.append('g').call(d3.axisLeft(y).tickFormat((n: any) => d3.format('.2s')(n)));
