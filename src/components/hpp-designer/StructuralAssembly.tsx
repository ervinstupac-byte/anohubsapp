...
// --- COMPONENT ---
// --- COMPONENT ---
export const StructuralAssembly: React.FC<{
  onComplete: () => void;
  onAssemblyChange: (parts: { partId: string; alignment?: number; timestamp: number }[]) => void;
}> = ({ onComplete, onAssemblyChange }) => {
  ...
  const [assemblyRecords, setAssemblyRecords] = useState<
    { partId: string; alignment?: number; timestamp: number }[]
  >([]); // New State
  ...
  const validateDrop = (partId: ComponentType): { valid: boolean; errorKey?: string } => {
    ...
    if (part.requiredBelow && !assembledParts.includes(part.requiredBelow)) {
      ...
      if (partId === 'HEAD_COVER' && !assembledParts.includes('WICKET_GATES'))
        return { valid: false, errorKey: 'seq_004' }; // Should trigger earlier? No, Head Cover needs Wicket Gates below it.
      if (partId === 'ROTOR' && !assembledParts.includes('HEAD_COVER'))
        return { valid: false, errorKey: 'seq_005' };
  ...
  const handleDrop = (partId: ComponentType) => {
    ...
    if (!validation.valid) {
      if (validation.errorKey) {
        ...
        setAuditLog(prev => [
          `[ERROR] ${t(`hpp_builder.assembly.components.${partId.toLowerCase()}`)}: ${t(`hpp_builder.assembly.errors.${validation.errorKey}`)}`,
          ...prev,
        ]);
      }
      return;
      ...
      if (alignmentVal > 0.05) {
        ...
        if (selectedAsset) {
          ...
          logActivity(
            activityAssetId,
            'MAINTENANCE',
            `Installation Deviation: ${partId} at ${alignmentVal} mm/m`,
            { oldVal: 0, newVal: alignmentVal }
          );
        }
      }
    }
  ...
  const submitAlignment = () => {
    ...
    if (isNaN(val) || val < 0) {
      showToast(
        t('validation.limits.negative', { field: 'Alignment', min: 0, unit: 'mm/m' }),
        'error'
      );
      return;
    }
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 min-h-[600px]">
      ...
      <div className="bg-slate-900/50 rounded-2xl p-6 border border-white/10">
        <h3 className="text-sm font-bold text-slate-400 uppercase tracking-widest mb-6">
          {t('hpp_builder.assembly.components.inventory')}
        </h3>
            ...
            return (
              ...
              >
                ...
                <span className="text-xs font-bold text-center text-slate-300">
                  {t(`hpp_builder.assembly.components.${part.id.toLowerCase()}`)}
                </span>
                {isInstalled && (
                  <span className="text-[10px] text-emerald-500 font-mono">INSTALLED</span>
                )}
              </motion.div>
            );
          })}
        ...
        <div className="relative w-64 flex flex-col-reverse items-center gap-1 z-10">
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <span className="text-sm font-bold text-cyan-100">
                      {t(`hpp_builder.assembly.components.${partId.toLowerCase()}`)}
                    </span>
                  </div>
          ...
          {assembledParts.length === 0 && (
            <div className="text-slate-600 text-sm font-mono animate-pulse">
              {t('hpp_builder.assembly.status.foundation_locked')}
            </div>
          )}
        </div>
      </div>
      ...
      <div className="bg-slate-950 rounded-2xl border border-white/10 p-0 overflow-hidden flex flex-col">
        <div className="bg-slate-900/80 p-4 border-b border-white/5">
          <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest">
            {t('hpp_builder.assembly.title')}
          </h3>
        </div>
        <div className="flex-1 p-4 overflow-y-auto space-y-3 font-mono text-xs">
          {auditLog.length === 0 && (
            <div className="text-slate-600 italic text-center mt-10">Waiting for assembly...</div>
          )}
          {auditLog.map((log, i) => (
            <div
              key={i}
              className={`p-2 rounded border-l-2 ${log.includes('[ERROR]') ? 'border-red-500 bg-red-950/10 text-red-200' : 'border-emerald-500 bg-emerald-950/10 text-emerald-200'}`}
            >
              {log}
            </div>
          ))}
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-cyan-500/30 p-8 rounded-2xl w-full max-w-md shadow-[0_0_50px_rgba(6,182,212,0.2)] animate-fade-in-up">
            <h3 className="text-xl font-bold text-white mb-2">
              {t('hpp_builder.assembly.prompts.measure_alignment')}
            </h3>
            <p className="text-slate-400 text-sm mb-6">
              Enter precision laser tracker value for{' '}
              {t(`hpp_builder.assembly.components.${alignmentModal.part?.toLowerCase()}`)}.
            </p>
            ...
            <div className="flex justify-end gap-3">
              <ModernButton
                variant="secondary"
                onClick={() => setAlignmentModal({ open: false, part: null })}
              >
                {t('common.cancel')}
              </ModernButton>
              <ModernButton variant="primary" onClick={submitAlignment}>
                {t('common.confirm')}
              </ModernButton>
            </div>
          </div>
