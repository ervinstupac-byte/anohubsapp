...
export const COMPONENT_INFO: Record<
  string,
  {
    ...
    metricKey: string;
    unit: string;
    aiType?: 'bearing' | 'cavitation' | 'efficiency';
  }
> = {
  runner_hub: {
...
const AIPredictionOverlay = ({
  type,
  assetId,
}: {
  type: 'bearing' | 'cavitation' | 'efficiency';
  assetId?: string;
}) => {
    ...
    <div className="mt-2 pt-2 border-t border-scada-border">
      <div className="flex items-center justify-between mb-1">
        <div className="flex items-center gap-1">
          <div
            className={`w-1.5 h-1.5 rounded-full ${prediction.probability > 0.5 ? 'bg-status-error animate-pulse' : 'bg-status-ok'}`}
          />
          <span className="text-status-warning text-[10px] font-mono uppercase font-bold tracking-wider">
            AI Forecast
          </span>
        </div>
        <span
          className={`text-[9px] font-bold ${prediction.probability > 0.5 ? 'text-status-error' : 'text-status-ok'}`}
        >
  ...
  useFrame((state, delta) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const scale = Math.min(
        0.15,
        cavitationIntensity * 0.1 * (1 + Math.sin(state.clock.elapsedTime * 10 + i))
      );
...
const RunnerMesh: React.FC<RunnerMeshProps> = ({
  ...
  useFrame((state, delta) => {
    if (active && groupRef.current) {
      ...
      const isTripped =
        currentRpm > 600 && (storeState.governor?.actualValue?.toNumber?.() ?? 0) < 10;
      ...
      groupRef.current.traverse(child => {
        if (child instanceof THREE.Mesh) {
          ...
          const isSelected =
            selectedPart === child.name ||
            (selectedPart === 'blade' && child.name.startsWith('blade'));
          const isHovered =
            hoveredPart === child.name ||
            (hoveredPart === 'blade' && child.name.startsWith('blade'));
          ...
          if (child.name === 'nose_cone' || child.name === 'runner_hub') {
            if (!heatmapMode && !isSelected) {
              // Only override if not in manual heatmap mode
          ...
          if (child.name.startsWith('blade')) {
            if (!heatmapMode && !isSelected) {
              ...
              if (useLivePhysics && surgePressure > 50) {
                const surgePulse =
                  1 + Math.sin(state.clock.elapsedTime * 30) * 0.02 * (surgePressure / 100);
  ...
  return (
    <group ref={groupRef}>
      ...
      {showInfoPanel && info && (
        <Html position={[2, 2, 0]} center style={{ pointerEvents: 'none' }}>
          <div className="w-64 bg-scada-panel border border-status-info/30 p-4 rounded-sm shadow-scada-card">
            <h3 className="text-status-info font-bold font-mono text-sm uppercase mb-1">
              {info.title}
            </h3>
      ...
      {Array.from({ length: isPelton ? 18 : isKaplan ? 5 : 13 }).map((_, i) => {
        ...
        const initialPitch = isKaplan
          ? (useTelemetryStore.getState().governor?.bladeAngle?.toNumber?.() ?? 0)
          : 0;
...
export const TurbineRunner3D = forwardRef<HTMLDivElement, TurbineRunner3DProps>(
  (
    ...
    if (!crashSafe)
      return (
        <div className="p-4 text-red-500 font-mono text-xs border border-red-500">
          3D RENDER FAILURE
        </div>
      );
    ...
    return (
      ...
      >
        ...
        >
          ...
          <ThreeErrorBoundary>
            {turbineType === 'pelton' ? <PeltonNozzles count={6} /> : <WicketGates />}
            <RunnerMesh
              rpm={rpm}
