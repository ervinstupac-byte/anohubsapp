...
export const EngineeringDossierCard: React.FC = () => {
  ...
  const filteredSources = useMemo(() => {
    return provenSources.filter(s => {
      ...
      const matchesSearch =
        ...
        (intel &&
          ((intel.title || '').toLowerCase().includes(term) ||
            (intel.currentAnalysis || '').toLowerCase().includes(term) ||
            (intel.operationalRecommendation || '').toLowerCase().includes(term)));
  ...
  const displayedSources = useMemo(
    () => filteredSources.slice(0, page * PAGE_SIZE),
  ...
  React.useEffect(() => {
    const handler = (e: any) => {
      try {
        ...
        if (!kw) return;
        const match = DOSSIER_LIBRARY.find(ds =>
          (ds.path + ' ' + (ds.justification || '')).toLowerCase().includes(kw)
        );
        if (match) {
    ...
    <>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-scada-muted font-mono uppercase">
                NC-9.0 Integrity Check: 854 Files Digested
              </p>
            </div>
          </div>
            ...
            {categories.map((cat, idx) => (
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <div
                    className={`p-2 rounded-sm bg-scada-panel ${cat.color} border border-scada-border`}
                  >
                    {cat.icon}
                  </div>
                  <span className="text-2xl font-mono font-black text-scada-text">{cat.count}</span>
                </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex-1 bg-scada-bg border border-scada-border rounded-sm overflow-hidden max-h-[160px] overflow-y-auto custom-scrollbar">
              <div className="p-2 border-b border-scada-border bg-scada-panel flex items-center justify-between">
                <span className="text-[9px] font-black text-scada-muted uppercase tracking-widest">
                  Recent Source Verification
                </span>
                <span className="text-[8px] text-status-info font-mono uppercase">
                  Turbine_Friend Module
                </span>
              </div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {intelMap[(source.path || '').toLowerCase()] && (
                          <span className="text-[8px] px-1.5 py-0.5 rounded-sm bg-status-error/10 text-status-error font-mono uppercase border border-status-error/20">
                            {intelMap[(source.path || '').toLowerCase()].classification ||
                              'NOMINAL'}
                          </span>
                        )}
                      </div>
        ...
        <div className="px-6 pb-6 mt-2">
          <div className="p-3 rounded-sm bg-scada-bg border border-scada-border flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <span className="text-[10px] text-scada-text font-black uppercase tracking-widest block">
                  Validated Engineering Intelligence
                </span>
                <span className="text-[9px] text-scada-muted font-mono leading-tight">
                  100% of AI diagnostic logic is cross-referenced against the AnoHUB Dossier
                  Library.
                </span>
              </div>
            </div>
            ...
            >
              <span className="text-[9px] font-black text-status-info uppercase tracking-tighter">
                Enter Vault
              </span>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 pb-20 lg:pb-6 animate-in fade-in duration-200">
          <div onClick={() => setIsModalOpen(false)} className="absolute inset-0 bg-black/90" />
                ...
                {filteredSources.length > 0 ? (
                  ...
                  <div className="py-20 flex flex-col items-center justify-center text-scada-muted text-center">
                    ...
                    <p className="font-mono text-sm uppercase">
                      NO EVIDENCE MATCHING SCAN PARAMETERS
                    </p>
                  </div>
                )}
              </div>
