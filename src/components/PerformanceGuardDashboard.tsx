            ...
            <div className="flex justify-between items-start mb-4">
              ...
              <div
                className={`px-3 py-1 rounded text-xs font-black uppercase ${
                  zoneInfo.zone === 'BEST_EFFICIENCY'
                    ? 'bg-emerald-500 text-black'
                    : zoneInfo.zone === 'VORTEX_ROPE'
                      ? 'bg-amber-500 text-black'
                      : zoneInfo.zone === 'CAVITATION_RISK'
            ...
            <div className="flex-1 flex flex-col justify-center space-y-6">
              <div className="text-center">
                ...
                <div className="text-3xl font-black text-white">
                  {hammerStats.criticalTime.toFixed(2)}s
                </div>
              ...
              <div className="p-4 rounded bg-slate-900 border border-amber-500/30 relative overflow-hidden">
                ...
                <div className="text-xs text-amber-500 font-bold uppercase mb-1">
                  Safety Limit Enforced
                </div>
                <div className="text-2xl font-black text-white">
                  {hammerStats.minClosingTime.toFixed(1)}s
                </div>
                <div className="text-[10px] text-slate-400 mt-1">
                  System will physically prevent servo moving faster than this to avoid{' '}
                  {hammerStats.maxSurgePressure.toFixed(1)} bar surge.
                </div>
              </div>
            </div>
          </GlassCard>
        ...
        <div className="col-span-12 lg:col-span-3 space-y-4">
          <GlassCard className="p-4">
            <h3 className="text-xs font-bold text-slate-400 uppercase mb-3">
              Water-to-Wire Loss Audit
            </h3>
            <div className="space-y-3">
              <LossBar
                label="Trash Rack"
                value={losses.trashRackLossKW}
                total={opPoint.powerOutput * 1000}
                color="bg-blue-500"
              />
              <LossBar
                label="Penstock Friction"
                value={losses.penstockFrictionLossKW}
                total={opPoint.powerOutput * 1000}
                color="bg-cyan-500"
                warning={losses.penstockFrictionLossKW > 50}
              />
              <LossBar
                label="Generator Heat"
                value={losses.generatorElecLossKW}
                total={opPoint.powerOutput * 1000}
                color="bg-orange-500"
              />
            </div>
          ...
          <GlassCard className="p-4 bg-gradient-to-br from-emerald-950/40 to-slate-900 border-emerald-500/30">
            ...
            <div className="text-2xl font-black text-white mb-1">€ 142,500</div>
            <p className="text-[10px] text-slate-400 leading-tight">
              Estimated annual savings via optimized efficiency operating points and cavitation
              prevention.
            </p>
          </GlassCard>
        </div>
  ...
  return (
    <div>
      ...
      <div className="w-full h-1.5 bg-slate-800 rounded-full overflow-hidden">
        <div
          className={`h-full ${warning ? 'bg-red-500' : color}`}
