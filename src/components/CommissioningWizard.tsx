...
export const CommissioningWizard: React.FC<CommissioningWizardProps> = ({ asset, onComplete }) => {
  ...
  const allRequiredComplete = steps.filter(s => s.required).every(s => completedSteps.has(s.id));
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-1">
        ...
        />
        {required && !completed && <AlertCircle className="w-3 h-3 text-amber-400" />}
      </div>
      <p
        className={`text-xs font-bold ${
          active ? 'text-white' : completed ? 'text-emerald-300' : 'text-slate-400'
        }`}
      >
        {label}
      </p>
