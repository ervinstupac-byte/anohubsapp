            ...
            <div className="px-6 py-3 bg-indigo-950/30 border-b border-indigo-500/10 flex items-center gap-2 overflow-x-auto">
              ...
              {historySteps.map((step, idx) => (
                <React.Fragment key={step.id}>
                  <span
                    className={`text-[10px] font-mono whitespace-nowrap ${step.status === 'current' ? 'text-white font-bold' : 'text-slate-500'}`}
                  >
                    {step.label}
                  </span>
                  {idx < historySteps.length - 1 && (
              ...
              <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                  <p className="text-[10px] text-indigo-400 font-black uppercase tracking-widest mb-1">
                    Intuition Engine v4.2
                  </p>
                  <h3 className="text-white font-bold text-lg leading-tight">
                    Guided Field Diagnosis
                  </h3>
                </div>
              </div>
                  ...
                  >
                    <div className="flex justify-between items-center relative z-10">
                      <span className="text-sm font-bold uppercase tracking-wider flex items-center gap-2">
                        <span className="text-[10px] bg-white/10 text-slate-400 px-1.5 rounded">
                          {idx + 1}
                        </span>
                        {opt.label}
                      </span>
