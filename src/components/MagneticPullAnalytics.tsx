  ...
  return (
    ...
    >
      <div className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-black/40 p-3 rounded-lg border border-white/5">
            ...
            <p className="text-xl font-mono font-black text-amber-500">
              {I.toFixed(1)} <span className="text-[10px]">A</span>
            </p>
          </div>
          <div className="bg-black/40 p-3 rounded-lg border border-white/5">
            ...
            <p className="text-xl font-mono font-black text-cyan-400">
              {e.toFixed(2)} <span className="text-[10px]">mm</span>
            </p>
          </div>
        </div>
        ...
        <div className="space-y-2">
          <div className="flex justify-between items-end">
            <p className="text-[10px] font-black uppercase text-slate-400">
              Magnetic Side Pull Force
            </p>
            <p className="text-lg font-mono font-black text-white">
              {force.toFixed(2)} <span className="text-[10px] text-slate-500">kN</span>
            </p>
          </div>
        ...
        <div
          className={`p-4 rounded-xl border ${diagnosis.status === 'CRITICAL' ? 'bg-red-500/10 border-red-500/30' : diagnosis.status === 'WARNING' ? 'bg-amber-500/10 border-amber-500/30' : 'bg-emerald-500/10 border-emerald-500/30'}`}
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <p
                className={`text-[10px] font-black uppercase tracking-widest ${diagnosis.status === 'CRITICAL' ? 'text-red-500' : diagnosis.status === 'WARNING' ? 'text-amber-500' : 'text-emerald-500'}`}
              >
                {diagnosis.status === 'CRITICAL'
                  ? 'Critical Alert'
                  : diagnosis.status === 'WARNING'
              ...
              <p className="text-[10px] text-slate-300 italic leading-snug">{diagnosis.message}</p>
            </div>
          </div>
          {diagnosis.status === 'CRITICAL' && (
