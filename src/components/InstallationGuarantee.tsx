...
const AUDIT_STAGES = [
  {
    ...
    requirement: '< 0.05 mm/m (Verticality)',
  },
  { id: 'shaft', field: 'shaft', name: 'Main Shaft Run-Out', requirement: '< 0.02 mm (Static)' },
  {
    id: 'bearing_clearance',
    field: 'bearing_clearance',
    name: 'Guide Bearing Clearance',
    requirement: 'OEM Tolerance',
  },
  {
    id: 'wicket_synchronization',
    field: 'wicket_synchronization',
    name: 'Wicket Gate Sync',
    requirement: '< 1% Deviation',
  },
  {
    id: 'commissioning_vibration',
    field: 'commissioning_vibration',
    name: 'Vibration Baseline',
    requirement: '< 2.5 mm/s (Overall)',
  },
];
...
export const InstallationGuarantee: React.FC = () => {
  ...
  const [audit, setAudit] = useState<AuditState>({
    stageStatus: AUDIT_STAGES.reduce(
      (acc, stage) => ({
        ...acc,
  ...
  const handleStageUpdate = (stageId: string, key: 'value' | 'status', value: string) => {
    ...
    if (key === 'value') {
      ...
      if (value !== '' && stage?.field) {
        ...
        const result = EngineeringSchema.pick({ [stage.field]: true } as any).safeParse({
          [stage.field]: numValue,
        });

        if (!result.success || !precisionPass) {
          errorMsg = !precisionPass
            ? `PRECISION_ERROR: Value ${value} violates 0.05mm standard`
  ...
  const handleSaveAudit = async () => {
    ...
    try {
      ...
      showToast(
        t('installationGuarantee.toastSuccess', {
          status: t(`installationGuarantee.${overallStatus.toLowerCase()}`),
        }),
        overallStatus === 'PASSED' ? 'success' : 'warning'
      );
    } catch (error: any) {
  ...
  const { overallStatus } = useMemo(() => {
    ...
    const status = failures
      ? 'FAILED'
      : stageEntries.some(s => s.status === 'PASS')
        ? 'PASSED'
        : 'PENDING';
    ...
    <div className="animate-fade-in pb-12 max-w-6xl mx-auto space-y-8">
      ...
      <div className="text-center space-y-2 mb-8">
        <h2 className="text-4xl font-black text-white tracking-tighter uppercase">
          {t('installationGuarantee.title').split(' ')[0]}{' '}
          <span className="text-cyan-400">{t('installationGuarantee.title').split(' ')[1]}</span>
        </h2>
      ...
      <GlassCard className="p-0 border-t-4 border-t-cyan-500">
        ...
        <div className="p-6 border-b border-white/5 bg-slate-900/50 flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">
                {t('installationGuarantee.activeAudit')}
              </p>
              <p className="text-lg font-bold text-white">
                {selectedAsset?.name || t('installationGuarantee.selectAsset')}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4 bg-black/20 px-6 py-3 rounded-xl border border-white/5">
            <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">
              {t('installationGuarantee.liveStatus')}
            </span>
        ...
        <div className="p-6 md:p-8 space-y-4">
          <div className="flex items-center gap-2 mb-6">
            ...
            <h3 className="text-sm font-bold text-cyan-400 uppercase tracking-widest">
              {t('installationGuarantee.checkpoints')}
            </h3>
          </div>
          ...
          {AUDIT_STAGES.map((stage, index) => {
            ...
            return (
              <div
                key={stage.id}
                className={`grid grid-cols-1 md:grid-cols-12 gap-4 items-center p-4 rounded-xl border transition-all duration-300 ${
                  isAlarm
              ...
              >
                <div className="md:col-span-5">
                  <div className="flex items-center gap-3">
                    <span
                      className={`text-xs font-mono ${isAlarm ? 'text-red-400' : 'text-slate-600'}`}
                    >
                      0{index + 1}
                    </span>
                    <div>
                      <p
                        className={`text-sm font-bold ${isAlarm ? 'text-red-200' : 'text-slate-200'}`}
                      >
                        {t(`installationGuarantee.checks.${stage.id}`)}
                      </p>
                      <p
                        className={`text-[10px] font-mono mt-0.5 ${isAlarm ? 'text-red-400' : 'text-cyan-400/80'}`}
                      >
                        {t(`installationGuarantee.checks.${stage.id}Req`)}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="md:col-span-4 relative group">
                  <input
                    ...
                    className={`w-full bg-slate-900 border rounded-lg px-3 py-2 text-white text-sm outline-none transition-all placeholder-slate-600 ${
                      isAlarm
                ...
                <div className="md:col-span-3">
                  <select
                    ...
                    className={`w-full border rounded-lg px-3 py-2 text-xs font-bold uppercase tracking-wider cursor-pointer transition-colors outline-none appearance-none text-center ${
                      stageData.status === 'PASS'
                        ? 'bg-emerald-500/20 border-emerald-500/50 text-emerald-400'
                        : stageData.status === 'FAIL'
        ...
        <div className="p-6 md:p-8 bg-slate-900/30 border-t border-white/5">
          <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3">
            {t('installationGuarantee.remarksTitle')}
          </h3>
          <textarea
            rows={3}
          ...
          <div className="flex flex-col items-end gap-2 mt-6">
            {isRiskCritical && (
              <div className="flex items-center gap-2 text-red-500 text-[10px] font-bold animate-pulse uppercase tracking-wider mb-1">
                ...
                <span>
                  {t(
                    'installationGuarantee.riskBlock',
                    'Audit Locked: Critical Risks Detected in Diagnostics'
                  )}
                </span>
              </div>
            )}
