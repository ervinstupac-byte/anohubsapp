...
import {
  ...
  CommissioningState,
  MilestoneResult,
} from '../../lib/commissioning/WizardService';
...
export const CommissioningWizard: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  const [formData, setFormData] = useState<CommissioningState>({
    ...
    bearings: {
      clearanceTop: 0.15,
      clearanceBottom: 0.15,
      clearanceLeft: 0.15,
      clearanceRight: 0.15,
      asperityDetected: false,
    },
  ...
  const handleNext = async () => {
    ...
    if (result.success) {
      ...
      if (currentStep === 5) {
        // Transitioning TO Certificate (Step 6)
        try {
  ...
  const forceProceedHydraulic = () => {
    ...
    setValidationResult({
      success: true,
      message: 'Quality Violation Accepted.',
      werkmeisterTip: 'You have chosen compromise. It will be recorded.',
    });
      ...
      <div className="w-full max-w-4xl bg-slate-900 border border-slate-700 rounded-xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 border-b border-slate-700 flex justify-between items-center bg-slate-950">
          <div>
            ...
            <p className="text-xs text-slate-400 font-mono mt-1">
              PROTOCOL NC-150 // UNIT BIRTH CERTIFICATE
            </p>
          </div>
          <div className="flex items-center gap-1">
            {STEPS.map((step, idx) => (
              <div
                key={step.id}
                className={`h-1.5 w-8 rounded-full ${idx <= currentStep ? 'bg-cyan-500' : 'bg-slate-700'}`}
              />
            ))}
          </div>
        </div>
        ...
        <div className="flex-1 p-8 overflow-y-auto">
          {currentStep === 0 && (
            <div className="text-center space-y-6 py-12">
              ...
              <p className="text-slate-400 max-w-lg mx-auto">
                This wizard enforces the "Architect's Standard". You cannot proceed if tolerances
                are not met. The resulting data will form the <strong>Genetic Baseline</strong> for
                the RCA Engine.
              </p>
              <button
                onClick={() => setCurrentStep(1)}
                className="px-8 py-3 bg-cyan-600 hover:bg-cyan-500 text-white rounded font-bold uppercase tracking-wider transition-colors"
              >
                Initialize Protocol
              </button>
            </div>
                ...
                <input
                  ...
                  onChange={e =>
                    setFormData({
                      ...
                      alignment: {
                        ...formData.alignment,
                        plumbnessDeviation: parseFloat(e.target.value),
                      },
                    })
                  }
          ...
          {currentStep === 2 && (
            <div className="space-y-6">
              <h3 className="text-xl font-bold text-white mb-4 border-b border-slate-700 pb-2">
                2. Bearing Clearances (The Feeler Gauge Gate)
              </h3>
                    ...
                    <input
                      ...
                      onChange={e =>
                        setFormData({
                          ...
                          bearings: {
                            ...formData.bearings,
                            [`clearance${pos}`]: parseFloat(e.target.value),
                          },
                        })
                      }
              ...
              <div className="flex items-center gap-3 bg-slate-800 p-4 rounded border border-slate-600">
                <input
                  ...
                  onChange={e =>
                    setFormData({
                      ...formData,
                      bearings: { ...formData.bearings, asperityDetected: e.target.checked },
                    })
                  }
                  className="w-5 h-5 accent-red-500"
                />
                <span className="text-white font-bold">
                  Asperity/Roughness Detected on Babbitt?
                </span>
              </div>
          ...
          {currentStep === 3 && (
            <div className="space-y-6">
              ...
              <div className="bg-slate-800 p-6 rounded-lg border border-slate-600">
                ...
                <select
                  ...
                  onChange={e =>
                    setFormData({
                      ...formData,
                      metallurgy: { ...formData.metallurgy, runnerMaterial: e.target.value as any },
                    })
                  }
              ...
              {(formData.metallurgy.runnerMaterial === 'Cast Steel' ||
                formData.metallurgy.runnerMaterial === 'Bronze') && (
                <div className="bg-red-900/20 border border-red-500/50 p-4 rounded animate-in fade-in slide-in-from-top-4 duration-500">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <span className="text-red-200 font-bold">
                      Soft Material Detected. Protection Mandatory.
                    </span>
                  </div>
                  <div className="flex items-center gap-3">
                    <input
                      ...
                      onChange={e =>
                        setFormData({
                          ...
                          metallurgy: {
                            ...formData.metallurgy,
                            ceramicCoatingApplied: e.target.checked,
                          },
                        })
                      }
                    ...
                    <span className="text-white">
                      Confirmed Application of Ceramic Coating (&gt;300µm)
                    </span>
                  </div>
                </div>
              )}
                  ...
                  <input
                    ...
                    onChange={e =>
                      setFormData({
                        ...
                        electrical: {
                          ...formData.electrical,
                          insulationResistance: parseFloat(e.target.value),
                        },
                      })
                    }
                    ...
                    <input
                      ...
                      onChange={e =>
                        setFormData({
                          ...
                          electrical: {
                            ...formData.electrical,
                            stabilizationTimeSeconds: parseInt(e.target.value),
                          },
                        })
                      }
                      className="flex-1 accent-cyan-500"
                    />
                    <span className="font-mono text-2xl text-cyan-400 w-16 text-right">
                      {formData.electrical.stabilizationTimeSeconds}s
                    </span>
                  </div>
                </div>
              </div>
          ...
          {currentStep === 5 && (
            <div className="space-y-6">
              <h3 className="text-xl font-bold text-white mb-4 border-b border-slate-700 pb-2">
                5. Hydraulic Tightness (The Trap)
              </h3>

              <div className="bg-slate-800 p-6 rounded-lg border border-slate-600 space-y-4">
                <p className="text-sm text-slate-400 mb-2">
                  Measure the guide vane end-gaps at closed position.
                </p>
                    ...
                    <input
                      ...
                      onChange={e =>
                        setFormData({
                          ...formData,
                          hydraulic: {
                            guideVaneGapTopAvg: parseFloat(e.target.value),
                            guideVaneGapBottomAvg: formData.hydraulic?.guideVaneGapBottomAvg || 0,
                    ...
                    <input
                      ...
                      onChange={e =>
                        setFormData({
                          ...formData,
                          hydraulic: {
                            guideVaneGapBottomAvg: parseFloat(e.target.value),
                            guideVaneGapTopAvg: formData.hydraulic?.guideVaneGapTopAvg || 0,
                            qualityViolation: false,
                          },
                        })
                      }
            ...
            <div className="flex flex-col items-center gap-6">
              ...
              <div className="flex justify-center gap-4 mt-4 w-full print:hidden">
                ...
                <button
                  ...
                  className="px-6 py-3 bg-cyan-600 hover:bg-cyan-500 text-white rounded font-bold flex items-center gap-2"
                >
          ...
          {validationResult && !validationResult.success && currentStep !== 6 && (
            <motion.div
              ...
              className="mt-6 bg-red-900/20 border-l-4 border-red-500 p-4 animate-in fade-in slide-in-from-bottom-2"
            >
              <h4 className="text-red-400 font-bold flex items-center gap-2">
                <Lock className="w-4 h-4" /> Gatekeeper Alert: {validationResult.message}
              </h4>
              <p className="text-red-200 mt-1 uppercase text-xs tracking-wider">
                TOLERANCE EXCEEDED
              </p>
              <p className="text-red-300/70 text-sm mt-2 italic">
                "{validationResult.werkmeisterTip}"
              </p>
