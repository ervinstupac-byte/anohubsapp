  ...
  return (
    <div className="relative w-full max-w-3xl mx-auto aspect-[1.414] bg-[#050505] p-8 md:p-12 text-center text-amber-50 shadow-2xl border-[16px] border-[#101010] overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col items-center gap-4 mb-8">
        <div
          className={`p-4 rounded-full border-2 ${isPerfect ? 'bg-amber-900/20 border-amber-500 text-amber-500' : 'bg-red-900/20 border-red-500 text-red-500'}`}
        >
          {isPerfect ? <Award className="w-12 h-12" /> : <AlertOctagon className="w-12 h-12" />}
        </div>
        <h1 className="text-3xl font-serif tracking-widest text-amber-500 uppercase">
          Sovereign Systems
        </h1>
        <div className="h-px w-32 bg-amber-500/50" />
        <h2 className="text-xl font-light tracking-[0.2em] text-slate-300">
      ...
      <div className="relative z-10 grid grid-cols-2 gap-8 text-left max-w-lg mx-auto font-mono text-sm leading-relaxed mb-12">
        ...
        <div className="space-y-1">
          ...
          <p className="text-white">
            Deviation:{' '}
            <span className="text-amber-300">{data.alignment.plumbnessDeviation} mm/m</span>
          </p>
        ...
        <div className="space-y-1">
          ...
          <p className="text-white">
            ...
            <span className="text-amber-300">
              {((data.bearings.clearanceTop + data.bearings.clearanceBottom) / 2).toFixed(3)} mm
            </span>
          </p>
        ...
        <div className="space-y-1">
          ...
          <p className="text-white">
            Runner: <span className="text-amber-300">{data.metallurgy.runnerMaterial}</span>
          </p>
          {data.metallurgy.ceramicCoatingApplied && (
            <p className="text-cyan-400 text-[10px] flex items-center gap-1">
              <ShieldCheck className="w-3 h-3" /> CERAMIC ARMORED
            </p>
          )}
        </div>
        ...
        <div className="space-y-1">
          ...
          <p className="text-white">
            Symmetry Gap:{' '}
            <span className={isPerfect ? 'text-amber-300' : 'text-red-500 font-bold'}>
              {Math.abs(
                (data.hydraulic?.guideVaneGapTopAvg || 0) -
                  (data.hydraulic?.guideVaneGapBottomAvg || 0)
              ).toFixed(3)}{' '}
              mm
            </span>
          </p>
      ...
      <div className="relative z-10 grid grid-cols-2 gap-12 max-w-xl mx-auto mt-auto pt-8 border-t border-slate-800">
        <div className="text-left">
          ...
          <p
            className={`text-xl font-bold tracking-wider ${isPerfect ? 'text-cyan-400' : 'text-red-500'}`}
          >
