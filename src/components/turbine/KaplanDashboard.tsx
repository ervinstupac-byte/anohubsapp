...
import { Gauge, Droplets, Cog, AlertCircle, TrendingUp, Wind, Waves } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <WicketGateControl position={kaplanData.wicket_gate_position || 0} />
      </div>
...
const BladeAngleVisualizer: React.FC<BladeAngleVisualizerProps> = ({
  angle,
  setpoint,
  tolerance,
}) => {
  ...
  return (
    <GlassCard
      className={`border-l-4 ${isOutOfTolerance ? 'border-l-red-500' : 'border-l-emerald-500'}`}
    >
      ...
      >
        <p
          className={`text-xs font-bold ${isOutOfTolerance ? 'text-red-400' : 'text-emerald-400'}`}
        >
...
const HubPositionMonitor: React.FC<HubPositionMonitorProps> = ({ position, tolerance }) => {
  ...
  return (
    <GlassCard
      className={`border-l-4 ${isExcessive ? 'border-l-red-500' : 'border-l-emerald-500'}`}
    >
      <div className="flex items-center gap-3 mb-4">
      ...
      <div className="grid grid-cols-2 gap-2 text-center">
        ...
        <div>
          <p className="text-2xl font-black text-amber-400">
            {((tolerance?.value || 0.02) * 1000).toFixed(0)}
          </p>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="p-4 bg-blue-950/20 rounded-lg border border-blue-500/20">
          ...
          <p className="text-3xl font-black text-white">
            {submersionDepth.toFixed(1)} <span className="text-lg text-slate-500">m</span>
          </p>
        </div>
        ...
        >
          ...
          <p
            className={`text-3xl font-black ${sealCritical ? 'text-red-400' : 'text-emerald-400'}`}
          >
...
const HydraulicRunawayMonitor: React.FC<HydraulicRunawayMonitorProps> = ({
  ...
  const riskLevel =
    pressureHigh && tensionHigh ? 'CRITICAL' : pressureHigh || tensionHigh ? 'HIGH' : 'OK';
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-3 gap-4 mb-4">
        <div
          className={`p-3 rounded-lg ${pressureHigh ? 'bg-red-950/30 border-2 border-red-500' : 'bg-slate-800/30'}`}
        >
        ...
        <div
          className={`p-3 rounded-lg ${tensionHigh ? 'bg-amber-950/30 border-2 border-amber-500' : 'bg-slate-800/30'}`}
        >
        ...
        <div
          className={`p-3 rounded-lg ${pipeChanged ? 'bg-amber-950/30 border-2 border-amber-500 animate-pulse' : 'bg-slate-800/30'}`}
        >
      ...
      {(riskLevel === 'CRITICAL' || riskLevel === 'HIGH' || pipeChanged) && (
        ...
        >
          <p className="text-xs font-bold text-white mb-2">
            ...
            {pipeChanged
              ? 'PATTERN MATCH: 2024-KM-HC-001 Incident!'
    ...
    <GlassCard className="border-l-4 border-l-purple-500">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-xs text-slate-500">
          {variant.replace('kaplan_', '').toUpperCase()}
        </span>
      </div>
