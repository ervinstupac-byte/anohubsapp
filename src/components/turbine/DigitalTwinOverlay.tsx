      ...
      <div className="flex items-center justify-between mb-6 pb-4 border-b border-white/10">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">
            Real-time 3D deviation analysis • {asset.turbine_family.toUpperCase()}{' '}
            {turbineModel.variant}
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="aspect-[16/10] bg-gradient-to-br from-slate-950 to-slate-900 rounded-xl border border-cyan-500/20 relative overflow-hidden">
            ...
            <AnimatePresence>
              {anomalies.map((anomaly, index) => (
                ...
                >
                  <div
                    className={`w-4 h-4 rounded-full ${getSeverityColor(anomaly.severity)} animate-ping absolute`}
                  />
        ...
        <div className="space-y-4">
          ...
          <div className="p-4 bg-purple-950/20 rounded-lg border border-purple-500/20">
            ...
            <div className="space-y-2">
              {Object.entries(turbineModel.getTolerances())
                .slice(0, 4)
                .map(([key, threshold]) => (
      ...
      return (
        <svg width="300" height="300" viewBox="0 0 300 300">
          ...
          {[...Array(18)].map((_, i) => {
            ...
            return (
              <circle key={i} cx={x} cy={y} r="8" fill="none" stroke={color} strokeWidth="1.5" />
            );
          })}
...
const MeasurementTooltip: React.FC<MeasurementTooltipProps> = ({
  label,
  value,
  unit,
  status,
    ...
    <div className="flex items-start gap-2">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-[9px] text-slate-500 mt-1">
          {anomaly.currentValue.toFixed(2)} (Expected: {anomaly.expectedRange[0].toFixed(2)}-
