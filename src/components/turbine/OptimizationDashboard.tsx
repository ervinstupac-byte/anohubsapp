        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="text-2xl font-black uppercase tracking-tighter mb-1">
              ...
              <span className="text-purple-400 ml-2">Analysis</span>
            </h2>
        ...
        <div className="flex gap-3">
          {(['conservative', 'recommended', 'aggressive'] as const).map(scenario => (
            <motion.button
              ...
              className={`flex-1 p-4 rounded-none border-2 transition-all ${
            ...
            >
              <p
                className={`text-xs uppercase font-black tracking-wider mb-2 ${
                  selectedScenario === scenario ? 'text-purple-400' : 'text-slate-500'
                }`}
              >
                {scenario}
              </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <GlassCard className="border-l-4 border-l-red-500">
          ...
          <div className="space-y-4">
            ...
            <MetricRow
              label="Annual Output"
              value={`${((asIsEfficiency / 100) * asset.capacity * 8760 * 0.7).toFixed(0)} MWh`}
              status="warning"
            />
            <MetricRow
              label="Revenue Lost"
              value={`$${(((100 - asIsEfficiency) / 100) * annualRevenue).toLocaleString()}/yr`}
              status="critical"
            />
        ...
        <GlassCard className="border-l-4 border-l-emerald-500">
          ...
          <div className="space-y-4">
            <MetricRow
              label="Efficiency"
              value={`${toBeEfficiency[selectedScenario]}%`}
              status="success"
              improvement={`+${efficiencyGain.toFixed(1)}%`}
            />
            <MetricRow
              label="Annual Output"
              value={`${((toBeEfficiency[selectedScenario] / 100) * asset.capacity * 8760 * 0.7).toFixed(0)} MWh`}
              status="success"
            />
            <MetricRow
              label="Additional Revenue"
              value={`$${additionalRevenue.toLocaleString()}/yr`}
              status="success"
              improvement="NEW"
            />
