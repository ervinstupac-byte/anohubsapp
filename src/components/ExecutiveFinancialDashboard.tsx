...
export const ExecutiveFinancialDashboard: React.FC<ExecutiveDashboardProps> = ({
  assets: propAssets,
  selectedAsset: propSelected,
}) => {
  ...
  const net = FinancialImpactEngine.calculateNetProfit(
    energyRevenue,
    0,
    0,
    Number(impact?.maintenanceBufferEuro ?? 0)
  );
    ...
    <div className="space-y-6 bg-scada-bg p-6">
      ...
      <div className="mb-6 border-b border-scada-border pb-4">
        <h2 className="text-3xl font-black uppercase tracking-tighter mb-2 text-scada-text font-header">
          Executive
          <span className="text-status-ok ml-2">Dashboard</span>
        </h2>
      ...
      <div className="bg-scada-panel border border-scada-border rounded-sm shadow-scada-card p-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-center">
          <div>
            <p className="text-xs text-scada-muted uppercase font-bold font-mono mb-1">
              Net Profit (Hourly)
            </p>
            <p className="text-3xl font-black text-scada-text font-mono tabular-nums">
              €{Number(net?.netProfit ?? 0).toFixed(0)}
            </p>
          </div>
          <div>
            <p className="text-xs text-scada-muted uppercase font-bold font-mono mb-1">
              Molecular Debt (Buffer)
            </p>
            <p className="text-3xl font-black text-status-ok font-mono tabular-nums">
              €{Number(impact?.maintenanceBufferEuro ?? 0).toFixed(0)}
            </p>
          </div>
          <div className="text-right">
            ...
            <p className="text-2xl font-black text-scada-text font-mono tabular-nums">
              €{Number(energyRevenue).toFixed(0)}/h
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-scada-panel border border-scada-border rounded-sm shadow-scada-card p-6">
        ...
        <div className="mt-6 p-4 bg-status-ok/5 border border-status-ok/30 rounded-sm">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-status-ok font-bold uppercase mb-1 font-mono">
                Total ROI (12 months)
              </p>
...
const FinancialKPI: React.FC<FinancialKPIProps> = ({
  icon: Icon,
  label,
  value,
  change,
  ...
  <div className="p-4 bg-scada-panel border border-scada-border rounded-sm shadow-scada-card">
    <div className="flex items-center justify-between mb-2">
      ...
      <div
        className={`text-sm font-bold font-mono ${positive ? 'text-status-ok' : 'text-status-error'}`}
      >
        {positive ? '+' : ''}
        {change.toFixed(1)}%
      </div>
...
const ROICard: React.FC<{
  ...
  <div className="p-4 bg-scada-bg border border-scada-border rounded-sm">
    <div className="flex items-center justify-between mb-3">
      ...
      <span
        className={`px-2 py-1 rounded-sm text-xs font-bold font-mono ${
          status === 'COMPLETED'
    ...
    <div className="space-y-2 text-xs font-mono">
      <div className="flex justify-between">
        ...
        <span className="text-scada-text font-bold tabular-nums">
          ${(investment / 1000).toFixed(0)}k
        </span>
      </div>
      <div className="flex justify-between">
        <span className="text-scada-muted">Returns:</span>
        <span className="text-status-ok font-bold tabular-nums">
...
const SavingsBar: React.FC<{
  ...
  <div>
    <div className="flex items-center justify-between mb-1">
      ...
      <span className="text-sm font-bold text-scada-text font-mono tabular-nums">
        ${(amount / 1000).toFixed(0)}k
      </span>
    </div>
    <div className="w-full h-2 bg-scada-bg rounded-sm overflow-hidden">
      <div style={{ width: `${percentage}%` }} className={`h-full ${color}`} />
    </div>
  </div>
);
  ...
  return (
    <div className={`p-4 border rounded-sm ${severityColors[severity]}`}>
      <div className="flex items-start justify-between mb-2">
        ...
        <div className="text-right">
          <p className="text-lg font-black text-status-error font-mono tabular-nums">
            {financialImpact}
          </p>
