    ...
    <div className="p-4 max-w-md mx-auto bg-black min-h-screen text-white font-sans">
      ...
      <div className="bg-slate-900/80 p-4 rounded-xl border border-slate-700 mb-6 backdrop-blur-md sticky top-4 z-50 shadow-2xl">
        ...
        <div className="flex gap-2 text-xs">
          <span className="px-2 py-0.5 rounded bg-amber-950 text-amber-500 border border-amber-800">
            STBY: 92 Days
          </span>
          ...
          {checks
            ...
            .map(item => (
              <div
                ...
                className={`p-4 rounded-lg border-l-4 transition-all cursor-pointer ${acknowledged.has(item.id) ? 'bg-slate-900 border-emerald-500 opacity-60' : 'bg-red-950/30 border-red-500 hover:bg-red-900/40'}`}
              >
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    {item.value && (
                      <span className="text-[10px] font-mono bg-red-950 px-1.5 py-0.5 rounded text-red-400">
                        {item.value}
                      </span>
                    )}
                  </div>
                  <div
                    className={`w-5 h-5 rounded border flex items-center justify-center ${acknowledged.has(item.id) ? 'bg-emerald-500 border-emerald-500' : 'border-red-500'}`}
                  >
        ...
        <div className="space-y-3">
          ...
          {checks
            ...
            .map(item => (
              <div
                key={item.id}
                className="p-3 bg-slate-900/50 rounded border border-slate-700 flex justify-between items-center"
              >
                <div className="flex items-center gap-3">
                  <div
                    className={`w-8 h-8 rounded-full flex items-center justify-center ${item.category === 'LAB_MATERIAL' ? 'bg-purple-900/30 text-purple-400' : 'bg-cyan-900/30 text-cyan-400'}`}
                  >
                    {item.category === 'LAB_MATERIAL' ? (
