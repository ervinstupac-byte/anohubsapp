...
export const PrecisionAudit: React.FC = () => {
  ...
  const [auditState, setAuditState] = useState<'IDLE' | 'ANALYZING' | 'CITATION' | 'COMPLETE'>(
    initialSource ? 'ANALYZING' : 'IDLE'
  );
  ...
  const generateFindings = () => {
    const assetType =
      ...
      (selectedDossier?.title?.toUpperCase().includes('FRANCIS') ? 'FRANCIS' : 'GENERIC');
    ...
    const simulatedFindings = [
      {
        ...
        justification:
          selectedDossier?.justification ||
          'Structural integrity verified against NC-5.8 baseline nominal values.',
      },
      {
        id: 2,
        ...
        justification:
          'Frequencies remain within 1.1x line frequency bounds per ISO 10816-5 protocols.',
      },
      {
        id: 3,
        ...
        justification:
          'Alignment vectors verified; eccentricity remains within safe operational envelope (Zone A).',
      },
    ];
              ...
              <div className="mt-6 w-full space-y-3">
                ...
                {selectedDossier && (
                  <div className="p-3 bg-white/5 border border-cyan-500/30 rounded text-center">
                    <p className="text-[8px] text-cyan-500 uppercase font-black tracking-widest mb-1">
                      Manual Target Active
                    </p>
                    <p className="text-[10px] text-slate-300 font-bold truncate">
                      {selectedDossier.path
                        .split('/')
                        .pop()
                        ?.replace('.html', '')
                        .replace(/-/g, ' ')
                ...
                <GlassCard variant="deep" className="p-4 border-white/10">
                  ...
                  {filteredArchive.map((file, idx) => {
                    ...
                    return (
                      ...
                      >
                        <div className="text-[10px] font-bold text-slate-300 group-hover:text-cyan-400 truncate mb-0.5">
                          {displayTitle}
                        </div>
                  ...
                  <div className="mt-4 text-[8px] font-mono text-slate-600 text-center uppercase">
                    Showing {filteredArchive.length} of{' '}
                    {searchTerm ? 'results' : '50 IEC 60041 compliant sources'}
                  </div>
                </GlassCard>
              </motion.div>
            )}
          ...
          <AnimatePresence mode="sync">
            {auditState === 'IDLE' || auditState === 'ANALYZING' ? (
              ...
              >
                ...
                <h2 className="text-2xl font-black text-slate-500 uppercase">
                  Standing by for Ingestion
                </h2>
