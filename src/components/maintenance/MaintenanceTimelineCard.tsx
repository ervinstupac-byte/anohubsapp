    ...
    <GlassCard title="Predictive Maintenance Timeline">
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <p className="text-[10px] text-slate-500 uppercase font-mono tracking-widest mb-1">
              Estimated Remaining Life
            </p>
            <div className="flex items-baseline gap-2">
              <span
                className={`text-3xl font-black font-mono ${getStatusColor(prediction.urgency)}`}
              >
          ...
          <div className="text-right">
            <div
              className={`inline-flex items-center gap-2 px-3 py-1 rounded-full border border-white/10 bg-white/5 ${getStatusColor(prediction.urgency)}`}
            >
        ...
        <div className="relative pt-6 pb-2">
          ...
          >
            <div
              className={`w-3 h-3 rotate-45 border border-slate-900 ${getProgressBarColor(prediction.urgency)}`}
            />
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-2 pt-4 border-t border-white/5">
          <div className="p-2 bg-slate-900/50 rounded flex flex-col items-center">
            ...
            <span
              className={`text-xs font-mono font-bold ${mechanical.vibration > 4.5 ? 'text-amber-400' : 'text-cyan-400'}`}
            >
          ...
          <div className="p-2 bg-slate-900/50 rounded flex flex-col items-center">
            ...
            <span
              className={`text-xs font-mono font-bold ${mechanical.bearingTemp > 70 ? 'text-amber-400' : 'text-cyan-400'}`}
            >
        ...
        <div className="flex items-start gap-2 p-3 rounded bg-blue-500/5 border border-blue-500/10">
          ...
          <p className="text-[10px] text-blue-200/70 leading-relaxed">
            Reasoning: <span className="text-white font-bold">{prediction.reason}</span>.
            High-precision aging model active.
          </p>
        </div>
      </div>
    </GlassCard>
