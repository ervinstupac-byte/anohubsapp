...
export const ServiceChecklistUI: React.FC = () => {
  ...
  const commitMeasurementToHistory = () => {
    ...
    if (currentItem.measurementConfig) {
      const minValue =
        currentItem.measurementConfig.nominalValue - currentItem.measurementConfig.tolerance * 3;
      const maxValue =
        currentItem.measurementConfig.nominalValue + currentItem.measurementConfig.tolerance * 3;
    ...
    const isPrecision =
      currentItem.measurementConfig && currentItem.measurementConfig.tolerance < 0.1;
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white p-6">
      ...
      <div className="mb-8">
        <div className="flex justify-between text-sm mb-2">
          <span>
            {activeChecklist.progress.completedItems} / {activeChecklist.progress.totalItems} Items
          </span>
          <span>
            {Math.round(
              (activeChecklist.progress.completedItems / activeChecklist.progress.totalItems) * 100
            )}
            %
          </span>
        </div>
        <div className="h-2 bg-slate-800 rounded-full overflow-hidden">
          <motion.div
            ...
            initial={{ width: 0 }}
            animate={{
              width: `${(activeChecklist.progress.completedItems / activeChecklist.progress.totalItems) * 100}%`,
            }}
      ...
      <AnimatePresence mode="sync">
        ...
        >
          ...
          {currentItem.type === 'MEASUREMENT' && currentItem.measurementConfig && (
            <div className="space-y-4">
              ...
              {response && (
                <motion.div
                  ...
                  className={`p-4 rounded border ${
                    response.validationResult.severity === 'OK'
                ...
                >
                  ...
                  {response.validationResult.deviation !== undefined && (
                    <p className="text-xs mt-2">
                      Deviation: {response.validationResult.deviation.toFixed(3)}{' '}
                      {currentItem.measurementConfig.unit}
                    </p>
                  )}
                </motion.div>
              )}
              ...
              <div className="text-xs text-slate-500 p-3 bg-slate-950 rounded">
                <strong>Tolerance:</strong> ± {currentItem.measurementConfig.tolerance}{' '}
                {currentItem.measurementConfig.unit}
                <br />
                <strong>Range:</strong> {currentItem.measurementConfig.minValue} -{' '}
                {currentItem.measurementConfig.maxValue} {currentItem.measurementConfig.unit}
              </div>
            </div>
          )}
          ...
          {currentItem.type === 'BOOLEAN' && (
            <div className="flex gap-4">
              <button
                ...
                className={`flex-1 py-4 rounded font-bold transition-all ${
          ...
          <button
            ...
            className={`mt-4 w-full py-3 rounded flex items-center justify-center gap-2 transition-all ${
              isRecording
