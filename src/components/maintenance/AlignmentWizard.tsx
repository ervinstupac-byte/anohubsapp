...
export const AlignmentWizard: React.FC<AlignmentWizardProps> = ({
  ...
  const addRunoutPoint = async () => {
    ...
    await CommissioningService.recordAlignmentPoint(sessionId, currentAngle, runout, isHorizontal);
    ...
    <GlassCard>
      <div className="mb-6">
        <div className="flex justify-between items-start">
          <div>
            <h3 className="text-2xl font-black uppercase tracking-tighter mb-2">
              <span className="text-white">
                {MECHANICAL_TOLERANCES.ALIGNMENT_MM_M} mm Alignment
              </span>
          ...
          <div
            className={`px-4 py-2 rounded-lg border flex items-center gap-3 ${isRotating ? 'bg-red-500/20 border-red-500/50' : 'bg-emerald-500/20 border-emerald-500/50'}`}
          >
            {isRotating ? (
              <>
                ...
                <div>
                  ...
                  <div className="text-sm font-black text-white">
                    ROTATING ({currentRpm.toFixed(0)} RPM)
                  </div>
                </div>
              </>
            ) : (
      ...
      {isRotating ? (
        ...
        <div className="p-12 text-center bg-red-950/20 border border-red-500/30 rounded-xl">
          ...
          <h2 className="text-2xl font-black text-white uppercase mb-2">
            Alignment Procedure Locked
          </h2>
          <p className="text-slate-400 max-w-md mx-auto">
            The turbine must be completely stopped before commencing shaft alignment. Detection of{' '}
            <b>{currentRpm.toFixed(1)} RPM</b> prevents access to this tool.
          </p>
        </div>
      ) : (
        // WIZARD CONTENT
          ...
          <div>
            ...
            {finalAlignment !== null && (
              ...
              >
                ...
                >
                  Standard: {MECHANICAL_TOLERANCES.ALIGNMENT_MM_M} mm/m{' '}
                  {finalAlignment <= MECHANICAL_TOLERANCES.ALIGNMENT_MM_M ? '✓ PASS' : '✗ FAIL'}
                </p>
                {isHorizontal && (
            ...
            <div className="p-4 bg-slate-800/30 rounded-lg border border-slate-700/50">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  <Bluetooth
                    className={`w-5 h-5 ${isBluetoothConnected ? 'text-cyan-400' : 'text-slate-500'}`}
                  />
    ...
    <div className="p-4 bg-slate-900/50 rounded-lg border border-slate-700/50">
      ...
      <svg width="400" height="400" viewBox="0 0 400 400" className="mx-auto">
        ...
        {[0, 45, 90, 135, 180, 225, 270, 315].map(angle => {
          ...
          return (
            <g key={angle}>
              <line x1={centerX} y1={centerY} x2={x} y2={y} stroke="#475569" strokeWidth="1" />
              <text
        ...
        {points.map((point, i) => {
          ...
          return (
            <circle key={i} cx={x} cy={y} r="4" fill="#06b6d4" stroke="#0e7490" strokeWidth="2" />
          );
        })}
