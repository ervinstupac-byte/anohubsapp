    ...
    return (
        <div className="bg-slate-900/50 rounded-2xl border border-white/5 p-6 backdrop-blur-sm">
            ...
            <div className="relative space-y-8">
                {/* Vertical Line */}
                <div className="absolute left-[19px] top-4 bottom-4 w-[2px] bg-gradient-to-b from-brand-500/50 via-slate-700/30 to-transparent"></div>

                {events.map((event, index) => {
                    const isUrgent = event.urgency === 'CRITICAL' || event.urgency === 'HIGH';
                    const colorClass = isUrgent ? 'text-red-400' : 'text-brand-400';
                    const bgClass = isUrgent ? 'bg-red-500' : 'bg-brand-500';
                    ...
                    return (
                        <div key={event.id} className="relative flex gap-6 group">
                            ...
                            <div className={`
                                ...
                                ${isUrgent ? 'bg-red-500 shadow-[0_0_20px_rgba(239,68,68,0.4)]' : 'bg-slate-800 border-slate-700 group-hover:bg-brand-600'}
                            `}>
                            ...
                            <div className={`
                                ...
                                ${isUrgent
                                    ...
                                    : 'bg-slate-800/40 border-white/5 hover:border-brand-500/30 hover:bg-slate-700/40'}
                            `}>
