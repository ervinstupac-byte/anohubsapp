...
import {
  ...
  MaintenanceEvent,
} from '../../features/maintenance/hooks/useMaintenancePrediction';
        ...
        {events.map((event, index) => {
          ...
          return (
            <div key={event.id} className="relative flex gap-6 group">
              ...
              >
                <Wrench
                  className={`w-4 h-4 ${isUrgent ? 'text-white' : 'text-slate-400 group-hover:text-white'}`}
                />
              </div>
                ...
                <div className="flex justify-between items-start mb-2">
                  <div>
                    ...
                    <div className="flex items-center gap-2 text-xs font-mono text-slate-400">
                      <span className="bg-slate-950/50 px-2 py-0.5 rounded text-[10px] uppercase">
                        {event.reason}
                      </span>
                      <span>•</span>
                ...
                <div className="mt-3">
                  ...
                  <div className="w-full h-1.5 bg-slate-950 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${Math.max(5, 100 - (event.daysRemaining / (event.daysRemaining + 30)) * 100)}%`,
                      }} // Rough accumulation viz
                    ></div>
                  </div>
                </div>
              </div>
