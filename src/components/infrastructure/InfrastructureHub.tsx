...
const PlantMaster = React.lazy(() =>
  import('./PlantMaster.tsx').then(m => ({ default: m.PlantMaster }))
);
const BidEvaluator = React.lazy(() =>
  import('./BidEvaluator.tsx').then(m => ({ default: m.BidEvaluator }))
);
const HydrologyLab = React.lazy(() =>
  import('./HydrologyLab.tsx').then(m => ({ default: m.HydrologyLab }))
);
    ...
    <div className="p-8 space-y-8 max-w-7xl mx-auto min-h-screen">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-slate-500 font-mono text-xs tracking-[0.3em] uppercase">
          Genesis Configuration & Multi-Site Management
        </p>
      </div>
