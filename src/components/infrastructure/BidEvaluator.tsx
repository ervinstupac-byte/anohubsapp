...
export const BidEvaluator: React.FC = () => {
  ...
  const runAnalysis = () => {
    ...
    const evaluation = HydraulicIntegrity.validateBidEfficiency(
      selectedBid.promised_efficiency,
      type
    );
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <GlassCard title="Inbound Offer Data">
          <div className="space-y-6">
            <ModernInput
              ...
              onChange={(e: any) =>
                setSelectedBid({ ...selectedBid, manufacturer: e.target.value })
              }
            />
            <div className="grid grid-cols-2 gap-4">
              <ModernInput
                ...
                onChange={(e: any) =>
                  setSelectedBid({ ...selectedBid, proposed_turbine_type: e.target.value })
                }
              >
              ...
              <ModernInput
                ...
                onChange={(e: any) =>
                  setSelectedBid({
                    ...selectedBid,
                    promised_efficiency: parseFloat(e.target.value),
                  })
                }
              />
            </div>
            <ModernInput
              ...
              onChange={(e: any) =>
                setSelectedBid({ ...selectedBid, price_eur: parseFloat(e.target.value) })
              }
            />

            <div className="flex gap-4 pt-4">
              <ModernButton variant="secondary" fullWidth onClick={runAnalysis}>
                Analyze Physics
              </ModernButton>
              <ModernButton variant="primary" fullWidth onClick={saveBid} isLoading={isLoading}>
                Archive Bid
              </ModernButton>
            </div>
          </div>
        ...
        <div className="space-y-6">
          <GlassCard
            title="Physics Audit Results"
            subtitle="Verdict based on fluid dynamics constraints"
          >
            {selectedBid.eval_verdict ? (
              <div
                className={`p-6 rounded-2xl border ${getVerdictStyle(selectedBid.eval_verdict)} transition-all duration-500`}
              >
                <div className="flex items-center gap-4 mb-4">
                  {selectedBid.eval_verdict === 'PLAUSIBLE' && (
                    <CheckCircle2 className="w-10 h-10" />
                  )}
                  {selectedBid.eval_verdict === 'MARKETING_LIE' && (
                    <AlertTriangle className="w-10 h-10" />
                  )}
                  {selectedBid.eval_verdict === 'IMPOSSIBLE' && (
                    <ShieldAlert className="w-10 h-10" />
                  )}

                  <div>
                    <h4 className="text-2xl font-black uppercase tracking-tighter">
                      {selectedBid.eval_verdict.replace('_', ' ')}
                    </h4>
                  ...
                  <p>
                    {selectedBid.eval_verdict === 'PLAUSIBLE'
                      ...
                      : selectedBid.eval_verdict === 'MARKETING_LIE'
                        ...
                        : 'Physics Violation: No known turbine configuration can achieve this efficiency at this scale.'}
                  </p>
                </div>
              </div>
            ) : (
          ...
          <div className="space-y-4">
            ...
            {bids.slice(0, 3).map(bid => (
              <div
                key={bid.id}
                className="glass-panel p-4 flex justify-between items-center border-white/5"
              >
                <div>
                  <h5 className="text-white font-bold">{bid.manufacturer}</h5>
                  <p className="text-[10px] text-slate-500 font-mono">
                    {bid.proposed_turbine_type} @ {bid.promised_efficiency}%
                  </p>
                </div>
                <div
                  className={`text-[10px] font-black px-2 py-1 rounded border ${getVerdictStyle(bid.eval_verdict)}`}
                >
                  {bid.eval_verdict}
                </div>
              </div>
