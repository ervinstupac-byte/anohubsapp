      ...
      <div className="flex justify-between items-center bg-slate-900/40 p-6 rounded-3xl border border-white/5 backdrop-blur-xl">
        <div>
          <h2 className="text-3xl font-black text-white tracking-widest uppercase mb-1">
            Bolt <span className="text-cyan-400">Torque</span>
          </h2>
          <p className="text-slate-400 text-sm font-light italic">
            Precision Tension & Elongation Control.
          </p>
        </div>
            ...
            <div className="space-y-3 p-4 bg-white/5 rounded-2xl border border-white/5">
              ...
              <div className="flex justify-between text-xs pt-2 border-t border-white/5">
                ...
                <span className="text-white font-mono font-bold">
                  M{liveMechanical.boltSpecs.diameter || activePart.size}
                </span>
              </div>
              <div className="flex justify-between text-xs">
                <span className="text-slate-500">Material Grade</span>
                <span className="text-cyan-400 font-bold">
                  {liveMechanical.boltSpecs.grade || activePart.grade}
                </span>
              </div>
              <div className="flex justify-between text-xs">
                <span className="text-slate-500">Total Bolts</span>
                <span className="text-white">
                  {liveMechanical.boltSpecs.count || activePart.bolts}
                </span>
              </div>
            </div>
          </GlassCard>
                ...
                <div className="p-3 bg-emerald-950/20 border border-emerald-500/20 rounded-lg">
                  ...
                  <div className="flex justify-between items-end">
                    <span className="text-lg font-mono font-bold text-white">
                      {liveBoltLoad.toFixed(1)} <span className="text-xs text-slate-500">kN</span>
                    </span>
          ...
          <GlassCard title="Star Sequence Pattern">
            ...
            <div className="grid grid-cols-4 gap-2 mt-4">
              {starSteps.map((step, idx) => (
                <div
                  key={idx}
                  className="bg-slate-950 border border-white/10 rounded p-2 text-center"
                >
        ...
        <div className="space-y-6">
          <GlassCard title="Hydraulic Tensioning" className="border-t-4 border-t-amber-500">
            <p className="text-[10px] text-slate-400 mb-6 uppercase tracking-widest font-bold">
              Micrometer Verification (±0.05mm)
            </p>

            <div className="space-y-4">
              <div>
                <label className="text-[10px] text-slate-500 font-black uppercase mb-2 block">
                  Target Elongation (mm)
                </label>
                <div className="text-2xl font-mono font-black text-white">
                  {calculations.elongation}
                </div>
              </div>

              <div className="pt-4 border-t border-white/5">
                <label className="text-[10px] text-amber-500 font-black uppercase mb-2 block">
                  Measured Value (mm)
                </label>
                <input
                  type="number"
              ...
              <div
                className={`mt-6 p-4 rounded-2xl border ${calculations.isElongationOk ? 'bg-emerald-500/10 border-emerald-500/30' : 'bg-red-500/10 border-red-500/30'} flex items-center gap-4`}
              >
                <div
                  className={`w-10 h-10 rounded-full flex items-center justify-center text-xl ${calculations.isElongationOk ? 'bg-emerald-500 text-black' : 'bg-red-500 text-white'}`}
                >
                  {calculations.isElongationOk ? '✓' : '⚠'}
                </div>
                <div>
                  <div
                    className={`text-xs font-black uppercase ${calculations.isElongationOk ? 'text-emerald-400' : 'text-red-500'}`}
                  >
                    {calculations.isElongationOk ? 'Precision Verified' : 'Out of Tolerance'}
                  </div>
                  <p className="text-[10px] text-slate-500">
                    Required: {calculations.elongation} ± 0.05mm
                  </p>
                </div>
              </div>
          ...
          <GlassCard title="Ano-Agent Advice">
            <div className="flex gap-4">
              ...
              <p className="text-xs text-slate-400 leading-relaxed italic">
                "Always use a cross-bolt pattern for the turbine cover. Lubricate the threads with
                Molykote for accurate torque-to-tension conversion."
              </p>
            </div>
          </GlassCard>
        </div>
