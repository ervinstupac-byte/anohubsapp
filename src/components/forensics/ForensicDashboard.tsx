...
const VisionAnalyzer = React.lazy(() =>
  import('./VisionAnalyzer').then(m => ({ default: m.VisionAnalyzer }))
);
const AudioSpectrogram = React.lazy(() =>
  import('./AudioSpectrogram').then(m => ({ default: m.AudioSpectrogram }))
);
const PostMortemMonitor = React.lazy(() =>
  import('./PostMortemMonitor').then(m => ({ default: m.PostMortemMonitor }))
);
  ...
  return (
    <div className="relative h-48 w-full bg-slate-950/50 rounded-xl overflow-hidden border border-slate-800">
      ...
      <div className="absolute inset-0 grid grid-cols-6 grid-rows-4 opacity-10 pointer-events-none">
        {[...Array(24)].map((_, i) => (
          <div key={i} className="border-r border-b border-cyan-500/30"></div>
        ))}
      </div>
...
export const ForensicDashboard: React.FC = () => {
  ...
  return (
    <div
      className={`min-h-screen bg-[#050505] text-slate-200 p-8 font-sans transition-colors duration-1000 ${status === 'ATTACK_IN_PROGRESS' ? 'shadow-[inset_0_0_100px_rgba(220,38,38,0.2)]' : ''}`}
    >
      {/* CRITICAL OVERLAY */}
      <AnimatePresence>
      ...
      <header className="mb-8 border-b border-white/10 pb-6 flex justify-between items-end relative z-10">
        <div>
          <h1 className="text-3xl font-black text-white uppercase tracking-tighter mb-2 flex items-center gap-3">
            <div
              className={`p-2 rounded-none transition-colors ${status === 'ATTACK_IN_PROGRESS' ? 'bg-red-500 animate-pulse' : 'bg-[#2dd4bf] text-black'}`}
            >
              {status === 'ATTACK_IN_PROGRESS' ? (
        ...
        <div className="flex gap-4">
          ...
          <div
            className={`flex items-center gap-2 px-4 py-2 rounded-full border ${status === 'ATTACK_IN_PROGRESS' ? 'bg-red-950/50 border-red-500 text-red-400' : 'bg-slate-900 border-slate-700 text-slate-400'}`}
          >
          ...
          <GlassCard className="p-0 overflow-hidden border-t-4 border-t-cyan-500">
            ...
            <div className="p-6">
              ...
              <div className="flex justify-between mt-4 text-xs font-mono text-slate-500">
                <span>
                  INBOUND: {trafficHistory[trafficHistory.length - 1]?.inbound.toFixed(1)} Mbps
                </span>
            ...
            <div className="flex-grow overflow-y-auto p-4 space-y-2 custom-scrollbar">
              {securityEvents.length === 0 ? (
                ...
                securityEvents.map((event, i) => (
                  <div
                    key={i}
                    className="text-[10px] font-mono p-2 bg-slate-900/80 border-l-2 border-red-500 text-slate-300"
                  >
                    {event}
                  </div>
                ))
