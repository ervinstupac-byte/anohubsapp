    ...
    <AnimatePresence>
      ...
      >
        ...
        <div className="bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 border-2 border-amber-500/80 rounded-lg shadow-2xl shadow-amber-500/20 overflow-hidden">
          {/* Header */}
          <div
            className={`px-6 py-3 ${severity === 'CRITICAL' ? 'bg-red-900/60' : 'bg-yellow-800/60'} border-b border-amber-500/50`}
          >
          ...
          <div className="p-6 space-y-4">
            ...
            <div className="grid grid-cols-3 gap-4 bg-slate-800/50 p-3 rounded border border-slate-700">
              <div className="flex items-center gap-2">
                ...
                <span className="text-slate-400 text-xs">
                  {t('rcaMentor.liveMetrics.bearingTemp')}:
                </span>
                <span className="text-cyan-300 font-mono text-sm">
                  {state.mechanical.bearingTemp.toFixed(1)}°C
                </span>
              </div>
              <div className="flex items-center gap-2">
                <TrendingUp className="w-4 h-4 text-purple-400" />
                <span className="text-slate-400 text-xs">
                  {t('rcaMentor.liveMetrics.vibration')}:
                </span>
                <span className="text-purple-300 font-mono text-sm">
                  {state.mechanical.vibration.toFixed(2)} mm/s
                </span>
              </div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-yellow-300 font-mono text-sm">
                  {state.mechanical.rpm.toFixed(0)}
                </span>
              </div>
            </div>
            ...
            <div>
              <div className="flex items-center gap-2 mb-2">
                ...
                <h3 className="text-sky-300 font-bold text-sm tracking-wider">
                  {t('rcaMentor.physicsTitle')}
                </h3>
              </div>
            ...
            <div>
              <div className="flex items-center gap-2 mb-2">
                ...
                <h3 className="text-yellow-300 font-bold text-sm tracking-wider">
                  {t('rcaMentor.guidanceTitle')}
                </h3>
              </div>
              <div className="bg-amber-950/30 p-4 rounded border border-amber-700/50">
                <ol className="space-y-2">
                  {guidanceSteps.map((step, idx) => (
                    <li
                      key={idx}
                      className={`text-amber-50 text-sm ${step.includes('IMMEDIATE') || step.includes('ODMAH') ? 'font-bold text-red-300' : ''}`}
                    >
                      {step}
                    </li>
                  ))}
