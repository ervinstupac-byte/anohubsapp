...
export const KillSwitch: React.FC<KillSwitchProps> = ({ onEngage, isActive }) => {
  ...
  return (
    <div className="relative w-full max-w-sm mx-auto mt-8">
      <div
        className={`p-1 rounded-full border-2 transition-all duration-300 ${isActive ? 'bg-emerald-900/20 border-emerald-500' : 'bg-red-950/30 border-red-500/50'}`}
      >
        <motion.button
          whileTap={{ scale: 0.98 }}
          drag="x"
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          <span
            className={`text-sm font-bold uppercase tracking-widest ${isActive ? 'text-emerald-400' : 'text-red-400/50'}`}
          >
