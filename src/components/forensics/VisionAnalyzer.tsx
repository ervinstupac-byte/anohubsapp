    ...
    setTimeout(() => {
      if (type === 'PITTED') {
        setAnalysisResult({
          ...
          advice:
            'Critical: Surface micro-jets have removed material. Recommended: Stellite Welding overlay immediately.',
        });
      } else {
        setAnalysisResult({
          diagnosis: 'ABRASIVE EROSION',
          ...
          advice:
            'Warning: Sand particles polishing the surface. Recommended: Apply Ceramic Coating (Tungsten Carbide).',
        });
      }
        ...
        {!image ? (
          <div className="text-center space-y-4">
            ...
            <div className="flex gap-2 justify-center mt-4">
              <button
                onClick={() => {
                  setImage('pitted');
                  simulateAnalysis('PITTED');
                }}
                className="px-3 py-1 bg-slate-800 text-xs rounded border border-slate-700 hover:bg-slate-700"
              >
                Simulate: Pitted
              </button>
              <button
              ...
              <button
                onClick={() => simulateAnalysis('HIGH_EROSION')}
                className="px-3 py-1 bg-amber-900/30 text-amber-500 text-xs rounded border border-amber-900/50 hover:bg-amber-900/50"
              >
                Test: High Erosion Rate
              </button>
            </div>
          </div>
        ) : (
          <div className="w-full h-full flex flex-col items-center">
            <div className="w-full h-48 bg-slate-900 rounded-lg mb-4 flex items-center justify-center relative overflow-hidden">
              ...
              <div
                className={`w-32 h-32 rounded-full ${image === 'pitted' ? 'bg-gradient-to-br from-slate-700 to-black opacity-80 backdrop-filter backdrop-blur-sm border-4 border-slate-600 border-dashed' : 'bg-gradient-to-r from-blue-900 via-slate-800 to-blue-900 opacity-90'}`}
              ></div>
              <div className="absolute inset-0 flex items-center justify-center">
                {isAnalyzing && (
                  <span className="animate-pulse text-[#2dd4bf] font-mono font-bold">
                    SCANNING TEXTURE...
                  </span>
                )}
              </div>
            ...
            {analysisResult && !isAnalyzing && (
              ...
              >
                <div className="flex justify-between items-start mb-2">
                  <div>
                    <h4
                      className={`text-xl font-black uppercase ${analysisResult.diagnosis === 'CAVITATION' ? 'text-red-500' : 'text-amber-500'}`}
                    >
                  ...
                  {analysisResult.diagnosis === 'CAVITATION' ? (
                ...
                <div className="bg-[#2dd4bf]/10 p-3 rounded border border-[#2dd4bf]/20">
                  <p className="text-xs text-[#2dd4bf] font-bold uppercase mb-1">
                    Actionable Advice:
                  </p>
