    ...
    <div className="bg-slate-900 border border-slate-700 rounded-xl p-4 shadow-lg">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          <Shield
            className={`w-5 h-5 ${analysis.color === '#ef4444' ? 'text-red-500 animate-pulse' : 'text-emerald-400'}`}
          />
      ...
      <div className="grid grid-cols-2 gap-4 mb-4">
        ...
        <div className="p-3 bg-black/20 rounded border border-white/5">
          ...
          <div className="text-2xl font-mono font-bold text-white">
            {Math.min(...anodes.map(a => CorrosionCore.estimateAnodeLifeYears(a))).toFixed(1)}{' '}
            <span className="text-sm text-slate-500">yrs</span>
          </div>
          <div className="text-[9px] text-slate-400 mt-1">Limiting Component</div>
        </div>
      </div>
