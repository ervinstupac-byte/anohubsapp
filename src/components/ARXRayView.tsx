      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-xs text-slate-400">
            See inside the turbine - Real-time component status
          </p>
        </div>
        ...
        {selectedComponent && (
          ...
          >
            ...
            <ComponentDetails
              ...
              status={componentStatus[selectedComponent as keyof typeof componentStatus]}
            />
          </motion.div>
        )}
      </AnimatePresence>
...
const ComponentLabel: React.FC<ComponentLabelProps> = ({
  ...
  const StatusIcon =
    status === 'HEALTHY' ? CheckCircle : status === 'WARNING' ? AlertCircle : XCircle;
  const statusColor =
    status === 'HEALTHY'
      ? 'text-emerald-400'
      : status === 'WARNING'
  ...
  return (
    ...
    >
      <div
        className={`px-2 py-1 rounded-lg backdrop-blur-md border transition-all ${
          isSelected
