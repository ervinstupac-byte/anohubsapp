...
export const SealingIntegrity: React.FC = () => {
  ...
  const integrityStatus = useMemo(() => {
    ...
    return {
      isAlert,
      message: isAlert
        ? 'Odstupanje na glavnoj zaptivci vratila. Provjeriti protok rashladne vode kroz zaptivku.'
        : 'Shaft Seal integrity within nominal parameters.',
  ...
  return (
    <GlassCard
      title="Shaft Seal Integrity (Zaptivka Vratila)"
      className={
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              <div
                className={`w-2 h-2 rounded-full ${assetTele?.drainagePumpActive ? 'bg-emerald-500 animate-pulse' : 'bg-slate-700'}`}
              />
              <span className="text-xs font-bold text-white uppercase">
                {assetTele?.drainagePumpActive ? 'RUNNING' : 'IDLE'}
              </span>
            </div>
          </div>
          <div className="text-right">
            ...
            <p className="text-lg font-mono font-black text-white">
              {assetTele?.reservoirLevel.toFixed(1)}m
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 py-3 border-y border-white/5">
          <div>
            ...
            <p className="text-xl font-mono font-black text-white">
              {integrityStatus.current} <span className="text-[10px] text-slate-400">cyc/d</span>
            </p>
          </div>
          <div>
            <p className="text-[9px] text-slate-500 uppercase font-black">Baseline (Expected)</p>
            <p className="text-xl font-mono font-black text-slate-400">
              {integrityStatus.baseline} <span className="text-[10px] text-slate-500">cyc/d</span>
            </p>
          </div>
        </div>
            ...
            <div className="flex gap-3">
              ...
              <div>
                <p className="text-[10px] font-black text-red-500 uppercase tracking-widest mb-1">
                  DEVIATION DETECTED (+{integrityStatus.deviation}%)
                </p>
