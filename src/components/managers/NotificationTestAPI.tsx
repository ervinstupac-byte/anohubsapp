    ...
    try {
      ...
      (window as any).__TEST__.injectPeltonFault = () => {
        const allow =
          ...
          (window as any).__ANOHUBS_TEST_API === true;
        if (!allow) {
          console.warn(
            '[__TEST__] injectPeltonFault blocked: set window.__ANOHUBS_TEST_API = true and reload (or run in non-production).'
          );
          return null;
        }
          ...
          telemetry.updateTelemetry({
            ...
            diagnosis: {
              ...
              messages: [
                {
                  code: diag.code,
                  en: diag.params?.message ?? diag.code,
                  bs: diag.params?.message ?? diag.code,
                },
              ],
