  ...
  return (
    <div className="relative flex flex-col items-center justify-center py-8">
      <div className="w-48 h-48 rounded-full relative flex items-center justify-center bg-slate-900/50 shadow-inner border border-slate-700">
        {/* Background Track */}
        <svg
          className="absolute inset-0 transform -rotate-90 w-full h-full p-2"
          viewBox="0 0 100 100"
        >
          <circle
            cx="50"
        ...
        <div className="text-center z-10">
          <div className={`text-4xl font-black ${color} tracking-tighter drop-shadow-md mb-1`}>
            {text}
          </div>
          <div className="text-[10px] text-slate-500 uppercase tracking-[0.2em] font-bold">
            {t('common.riskLevel', 'Risk Level')}
          </div>
        </div>

        {/* Inner Glow */}
        <div
          className={`absolute inset-0 rounded-full bg-gradient-to-tr from-transparent via-transparent to-${color.split('-')[1]}-500/10 opacity-50`}
        ></div>
      </div>
    </div>
  );
...
export const QuestionnaireSummary: React.FC = () => {
  ...
  const getRiskLevel = (highCount: number, mediumCount: number) => {
    ...
    if (totalScore > 10)
      return { text: 'High Risk', color: 'text-red-400', level: 'High' as const };
    if (totalScore > 5)
      return { text: 'Medium Risk', color: 'text-amber-400', level: 'Medium' as const };
  ...
  const consultationText = useMemo(() => {
    if (riskIndicator.level === 'High') {
      return t(
        'consultation.high',
        'URGENT: Your asset shows signs of critical "Execution Gap". Immediate alignment verification (<0.05mm/m) and vibration spectral analysis (ISO 10816) are recommended to prevent catastrophic failure. Review MIV and Guide Vane protocols immediately.'
      );
    } else if (riskIndicator.level === 'Medium') {
      return t(
        'consultation.medium',
        'WARNING: Deviations detected. While immediate failure is unlikely, efficiency losses are accruing. Schedule a comprehensive "Ownership Maintenance" audit and verify sensor calibration within the next maintenance window.'
      );
    } else {
      return t(
        'consultation.low',
        'OPTIMAL: Asset is operating within the Standard of Excellence. Continue predictive maintenance logging and ensure annual "Digital Twin" calibration to maintain this status.'
      );
    }
    ...
    try {
      const payload = {
        asset_name: selectedAsset
          ? selectedAsset.name
          : `HPP-${operationalData.turbineType || 'Unspecified'}`,
    ...
    <div className="animate-fade-in pb-12 max-w-6xl mx-auto space-y-8">
      ...
      <div className="text-center space-y-4 animate-fade-in-up pt-6">
        <h2 className="text-4xl md:text-5xl font-black text-white tracking-tighter">
          {t('questionnaire.title', 'Execution Gap').split(' ')[0]}{' '}
          <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-500">
            {t('questionnaire.title', 'Execution Gap Analysis').split(' ').slice(1).join(' ')}
          </span>
        </h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
          <GlassCard className="border-t-4 border-t-cyan-500">
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2 text-center">
              {t('questionnaire.overallAssessment')}
            </h3>
            ...
            <div className="mt-4 pt-6 border-t border-slate-700/50 space-y-3">
              <div className="flex justify-between items-center p-3 bg-red-900/10 rounded-lg border border-red-500/10">
                <span className="text-xs font-bold text-red-400 uppercase tracking-wider">
                  {t('questionnaire.highPriorityAlerts')}
                </span>
          ...
          <GlassCard className="bg-cyan-900/10 border-cyan-500/20">
            <div className="flex items-center gap-3 mb-3">
              ...
              <h4 className="font-bold text-cyan-300 text-sm uppercase tracking-wide">
                {t('questionnaire.conceptTitle')}
              </h4>
            </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {analysis.highRisk.length > 0 && (
            <GlassCard
              className="border-l-4 border-l-red-500 animate-scale-in"
            ...
            >
              <div className="flex items-start gap-4 mb-6">
                ...
                <div>
                  <h3 className="text-lg font-bold text-white">
                    {t('questionnaire.criticalIndicators')}
                  </h3>
                  <p className="text-xs text-red-300 font-medium mt-1">
                    {t('questionnaire.criticalIndicatorsDesc')}
                  </p>
                </div>
              </div>
              <ul className="space-y-3">
                {analysis.highRisk.map(q => (
                  <li
                    key={q.id}
                    className="flex items-start gap-3 p-4 rounded-xl bg-red-950/30 border border-red-500/10 hover:border-red-500/30 transition-colors"
                  >
                    <span className="text-red-500 mt-0.5 text-lg">•</span>
                    <span className="text-sm text-slate-300 font-medium">
                      {t(`questions.${q.id}.text`, q.text)}
                    </span>
                  </li>
                ))}
              </ul>
          ...
          {analysis.mediumRisk.length > 0 && (
            <GlassCard
              className="border-l-4 border-l-amber-500 animate-scale-in"
            ...
            >
              <div className="flex items-start gap-4 mb-6">
                ...
                <div>
                  <h3 className="text-lg font-bold text-white">
                    {t('questionnaire.operationalWarnings')}
                  </h3>
                  <p className="text-xs text-amber-200/70 font-medium mt-1">
                    {t('questionnaire.operationalWarningsDesc')}
                  </p>
                </div>
              </div>
              <ul className="space-y-3">
                {analysis.mediumRisk.map(q => (
                  <li
                    key={q.id}
                    className="flex items-start gap-3 p-4 rounded-xl bg-amber-950/20 border border-amber-500/10 hover:border-amber-500/30 transition-colors"
                  >
          ...
          {analysis.highRisk.length === 0 && analysis.mediumRisk.length === 0 && (
            <GlassCard className="border-emerald-500/30 text-center py-12">
              ...
              <h3 className="text-2xl font-bold text-white mb-2">
                {t('questionnaire.systemOptimal')}
              </h3>
              <p className="text-slate-400 max-w-md mx-auto">
                {t('questionnaire.systemOptimalDesc')}
              </p>
            </GlassCard>
          )}
