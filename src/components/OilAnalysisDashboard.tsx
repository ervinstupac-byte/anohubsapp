...
export const OilAnalysisDashboard: React.FC = () => {
  ...
  const babbittTotal =
    currentSample.metalContent.tin +
    currentSample.metalContent.lead +
    currentSample.metalContent.copper;
    ...
    <div className="space-y-6">
      ...
      <GlassCard className="p-6">
        ...
        <div className="space-y-3">
          <MetalBar
            label="Iron (Fe)"
            value={currentSample.metalContent.iron}
            max={150}
            critical={100}
            color="slate"
          />
          <MetalBar
            label="Babbitt Total (Sn+Pb+Cu)"
            value={babbittTotal}
            max={100}
            critical={50}
            color="amber"
          ...
          <MetalBar
            label="Copper (Cu)"
            value={currentSample.metalContent.copper}
            max={50}
            critical={30}
            color="orange"
          />
          <MetalBar
            label="Lead (Pb)"
            value={currentSample.metalContent.lead}
            max={50}
            critical={30}
            color="red"
          />
          <MetalBar
            label="Tin (Sn)"
            value={currentSample.metalContent.tin}
            max={50}
            critical={30}
            color="yellow"
          />
...
const ChemicalParameter: React.FC<{
  ...
  const isDanger =
    (dangerIfAbove && value > dangerIfAbove) || (dangerIfBelow && value < dangerIfBelow);
...
const FindingCard: React.FC<{ finding: any }> = ({ finding }) => {
  ...
  return (
    <div
      className={`p-4 rounded-lg border ${severityColors[finding.severity as keyof typeof severityColors]}`}
    >
      <div className="flex items-start gap-2 mb-2">
        <span className="text-xs font-bold px-2 py-1 rounded bg-black/30">{finding.category}</span>
