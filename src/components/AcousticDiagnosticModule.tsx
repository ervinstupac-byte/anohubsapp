    ...
    if (now - lastWarningTime.current > 15000) {
      ...
      if (metrics.cavitationIntensity > 7) {
        const msg = new SpeechSynthesisUtterance(
          'Detektovana intenzivna kavitacija. Provjerite pritisak usisne cijevi ili smanjite otvor lopatica.'
        );
        msg.lang = 'bs-BA';
        ...
        const msg = new SpeechSynthesisUtterance(
          'Detektovano ultrazvučno šištanje. Moguće mikro-curenje ulja ili zraka. Provjerite spojeve crijeva.'
        );
        msg.lang = 'bs-BA';
        window.speechSynthesis.speak(msg);
        lastWarningTime.current = now;
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      for (let i = 0; i < barCount; i++) {
        ...
        if (i >= 40 && metrics.ultrasonicLeakIndex > 5)
          color = 'rgba(34, 211, 238, 0.9)'; // Cyan (Ultrasonic)
        else if (i > 25 && i < 35 && metrics.cavitationIntensity > 5)
          color = 'rgba(239, 68, 68, 0.8)'; // Red (Cavitation)
        else if (i > 10 && i < 20 && metrics.bearingGrindIndex > 5)
          color = 'rgba(245, 158, 11, 0.8)'; // Amber (Grind)
      ...
      <div className="space-y-4">
        ...
        <div className="relative h-32 bg-black/60 rounded-xl border border-white/5 p-2 overflow-hidden">
          ...
          <div className="absolute top-2 left-2 flex gap-2">
            <span className="text-[8px] font-black bg-cyan-500/20 text-cyan-400 px-1 border border-cyan-500/30 rounded uppercase">
              Acoustic Logic v4.1
            </span>
            <span className="text-[8px] font-black bg-slate-500/20 text-slate-400 px-1 border border-slate-500/30 rounded uppercase tracking-widest">
              Ext. Mic: Connected
            </span>
          </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div
            className={`p-3 rounded-xl border transition-all ${metrics.cavitationIntensity > 7 ? 'bg-red-500/10 border-red-500/30 shadow-[0_0_15px_rgba(239,68,68,0.1)]' : 'bg-black/40 border-white/5'}`}
          >
            ...
            <p
              className={`text-2xl font-mono font-black ${metrics.cavitationIntensity > 7 ? 'text-red-500' : metrics.cavitationIntensity > 4 ? 'text-amber-500' : 'text-emerald-500'}`}
            >
              {metrics.cavitationIntensity.toFixed(1)}
            </p>
            <div className="w-full h-1 bg-white/5 rounded-full mt-2 overflow-hidden">
              <div
                className={`h-full transition-all duration-300 ${metrics.cavitationIntensity > 7 ? 'bg-red-500' : 'bg-emerald-500'}`}
                style={{ width: `${metrics.cavitationIntensity * 10}%` }}
              />
            </div>
          </div>

          <div
            className={`p-3 rounded-xl border transition-all ${metrics.ultrasonicLeakIndex > 7 ? 'bg-cyan-500/10 border-cyan-500/30' : 'bg-black/40 border-white/5'}`}
          >
            <div className="flex justify-between items-start mb-2">
            ...
            <p
              className={`text-2xl font-mono font-black ${metrics.ultrasonicLeakIndex > 7 ? 'text-cyan-400' : 'text-white'}`}
            >
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-black/40 p-3 rounded-xl border border-white/5">
            ...
            <p
              className={`text-lg font-mono font-black ${metrics.acousticBaselineMatch > 0.9 ? 'text-emerald-400' : 'text-amber-400'}`}
            >
          ...
          <div className="bg-black/40 p-3 rounded-xl border border-white/5">
            ...
            <p
              className={`text-lg font-mono font-black ${metrics.bearingGrindIndex > 7 ? 'text-red-500' : 'text-white'}`}
            >
        ...
        {(metrics.cavitationIntensity > 7 || metrics.ultrasonicLeakIndex > 7) && (
          <div
            className={`flex items-center p-3 rounded-xl gap-3 border ${metrics.ultrasonicLeakIndex > 7 ? 'bg-cyan-500/20 border-cyan-500/40' : 'bg-red-500/20 border-red-500/40'}`}
          >
            <span className="text-xl">{metrics.ultrasonicLeakIndex > 7 ? '🚨' : '🎙️'}</span>
            <p
              className={`text-[9px] font-bold leading-tight uppercase ${metrics.ultrasonicLeakIndex > 7 ? 'text-cyan-200' : 'text-red-200'}`}
            >
              Ano-Agent:{' '}
              {metrics.ultrasonicLeakIndex > 7
                ? 'Detektovano ultrazvučno šištanje. Moguće mikro-curenje. Provjerite spojeve.'
                : 'Detektovana intenzivna kavitacija. Provjerite pritiske.'}
            </p>
          </div>
        )}
      </div>
