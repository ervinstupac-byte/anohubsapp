    ...
    <div className="fixed bottom-4 right-4 z-50">
      <AnimatePresence>
        {isActive && (
          ...
          >
            <GlassCard
              className={`p-4 w-80 border-l-4 shadow-2xl backdrop-blur-xl ${alarmTriggered ? 'border-red-500 bg-red-950/80' : 'border-emerald-500 bg-slate-900/80'}`}
            >
              <div className="flex justify-between items-start mb-2">
                <h4
                  className={`text-xs font-black uppercase flex items-center gap-2 ${alarmTriggered ? 'text-red-400 animate-pulse' : 'text-emerald-400'}`}
                >
                  {alarmTriggered ? (
