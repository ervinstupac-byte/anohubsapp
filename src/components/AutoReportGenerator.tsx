  ...
  const handleGenerate = async () => {
    ...
    ForensicReportService.openAndDownloadBlob(
      ...
      true,
      {
        ...
        reportType: 'SERVICE_AUDIT',
        metadata: { serviceType, engineerName },
      }
    );
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="text-center mb-6">
        ...
        <p className="text-sm text-slate-400">AI-powered PDF izvještaj za {selectedAsset.name}</p>
      </div>
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-slate-700">
                <th className="text-left py-2 px-2 text-slate-400 font-bold uppercase">
                  Parametar
                </th>
                ...
                <tr key={index} className="border-b border-slate-800">
                  ...
                  <td className="py-2 px-2">
                    <input
                      ...
                      value={m.asFound}
                      onChange={e =>
                        updateMeasurement(index, 'asFound', parseFloat(e.target.value))
                      }
                  ...
                  <td className="py-2 px-2">
                    <input
                      ...
                      value={m.standard}
                      onChange={e =>
                        updateMeasurement(index, 'standard', parseFloat(e.target.value))
                      }
                      className="w-20 px-2 py-1 bg-slate-900/50 border border-slate-700 rounded text-white text-xs"
                    />
                  </td>
                  <td className="py-2 px-2">
                    ...
                    >
                      {m.improvement > 0 ? '+' : ''}
                      {m.improvement.toFixed(1)}%{m.improvement < 0 && ' ✓'}
                    </span>
                  </td>
                </tr>
              ))}
      ...
      <GlassCard className="p-6 bg-gradient-to-br from-purple-950/30 to-pink-950/30 border-purple-500/30">
        ...
        <ul className="space-y-2 text-sm text-slate-300">
          <li className="flex items-start gap-2">
            ...
            <span>
              <strong className="text-purple-400">Akustična analiza:</strong> "Kavitacija smanjena
              za 15% nakon optimizacije kuta lopatica"
            </span>
          </li>
          <li className="flex items-start gap-2">
            <CheckCircle className="w-4 h-4 text-emerald-400 mt-0.5" />
            <span>
              <strong className="text-purple-400">Vibracije:</strong> Procjena produženja životnog
              vijeka ležajeva
            </span>
          </li>
          <li className="flex items-start gap-2">
            <CheckCircle className="w-4 h-4 text-emerald-400 mt-0.5" />
            <span>
              <strong className="text-purple-400">12-mjesečne preporuke:</strong> Prilagođeno tipu
          ...
          <li className="flex items-start gap-2">
            <CheckCircle className="w-4 h-4 text-emerald-400 mt-0.5" />
            <span>
              <strong className="text-purple-400">Posljedice ignorisanja:</strong> Procjena rizika i
              troškova zastoja
            </span>
          </li>
        </ul>
