...
export const MissionControl: React.FC = () => {
  ...
  const getNodeClass = (nodeId: number) => {
    ...
    if (step === nodeId)
      return 'border-cyan-500 bg-cyan-950/10 shadow-none opacity-100 translate-x-2';
          ...
          <div className="flex items-center gap-4 text-center md:text-left">
            ...
            <div>
              <div className="flex items-center justify-center md:justify-start gap-2 mb-1">
                <span className="px-2 py-0.5 rounded-none bg-cyan-950 text-cyan-500 text-[10px] font-black border border-cyan-900/50 uppercase tracking-widest">
                  SOP-MC-001
                </span>
          ...
          <div className="flex items-center gap-6">
            <div className="text-right">
              <div className="text-[9px] text-slate-500 font-black uppercase tracking-widest mb-1">
                {t('francis.missionControl.systemStatus')}
              </div>
              <div
                className={`text-xl font-black tracking-tighter uppercase tabular-nums ${step === 5 ? 'text-emerald-500' : 'text-amber-500 animate-pulse'}`}
              >
                {step === 5
                  ? t('francis.missionControl.unitReady')
                  : t('francis.missionControl.sequenceHold')}
              </div>
            </div>
            <button
      ...
      <main className="max-w-6xl mx-auto px-4 md:px-8">
        ...
        <div className="mb-12 p-8 bg-slate-900/60 rounded-none border border-white/5 backdrop-blur-md relative overflow-hidden group">
          <div className="flex justify-between items-center border-b border-white/5 pb-4 mb-6">
            ...
            <div className="px-3 py-1 bg-purple-600 text-white text-[9px] font-black rounded-none shadow-none uppercase tracking-widest">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {(['n2', 'sync', 'dp', 'angle'] as const).map(f => (
              <button
                ...
                className={`p-4 rounded-none border transition-all duration-300 group/fault ${
                  faults[f]
              ...
              >
                ...
                <div
                  className={`text-[11px] font-black uppercase ${faults[f] ? 'text-red-500' : 'text-slate-400'}`}
                >
                  {f === 'n2'
                    ? 'Low N2 Pres'
                    : f === 'sync'
        ...
        <div className="relative pl-12 md:pl-20 py-8">
          ...
          {[1, 2, 3, 4].map(nodeId => {
            ...
            const isBlocked =
              (nodeId === 1 && faults.n2) ||
              (nodeId === 2 && faults.sync) ||
              (nodeId === 3 && faults.dp) ||
              (nodeId === 4 && faults.angle);

            return (
              <div
                key={nodeId}
                className={`relative mb-16 p-10 rounded-none border transition-all duration-700 backdrop-blur-md group/node ${getNodeClass(nodeId)}`}
              >
                {/* Node Bullet */}
                <div
                  className={`absolute -left-[3.35rem] md:-left-[4.35rem] top-12 w-8 h-8 rounded-none border-4 transition-all duration-700 z-10 flex items-center justify-center font-black text-[10px] ${getCircleClass(nodeId)} ${step >= nodeId ? 'text-white' : 'text-slate-700'}`}
                >
                  {nodeId}
                </div>
                ...
                <div className="flex flex-col md:flex-row justify-between items-start gap-8 relative z-10">
                  <div className="flex-1 space-y-4">
                    <div className="flex items-center gap-4">
                      <Icon
                        className={`w-10 h-10 ${step >= nodeId ? 'text-cyan-400' : 'text-slate-700'}`}
                      />
                      <div>
                        <h3
                          className={`text-2xl font-black uppercase tracking-tighter ${step >= nodeId ? 'text-white shadow-none' : 'text-slate-700'}`}
                        >
                          {t(`francis.missionControl.${nodeKey}.title`)}
                        </h3>
                        <p
                          className={`text-sm font-bold italic transition-colors leading-relaxed uppercase tracking-tighter ${step >= nodeId ? 'text-slate-400' : 'text-slate-800'}`}
                        >
                    ...
                    {/* Dynamic Data Module */}
                    <div
                      className={`p-6 rounded-none border transition-all duration-500 ${step >= nodeId ? 'bg-black/40 border-white/5' : 'bg-transparent border-transparent opacity-20'}`}
                    >
                      {nodeId === 1 && (
                        <div className="grid grid-cols-2 gap-8">
                          <div>
                            <span className="text-[10px] text-slate-500 uppercase font-black block mb-2 tracking-widest">
                              Oil Component Matrix
                            </span>
                            <div className="text-xl font-black text-white font-mono italic">
                              14.2 <span className="text-xs opacity-40 lowercase italic">MPa</span>
                            </div>
                          </div>
                          <div>
                            <span className="text-[10px] text-slate-500 uppercase font-black block mb-2 tracking-widest">
                              N2 Pre-Charge Intelligence
                            </span>
                            <div
                              className={`text-xl font-black font-mono italic ${faults.n2 ? 'text-red-500 animate-pulse' : 'text-emerald-500'}`}
                            >
                      ...
                      {nodeId === 2 && (
                        <div className="grid grid-cols-2 gap-8">
                          <div>
                            <span className="text-[10px] text-slate-500 uppercase font-black block mb-2 tracking-widest">
                              Vane Sync Delta (1-20)
                            </span>
                            <div
                              className={`text-xl font-black font-mono italic ${faults.sync ? 'text-red-500 animate-pulse' : 'text-emerald-500'}`}
                            >
                          ...
                          <div>
                            ...
                            <div className="text-xl font-black text-emerald-500 font-mono italic uppercase tracking-tighter">
                              Active Stability
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="space-y-6">
                          <div className="flex justify-between items-center mb-2">
                            <div className="flex flex-col">
                              <span className="text-[9px] text-slate-500 uppercase font-black tracking-widest italic">
                                Upstream Potential
                              </span>
                            ...
                            <div className="flex flex-col text-right">
                              <span className="text-[9px] text-slate-500 uppercase font-black tracking-widest italic">
                                Spiral Velocity Head
                              </span>
                              <span
                                className={`text-lg font-black font-mono ${faults.dp && forensicPressure >= 11 ? 'text-red-500 animate-pulse' : forensicPressure >= 14.5 ? 'text-emerald-500' : 'text-white'}`}
                              >
                          ...
                          <div className="w-full h-2.5 bg-slate-900 rounded-none border border-white/5 overflow-hidden shadow-none">
                            <div
                              className={`h-full transition-all duration-300 ${faults.dp && forensicPressure >= 11 ? 'bg-red-500' : 'bg-cyan-500'} shadow-none`}
                              style={{ width: `${(forensicPressure / 14.5) * 100}%` }}
                            />
                          </div>
                        </div>
                      )}
                      {nodeId === 4 && (
                        <div className="grid grid-cols-2 gap-8">
                          ...
                          <div>
                            <span className="text-[10px] text-slate-500 uppercase font-black block mb-2 tracking-widest">
                              Differential Target Angle
                            </span>
                            <div
                              className={`text-xl font-black font-mono italic ${faults.angle ? 'text-red-500 animate-pulse' : 'text-white'}`}
                            >
                    ...
                    {isBlocked && step === nodeId && (
                      <div className="p-6 bg-red-950/20 border-2 border-red-500/50 rounded-none flex items-center gap-6 animate-[shake_0.5s_infinite]">
                        ...
                        <div>
                          <h4 className="text-red-500 font-black text-[10px] uppercase tracking-widest italic mb-1">
                            Safety Critical Blockage
                          </h4>
                          <p className="text-xs text-red-200 font-bold uppercase tracking-tight">
                            {nodeId === 1
                              ? 'N2 PRESSURE CRITICAL BREACH - MANUAL AUDIT REQUIRED'
                              : nodeId === 2
                                ? 'VANE 4 DEVIATION DETECTED (+2.1mm) - CHECK RING BOLTS'
                                : nodeId === 3
                                  ? 'DELTA P > 3 BAR - CAVITATION RISK IN SPIRAL CASE'
                    ...
                    {step > nodeId ? (
                      ...
                      nodeId === 3 && !isFilling && forensicPressure < 14.5 ? (
                        ...
                        <button
                          ...
                          className={`w-full py-5 rounded-none text-sm font-black uppercase tracking-[0.2em] transition-all duration-300 italic flex items-center justify-center gap-4 group/btn ${
                            isBlocked
                        ...
                        >
                          ...
                          {nodeId === 4
                            ? t('francis.missionControl.actions.openMiv')
                            : nodeId === 3
                              ? t('francis.missionControl.actions.fillingComplete')
                              : nodeId === 2
                                ? t('francis.missionControl.actions.confirmZero')
                                : t('francis.missionControl.actions.confirm')}
                        </button>
                      )
                    ) : (
        ...
        <div className="mt-12 bg-red-950/20 border-l-[16px] border-red-600 p-12 rounded-none border border-red-900/20 relative group overflow-hidden">
          ...
          <div className="flex items-center gap-8 mb-10 relative z-10">
            ...
            <div>
              <h2 className="text-3xl font-black text-white uppercase tracking-tighter mb-1 italic">
                {t('francis.missionControl.esdTitle')}
              </h2>
              <div className="px-4 py-1 bg-red-900/40 text-red-500 text-[10px] font-black uppercase tracking-widest rounded-none border border-red-900/50 inline-block italic">
                Emergency Shutdown Protocol Active
              </div>
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
            <div className="flex items-center gap-5 p-6 bg-black/40 rounded-none border border-red-500/20 group/stat">
              ...
              <span className="text-sm font-black text-red-100 uppercase tracking-tighter italic">
                Gravity Weights: <span className="text-emerald-500">ARMED & NOMINAL</span>
              </span>
            </div>
            <div className="flex items-center gap-5 p-6 bg-black/40 rounded-none border border-red-500/20 group/stat">
              <div className="w-4 h-4 rounded-none bg-emerald-500 shadow-none group-hover:scale-125 transition-transform" />
              <span className="text-sm font-black text-red-100 uppercase tracking-tighter italic">
                Hydraulic Integrity: <span className="text-emerald-500">NOMINAL FLUX</span>
              </span>
            </div>
          </div>
        </div>
