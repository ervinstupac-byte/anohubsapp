  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 font-sans pb-12">
        ...
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="flex items-center gap-4 text-center md:text-left">
            <div className="p-4 bg-blue-600 rounded-none border border-white/10 shadow-none relative group overflow-hidden">
              <div
                className="absolute inset-x-0 bottom-0 bg-blue-400/20 animate-[pulse_2s_infinite]"
      ...
      <main className="max-w-6xl mx-auto px-4 md:px-8 space-y-8">
        ...
        <GlassCard
          title="Hydraulic Accumulation & Pumping Intelligence"
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 relative z-10">
            <div className="p-6 bg-black/60 rounded-none border border-white/5">
              ...
              <p className="text-3xl font-black text-white font-mono tracking-tighter">
                ...
                <span className="text-xs text-slate-500 uppercase ml-2">% Vol</span>
              </p>
            </div>
            <div className="p-6 bg-black/60 rounded-none border border-white/5">
              <p className="text-[10px] text-blue-500 uppercase font-black mb-2 tracking-[0.2em] flex items-center gap-2">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {/* Pump Logic Module */}
          <GlassCard
            title={t('francis.drainagePumps.s1Title')}
            icon={<Activity className="text-blue-400" />}
          >
            <div className="space-y-4">
              {[1, 2, 3, 4].map(level => {
                const isActive =
                  waterLevel > (level === 1 ? 0 : level === 2 ? 40 : level === 3 ? 70 : 90);
                return (
                  <div
                    key={level}
                    className={`p-6 rounded-none border flex justify-between items-center transition-all duration-500 group/level ${
                      isActive
                        ? level === 4
                          ? 'bg-red-950/20 border-red-500 shadow-none animate-pulse'
                          : 'bg-blue-950/20 border-blue-500/50'
                        : 'bg-black/40 border-white/5 opacity-40 hover:opacity-100 hover:border-white/10'
                    }`}
                  >
                    <div className="font-black text-white text-xs uppercase tracking-tighter group-hover/level:pl-2 transition-all">
                      {t(`francis.drainagePumps.l${level}T`)}
                    </div>
                    <div
                      className={`font-mono font-black text-sm uppercase ${isActive ? (level === 4 ? 'text-red-500' : 'text-blue-400') : 'text-slate-600'}`}
                    >
            ...
            <div className="mt-8 p-6 bg-black/40 rounded-none border border-white/5 flex items-start gap-4">
              ...
              <div>
                <strong className="block text-white text-[11px] font-black uppercase tracking-widest mb-1">
                  {t('francis.drainagePumps.dutyTitle')}
                </strong>
                <p className="text-[10px] text-slate-400 font-bold leading-relaxed italic">
                  {t('francis.drainagePumps.dutyDesc')}
                </p>
              </div>
            </div>
          </GlassCard>
          ...
          <div className="space-y-8">
            <GlassCard
              title={t('francis.drainagePumps.s2Title')}
            ...
            >
              <div className="space-y-4">
                {[1, 2, 3].map(m => (
                  <div
                    key={m}
                    className="p-6 bg-black/40 rounded-none border border-white/5 hover:border-emerald-500/30 transition-all group/m"
                  >
                    <h4 className="text-emerald-500 font-black text-[10px] uppercase mb-2 tracking-[0.2em] group-hover/m:tracking-[0.3em] transition-all">
                      {t(`francis.drainagePumps.m${m}T`)}
                    </h4>
                    <p className="text-[11px] text-slate-400/80 font-bold leading-relaxed italic group-hover/m:text-slate-200 transition-colors">
                      {t(`francis.drainagePumps.m${m}D`)}
                    </p>
                  </div>
                ))}
              </div>
            ...
            <section className="bg-amber-950/10 border-l-[12px] border-amber-600 p-8 rounded-none shadow-none backdrop-blur-sm border border-amber-900/20 relative group overflow-hidden">
              ...
              <div className="flex items-center gap-4 mb-4 text-amber-500 relative z-10">
                ...
                <h3 className="text-lg font-black uppercase tracking-tighter">
                  {t('francis.drainagePumps.s3Title')}
                </h3>
              </div>
