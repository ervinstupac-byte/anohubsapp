      ...
      <main className="max-w-6xl mx-auto px-4 md:px-8 space-y-12">
        ...
        <div className="grid grid-cols-1 gap-16">
          ...
          <section className="space-y-8">
            <div className="flex items-center gap-4 bg-red-950/20 p-4 rounded-none border border-red-900/50">
              <span className="px-4 py-1.5 bg-red-600 text-white text-[11px] font-black rounded-none shadow-none">
                P-01
              </span>
              <h2 className="text-2xl font-black text-white uppercase tracking-tighter">
                {t('francis.emergencyProtocols.p1.title')}
              </h2>
            </div>

            <div className="grid md:grid-cols-2 gap-8">
              <GlassCard
                title={t('francis.emergencyProtocols.p1.scenario')}
                icon={<ZapOff className="text-red-500" />}
              >
                <p className="text-sm text-slate-400 font-bold italic leading-relaxed mb-8">
                ...
                <div className="p-8 bg-black/60 rounded-none border border-red-500/20 shadow-none group/warn overflow-hidden relative">
                  ...
                  <h4 className="text-red-500 text-[10px] font-black uppercase mb-4 tracking-[0.2em] relative z-10">
              ...
              <GlassCard
                title={t('francis.emergencyProtocols.p1.autoResp')}
              ...
              >
                <div className="space-y-4">
                  {[1, 2, 3].map(num => (
                    <div
                      key={num}
                      className="p-5 bg-black/40 rounded-none border border-white/5 flex gap-5 items-center group/step hover:bg-black/60 transition-all border-l-4 border-l-red-600"
                    >
                      <div className="text-red-500 font-black text-xs uppercase opacity-40 group-hover/step:opacity-100 transition-opacity">
                        0{num}
                      </div>
            ...
            <div className="p-10 bg-black/40 rounded-none border border-white/10 shadow-none relative group overflow-hidden">
              ...
              <h3 className="text-amber-500 font-black mb-8 flex items-center gap-4 uppercase text-lg tracking-tighter relative z-10">
                ...
                {t('francis.emergencyProtocols.p1.manualHead')}
              </h3>
              <div className="grid md:grid-cols-3 gap-6 relative z-10">
                {[1, 2, 3].map(num => (
                  <div
                    key={num}
                    className={`p-8 rounded-none border transition-all duration-500 group/man ${num === 3 ? 'bg-red-950/20 border-red-500/50' : 'bg-slate-900/60 border-white/5 hover:border-amber-500/30'}`}
                  >
                    ...
                    <div className="mt-4 pt-4 border-t border-white/5 flex justify-end">
                      <ShieldAlert
                        className={`w-5 h-5 ${num === 3 ? 'text-red-500' : 'text-amber-500/20'}`}
                      />
                    </div>
                  </div>
                ))}
          ...
          <section className="space-y-8">
            <div className="flex items-center gap-4 bg-amber-950/20 p-4 rounded-none border border-amber-900/50">
              <span className="px-4 py-1.5 bg-amber-600 text-white text-[11px] font-black rounded-none shadow-none">
                P-02
              </span>
              <h2 className="text-2xl font-black text-white uppercase tracking-tighter">
                {t('francis.emergencyProtocols.p2.title')}
              </h2>
            </div>
              ...
              <table className="w-full text-left text-[11px] border-collapse">
                <thead>
                  <tr className="bg-amber-900/40 text-amber-500 uppercase font-black tracking-[0.2em]">
                    <th className="p-6 border-b border-white/5">
                      {t('francis.emergencyProtocols.p2.thConc')}
                    </th>
                    <th className="p-6 border-b border-white/5">
                      {t('francis.emergencyProtocols.p2.thStatus')}
                    </th>
                    <th className="p-6 border-b border-white/5 text-right font-black">
                      {t('francis.emergencyProtocols.p2.thAction')}
                    </th>
                  </tr>
                </thead>
                <tbody className="text-slate-300">
                  {[
                    ...
                    <tr
                      key={idx}
                      className={`border-b border-white/5 hover:bg-white/5 transition-colors group ${row.key === 'Em' ? 'bg-red-950/10' : ''}`}
                    >
                      <td className="p-6 font-mono font-black text-lg tracking-tighter text-white">
                        {row.range} <span className="text-xs opacity-40 font-sans">ppm</span>
                      </td>
                      <td className={`p-6 font-black uppercase tracking-widest ${row.color}`}>
                        {t(`francis.emergencyProtocols.p2.st${row.key}`)}
                      </td>
                      <td className="p-6 text-right font-bold opacity-80 group-hover:opacity-100 transition-opacity italic">
                        {t(`francis.emergencyProtocols.p2.act${row.key}`)}
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        <section className="bg-black/40 p-10 rounded-none border border-cyan-900/30 relative group overflow-hidden">
          ...
          <div className="flex items-center gap-4 border-b border-white/5 pb-6 mb-10 relative z-10">
            ...
            <h2 className="text-2xl font-black text-white uppercase tracking-tighter shadow-none">
              Frequency Desync Intelligence
            </h2>
            <div className="ml-auto px-4 py-1.5 bg-cyan-600 text-white text-[11px] font-black rounded-none shadow-none">
              P-06
            </div>
          </div>
            ...
            <div className="p-10 bg-black/60 rounded-none border border-white/5 flex justify-between items-center group/freq">
              <div className="space-y-1">
                ...
                <div className="text-5xl font-black text-red-500 font-mono tracking-tighter group-hover/freq:scale-105 transition-transform duration-500">
                  {gridFreq}{' '}
                  <span className="text-sm font-normal text-slate-500 lowercase opacity-40">
                    Hz
                  </span>
                </div>
              </div>
        ...
        <div className="mt-16 pt-12 border-t border-white/5 text-center">
          <div className="inline-flex items-center gap-3 px-6 py-2 bg-slate-900/60 rounded-none border border-white/5 mb-6">
            ...
            <span className="text-[10px] text-slate-500 font-black tracking-[0.3em] uppercase">
