    ...
    <div className="min-h-screen bg-slate-950 text-slate-200 font-sans pb-12">
      ...
      <main className="max-w-6xl mx-auto px-4 md:px-8 space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 relative z-10">
            <div className="p-6 bg-black/60 rounded-none border border-white/5">
              ...
              <p className="text-3xl font-black text-white font-mono tracking-tighter">
                {trashBuildUp}%{' '}
                <span className="text-xs text-slate-500 uppercase ml-2">Surface</span>
              </p>
            </div>
            <div className="p-6 bg-black/60 rounded-none border border-white/5">
              <p className="text-[10px] text-cyan-500 uppercase font-black mb-2 tracking-[0.2em] flex items-center gap-2">
              ...
              <p
                className={`text-3xl font-black font-mono tracking-tighter ${isAlarm ? 'text-red-500' : 'text-emerald-400'}`}
              >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="space-y-8">
              ...
              <div className="relative h-64 bg-slate-950 rounded-none border border-white/5 overflow-hidden flex items-end justify-between px-16 pt-16 group/vis shadow-none">
                ...
                <div className="w-24 relative z-10 flex flex-col items-center">
                  <div className="absolute -top-8 text-[10px] font-black text-cyan-500 uppercase tracking-widest">
                    Upstream
                  </div>
                  <div
                    className="w-full bg-cyan-600/40 border-t-2 border-cyan-400 transition-all duration-700 shadow-none rounded-none"
                    style={{ height: '85%' }}
                  >
                ...
                <div className="absolute inset-y-0 left-1/2 -translate-x-1/2 w-6 bg-slate-900 border-x border-white/10 z-20 flex flex-col justify-evenly items-center py-4">
                  {[...Array(12)].map((_, i) => (
                    <div
                      key={i}
                      className={`w-full h-1.5 shadow-none transition-colors duration-500 ${trashBuildUp > i * 8 ? 'bg-amber-600 shadow-none' : 'bg-slate-700/30'}`}
                    />
                  ))}
                </div>
                ...
                {/* Downstream */}
                <div className="w-24 relative z-10 flex flex-col items-center">
                  <div className="absolute -top-8 text-[10px] font-black text-emerald-500 uppercase tracking-widest">
                    Downstream
                  </div>
                  <div
                    className="w-full bg-emerald-600/40 border-t-2 border-emerald-400 transition-all duration-700 shadow-none rounded-none"
              ...
              <div className="p-6 bg-black/40 border border-white/5 rounded-none flex justify-between items-center group hover:border-cyan-500/30 transition-all">
                <div>
                  ...
                  <span
                    className={`text-lg font-black uppercase tracking-tighter ${isAlarm ? 'text-red-500' : 'text-emerald-500'}`}
                  >
                    {isAlarm ? 'Critical - Clean Now' : 'Nominal Flux'}
                  </span>
                </div>
                <ShieldAlert
                  className={`w-10 h-10 ${isAlarm ? 'text-red-500 animate-pulse' : 'text-emerald-500/20'}`}
                />
              </div>
            </div>
          </GlassCard>
          ...
          {/* Sediment Monitor */}
          <GlassCard
            title="Sediment Erosion Analysis"
            icon={<Droplets className="text-amber-500" />}
          >
            <div className="space-y-6">
              <div className="p-8 bg-black/40 rounded-none border border-white/5 relative overflow-hidden group">
                ...
                <div className="h-40 bg-slate-950 rounded-none relative overflow-hidden border border-white/5 mb-6">
                  <div className="absolute inset-0 opacity-20 surface-noise animate-pulse-soft" />
                  <div className="absolute inset-0 flex flex-col items-center justify-center space-y-2">
                    <Cpu className="w-8 h-8 text-amber-500 opacity-40" />
                    <span className="text-[10px] font-black text-slate-600 uppercase tracking-widest">
                      Scanning Particle Load
                    </span>
                  </div>
                </div>
                <p className="text-xs text-slate-300 font-bold leading-relaxed relative z-10 italic">
                  High sediment load (1250 PPM) detected. Neural analysis predicts 15% accelerated
                  erosion on runner leading edges if load persists {'>'} 48h.
                </p>
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 bg-black/60 rounded-none border border-white/5 flex flex-col items-center text-center">
                  ...
                  <span className="text-xl font-black text-amber-400 font-mono tracking-tighter">
                    7.2 <span className="text-[10px] opacity-40">Mohs</span>
                  </span>
                </div>
                <div className="p-4 bg-black/60 rounded-none border border-white/5 flex flex-col items-center text-center">
                  <span className="text-[9px] text-slate-500 font-black uppercase tracking-widest mb-1">
                  ...
                  <span className="text-xl font-black text-white font-mono tracking-tighter">
                    0.12 <span className="text-[10px] opacity-40">mm/yr</span>
                  </span>
                </div>
              </div>
            </div>
