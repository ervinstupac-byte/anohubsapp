    ...
    <div className="min-h-screen bg-slate-950 text-slate-200 font-sans pb-12">
      ...
      <main className="max-w-6xl mx-auto px-4 md:px-8 space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {/* Corona Detection */}
          <GlassCard
            title={t('francis.elecHealth.cTitle')}
          ...
          >
            <div className="space-y-6">
              {[1, 2].map(i => (
                <div
                  key={i}
                  className="p-6 bg-black/40 border border-indigo-500/20 rounded-none hover:bg-indigo-900/10 transition-all group"
                >
          ...
          {/* Thermal Budget */}
          <GlassCard
            title={t('francis.elecHealth.tTitle')}
            icon={<Thermometer className="text-indigo-400" />}
          >
            <div className="overflow-hidden rounded-none border border-white/5 bg-black/40 mb-6 shadow-none">
              <table className="w-full text-left text-xs border-collapse">
                ...
                <tbody className="text-slate-300">
                  {[
                    ...
                    <tr
                      key={idx}
                      data-hotspot-id={row.active ? 'stator_core' : undefined}
                      className={`border-b border-white/5 hover:bg-white/5 transition-colors ${row.active ? 'bg-indigo-900/10' : ''}`}
                    >
                      <td
                        className={`p-4 font-black uppercase tracking-tighter ${row.active ? 'text-indigo-400' : ''}`}
                      >
                        {row.class}
                      </td>
                      <td className="p-4 font-mono font-black">{row.max}</td>
                      <td className="p-4 text-right font-mono font-black text-indigo-300">
                        {row.hotspot}
                      </td>
                    </tr>
                  ))}
            ...
            <div className="flex items-center gap-4 p-4 bg-indigo-950/20 rounded-none border border-indigo-500/20">
        ...
        {/* Interlock Logic */}
        <section className="bg-slate-900/60 p-8 rounded-none border border-white/5 relative overflow-hidden group">
          ...
          <div className="flex items-center gap-3 border-b border-white/5 pb-4 mb-8">
            <div className="w-10 h-10 rounded-none bg-amber-600/20 flex items-center justify-center border border-amber-500/30">
          ...
          <div className="grid md:grid-cols-4 gap-8">
            <div className="md:col-span-3 space-y-6">
              <div className="p-8 bg-amber-950/10 border border-amber-500/20 rounded-none relative">
                <div className="absolute top-0 right-0 px-4 py-1 bg-amber-600 text-black text-[9px] font-black uppercase rounded-none tracking-widest">
                  Logic Rule
                </div>
            ...
            <div className="space-y-4">
              {[
                ...
                <div
                  key={idx}
                  className="p-4 bg-black/60 rounded-none border border-white/5 flex justify-between items-center group/stat"
                >
                  <span className="text-[10px] text-slate-500 font-black uppercase tracking-widest group-hover/stat:text-white transition-colors">
                    {stat.label}
                  </span>
                  <div
                    className={`w-3 h-3 rounded-none ${stat.color} opacity-40 group-hover/stat:opacity-100 transition-opacity ${stat.active ? 'shadow-none' : ''}`}
                  />
                </div>
              ))}
            </div>
