  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 font-sans pb-12">
          ...
          <div className="flex items-center gap-4 text-center md:text-left">
            ...
            <div>
              <div className="flex items-center justify-center md:justify-start gap-2 mb-1">
                <span className="px-2 py-0.5 rounded-none bg-slate-900 border border-slate-800 text-slate-500 text-[10px] font-black uppercase tracking-widest">
                  SOP-PEN-002
                </span>
                <NeuralPulse />
              </div>
      ...
      <main className="max-w-6xl mx-auto px-4 md:px-8 space-y-8">
        ...
        <GlassCard
          title="Penstock Structural Intelligence"
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 relative z-10">
            <div className="p-6 bg-black/60 rounded-none border border-white/5">
              ...
              <p className="text-3xl font-black text-white font-mono tracking-tighter">
                {hoopStress.toFixed(1)}{' '}
                <span className="text-xs text-slate-500 uppercase ml-1">MPa</span>
              </p>
            </div>
            <div className="p-6 bg-black/60 rounded-none border border-white/5">
              <p className="text-[10px] text-slate-500 uppercase font-black mb-2 tracking-[0.2em] flex items-center gap-2">
              ...
              <p
                className={`text-3xl font-black font-mono tracking-tighter ${isDanger ? 'text-red-500' : 'text-emerald-400'}`}
              >
        ...
        <GlassCard title="Penstock Configuration" className="relative overflow-hidden">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 relative z-10">
            <div className="p-6 bg-black/60 rounded-none border border-white/5">
              ...
              <p className="text-3xl font-black text-white font-mono tracking-tighter">
                {(penstockDiameter * 1000).toFixed(0)}{' '}
                <span className="text-xs text-slate-500 uppercase ml-1">mm</span>
              </p>
            </div>
            <div className="p-6 bg-black/60 rounded-none border border-white/5">
              <p className="text-[10px] text-slate-500 uppercase font-black mb-2 tracking-[0.2em] flex items-center gap-2">
            ...
            <div className="p-6 bg-black/60 rounded-none border border-white/5">
              ...
              <p className="text-3xl font-black text-white font-mono tracking-tighter">
                {(penstockThickness * 1000).toFixed(1)}{' '}
                <span className="text-xs text-slate-500 uppercase ml-1">mm</span>
              </p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {/* Joukowsky Analysis */}
          <GlassCard
            title={t('francis.penstock.hammer')}
          ...
          >
            <div className="space-y-6">
              <div className="p-8 bg-red-950/10 border border-red-500/20 rounded-none relative group overflow-hidden">
                <div className="absolute top-0 right-0 px-4 py-1 bg-red-600 text-white text-[9px] font-black uppercase rounded-none tracking-widest animate-pulse">
                  IEC 60041 Standard
                </div>
                <h4 className="text-red-500 text-[10px] font-black uppercase mb-4 tracking-[0.2em]">
                  {t('francis.penstock.thickness')} ANALYSIS
                </h4>
                ...
                <div className="p-4 bg-black/60 rounded-none border border-white/5 flex justify-between items-center group/opt">
                  ...
                  <span className="text-xl font-black text-red-400 font-mono tracking-tighter">
                    +{surgePressure.toFixed(1)} <span className="text-[10px] opacity-40">BAR</span>
                  </span>
                </div>
              </div>
          ...
          {/* Thickness Monitor */}
          <GlassCard
            title={t('francis.penstock.thickness')}
            icon={<Octagon className="text-slate-400" />}
          >
            <div className="space-y-4">
              {[
                ...
                <div
                  key={idx}
                  className="p-6 bg-black/40 rounded-none border border-white/5 hover:border-slate-500 transition-all group overflow-hidden relative"
                >
                  <div
                    className={`absolute top-0 right-0 px-4 py-1 text-[9px] font-black uppercase rounded-none tracking-widest ${section.status === 'GOOD' ? 'bg-emerald-600 text-white' : 'bg-amber-600 text-black animate-pulse'}`}
                  >
                    {section.status}
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    <div className="flex flex-col">
                      ...
                      <span className="text-lg font-black text-white font-mono tracking-tighter">
                        {section.current} <span className="text-[9px] opacity-40">mm</span>
                      </span>
                    </div>
                    <div className="flex flex-col">
                      <span className="text-[8px] text-slate-500 font-black uppercase tracking-widest">
                      ...
                      <span className="text-lg font-black text-slate-400 font-mono tracking-tighter">
                        {section.original} <span className="text-[9px] opacity-40">mm</span>
                      </span>
                    </div>
                    <div className="flex flex-col items-end">
                      <span className="text-[8px] text-slate-500 font-black uppercase tracking-widest text-right">
                        Loss
                      </span>
                      <span className="text-lg font-black text-red-400 font-mono tracking-tighter">
                        -{(section.original - section.current).toFixed(1)}{' '}
                        <span className="text-[9px] opacity-40">mm</span>
                      </span>
                    </div>
                  </div>
                </div>
        ...
        <section className="bg-slate-900/60 p-8 rounded-none border border-white/5 relative group overflow-hidden">
          ...
          <div className="grid md:grid-cols-2 gap-8">
            <div className="p-8 bg-black/40 rounded-none border border-white/10 group hover:bg-black/60 transition-all border-l-4 border-l-indigo-600">
              <h4 className="text-indigo-400 text-[10px] font-black uppercase mb-4 tracking-[0.2em]">
              ...
              <p className="text-xs text-slate-300 font-bold leading-relaxed mb-6 italic opacity-80 group-hover:opacity-100 transition-opacity">
                Neural SDOF model suggests localized fatigue accumulation at Mid-Section elbow due
                to 4Hz pressure pulses during grid stabilization.
              </p>
              <button className="px-6 py-2 bg-indigo-600 text-white rounded-none text-[10px] font-black uppercase tracking-widest hover:bg-indigo-500 shadow-none transition-all">
                Request Ultrasonic Verification
              </button>
            </div>
            ...
            <div className="p-8 bg-black/40 rounded-none border border-white/10 group hover:bg-black/60 transition-all border-l-4 border-l-amber-600">
              <h4 className="text-amber-500 text-[10px] font-black uppercase mb-4 tracking-[0.2em]">
                Safety Lifecycle (SIL)
              </h4>
              <p className="text-xs text-slate-300 font-bold leading-relaxed mb-6 italic opacity-80 group-hover:opacity-100 transition-opacity">
                Penstock burst safety factor of {burstSF.toFixed(2)} is within SIL-2 operational
                envelope. Scheduled UT thickness scan due in 124 days.
              </p>
              <button className="px-6 py-2 bg-amber-600 text-white rounded-none text-[10px] font-black uppercase tracking-widest hover:bg-amber-500 shadow-none transition-all">
                Audit SCADA Logs
              </button>
            </div>
          </div>
