  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 font-sans pb-12">
      ...
      <main className="max-w-6xl mx-auto px-4 md:px-8 space-y-8">
        ...
        <GlassCard
          title={t('francis.excitation.monitor')}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 relative z-10">
            <div className="md:col-span-2 p-8 bg-black/60 rounded-none border border-white/5 shadow-none space-y-6">
              <div className="flex justify-between items-center mb-2">
                ...
                <span
                  className={`text-2xl font-black font-mono tracking-tighter tabular-nums ${voltagePerc >= 95 ? 'text-emerald-400' : 'text-amber-400'}`}
                >
                  {voltagePerc.toFixed(1)}%
                </span>
              </div>
              ...
              <div className="grid grid-cols-2 gap-6 pt-4">
                <div className="p-4 bg-white/5 rounded-none border border-white/10">
                  ...
                  <span className="text-xl font-black text-cyan-400 font-mono tracking-tighter">
                    {fieldCurrent} <span className="text-[10px] opacity-60 uppercase">Amps</span>
                  </span>
                </div>
                <div className="p-4 bg-white/5 rounded-none border border-white/10">
                  <span className="text-[9px] text-slate-500 font-black uppercase tracking-widest block mb-1">
                  ...
                  <span className="text-xl font-black text-white font-mono tracking-tighter">
                    {thyristorTemp} <span className="text-[10px] opacity-60 uppercase">°C</span>
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col items-center justify-center p-8 bg-black/40 rounded-none border border-white/5 group">
              {voltagePerc >= 95 ? (
                <div className="animate-in zoom-in-95 duration-500 flex flex-col items-center text-center space-y-4">
                  ...
                  <span className="text-xs font-black text-emerald-400 uppercase tracking-widest leading-relaxed">
                    System Stable
                    <br />
                    Ready for Sync
                  </span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {/* Control Panel */}
          <GlassCard
            title="Excitation Control Panel"
          ...
          >
            <div className="space-y-6">
              ...
              <div className="bg-black/40 p-6 rounded-none border border-white/5 flex items-center justify-between group hover:border-white/10 transition-all">
                <div>
                  <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">
                    {t('francis.excitation.fieldBreaker')}
                  </h3>
                  <div
                    className={`text-lg font-black tracking-tighter uppercase ${fieldBreaker ? 'text-emerald-500' : 'text-red-500'}`}
                  >
                    {fieldBreaker ? 'Closed (Active)' : 'Open (Isolated)'}
                  </div>
                </div>
                <button
                  onClick={toggleFieldBreaker}
              ...
              {/* Field Flashing */}
              <div
                className={`bg-black/40 p-8 rounded-none border border-white/5 transition-all duration-500 overflow-hidden relative ${fieldBreaker ? 'opacity-100' : 'opacity-40 grayscale pointer-events-none'}`}
              >
                ...
                <div className="flex items-center justify-between mb-4 relative z-10">
                  <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-widest">
                    {t('francis.excitation.flashing')}
                  </h3>
                  {fieldFlashing && (
                    <span className="px-3 py-1 bg-amber-900/30 text-amber-500 text-[9px] font-black border border-amber-800 rounded-none uppercase tracking-widest animate-pulse">
                      Flashing Active
                    </span>
                  )}
                </div>
          ...
          <div className="space-y-6">
            <section className="bg-red-900/10 border-l-[12px] border-red-600 p-8 rounded-none shadow-none backdrop-blur-sm border border-red-900/20 relative group">
              ...
              <div className="flex items-start gap-6 relative z-10">
                ...
                <div>
                  <h2 className="text-red-500 font-extrabold text-xl uppercase tracking-tighter mb-4">
                  ...
                  <ul className="space-y-3 text-[11px] font-bold text-slate-300">
                    <li className="flex gap-4">
                      ...
                      <span>
                        Field breaker inhibited if static rectifier temperature {'>'} 85°C.
                      </span>
                    </li>
                    <li className="flex gap-4">
                      <span className="text-red-600">▸</span>
            ...
            <section className="bg-slate-900/60 p-8 rounded-none border border-white/5 space-y-6">
              ...
              <div className="p-6 bg-black/40 rounded-none border border-white/10 space-y-4">
                ...
                <p className="text-[10px] text-slate-400 font-bold italic leading-relaxed">
                  Neural analysis indicates optimal harmonic suppression in the thyristor bridge. No
                  sign of bridge unbalance.
                </p>
              </div>
            </section>
          </div>
