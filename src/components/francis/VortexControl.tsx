  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 font-sans pb-12">
      ...
      <main className="max-w-6xl mx-auto px-4 md:px-8 space-y-8">
        ...
        <GlassCard
          title="Draft Tube Cavitation & Vortex Intelligence"
          className="relative overflow-hidden group"
        >
          <div
            data-hotspot-id="draft_tube"
          ...
          >
            ...
            <div className="p-6 bg-black/60 rounded-none border border-white/5">
              ...
              <p
                className={`text-3xl font-black font-mono tracking-tighter ${isVortexLikely ? 'text-amber-500' : 'text-emerald-400'}`}
              >
            ...
            <div className="p-6 bg-black/60 rounded-none border border-white/5">
              ...
              <p className="text-3xl font-black text-white font-mono tracking-tighter uppercase tabular-nums">
                {telemetry.mechanical?.acousticMetrics?.cavitationIntensity?.toFixed(1) ?? '2.4'}{' '}
                <span className="text-[10px] text-slate-500 font-bold ml-1">RMS</span>
              </p>
            </div>
          </div>
        </GlassCard>
          ...
          <div className="flex flex-col md:flex-row justify-between items-start gap-8 relative z-10 text-pretty">
            <div className="flex-grow">
              <h2 className="text-emerald-500 font-extrabold text-2xl uppercase tracking-tighter mb-4 flex items-center gap-3">
                ...
                <span
                  className={`px-3 py-1 bg-emerald-900 border border-emerald-800 text-white text-[10px] font-black rounded-none uppercase tracking-widest`}
                >
            ...
            <div className="w-full md:w-80 p-6 bg-slate-950/60 border border-slate-800 rounded-none shadow-none">
              <h4 className="text-emerald-400 text-[10px] font-black uppercase mb-6 tracking-[0.2em]">
                {t('francis.vortex.freqCalc')} (RHEINGANS)
              </h4>
              <ul className="space-y-4 text-[11px] text-slate-400 font-mono">
                <li className="flex justify-between border-b border-white/5 pb-2">
                  <span>Rated Speed (fn):</span>{' '}
                  <span className="text-white">
                    {((telemetry.mechanical?.rpm ?? 0) / 60).toFixed(2)} Hz
                  </span>
                </li>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <GlassCard title={t('francis.vortex.s3Title')} icon={<Wind className="text-cyan-400" />}>
            <div className="space-y-6">
              <p className="text-[11px] text-slate-400 font-bold leading-relaxed">
                {t('francis.vortex.airDesc')}
              </p>
              <div className="grid grid-cols-1 gap-3">
                {[1, 2, 3].map(num => (
                  <div
                    key={num}
                    className="p-4 bg-white/5 rounded-none border border-white/10 flex items-center gap-4 group/opt hover:bg-white/10 transition-all"
                  >
              ...
              <div className="p-6 bg-red-950/10 border border-red-500/20 rounded-none">
                <div className="flex justify-between items-center mb-4">
                  <h4 className="text-red-500 text-[10px] font-black uppercase tracking-widest">
                    {t('francis.vortex.compCheck')}
                  </h4>
                  <ShieldAlert className="w-5 h-5 text-red-500/40" />
                </div>
                <p className="text-[11px] text-slate-300 font-mono mb-1">
                  {t('francis.vortex.compSpecs')}
                </p>
                <p className="text-[9px] text-red-400 font-black italic uppercase leading-tight">
                  {t('francis.vortex.compWarn')}
                </p>
              </div>
            </div>
          </GlassCard>
          ...
          <GlassCard
            title={t('francis.vortex.s4Title')}
              ...
              <table className="w-full text-left text-[10px] border-collapse">
                ...
                <tbody className="text-slate-300">
                  {[1, 2, 3].map(rowIdx => (
                    ...
                    >
                      <td
                        className={`p-4 font-black uppercase tracking-tighter group-hover:pl-6 transition-all ${rowIdx === 3 ? 'text-amber-500' : 'text-white'}`}
                      >
                        {t(`francis.vortex.td${rowIdx}_1`)}
                      </td>
                      <td className="p-4 font-bold opacity-70 group-hover:opacity-100 italic">
                        {t(`francis.vortex.td${rowIdx}_2`)}
                      </td>
                      <td
                        className={`p-4 text-right font-black uppercase tracking-tighter ${rowIdx === 3 ? 'text-red-500' : 'text-emerald-400'}`}
                      >
        ...
        <section className="bg-slate-900/60 p-8 rounded-none border border-white/5 space-y-8 relative overflow-hidden group">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {[1, 2, 3].map(step => (
              <div
                key={step}
                className="p-8 bg-black/40 rounded-none border border-white/5 hover:bg-black/60 transition-all group/step border-t-4 border-t-emerald-600"
              >
                <div className="text-emerald-500 text-[10px] font-black uppercase mb-4 tracking-[0.2em]">
                  {t(`francis.vortex.step${step}Title`)}
                </div>
          ...
          <div className="bg-amber-950/10 border-2 border-amber-500/30 p-8 rounded-none flex gap-6 items-center">
            ...
            <div>
              <span className="text-amber-500 text-[11px] font-black uppercase tracking-widest block mb-2">
                {t('francis.vortex.opRule')}
              </span>
              <p className="text-sm text-slate-200 font-bold leading-relaxed italic">
                {t('francis.vortex.ruleText')}
              </p>
            </div>
          </div>
        </section>
