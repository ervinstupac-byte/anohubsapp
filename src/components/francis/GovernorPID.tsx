        ...
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="text-center md:text-left">
            ...
            <p className="text-blue-400 font-bold mt-1 uppercase text-xs tracking-widest flex items-center gap-2">
              <NeuralPulse /> {t('francis.governorPID.subtitle')} // NC-9.0 AI-TUNED
            </p>
          </div>
      ...
      <main className="max-w-6xl mx-auto px-4 md:px-8 space-y-8">
              ...
              <div className="relative w-40 h-40 flex items-center justify-center">
                <svg className="w-full h-full transform -rotate-90">
                  ...
                  <motion.circle
                    ...
                    animate={{
                      strokeDashoffset: 440 * (1 - Math.min(1, Math.abs(governorMath.error / 2))),
                    }}
                    className={
                      Math.abs(governorMath.error) < 0.2
                        ? 'text-emerald-500'
            ...
            <div className="lg:col-span-3 space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="p-4 bg-white/5 rounded-none border border-white/10 hover:border-blue-500/50 transition-colors">
                  ...
                  <p
                    className={`text-3xl font-black font-mono tracking-tighter ${Math.abs(governorMath.error) < 0.2 ? 'text-emerald-400' : 'text-amber-400 animate-pulse'}`}
                  >
                    {governor.actualValue.toFixed(2)}{' '}
                    <span className="text-xs text-slate-500">Hz</span>
                  </p>
                </div>
                <div className="p-4 bg-blue-500/10 rounded-none border border-blue-500/20">
                  ...
                  <p className="text-3xl font-black text-blue-400 font-mono tracking-tighter">
                    {governorMath.controlSignal.toFixed(1)}
              ...
              {/* Narrative Box */}
              <div
                className={`p-6 rounded-none border-2 transition-all ${Math.abs(governorMath.error) > 0.5 ? 'bg-amber-500/10 border-amber-500/30' : 'bg-blue-500/10 border-blue-500/30 shadow-none'}`}
              >
              ...
              <div className="flex flex-wrap justify-between items-center px-6 py-4 bg-black/40 rounded-none border border-white/5 shadow-none gap-4">
                <div className="flex gap-8">
                  <div className="text-center group/gain">
                    ...
                    <span className="text-sm font-mono font-black text-white group-hover/gain:text-blue-400 transition-colors">
                      {governor.kp.toFixed(2)}
                    </span>
                  </div>
                  <div className="text-center group/gain">
                    <span className="text-[9px] text-slate-500 uppercase font-black block mb-1">
              ...
              <ul className="grid md:grid-cols-2 gap-x-12 gap-y-4 text-xs font-bold text-slate-300">
                {[
                  ...
                  <li key={idx} className="flex gap-4 group cursor-help">
                    <span className="text-red-600 group-hover:scale-150 transition-transform">
                      ▸
                    </span>
                    <span>
                      <strong className="text-red-100 uppercase tracking-tighter mr-2">
                        {t(`francis.governorPID.${item.title}`)}:
                      </strong>
                      <span className="text-slate-400 group-hover:text-slate-100 transition-colors">
                        {t(`francis.governorPID.${item.desc}`)}
                      </span>
                    </span>
                  </li>
                ))}
        ...
        <section className="bg-slate-900/50 p-8 rounded-none border border-white/5">
          ...
          <div className="space-y-8">
            ...
            <div className="bg-gradient-to-br from-blue-600/10 to-transparent border border-blue-500/20 rounded-none p-8 relative overflow-hidden">
              ...
              <h4 className="text-blue-400 font-black uppercase text-[10px] mb-6 tracking-[0.4em] text-center relative z-10">
                {t('francis.governorPID.loopTitle')}
              </h4>
          ...
          <section className="bg-white/5 rounded-none border border-white/10 overflow-hidden backdrop-blur-md">
            ...
            <div className="p-8">
              <p className="text-sm text-slate-300 mb-6 leading-relaxed">
                <strong className="text-white">{t('francis.governorPID.dbDef')}</strong>{' '}
                {t('francis.governorPID.dbDesc')}
              </p>
              <div className="bg-black/40 p-4 rounded-none border border-white/5 flex justify-between items-center">
                <span className="text-xs text-slate-500 font-black uppercase">
                  {t('francis.governorPID.dbParam')}
                </span>
                <span className="text-lg font-mono font-black text-white">
                  {t('francis.governorPID.rpmRangeValue')}
                </span>
              </div>
            </div>
          </section>
            ...
            <div className="p-8">
              <ul className="space-y-4">
                {[
                  {
                    id: 'overshoot',
                    val: t('francis.governorPID.overshootVal'),
                    desc: 'overshootDesc',
                  },
                  {
                    id: 'settleTime',
                    val: t('francis.governorPID.settleTimeVal'),
                    desc: 'settleDesc',
                  },
                  {
                    id: 'oscillations',
                    val: t('francis.governorPID.oscillationsVal'),
                    desc: 'oscDesc',
                  },
                ].map((row, idx) => (
                  <li
        ...
        <div className="bg-black/60 p-6 rounded-none flex flex-wrap gap-8 items-center justify-center text-[10px] border border-white/5">
          <div className="flex items-center gap-2">
            ...
            <span className="uppercase font-black text-white tracking-widest">
              {t('francis.governorPID.checklist')}:
            </span>
          </div>
          {['software', 'monitor', 'docs', 'mechTools', 'safety'].map((item, idx) => (
            <div
              key={idx}
              className="flex items-center gap-2 text-slate-500 font-bold uppercase tracking-tighter"
            >
