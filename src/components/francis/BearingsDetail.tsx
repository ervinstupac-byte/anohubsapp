...
export const BearingsDetail: React.FC = () => {
    ...
    return (
        <div className="min-h-screen bg-slate-950 text-slate-300 font-sans pb-12">
            ...
            {/* Header */}
            <header className={`bg-black/40 border-b-2 ${isCriticalTemp ? 'border-red-600 animate-pulse' : 'border-amber-500'} py-8 px-4 md:px-8 mb-8 sticky top-0 z-50 backdrop-blur-md shadow-none transition-all`}>
                <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center gap-6">
                    <div className="flex items-center gap-4 text-center md:text-left">
                        <div className={`p-3 ${isCriticalTemp ? 'bg-red-600' : 'bg-amber-600'} rounded-none border border-white/10 shadow-none`}>
                            <RefreshCw className="text-white w-8 h-8 animate-spin-slow" />
                        </div>
                        <div>
                            <div className="flex items-center justify-center md:justify-start gap-2 mb-1">
                                <span className="px-2 py-0.5 rounded-none bg-amber-950 text-amber-500 text-[10px] font-black border border-amber-900/50 uppercase tracking-widest">SOP-ROT-001</span>
                                <NeuralPulse />
                            </div>
            ...
            <main className="max-w-6xl mx-auto px-4 md:px-8 space-y-8">
                ...
                <GlassCard title="Real-Time Thermal Intelligence" className="relative overflow-hidden group">
                    <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 relative z-10">
                        ...
                        <div className="lg:col-span-1 flex flex-col justify-center items-center p-6 bg-black/60 rounded-none border border-white/5 shadow-none">
                                ...
                                <svg className="w-full h-full transform -rotate-90" data-hotspot-id="upper_guide_bearing">
                                    ...
                                    <circle
                                        ...
                                        className={isCriticalTemp ? 'text-red-500 shadow-none' : isHighTemp ? 'text-amber-500 shadow-none' : 'text-emerald-500'}
                                    />
                                </svg>
                        ...
                        <div className="lg:col-span-3 space-y-8">
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                <div className="p-4 bg-white/5 rounded-none border border-white/10">
                                    <p data-hotspot-id="upper_guide_bearing" className="text-[10px] text-slate-500 uppercase font-black mb-2 tracking-widest flex items-center gap-2">
                                        <Thermometer className="w-3 h-3 text-brand-400" /> Upper Guide Bearing
                                    </p>
                            ...
                            <div className={`p-6 rounded-none border-2 transition-all ${isHighTemp ? 'bg-red-500/10 border-red-500/30 shadow-none' : 'bg-blue-500/10 border-blue-500/30 shadow-none'}`}>
                                <div className="flex items-center gap-3 mb-3">
                                    <div data-hotspot-id="guide_bearing_indicator" className={`w-2 h-2 rounded-none ${isHighTemp ? 'bg-red-500' : 'bg-blue-400'}`} />
                ...
                <section className={`bg-slate-900/40 backdrop-blur-sm rounded-none p-8 border-l-[12px] ${isCriticalTemp ? 'border-red-600 animate-pulse' : 'border-red-600'} border border-white/5 shadow-none overflow-hidden relative`}>
