    ...
    <div className="min-h-screen bg-slate-950 text-slate-200 font-sans pb-12 selection:bg-cyan-500/30">
        ...
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="text-center md:text-left">
            ...
            <p className="text-cyan-400 font-bold mt-1 uppercase text-xs tracking-widest flex items-center gap-2">
              <NeuralPulse /> {t('francis.coolingWater.subtitle') || 'THERMAL REJECTION LOGIC'} //
              NC-9.0 CRYOSYNC
            </p>
          </div>
          ...
          <div className="flex gap-4">
            <div className="bg-black/40 border border-cyan-500/30 px-4 py-2 rounded-none flex flex-col items-end">
              <span className="text-[8px] text-cyan-500 font-black uppercase tracking-tighter">
                Current Flow
              </span>
              <span className="text-xl font-black font-mono text-white">
                {coolingFlow} <span className="text-[10px] text-slate-500">L/s</span>
              </span>
            </div>
            <button
      ...
      <main className="max-w-6xl mx-auto px-4 md:px-8 space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {/* Auto-Strainers */}
          <GlassCard
            title={t('francis.coolingWater.s1Title')}
              ...
              <div className="bg-black/40 p-6 rounded-none border border-white/5 mb-6">
                <div className="flex justify-between items-center mb-4">
                  <span className="text-slate-400 text-xs font-black uppercase tracking-widest">
                    {t('francis.coolingWater.dpVal')}
                  </span>
                  <span className="text-emerald-400 font-mono font-black text-xl bg-emerald-900/20 px-4 py-1 rounded-none border border-emerald-500/20">
                    0.12 BAR
                  </span>
                </div>
                ...
                <p className="text-[10px] text-slate-500 mt-4 italic font-bold leading-relaxed">
                  {t('francis.coolingWater.dpDesc')}
                </p>
              </div>

              <div className="space-y-4">
                <h4 className="text-cyan-400 font-black text-[10px] uppercase tracking-[0.2em] border-b border-white/5 pb-2">
                  {t('francis.coolingWater.flushLogic')}
                </h4>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="p-4 bg-white/5 rounded-none border border-white/10 hover:bg-white/10 transition-colors">
                    ...
                    <strong className="block text-white text-[10px] font-black uppercase mb-1">
                      {t('francis.coolingWater.trig')}
                    </strong>
                  ...
                  <div className="p-4 bg-red-900/10 rounded-none border border-red-500/20 hover:bg-red-900/20 transition-colors">
                    ...
                    <strong className="block text-red-400 text-[10px] font-black uppercase mb-1">
                      {t('francis.coolingWater.jam')}
                    </strong>
                    <span className="text-[9px] text-slate-500 font-bold uppercase tracking-tighter">
                      Manual Override
                    </span>
          ...
          <GlassCard
            title={t('francis.coolingWater.s2Title')}
          ...
          >
                ...
                <thead>
                  <tr className="bg-cyan-900/40 text-cyan-400 font-black text-[9px] uppercase tracking-[0.2em]">
                    <th className="p-4 border-b border-white/5">
                      {t('francis.coolingWater.thCol')}
                    </th>
                    <th className="p-4 border-b border-white/5 text-right">
                      {t('francis.coolingWater.thFlow')}
                    </th>
                    <th className="p-4 border-b border-white/5 text-right font-black">
                      {t('francis.coolingWater.thStat')}
                    </th>
                  </tr>
                </thead>
            ...
            <div className="mt-6 p-6 bg-gradient-to-br from-cyan-950/40 to-black rounded-none border border-cyan-500/20 flex justify-between items-center group overflow-hidden relative">
              ...
              <div className="relative z-10 flex flex-col">
                <span className="text-[10px] text-cyan-500 font-black uppercase tracking-[0.2em] mb-1">
                  {t('francis.coolingWater.inletTemp')}
                </span>
            ...
            {[
              {
                ...
                label: t('francis.coolingWater.modeWinter'),
              },
              {
                id: 'summer',
                icon: Sun,
                ...
                label: t('francis.coolingWater.modeSummer'),
              },
              {
                id: 'failure',
                icon: AlertTriangle,
                ...
                label: t('francis.coolingWater.modeFail'),
              },
            ].map(tab => (
              <button
                key={tab.id}
                onClick={() => setActiveTab(tab.id as any)}
                className={`flex-1 p-6 rounded-none border-2 transition-all duration-500 flex flex-col items-center gap-4 group relative overflow-hidden ${
                  activeTab === tab.id
              ...
              >
                <div
                  className={`p-4 rounded-none ${activeTab === tab.id ? 'bg-white/10' : 'bg-white/5'} transition-colors`}
                >
                  <tab.icon
                    className={`w-8 h-8 ${activeTab === tab.id ? tab.color : 'text-slate-600'}`}
                  />
                </div>
                <span
                  className={`font-black uppercase tracking-[0.2em] text-xs ${activeTab === tab.id ? 'text-white' : 'text-slate-500'}`}
                >
                  {tab.label}
                </span>
                {activeTab === tab.id && (
                  <div
                    className={`absolute bottom-0 left-0 right-0 h-1 ${tab.id === 'winter' ? 'bg-cyan-500' : tab.id === 'summer' ? 'bg-amber-500' : 'bg-red-500'}`}
                  />
                )}
              </button>
            ))}
          ...
          <div className="bg-black/60 p-8 rounded-none border border-white/5 min-h-[200px] flex items-center justify-center relative overflow-hidden shadow-none">
            ...
            {activeTab === 'winter' && (
              <div className="animate-in fade-in zoom-in-95 duration-500 max-w-2xl text-center space-y-4">
                <h4 className="text-2xl font-black text-cyan-400 uppercase tracking-tighter">
                  {t('francis.coolingWater.modeRec')}
                </h4>
                <p className="text-slate-300 text-lg font-bold leading-relaxed">
                  {t('francis.coolingWater.descWinter')}
                </p>
            ...
            {activeTab === 'summer' && (
              <div className="animate-in fade-in zoom-in-95 duration-500 max-w-2xl text-center space-y-4">
                <h4 className="text-2xl font-black text-amber-400 uppercase tracking-tighter">
                  {t('francis.coolingWater.modeFull')}
                </h4>
                <p className="text-slate-300 text-lg font-bold leading-relaxed">
                  {t('francis.coolingWater.descSummer')}
                </p>
                <div className="flex justify-center gap-2 pt-4">
            ...
            {activeTab === 'failure' && (
              <div className="animate-in fade-in zoom-in-95 duration-500 max-w-2xl text-center space-y-4">
                <h4 className="text-2xl font-black text-red-500 uppercase tracking-tighter">
                  {t('francis.coolingWater.modeMan')}
                </h4>
                <p className="text-red-200 text-lg font-bold leading-relaxed italic">
                  {t('francis.coolingWater.descFail')}
                </p>
                <div className="flex justify-center gap-2 pt-4">
