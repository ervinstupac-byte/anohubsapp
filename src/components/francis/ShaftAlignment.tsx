...
import {
  ShaftOrbitPlot,
  ShaftOrbitPlotHandle,
} from '../../features/telemetry/components/ShaftOrbitPlot';
...
export const ShaftAlignment: React.FC = () => {
  ...
  const totalOffset = Math.sqrt(
    Math.pow(measurements.vOffset, 2) + Math.pow(measurements.hOffset, 2)
  );
  ...
  const handlePinBaseline = () => {
    ...
    if (points) {
      ...
      logAction(
        'PIN_BASELINE',
        'Baseline orbit reference captured and persisted to Neural Core.',
        'SUCCESS'
      );
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 font-sans pb-12">
        ...
        <div className="max-w-6xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="flex items-center gap-2 mb-1">
                <span className="px-2 py-0.5 rounded-none bg-amber-950 text-amber-400 text-[10px] font-black border border-amber-900/50 uppercase tracking-widest">
                  SOP-MECH-005
                </span>
          ...
          <div className="flex items-center gap-4">
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-emerald-600/10 border border-emerald-500/30 rounded-none text-[10px] font-black text-emerald-400 hover:bg-emerald-600 hover:text-white transition uppercase tracking-widest"
            >
              <FileSearch className="w-3 h-3" /> Dossier
            </button>
            <button
              onClick={() => navigate(`/${ROUTES.FRANCIS.HUB}`)}
              className="p-2 bg-white/5 border border-white/10 rounded-none hover:bg-white/10 transition"
            >
      ...
      <main
        className={`max-w-6xl mx-auto px-4 md:px-8 ${densityMode === 'compact' ? 'space-y-6' : 'space-y-12'}`}
      >
        ...
        <section className="bg-slate-900/40 border border-white/5 rounded-none p-1 overflow-hidden backdrop-blur-sm relative">
          ...
          <div className={`grid grid-cols-1 lg:grid-cols-12 ${spacing.sectionGap}`}>
            {/* Inputs */}
            <div
              className={`col-span-12 lg:col-span-4 bg-black/20 ${spacing.cardPadding} rounded-none`}
            >
              ...
              <div className="space-y-5">
                <div className="space-y-1">
                  ...
                  <div className="text-[9px] text-slate-600 font-mono">
                    Tier: {tolerances.tier.max === 99999 ? '>3600' : `<${tolerances.tier.max}`} RPM
                  </div>
                </div>
                ...
                <div className="pt-4 border-t border-white/5">
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <span className="text-xs font-mono font-black text-white">
                      {tolerances.maxOffsetMm.toFixed(3)} mm
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-xs font-mono font-black text-white">
                      {tolerances.maxAngularityMmPerM.toFixed(3)} mm/m
                    </span>
                  </div>
                </div>
              </div>
            ...
            {/* Visualizer Target */}
            <div
              data-hotspot-id="shaft"
              className="col-span-12 lg:col-span-5 flex flex-col items-center justify-center p-6 relative"
            >
              ...
              <div className="mt-6 flex gap-4 text-center">
                <div>
                  ...
                  <div
                    className={`text-sm font-black uppercase ${offsetStatus === 'pass' ? 'text-emerald-400' : 'text-red-500'}`}
                  >
                    {offsetStatus}
                  </div>
                </div>
                <div>
                  ...
                  <div
                    className={`text-sm font-black uppercase ${angularStatus === 'pass' ? 'text-emerald-400' : 'text-red-500'}`}
                  >
                    {angularStatus}
                  </div>
                </div>
            ...
            <div className="col-span-12 lg:col-span-3 bg-slate-950/50 p-6 rounded-none border-l border-white/5">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={e =>
                      setMeasurements({ ...measurements, vOffset: Number(e.target.value) })
                    }
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={e =>
                      setMeasurements({ ...measurements, hOffset: Number(e.target.value) })
                    }
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={e =>
                      setMeasurements({ ...measurements, gapDiff: Number(e.target.value) })
                    }
              ...
              <button
                onClick={() =>
                  confirmStep('manual_entry', `Manual Alignment Input: ${totalOffset.toFixed(3)}mm`)
                }
        ...
        <div className={`grid grid-cols-1 lg:grid-cols-3 ${spacing.sectionGap}`}>
          <div className="lg:col-span-2">
            <GlassCard
              title="Shaft Orbit Dynamic Stability"
              ...
              <div className="flex flex-col md:flex-row gap-6 items-center">
                <div className="p-4 bg-black/40 rounded-none border border-white/5 shadow-none">
                  <ShaftOrbitPlot
                    ...
                    centerPath={[]}
                    onAnalysis={analysis => {
                      const currentCenterPath =
                        useTelemetryStore.getState().mechanical.centerPath || [];
                      setMechanical({
                        centerPath: [...currentCenterPath.slice(-49), analysis.currentCenter],
                      });
                    }}
                  />
                </div>
                <div className="flex-1 w-full space-y-4">
                  ...
                  <div className={`grid grid-cols-2 ${spacing.sectionGap}`}>
                    ...
                    <div className="p-4 bg-black/40 rounded-none border border-white/5">
                      ...
                      <div className="text-xl font-black font-mono text-amber-400">
                        {new Decimal(orbitAnalysis.centerMigration).mul(1000).toFixed(1)}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="flex-1 py-2 bg-indigo-600 hover:bg-indigo-500 text-white text-[10px] font-black uppercase rounded-none"
                    >
                      Pin Baseline
                    </button>
                    <button
                      onClick={handleResetBaseline}
                      className="px-4 py-2 bg-red-950/20 text-red-500 text-[10px] font-black uppercase rounded-none border border-red-500/20"
                    >
                      Reset
                    </button>
                  </div>
            ...
            <div className="bg-red-950/10 border-l-[4px] border-red-500 p-6 rounded-none border-y border-r border-red-500/10 h-full flex flex-col">
              ...
              <div className="grid grid-cols-2 gap-3 mb-6">
                {['c1', 'c2', 'c3', 'c4'].map((id, i) => (
                  <div
                    key={id}
                    className="bg-black/40 p-3 rounded-none border border-white/5 text-center"
                  >
                    <span className="text-[8px] text-slate-500 font-black uppercase block mb-1">
                      Foot {i + 1}
                    </span>
                    <span
                      className={`text-xs font-mono font-bold ${i === 3 ? 'text-red-400 animate-pulse' : 'text-emerald-400'}`}
                    >
                      {i === 3 ? '0.08' : '0.02'}
                    </span>
                  </div>
              ...
              <div className="mt-auto p-4 bg-red-500/10 rounded-none border border-red-500/20 text-center">
                <div className="text-[9px] text-red-300 font-bold uppercase tracking-tight">
                  Shim Correction Required on Foot 4
                </div>
              </div>
            </div>
          </div>
