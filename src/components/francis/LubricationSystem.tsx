      ...
      <main className="max-w-6xl mx-auto px-4 md:px-8 space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          <GlassCard className="lg:col-span-1 flex flex-col items-center justify-center text-center p-8 bg-black/60 relative overflow-hidden group">
            ...
            <span className="text-[10px] text-emerald-500 font-black uppercase tracking-widest mb-2 relative z-10">
              System Status
            </span>
            <CheckCircle2 className="w-12 h-12 text-emerald-500 mb-4 relative z-10 animate-pulse" />
            <span className="text-3xl font-black text-white uppercase tracking-tighter relative z-10">
              Active
            </span>
            <span className="text-[10px] text-slate-500 font-bold mt-2 relative z-10">
              Last Cycle: 12m ago
            </span>
          </GlassCard>
              ...
              <div className="flex gap-8">
                <div className="text-right">
                  ...
                  <div className="text-xl font-black text-emerald-500 tabular-nums">
                    12/24 <span className="text-[10px] opacity-60">Cycles</span>
                  </div>
                </div>
                ...
                <thead>
                  <tr className="bg-emerald-900/40 text-emerald-400 uppercase font-black text-[9px] tracking-[0.2em]">
                    <th className="p-4 border-b border-white/5">
                      {t('francis.lubrication.thZone')}
                    </th>
                    <th className="p-4 border-b border-white/5">
                      {t('francis.lubrication.thPts')}
                    </th>
                    <th className="p-4 border-b border-white/5">
                      {t('francis.lubrication.thDose')}
                    </th>
                    <th className="p-4 border-b border-white/5 text-right font-black">
                      {t('francis.lubrication.thFb')}
                    </th>
                  </tr>
                </thead>
                  ...
                  {[
                    {
                      ...
                      dose: '5cc',
                      fb: 'fbSw',
                      fbColor: 'text-emerald-400 font-black',
                    },
                    {
                      zone: 'tdTop',
                      pts: '20 (Points)',
                    ...
                    >
                      <td className="p-4 text-emerald-500 font-black uppercase tracking-tighter group-hover:pl-6 transition-all">
                        {t(`francis.lubrication.${row.zone}`)}
                      </td>
                      <td className="p-4 font-bold">{row.pts}</td>
                      <td className="p-4 font-mono font-black">{row.dose}</td>
                      <td
                        className={`p-4 text-right ${row.fbColor} uppercase text-[10px] tracking-widest`}
                      >
                        {t(`francis.lubrication.${row.fb}`)}
                      </td>
                    </tr>
                  ))}
                </tbody>
          ...
          <section className="bg-slate-900/60 p-8 rounded-none border border-white/5 space-y-8">
            ...
            <div className="grid gap-6">
              {[1, 2, 3].map(num => (
                <div
                  key={num}
                  className="p-5 bg-black/40 border border-white/5 rounded-none group hover:border-blue-500/30 transition-all flex justify-between items-center"
                >
                  ...
                  <div
                    className={`px-4 py-1 rounded-none text-[9px] font-black uppercase tracking-widest border ${num === 2 ? 'bg-slate-800 text-slate-500 border-slate-700' : 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20'}`}
                  >
          ...
          <section className="bg-slate-900/60 p-8 rounded-none border border-white/5 space-y-8 relative overflow-hidden">
            ...
            <div className="grid gap-4 relative z-10">
              {[1, 2, 3].map(num => (
                <div
                  key={num}
                  className="flex items-center gap-4 p-5 bg-black/60 border border-white/10 rounded-none hover:bg-emerald-500/10 hover:border-emerald-500/30 transition-all group"
                >
