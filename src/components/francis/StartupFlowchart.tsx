...
export const StartupFlowchart: React.FC = () => {
  ...
  const VerticalConnector = ({ active, current }: { active: boolean; current: boolean }) => (
    <div className="relative w-1.5 h-16 my-2 group">
      <div
        className={`absolute inset-0 w-full h-full rounded-none transition-all duration-1000 ${active ? 'bg-cyan-500 shadow-none' : 'bg-slate-900 border-x border-white/5'}`}
      />
      {current && (
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-4 h-4 bg-cyan-400 rounded-none animate-ping opacity-40" />
      )}
    </div>
  );
  ...
  const StepNode = ({ step, index }: { step: any; index: number }) => {
    ...
    let cardStyle = isActive
      ? 'bg-slate-900/60 border-cyan-500 shadow-none'
    ...
    return (
      <div className="flex flex-col items-center group/node">
        <div
          ...
          className={`relative w-[450px] p-10 rounded-none border transition-all duration-700 cursor-pointer backdrop-blur-md ${cardStyle}`}
        >
          <div className="flex justify-between items-start gap-8">
            <div className="flex-1">
              <div className="flex items-center gap-3 mb-4">
                <span
                  className={`px-3 py-1 rounded-none text-[10px] font-black uppercase tracking-widest border italic transition-colors ${isActive ? 'bg-cyan-950 text-cyan-500 border-cyan-800' : 'bg-slate-900 text-slate-600 border-white/5'}`}
                >
              ...
              <h3
                className={`text-2xl font-black uppercase tracking-tighter mb-4 italic transition-colors ${isActive ? 'text-white' : 'text-slate-700'}`}
              >
                {t(`francis.startupFlowchart.${step.key}.title`)}
              </h3>
              <p
                className={`text-sm font-bold italic transition-colors leading-relaxed uppercase tracking-tighter ${isActive ? 'text-slate-400' : 'text-slate-800'}`}
              >
            ...
            <div
              className={`p-6 rounded-none border transition-all duration-700 ${isActive ? 'bg-cyan-600 text-white shadow-none rotate-0' : 'bg-slate-900 text-slate-800 border-white/5 rotate-12'}`}
            >
          ...
          {(step.type === 'check' || step.type === 'decision') && (
            <div
              className={`mt-10 pt-8 border-t border-white/5 flex gap-8 text-[11px] font-black uppercase tracking-widest transition-opacity duration-700 ${isActive ? 'opacity-100' : 'opacity-0'}`}
            >
              <div className="flex items-center gap-3 text-emerald-500 italic bg-emerald-950/20 px-6 py-2 rounded-none border border-emerald-900/30">
                <CheckCircle className="w-4 h-4" />
                {step.key === 'step4'
                  ? t('francis.startupFlowchart.decisions.yesEqualized')
                  : t('francis.startupFlowchart.decisions.yes')}
              </div>
              <div className="flex items-center gap-3 text-red-500 italic bg-red-950/10 px-6 py-2 rounded-none border border-red-900/20 opacity-40 hover:opacity-100 transition-opacity">
                <XCircle className="w-4 h-4" />
                {step.fail === 'leak'
                  ? t('francis.startupFlowchart.decisions.noLeak')
                  : step.fail === 'abort'
                    ? t('francis.startupFlowchart.decisions.noAbort')
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 font-sans pb-24">
      ...
      <main className="max-w-4xl mx-auto px-4 flex flex-col items-center">
        ...
        <div
          className={`
                    w-40 h-40 mt-8 rounded-none flex flex-col items-center justify-center font-black transition-all duration-1000 border-4 relative overflow-hidden group
                    ${activeStep === 7 ? 'bg-blue-600 border-blue-400 text-white shadow-none scale-110' : 'bg-black/40 border-white/5 text-slate-800 opacity-20'}
                `}
        >
        ...
        {activeStep > 0 && (
          <button
            ...
            className="mt-24 px-8 py-3 bg-red-950/20 border border-red-500/30 text-red-500 text-[10px] font-black uppercase tracking-[0.4em] rounded-none hover:bg-red-600 hover:text-white transition-all italic"
          >
