  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 font-sans pb-12">
      ...
      <main className="max-w-6xl mx-auto px-4 md:px-8 space-y-8">
        ...
        <GlassCard
          title="Main Inlet Valve Diagnostic Site"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {/* Calibration Hub */}
          <GlassCard
            title={t('francis.mivDistributor.calibration.title')}
          ...
          <GlassCard
            title={t('francis.mivDistributor.synchronicity.title')}
              ...
              <table className="w-full text-left text-[10px] border-collapse">
                <thead>
                  <tr className="bg-blue-900/40 text-blue-400 uppercase font-black tracking-[0.2em]">
                    <th className="p-4 border-b border-white/5">
                      {t('francis.mivDistributor.cylinder.thComponent')}
                    </th>
                    <th className="p-4 border-b border-white/5">
                      {t('francis.mivDistributor.cylinder.thCheck')}
                    </th>
                    <th className="p-4 border-b border-white/5 text-right font-black">
                      {t('francis.mivDistributor.cylinder.thFreq')}
                    </th>
                  </tr>
                </thead>
                <tbody className="text-slate-300">
                  {[
                    ...
                    >
                      <td className="p-4 font-black uppercase group-hover:pl-6 transition-all">
                        {t(`francis.mivDistributor.cylinder.${row.key}`)}
                      </td>
                      <td className="p-4 font-bold opacity-70 group-hover:opacity-100">
                        {t(`francis.mivDistributor.cylinder.${row.key}Check`)}
                      </td>
                      <td className="p-4 text-right font-black text-blue-400 uppercase tracking-tighter">
                        {t(`francis.mivDistributor.cylinder.${row.key}Freq`)}
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            <div className="overflow-hidden rounded-none border border-white/10 bg-black/40">
              <table className="w-full text-left text-[10px] border-collapse">
                <thead>
                  <tr className="bg-blue-900/40 text-blue-400 uppercase font-black tracking-[0.2em]">
                    <th className="p-4 border-b border-white/5">
                      {t('francis.mivDistributor.hose.thTag')}
                    </th>
                    <th className="p-4 border-b border-white/5">
                      {t('francis.mivDistributor.hose.thExp')}
                    </th>
                    <th className="p-4 border-b border-white/5 text-right font-black">
                      {t('francis.mivDistributor.hose.thStatus')}
                    </th>
                  </tr>
                </thead>
                <tbody className="text-slate-300">
                  {[
                    {
                      tag: 'HOSE-MIV-01',
                      exp: t('francis.mivDistributor.hose.expJan25'),
                      status: 'crit',
                    },
                    {
                      tag: 'HOSE-MIV-02',
                    ...
                    {
                      tag: 'HOSE-HPU-05',
                      exp: t('francis.mivDistributor.hose.expJun27'),
                      status: 'ok',
                    },
                  ].map((row, idx) => (
                    <tr
                    ...
                    >
                      ...
                      <td
                        className={`p-4 text-right font-black uppercase tracking-tighter ${row.status === 'crit' ? 'text-red-500 animate-pulse' : 'text-emerald-500'}`}
                      >
                        {row.status === 'crit'
                          ? t('francis.mivDistributor.hose.crit')
                          : t('common.ok')}
                      </td>
                    </tr>
                  ))}
