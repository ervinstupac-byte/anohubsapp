...
export const FleetOverview: React.FC<FleetOverviewProps> = React.memo(({ onToggleMap, showMap, onRegisterAsset }) => {
    ...
    return (
        <div className="p-3 sm:p-4 border-b border-white/10 glass">
            <div className="flex justify-between items-center mb-3 sm:mb-4 gap-2">
                <h3 className="text-[10px] sm:text-xs font-header font-black uppercase tracking-wider sm:tracking-widest text-slate-400 truncate">{t('sidebar.fleetOverview')}</h3>
                <button
                    onClick={onToggleMap}
                    className={`text-[9px] sm:text-[10px] font-bold px-2 py-1 rounded-lg border transition-colors whitespace-nowrap ${showMap ? 'bg-brand-500/15 text-brand-300 border-brand-500/40' : 'bg-white/5 text-slate-500 border-white/10 hover:text-white'}`}
                >
                ...
                {assets.map(asset => (
                    <div key={asset.id} className="flex flex-col gap-1">
                        <button
                            ...
                            className={`
                                w-full flex items-center gap-3 px-3 py-2.5 rounded-xl border transition-all group relative overflow-hidden
                                ${selectedAsset?.id === asset.id
                                    ? 'bg-brand-500/10 border-brand-500/50 text-brand-300 font-bold'
                        ...
                        >
                            ...
                            <div className={`
                                w-2 h-2 rounded-full shrink-0
                                 ${asset.status === 'Critical' ? 'bg-rose-500 animate-pulse' :
                            ...
                            <div className="flex items-center gap-3 flex-1 min-w-0">
                                ...
                                <div className="flex items-end gap-1 h-3 opacity-75">
                                    {[1, 2, 3].map(i => (
                                        <div
                                            key={i}
                                            className={`w-[3px] rounded-full ${selectedAsset?.id === asset.id ? 'bg-brand-400' : 'bg-slate-600'}`}
                                            style={{
                                                height: `${40 + (i * 20)}%`
                                            }}
                                        />
                            ...
                            {/* Selected Indicator */}
                            {selectedAsset?.id === asset.id && (
                                <span className="text-[10px] text-brand-300 font-mono">ACTIVE</span>
                            )}
                        </button>
            ...
            <button
                ...
                className="w-full mt-3 py-2 px-2 flex items-center justify-center gap-1 sm:gap-2 border border-dashed border-white/15 rounded-xl text-[9px] sm:text-[10px] font-bold text-slate-500 hover:text-brand-300 hover:border-brand-400/50 hover:bg-brand-500/10 transition-all uppercase tracking-wider"
            >
