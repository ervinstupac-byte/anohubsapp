...
export const SimulationController: React.FC = () => {
  ...
  const runScenario = (type: DemoScenario) => {
    ...
    const targets: any = {
      ...
      WATER_HAMMER: {
        // Legacy support, mapped to STRUCTURAL_ANOMALY if needed or kept custom
        'physics.surgePressureBar': 145,
        'physics.waterHammerPressureBar': 85,
        'hydraulic.flow': 12.5,
      },
      BEARING_HAZARD: {
        // Renamed from BEARING_FAILURE
      ...
      CHRONIC_MISALIGNMENT: {
        // Mapped from GRID_LOSS or new
        'mechanical.rpm': 625,
        'mechanical.vibrationX': 7.2,
        'mechanical.vibrationY': 1.5,
      },
      STRUCTURAL_ANOMALY: {
        // Mapped from INFRASTRUCTURE_STRESS
          ...
          >
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex gap-2">
                <div
                  className={`w-2 h-2 rounded-full ${activeScenario ? 'bg-red-500 animate-pulse' : 'bg-emerald-500'}`}
                />
              </div>
            </div>
              ...
              {scenarios.map(s => (
                ...
                >
                  <div className="flex items-center gap-3">
                    <s.icon
                      className={`w-4 h-4 ${activeScenario === s.id ? `text-${s.color}-400` : 'text-slate-500'}`}
                    />
      ...
      <button
        ...
        className={`w-14 h-14 rounded-full flex items-center justify-center shadow-2xl transition-all ${
          isOpen
            ...
            : 'bg-cyan-500 text-black hover:scale-110 active:scale-95'
        }`}
      >
