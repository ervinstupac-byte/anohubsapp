    ...
    <div className="relative w-full h-[400px] bg-slate-950/50 rounded-xl overflow-hidden border border-white/5 shadow-inner">
      ...
      {plants.map(plant => (
        ...
        >
          ...
          {events.some(
            e =>
              (e.sourcePlant === plant.id || e.targetPlant === plant.id) &&
              Date.now() - e.timestamp < 2000
          ) && <div className="absolute inset-0 rounded-full bg-purple-500/30 animate-ping"></div>}

          <div
            className={`w-4 h-4 rounded-full border-2 ${plant.healthScore < 90 ? 'bg-amber-500 border-amber-300' : 'bg-cyan-500 border-cyan-300'} shadow-[0_0_20px_rgba(6,182,212,0.5)] transition-all group-hover:scale-125`}
          ></div>
          ...
          <div className="absolute top-6 left-1/2 -translate-x-1/2 bg-slate-900/90 border border-white/10 px-2 py-1 rounded text-center min-w-[120px] opacity-100 transition-opacity z-10">
            ...
            <div className="text-[9px] font-mono text-cyan-400">
              Eff: {plant.efficiency.toFixed(1)}% | RI: {plant.healthScore}
            </div>
          </div>
        </div>
      ))}
        ...
        <div className="bg-slate-900/40 rounded-xl border border-white/5 flex flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
            {hiveEvents.map(event => (
              <div
                key={event.id}
                className="bg-slate-950/50 p-3 rounded border border-white/5 animate-in slide-in-from-right-2"
              >
                <div className="flex justify-between items-start mb-1">
                  <span
                    className={`text-[9px] font-bold px-1.5 py-0.5 rounded ${event.type === 'WEIGHT_SYNC' ? 'bg-purple-500/20 text-purple-400' : 'bg-amber-500/20 text-amber-400'}`}
                  >
                ...
                <div className="mt-2 flex items-center text-[9px] text-slate-500 gap-2">
                  ...
                  <span className="text-slate-400">
                    {plants.find(p => p.id === event.targetPlant)?.name.split(' ')[1]}
                  </span>
                </div>
              </div>
            ))}
