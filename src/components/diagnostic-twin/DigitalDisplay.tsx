...
export const DigitalDisplay: React.FC<DigitalDisplayProps> = React.memo(
  ({ value, label, unit, color = 'cyan', className = '' }) => {
    ...
    return (
      <div
        className={`flex flex-col items-center bg-gradient-to-br ${bgClasses[color]} border-2 ${borderClasses[color]} rounded-xl p-4 backdrop-blur-md shadow-xl transition-all duration-300 hover:shadow-2xl hover:scale-[1.02] ${className}`}
      >
        <span className="text-xs font-black text-slate-400 uppercase tracking-[0.25em] mb-2">
          {label}
        </span>
