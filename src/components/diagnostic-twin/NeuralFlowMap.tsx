...
const TurbineUnit: React.FC<{
  ...
  ({ id, name, status, mw, eccentricity, vibration, onAlertClick, onComponentClick }) => {
    ...
    const colorMap = {
      ...
      orange:
        'border-amber-400 group-hover:border-amber-300 bg-gradient-to-br from-amber-500/10 via-orange-500/5 to-transparent shadow-[0_0_60px_rgba(245,158,11,0.2),inset_0_0_40px_rgba(245,158,11,0.1)]',
    ...
    return (
      <div className="relative group perspective-1000">
        <Tooltip
          content={`${name}: Eccentricity ${eccentricity.toFixed(3)} | Vibration ${(vibration * 1000).toFixed(1)}μm`}
        >
          <motion.div
          ...
          >
            ...
            <div
              className={`w-40 h-40 rounded-full border-[3px] border-dashed ${status === 'running' ? 'border-cyan-400/30 animate-[spin_8s_linear_infinite]' : 'border-white/5'}`}
            ></div>
...
export const NeuralFlowMap: React.FC = React.memo(() => {
  ...
  const handleOpenDossier = (path: string) => {
    ...
    if (!source) {
      ...
      source = DOSSIER_LIBRARY.find(
        d =>
          d.path.toLowerCase() === normalized ||
          `/archive/${d.path.replace(/^\/+/, '')}`.toLowerCase() === normalized
      ) as any;
      if (!source) {
        source = DOSSIER_LIBRARY.find(
          d =>
            normalized.endsWith((d.path || '').toLowerCase()) ||
            normalized.endsWith(('/archive/' + d.path).toLowerCase())
        ) as any;
      }
    }
    ...
    async function fetchSummary(p: string) {
      try {
        ...
        const extract = (label: string) => {
          const re = new RegExp(
            `<h[1-6][^>]*>\\s*${label}\\s*</h[1-6][^>]*>([\s\S]*?)(?:<h[1-6]|$)`,
            'i'
          );
        ...
        const recommendation =
          extract('Operational Recommendation') || extract('Recommendation') || null;
  ...
  return (
    <div
      ...
      className="flex-1 relative overflow-hidden flex flex-col items-center justify-center p-4 sm:p-6 md:p-12"
    >
      <div
      ...
      <div
        style={{ background: '#0b2530' }}
        className="relative z-10 w-full max-w-6xl border border-white/10 p-6 sm:p-8 md:p-16 rounded-[2.5rem] backdrop-blur-3xl shadow-[0_40px_100px_rgba(0,0,0,0.6)] overflow-hidden"
      >
        ...
        <div className="absolute top-4 left-4 sm:top-8 sm:left-8 flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-4 z-20">
          ...
          <div
            ...
            className="ml-3 hidden sm:inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-300 font-mono text-xs animate-pulse"
          >
        ...
        <Tooltip content={isoJustification}>
          <div
            className={`absolute top-4 right-4 sm:top-8 sm:right-8 text-[10px] font-mono px-5 py-2.5 rounded-full backdrop-blur-xl flex items-center gap-3 border transition-all z-20 cursor-help ${
              isoZone === 'ZONE_D'
        ...
        <div className="flex flex-col sm:flex-row justify-around items-center sm:items-end pt-16 sm:pt-20 pb-10 sm:pb-16 relative gap-16 sm:gap-8">
          <TurbineUnit
            ...
            onAlertClick={() =>
              handleOpenDossier(
                '/archive/turbine_friend/francis_h/francis_symptom_dictionary/index.html'
              )
            }
            onComponentClick={() =>
              handleOpenDossier(
          ...
          <div className="relative flex flex-col items-center justify-center sm:mx-4">
            ...
            {demoMode.active && (
              ...
              >
                <div className="text-[10px] text-amber-400 font-black uppercase tracking-widest text-center mb-1">
                  Fleet Health Delta
                </div>
        ...
        <div className="mt-6 flex gap-4 justify-center">
          <div
            className="p-3 bg-white/5 border border-emerald-400/10 rounded-lg text-center"
            style={{ color: '#00ff7f' }}
          >
            <div className="text-xs text-emerald-300 font-mono">Health (Global)</div>
            <div className="text-2xl font-black text-white">
              {intelReport ? `${intelReport.globalHealthIndex}%` : `${integrity.healthPercent}%`}
            </div>
          </div>
          <div className="p-3 bg-white/5 border border-cyan-400/10 rounded-lg text-center">
            ...
            <div className="text-2xl font-black text-white">
              {intelReport
                ? `${Math.round((intelReport.totalFiles || 0) * 2)}h`
                : `${integrity.mtbfEstimateHours}h`}
            </div>
          </div>
          <div className="p-3 bg-white/5 border border-red-400/10 rounded-lg text-center">
            ...
            <div className="text-2xl font-black text-white">
              {intelReport
                ? `${Math.round(intelReport.baselineAverageScore || 0)}`
                : `${integrity.riskScore}`}
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6 mt-12">
        <DigitalDisplay
          value={(hydraulic?.flow ?? 0).toFixed(1)}
          label="NOMINAL_FLOW"
          unit="m³/s"
          color="cyan"
        />
        <DigitalDisplay
          value={(hydraulic?.head ?? 0).toFixed(0)}
          label="GROSS_HEAD"
          unit="m"
          color="cyan"
        />
        <DigitalDisplay
          ...
          label="ECCENTRICITY"
          color={orbit.eccentricity > 0.8 ? 'red' : 'cyan'}
        />
        <Tooltip content={`ISO 10816-5 Protocol Compliance: ${isoJustification}`}>
          <div className="cursor-help">
            <DigitalDisplay
              value={(vibration.x * 1000).toFixed(1)}
              label="VIB_X_PEAK"
              unit="μm"
              color={vibration.x > 0.05 ? 'red' : 'cyan'}
            />
          </div>
        </Tooltip>
      </div>
      ...
      {selectedFile && (currentAnalysis || operationalRecommendation) && (
        <div className="max-w-3xl mt-8 p-6 bg-white/5 border border-white/10 rounded-2xl">
                ...
                <div className="mb-4 p-3 rounded border border-white/5 bg-black/5">
                  ...
                  <div className="flex items-center gap-4 mt-2">
                    ...
                    <div className="text-xs text-amber-300">
                      Maintenance: Required in {dueLabel}
                    </div>
                  </div>
                </div>
              );
          ...
          {currentAnalysis && (
            <div className="mb-4">
              <div className="text-xs text-amber-300 uppercase font-mono mb-1">
                Current Analysis
              </div>
          ...
          {operationalRecommendation && (
            <div>
              <div className="text-xs text-emerald-300 uppercase font-mono mb-1">
      ...
      <div
        className="fixed right-6 bottom-6 w-80 h-28 rounded-lg border border-white/10 bg-black/20 p-3 font-mono text-xs text-emerald-200 shadow-lg"
        style={{ background: '#021616' }}
      >
        <div className="uppercase text-[10px] font-black text-emerald-300 mb-1">
          Integrity Console
        </div>
      ...
      {diagnosticAlerts.length > 0 && (
        <div className="mt-6 p-5 bg-red-950/30 border border-red-500/30 rounded-2xl animate-pulse">
          <div className="flex items-center gap-3 mb-3">
            ...
            <h4 className="text-red-400 font-black uppercase text-sm tracking-widest text-white">
              {t('neuralFlow.detectionActive')}
            </h4>
          </div>
            ...
            {(intelReport.alerts || []).map((a: any, i: number) => (
              <div key={i} className="p-3 bg-white/5 border border-red-500/10 rounded-lg">
                ...
                <div className="text-xs text-amber-300 mt-1">
                  Due: {a.maintenanceDueDays ?? 'N/A'} days
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="mt-8 w-full max-w-4xl">
        <div className="flex gap-3 items-center mb-3">
          <button onClick={() => setTopicFilter('Runner')} className="px-3 py-1 bg-white/5 rounded">
            Runner
          </button>
          <button
            onClick={() => setTopicFilter('Stator/Generator')}
            className="px-3 py-1 bg-white/5 rounded"
          >
            Generator
          </button>
          <button onClick={() => setTopicFilter(null)} className="px-3 py-1 bg-white/5 rounded">
            All
          </button>
        </div>
        ...
        {topicFilter && intelReport && (
          <div className="p-4 bg-white/5 border rounded">
            ...
            {((intelReport.grouped && intelReport.grouped[topicFilter]) || []).length === 0 && (
              <div className="text-slate-400">No anomalies for this topic.</div>
            )}
            {((intelReport.grouped && intelReport.grouped[topicFilter]) || []).map(
              (r: any, idx: number) => (
                <div key={idx} className="mb-3 p-2 border-t border-white/5">
                  <div className="text-sm font-bold text-white">{r.title}</div>
                  <div className="text-xs text-slate-400">
                    {r.operationalRecommendation || r.currentAnalysis || r.path}
                  </div>
                  <div className="text-xs text-amber-300">
                    Score: {r.score} • Due: {r.maintenanceDueDays ?? 'N/A'}
                  </div>
                </div>
              )
            )}
