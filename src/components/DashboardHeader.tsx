...
export const DashboardHeader: React.FC<DashboardHeaderProps> = ({ onToggleSidebar, title }) => {
  ...
  const badgeLabel = t(
    `dashboard.riskStatus.${isCritical ? 'critical' : isWarning ? 'warning' : 'normal'}`
  );
    ...
    <>
      <header className="sticky top-0 z-30 h-16 border-b border-white/5 bg-slate-950/90 backdrop-blur-md flex items-center justify-between px-6 shadow-[0_4px_30px_rgba(0,0,0,0.5)]">
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-center gap-3 text-xs font-mono tracking-wider">
            ...
            <div className="relative">
              ...
              >
                <Database
                  className={`w-3.5 h-3.5 ${selectedAsset ? 'text-cyan-400' : 'text-slate-600'}`}
                />
                <div className="flex flex-col items-start">
                  <span className={`font-bold ${selectedAsset ? 'text-white' : 'text-slate-500'}`}>
                    {selectedAsset
                      ? selectedAsset.name.toUpperCase()
                      : t('header.fleetView', 'FLEET VIEW')}
                  </span>
                  {selectedAsset && (
                    <span className="text-[9px] text-cyan-400/70">
                      {(selectedAsset.turbine_type || selectedAsset.type || 'HPP').toUpperCase()} |{' '}
                      {selectedAsset.capacity?.toFixed(1) || '—'} MW
                    </span>
                  )}
                </div>
                <ChevronDown
                  className={`w-3 h-3 text-slate-500 transition-transform ${isQuickActionsOpen ? 'rotate-180' : ''}`}
                />
              </button>
              ...
              {isQuickActionsOpen && (
                <div className="absolute top-full left-0 mt-2 w-56 bg-slate-900/95 backdrop-blur-md border border-white/10 rounded-xl shadow-[0_0_40px_rgba(0,0,0,0.5)] z-50 overflow-hidden animate-scale-in">
                  <div className="p-2 border-b border-white/5">
                    <span className="text-[9px] text-slate-500 font-black uppercase tracking-widest px-2">
                      {t('header.quickActions', 'Quick Actions')}
                    </span>
                  </div>
                  <div className="p-1">
                    <button
                    ...
                    >
                      ...
                      <div>
                        <span className="text-sm font-bold">
                          {t('header.editSpecs', 'Edit Technical Specs')}
                        </span>
                        <p className="text-[9px] text-slate-500">
                          {t('header.editSpecsDesc', 'HPP Builder Studio')}
                        </p>
                      </div>
                    </button>
                    <button
                    ...
                    >
                      ...
                      <div>
                        <span className="text-sm font-bold">
                          {t('header.viewAnalytics', 'View Live Analytics')}
                        </span>
                        <p className="text-[9px] text-slate-500">
                          {t('header.viewAnalyticsDesc', 'Executive Dashboard')}
                        </p>
                      </div>
                    </button>
                    <button
                    ...
                    >
                      ...
                      <div>
                        <span className="text-sm font-bold">
                          {t('header.performMaintenance', 'Perform Maintenance')}
                        </span>
                        <p className="text-[9px] text-slate-500">
                          {t('header.performMaintenanceDesc', 'Field Toolbox')}
                        </p>
                      </div>
                    </button>
                  </div>
                  <div className="p-2 border-t border-white/5">
                    ...
                    >
                      ...
                      <span className="text-xs font-bold">
                        {t('header.viewFleet', 'View Full Fleet')}
                      </span>
                    </button>
                    <button
                      onClick={() => {
                    ...
                    >
                      ...
                      <span className="text-xs font-bold">
                        {t('header.forensicExport', 'Forensic Export')}
                      </span>
                    </button>
                  </div>
                </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold text-amber-500/90">{getMissionMode()}</span>
            </div>
            ...
            >
              ...
              <span
                className={`font-bold ${
                  hiveStatus?.connected ? 'text-emerald-500' : 'text-amber-400'
                }`}
              >
        ...
        <div className="flex items-center gap-4">
          ...
          >
            <div
              className={`w-5 h-5 rounded-full ${themeMode === 'tactical-red' ? 'bg-red-500' : 'bg-slate-500'}`}
            />
          </button>
          ...
          <button
            ...
            title={
              isCommanderMode ? 'Switch to Developer View' : 'Switch to Commander Mode (NC-9.0)'
            }
          ...
          <button
            ...
            title={
              isHeritageCertified
                ? 'Heritage Certified - Tier 1 Asset'
                : 'Heritage Certification Pending'
            }
          >
            <ShieldCheck
              className={`w-5 h-5 ${isHeritageCertified ? 'animate-pulse' : 'opacity-40'}`}
            />
            {isHeritageCertified && (
          ...
          <div
            ...
            className={`cursor-pointer hover:scale-105 transition-transform relative ${
              badgeStatus === 'critical' ? 'animate-critical-flash rounded-lg' : ''
            }`}
            title={`Risk Status: ${badgeLabel}`}
          >
            {badgeStatus === 'critical' && (
              <div className="absolute -inset-1 rounded-lg border border-red-500/50 animate-ping opacity-50" />
            )}
            <DigitalPanel
