  ...
  return (
    <div className="space-y-2">
      ...
      <div className="text-xs text-slate-400">
        Precision: 0.01{unit} increments | Range: {min.toFixed(2)}-{max.toFixed(2)}
        {unit}
      </div>
    </div>
  );
...
export const DigitalSignatureInput: React.FC<DigitalSignatureInputProps> = ({
  ...
  return (
    <div className="space-y-4 p-4 bg-slate-900 border border-slate-700 rounded">
      ...
      <div>
        <label className="block text-sm font-bold text-white mb-2">Engineer Name</label>
        <input
          type="text"
          value={engineerName}
      ...
      <div>
        <label className="block text-sm font-bold text-white mb-2">Engineer License Number</label>
        <input
          type="text"
      ...
      <div className="text-xs text-slate-400 p-2 bg-blue-950/20 border border-blue-500/30 rounded">
        <strong className="text-blue-300">Note:</strong> Measurements will be cryptographically
        signed with SHA-256 hash for audit trail integrity.
      </div>
    </div>
  );
};
...
export const MeasurementMethodSelector: React.FC<MeasurementMethodSelectorProps> = ({
  ...
  return (
    <div className="space-y-2">
      <label className="block text-sm font-bold text-white">Measurement Method</label>
      <div className="grid grid-cols-2 gap-3">
        {methods.map(m => (
          <button
            key={m.value}
            onClick={() => onChange(m.value)}
            className={`py-3 px-4 rounded font-bold transition-all flex items-center gap-2 justify-center ${
              method === m.value
