      ...
      content: (
        <div className="space-y-6">
          <p className="leading-relaxed text-slate-300 font-light text-lg">
            <Trans
              i18nKey="genderEquity.strategy.impContent"
        ...
        <div className="space-y-4">
          <div className="grid gap-4">
            <div className="bg-slate-900/40 p-5 rounded-xl border border-white/5 hover:border-cyan-500/30 transition-colors">
              <strong className="block text-cyan-400 mb-2 text-sm uppercase tracking-wider">
                {t('genderEquity.strategy.card1Title')}
              </strong>
              <span className="text-slate-300 text-sm leading-relaxed">
                <Trans
                  i18nKey="genderEquity.strategy.card1Content"
            ...
            <div className="bg-slate-900/40 p-5 rounded-xl border border-white/5 hover:border-cyan-500/30 transition-colors">
              <strong className="block text-cyan-400 mb-2 text-sm uppercase tracking-wider">
                {t('genderEquity.strategy.card2Title')}
              </strong>
              <span className="text-slate-300 text-sm leading-relaxed">
                <Trans
                  i18nKey="genderEquity.strategy.card2Content"
                  components={{ strong: <strong className="text-white" /> }}
                />
              </span>
            </div>
      ...
      <div className="relative text-center space-y-4 pt-6">
        ...
        <div className="pt-8">
          <h2 className="text-4xl md:text-6xl font-black text-white tracking-tighter mb-4">
            {t('genderEquity.title').split(' ')[0]}{' '}
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-500">
              {t('genderEquity.title').split(' ').slice(1).join(' ')}
            </span>
          </h2>
          ...
          {STRATEGY_SECTIONS.map(section => (
            ...
            >
              ...
              >
                <div>
                  <h3
                    className={`text-xl font-bold transition-colors ${openSectionId === section.id ? 'text-white' : 'text-slate-300'}`}
                  >
                    {section.title}
                  </h3>
                ...
                <span
                  className={`text-2xl transition-transform duration-300 ${openSectionId === section.id ? 'rotate-180 text-cyan-400' : 'text-slate-500'}`}
                >
                  ▼
                </span>
              </button>
