...
export const StructuralIntegrity: React.FC = () => {
  ...
  const [selectedAssetId, setSelectedAssetId] = useState<string>(
    assets[0] ? idAdapter.toStorage(assets[0].id) : ''
  );

  const assetTele = telemetry[String(selectedAssetId)];

  const relevantDiagnoses = activeDiagnoses.filter(
    d =>
  ...
  return (
    <div className="p-8 space-y-8 animate-fade-in max-w-7xl mx-auto pb-24">
      ...
      <div className="flex justify-between items-end">
        ...
        >
          {assets.map(a => (
            <option key={a.id} value={idAdapter.toStorage(a.id)}>
              {a.name}
            </option>
          ))}
        </select>
          ...
          <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="p-6 bg-white/5 rounded-3xl border border-white/5">
              ...
              <p className="text-3xl font-black text-white">
                {assetTele.piezometricPressure.toFixed(2)}{' '}
                <span className="text-sm opacity-40">bar</span>
              </p>
              <div className="mt-4 flex items-center gap-2">
                <div
                  className={`w-2 h-2 rounded-full ${assetTele.piezometricPressure > 4.5 ? 'bg-red-500 animate-pulse' : 'bg-emerald-500'}`}
                />
                <span className="text-[9px] text-slate-400 font-bold uppercase">
                  {assetTele.piezometricPressure > 4.5
                    ? 'High Seepage Potential'
                    : 'Nominal Saturation'}
                </span>
              </div>
            </div>
            ...
            <div className="p-6 bg-white/5 rounded-3xl border border-white/5">
              ...
              <p className="text-3xl font-black text-white">
                {assetTele.seepageRate.toFixed(1)} <span className="text-sm opacity-40">l/min</span>
              </p>
            </div>
            ...
            <div className="p-6 bg-white/5 rounded-3xl border border-white/5">
              ...
              <p className="text-3xl font-black text-cyan-400">
                {assetTele.reservoirLevel.toFixed(1)} <span className="text-sm opacity-40">m</span>
              </p>
            </div>
          </div>
          ...
          <div className="mt-8 h-48 bg-black/20 rounded-2xl border border-white/5 relative overflow-hidden flex items-end px-4 gap-1">
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <span className="text-[10px] text-white/20 font-black uppercase tracking-widest">
                Real-time Piezometric Drift Analysis
              </span>
            </div>
            {/* ALERT OVERLAY */}
            {assetTele.piezometricPressure > 4.5 && (
          ...
          <div className="mt-6 space-y-8">
            <div className="text-center p-8 bg-indigo-500/5 rounded-[2.5rem] border border-indigo-500/10 relative overflow-hidden">
              ...
              <p className="text-5xl font-black text-white">
                {assetTele.foundationDisplacement.toFixed(3)}
              </p>
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-end">
                <span className="text-[10px] text-slate-500 font-black uppercase">
                  Foundation vs Rotor Correlation
                </span>
                <span
                  className={`text-[10px] font-black ${assetTele.foundationDisplacement > 0.1 ? 'text-orange-400' : 'text-emerald-400'}`}
                >
                  {assetTele.foundationDisplacement > 0.1
                    ? 'DECOUPLED VIBRATION'
                    : 'COHERENT SYSTEM'}
                </span>
              </div>
              ...
              <p className="text-[9px] text-slate-500 leading-relaxed italic">
                High coherence detected. Current vibration profile matches mechanical imbalance
                footprint, not structural concrete failure.
              </p>
            </div>
          </div>
        </GlassCard>
          ...
          <div className="mt-4 space-y-4">
            {relevantDiagnoses.length > 0 ? (
              relevantDiagnoses.map((diag, i) => (
                <div
                  key={i}
                  className="p-4 bg-red-500/5 border border-red-500/20 rounded-2xl flex gap-4 items-start"
                >
                  ...
                  <div>
                    <h4 className="text-white font-black uppercase text-xs mb-1">
                      {diag.diagnosis?.diagnosis || 'Critical structural anomaly'}
                    </h4>
                    <p className="text-[10px] text-slate-400 leading-relaxed">{diag.message}</p>
                    <div className="mt-3 flex gap-2">
                      <span className="px-2 py-0.5 bg-red-500 text-white text-[8px] font-black rounded">
                        ACTION REQUIRED
                      </span>
                      <span className="px-2 py-0.5 bg-white/5 text-slate-400 text-[8px] font-black rounded uppercase">
                        Source: {diag.source}
                      </span>
                    </div>
                  </div>
              ...
              <div className="p-8 text-center bg-emerald-500/5 border border-emerald-500/10 rounded-3xl">
                <p className="text-sm text-emerald-400 font-black uppercase">
                  Concrete & Dam Integrity Optimal
                </p>
                <p className="text-[10px] text-slate-500 mt-2 uppercase font-bold tracking-widest">
                  Zero micro-drift detected across all foundations
                </p>
              </div>
            )}
          </div>
        ...
        <div className="p-8 rounded-[2.5rem] bg-indigo-600/10 border border-indigo-500/20 flex flex-col justify-between group overflow-hidden relative">
          ...
          <div>
            ...
            <p className="text-sm text-slate-400 leading-relaxed max-w-sm">
              Generate a cryptographically signed structural health report for regulatory
              compliance. Includes piezometric stability history and seismic sensor logs.
            </p>
          </div>
          <div className="mt-8 flex gap-4">
            <ModernButton variant="primary" className="h-12 px-8">
              GENERATE REPORT
            </ModernButton>
            <div className="flex flex-col justify-center">
              <span className="text-[10px] text-indigo-400 font-black uppercase mb-0.5">
                Verification Chain
              </span>
