...
export const InvestorBriefing: React.FC = () => {
  ...
  useEffect(() => {
    ...
    let annualGenerationGWh =
      currentDesign?.calculations.energyGWh || (powerMW * 8760 * 0.5) / 1000;
  ...
  const handleDownloadReport = () => {
    const assetName =
      selectedAsset?.name ||
      (currentDesign
        ? t('investorBriefing.hppConceptDesign', 'HPP Concept Design')
        : t('investorBriefing.genericProject', 'Generic Project'));
    ...
    ForensicReportService.openAndDownloadBlob(
      blob,
      `AnoHUB_Financial_Briefing_${assetName}.pdf`,
      true,
      {
        assetId: selectedAsset ? idAdapter.toDb(selectedAsset.id) : undefined,
        projectState: {
          market: { energyPrice: params.electricityPrice * 1000 },
          hydraulic: { baselineOutputMW: kpis.powerMW },
        },
  ...
  const handleDownloadIncidentReport = () => {
    ...
    ForensicReportService.openAndDownloadBlob(
      blob,
      `INCIDENT_REPORT_${selectedAsset.name}.pdf`,
      true,
      {
      ...
      <div className="text-center space-y-6 pt-6">
        ...
        <div>
          <h2 className="text-4xl md:text-5xl font-black text-white tracking-tighter mb-4">
            {t('investorBriefing.title', 'Investor Briefing').split(' ')[0]}{' '}
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-500">
              {t('investorBriefing.title', 'Investor Briefing').split(' ')[1]}
            </span>
          </h2>
          ...
          >
            <div className="space-y-4">
              <ModernInput
                ...
                onChange={e =>
                  setParams({ ...params, electricityPrice: parseFloat(e.target.value) || 0 })
                }
                icon={<span>€</span>}
              />
              <ModernInput
                ...
                onChange={e =>
                  setParams({ ...params, interestRate: parseFloat(e.target.value) || 0 })
                }
                icon={<span>%</span>}
              />
              <ModernInput
            ...
            <div className="mt-6 p-4 rounded-xl border flex items-start gap-3 transition-colors bg-slate-900/50 border-slate-700">
              <div className="text-2xl mt-1">
                {selectedAsset ? '🏭' : currentDesign ? '⚡' : '📝'}
              </div>
              <div>
                <h4 className="text-slate-300 font-bold text-xs uppercase tracking-wider mb-1">
                  {selectedAsset
                    ? t('investorBriefing.activeAssetData', 'Active Asset Data')
                    : currentDesign
                      ? t('investorBriefing.designLink', 'HPP Design Studio Link')
                      : t('investorBriefing.genericModel', 'Generic Model')}
                </h4>
                <p className="text-xs text-slate-400 leading-relaxed">
                  <Trans
                    i18nKey="investorBriefing.calculatingBasedOn"
                    values={{ power: kpis.powerMW.toFixed(1), energy: kpis.energyGWh.toFixed(1) }}
                  >
                    Calculating based on <strong className="text-white ml-1">0 MW</strong> capacity
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <GlassCard>
              <p className="text-xs font-bold text-emerald-400 uppercase tracking-widest mb-2">
                {t('investorBriefing.annualRevenue')}
              </p>
            ...
            <GlassCard>
              <p className="text-xs font-bold text-blue-400 uppercase tracking-widest mb-2">
                {t('investorBriefing.estimatedCapex', 'Estimated CAPEX')}
              </p>
              <div className="text-3xl font-black text-white mb-1">
                €{(kpis.capex / 1000000).toFixed(1)}M
              </div>
              <p className="text-xs text-slate-500">
                {t('investorBriefing.capexBasis', 'Based on €1.8M/MW avg.')}
              </p>
            </GlassCard>
            ...
            <GlassCard className="border-cyan-500/20 bg-cyan-950/10">
              <p className="text-xs font-bold text-cyan-400 uppercase tracking-widest mb-2">
                Stored Asset Capital
              </p>
              <div className="text-3xl font-black text-white mb-1">
                €{(getTotalInventoryValue() / 1000).toFixed(1)}k
              </div>
              ...
              <div>
                ...
                <div className="h-2 bg-slate-800 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{ width: `${Math.min((kpis.opex / kpis.revenue) * 100, 100)}%` }}
                  ></div>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-4 w-full md:w-auto">
              ...
              {activeIncident && activeIncident.assetId === selectedAsset?.id && (
                <ModernButton
                  ...
                  variant="secondary"
                  className="min-w-[220px] border-red-500 text-red-500 hover:bg-red-500 hover:text-white shadow-red-500/20 animate-pulse"
                  icon={<span>🚨</span>}
                  fullWidth
                >
