  ...
  const insights = useMemo(() => {
    ...
    if (sag > 0.02) {
      reports.push({
        ...
        detail:
          'Prekomjerno trošenje donje polovine blazine ležaja (Radial babbitt wear detected).',
      });
    } else if (sag > 0.015) {
      reports.push({
        id: 'shaft_sag',
    ...
    if (responseLag > 0.5 && oilTemp > 50) {
      reports.push({
        ...
        detail:
          'Ulje gubi projektovana svojstva - hitna filtracija ili zamjena (Oxidation/Emulsion suspected).',
      });
              ...
              >
                <div className="flex justify-between items-start mb-1">
                  <div className="flex items-center gap-2">
                    <div
                      className={`w-2 h-2 rounded-full ${item.status === 'CRITICAL' ? 'bg-red-500' : 'bg-amber-500 animate-pulse'}`}
                    />
                    <h4 className="text-[9px] font-black text-white uppercase tracking-tighter">
                      {item.label}
                    </h4>
                  </div>
          ...
          <div className="bg-black/20 p-2 rounded-lg border border-white/5">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-mono text-white">
                {(assetTele.shaftSag * 1000).toFixed(1)} µm
              </span>
            </div>
          </div>
          <div className="bg-black/20 p-2 rounded-lg border border-white/5">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-[9px] font-mono text-slate-400">
                {(1 - assetTele.responseTimeIndex).toFixed(2)}
              </span>
            </div>
          </div>
        </div>
