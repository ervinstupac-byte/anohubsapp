...
import {
  GalvanicCorrosionService,
  CathodicProtectionSystem,
} from '../services/GalvanicCorrosionService';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="mb-6">
        ...
        <p className="text-sm text-slate-400">Cathodic Protection - {system.turbineType} Turbine</p>
      </div>

      {/* System Status */}
      <GlassCard
        className={`p-6 border-2 ${
          system.overallProtection === 'EXCELLENT'
            ? 'border-emerald-500 bg-emerald-950/20'
            : system.overallProtection === 'GOOD'
              ? 'border-green-500 bg-green-950/20'
              : system.overallProtection === 'MARGINAL'
      ...
      >
        <div className="flex items-center justify-between">
          ...
          <Shield
            className={`w-16 h-16 ${
              system.overallProtection === 'EXCELLENT' || system.overallProtection === 'GOOD'
                ? 'text-emerald-400'
                : 'text-amber-400'
            }`}
          />
      ...
      <div className="grid grid-cols-3 gap-4">
        <GlassCard
          className={`p-4 ${
            system.averageVoltage < -800 ? '' : 'border-2 border-amber-500 bg-amber-950/20'
          }`}
        >
        ...
        <GlassCard
          className={`p-4 ${
            system.generatorGroundingResistance > 1.0 ? 'border-2 border-red-500 bg-red-950/20' : ''
          }`}
        >
          <div className="flex items-center gap-2 mb-2">
          ...
          <p
            className={`text-3xl font-black ${
              system.generatorGroundingResistance > 1.0 ? 'text-red-400' : 'text-white'
            }`}
          >
        ...
        <GlassCard
          className={`p-4 ${
            system.strayCurrentDetected ? 'border-2 border-red-500 bg-red-950/20' : ''
          }`}
        >
          <div className="flex items-center gap-2 mb-2">
            <AlertTriangle
              className={`w-4 h-4 ${system.strayCurrentDetected ? 'text-red-400' : 'text-emerald-400'}`}
            />
            <p className="text-xs text-slate-400 uppercase font-bold">Stray Current</p>
          </div>
          <p
            className={`text-2xl font-black ${system.strayCurrentDetected ? 'text-red-400' : 'text-emerald-400'}`}
          >
            {system.strayCurrentDetected ? 'DETECTED' : 'NONE'}
          </p>
          <p className="text-xs text-red-300">
            {system.strayCurrentDetected ? '⚠️ Critical' : '✓ Good'}
          </p>
        </GlassCard>
      </div>
      ...
      <GlassCard className="p-6">
        <h3 className="text-lg font-black text-white mb-4">
          Zinc Anode Status ({system.anodes.length} units)
        </h3>
