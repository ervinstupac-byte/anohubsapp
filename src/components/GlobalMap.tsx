  ...
  return (
    <div className="relative h-full w-full overflow-hidden bg-slate-950 isolate">
      ...
      <div className="absolute top-4 left-4 right-4 z-[1000] flex justify-between items-start pointer-events-none">
        ...
        <div className="flex gap-3 pointer-events-auto">
          ...
          <GlassCard className="py-2 px-4 flex flex-col items-center min-w-[100px] bg-slate-900/90 backdrop-blur-xl border-slate-700">
            <span className="text-[10px] text-slate-400 uppercase font-bold tracking-widest">
              {t('globalMap.liveMw', 'Live MW')}
            </span>
            <span className="text-xl font-black text-cyan-400">{stats.power}</span>
          </GlassCard>
          <GlassCard
            className={`py-2 px-4 flex flex-col items-center min-w-[80px] bg-slate-900/90 backdrop-blur-xl ${stats.critical > 0 ? 'border-red-500 shadow-[0_0_15px_rgba(239,68,68,0.4)]' : 'border-slate-700'}`}
          >
            <span className="text-[10px] text-slate-400 uppercase font-bold tracking-widest">
              {t('globalMap.alerts', 'Alerts')}
            </span>
            <span
              className={`text-xl font-black ${stats.critical > 0 ? 'text-red-500 animate-pulse' : 'text-emerald-500'}`}
            >
              {stats.critical}
            </span>
          </GlassCard>
      ...
      <div className="absolute top-24 left-6 z-[1000] w-80 space-y-4 pointer-events-auto">
        <GlassCard
          variant="commander"
          className="bg-slate-900/60 backdrop-blur-2xl border-cyan-500/20 shadow-[0_0_50px_rgba(6,182,212,0.1)]"
        >
            ...
            {['UNIT_01', 'UNIT_02', 'UNIT_03'].map((unit, idx) => (
              ...
              >
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span
                    className={`text-[10px] font-mono font-bold ${idx === 1 ? 'text-emerald-400' : 'text-cyan-400'}`}
                  >
            ...
            <div className="flex items-start gap-3 bg-emerald-500/10 border border-emerald-500/20 p-3 rounded-xl">
              ...
              <div>
                ...
                <p className="text-[10px] text-slate-300 leading-relaxed italic">
                  "Unit 02 has the lowest Fatigue Index; prioritize it for high-load grid
                  stabilization to preserve Unit 01."
                </p>
              </div>
            </div>
          </div>
        ...
        {assets.map(asset => {
          ...
          const statusDisplay = liveData?.status
            ? liveData.status === 'CRITICAL'
              ? t('globalMap.status.critical')
              : liveData.status === 'WARNING'
                ? t('globalMap.status.warning')
                : liveData.status
            : t('globalMap.syncing');
          ...
          return (
            ...
            >
              <Popup className="custom-popup" closeButton={false}>
                <div
                  className={`bg-slate-900 text-white p-4 rounded-xl border shadow-2xl min-w-[260px] relative overflow-hidden ${
                    isCritical
                ...
                >
                  ...
                  <div className="grid grid-cols-2 gap-2 mb-4 pl-2">
                    <div className="bg-slate-800/50 p-2 rounded border border-white/5">
                      <div className="text-[9px] text-slate-500 uppercase font-bold mb-0.5">
                        {t('globalMap.vibration', 'Vibration')}
                      </div>
                      <div
                        className={`text-sm font-mono font-bold ${isCritical ? 'text-red-400' : 'text-white'}`}
                      >
                        {liveData?.vibration.toFixed(3) || '0.00'}{' '}
                        <span className="text-[9px] text-slate-500">mm/s</span>
                      </div>
                    </div>
                    <div className="bg-slate-800/50 p-2 rounded border border-white/5">
                      <div className="text-[9px] text-slate-500 uppercase font-bold mb-0.5">
                        {t('globalMap.temp', 'Temp')}
                      </div>
