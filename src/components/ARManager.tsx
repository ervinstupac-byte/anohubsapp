...
export const ARManager: React.FC = () => {
  ...
  const drawOverlay = useCallback(() => {
    ...
    if (mode === 'RECOGNITION') {
      ...
      ctx.fillStyle = '#22d3ee';
      ctx.fillText(
        `↓ HYDRAULIC FLOW (${assetTele?.cylinderPressure.toFixed(1) || 45} bar)`,
        arrowX + 5,
        arrowY
      );
    ...
    if (mode === 'FORENSICS_REPLAY' && frozenBuffer) {
      ...
      frozenBuffer.forEach((point, i) => {
        ...
        ctx.strokeStyle =
          point.dpdt > 10 ? `rgba(220, 38, 38, ${opacity})` : `rgba(34, 211, 238, ${opacity})`;
        ctx.beginPath();
      ...
      <div className="absolute inset-0 flex flex-col z-20 pointer-events-none">
          ...
          <div className="space-y-1">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-xl font-black text-white uppercase tracking-tighter">
                AR FIELD GUIDE <span className="text-cyan-400">v2.0</span>
              </h2>
            </div>
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest pl-6">
              Spatial AI Inspection Module
            </p>
          </div>
        ...
        <main className="flex-grow flex items-center justify-center">
          ...
          {isRemoteCalling && (
            <div className="absolute top-24 right-6 w-48 aspect-video bg-slate-900/80 border border-cyan-500/30 rounded-xl overflow-hidden pointer-events-auto">
              <div className="absolute top-2 left-2 px-2 py-0.5 bg-cyan-500 rounded text-[8px] font-black text-black">
                REMOTE: ING. STEVAN
              </div>
              <div className="w-full h-full flex flex-col items-center justify-center gap-2">
                <span className="text-2xl">👨‍🔧</span>
                <div className="text-[8px] text-cyan-400 font-mono font-bold uppercase">
                  Streaming live feed...
                </div>
              </div>
            </div>
        ...
        <footer className="p-6 pointer-events-auto bg-gradient-to-t from-black/80 to-transparent">
          ...
          <div className="mt-4 p-4 bg-slate-950/80 border border-white/5 rounded-2xl flex items-center gap-4">
            <div className="w-10 h-10 bg-cyan-500/10 rounded-full flex items-center justify-center text-cyan-400 text-xl border border-cyan-500/20">
              🤖
            </div>
            <div className="flex-grow">
              <p className="text-[10px] text-slate-500 font-black uppercase mb-1">
                Ano-Agent Visual Analysis
              </p>
              <p className="text-xs text-white leading-tight italic">
                {mode === 'RECOGNITION' &&
                  'Component identified. Pressure levels matching Telemetry (45.2 bar). Flow direction nominal.'}
                {mode === 'ASSEMBLY' &&
                  'CRITICAL ALERT: Wrong pipe diameter (16mm) detected on primary circuit. Risks hydraulic hammer.'}
                {mode === 'GAUGE' &&
                  'Synchronizing visual sensor reading with ProjectContext... (45 bar captured).'}
                {mode === 'SPATIAL' && spatialDeviation > 0.05
                  ? 'TOLERANCE EXCEEDED. Alignment deviation: >0.05mm. Adjust base bolts immediately.'
                  : 'Spatial alignment stable.'}
              </p>
            </div>
          </div>
        </footer>
      ...
      {isRemoteCalling && (
        <div className="absolute inset-0 z-40 pointer-events-none">
          <svg className="w-full h-full">
            ...
            <text x="45%" y="30%" className="fill-red-500 text-[10px] font-bold uppercase">
              ING: LOOSEN THIS BOLT FIRST
            </text>
          </svg>
        </div>
      )}
