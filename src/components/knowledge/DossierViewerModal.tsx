...
export const DossierViewerModal: React.FC<DossierViewerModalProps> = ({
  ...
  const normalizedPath = resolvedPathCandidate.startsWith('/')
  ...
  const activePath = (() => {
    if (normalizedPath.includes('AnoHub_site')) {
      return normalizedPath
        .replace('/src/AnoHub_site/', '/docs/archive/')
        .replace('/AnoHub_site/', '/docs/archive/');
    }
    return normalizedPath;
  })();
    ...
    navigate('/precision-audit', {
      state: {
        sourceFile: sourceData || {
          path: filePath,
          title: title,
          justification: 'Direct audit from viewer.',
        },
      },
    });
  };
                  ...
                  {isChecking ? (
                    ...
                    <div className="flex flex-col items-center justify-center h-full text-center p-8">
                      ...
                      <p className="text-slate-400 max-w-md">
                        The requested file could not be located in the secure archive. It may have
                        been expunged or moved to cold storage.
                      </p>
                    </div>
                  ) : (
                      ...
                      <div className="p-6 space-y-8 overflow-y-auto custom-scrollbar">
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="flex flex-wrap gap-2">
                            {['ENGINEERING', 'AUDIT', 'COMPLIANCE', '2024'].map(tag => (
                              <span
                                key={tag}
                                className="px-2 py-1 rounded bg-slate-800 border border-slate-700 text-[10px] text-slate-300 font-mono"
                              >
                                #{tag}
                              </span>
                            ))}
                        ...
                        <div className="p-4 rounded-xl bg-emerald-950/20 border border-emerald-500/20">
                          ...
                          <p className="text-[10px] text-emerald-200/60 leading-relaxed">
                            This document's integrity has been cryptographically verified against
