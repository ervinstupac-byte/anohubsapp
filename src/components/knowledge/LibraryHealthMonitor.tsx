...
export const LibraryHealthMonitor: React.FC = () => {
  ...
  const [auditLog, setAuditLog] = useState<
    Array<{ name: string; status: 'SUCCESS' | 'ERROR'; hash?: string }>
  >([]);
        ...
        batch.map(async file => {
          ...
          try {
            ...
            if (!response || !response.ok) {
              ...
              if (response) {
                (response as any)._fetchedUrl = response.ok
                  ? lowerUrl
            ...
            if (response && response.ok) {
              ...
              return {
                ok: true,
                name: file.path,
                embedded: embedded || 'NOT_FOUND',
                computed,
                verified,
                fetchedUrl: (response as any)._fetchedUrl,
              };
            }
      ...
      <div className="flex justify-between items-end border-b border-white/5 pb-6">
        ...
        <div className="text-right">
          ...
          <div className="flex items-center gap-2 px-3 py-1 bg-emerald-500/10 border border-emerald-500/20 rounded-full">
            ...
            <span className="text-[10px] font-black text-emerald-500 font-mono tracking-widest">
              ENCRYPTED_DATA_BRIDGE_ACTIVE
            </span>
          </div>
        </div>
      </div>
            ...
            <div className="space-y-8">
              <div>
                ...
                <div className="relative pt-10 pb-20 flex flex-col items-center">
                  ...
                  <div className="relative w-48 h-48 border-4 border-white/5 rounded-full flex items-center justify-center p-4">
                    <div
                      className={`absolute inset-0 border-4 ${isAuditing ? 'border-h-gold' : 'border-h-cyan'} border-t-transparent rounded-full animate-[spin_3s_linear_infinite]`}
                    />
                    <div className="text-center">
                      <div className="text-4xl font-black text-white">
                        {isAuditing ? auditProgress : auditResults?.real || totalFiles}
                      </div>
                      <div
                        className={`text-[10px] font-mono ${isAuditing ? 'text-h-gold' : 'text-h-cyan'} uppercase tracking-tighter`}
                      >
                  ...
                  <div className="flex justify-between mt-2 text-[9px] font-mono font-bold">
                    <span className={auditResults?.missing ? 'text-h-red' : 'text-h-cyan'}>
                      {auditResults
                        ? `${auditResults.real} REAL / ${auditResults.missing} MISSING`
              ...
              <div className="p-4 bg-h-cyan/5 border border-h-cyan/20 rounded-xl space-y-2">
                <p className="text-[10px] leading-relaxed italic text-slate-400">
                  "50 Hardened Engineering Dossiers (IEC 60041 Compliant) now active. All legacy
                  technical journals, NDT reports, and mechanical core data are synchronized into
                  the neural bridge."
                </p>
              </div>
            </div>
            ...
            {auditResults && (
              <div className="mt-6 pt-6 border-t border-white/5 grid grid-cols-2 gap-4">
                <div className="p-3 bg-emerald-500/10 border border-emerald-500/20 rounded-xl">
                  <div className="text-[9px] font-mono text-emerald-500 uppercase mb-1">
                    Active Assets
                  </div>
                ...
                <div className="p-3 bg-h-red/10 border border-h-red/20 rounded-xl">
                  <div className="text-[9px] font-mono text-h-red uppercase mb-1">
                    Missing / Simulated
                  </div>
          ...
          <GlassCard variant="commander" className="border-white/5">
            ...
            <div className="flex items-center justify-between relative px-8 pb-12">
              ...
              <div className="relative z-10 flex flex-col items-center gap-4">
                ...
                >
                  <Search
                    className={`w-8 h-8 ${scanState === 'scanning' ? 'text-orange-400' : 'text-slate-600'}`}
                  />
                </motion.div>
                <span
                  className={`text-[10px] font-black uppercase tracking-widest ${scanState === 'scanning' ? 'text-orange-400' : 'text-slate-500'}`}
                >
                  1. Scanning
                </span>
              </div>
              ...
              <div className="relative z-10 flex flex-col items-center gap-4">
                ...
                >
                  <Activity
                    className={`w-8 h-8 ${scanState === 'validating' ? 'text-h-cyan' : 'text-slate-600'} ${scanState === 'validating' ? 'animate-pulse' : ''}`}
                  />
                </motion.div>
                <span
                  className={`text-[10px] font-black uppercase tracking-widest ${scanState === 'validating' ? 'text-h-cyan' : 'text-slate-500'}`}
                >
                  2. Validating (ISO)
                </span>
              </div>
              ...
              <div className="relative z-10 flex flex-col items-center gap-4">
                ...
                >
                  <CheckCircle2
                    className={`w-8 h-8 ${scanState === 'integrated' ? 'text-h-gold' : 'text-slate-600'}`}
                  />
                </motion.div>
                <span
                  className={`text-[10px] font-black uppercase tracking-widest ${scanState === 'integrated' ? 'text-h-gold' : 'text-slate-500'}`}
                >
                  3. Integrated
                </span>
              </div>
              ...
              <div className="space-y-3">
                {[
                  ...
                  >
                    ...
                    <span
                      className={`text-[9px] font-black px-2 py-0.5 rounded cursor-default ${item.status === 'ACTIVE' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-orange-500/20 text-orange-400'}`}
                    >
                      {item.status}
                    </span>
                  </div>
              ...
              <div className="space-y-1 h-64 overflow-y-auto custom-scrollbar pr-2">
                {auditLog.length > 0 ? (
                  auditLog.map((log, i) => (
                    <div
                      key={i}
                      className="flex flex-col py-2 border-b border-white/5 last:border-0 hover:bg-white/5 px-2 rounded transition-colors group"
                    >
                      <div className="flex items-center gap-3">
                        <div
                          className={`w-1.5 h-1.5 rounded-full ${log.status === 'SUCCESS' ? 'bg-h-cyan' : 'bg-red-500'}`}
                        />
                        <span className="text-[10px] text-slate-400 font-mono truncate max-w-[150px]">
                          {log.name}
                        </span>
                        <span
                          className={`text-[8px] font-black px-1.5 py-0.5 rounded ml-auto ${log.status === 'SUCCESS' ? 'text-emerald-400 bg-emerald-500/10' : 'text-red-400 bg-red-500/10'}`}
                        >
                          {log.status}
                        </span>
                      </div>
                      <div className="mt-1 flex items-center gap-2 pl-4">
                        ...
                        <span className="text-[8px] font-mono text-h-cyan truncate">
                          {(log as any).hash}
                        </span>
                        {(log as any).computed && (
                          <>
                        ...
                        <span
                          className={`ml-auto text-[9px] font-black px-1.5 py-0.5 rounded ${(log as any).verified ? 'bg-emerald-500/10 text-emerald-400' : 'bg-red-500/10 text-red-400'}`}
                        >
                          {(log as any).verified ? 'VERIFIED' : 'MISMATCH'}
                        </span>
                      </div>
                    </div>
                  ))
