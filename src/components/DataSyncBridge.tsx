...
export const DataSyncBridge: React.FC = () => {
  ...
  const { updateTelemetry, hydraulic, mechanical, penstock, identity } = useTelemetryStore();
  ...
  useEffect(() => {
    ...
    console.log(
      `[CEREBRO Sync] Bridging Asset: ${selectedAsset.name} (${selectedAsset.turbine_type})`
    );
    ...
    if (selectedAsset.specs) {
      ...
      if (specs.boltDiameter || specs.boltCount || specs.boltGrade) {
        ...
        const safeCount = specs.boltCount
          ? parseInt(specs.boltCount.toString())
          : mechanical.boltSpecs?.count || 12;
        const safeGrade = specs.boltGrade || mechanical.boltSpecs?.grade || '8.8';
        const safeDiameter = specs.boltDiameter
          ? parseInt(specs.boltDiameter.toString())
          : mechanical.boltSpecs?.diameter || 24;
