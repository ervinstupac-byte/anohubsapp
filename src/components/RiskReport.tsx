...
export const RiskReport: React.FC = () => {
  ...
  const localRiskData = useMemo(() => {
    ...
    QUESTIONS.forEach(q => {
      ...
      if (answer && answer === q.critical) {
        ...
        answer &&
        (answer.includes('No') || answer.includes('Unknown') || answer.includes('Partial'))
      ) {
        calculatedScore += 10;
      }
    });
      ...
      try {
        ...
        if (res?.dec25Present) {
          ...
          if (
            fEx &&
            res?.forecast &&
            typeof fEx.weeksUntil === 'number' &&
            typeof res.forecast.weeksUntil === 'number'
          ) {
  ...
  const handleUploadToHQ = async () => {
    ...
    try {
      ...
      showToast(
        t('riskReport.archiveSuccess', 'Dossier successfully archived to HQ Cloud.'),
        'success'
      );
    } catch (error: any) {
      console.error('Upload failed:', error);
      showToast(
        t('riskReport.archiveError', {
          defaultValue: `Archiving failed: ${error.message}`,
          error: error.message,
        }),
        'error'
      );
  ...
  const getStatusBadge = (status: 'draft' | 'synced' | 'none') => {
    if (status === 'draft') {
      return (
        <span className="px-2 py-0.5 bg-amber-500/20 border border-amber-500/50 rounded text-[10px] font-bold text-amber-400 uppercase tracking-wider">
          🟡 Local Draft
        </span>
      );
    } else if (status === 'synced') {
      return (
        <span className="px-2 py-0.5 bg-emerald-500/20 border border-emerald-500/50 rounded text-[10px] font-bold text-emerald-400 uppercase tracking-wider">
          🟢 Synced
        </span>
      );
    }
    ...
    <div className="animate-fade-in pb-12 space-y-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex items-center gap-3 bg-slate-900/80 border border-slate-700 rounded-full px-4 py-1.5 backdrop-blur-md">
          <span className="text-[10px] text-slate-500 uppercase font-bold tracking-widest">
            {t('riskReport.activeContext', 'Active Context')}
          </span>
          <div className="h-4 w-px bg-slate-700"></div>
          {selectedAsset ? (
            <div className="flex items-center gap-2 text-sm font-mono text-cyan-400">
              <span
                className={`w-2 h-2 rounded-full ${selectedAsset.status === 'Operational' ? 'bg-emerald-500' : 'bg-red-500'}`}
              ></span>
              {selectedAsset.name}
            </div>
          ) : (
            <span className="text-sm text-slate-500 italic">
              {t('riskReport.noAsset', 'No Asset Selected')}
            </span>
          )}
        </div>
      </div>
      ...
      <div className="text-center py-6 animate-fade-in-up">
        <h1 className="text-4xl md:text-5xl font-black text-white tracking-tighter uppercase mb-2">
          {t('riskReport.title', 'Project')}{' '}
          <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-500">
            {t('riskReport.dossier', 'Master Dossier')}
          </span>
        </h1>
        <p className="text-slate-400 text-lg max-w-2xl mx-auto font-light">
          {t(
            'riskReport.subtitle',
            'Comprehensive risk and design summary for enterprise archiving.'
          )}
        </p>
      </div>
      ...
      {!selectedAsset ? (
        <GlassCard className="text-center py-20 border-dashed border-slate-700">
          ...
          <h3 className="text-2xl font-bold text-white mb-2">
            {t('riskReport.assetNotLoaded', 'Asset Not Loaded')}
          </h3>
          <p className="text-slate-400 mb-8">
            {t('riskReport.selectAssetPrompt', 'Please select an asset to generate its Dossier.')}
          </p>
          <ModernButton onClick={() => navigateTo('globalMap')} variant="secondary">
            {t('hub.operationalModules', 'Select Asset')}
          </ModernButton>
        </GlassCard>
      ) : loading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-fade-in-up">
          ...
          >
            {riskData ? (
              <div className="space-y-6">
                {/* STATUS BADGE */}
                <div className="flex justify-end">{getStatusBadge(riskDataStatus)}</div>
                ...
                <div className="flex justify-between items-center p-4 rounded-xl border bg-slate-900/50 border-slate-700">
                  <span className="text-slate-400 text-sm font-bold uppercase tracking-wider">
                    {t('riskReport.riskLevel', 'Risk Level')}
                  </span>
                  {(() => {
                    ...
                    if (pf !== null) {
                      return (
                        <span
                          className={`text-xl font-black uppercase px-3 py-1 rounded border text-red-400 border-red-500/50 bg-red-500/10`}
                        >
                          {`P_f: ${pf.toFixed(1)}%`}
                        </span>
                      );
                    ...
                    return (
                      <span
                        className={`text-xl font-black uppercase px-3 py-1 rounded border ${getRiskColor(riskData.risk_level)}`}
                      >
                        {riskData.risk_level}
                      </span>
                    );
                ...
                <div className="mt-3 text-sm text-slate-400 space-y-2">
                  {forecast && forecast.predictedTimestamp ? (
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-mono text-sm text-amber-300">
                        {new Date(forecast.predictedTimestamp).toLocaleString()}
                      </span>
                    </div>
                  ) : (
                    <div className="text-xs text-slate-500 italic">
                      No predicted breach within forecast window.
                    </div>
                  )}
                  ...
                  {forecast ? (
                    forecast.confidence < 0.85 ? (
                      <div className="text-sm text-rose-300 font-bold uppercase">
                        Risk Evaluation: Data Insufficient (confidence{' '}
                        {(forecast.confidence || 0).toFixed(3)})
                      </div>
                    ) : (
                      <div className="text-sm text-slate-400">
                        Forecast confidence: {(forecast.confidence || 0).toFixed(3)} — samples:{' '}
                        {sampleCount ?? 'N/A'}
                      </div>
                    )
                  ) : null}
                  ...
                  {dec25Present ? (
                    <div className="mt-2 p-3 rounded bg-red-900/10 border border-red-600/20 text-red-300 text-sm font-mono">
                      <div className="font-bold uppercase">
                        Model-Distorting Anomaly — Dec 25 Flow Surge
                      </div>
                      <div>
                        Current risk / forecast is heavily influenced by a Dec 25 hourly outlier.
                      </div>
                      {dec25DeltaWeeks !== null ? (
                        <div>
                          Estimated forecast change due to Dec 25: {dec25DeltaWeeks.toFixed(0)}{' '}
                          weeks (approx).
                        </div>
                      ) : null}
                    </div>
                ...
                <div className="p-4 bg-slate-900/80 rounded-xl border border-slate-700/50">
                  <p className="text-xs text-slate-500 font-mono mb-2 uppercase tracking-widest">
                    {t('riskReport.engineerNotes', 'Engineer Notes')}
                  </p>
                  <p className="text-sm text-slate-300 italic">
                    "
                    {riskData.description ||
                      t('riskReport.noNotes', 'No additional notes provided.')}
                    "
                  </p>
                </div>
              </div>
              ...
              <div className="flex flex-col items-center justify-center h-64 text-center">
                <p className="text-slate-500 mb-6">
                  {t('riskReport.noLatestRisk', 'No latest risk assessment found.')}
                </p>
                <ModernButton
          ...
          <GlassCard
            ...
            subtitle={
              designData
                ? `REV: ${designData.created_at ? designData.created_at.slice(0, 10) : 'DRAFT'}`
                : 'NO DATA'
            }
            className="h-full flex flex-col"
            action={<span className="text-2xl">⚙️</span>}
          >
            {designData ? (
              <div className="space-y-6">
                {/* STATUS BADGE */}
                <div className="flex justify-end">{getStatusBadge(designDataStatus)}</div>
                ...
                <div className="p-6 rounded-xl bg-gradient-to-br from-slate-900 to-slate-800 border border-slate-700 flex justify-between items-end">
                  <div>
                    <span className="text-slate-400 text-xs font-bold uppercase tracking-wider block mb-1">
                      {t('riskReport.ratedPower', 'Rated Power Output')}
                    </span>
                    <span className="text-4xl font-black text-white tracking-tighter">
                      {designData.calculations?.powerMW}{' '}
                      <span className="text-lg font-medium text-slate-500">MW</span>
                    </span>
                  </div>
                  <div className="text-right">
                    <span className="text-slate-400 text-xs font-bold uppercase tracking-wider block mb-1">
                      {t('riskReport.annualGen', 'Annual Energy Gen.')}
                    </span>
                    <span className="text-2xl font-bold text-emerald-400">
                      {designData.calculations?.energyGWh || designData.calculations?.annualGWh}{' '}
                      <span className="text-sm text-slate-500">GWh</span>
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-col items-center justify-center h-64 text-center">
                <p className="text-slate-500 mb-6">
                  {t('riskReport.noLatestDesign', 'No latest technical design found.')}
                </p>
                <ModernButton
          ...
          <div
            className="lg:col-span-2 mt-4 animate-fade-in-up"
              ...
              <div className="bg-slate-900 rounded-[15px] p-8 text-center relative overflow-hidden">
                ...
                <div className="relative z-10">
                  <h3 className="text-2xl font-bold text-white mb-2 tracking-tight">
                    {t('riskReport.compileTitle', 'Compile Master Dossier')}
                  </h3>
                  <p className="text-slate-400 mb-8 max-w-2xl mx-auto text-sm">
                    {t(
                      'riskReport.compileDesc',
                      'Combine the latest Risk Diagnostic and Technical Design into a single, comprehensive PDF document for official use.'
                    )}
                  </p>
                  ...
                  <div className="flex flex-col sm:flex-row justify-center gap-4">
                    ...
                    >
                      {!hasData
                        ? t('riskReport.noDataButton', 'No Data')
                        : t('riskReport.submitArchive', 'Submit to HQ Archive')}
                    </ModernButton>
                  </div>
                </div>
              </div>
