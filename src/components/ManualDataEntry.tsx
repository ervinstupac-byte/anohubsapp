...
import {
  calculateOperatingZone,
  calculateCavitationRisk,
} from '../features/physics-core/PhysicsCalculations.logic';
import {
  ErrorHandlerService,
  CavitationStatus,
  ErosionStatus,
...
export const ManualDataEntry: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<
    'manual' | 'scenario' | 'kernel' | 'processes' | 'console'
  >('manual');
  ...
  const [drTurbineResult, setDrTurbineResult] = useState<{
    cards: ActionCard[];
    healthScore: number;
    voiceMessage: string;
  } | null>(null);
  ...
  useEffect(() => {
    ...
    const harvest = ResonanceHarvesterManager.harvestVibrationEnergy(
      formState.vibration,
      formState.activePower
    );
    ...
    const yearsAdded = LifeExtensionEngine.calculateLifeExtension(
      20,
      235,
      currentStress,
      mitigationFactor
    ...
    const mawp = StructuralIntegrityService.calculateMAWP(
      formState.yieldStrength,
      formState.wallThickness,
      formState.penstockDia
    );
    ...
    const simulatedPoints = Array(formState.pitCount)
      .fill(0)
      .map(() => ({ x: 0, y: 0, z: -formState.pitDepth }));
    ...
    const hydSim = HydraulicTransientSafety.simulateHardwareChange(
      {
        pipeDiameterMM: formState.pipeDiaOld,
        pipeLengthM: 10,
        oilViscosityCst: 46,
        systemPressureBar: 160,
        actuatorVolumeL: 50,
        accumulators: false,
      },
      {
        pipeDiameterMM: formState.pipeDiaNew,
        pipeLengthM: 10,
        oilViscosityCst: 46,
        systemPressureBar: 160,
        actuatorVolumeL: 50,
    ...
    const sigB = Array.from({ length: 100 }, (_, i) =>
      formState.vibration > 0.1
        ? Math.sin(i / 10) + Math.random() * 0.1
        : Math.cos(i / 10) + Math.random() * 0.1
    );
    ...
    const galvanicAlerts = GalvanicCorrosionService.analyzeCathodicProtection({
      ...
      averageVoltage: formState.anodeVoltage,
      generatorGroundingResistance: 0.5,
      ...
      anodes: [
        {
          ...
          current: 100,
          consumptionRate: 2,
          estimatedLifeRemaining: formState.anodeLife,
        },
      ],
    });
    ...
    const airGapAction = airGapSentinel.addMeasurement({
      timestamp: new Date().toISOString(),
      airGapSensorsMm: [
        formState.gapSensor1,
        formState.gapSensor2,
        formState.gapSensor3,
        formState.gapSensor4,
      ],
    ...
    const brakeReadiness = brakeGuardian.evaluateReadiness(
      formState.brakePadWear,
      formState.brakeHydraulicPressure
    );
    ...
    const inertia = gridGuardian.assessInertia({
      timestamp: Date.now(),
      frequencyHz: formState.frequency,
    });
    ...
    // 31. Dewatering
    const sumpStatus = DewateringSovereign.monitorSump(
      'SUMP-1',
      formState.sumpLevel,
      formState.sumpLevel > 60,
      false
    );
    ...
    // 33. Pneumatic
    const airStatus = PneumaticSystemManager.monitorSystem(
      formState.airPressure,
      formState.airPressure < 7
    );
    ...
    formState.oilTan,
    formState.oilWater,
    formState.oilParticles4um,
    formState.oilParticles6um,
    formState.oilParticles14um,
    formState.penstockDia,
    formState.wallThickness,
    formState.yieldStrength,
    formState.pitDepth,
    formState.pitCount,
    formState.pipeDiaOld,
    formState.pipeDiaNew,
    formState.acousticSim,
    formState.anodeVoltage,
    formState.anodeLife,
  ...
  const handleArchive = async () => {
    ...
    addSnapshot({
      ...
      oracleWisdom: {
        title:
          analysis?.zone.zone === 'OPTIMAL' ? 'User Knowledge Capture' : 'CRITICAL OBSERVATION',
      ...
      videoForensics: videoResult
        ? {
            anomalies: videoResult.detectedAnomalies.map(a => ({
              type: a.type,
              severity: a.severity,
            })),
        ...
        <div className="space-y-4">
              ...
              <div className="bg-slate-950 p-2 rounded border border-slate-800">
                ...
                <div className="grid grid-cols-3 gap-1">
                  <input
                    ...
                    className="bg-transparent text-xs text-center w-full focus:outline-none"
                  />
                  <input
                    type="number"
                    placeholder="CH4"
                  ...
                  <input
                    type="number"
                    placeholder="C2H2"
                    value={formState.c2h2}
                    onChange={e => handleChange('c2h2', e.target.value)}
                    className="bg-transparent text-xs text-center w-full focus:outline-none"
                  />
                </div>
              </div>
              ...
              <div className="bg-slate-950 p-2 rounded border border-slate-800">
                ...
                <div className="grid grid-cols-2 gap-1">
                  <input
                    type="number"
                    placeholder="pC"
                    value={formState.q_pC}
                    onChange={e => handleChange('q_pC', e.target.value)}
                    className="bg-transparent text-xs text-center w-full focus:outline-none"
                  />
                  <input
                    type="number"
              ...
              <div className="bg-slate-950 p-2 rounded border border-slate-800">
                ...
                <div className="grid grid-cols-2 gap-1">
                  <input
                    type="number"
                    placeholder="Flow"
                    value={formState.sealFlow}
                    onChange={e => handleChange('sealFlow', e.target.value)}
                    className="bg-transparent text-xs text-center w-full focus:outline-none"
                  />
                  <input
                    type="number"
                    placeholder="Pit Lvl"
                    value={formState.pitLevel}
                    onChange={e => handleChange('pitLevel', e.target.value)}
                    className="bg-transparent text-xs text-center w-full focus:outline-none"
                  />
                </div>
              </div>
              ...
              <div className="bg-slate-950 p-2 rounded border border-slate-800">
                ...
                <div className="grid grid-cols-2 gap-1">
                  <input
                    type="number"
                    placeholder="Bar"
                    value={formState.hpuPressure}
                    onChange={e => handleChange('hpuPressure', e.target.value)}
                    className="bg-transparent text-xs text-center w-full focus:outline-none"
                  />
                  <input
                    type="number"
                    placeholder="Run(s)"
                    value={formState.pumpRunTime}
            ...
            <div className="bg-slate-950 p-2 rounded border border-slate-800 flex items-center gap-4">
              ...
              <div className="flex-1">
                <div className="text-[10px] text-slate-500 mb-1">
                  Upload component photo for AI analysis (Cracks, Corrosion, Debris)
                </div>
                {visionResult ? (
                  <div className="text-xs font-mono">
                    <span
                      className={
                        visionResult.detections.length > 0 ? 'text-amber-400' : 'text-emerald-400'
                      }
                    >
                    ...
                    {videoResult && (
                      <div className="mt-1 pt-1 border-t border-slate-800 text-[9px] text-cyan-300">
                        AR: {videoResult.detectedAnomalies.length} Anomalies | Audio:{' '}
                        {videoResult.audioAnalysis.detectedPattern}
                      </div>
                    )}
                  </div>
                ) : (
          ...
          {analysis && (
            <div className="space-y-2 mt-4">
              ...
              {monsterResult &&
                (monsterResult.overallRisk === 'HIGH' ||
                  monsterResult.overallRisk === 'CRITICAL') && (
                  <div className="p-3 rounded border text-xs bg-orange-950/40 border-orange-500">
                    ...
                    <div className="grid grid-cols-2 gap-2 mb-2">
                      <div>
                        ...
                        <span
                          className={`font-bold ${monsterResult.cavitation.riskLevel === 'SAFE' ? 'text-emerald-400' : 'text-red-400'}`}
                        >
                          {monsterResult.cavitation.sigma.toFixed(3)} (
                          {monsterResult.cavitation.riskLevel})
                        </span>
                      </div>
                      <div>
                        <span className="text-[9px] text-slate-400 block">EROSION</span>
                        <span
                          className={`font-bold ${monsterResult.erosion.riskLevel === 'LOW' ? 'text-emerald-400' : 'text-orange-400'}`}
                        >
              ...
              >
                ...
                <ul className="list-disc pl-5 space-y-1 opacity-90">
                  ...
                  {analysis.cavitation.risk === 'LOW' && <li>Cavitation Margin: Healthy</li>}
                </ul>
              </div>
              ...
              >
                ...
                <ul className="list-disc pl-5 space-y-1 opacity-90">
                  ...
                  {analysis.vibration.recommendations.length > 0 ? (
                    analysis.vibration.recommendations.map((rec, i) => <li key={i}>{rec}</li>)
                  ) : (
              ...
              {analysis.oilAnalysis && (
                <div
                  className={`p-3 rounded border text-xs ${
                    analysis.oilAnalysis.overallHealth === 'EXCELLENT' ||
                    analysis.oilAnalysis.overallHealth === 'GOOD'
                      ? 'bg-emerald-950/30 border-emerald-800 text-emerald-400'
                      : 'bg-orange-950/30 border-orange-800 text-orange-400'
                  }`}
                >
                  <div className="font-bold uppercase mb-1">
                    Oil Health: {analysis.oilAnalysis.overallHealth}
                  </div>
              ...
              {analysis.aiPrediction?.synergeticRisk.detected && (
                <div className="p-3 rounded border text-xs bg-purple-950/30 border-purple-800 text-purple-400">
                  <div className="font-bold uppercase mb-1 flex items-center gap-2">
                    ...
                    AI Risk Detected (
                    {(analysis.aiPrediction.synergeticRisk.probability * 100).toFixed(0)}%)
                  </div>
              ...
              {damageReport && (
                <div className="p-3 rounded border text-xs bg-pink-950/30 border-pink-800 text-pink-400">
                  ...
                  <div className="font-bold mb-1">
                    {damageReport.damageType} ({damageReport.estimatedSeverity})
                  </div>
                  <ul className="list-disc pl-5 space-y-1 opacity-90">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="p-2 rounded border text-xs bg-slate-950/50 border-slate-800 text-slate-400">
                  ...
                  <div className="text-[10px] text-slate-500">
                    €{analysis.energyHarvest?.annualEur.toFixed(0)}/yr
                  </div>
                </div>
                <div className="p-2 rounded border text-xs bg-slate-950/50 border-slate-800 text-slate-400">
              ...
              {analysis.structural && (
                <div
                  className={`mt-2 p-2 rounded border text-xs ${
                    analysis.structural.status === 'NORMAL'
                ...
                >
                  ...
                  <div className="text-[10px] text-slate-500">
                    MAWP: {analysis.structural.mawp.toFixed(1)} bar
                  </div>
                </div>
              )}
                ...
                <div className="grid grid-cols-2 gap-2 mt-2">
                  {analysis.thermal && (
                    <div className="p-2 rounded border text-xs bg-slate-950/50 border-slate-800 text-slate-400">
                      ...
                      <div className="text-[10px] text-slate-500">
                        {analysis.thermal.action} (Valve: {analysis.thermal.valveRec}%)
                      </div>
                    </div>
                  )}
                  {analysis.airGap && (
                    <div
                      className={`p-2 rounded border text-xs ${analysis.airGap.action === 'NO_ACTION' ? 'bg-slate-950/50 border-slate-800' : 'bg-red-950/30 border-red-500'}`}
                    >
                      ...
                      <div className="text-[10px] text-slate-500">
                        UMP: {analysis.airGap.ump.toFixed(0)}N
                      </div>
                    </div>
                  )}
                </div>
                  ...
                  {analysis.acoustic && (
                    <div
                      className={`p-2 rounded border text-xs ${
                        analysis.acoustic.severity === 'NORMAL'
                    ...
                    >
                      ...
                      <div className="text-[9px] opacity-70">
                        Conf: {analysis.acoustic.confidence.toFixed(0)}%
                      </div>
                    </div>
                  )}
                  {analysis.galvanic && (
                    <div
                      className={`p-2 rounded border text-xs ${
                        analysis.galvanic.protection === 'GOOD'
                          ? 'bg-slate-950/50 border-slate-800'
              ...
              {analysis.synergy?.correlated && (
                <div className="mt-2 p-2 rounded border text-xs bg-indigo-950/30 border-indigo-500 text-indigo-300">
                  ...
                  <div>
                    Vibration & Load highly correlated - indicating mechanical coupling issue.
                  </div>
                </div>
              )}
            </div>
