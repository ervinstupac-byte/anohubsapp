...
const KaplanDashboard = ({ turbineModel }: any) => (
  <div className="p-4 text-white">Kaplan Dashboard Running...</div>
);
const FrancisDashboard = ({ turbineModel }: any) => (
  <div className="p-4 text-white">Francis Dashboard Running...</div>
);
const PeltonDashboard = ({ turbineModel }: any) => (
  <div className="p-4 text-white">Pelton Dashboard Running...</div>
);
const ForensicTimeline = ({ asset, turbineModel }: any) => (
  <div className="p-4 text-white">Forensic Timeline Running...</div>
);
const DigitalTwinOverlay = ({ asset, turbineModel }: any) => (
  <div className="p-4 text-white">Digital Twin Overlay Placeholder</div>
);
const OptimizationDashboard = ({ asset, turbineModel }: any) => (
  <div className="p-4 text-white">Optimization Dashboard Placeholder</div>
);
...
export const UniversalFleetDashboard: React.FC = () => {
  ...
  const turbineModel = useMemo(() => {
    ...
    return TurbineFactory.create(asset.turbine_family, asset.turbine_variant, asset.turbine_config);
  }, [asset]);
          ...
          <div className="flex items-center justify-between">
            ...
            <div>
              ...
              <p className="text-sm text-slate-400">
                {asset.turbine_config.manufacturer} • SN: {asset.turbine_config.serial_number} •
                {asset.capacity} MW
              </p>
            </div>
      ...
      <AnimatePresence mode="sync">
        ...
        {viewMode === 'DIGITAL_TWIN' && (
          <motion.div
            ...
            transition={{ duration: 0.3 }}
          >
            <DigitalTwinOverlay asset={selectedAsset} turbineModel={turbineModel} />
          </motion.div>
        )}
        ...
        {viewMode === 'CONSULTING' && (
          <motion.div
            ...
            transition={{ duration: 0.3 }}
          >
            <OptimizationDashboard asset={selectedAsset} turbineModel={turbineModel} />
          </motion.div>
        )}
        ...
        {viewMode === 'FORENSICS' && (
          <motion.div
            ...
            transition={{ duration: 0.3 }}
          >
            <ForensicTimeline asset={selectedAsset} turbineModel={turbineModel} />
          </motion.div>
        )}
      </AnimatePresence>
