      ...
      {data.map((value, i) => {
        ...
        return (
          <div
            ...
            style={{
              backgroundColor:
                intensity === 0 ? '#1e293b' : `rgba(34, 197, 94, ${intensity / 100})`,
      ...
      {!selectedAsset ? (
        <GlassCard className="text-center py-20">
          <p className="text-slate-500 uppercase tracking-widest font-bold">
            Select an Asset to view Maintenance Schedule
          </p>
        </GlassCard>
      ) : (
        <>
            ...
            <div className="lg:col-span-2 space-y-6">
              <GlassCard title="Service Protocols & Schedule" className="bg-slate-900/80">
                <div className="space-y-4">
                  {protocols.map(proto => {
                    const nextDate = predictServiceDate(
                      idAdapter.toNumber(selectedAsset.id) || 0,
                      proto.threshold
                    );
                    ...
                    return (
                      <div
                        key={proto.id}
                        className="p-4 rounded-xl bg-slate-950/50 border border-white/5 group hover:border-emerald-500/30 transition-all"
                      >
                        <div className="flex justify-between items-start mb-3">
                            ...
                            {(() => {
                              ...
                              const handleGenerateOrder = (e: React.MouseEvent) => {
                                ...
                                ForensicReportService.openAndDownloadBlob(
                                  blob,
                                  `PO_${proto.id}_${selectedAsset.name}.pdf`,
                                  true,
                                  {
                          ...
                          <div className="text-right">
                            <p
                              className={`text-xs font-mono font-black ${progress > 80 ? 'text-orange-400 animate-pulse' : 'text-slate-300'}`}
                            >
                            ...
                            {progress > 80 && (
                              <div className="flex flex-col gap-2 mt-2">
                                <button
                                  onClick={async () => {
                                    showToast(
                                      'SYSTEM ALERT: Bearing Overheat likely. Initializing Work Order...',
                                      'info'
                                    );
                                ...
                                <button
                                  onClick={() =>
                                    navigate(
                                      `/${ROUTES.MAINTENANCE.ROOT}/${ROUTES.MAINTENANCE.AR_GUIDE}`
                                    )
                                  }
                        ...
                        <div className="space-y-1">
                          <div className="flex justify-between text-[9px] font-bold uppercase">
                            <span className="text-slate-500">
                              Service Threshold: {proto.threshold}h
                            </span>
                            <span
                              className={progress > 80 ? 'text-orange-400' : 'text-emerald-500'}
                            >
                              {progress.toFixed(1)}%
                            </span>
                          </div>
