...
export const DiagnosticRCA: React.FC<DiagnosticRCAProps> = ({
  diagnosis: externalDiagnosis,
  specs: externalSpecs,
}) => {
    ...
    <div className="w-full bg-slate-800 rounded-xl border border-slate-600/50 overflow-hidden shadow-2xl animate-in fade-in slide-in-from-right-4 duration-700">
      ...
      <div className="p-3 bg-slate-900/50 border-b border-white/5 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <Brain
            className={`w-4 h-4 ${isThinking ? 'text-cyan-400 animate-pulse' : 'text-slate-400'}`}
          />
      ...
      <div className="p-2 space-y-2">
            ...
            <motion.div
              ...
              className={`rounded border p-3 cursor-pointer transition-colors ${
                result.severity === 'CRITICAL'
                  ? 'bg-red-900/10 border-red-500/40 hover:bg-red-900/20'
        ...
        {results.length > 0 && (
          ...
          >
            ...
            <span className="text-[9px] font-black tracking-widest text-slate-400 group-hover:text-white">
            ...
            >
              ...
              <div className="flex items-center gap-4 mb-8 border-b border-cyan-500/20 pb-6">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    <span className="px-2 py-0.5 bg-red-500 text-black text-xs font-black uppercase">
                      CRITICAL FAULT DETECTED
                    </span>
                    <span className="text-xs font-mono text-cyan-600">
                      ID: RCA-{Date.now().toString().slice(-6)}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-amber-950/20 border border-amber-500/30 p-6 flex gap-4">
                ...
                <div>
                  <label className="text-[10px] font-black text-amber-700 uppercase tracking-widest mb-1 block">
                    Werkmeister Field Recommendation
                  </label>
