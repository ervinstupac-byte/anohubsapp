...
const HydroschoolSimulator: React.FC = () => {
    ...
    (async () => {
      try {
        ...
        const { data: design } = await supabase
          ...
          .select(
            "parameters->>family as family, parameters->>variant as variant, parameters->'design_points' as design_points"
          )
          .eq('asset_id', assetId)
  ...
  const fetchLatestTelemetry = async () => {
    ...
    try {
      ...
      const sensorSamples = (sensorData || []).map((s: any) => ({
      ...
      const result = evaluateAging(
        telemetryRef.current.map(s => ({
          eta: s.efficiency ?? undefined,
          output_power: s.output_power,
          francis_data: s.francis_data,
        })),
        designEta ?? undefined,
        hydroContext ?? undefined
      );
  ...
  const computeDiagnostics = () => {
    try {
      const flows = telemetryWindow
        .map(
          s =>
            ((s.francis_data && (s.francis_data.flow || s.francis_data.flowRate)) ??
              s.flow ??
              s.Q) ||
            0
        )
        .filter(n => n > 0);
      const heads = telemetryWindow
        .map(
          s =>
            ((s.francis_data && (s.francis_data.head || s.francis_data.GrossHead)) ??
              s.H ??
              s.head) ||
            0
        )
        .filter(n => n > 0);
      const vibs = telemetryWindow
        .map(
          s =>
            ((s.francis_data && (s.francis_data.stay_ring_vibration || s.francis_data.vibration)) ??
              s.vibration) ||
            0
        )
        .filter(n => n > 0);
      ...
      if (meanFlow !== null && hydroContext?.design_flow_cms) {
        ...
        if (ratio < 0.6)
          items.push(
            'Part-load operation — mean flow < 60% of design. Runner sees off-design loading; cavitation and runner wear accelerate.'
          );
        else if (ratio < 0.9)
          items.push('Below-design flow — turbine operates off-optimum, reducing efficiency.');
      ...
      if (meanHead !== null && hydroContext?.gross_head_m) {
        ...
        if (ratioH < 0.8)
          items.push(
            'Head mismatch — actual head below design, shifting operating point and reducing η.'
          );
      }
      ...
      if (meanVib !== null) {
        if (meanVib > 4.0)
          items.push(
            'Elevated vibration — mechanical fatigue risk increases; fatigue shortens operational lifetime.'
          );
  ...
  return (
    <div className="bg-[#07101a] p-6 rounded border border-slate-800">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
        <label className="flex flex-col">
          ...
          <input
            type="number"
            value={P_kw}
            onChange={e => setP_kw(Number(e.target.value))}
            className="mt-1 p-2 bg-[#02111a] border border-slate-700 rounded text-slate-100"
          />
        </label>
        <label className="flex flex-col">
        ...
        <div className="col-span-1 md:col-span-1">
          ...
          <div className="text-slate-400 text-sm mt-1">
            Formula: η = P / (ρ·g·Q·H) — using ρ=1000 kg/m³, g=9.80665 m/s²
          </div>
          <div className="mt-2 text-sm text-slate-300">
            <div>Calculation (live):</div>
            <div className="font-mono text-xs text-slate-200 mt-1">
              P = {P_kw} kW ({new Decimal(P_kw).times(1000).toFixed(0)} W)
            </div>
            <div className="font-mono text-xs text-slate-200">
              Q = {Q} m³/s · H = {H} m
            </div>
            <div className="font-mono text-xs text-slate-200">
              η = {`P / (ρ·g·Q·H)`} = {eta.toFixed(4)}
            </div>
          </div>
        </div>
        ...
        <div className="text-right">
          ...
          <div className="mt-2 text-slate-200 max-w-sm">
            Persistent operation outside the optimal efficiency band accelerates wear. Hydroschool
            can connect to live telemetry and compute aging pressure.
          </div>
        </div>
      </div>
        ...
        <div className="col-span-1">
          ...
          <div className="p-4 bg-black/5 rounded border border-slate-800">
            ...
            <div className="text-2xl font-mono text-white mt-2">
              {agingResult ? agingResult.warningLevel : '—'}
            </div>
            <div className="text-xs text-slate-400 mt-2">
              {agingResult ? agingResult.explanation : 'No telemetry yet'}
            </div>
            {diagItems.length > 0 && (
              <div className="mt-3 text-sm text-slate-200">
                <div className="text-xs text-amber-200 font-bold">
                  Why this matters for 50-year preservation:
                </div>
            ...
            {agingResult && (
              <div className="mt-3 text-xs text-slate-300">
                <strong>50-year vision:</strong> These indicators determine whether the machine can
                meet a 50+ year service life — sustained off-design operation, cavitation, or
                elevated vibration materially reduce remaining life.
              </div>
            )}
            <div className="mt-3 w-full bg-slate-900 rounded h-3 overflow-hidden">
              <div
                className="h-3 bg-gradient-to-r from-red-600 to-yellow-400"
                style={{ width: `${agingResult ? Math.min(100, agingResult.agingScore) : 0}%` }}
              />
            </div>
          </div>
        </div>
      ...
      <div className="mt-4 text-slate-400 text-sm">
        Note: DB-driven mode requires Supabase credentials. Live mode connects to
        `dynamic_sensor_data` for this asset.
      </div>
    </div>
  );
};
