...
export const StrategicPlanningDashboard: React.FC = () => {
  ...
  const { site, penstock, mechanical, hydraulic, physics, identity, setConfig } =
    useTelemetryStore();
  const {
    t,
    i18n: { language },
  ...
  const technicalState = React.useMemo(
    () => ({
      ...
      physics: {
        ...
        boltSafetyFactor:
          physics?.boltSafetyFactor ?? DEFAULT_TECHNICAL_STATE.physics.boltSafetyFactor,
        ...
        staticPressureBar:
          physics?.staticPressureBar ?? DEFAULT_TECHNICAL_STATE.physics.staticPressureBar,
        surgePressureBar:
          physics?.surgePressureBar ?? DEFAULT_TECHNICAL_STATE.physics.surgePressureBar,
        waterHammerPressureBar:
          physics?.waterHammerPressureBar ?? DEFAULT_TECHNICAL_STATE.physics.waterHammerPressureBar,
  ...
  const activeAlerts = React.useMemo(() => {
    const alerts = [];
    if (systemHealth.analysis.cavitationRisk)
      alerts.push('Potential Cavitation detected! High velocity at low head.');
    if (technicalState.physics.boltSafetyFactor < 1.5)
      alerts.push('Bolt Safety Factor Critical (<1.5). Increase Grade or Torque.');
    return alerts;
  ...
  const [impact, setImpact] = useState<ImpactAnalysis & { recommendations?: any[] }>({
    warnings: [],
    recommendations: [],
    safetyFactor: 0,
    hoopStressMPa: 0,
  ...
  return (
    <div
      className={`min-h-screen grid grid-cols-12 gap-0 ${THEME.bg} text-scada-text font-sans overflow-hidden`}
    >
      ...
      <div className={`col-span-9 p-8 flex flex-col h-screen overflow-y-auto ${THEME.bg}`}>
        ...
        <header className="mb-6 flex justify-between items-end border-b border-scada-border pb-4">
          <div>
            ...
            <p className="text-[10px] text-scada-muted font-mono tracking-widest uppercase flex items-center gap-2">
              <span
                className={isCalibrating ? 'text-status-warning animate-pulse' : 'text-status-ok'}
              >
                {isCalibrating
                  ? t('hpp.recalculating', 'PHYSICS ENGINE RECALCULATING...')
                  : t('hpp.physics_ready', 'SYSTEM READY')}
              </span>
            </p>
          </div>
        </header>
        ...
        <div className="flex h-[calc(100vh-160px)] border border-scada-border rounded-sm overflow-hidden">
          {/* LEFT: COMPONENT TREE */}
          <ComponentTree selectedId={activeTab} onSelect={(id: any) => setActiveTab(id)} />
          ...
          <div className="flex-1 bg-scada-bg p-8 overflow-y-auto relative">
            ...
            {(activeTab === 'HYDRAULICS' || activeTab === 'PENSTOCK') && (
              <div key="HYDRAULICS" className="h-full">
                <div className="mb-4">
                  <h2 className="text-xl font-bold text-scada-text uppercase tracking-tight font-header">
                    Hydraulic Geometry
                  </h2>
                  <p className="text-sm text-scada-muted">
                    Define head, flow, and material physics.
                  </p>
                </div>
                <HydraulicsPanel />
                ...
                <div className="mt-8 pt-8 border-t border-scada-border">
                  <h3 className="text-sm font-bold text-scada-muted uppercase mb-4">
                    Quick Adjust
                  </h3>
                  <div className="grid grid-cols-2 gap-8">
                    <SliderGroup
                      label="Gross Head (m)"
                      name="grossHead"
                      value={technicalState.site.grossHead}
                      min={1}
                      max={500}
            ...
            {(activeTab === 'MECHANICAL' || activeTab === 'BOLTS') && (
              <div key="MECHANICAL">
                <div className="mb-4">
                  ...
                  <p className="text-sm text-scada-muted">
                    Manage connections, bolts, and alignment tolerances.
                  </p>
                </div>
                <MechanicalPanel />
              </div>
              ...
              >
                <div className="flex flex-col items-center justify-center h-full text-scada-muted">
                  <span className="text-2xl font-black text-scada-text font-mono tabular-nums">
                    {formatNumber(feasibility?.annualProductionMWh || 0, language, 1)}{' '}
                    <span className="text-sm text-scada-muted">GWh</span>
                  </span>
                  <div className="text-[10px] text-scada-muted uppercase mt-1">
                    {t('physics.annualProd', 'Annual Production')}
                  </div>
                </div>
                <div className="p-4 bg-scada-panel rounded-none border border-scada-border mt-4">
                  <span className="text-2xl font-black text-scada-text font-mono tabular-nums">
      ...
      <div className="col-span-3 border-l border-scada-border bg-scada-panel p-0 flex flex-col h-screen">
        ...
        <div className="flex-grow overflow-y-auto p-6 space-y-4">
          <div className="md:col-span-1">
            ...
            >
              ...
              <span className="text-xs uppercase tracking-widest">
                {t('common.generatePDF', 'PDF Generieren')}
              </span>
            </button>

            <div className="mt-4 bg-scada-bg p-4 rounded-sm border border-scada-border space-y-3">
              <h3 className="text-[10px] font-black text-scada-muted uppercase tracking-tighter mb-1">
                Head Analysis
              </h3>

              <div className="flex justify-between items-center text-xs">
                <span className="text-scada-muted font-mono uppercase">Gross Head (Hg)</span>
                <span className="text-scada-text font-mono tabular-nums">
                  {formatNumber(technicalState.site.grossHead, language, 1)} m
                </span>
              </div>
              ...
              <div className="flex justify-between items-center text-xs text-status-warning">
                ...
                <span className="font-mono tabular-nums">
                  -{formatNumber(feasibility?.frictionLoss || 0, language, 2)} m
                </span>
              </div>
              ...
              <div className="flex justify-between items-center text-sm">
                <span className="text-status-info font-bold uppercase tracking-widest text-[10px] font-mono">
                  Net Head (Hn)
                </span>
                <span className="text-scada-text font-black font-mono tabular-nums">
                  {formatNumber(feasibility?.netHead || 0, language, 2)} m
                </span>
              </div>
              ...
              <div className="pt-2">
                <div className="flex justify-between text-[9px] text-scada-muted uppercase mb-1 font-mono">
                  ...
                  <span>
                    {(
                      ((feasibility?.netHead || 1) / (technicalState.site.grossHead || 1)) *
                      100
                    ).toFixed(1)}
                    %
                  </span>
                ...
                <div className="h-1 bg-scada-bg rounded-full overflow-hidden border border-scada-border">
                  <div
                    ...
                    style={{
                      width: `${((feasibility?.netHead || 0) / (technicalState.site.grossHead || 1)) * 100}%`,
                    }}
                  />
                </div>
              </div>
          ...
          <div className="bg-scada-bg border border-scada-border p-6 rounded-sm">
            <h3 className="text-xs font-bold text-scada-muted uppercase mb-4 tracking-widest">
              {t('common.advice', 'Actionable Advice')}
            </h3>
            ...
            <div className="flex items-start gap-4 mb-6">
              ...
              <div>
                <div className="text-lg font-bold text-status-ok mb-1 font-mono">
                  {feasibility?.recommendedAggregates.count}x{' '}
                  {feasibility?.recommendedAggregates.type}
                </div>
            ...
            <div className="space-y-3">
              {impact?.warnings.map((w, i) => (
                <div
                  key={i}
                  className="flex items-start gap-3 p-3 bg-status-error/10 border border-status-error/20 rounded-sm"
                >
          ...
          {technicalState.penstock.material === 'STEEL' &&
            technicalState.site.waterQuality !== 'CLEAN' && (
              <div className="p-4 rounded-none bg-status-info/10 border border-status-info/30 flex gap-3 items-start">
                <Info className="w-4 h-4 text-status-info shrink-0 mt-0.5" />
                <p className="text-xs text-status-info leading-relaxed font-mono">
                  <b>Engineer's Note:</b> Bare steel is rarely used in Balkans due to moderate
                  acidity in mountain streams. Highly recommend at least Epoxy Paint.
                </p>
              </div>
            )}
          ...
          {technicalState.mechanical.boltSpecs.grade === '4.6' && (
            <div className="p-4 rounded-sm bg-status-warning/10 border border-status-warning/30 flex gap-3 items-start">
              ...
              <p className="text-xs text-status-warning leading-relaxed font-mono">
                <b>Legacy Case #22:</b> Class 4.6 bolts sheared during testing at 'Mala Rijeka'.
                Ensure strict torque control if budget prevents upgrade to 8.8.
              </p>
            </div>
          )}
        </div>
...
const KPIMini = ({ label, value, unit, color = 'text-scada-text' }: any) => (
  <div className="flex flex-col">
    ...
    <span className={`text-lg font-black font-mono ${color}`}>
      {value} <span className="text-xs text-scada-muted">{unit}</span>
    </span>
  </div>
);
    ...
    <div className="flex flex-wrap gap-2">
      {options.map((opt: string) => (
        <button
          ...
          className={`px-3 py-2 rounded-none text-[10px] font-bold uppercase transition-all border ${
            value === opt
