...
export default function ComponentInfoPanel({
  ...
  const fuzzySearch = useCallback(async (q: string) => {
    ...
    const { data: a, error: e1 } = await supabase
      .from('component_encyclopedia')
      .select('*')
      .ilike('component_name', qlike)
      .limit(30);
    const { data: b, error: e2 } = await supabase
      .from('component_encyclopedia')
      .select('*')
      .ilike('description', qlike)
      .limit(30);
    setLoading(false);
    if (e1 && e2) {
    ...
    <div className="bg-white/95 border p-3 rounded shadow-lg w-96 max-w-full">
      <div className="flex items-center justify-between mb-2">
        ...
        <div className="flex gap-2">
          ...
          {onClose && (
            <button onClick={onClose} className="text-xs px-2 py-1 bg-red-100 rounded">
              Close
            </button>
          )}
        </div>
      ...
      {entry ? (
        <div>
          ...
          {entry.physics_principle && (
            <div className="mt-2 text-xs text-slate-600">Physics: {entry.physics_principle}</div>
          )}
          {entry.common_failure_modes && (
            <div className="mt-2 text-xs text-slate-600">
              Common failures: {JSON.stringify(entry.common_failure_modes)}
            </div>
          )}
        </div>
        ...
        <div className="text-sm text-slate-500">
          No encyclopedia entry found. Try searching manually.
        </div>
      )}
    </div>
  );
