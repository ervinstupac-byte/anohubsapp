      ...
      <div className="flex justify-between items-center bg-slate-900/40 p-6 rounded-3xl border border-white/5 backdrop-blur-xl">
        <div>
          <h2 className="text-3xl font-black text-white tracking-widest uppercase mb-1">
            Intuition <span className="text-amber-400">Log</span>
          </h2>
          <p className="text-slate-400 text-sm font-light italic">
            "The Old Man's Ear" - Textual Experience Correlation.
          </p>
        </div>
        ...
        <div className="lg:col-span-1 space-y-6">
          <GlassCard title="Enter Observation" className="border-l-4 border-l-amber-500">
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="text-[10px] text-slate-500 font-black uppercase tracking-widest block mb-1">
                  Active Asset
                </label>
          ...
          {fieldDiagnoses.length > 0 && (
            <GlassCard
              title="Ano-Agent Feedback"
              className="border-l-4 border-l-cyan-500 animate-pulse"
            >
              <div className="space-y-4">
                {fieldDiagnoses.map((d, i) => (
                  <div key={i} className="p-4 bg-cyan-500/10 border border-cyan-500/30 rounded-xl">
                    <p className="text-[10px] text-cyan-400 font-black uppercase mb-1">
                      Pattern Identified
                    </p>
        ...
        <div className="lg:col-span-2">
          <GlassCard title="Log History" className="h-full">
            <div className="space-y-4 max-h-[600px] overflow-y-auto pr-2 custom-scrollbar">
              {shiftLogs.length === 0 ? (
                <p className="text-center py-20 text-slate-600 uppercase font-black tracking-widest text-xs">
                  No entries recorded
                </p>
              ) : (
                shiftLogs.map(log => (
                  <div
                    key={log.id}
                    className="p-4 bg-white/5 border border-white/5 rounded-2xl hover:bg-white/10 transition-all border-l-2 border-l-slate-700"
                  >
