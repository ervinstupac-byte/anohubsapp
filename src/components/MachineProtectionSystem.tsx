  ...
  const insights = useMemo(() => {
    ...
    if (dpdt > 5) {
      reports.push({
        ...
        detail:
          'Hydraulic hammer detected. Bypass valve activated. Throttling servomotor response.',
      });
    }
    ...
    if (phaseShift > 15) {
      reports.push({
        ...
        detail:
          assetTele.vibration > 0.06
            ? 'DANGER: Broken joint bolt suspected.'
            : 'Labavost u ležišnim blazinama (Bearing Looseness).',
      });
    }
    ...
    if (thickness < 0.5 || temp > 75) {
      reports.push({
        ...
        detail:
          thickness < 0.3
            ? 'CRITICAL: Metadata risk (Dry friction). Manual HP lift injection active.'
            : 'WARNING: Thinning oil film. Viscosity decreasing.',
      });
    }
      ...
      <div className="space-y-4">
        {insights.length === 0 ? (
          ...
          <div className="space-y-3">
            {insights.map(item => (
              <div
                key={item.id}
                className={`p-3 rounded-xl border transition-all ${item.status === 'CRITICAL' ? 'bg-red-500/10 border-red-500/30' : 'bg-amber-500/10 border-amber-500/30'}`}
              >
                <div className="flex justify-between items-start mb-2">
                  <div className="flex items-center gap-2">
                    <div
                      className={`w-2 h-2 rounded-full ${item.status === 'CRITICAL' ? 'bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.8)]' : 'bg-amber-500 animate-pulse'}`}
                    />
                ...
                <p className="text-[10px] text-slate-300 leading-tight italic">{item.detail}</p>
        ...
        <div className="pt-4 border-t border-white/5 grid grid-cols-2 gap-2">
          <div className="p-2 bg-slate-900/60 rounded border border-white/5 flex flex-col items-center">
            <span className="text-[8px] text-slate-500 font-black uppercase mb-1">
              Cylinder Bypass
            </span>
            <div
              className={`text-[10px] font-black ${assetTele.bypassValveActive ? 'text-cyan-400 animate-pulse' : 'text-slate-700'}`}
            >
          ...
          <div className="p-2 bg-slate-900/60 rounded border border-white/5 flex flex-col items-center">
            ...
            <div
              className={`text-[10px] font-black ${assetTele.hydrostaticLiftActive ? 'text-amber-400 animate-pulse' : 'text-slate-700'}`}
            >
