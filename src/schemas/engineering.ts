...
export const EngineeringSchema = z.object({
  foundation: z
    .number()
    .max(0.05, { message: 'STANDARD_EXCELLENCE_VIOLATION: Foundation deviation > 0.05 mm/m' }),
  shaft: z
    .number()
    .max(0.02, { message: 'STANDARD_EXCELLENCE_VIOLATION: Shaft run-out > 0.02 mm' }),
  bearing_clearance: z.number().optional(),
  wicket_synchronization: z
    .number()
    .max(1, { message: 'STANDARD_EXCELLENCE_VIOLATION: Wicket sync deviation > 1%' }),
  commissioning_vibration: z
    .number()
    .max(2.5, { message: 'STANDARD_EXCELLENCE_VIOLATION: Vibration > 2.5 mm/s' }),
  ...
  cooling_delta_t: z
    .number()
    .max(15, { message: 'PREDICTIVE_ALARM: Delta T excessive - possible scaling.' })
    .optional(),
});
