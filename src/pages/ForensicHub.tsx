...
} from '../services/InstitutionalKnowledgeService';
...
import {
  ...
  DamStabilityGauge,
  SeismicData,
} from '../services/CivilSecurityModule';
import {
  PredictiveProcurementService,
  ProcurementRequisition,
} from '../services/PredictiveProcurementService';
...
import {
  useTelemetryStore,
  DiagnosticSnapshot,
} from '../features/telemetry/store/useTelemetryStore';
...
const ForensicHub: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<
    'GENERATOR' | 'RUNNER' | 'OVERVIEW' | 'KINETIC' | 'LOGISTICS'
  >('OVERVIEW');
  ...
  const [blackStartStatus, setBlackStartStatus] = useState<{
    active: boolean;
    step: number;
  } | null>(null);
  ...
  const { snapshots, addSnapshot, playbackSnapshot, setPlaybackSnapshot, setExecutiveResult } =
  ...
  useEffect(() => {
    ...
    setInventory(
      [...parts, ...allParts].filter(
        (v, i, a) => a.findIndex(t => t.componentId === v.componentId) === i
      )
    );
    ...
    if (ROIMonitorService.exportEvents().length === 0) {
      ...
      ROIMonitorService.recordEvent({
        timestamp: Date.now() - 100000,
        type: 'PREVENTED_FAILURE',
        amount: 45000,
        description: 'Early bearing failure detection prevented outage',
      });
    }
    ...
    if (AuditTrailService.getAuditTrail('UNIT-1').length === 0) {
      AuditTrailService.logDecision('UNIT-1', {
        ...
        confidence: 85,
        actionPlan: {
          step1_immediate: 'Log event',
          step2_field: 'Inspect draft tube',
          step3_longterm: 'Check alignment',
        },
      });
      AuditTrailService.logDecision('UNIT-1', {
        ...
        confidence: 99,
        actionPlan: {
          step1_immediate: 'Continue operation',
          step2_field: 'None',
          step3_longterm: 'None',
        },
      });
    ...
    const optReport = ConsultingEngine.generateOptimizationReport(
      ...
      {
        getTolerances: () => ({
          vibration_limit: { value: 4.5 },
          shaft_alignment: { value: 0.05 },
        }),
      } as any,
      [
        {
          type: 'geodetic',
          timestamp: Date.now(),
          data: { shaft_deviation: 0.08, foundation_settlement: 1.5 },
        },
        {
          type: 'vibration',
          timestamp: Date.now(),
          data: { rms_vibration: 5.2, dominant_frequency: 120, running_speed: 428 },
        },
      ] as any[],
      []
    );
    ...
    EcoGovernanceUnit.monitorWaterQuality({
      dissolvedOxygen: 6.5,
      temperature: 18,
      pH: 7.2,
      turbidity: 5,
    ...
    const h2Check = H2Synthesizer.checkElectrolysisTrigger({
      islandMode: false,
      marketDemand: 20,
      excessPower: 5,
    });
    ...
    CivilSecurityModule.registerPiezometer({
      id: 'PZ-01',
      location: 'Foundation Left',
      elevation: 85,
      pressure: 2.1,
      upliftForce: 0,
      timestamp: Date.now(),
    ...
    CivilSecurityModule.registerPiezometer({
      id: 'PZ-02',
      location: 'Foundation Right',
      elevation: 85,
      pressure: 1.8,
      upliftForce: 0,
      timestamp: Date.now(),
    });
    CivilSecurityModule.registerSeismicEvent({
      stationId: 'SEIS-MAIN',
      peakGroundAcceleration: 0.01,
      frequency: 12,
      magnitude: 1.2,
      timestamp: Date.now(),
    });
  ...
  useEffect(() => {
    ...
    const alarmCodes = activeAlarms.map(a =>
      a.message.includes('MECHANICAL') ? 'SOVEREIGN_MECHANICAL_ALARM' : a.id
    );
  ...
  const handleHotspotSelect = (id: string, context: any) => {
    ...
    else if (label.includes('TORQUE') || label.includes('ANCHOR') || label.includes('COVER'))
      oracleKey = 'LOOSENESS_PHASE';
  ...
  const handleGenerateReport = async () => {
    ...
    if (currentEff < targetEff) {
      ...
      setCalculatedLoss(
        `EFFICIENCY ALERT: ${currentEff.toFixed(2)} < ${targetEff}. HOURLY LOSS: €${lossEur.toFixed(2)}`
      );
    } else {
  ...
  const handleGenerateServiceReport = async () => {
    ...
    const simulatedMeasurements = [
      ...
      {
        parameter: 'Cavitation (Sigma)',
        asFound: 0.8,
        asLeft: 1.1,
        unit: 'σ',
        standard: 1.0,
        improvement: -37,
      }, // Negative improvement handled by logic
      {
    ...
    alert(
      'Service Report Generation Started. AI Insights:\n\n' +
  ...
  const handleDownloadSnapshot = (snap: DiagnosticSnapshot) => {
    ...
    ForensicReportService.openAndDownloadBlob(
      blob,
      `Forensic_Snapshot_${snap.id.substring(0, 8)}.pdf`,
      true
    );
  };
      ...
      <header className="h-16 border-b border-slate-800 bg-slate-900/50 flex items-center justify-between px-6 backdrop-blur-md z-10">
        ...
        <div className="flex items-center gap-4">
          ...
          >
            ...
            <span
              className={`text-xs font-bold uppercase tracking-wider ${rpm > 5 ? 'text-red-400' : 'text-white'}`}
            >
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 relative bg-grid-pattern">
          ...
          {activeTab === 'KINETIC' ? (
            ...
            <ForensicVisualizer viewMode={activeTab as any} onHotspotSelect={handleHotspotSelect} />
          )}
          ...
          <AnimatePresence>
            {calculatedLoss && (
              ...
              >
                ...
                <div className="text-xs font-mono text-red-200">{calculatedLoss}</div>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
            ...
            {rightPanelTab === 'ORACLE' ? (
              <>
                    ...
                    {selectedContext ? (
                      ...
                      >
                        <div className="text-cyan-400 font-bold text-lg mb-1">
                          {selectedContext.component}
                        </div>
                        <div className="text-slate-400 text-sm font-mono">
                          {selectedContext.value || 'Monitoring...'}
                        </div>
                      </motion.div>
                    ) : (
                      <motion.div
                      ...
                      >
                        <span className="text-slate-600 text-xs uppercase font-bold">
                          Select a component
                        </span>
                      </motion.div>
                    )}
                ...
                <div className="flex-1">
                  <h3 className="text-xs font-black text-slate-500 uppercase tracking-widest mb-4 flex items-center justify-between">
                    ...
                    <span className="px-2 py-0.5 bg-cyan-900/30 text-cyan-400 rounded text-[10px]">
                      LIVE
                    </span>
                  </h3>
                      ...
                      onClick={async () => {
                        const causeNode = {
                          ...
                          value: playbackSnapshot
                            ? playbackSnapshot.telemetry.vibrationX || 0
                            : 4.5,
                          contribution: 1.0,
                        ...
                        const chain = {
                          ...
                          description: playbackSnapshot
                            ? `Replay of ${playbackSnapshot.id}`
                            : 'Manual Trigger via Forensic Hub',
                        };
                    ...
                    <div className="mb-4">
                      ...
                      <div className="space-y-2">
                        {auditEntries.map(entry => (
                          <div
                            key={entry.id}
                            className="p-2 rounded bg-slate-900/40 border border-slate-700 text-[10px]"
                          >
                            <div className="flex justify-between text-slate-400 mb-1">
                              <span>{new Date(entry.timestamp).toLocaleTimeString()}</span>
                              <span
                                className={
                                  entry.severity === 'CRITICAL' ? 'text-red-400' : 'text-cyan-400'
                                }
                              >
                                {entry.action}
                              </span>
                      ...
                      >
                          ...
                          <div className="mb-2 p-2 bg-slate-900/50 rounded border border-slate-700">
                            <div
                              className={`text-[9px] font-mono font-bold uppercase ${
                                snap.physicsAnalysis.zone.zone === 'OPTIMAL'
                                  ? 'text-emerald-400'
                                  : 'text-amber-400'
                              }`}
                            >
                            ...
                            <div
                              className={`text-[9px] ${
                                snap.physicsAnalysis.cavitation.risk === 'LOW'
                                  ? 'text-slate-400'
                                  : 'text-red-400'
                              }`}
                            >
                              ...
                              <div className="mt-2 pt-2 border-t border-slate-700/50">
                                <div
                                  className={`text-[9px] font-bold ${
                                    snap.physicsAnalysis.vibration.severity === 'NOMINAL'
                                      ? 'text-indigo-400'
                                      : 'text-red-400'
                                  }`}
                                >
                              ...
                              <div className="mt-2 pt-2 border-t border-slate-700/50">
                                <div
                                  className={`text-[9px] font-bold uppercase flex items-center gap-1 ${
                                    snap.oilAnalysis.overallHealth === 'EXCELLENT' ||
                                    snap.oilAnalysis.overallHealth === 'GOOD'
                                      ? 'text-emerald-400'
                                      : 'text-orange-400'
                                  }`}
                                >
                                ...
                                {snap.energyHarvest && (
                                  <div>
                                    ...
                                    <div className="text-[8px] text-slate-400">
                                      {snap.energyHarvest.powerW.toFixed(0)}W / €
                                      {snap.energyHarvest.annualEur.toFixed(0)}
                                    </div>
                                  </div>
                                )}
                                {snap.lifeExtension && (
                              ...
                              <div className="mt-2 pt-2 border-t border-slate-700/50">
                                {snap.structuralIntegrity && (
                                  <div className="flex justify-between items-center text-[9px] mb-1">
                                    ...
                                    <span
                                      className={`font-bold ${snap.structuralIntegrity.status === 'NORMAL' ? 'text-emerald-400' : 'text-red-400'}`}
                                    >
                                      {snap.structuralIntegrity.marginPct.toFixed(1)}% (
                                      {snap.structuralIntegrity.status})
                                    </span>
                                  </div>
                                )}
                            ...
                            {(snap.acousticFingerprint ||
                              snap.crossCorrelation ||
                              snap.galvanicCorrosion) && (
                              <div className="mt-2 pt-2 border-t border-slate-700/50">
                                {snap.acousticFingerprint && (
                                  <div
                                    className={`text-[9px] font-bold uppercase flex items-center gap-1 mb-1 ${
                                      snap.acousticFingerprint.severity === 'NORMAL'
                                        ? 'text-purple-300'
                                        : 'text-purple-400 animate-pulse'
                                    }`}
                                  >
                                    <Activity className="w-2 h-2" />
                                    ACOUSTIC: {snap.acousticFingerprint.primaryPattern} (
                                    {snap.acousticFingerprint.confidence.toFixed(0)}%)
                                  </div>
                                )}
                                {snap.crossCorrelation?.correlated && (
                                  <div className="text-[9px] font-bold text-indigo-400 flex items-center gap-1 mb-1">
                                    <TrendingUp className="w-2 h-2" />
                                    SYNERGY: {snap.crossCorrelation.pair} (r=
                                    {snap.crossCorrelation.r.toFixed(2)})
                                  </div>
                                )}
                                {snap.galvanicCorrosion && (
                                  <div
                                    className={`text-[9px] font-bold flex items-center gap-1 ${
                                      snap.galvanicCorrosion.protectionLevel === 'RISK'
                                        ? 'text-red-400'
                                        : 'text-orange-300'
                                    }`}
                                  >
                              ...
                              <div className="mt-2 pt-2 border-t border-slate-700/50">
                                {snap.thermalAnalysis && (
                                  <div className="text-[9px] font-bold text-blue-300 flex items-center gap-1 mb-1">
                                    <TrendingUp className="w-2 h-2" />
                                    THERMAL: {snap.thermalAnalysis.viscosity} cP (
                                    {snap.thermalAnalysis.action})
                                  </div>
                                )}
                                {snap.wicketGateAnalysis && (
                                ...
                                {snap.generatorAirGap && (
                                  <div
                                    className={`text-[9px] font-bold flex items-center gap-1 ${
                                      snap.generatorAirGap.action !== 'NO_ACTION'
                                        ? 'text-red-400'
                                        : 'text-emerald-300'
                                    }`}
                                  >
                            ...
                            {(snap.transformerAnalysis ||
                              ...
                              <div className="mt-2 pt-2 border-t border-slate-700/50">
                                {snap.transformerAnalysis && (
                                  <div
                                    className={`text-[9px] font-bold flex items-center gap-1 mb-1 ${
                                      snap.transformerAnalysis.faultType !== 'NORMAL'
                                        ? 'text-red-400'
                                        : 'text-emerald-300'
                                    }`}
                                  >
                                ...
                                {snap.statorAnalysis && (
                                  <div
                                    className={`text-[9px] font-bold flex items-center gap-1 mb-1 ${
                                      snap.statorAnalysis.severity !== 'LOW'
                                        ? 'text-orange-400'
                                        : 'text-emerald-300'
                                    }`}
                                  >
                                ...
                                {snap.shaftSealAnalysis && (
                                  <div
                                    className={`text-[9px] font-bold flex items-center gap-1 mb-1 ${
                                      snap.shaftSealAnalysis.action !== 'NO_ACTION'
                                        ? 'text-purple-400'
                                        : 'text-slate-300'
                                    }`}
                                  >
                                    <TrendingUp className="w-2 h-2" />
                                    SEAL: {(snap.shaftSealAnalysis.probability * 100).toFixed(0)}%
                                    Risk
                                  </div>
                                )}
                                {snap.governorAnalysis && (
                                  <div
                                    className={`text-[9px] font-bold flex items-center gap-1 mb-1 ${
                                      snap.governorAnalysis.action !== 'NO_ACTION'
                                        ? 'text-amber-400'
                                        : 'text-slate-300'
                                    }`}
                                  >
                        ...
                        <div className="flex gap-2">
                          <button
                            ...
                            className={`flex-1 py-1.5 rounded text-[10px] font-bold uppercase tracking-wider flex items-center justify-center gap-1 transition-colors ${
                              playbackSnapshot?.id === snap.id
              ...
              <div className="flex-1 flex flex-col">
                ...
                {daoOrders.length > 0 && (
                  <div className="mt-4 border-t border-slate-700 pt-4">
                    <h4 className="text-[10px] font-bold text-slate-500 uppercase mb-2">
                      Smart Contract Orders (DAO)
                    </h4>
                    {daoOrders.map(order => (
                      <div
                        key={order.orderId}
                        className="p-2 bg-slate-900/50 rounded border border-slate-700 mb-2"
                      >
                        <div className="flex justify-between items-center mb-1">
                          <span className="text-[10px] text-white font-bold">{order.partId}</span>
                          <span className="text-[9px] text-cyan-400 bg-cyan-950/30 px-1 rounded">
                            {order.status}
                          </span>
                        </div>
                      ...
                      >
                          ...
                          <div className="flex-1">
                            ...
                            <div className="flex items-center gap-2 text-[10px] text-slate-400">
                              ...
                              <span className="text-emerald-400">
                                {(result.entry.confidenceScore * 100).toFixed(0)}% Confidence
                              </span>
                            </div>
                          </div>
                        ...
                        <div className="flex flex-wrap gap-1">
                          {result.entry.tags.map(tag => (
                            <span
                              key={tag}
                              className="px-1.5 py-0.5 bg-blue-900/20 text-blue-300 rounded text-[9px] border border-blue-500/20"
                            >
                              #{tag}
                            </span>
                          ))}
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {(
                      ...
                      <button
                        ...
                        onClick={() => {
                          const result =
                            LegacyBridgeService.getInstance().generateLegacyReport(type);
                          if (result) {
              ...
              <div className="flex-1 flex flex-col gap-4 overflow-y-auto pr-2">
                <h3 className="text-xs font-black text-slate-500 uppercase tracking-widest mb-4 flex items-center justify-between">
                  ...
                  <span className="px-2 py-0.5 bg-teal-900/30 text-teal-400 rounded text-[10px]">
                    ROI FOCUSED
                  </span>
                </h3>
                {optimizationReport ? (
                  <div className="space-y-4">
                    {optimizationReport.recommendations.map((rec: any, i: number) => (
                      <div
                        key={i}
                        className="p-3 rounded-lg border bg-slate-800/40 border-slate-700 hover:bg-slate-800 transition-colors"
                      >
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          ...
                          <div>
                            ...
                            <span className="text-sm font-bold text-white">
                              {fishState.gateOpeningPct.toFixed(1)}%
                            </span>
                          </div>
                        </div>
                      ...
                      <div className="p-4 rounded-xl border bg-purple-950/20 border-purple-500/30">
                        ...
                        <div className="space-y-1">
                          {basinAction.unitActions.map((ua, i) => (
                            <div
                              key={i}
                              className="text-[10px] flex justify-between bg-slate-900/50 p-1.5 rounded"
                            >
                      ...
                      <div className="p-4 rounded-xl border bg-blue-950/20 border-blue-500/30">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          ...
                          <div>
                            ...
                            <span className="text-sm font-bold text-white">
                              {h2Session.h2Produced.toFixed(1)} Nm³
                            </span>
                          </div>
                        </div>
                        <div className="mt-2 text-[10px] text-slate-400 font-mono">
              ...
              <div className="flex-1 flex flex-col gap-4 overflow-y-auto pr-2">
                <h3 className="text-xs font-black text-slate-500 uppercase tracking-widest mb-4 flex items-center justify-between">
                  ...
                  <span className="px-2 py-0.5 bg-red-900/30 text-red-400 rounded text-[10px]">
                    LIVE
                  </span>
                </h3>
                ...
                {damStability && (
                  <div
                    className={`p-4 rounded-xl border ${
                      damStability.status === 'SAFE'
                        ? 'bg-emerald-950/20 border-emerald-500/30'
                        : damStability.status === 'WARNING'
                  ...
                  >
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <span className="text-lg font-mono text-white">
                          {damStability.safetyFactor.toFixed(2)}
                        </span>
                      </div>
                      <div>
                        <span className="text-[9px] text-slate-500 block">UPLIFT PRESS.</span>
                        <span className="text-lg font-mono text-white">
                          {damStability.upliftPressure.toFixed(2)} bar
                        </span>
                      </div>
                    </div>
                        ...
                        <div className="flex justify-between items-center mb-1">
                          ...
                          <span
                            className={`text-[9px] px-1 rounded ${
                              event.peakGroundAcceleration > 0.05
                  ...
                  {integrityReport ? (
                    <div className="p-3 bg-slate-900/50 rounded border border-slate-700">
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <span
                          className={`text-[10px] font-bold ${Object.values(integrityReport.tableStatuses).every(s => s.exists) ? 'text-emerald-400' : 'text-red-400'}`}
                        >
                          {
                            Object.values(integrityReport.tableStatuses).filter(s => s.exists)
                              .length
                          }{' '}
                          / {Object.keys(integrityReport.tableStatuses).length}
                        </span>
                      </div>
                      ...
                      {integrityReport.physicsCheck && (
                        <div className="flex justify-between items-center mb-2">
                          ...
                          <span
                            className={`text-[10px] font-bold ${integrityReport.physicsCheck.ok ? 'text-emerald-400' : 'text-red-400'}`}
                          >
                      ...
                      {integrityReport.financialCheck && (
                        <div className="flex justify-between items-center">
                          ...
                          <span
                            className={`text-[10px] font-bold ${integrityReport.financialCheck.ok ? 'text-emerald-400' : 'text-red-400'}`}
                          >
              ...
              <div className="flex-1 flex flex-col gap-4 overflow-y-auto pr-2">
                ...
                <div className="p-4 rounded-xl border bg-emerald-950/20 border-emerald-500/30">
                  ...
                  <div className="space-y-2">
                    {roiEvents.slice(0, 3).map((e, i) => (
                      <div
                        key={i}
                        className="text-[10px] flex justify-between items-center border-b border-slate-800 pb-1 last:border-0"
                      >
                ...
                {complianceReport && (
                  <div className="p-4 rounded-xl border bg-blue-950/20 border-blue-500/30">
                    ...
                    <div className="flex justify-between items-center text-xs">
                      ...
                      <span
                        className={`font-black ${complianceReport.ecologistProof.status === 'COMPLIANT' ? 'text-emerald-400' : 'text-red-400'}`}
                      >
                    ...
                    <div className="flex justify-between items-center text-xs mt-1">
                      ...
                      <span className="text-white font-mono">
                        {complianceReport.safetyAudit.totalTripEvents}
                      </span>
                    </div>
                  </div>
                )}

                <h3 className="text-xs font-black text-slate-500 uppercase tracking-widest mt-2">
                  Strategic Planner
                </h3>
                      ...
                      <div className="grid grid-cols-2 gap-2 text-xs font-mono">
                        <div className="bg-slate-900/50 p-2 rounded">
                          ...
                          <span className="text-white font-bold">
                            {new Date(outagePlan.optimalWindow.start).toLocaleDateString()}
                          </span>
                          <span className="block text-slate-400">
                            {new Date(outagePlan.optimalWindow.start).toLocaleTimeString([], {
                              hour: '2-digit',
                              minute: '2-digit',
                            })}
                          </span>
                        ...
                        <div className="bg-slate-900/50 p-2 rounded">
                          ...
                          <span className="text-emerald-400 font-bold">
                            €{outagePlan.optimalWindow.avgPrice.toFixed(2)}
                          </span>
                            ...
                            <li key={i} className="text-xs">
                              ...
                              <div className="flex flex-wrap gap-1">
                                {b.components.map((c: string) => (
                                  <span
                                    key={c}
                                    className="px-1.5 py-0.5 bg-amber-900/40 text-amber-300 rounded text-[10px] border border-amber-700/50"
                                  >
                                    {c}
                                  </span>
                                ))}
                ...
                <div className="p-4 rounded-xl border bg-slate-800/40 border-slate-700 mt-4">
                  ...
                  <div className="text-xs text-slate-400 mb-3">
                    Autonomous recovery from total grid blackout using H2 fuel cells and DC battery
                    bank.
                  </div>
                  <button
                    onClick={() => {
                  ...
                  >
                    {blackStartStatus?.active
                      ? `Recovering... Step ${blackStartStatus.step}/8`
                      : 'Test Black Start'}
                  </button>
                </div>
              </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-xl font-black text-white uppercase tracking-widest">
                      Emergency SOP Generator
                    </h2>
                    <div className="text-xs text-red-400 font-mono">
                      LIVE GENERATED • {new Date(sopData.generatedAt).toLocaleString()}
                    </div>
                  </div>
                </div>
                <button
                  ...
                  <div className="p-6 bg-slate-950/50 rounded-xl border border-slate-800">
                    ...
                    <ul className="space-y-3">
                      {sopData.activeCompensations.map((comp, i) => (
                        <li
                          key={i}
                          className="text-xs text-slate-300 bg-amber-950/20 p-3 rounded border border-amber-500/20"
                        >
                          {comp}
                        </li>
                      ))}
                  ...
                  <div className="p-6 bg-slate-950/50 rounded-xl border border-slate-800">
                    ...
                    <ul className="space-y-3">
                      {sopData.manualInterventions.map((action, i) => (
                        <li
                          key={i}
                          className="text-xs text-slate-300 bg-cyan-950/20 p-3 rounded border border-cyan-500/20"
                        >
                          {action}
                        </li>
                      ))}
                  ...
                  <div className="p-6 bg-red-950/10 rounded-xl border border-red-500/30">
                    ...
                    <div className="space-y-2">
                      {sopData.emergencyProcedures.map((proc, i) => (
                        <div
                          key={i}
                          className={`text-xs p-2 rounded ${proc.startsWith('===') ? 'font-black text-red-400 mt-4 bg-red-950/30' : 'text-slate-300 pl-4 border-l-2 border-slate-700'}`}
                        >
                          {proc}
                        </div>
                      ))}
            ...
            >
              <div className="relative">
                ...
                <AlignmentWizard
                  ...
                  asset={
                    selectedAsset ? mapAssetToEnhancedAsset(selectedAsset) : (undefined as any)
                  }
                  onComplete={() => {
