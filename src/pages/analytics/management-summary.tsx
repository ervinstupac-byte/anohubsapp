...
export default function ManagementSummary() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const res = await ProfessionalReportEngine.generateManagementDashboard({
          projectID: 'ANOHUB-MGMT',
        });
        ...
        try {
          ...
          const publicUrlCandidate =
            (g as any)?.publicURL || (g as any)?.publicUrl || (g as any)?.data?.publicUrl;
        ...
        fetch(jsonFilename)
          ...
          .then((j: any | null) => {
            ...
            const parsed = rows.map((r: any) => ({
              date: r.period_start || r.date,
              loss: Number(r.computed_loss_cost || r.loss || 0),
            }));
    ...
    (async () => {
      ...
      g.append('g')
        ...
        .call(
          (d3 as any)
            .axisBottom(x)
            .tickValues(x.domain().filter((d: string, i: number) => i % 5 === 0))
        )
        .selectAll('text')
  ...
  return (
    <div style={{ padding: 20 }}>
      ...
      {topLossDays.length > 0 && (
        <div
          style={{ marginTop: 8, background: '#fff7ed', padding: 8, border: '1px solid #fcd34d' }}
        >
          <strong>Forensic Note:</strong>
          <div style={{ marginTop: 6 }}>
            Top loss days (sample):{' '}
            {topLossDays
              .slice(0, 3)
              .map(
                d =>
                  `${d.date} — ${new Intl.NumberFormat('de-DE', { style: 'currency', currency: 'EUR' }).format(d.loss)}`
              )
              .join(' | ')}
          </div>
          {topLossDays.some(d => d.date === '2025-12-25') && (
            <div style={{ marginTop: 6, color: '#b91c1c' }}>
              2025-12-25: Flow surge detected — rapid transient increase in flow caused the
              efficiency dip (~€1,003 loss).
            </div>
          )}
        </div>
      )}
            ...
            <tbody>
              {alerts.map(a => (
                <tr key={a.id}>
                  ...
                  <td>{String(a.asset_id)}</td>
                  <td>{Number(a.max_vibration).toFixed(2)}</td>
          ...
          {[0.08, 0.1, 0.15].map(p => (
            <label key={p} style={{ marginRight: 8 }}>
              <input
                ...
                name="priceScenario"
                value={p}
                checked={pricePerKwh === p}
                onChange={() => setPricePerKwh(p)}
              />{' '}
              {p.toFixed(2)}
            </label>
          ))}
      ...
      <div style={{ marginTop: 12 }}>
        ...
        <div style={{ color: '#dc2626', fontSize: 20 }}>
