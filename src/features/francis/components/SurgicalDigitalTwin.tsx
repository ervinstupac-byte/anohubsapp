 ...
 */
const SurgicalDigitalTwin: React.FC<Props> = ({
  viewMode,
  setViewMode,
  activeAssetId,
  xrayEnabled,
  ...
  useEffect(() => {
    ...
    fetch(path)
      ...
      .catch(err => {
        ...
        setSvgContent(
          `<div class="text-rose-500 font-mono text-xs uppercase">[LOAD_FAILURE] ${err.message}</div>`
        );
      });
  }, [viewMode]);
  ...
  useEffect(() => {
    ...
    const onPointerUp = (ev: PointerEvent) => {
      if (pointerId !== null) {
        try {
          (ev.target as Element).releasePointerCapture(pointerId);
        } catch (e) {}
      }
  ...
  useEffect(() => {
    ...
    const xrayGroups = Array.from(
      svgRoot.querySelectorAll('#group-runner, #group-seal')
    ) as HTMLElement[];
  ...
  return (
    <div className="w-full h-full flex items-center justify-center p-4 svg-outer relative">
      ...
      <div className="absolute top-3 right-3 z-30 flex flex-col gap-2">
        <button onClick={zoomIn} className="px-2 py-1 bg-black/40 text-white rounded">
          +
        </button>
        <button onClick={zoomOut} className="px-2 py-1 bg-black/40 text-white rounded">
          −
        </button>
        <button onClick={resetZoom} className="px-2 py-1 bg-black/40 text-white rounded">
          Reset
        </button>
