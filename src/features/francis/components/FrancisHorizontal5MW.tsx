...
export const FrancisHorizontal5MW: React.FC = () => {
    ...
    (async () => {
      try {
        const { data } = await supabase
          ...
          .select(
            "parameters->>family as family, parameters->>variant as variant, parameters->'design_points' as design_points, parameters->'hotspot_map' as hotspot_map, parameters->'specs' as specs"
          )
          .eq('asset_id', assetId)
        ...
        const parsed: any = {};
        try {
          parsed.family = (data as any).family;
        } catch {}
        try {
          parsed.variant = (data as any).variant;
        } catch {}
        try {
        ...
        try {
          ...
          console.warn('Failed to load threshold configs', e);
        }
      } catch (e) {
  ...
  useEffect(() => {
    ...
    if (missing.length > 0)
      console.warn('Missing hotspot_map entries for expected instrumented ids:', missing);
  }, [design]);
  ...
  useEffect(() => {
    ...
    async function loadDiagnostics() {
      ...
      try {
        ...
        const expertCurve = await fetchExpertCurve(
          assetId,
          (design as any)?.family,
          (design as any)?.variant
        );

        // fetch hydrology_context for asset's plant (if available)
        try {
          const plantId =
            (selectedAsset as any)?.specs?.plant_id ||
            (selectedAsset as any)?.plantId ||
            (selectedAsset as any)?.plant_id;
          if (plantId) {
            const { data: hc } = await supabase
              .from('hydrology_context')
              .select('*')
              .eq('plant_id', plantId)
              .single();
          ...
          console.warn('Failed to load hydrology_context', e);
        }

        setExpertCurveState(expertCurve || null);

        const result = evaluateAging(
          telemetryWindow,
          (design as any)?.design_points?.design_eta,
          {
            design_flow_cms: (design as any)?.specs?.design_flow_cms,
            gross_head_m: (design as any)?.specs?.gross_head_m,
          },
          expertCurve
        );
        if (!mounted) return;
        ...
        try {
          document
            .querySelectorAll('[data-hotspot-id].francis-selected')
            .forEach(el => el.classList.remove('francis-selected'));
          const els = Array.from(
            document.querySelectorAll(`[data-hotspot-id="${selectedHotspot}"]`)
          );
  ...
  const HOTSPOT_GUIDANCE: Record<string, string> = {
    shaft:
      'Main mechanical linkage between the runner and generator. Monitor vibration spectral analysis for 1x frequency peaks in telemetry_logs. Excessive vibration accelerates fatigue—keep levels within expert_knowledge limits for 50+ year asset life.',
    wicket_gates:
      'Vanes regulating flow and direction. Asymmetric gate positioning or mechanical play causes turbulence and runner cavitation. Operating outside optimal expert_efficiency_curves triggers the Aging Meter and reduces long-term efficiency η.',
  };
  ...
  const advisory = (() => {
    ...
    const vib = Number(
      latestTelemetry.vibration ?? latestTelemetry.francis_data?.stay_ring_vibration ?? 0
    );
    const temp = Number(
      latestTelemetry.temperature ?? latestTelemetry.francis_data?.temperature ?? 0
    );
    ...
    if (dev > effThresh)
      parts.push(
        `Efficiency Δ ${(dev * 100).toFixed(2)}% (threshold ${(effThresh * 100).toFixed(2)}%)`
      );
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div>
            ...
            {Object.keys(hotspots).length === 0 && (
              <div className="text-sm text-slate-500">No hotspot_map defined for this asset.</div>
            )}
            ...
            <div className="bg-black/5 p-4 rounded border border-slate-800">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <div className="text-sm">
                    {latestTelemetry?.output_power ? String(latestTelemetry.output_power) : '—'}
                  </div>
                </div>
                <div>
              ...
              <div className="mt-4">
                ...
                <div className="mt-2 w-full bg-slate-900 rounded h-3 overflow-hidden">
                  ...
                  <div
                    className={`h-3 bg-gradient-to-r ${eta.greaterThan(0.85) ? 'from-emerald-500 to-green-400' : eta.greaterThan(0.7) ? 'from-yellow-500 to-yellow-400' : 'from-red-600 to-red-400'}`}
                    style={{ width: `${100 - eta.times(100).toNumber()}%` }}
                  />
                </div>
                <div className="text-xs text-slate-400 mt-2">
                  Lower value indicates higher cumulative aging pressure.
                </div>
              </div>
              ...
              <div className="mt-4 text-sm text-slate-200">
                {selectedHotspot ? (
                  <>
                    ...
                    <div className="mt-3 p-3 bg-black/10 rounded border border-slate-800 text-sm text-slate-300">
                      ...
                      <div className="text-xs text-slate-300">
                        Head: {hydrologyContext?.gross_head_m ?? design?.specs?.gross_head_m ?? '—'}{' '}
                        m
                      </div>
                      <div className="text-xs text-slate-300">
                        Design Flow:{' '}
                        {hydrologyContext?.design_flow_cms ?? design?.specs?.design_flow_cms ?? '—'}{' '}
                        m³/s
                      </div>
                      ...
                      <div className="font-mono text-sm text-white">
                        {(() => {
                          const q = Number(
                            latestTelemetry?.francis_data?.flowRate ?? latestTelemetry?.flow ?? 0
                          );
                          const opt = expertCurveState
                            ? interpolateOptimalEta(expertCurveState, q)
                            : null;
                          const optStr = opt == null ? '—' : `${(opt * 100).toFixed(2)}%`;
                          const dev =
                            opt != null && eta
                              ? ((eta.toNumber() - opt) * 100).toFixed(2) + '%'
                              : '—';
                          return `${optStr} (Δ ${dev})`;
                        })()}
                      </div>
                    ...
                    <div className="text-slate-400 text-xs mt-1">
                      Linked DB id: {String(hotspots[selectedHotspot])}
                    </div>
                    <div className="mt-3 flex items-center gap-2">
                      ...
                      {hotspotAging && (
                        <div className="text-sm ml-2">
                          ...
                          <div
                            className={`font-black ${hotspotAging.warningLevel === 'CRITICAL' ? 'text-red-400' : hotspotAging.warningLevel === 'WARNING' ? 'text-amber-400' : 'text-emerald-400'}`}
                          >
                            {hotspotAging.warningLevel}
                          </div>
                        </div>
                      )}
                    </div>
                          ...
                          {hotspotLogs.map((l: any) => (
                            <li key={l.id} className="border-b border-white/5 py-1">
                              <div className="text-xs text-slate-400">
                                {new Date(l.created_at).toLocaleString()} — {l.severity}
                              </div>
                  ...
                  <div className="text-slate-400">
                    Select a hotspot to view part mapping and guidance.
                  </div>
                )}
              </div>
            </div>
