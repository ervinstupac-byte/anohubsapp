...
import { ScenarioControl } from '../../../components/demo/ScenarioControl';
import { SystemAuditLog } from '../../../components/ui/SystemAuditLog';
...
export const UniversalTurbineDashboard: React.FC = () => {
    ...
    const rcaResults = useTelemetryStore((state: any) => state.rcaResults);
    ...
    const handleExportPDF = () => {
        ...
        toast.innerHTML = `
            <div class="p-2 bg-emerald-500/20 rounded-full">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
            </div>
            <div>
                    ...
                    onClick={() => {
                        import('../../../services/demo/DemoStorytellerService').then(({ DemoStorytellerService }) => {
                            DemoStorytellerService.runMasterDemo(
                                async (type: TurbineType) => setDemoOverrideType(type),
                                (level: string, src: string, msg: string) => console.log(`[${level}] ${src}: ${msg}`)
                            );
                        });
                    }}
                            ...
                            {activeType === 'PELTON' && (
                                <div className="space-y-4">
                                    <div className="p-4 bg-fuchsia-950/30 border border-fuchsia-500/30 rounded-lg">
                                        <h3 className="text-fuchsia-400 font-bold mb-1">Nozzle Status</h3>
                                        <p className="text-[10px] text-slate-400 mb-3">N1✓ N2✓ N3⚠ N4✓ N5✓ N6✓</p>
                                        <h4 className="text-fuchsia-400 font-bold mb-2">Multi-Nozzle Force Balance</h4>
