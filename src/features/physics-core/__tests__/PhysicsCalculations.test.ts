...
describe('Physics Calculation Logic', () => {
  ...
  describe('Structural: Hoop Stress (Barlow)', () => {
    ...
    it('scales hoop stress linearly with diameter', () => {
      ...
      const surgePressurePa = new Decimal(0);
      const thickness = new Decimal(0.02);
      const stress2m = calculateHoopStress(
        head,
        surgePressurePa,
        new Decimal(2),
        thickness
      ).toNumber();
      const stress3m = calculateHoopStress(
        head,
        surgePressurePa,
        new Decimal(3),
        thickness
  ...
  describe('Turbine: Threshold Tables', () => {
    it('returns Francis thresholds', () => {
      expect(getTurbineThresholds('FRANCIS')).toEqual({
        foundationMax: 0.04,
        shaftMax: 0.015,
        vibrationMax: 1.8,
      });
    ...
    it('returns defaults for unknown turbine type', () => {
      expect(getTurbineThresholds('UNKNOWN')).toEqual({
        foundationMax: 0.05,
        shaftMax: 0.02,
        vibrationMax: 2.0,
      });
