...
export const calculateOperatingZone = (
  ...
  if (loadPct > 110) {
    return {
      zone: 'OVERLOAD',
      message: 'Exceeding rated capacity. Monitor temperature.',
      efficiencyDetails: 'Efficiency drops due to turbulence.',
    };
  } else if (loadPct >= 80) {
    return {
      zone: 'OPTIMAL',
      message: 'Operating within peak efficiency range.',
      efficiencyDetails: 'Peak efficiency.',
    };
  } else if (loadPct >= 50) {
    return {
      zone: 'PARTIAL',
      message: 'Acceptable but suboptimal efficiency.',
      efficiencyDetails: 'Efficiency reduced by ~5-10%.',
    };
  } else {
    return {
      zone: 'ROUGH_ZONE',
      message: 'High turbulence and cavitation risk. Avoid prolonged operation.',
      efficiencyDetails: 'Efficiency significantly degraded.',
    };
  }
};
 ...
 */
export const calculateFrictionFactor = (
  Re: Decimal,
  roughnessMM: Decimal,
  diameter: Decimal
): Decimal => {
...
export const calculateWaveVelocity = (
  diameter: Decimal,
  thickness: Decimal,
  modulusPa: Decimal
): Decimal => {
