...
import { getLastAutoTableFinalY, setGStateOpacity } from '../../utils/pdfHelpers';
...
export const ProfessionalReportEngine = {
  generateTechnicalAudit: async (
    state: TechnicalProjectState,
    projectID: string = 'ANOHUB-2025-X'
  ) => {
    const doc = new jsPDF();
    const profile = ProfileLoader.getProfile(
      state.selectedAsset?.turbine_type || state.identity?.turbineType || 'FRANCIS'
    );
    ...
    if (
      state.selectedAsset?.specs?.assemblySequence &&
      state.selectedAsset.specs.assemblySequence.length > 0
    ) {
      doc.addPage();
    ...
    try {
      ...
      const computedLoss =
        (impact.lostRevenueEuro || 0) +
        (impact.potentialDamageEUR || 0) +
        (impact.leakageCostYearly || 0);

      // Estimate investment (I_total): prefer explicit buffer or fall back to maintenanceBuffer
      const I_total =
        (state.financials && (state.financials.maintenanceBufferEuro || 0)) ||
        impact.maintenanceBufferEuro ||
        150000;

      // P_avg (MW)
      const P_avg_MW =
        (state.hydraulic?.baselineOutputMW && (state.hydraulic.baselineOutputMW as any).toNumber
          ? (state.hydraulic.baselineOutputMW as any).toNumber()
          : (state.hydraulic?.baselineOutputMW as any)) || 5;
      ...
      let currentEff =
        typeof state.hydraulic?.efficiency === 'number' ? state.hydraulic.efficiency : null;
      try {
        let baselineOutput: any =
          state.hydraulic?.baselineOutputMW && (state.hydraulic.baselineOutputMW as any).toNumber
            ? (state.hydraulic.baselineOutputMW as any).toNumber()
            : (state.hydraulic?.baselineOutputMW as any);
        if (baselineOutput == null) baselineOutput = 5;
        if (baselineOutput && typeof (baselineOutput as any).toNumber === 'function')
          baselineOutput = (baselineOutput as any).toNumber();

        const flowVal =
          (state.hydraulic as any)?.flowRate ??
          (state.hydraulic as any)?.flow ??
          (state.hydraulic as any)?.designFlow;
        const headVal =
          (state.hydraulic as any)?.netHead ??
          (state.hydraulic as any)?.head ??
          (state.hydraulic as any)?.waterHead ??
          (state.hydraulic as any)?.grossHead;

        const etaRes = computeEta({
          powerMW: Number(baselineOutput),
          flow: Number(flowVal),
          head: Number(headVal),
        });
      ...
      if (currentEff === null || currentEff === undefined) {
        ...
        const designEff = designEfficiencyFor(
          (state.hydraulic as any)?.flow || (state.hydraulic as any)?.flowRate || 9.0,
          (state.hydraulic as any)?.head || (state.hydraulic as any)?.netHead || 64.5
        );
      ...
      const saveRes = await reportService.saveReport({
        ...
        metadata: {
          ...
          break_even_date: T_ROI_years
            ? new Date(Date.now() + Math.round(T_ROI_years * 365 * 24 * 3600 * 1000)).toISOString()
            : undefined,
      ...
      if (saveRes?.error) {
        console.warn(
          '[ProfessionalReportEngine] reportService.saveReport returned error:',
          saveRes.error
        );
      }
    } catch (e) {
      console.error(
        '[ProfessionalReportEngine] failed to persist report metadata:',
        (e as any)?.message || e
      );
    }
  ...
  generateManagementDashboard: async (opts?: { projectID?: string; assetId?: string | number }) => {
    ...
    const { data: rows, error } = await supabase
      .from('eta_aggregates')
      .select(
        'id, asset_id, period_start, period_end, avg_eta, optimal_eta, computed_loss_cost, metadata'
      )
    ...
    const trend = Array.from(byDate.entries()).map(([date, v]) => ({
      date,
      avg_eta: v.count ? v.sum / v.count : 0,
      samples: v.count,
    }));
    ...
    if (assetIds.length > 0) {
      const { data: tcfgs } = await supabase
        .from('threshold_configs')
        .select('asset_id, vibration_mm_s')
        .in('asset_id', assetIds);
      (tcfgs || []).forEach((t: any) => {
        thresholdsMap[String(t.asset_id)] = Number(t.vibration_mm_s || 4.5);
      });
    }
    ...
    for (const r of list) {
      ...
      if (meta.francis && meta.francis.max_vibration !== undefined)
        vibCandidates.push(Number(meta.francis.max_vibration));

      const maxVib = vibCandidates
    ...
    try {
      const { data: finRows, error: finErr } = await supabase
        .from('asset_financials_with_eta')
        .select('computed_loss_cost, period_start, period_end')
        .gte('period_start', startStr)
        .lte('period_end', endStr);
      if (!finErr && Array.isArray(finRows) && finRows.length > 0) {
        const sumFin = finRows.reduce(
          (acc: number, fr: any) => acc + Number(fr.computed_loss_cost || 0),
          0
        );
    ...
    if (alerts.length === 0) {
      doc.text(
        'No vibration alerts exceeding configured thresholds were detected in the 30-day window.',
        20,
        ty
      );
      ty += 8;
    ...
    try {
      ...
      const outDir = isVercel
        ? process.env.REPORT_TMP_DIR || '/tmp/anohub'
...
const drawExecutiveRiskBrief = (doc: jsPDF, state: TechnicalProjectState, id: string) => {
  ...
  doc.setTextColor(220, 38, 38);
  doc.text(
    `${(state.financials?.lostRevenueEuro || 0).toLocaleString('de-DE', { style: 'currency', currency: 'EUR' })}`,
    MARGIN + 80,
    95
  );
  ...
  doc.setTextColor(33, 33, 33);
  doc.text(
    `${(state.financials?.maintenanceBufferEuro || 0).toLocaleString('de-DE', { style: 'currency', currency: 'EUR' })}`,
    MARGIN + 80,
    105
  );
  ...
  doc.setFontSize(10);
  doc.text(
    `${(state.identity.turbineType || 'ASSET').toUpperCase()} HEALTH SCORE`,
  ...
  const riskData = [
    [
      ...
      state.structural.drf ? `${state.structural.drf.toFixed(1)}%` : 'NOMINAL',
      `${state.structural.longevityLeak || '0'} Years Lost`,
    ],
    ...
    [
      'Grid Service Stress',
      state.specializedState?.sensors?.gridFrequency &&
      Math.abs(50 - state.specializedState.sensors.gridFrequency) > 0.1
        ? 'ACTIVE'
        : 'NOMINAL',
      'Accelerated Aging 1.5x',
    ],
  ];
  ...
  doc.setFontSize(9);
  doc.setTextColor(100, 100, 100);
  doc.text('Strategische Empfehlung:', MARGIN, getLastAutoTableFinalY(doc) + 15);
  ...
  doc.text(
    doc.splitTextToSize(recommendation, PAGE_WIDTH - MARGIN * 2),
    MARGIN,
    getLastAutoTableFinalY(doc) + 22
  );
...
const drawFinancialImpact = (doc: jsPDF, state: TechnicalProjectState, hasCavitation: boolean) => {
  ...
  doc.setTextColor(220, 38, 38);
  doc.setFont('helvetica', 'bold');
  const formattedLoss = new Intl.NumberFormat('de-DE', {
    style: 'currency',
    currency: 'EUR',
  }).format(lossEUR);
  ...
  const descLines = doc.splitTextToSize(
    lossDesc + (hasCavitation ? `, was Mehrkosten von ${formattedLoss} verursacht.` : ''),
    90
  );
...
const drawTechnicalDetails = (doc: jsPDF, state: TechnicalProjectState) => {
  ...
  const data = [
    ...
    [
      'Mechanik',
      'Vibration',
      `${state.mechanical.vibration} mm/s`,
      PhysicsEngine.getVibrationVerdict(state.mechanical.vibration),
    ],
    [
      'Mechanik',
      'Lager-Temp',
      `${state.mechanical.bearingTemp} °C`,
      PhysicsEngine.getBearingTempVerdict(state.mechanical.bearingTemp),
    ],
    [
      'Mechanik',
      'Isolation',
      `${state.mechanical.insulationResistance || 'N/A'} MOhm`,
      PhysicsEngine.getInsulationVerdict(state.mechanical.insulationResistance || 500),
    ],
    [
      'Hydraulik',
      'Axialspiel',
      `${state.mechanical.axialPlay || 'N/A'} mm`,
      PhysicsEngine.getAxialPlayVerdict(state.mechanical.axialPlay || 0),
    ],
  ];
...
const drawExpertInsights = (doc: jsPDF, state: TechnicalProjectState) => {
  ...
  const insight =
    'Kavitation an den Laufschaufeln deutet auf Betrieb außerhalb des Bestpunktes hin. Empfehlung: Stellite-Aufschweißung prüfen.';
...
const drawFooter = (doc: jsPDF, pageNum: number) => {
  ...
  doc.setFontSize(8);
  doc.text(
    `Platform Report | Vertraulich | ${new Date().toLocaleDateString()}`,
    MARGIN,
    PAGE_HEIGHT - 8
  );
};
...
const drawFullAssetHealthCertificate = (doc: jsPDF, state: TechnicalProjectState, profile: any) => {
  ...
  doc.text(
    `360° TECHNICAL AUDIT PORTAL // NC-4.2 STANDARD // ${state.identity.assetName || 'UNIT_X'}`,
    MARGIN,
    38
  );
  ...
  profile.ui_manifest.passport_sections.forEach((section: any, idx: number) => {
    ...
    const tableBody = section.fields.map((field: any) => {
      ...
      return [i18n.t(field.label), `${val} ${field.unit || ''}`, status];
    });
    ...
    autoTable(doc, {
      ...
      didDrawCell: data => {
        if (data.section === 'body' && data.column.index === 2) {
          ...
          if (status.includes('CRITICAL') || status.includes('UNACCEPTABLE'))
            doc.setTextColor(220, 38, 38);
          else if (
            status.includes('WARNING') ||
            status.includes('SATISFACTORY') ||
    ...
    y = getLastAutoTableFinalY(doc) + 15;
  ...
  const urgencyText =
    urgency === 5
      ? 'IMMEDIATE SHUTDOWN AND INSPECTION STRONGLY ADVISED'
      : urgency === 4
        ? 'SCHEDULE REVISION WITHIN 30 DAYS'
        : urgency === 3
  ...
  const calculations = passport.calculations || {
    insulationAlert: undefined,
    bearingLifeImpact: undefined,
  };
  doc.setTextColor(15, 23, 42);
  doc.setFontSize(10);
  doc.setFont('helvetica', 'bold');
  ...
  [...inferences.alerts, ...inferences.conclusions].forEach(inf => {
    ...
    doc.setTextColor(220, 38, 38);
    doc.text(
      `[${(inf as any).standard || (inf as any).kbReference || 'KB-REF'}] ${(inf as any).parameter || (inf as any).symptom}`,
      MARGIN + 5,
      y
    );
    y += 4;
  ...
  doc.text(
    `ANO-SOT-NC4.2-VERIFIED // ${profile.type} SPECIALIZATION // DIGITAL TWIN SYNCHRONIZED`,
    MARGIN,
    285
  );
  ...
  doc.setTextColor(100, 100, 100);
  doc.text(
    'Strategic actions derived from AI Forensic Audit and structural health triggers.',
    MARGIN,
    y
  );
  y += 15;
  ...
  if (maintenanceData.length > 0) {
    ...
    doc.text(
      'No critical maintenance triggers detected in the current operational window.',
      MARGIN,
      y
    );
  }
...
const drawExecutiveActionSummary = (doc: jsPDF, yStart: number, state: TechnicalProjectState) => {
  const inferences = ExpertInference.analyze(state);
  const actionData = [...inferences.alerts, ...inferences.conclusions].map(inf => [
    (inf as any).parameter || (inf as any).symptom,
  ...
  return getLastAutoTableFinalY(doc) + 10;
};
...
const drawEngineeringRecoveryPlan = (doc: jsPDF, state: TechnicalProjectState, id: string) => {
  ...
  inferences.recoveryPaths.forEach(path => {
    path.actions.forEach(action => {
      recoveryData.push([
        ...
        {
          content: action.mitigationImpact,
  ...
  if (recoveryData.length > 0) {
    ...
    const finalY = getLastAutoTableFinalY(doc, 150);
    ...
    doc.setFontSize(10);
    doc.setTextColor(33, 33, 33);
    const totalExtension = inferences.recoveryPaths.reduce(
      (acc, p) => acc + p.estimatedLifeExtension,
      0
    );
    doc.text(
      `Total Predicted Life Extension (${state.identity.turbineType}): +${totalExtension.toFixed(1)} Years`,
      MARGIN + 5,
      finalY + 30
    );
    doc.setFontSize(8);
    doc.setTextColor(100, 100, 100);
    doc.text(
      '* Based on Cubic Stress-Life Relationship (Lext = Lrem * (sigma_limit / sigma_actual)^3)',
      MARGIN + 5,
      finalY + 35
    );
  } else {
...
const drawLongevityCertificate = (doc: jsPDF, state: TechnicalProjectState, id: string) => {
  ...
  doc.setFont('helvetica', 'normal');
  doc.setTextColor(100, 100, 100);
  doc.text('Forensic Longevity Audit // NC-4.2 Precision Protocol', PAGE_WIDTH / 2, 68, {
    align: 'center',
  });
  ...
  doc.setTextColor(71, 85, 105);
  const mathText =
    'Longevity Leak is calculated based on the Dynamic Risk Factor (DRF). The DRF utilizes a cubic relationship between vibration intensity and bearing fatigue life (Life proportional to 1/Vibration cubed). When dynamic loads exceed 48% of the design baseline (The 48% Rule), degradation accelerates exponentially, leaking years from the 50-year investment horizon.';
  ...
  if (alignment <= 0.05) {
    ...
    doc.setFont('helvetica', 'bold');
    doc.text(
      'PRECISION ALERT: Alignment exceeds 0.05 mm/m Golden Standard.',
      MARGIN + 10,
      warningY + 12
    );
  }
...
const drawTribologyAndThermalExpansion = (doc: jsPDF, state: TechnicalProjectState, id: string) => {
  ...
  doc.setTextColor(100, 100, 100);
  doc.text(
    'Forensic Fluid Intelligence // Chemical Integrity & Expansion Compensation',
    MARGIN,
    68
  );
  ...
  const oilData = [
    ...
    [
      ...
      `${tan} mgKOH/g`,
      '< 0.5 mgKOH/g',
      tan > 0.5 ? 'CRITICAL' : 'OPTIMAL',
    ],
    [
      'Viscosity @ 40°C',
      `${viscosity} cSt`,
      '41.4 - 50.6',
      viscosity < 41.4 || viscosity > 50.6 ? 'WARNING' : 'OPTIMAL',
    ],
  ];
  ...
  const tableY = getLastAutoTableFinalY(doc) + 15;
  doc.setFontSize(14);
  doc.setTextColor(15, 23, 42);
  doc.setFont('helvetica', 'bold');
  ...
  if (isHeritageCertified) {
    ...
    doc.setFont('helvetica', 'normal');
    doc.text(
      'PRECISION ALIGNED // CHEMICALLY NEUTRAL // 50-YEAR TARGET REACHABLE',
      PAGE_WIDTH / 2,
      stampY + 35,
      { align: 'center' }
    );
    ...
    doc.setFont('helvetica', 'bold');
    doc.text(
      'CERTIFICATION PENDING: Asset does not currently meet Tier 1 Heritage standards.',
      MARGIN + 10,
      warningY + 10
    );
    doc.setFontSize(8);
    doc.setFont('helvetica', 'normal');
    doc.text(
      'Action Required: Align to 0.05 mm/m and purify oil (Water < 500 ppm, TAN < 0.5).',
      MARGIN + 10,
      warningY + 18
    );
  }
...
const drawRevitalizationRoadmap = (doc: jsPDF, state: TechnicalProjectState, id: string) => {
  ...
  roadmap.forEach(plan => {
    roadmapData.push([
      {
        content: plan.priority,
        styles: { textColor: plan.priority === 'HIGH' ? [220, 38, 38] : [245, 158, 11] },
      },
      plan.category,
      plan.action,
      plan.impact,
      {
        content: `${plan.roiRatio.toFixed(1)}x`,
        styles: { fontStyle: 'bold', textColor: [16, 185, 129] },
      },
    ]);
  });
  ...
  const finalY = getLastAutoTableFinalY(doc) + 15;
  ...
  doc.setFont('helvetica', 'italic');
  doc.text(
    '"Precision is not a luxury; it is the root of asset longevity."',
    PAGE_WIDTH / 2,
    finalY,
    { align: 'center' }
  );
  ...
  doc.setTextColor(100, 100, 100);
  doc.text(
    'All projections focus on a 50-year longevity horizon for Francis < 5 MW units.',
    PAGE_WIDTH / 2,
    finalY + 10,
    { align: 'center' }
  );
...
const drawMathProofAppendix = (doc: jsPDF, state?: any) => {
  ...
  doc.setFont('helvetica', 'normal');
  doc.text(
    'where DRF = Deviation Ratio Factor = (Actual Alignment) / (0.05 mm/m)',
    PAGE_WIDTH / 2,
    y + 28,
    { align: 'center' }
  );
  ...
  doc.setFont('helvetica', 'normal');
  doc.text(
    'Temperature rise factor applied to bearing thermal calculations',
    PAGE_WIDTH / 2,
    y + 22,
    { align: 'center' }
  );
  ...
  if (state) {
    ...
    doc.setFillColor(
      alignment <= 0.05 ? 236 : 254,
      alignment <= 0.05 ? 253 : 242,
      alignment <= 0.05 ? 245 : 242
    );
    ...
    if (alignment <= 0.05) {
      ...
      doc.setFont('helvetica', 'bold');
      doc.text(
        `⚠ LONGEVITY LEAK: -${yearsLost.toFixed(1)} Years Expected Life`,
        MARGIN + 10,
        y + 42
      );
    }
  ...
  doc.setTextColor(148, 163, 184);
  doc.text(
    'AnoHUB NC-4.2 // Pro-Bono Tool for 50-Year Asset Longevity // Mathematical Proof Document',
    PAGE_WIDTH / 2,
    PAGE_HEIGHT - 10,
    { align: 'center' }
  );
...
const drawMechanicalInstallationSchedule = (
  doc: jsPDF,
  state: TechnicalProjectState,
  id: string
) => {
  ...
  const assemblyData = ((state as any).specs?.assemblySequence || []).map(
    (record: any, index: number) => {
      const alignment =
        record.alignment !== undefined ? `${record.alignment.toFixed(3)} mm/m` : 'N/A';
  ...
  if (assemblyData.length > 0) {
    ...
    // Certification Seal
    const finalY = getLastAutoTableFinalY(doc) + 20;
    doc.setDrawColor(100, 100, 100);
    doc.setLineWidth(1);
    ...
    doc.text(
      'By signing, I certify that the installed components meet the NC-4.2 specifications.',
      MARGIN + 10,
      finalY + 38
    );
  } else {
