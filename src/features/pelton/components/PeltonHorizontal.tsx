...
export const PeltonHorizontal: React.FC = () => {
  // Injector state: 1, 2, 3
  const [injectorCount, setInjectorCount] = useState<number>(2);

  // Determine background based on injector count
  const backgroundImage = injectorCount === 3 
    ? '/assets/pic.s_Background/peltonHorizontal_x3nozz.png'
  ...
  return (
    <div className="relative min-h-[calc(100vh-64px)] overflow-hidden flex flex-col p-6 md:p-12 w-full">
      ...
      <div 
        ...
        style={{ backgroundImage: `url('${backgroundImage}')`, filter: 'grayscale(30%)' }} 
      />
      ...
      >
        <h2 className="text-3xl md:text-4xl font-black mb-10 text-white tracking-widest uppercase text-center shadow-black drop-shadow-md">
          Pelton — <span className="text-cyan-400">Horizontal</span> 
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center p-4 bg-slate-900/60 hover:bg-slate-800/80 transition-colors rounded-xl border border-white/5">
                ...
                <span className="font-mono text-white font-bold text-lg">450.00 m</span>
              </div>
              <div className="flex justify-between items-center p-4 bg-slate-900/60 hover:bg-slate-800/80 transition-colors rounded-xl border border-white/5">
                <span className="text-slate-300 text-xs font-semibold uppercase tracking-wider">Rated Discharge (Qₙ)</span>
                <span className="font-mono text-white font-bold text-lg">{(1.5 * injectorCount).toFixed(2)} m³/s</span>
              </div>
              <div className="flex justify-between items-center p-4 bg-slate-900/60 hover:bg-slate-800/80 transition-colors rounded-xl border border-white/5">
                <span className="text-slate-300 text-xs font-semibold uppercase tracking-wider">Turbine Output (Pₜ)</span>
                <span className="font-mono text-cyan-400 font-bold text-2xl">{(6000 * (injectorCount/2)).toFixed(0)} kW</span>
              </div>
            </div>
          </div>

          <div className="space-y-6">
            <h3 className="font-bold text-amber-400 uppercase tracking-[0.2em] text-xs border-b border-white/10 pb-3">Component Info</h3>
            <div className="space-y-4">
              <div className="flex justify-between items-center p-4 bg-slate-900/60 hover:bg-slate-800/80 transition-colors rounded-xl border border-white/5">
                <span className="text-slate-300 text-xs font-semibold uppercase tracking-wider">Shaft Orientation</span>
                <span className="font-mono text-cyan-400 font-bold text-lg">Horizontal</span>
              </div>
              <div className="flex justify-between items-center p-4 bg-slate-900/60 hover:bg-slate-800/80 transition-colors rounded-xl border border-white/5">
                <span className="text-slate-300 text-xs font-semibold uppercase tracking-wider">Runner Overhang</span>
                <span className="font-mono text-slate-300 text-lg">Single/Double</span>
              </div>
            </div>
          </div>
        </div>
...
export default PeltonHorizontal;
