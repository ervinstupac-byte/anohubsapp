...
export const usePhysicsMetrics = (): PhysicsMetrics => {
  const { physics, diagnosis, mechanical, hydraulic, identity, site, penstock, lastUpdate } =
    useTelemetryStore();
  ...
  return useMemo(() => {
    ...
    const maintenanceUrgency = PhysicsEngine.calculateMaintenanceUrgency(simulatedState);
    ...
    const safetyFactor = diagnosis?.safetyFactor
      ? diagnosis.safetyFactor.toNumber()
      : alertLevel === 'CRITICAL'
        ? 0.8
        : 1.5;
    ...
    const isCavitating =
      hydraulic.flow > (SYSTEM_CONSTANTS.THRESHOLDS.FLOW.CAVITATION_CRITICAL || 42.5);

    const isMisaligned = mechanical.alignment > 0.05; // 0.05 mm/m Heritage Law (not in constants)

    const isOverheating =
      mechanical.bearingTemp > (SYSTEM_CONSTANTS.THRESHOLDS.BEARING_TEMP.WARNING_MAX || 80);

    const hasExcessiveVibration =
      mechanical.vibration >
      (SYSTEM_CONSTANTS.THRESHOLDS.VIBRATION_ISO_10816.UNSATISFACTORY_MAX || 4.5);
