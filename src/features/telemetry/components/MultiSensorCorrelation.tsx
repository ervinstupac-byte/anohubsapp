...
export const MultiSensorCorrelation: React.FC = () => {
  ...
  return (
    <GlassCard
      className={`border-l-4 ${detected ? 'border-l-red-500 bg-red-950/20' : 'border-l-cyan-500 bg-slate-900/40'}`}
    >
      ...
      <div className="flex items-center gap-6">
        ...
        <svg width="160" height="160" viewBox="0 0 160 160" className="flex-shrink-0">
          ...
          <line
            x1={centerX}
            y1={centerY}
            x2={centerX + radius * Math.cos((30 * Math.PI) / 180)}
            y2={centerY + radius * Math.sin((30 * Math.PI) / 180)}
            stroke="#475569"
            strokeWidth="1"
          />
          <line
            x1={centerX}
            y1={centerY}
            x2={centerX + radius * Math.cos((150 * Math.PI) / 180)}
            y2={centerY + radius * Math.sin((150 * Math.PI) / 180)}
            stroke="#475569"
            strokeWidth="1"
          />
          ...
          {/* Center indicator */}
          <circle cx={centerX} cy={centerY} r="6" fill={detected ? '#ef4444' : '#0891b2'} />
        ...
        <div className="flex-1 space-y-2">
          <div className="flex items-center gap-2">
            <div
              className={`w-2 h-2 rounded-none ${triggers.acoustic ? 'bg-amber-500' : 'bg-slate-600'}`}
            ></div>
            <span
              className={`text-xs font-bold ${triggers.acoustic ? 'text-amber-400' : 'text-slate-500'}`}
            >
          ...
          <div className="flex items-center gap-2">
            <div
              className={`w-2 h-2 rounded-none ${triggers.thermal ? 'bg-red-500' : 'bg-slate-600'}`}
            ></div>
            <span
              className={`text-xs font-bold ${triggers.thermal ? 'text-red-400' : 'text-slate-500'}`}
            >
          ...
          <div className="flex items-center gap-2">
            <div
              className={`w-2 h-2 rounded-none ${triggers.hydraulic ? 'bg-blue-500' : 'bg-slate-600'}`}
            ></div>
            <span
              className={`text-xs font-bold ${triggers.hydraulic ? 'text-blue-400' : 'text-slate-500'}`}
            >
          ...
          <div
            className={`mt-4 p-3 rounded-none border ${detected ? 'bg-red-950/30 border-red-500/30' : 'bg-slate-900/30 border-slate-700/30'}`}
          >
