...
export const ShaftOrbitPlot = React.memo(
  forwardRef<ShaftOrbitPlotHandle, ShaftOrbitPlotProps>(
    (
      {
        vibrationX,
        vibrationY,
      ...
      const analysis = useMemo(() => {
        ...
        if (history.length < 2)
          return { eccentricity: 0, isElliptical: false, isStructuralLoosenessConfirmed: false };
        ...
        const isStructuralLoosenessConfirmed =
          eccentricity.gt(0.8) && new Decimal(acousticNoiseFloor).gt(85);
      ...
      useEffect(() => {
        ...
        ctx.strokeStyle = isStructuralLoosenessConfirmed
          ? '#f43f5e'
          : isElliptical
            ? '#ef4444'
            : '#22d3ee';
        ...
        ctx.lineTo(
          peakX - 10 * Math.cos(arrowAngle - Math.PI / 6),
          peakY - 10 * Math.sin(arrowAngle - Math.PI / 6)
        );
        ctx.moveTo(peakX, peakY);
        ctx.lineTo(
          peakX - 10 * Math.cos(arrowAngle + Math.PI / 6),
