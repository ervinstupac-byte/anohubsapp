...
export const VibrationAnalyzer: React.FC = () => {
  ...
  const powerKW = useTelemetryStore(
    state => state.hydraulic?.powerKW ?? state.mechanical?.powerKW ?? 0
  );
  ...
  // We look for 'Shaft Plumbness' or similar tags in the latest logs
  const maintenanceFactors = useMemo(() => {
    const plumbnessLog = logs.find(
      l =>
        l.commentBS.toLowerCase().includes('runout') ||
        l.commentBS.toLowerCase().includes('plumbness')
    );
  ...
  const spectrum = useMemo(() => {
    ...
    for (let f = 0; f < 200; f += 2) {
      ...
      // NC-130: Peak at 2x RPM (Misalignment)
  ...
  // We Map our synthetic spectrum back to the Expert's expected input
  const verdict = useMemo(() => {
    const expertInput = [
      { frequencyHz: f0, amplitudeMmS: spectrum.find(p => Math.abs(p.f - f0) < 2)?.amp || 0 },
      {
        frequencyHz: f0 * 2,
        amplitudeMmS: spectrum.find(p => Math.abs(p.f - f0 * 2) < 2)?.amp || 0,
      }, // 2x Peak
      {
        frequencyHz: fBlade,
        amplitudeMmS: spectrum.find(p => Math.abs(p.f - fBlade) < 2)?.amp || 0,
      },
    ];
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 p-4">
            ...
            <circle
              ...
              onMouseEnter={e => {
                setTooltip({
                  x: e.clientX,
                  y: e.clientY,
                  title: '1x Mean Runout',
                  text: 'Fundamental frequency. High amplitude indicates Mass Unbalance.',
                });
            ...
            <circle
              ...
              onMouseEnter={e => {
                setTooltip({
                  x: e.clientX,
                  y: e.clientY,
                  title: '2x RPM Misalignment',
                  text: 'Classic signature of Shaft Misalignment. Correlate with Coupling Bolt stress.',
                });
            ...
            <circle
              ...
              onMouseEnter={e => {
                setTooltip({
                  x: e.clientX,
                  y: e.clientY,
                  title: 'Blade Pass Frequency',
                  text: 'Hydraulic turbulence. Check Guide Vane opening and Runner End-Gaps.',
                });
          ...
          <AnimatePresence>
            {tooltip && (
              ...
              >
                ...
                <p className="text-[10px] text-slate-300 leading-tight">{tooltip.text}</p>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
      ...
      <div className="lg:col-span-4 flex flex-col gap-4">
        <div
          className={`rounded-none border p-6 flex flex-col h-full justify-between transition-colors duration-500 ${
            verdict.danger
          ...
          <div>
            ...
            <h2
              className={`text-xl font-black uppercase tracking-tighter mb-2 ${
                verdict.danger ? 'text-amber-400' : 'text-cyan-400'
              }`}
            >
              {verdict.cause === 'Normal Spectral Signature'
                ? 'Vibrational Stability: Nominal'
                : verdict.cause}
            </h2>
            <p className="text-sm text-slate-300 leading-relaxed">{verdict.recommendation}</p>
          </div>
          ...
          <div className="mt-8 space-y-4">
            ...
            <div className="flex gap-2">
              ...
              <div className="flex-1 flex flex-col bg-slate-700/30 p-3 rounded-none border border-slate-700">
                ...
                <span
                  className={`text-sm font-mono font-bold ${maintenanceFactors.shaftPlumbnessDeviation > 0.05 ? 'text-amber-500' : 'text-emerald-500'}`}
                >
