...
export const LiveMetricToken = React.memo<LiveMetricTokenProps>(({ sensorId }) => {
  ...
  const metric = React.useMemo(() => {
    ...
    if (sensorId === 'TMP-404-X') {
      return {
        ...
        unit: '°C',
        status:
          mechanical.bearingTemp > 80
            ? 'critical'
            : mechanical.bearingTemp > 65
              ? 'warning'
              : 'nominal',
        history: [] as number[],
      };
  ...
  if (!metric)
    return (
      <span className="inline-block px-1.5 py-0.5 bg-red-950/20 text-red-500 font-mono text-[10px] font-bold rounded border border-red-500/30 uppercase tracking-widest">
        ERR: {sensorId}
      </span>
    );
  ...
  return (
    <span
      className={`inline-flex items-center gap-2 align-middle px-2 py-0.5 mx-1 rounded-sm border backdrop-blur-sm transition-all duration-300 group overflow-hidden relative ${
        isCritical
          ? 'bg-red-950/40 border-red-500/50 shadow-[0_0_10px_rgba(239,68,68,0.2)]'
          : isWarning
    ...
    >
      ...
      <span
        className={`font-mono font-bold text-[11px] tabular-nums relative z-10 ${
      ...
      {metric.history && metric.history.length > 0 && (
        <div className="w-10 h-3 opacity-80 group-hover:opacity-100 transition-opacity flex items-center">
          <Sparkline
            ...
            width={40}
            height={12}
            color={isCritical ? '#f87171' : isWarning ? '#fbbf24' : '#22d3ee'}
          />
        </div>
      )}
    </span>
