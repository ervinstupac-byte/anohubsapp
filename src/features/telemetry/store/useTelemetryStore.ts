...
} from '../../../services/MasterIntelligenceEngine';
import {
  EnhancedAsset,
...
} from '../../../services/core/VibrationForensics';
import {
  Sovereign_Executive_Engine,
  ExecutiveState,
  PermissionTier,
} from '../../../services/Sovereign_Executive_Engine';
...
import {
  ...
  saveTelemetryBatch,
  getRecentHistory,
} from '../../../services/PersistenceService'; // NC-24000 Refined
...
interface TelemetryState {
  ...
  setConfig: (
    config: Partial<Pick<TechnicalProjectState, 'identity' | 'site' | 'penstock'>>
  ) => void;
  ...
  persist(
    (set, get) => ({
      ...
      addManualRule: (rule: string) =>
        set(state => ({ manualRules: [...state.manualRules, rule] })),

      images: [],
      addInspectionImage: (assetId: number, component: string, imageData: InspectionImage) =>
        set(state => ({
      ...
      applyMitigation: (mitigation: string) =>
        set(state => ({
          ...
          physics:
            mitigation === 'STRUCTURAL_RISK'
              ? { ...state.physics, hoopStressMPa: (state.physics.hoopStressMPa || 0) * 0.85 }
              : state.physics,
        })),
      ...
      fleet: [
        ...
        },
        {
          ...
          location: { x: 30, y: 70 },
          type: 'Pelton',
          commissioned: '2024-04-22',
          baselineEff: 90.5,
          efficiency: 90.4,
          healthScore: 99,
          load: 45,
          activeAlerts: 0,
          description: 'Pelton - High Head',
        },
        {
          id: 'UNIT-05',
          name: 'Bulb Epsilon',
          location: { x: 60, y: 80 },
          type: 'Bulb',
          commissioned: '2024-05-05',
          baselineEff: 93.0,
          efficiency: 88.5,
          healthScore: 91,
          load: 78,
          activeAlerts: 1,
          description: 'Bulb - Low Head',
        },
        {
          id: 'UNIT-06',
          name: 'Francis Zeta',
          location: { x: 10, y: 50 },
          type: 'Francis',
          commissioned: '2024-06-12',
          baselineEff: 94.8,
          efficiency: 94.8,
          healthScore: 100,
          load: 88,
          activeAlerts: 0,
          description: 'Francis - Peak Load',
        },
      ] as PlantStatus[],
      hiveEvents: [],
      ...
      addHiveEvent: event =>
        set(state => ({ hiveEvents: [event, ...state.hiveEvents].slice(0, 50) })),

      addSnapshot: (snapshot: DiagnosticSnapshot) =>
        set(state => {
          // Debounce: Don't add same pathology within 60s
          const lastTrigger = state.snapshots.find(
            s => s.pathology === snapshot.pathology && s.timestamp > snapshot.timestamp - 60000
          );
      ...
      setPlaybackSnapshot: (snapshot: DiagnosticSnapshot | null) =>
        set(state => {
          if (!snapshot) {
      ...
      updateTelemetry: (payload: TelemetryUpdatePayload) => {
        ...
        const simulationState: TechnicalProjectState = {
          ...
          ...state,
          physics: state.physics as TechnicalProjectState['physics'],
        ...
        if (payload.mechanical || payload.hydraulic) {
          saveTelemetryBatch(payload as unknown as Record<string, unknown>).catch(() => {});
        }
        ...
        set(state => {
          ...
          const nextMechanical = {
            ...
            bearingTemp:
              payload.mechanical?.bearingTemp ??
              dynamicPhysics.mechanical?.bearingTemp ??
              state.mechanical.bearingTemp,
          };

          // Merge Governor Dynamics
          const nextGovernor = payload.governor
            ? { ...state.governor, ...payload.governor }
            : dynamicPhysics.governor
              ? { ...state.governor, ...dynamicPhysics.governor }
              : state.governor;
          ...
          return {
            ...
            fluidIntelligence: payload.fluidIntelligence
              ? { ...state.fluidIntelligence, ...payload.fluidIntelligence }
              : state.fluidIntelligence,
        ...
        const cavitationRisk = (() => {
          ...
          const threshold = currentState.hydraulic.cavitationThreshold
            ? currentState.hydraulic.cavitationThreshold.toNumber()
            : 0.5;
        ...
        const operatingZone = (() => {
          ...
          if (eff < 0.7) return 'ROUGH'; // Rough zone typically correlates with low efficiency
          return 'OPTIMAL';
        })();
        ...
        const executiveInputs = {
          ...
          ph: 7.0,
          turbineType: currentState.identity.turbineType || 'FRANCIS',
          // NC-300: Physics Integration
          physicsAnalysis: {
            cavitation: { risk: cavitationRisk },
            zone: { zone: operatingZone },
            surgePressureBar:
              typeof currentState.physics.surgePressureBar === 'number'
        ...
        try {
          const execResult = sovereignEngine.executeCycle(
            executiveInputs as unknown as Parameters<Sovereign_Executive_Engine['executeCycle']>[0],
            {
      ...
      runDeepAnalysis: async () => {
        ...
        const simulatedAsset: any = {
          ...
          telemetry: {
            latest: {
              ...
              } as Partial<TechnicalProjectState['specializedState']>,
            },
            history: [],
          },
        };
        ...
        try {
          ...
          const diagnosis = await MasterIntelligenceEngine.analyzeAsset(
            simulatedAsset as EnhancedAsset,
            history as CompleteSensorData[]
          );
        ...
        switch (scenario) {
          ...
          case 'CHRONIC_MISALIGNMENT':
            updateTelemetry({
              ...
              specializedState: {
                modules: get().specializedState?.modules || {},
                healthScore: get().specializedState?.healthScore || 0,
                activeRisks: get().specializedState?.activeRisks || [],
                sensors: get().specializedState?.sensors,
                acoustic: {
                  fingerprintMatch: 85,
                  harmonics: {
                    1: 2.0,
      ...
      hydrate: async () => {
        ...
        history.forEach(snap => {
          const d = snap.dataBlob as unknown as {
            mechanical?: Partial<MechanicalStream>;
            hydraulic?: Partial<HydraulicStream>;
            [k: string]: any;
          } | null; // Type assertion for now
          if (!d) return;
          if (d.mechanical?.vibrationX) {
            recoveredBuffer.vibrationX.push({
              value: d.mechanical.vibrationX,
              timestamp: snap.timestamp,
            });
            signalHistoryManager.record('vibrationX', d.mechanical.vibrationX, snap.timestamp);
          }
          if (d.mechanical?.vibrationY) {
            recoveredBuffer.vibrationY.push({
              value: d.mechanical.vibrationY,
              timestamp: snap.timestamp,
            });
            signalHistoryManager.record('vibrationY', d.mechanical.vibrationY, snap.timestamp);
          }
          if (d.mechanical?.bearingTemp) {
            recoveredBuffer.bearingTemp.push({
              value: d.mechanical.bearingTemp,
              timestamp: snap.timestamp,
            });
