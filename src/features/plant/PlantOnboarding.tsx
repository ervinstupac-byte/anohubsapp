    ...
    try {
      ...
      try {
        const { data: assetData, error: assetErr } = await supabase
          .from('assets')
          .insert([{ name: `${plantName} Asset`, plant_id: plantId }])
          .select('id')
          .single();
      ...
      const { data: hydroData, error: hydroErr } = await supabase
        .from('hydrology_context')
        .insert([hydrologyPayload])
        .select('id')
        .single();
      ...
      setMessage(
        'Plant onboarded successfully. Please link assets to this plant and update pricing for specific assets.'
      );
    } catch (err: any) {
