...
export const calculateMaintenancePrediction = (input: PredictionInput): MaintenancePrediction => {
  ...
  const vUnsatisfactory = new Decimal(
    SYSTEM_CONSTANTS.THRESHOLDS.VIBRATION_ISO_10816.UNSATISFACTORY_MAX
  );
  ...
  if (telemetry.currentVibrationMMs > 1.1) {
    // 1.1 is "Good" max
  ...
  if (safeRemaining < 720) {
    // < 1 month (720h)
    urgency = 'CRITICAL';
  } else if (safeRemaining < 2160) {
    // < 3 months
    urgency = 'PRIORITY';
  ...
  return {
    ...
    urgency,
    primaryStressor:
      primaryStressor === 'AGE' && degradation.gt(1.05) ? 'VIBRATION' : primaryStressor,
  };
};
