...
export const calculateMaintenancePrediction = (input: PredictionInput): MaintenancePrediction => {
  ...
  const lifePercent = designLife.gt(0) ? remainingNative.div(designLife).mul(100).toNumber() : 0;
  ...
  if (safeRemaining < 24 * 7)
    urgency = 'CRITICAL'; // < 1 week
  else if (safeRemaining < 24 * 30)
    urgency = 'PRIORITY'; // < 1 month
