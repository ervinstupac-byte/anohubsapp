...
export const validateMechanicalState = (
  ...
  if (Math.abs(alignmentMmM) > MECHANICAL_TOLERANCES.ALIGNMENT_MM_M) {
    ...
    details.push(
      `ALIGNMENT CRITICAL: ${alignmentMmM} mm/m exceeds limit (${MECHANICAL_TOLERANCES.ALIGNMENT_MM_M} mm/m)`
    );
  } else {
  ...
  if (
    wearingGapMm < MECHANICAL_TOLERANCES.WEARING_GAP_MIN_MM ||
    wearingGapMm > MECHANICAL_TOLERANCES.WEARING_GAP_MAX_MM
  ) {
    isValid = false;
    alarms.push(SOVEREIGN_MECHANICAL_ALARM);
    details.push(
      `WEARING GAP DEVIATION: ${wearingGapMm} mm (Target: ${MECHANICAL_TOLERANCES.WEARING_GAP_MIN_MM}-${MECHANICAL_TOLERANCES.WEARING_GAP_MAX_MM} mm)`
    );
  } else {
  ...
  if (surfaceFinishRa !== undefined) {
    if (surfaceFinishRa > MECHANICAL_TOLERANCES.SURFACE_FINISH_RA_UM) {
      ...
      alarms.push(SOVEREIGN_MECHANICAL_ALARM);
      details.push(
        `SURFACE DEGRADATION: Ra ${surfaceFinishRa} μm > ${MECHANICAL_TOLERANCES.SURFACE_FINISH_RA_UM} μm`
      );
    } else {
