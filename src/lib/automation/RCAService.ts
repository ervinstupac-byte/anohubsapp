...
export class RCAService {
  ...
  public analyze(input: RCAInput): RCAResult[] {
    ...
    if (input.metrics.bearingTempRateOfChange > 0.5) {
      ...
      evidenceMis.push(
        `Bearing Temp rising fast (+${input.metrics.bearingTempRateOfChange.toFixed(1)}°C/min)`
      );
    }
    ...
    if (input.maintenance.shaftPlumbnessDeviation > 0.05) {
      ...
      evidenceMis.push(
        `Static Plumbness Deviation: ${input.maintenance.shaftPlumbnessDeviation}mm`
      );
    }
    ...
    if (input.maintenance.bearingClearances) {
      ...
      if (asymmetry > 0.05) {
        ...
        evidenceMis.push(
          `Bearing Clearance Asymmetry: ${asymmetry.toFixed(3)}mm (from baseline DNA)`
        );
      }
    }
    ...
    if (misalignmentScore > 0.4) {
      results.push({
        ...
        werkmeisterRecommendation:
          'Check coupling bolts and verifying hot alignment immediately. Do not wait for trip.',
      });
    }
    ...
    if (harmonicCount > 4) {
      looseScore += 0.35;
      evidenceLoose.push(
        'Multiple harmonics present (Comb Spectrum) indicating mechanical looseness'
      );
    }
    ...
    if (looseScore > 0.4) {
      results.push({
        ...
        werkmeisterRecommendation:
          'Torque check all foundation bolts. Inspect grouting for cracks.',
      });
    }
    ...
    if (input.metrics.efficiencyPercent < 88.0 && input.metrics.efficiencyPercent > 0) {
      cavScore += 0.4;
      evidenceCav.push(
        `Efficiency dropped to ${input.metrics.efficiencyPercent.toFixed(1)}% (Target: >92%)`
      );
    }

    // Check for broadband noise (High Freq)
    // Material Sensitivity Logic (NC-140 Expert Update)
    const isSoftMaterial =
      input.specifications.runnerMaterial === 'Bronze' ||
      input.specifications.runnerMaterial === 'Cast Steel';
    const noiseThreshold = isSoftMaterial ? 0.3 : 0.5; // Lower threshold for softer materials

    const highFreqNoise = input.peaks.find(
      p => p.frequencyHz > 150 && p.amplitudeMmS > noiseThreshold
    );

    if (highFreqNoise) {
      cavScore += 0.45;
      evidenceCav.push(
        `High Frequency hydraulic noise detected (> 150Hz, > ${noiseThreshold}mm/s)`
      );

      if (isSoftMaterial) {
        cavScore += 0.15; // Expert penalty for material
        evidenceCav.push(
          `Material Risk: ${input.specifications.runnerMaterial} requires stricter cavitation limits.`
        );
      }
    }
    ...
    if (cavScore > 0.4) {
      ...
      if (isSoftMaterial) {
        recommendation =
          'Inspect runner blades for pitting immediately. If material is Carbon Steel/Bronze, consider scheduling application of cavitation-resistant ceramic coating during next outage.';
      }
