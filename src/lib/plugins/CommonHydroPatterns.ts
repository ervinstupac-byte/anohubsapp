export const COMMON_HYDRO_PATTERNS = [
  {
    ...
    physicsNarrative: (probability: number) =>
      `CRITICAL: Massive pressure surge detected (${(probability * 150).toFixed(0)} Bar). Kinetic energy is reflecting back through the Penstock, risking structural breach of MIV seals.`,
    conditions: [
    ...
    actions: [
      ...
      {
        type: 'OPEN_SOP',
        label: 'Water Hammer Protocol',
        targetId: 'Francis_SOP_Water_Hammer',
        icon: 'FileText',
      },
    ],
  },
  {
    ...
    physicsNarrative: (probability: number) =>
      `Grid Separation detected. RPM spiking due to load rejection (${(probability * 25).toFixed(1)}% overspeed). Braking sequence should be verified for immediate engagement.`,
    conditions: [
    ...
    actions: [
      ...
      {
        type: 'OPEN_SOP',
        label: 'Braking Protocol',
        targetId: 'Francis_SOP_Braking_System',
        icon: 'FileText',
      },
    ],
  },
