...
export class KaplanEngine extends BaseEngine {
   ...
   */
  public async calculateEfficiencyAsync(
    head: number,
    flow: number,
    alpha: number
  ): Promise<number> {
    try {
      return await KaplanEngine.runAsyncCalculation<number>('CALCULATE_EFFICIENCY', {
        head,
        flow,
        alpha,
      });
  ...
  public async calculateWaterHammerAsync(waveSpeed: number, deltaV: number): Promise<number> {
    try {
      return await KaplanEngine.runAsyncCalculation<number>('CALCULATE_WATER_HAMMER', {
        waveSpeed,
        deltaV,
      });
    } catch (err) {
  ...
  public getConfidenceScore(head?: number, guideVaneOpening?: number, bladeAngle?: number): number {
    if (
      typeof head !== 'number' ||
      typeof guideVaneOpening !== 'number' ||
      typeof bladeAngle !== 'number'
    ) {
      return 50;
    }
