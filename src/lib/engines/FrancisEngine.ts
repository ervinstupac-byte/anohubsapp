...
export class FrancisEngine extends BaseEngine {
    ...
    generateSpecs(head: number, flow: number): import('./types.ts').TurbineSpecs {
        return {
            ...
            wicketGates: 'Adjustable', // Added back if needed or mapped to TurbineSpecs
    ...
    /**
     * Get confidence score based on specific speed and head range
     * Francis turbines operate best at n_sq 70-350 and head 40-400m
     */
    public getConfidenceScore(head?: number, flow?: number): number {
        if (typeof head !== 'number' || typeof flow !== 'number') return 50;
        
        const n_sq = this.calculateSpecificSpeed(head, flow);
        
        // High confidence when specific speed is in optimal range
        let score = 70;
        if (n_sq >= 70 && n_sq <= 350) score += 20;
        else if (n_sq >= 50 && n_sq <= 400) score += 10;
        
        // Bonus for head in Francis range
        if (head >= 40 && head <= 400) score += 10;
