...
export class WizardService {
  ...
  static validateBearings(data: CommissioningState['bearings']): MilestoneResult {
    if (data.asperityDetected) {
      return {
        ...
        werkmeisterTip:
          'Do not scrape. Inspect for transport damage or debris. Resurfacing required.',
      };
    }
    ...
    if (max - min > 0.05) {
      return {
        ...
        werkmeisterTip:
          'Check centering of the shaft in the seal housing before locking the bearing.',
      };
    }
  ...
  static validateMetallurgy(data: CommissioningState['metallurgy']): MilestoneResult {
    ...
    if (isSoft && !data.ceramicCoatingApplied) {
      return {
        ...
        werkmeisterTip:
          'Without ceramic coating, this runner will pit within 18 months. Schedule application.',
      };
    }
  ...
  static validateElectrical(data: CommissioningState['electrical']): MilestoneResult {
    if (data.insulationResistance < 100) {
      // 100 MOhm
      return {
        success: false,
  ...
  static validateHydraulic(data: CommissioningState['hydraulic']): MilestoneResult {
    ...
    if (asymmetry > 0.05) {
      return {
        ...
        werkmeisterTip:
          'This gap difference will leak revenue forever. Re-adjust the link rods now.',
      };
    }
  ...
  static generateCertificate(state: CommissioningState): string {
    const qualityFlag = state.hydraulic.qualityViolation
      ? '** QUALITY VIOLATION LOGGED **'
      : 'BORN PERFECT';
