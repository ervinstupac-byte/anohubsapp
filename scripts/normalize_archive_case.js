...
function toLowerSegments(p, baseDir) {
  // return path with each segment lowercased relative to baseDir
...
async function main() {
  ...
  // Prepare rename plan: deepest first
  const plan = all
  ...
  for (const orig of plan) {
    ...
    // if already exists and is same inode, skip
    try {
  ...
  // Update DossierLibrary.ts paths to lowercase
