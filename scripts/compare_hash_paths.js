...
// Normalize mapping keys to repo-style paths (archive/...)
const keys = Object.keys(mapping);
function normalizeKey(k){
  // mapping keys may have windows backslashes and leading "public\\"
  let s = k.replace(/\\/g,'/');
  s = s.replace(/^public\//,'');
  s = s.replace(/^\.\//,'');
  return s;
...
const normToOrig = {};
const normKeys = keys.map(k => { const n = normalizeKey(k); normToOrig[n]=k; return n; });
